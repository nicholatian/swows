...
struct BikeHistoryInputInfo
{
	...
	u32 dirHistoryMask; // mask applied so that way only the recent nybble
			    // (the recent input) is checked
	u32 abStartSelectHistoryMask; // mask applied so that way only the
				      // recent nybble (the recent input) is
				      // checked
	const u8* dirTimerHistoryList; // list of timers to check for direction
				       // before the button+dir combination can
				       // be verified.
	const u8* abStartSelectHistoryList; // list of timers to check for
					    // buttons before the button+dir
