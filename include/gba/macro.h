...
// unit size (2 or 4 bytes) and then combined with the DMA control flags using
...
#define DmaCopy16( dmaNum, src, dest, size ) \
	DMA_COPY( dmaNum, src, dest, size, 16 )
#define DmaCopy32( dmaNum, src, dest, size ) \
	DMA_COPY( dmaNum, src, dest, size, 32 )
