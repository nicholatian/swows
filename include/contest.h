...
enum
{
	...
	// Prints the ContestResourcesField1C::unk_C value as a bitstring for
	// each contestant.
	CONTEST_DEBUG_MODE_PRINT_UNK_C,
	// Prints the ContestResourcesField1C::unk_D value as a bitstring for
	// each contestant.
...
struct Shared1A004
{
	u16 cachedWindowPalettes[16][16]; // Saved palette data before a move
					  // happens?
...
struct ContestantStatus
{
	...
	u8 conditionMod : 2; // 1: just raised condition; 2: appeal greatly
			     // improved by condition
	u8 turnOrderMod : 2; // 1: defined; 2: random
	/*0x11*/ u8 turnOrderModAction : 2; // 1: made first; 2: made last; 3:
					    // made random
	u8 turnSkipped : 1;
	u8 exploded : 1;
