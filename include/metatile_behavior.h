...
#define TILE_IS_WILD( x ) ( (x)&TILE_FLAG_ENCOUNTER_TILE )
#define TILE_IS_SURF( x ) ( (x)&TILE_FLAG_SURFABLE )

#define TILE_WATER_WILD_ENC( x ) ( MetatileBehavior_IsWaterWildEncounter( x ) )
#define TILE_WATER_BRIDGE_WILD_ENC( x ) \
	( MetatileBehavior_IsBridge( x ) || TILE_WATER_WILD_ENC( x ) )
#define TILE_LAND_WILD_ENC( x ) ( MetatileBehavior_IsLandWildEncounter( x ) )

extern const u8 mt_attrs[];

bool8 MetatileBehavior_IsNonOceanSurfableWater( u8 );
