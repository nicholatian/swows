...
// must be at the beginning. There are also a number of macros which differ
// from one file to the next due to the method of obtaining the result, such as
// these below. Because of this, there is a theory (Two Team Theory) that
// states that these programming projects had more than 1 "programming team"
// which utilized different macros for each of the files that were worked on.
...
#define DEX_FLAGS_NO \
	( ( POKEMON_SLOTS_NUMBER / 8 ) + \
		( ( POKEMON_SLOTS_NUMBER % 8 ) ? 1 : 0 ) )

struct Pokedex
{
	/*0x00*/ u8 order;
	/*0x01*/ u8 mode;
	/*0x02*/ u8 nationalMagic; // must equal 0xDA in order to have National
				   // mode
	/*0x03*/ u8 unknown2;
...
struct BerryPickingResults // possibly used in the game itself? Size may be
			   // wrong as well
{
	u32 bestScore;
	u16 berriesPicked;
...
struct BattleFrontier
{
	...
	/*0x738*/ struct EmeraldBattleTowerRecord towerRecords[5]; // From
								   // record
								   // mixing.
	/*0xBD4*/ u16 field_BD4;
	/*0xBD6*/ u16 field_BD6;
	...
	/*0xCB0*/ u16 field_CB0;
	/*0xCB2*/ u16 curChallengeBattleNum; // In case of battle pyramid, the
					     // floor.
...
struct SaveBlock2
{
	...
	/*0x14*/ u16
		optionsTextSpeed : 3; // OPTIONS_TEXT_SPEED_[SLOW/MID/FAST]
	u16 optionsWindowFrameType : 5; // Specifies one of the 20 decorative
					// borders for text boxes
	u16 optionsSound : 1; // OPTIONS_SOUND_[MONO/STEREO]
	u16 optionsBattleStyle : 1; // OPTIONS_BATTLE_STYLE_[SHIFT/SET]
	u16 optionsBattleSceneOff : 1; // whether battle animations are
				       // disabled
	...
	/*0x57C*/ struct RankingHall2P hallRecords2P[2][3]; // From record
							    // mixing.
	/*0x624*/ u16 contestLinkResults[5][4]; // 4 positions for 5
						// categories.
...
struct SecretBase
{
	...
	/*0x1AA5*/ u8 trainerId[4]; // byte 0 is used for determining trainer
				    // class
	/*0x1AA9*/ u8 language;
...
struct SaveBlock1
{
	...
	/*0x1C*/ struct WarpData lastHealLocation; // used by white-out and
						   // teleport
	...
	/*0xC70*/ struct EventObjectTemplate
		eventObjectTemplates[EVENT_OBJECT_TEMPLATES_COUNT];
	...
	/*0x2E20*/ u8 additionalPhrases[8]; // bitfield for 33 additional
					    // phrases in easy chat system
	/*0x2E28*/ OldMan oldMan;
	/*0x2e64*/ struct EasyChatPair easyChatPairs[5]; // Dewford trend [0]
							 // and some other
							 // stuff
	/*0x2e90*/ struct ContestWinner contestWinners[13]; // 0 - 5 used in
							    // contest hall, 6
							    // - 7 unused?, 8 -
							    // 12 museum
