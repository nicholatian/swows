...
gfx2obj takes all of its metadata hints from the file extension
provided. It uses this format (regex):
...
class Metadata:
	...
	def __init__(self, fname):
		...
		if len(splits) < 3:
			raise Exception(
			'Incomplete extension in filename ' + fname)
		...
		if 'pl' in outs:
			self.pal_lz = True
		palct_ = outs.replace('i', '').replace('m',
			'').replace('p', '').replace('l', '')
		if len(palct_) > 0:
		...
		if bpp != '1' and bpp != '4' and bpp != '8':
			raise Exception (
			'Invalid bpp specified in filename ' + fname)
		self.bpp = bpp
		form = splits[-3][1:]
		if form.startswith('t'):
			...
			raise Exception(
			'Invalid form specified in filename ' + fname)
...
def main(args):
	...
	if args[1] == '-':
		...
		return 2
	egname = run(['mangledeggs', '-i', args[1].split('.', 1)[0] +
		'.0'])[:-3]
	#name = args[1].split('data/')[-1].split('.', 1)[0].replace('/',
	#	'_')
	...
	if meta.pal:
		...
		if meta.palct > 0:
			...
			gritopts.append('-pn2')
	run(['grit', args[1]] + gritopts + ['-ftb', '-fh!', '-o' +
		tmpout + '.bin'])
	asmfiles = []
	binfiles = []
	if meta.img:
		...
		if meta.img_lz:
			outname += 'lz'
		run(['bin2asm', '-s', outname, tmpout + '.img.bin',
			tmpout + '.img.s'])
	...
	if meta.map:
		...
		if meta.map_lz:
			outname += 'lz'
		import bin2asm
		run(['bin2asm', '-s', outname, tmpout + '.map.bin',
			tmpout + '.map.s'])
	...
	if meta.pal:
		...
		if meta.pal_lz:
			outname += 'lz'
		import bin2asm
		run(['bin2asm', '-s', outname, tmpout + '.pal.bin',
			tmpout + '.pal.s'])
	...
	if argc > 2:
		outfname = args[2]
	run(['arm-none-eabi-as', '-mcpu=arm7tdmi', '-o', outfname] +
		asmfiles)
