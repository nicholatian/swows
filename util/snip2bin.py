...
## scrip2o, it produces an object code file, passing on symbol
## resolution to the linker transparently.
...
Takes <input>, a Pok\u00E9Snip compatible ASCII text file, and
serialises it into its binary equivalent, passing symbol resolution onto
the linker by outputting an object code file [output] (or stdout if
omitted). If <input> is \u2018-\u2019, stdin is read for text input.
'''

TABLE = {
	' ': 0x00, '\\e': 0x1B, '[d]': 0x2A, '&': 0x2D, '+': 0x2E,
	'[Lv]': 0x34, '=': 0x35, ';': 0x36, '[PK]': 0x53, '[MN]': 0x54,
	'[PO]': 0x55, '[KE]': 0x56, '[BL]': 0x56, '[OC]': 0x57,
	'[K]': 0x58, '%': 0x5B, '(': 0x5C, ')': 0x5D, '[U]': 0x79,
	'[D]': 0x7A, '[L]': 0x7B, '[R]': 0x7C, '<': 0x85, '>': 0x86,
	...
	'x': 0xEC, 'y': 0xED, 'z': 0xEE, ':': 0xF0, '\\l': 0xFA,
	'\\p': 0xFB, '\\c': 0xFC, '\\v': 0xFD, '\\n': 0xFE, '\\0': 0xFF
}
...
def convert(text):
	...
	for ch in text:
		if hexcount >= 2:
			...
			if inbrace:
				raise Exception(
					'Cannot nest brackets/bracing')
			inbrace = True
			bracebuf += ch
		elif ch == ']':
			if not inbrace:
				raise Exception(
					'No matching bracket/brace')
			bracebuf += ch
			inbrace   = False
...
def main(args):
	...
	if outfile == '':
		...
		if r < output_sz and not silent:
			print2('WARNING: Some of the output did not ' +
				'make it through')
	else:
		f = open(outfile, 'wb')
		r = f.write(output)
