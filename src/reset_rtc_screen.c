...
static void PrintTime(
	u8 windowId, u8 x, u8 y, u16 days, u8 hours, u8 minutes, u8 seconds )
{
	u8* dest = gStringVar4;
	...
	AddTextPrinterParameterized(
		windowId, 1, gStringVar4, x, y, TEXT_SPEED_FF, NULL );
}

static void ShowChooseTimeWindow(
	u8 windowId, u16 days, u8 hours, u8 minutes, u8 seconds )
{
	DrawStdFrameWithCustomTileAndPalette( windowId, FALSE, 0x214, 0xE );
	PrintTime( windowId, 0, 1, days, hours, minutes, seconds );
	AddTextPrinterParameterized(
		windowId, 1, gText_Confirm2, 126, 1, 0, NULL );
	schedule_bg_copy_tilemap_to_vram( 0 );
}
...
static void Task_ResetRtc_3( u8 taskId ) { gTasks[taskId].data[0] = 1; }
...
static void Task_ResetRtc_1( u8 taskId )
{
	s16* data    = gTasks[taskId].data;
	u8 selection = data[2];
	const struct ResetRtcStruct* selectionInfo =
		&sUnknown_08510428[selection - 1];
...
static void Task_ShowResetRtcPrompt( u8 taskId )
{
	...
	case 0:
		...
		AddTextPrinterParameterized(
			0, 1, gText_PresentTime, 0, 1, TEXT_SPEED_FF, 0 );
		PrintTime( 0,
			0,
			17,
			gLocalTime.days,
			...
			gLocalTime.seconds );
		AddTextPrinterParameterized(
			0, 1, gText_PreviousTime, 0, 33, TEXT_SPEED_FF, 0 );
		PrintTime( 0,
			0,
			49,
	...
	case 3:
		if( gTasks[data[1]].data[0] )
		{
			...
			else
			{
				DestroyTask( data[1] );
				RtcReset( );
				RtcCalcLocalTimeOffset( gLocalTime.days,
					gLocalTime.hours,
					gLocalTime.minutes,
					gLocalTime.seconds );
