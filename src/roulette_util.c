...
u8 sub_8151744( struct UnkStruct3* r0 )
{
	...
	for( i = 0; i < r0->var04.var04; i++ )
	{
		struct PlttData* faded =
			(struct PlttData*)&gPlttBufferFaded[r0->var04.var02 +
				i];
		struct PlttData* unfaded =
			(struct PlttData*)&gPlttBufferUnfaded[r0->var04.var02 +
				i];

		switch( r0->var00_0 )
		{
		case 1:
			if( faded->r + r0->var03 >= 0 &&
				faded->r + r0->var03 < 32 )
				faded->r += r0->var03;
			if( faded->g + r0->var03 >= 0 &&
				faded->g + r0->var03 < 32 )
				faded->g += r0->var03;
			if( faded->b + r0->var03 >= 0 &&
				faded->b + r0->var03 < 32 )
				faded->b += r0->var03;
			break;
		case 2:
...
u8 sub_815194C( struct UnkStruct3* r0 )
{
	...
	case 1:
		for( rg2 = 0; rg2 < r0->var04.var04; rg2++ )
			gPlttBufferFaded[r0->var04.var02 + rg2] =
				r0->var04.var00;
		r0->var00_0++;
		break;
	case 2:
		for( rg2 = 0; rg2 < r0->var04.var04; rg2++ )
			gPlttBufferFaded[r0->var04.var02 + rg2] =
				gPlttBufferUnfaded[r0->var04.var02 + rg2];
		r0->var00_0--;
		break;
	}
	return 1;
		...
		for( i = 0; i < 16; i++ )
		{
			if( ( r0->var02 >> i ) & 1 )
			{
				if( --r0->var04[i].var01 ==
					0xFF ) // if underflow ?
				{
					if( r0->var04[i].var04.var00 &
						0x8000 ) // PlttData->unused_15
							 // ?
...
void InitPulseBlend( struct PulseBlend* pulseBlend )
{
	...
	memset( &pulseBlend->pulseBlendPalettes,
		0,
	...
	const struct PulseBlendSettings* settings )
{
	...
	else
	{
		while( ++i < 16 )
		{
			...
			{
				pulseBlendPalette =
					&pulseBlend->pulseBlendPalettes[i];
				break;
			}
		}
	}
	...
	memcpy( &pulseBlendPalette->pulseBlendSettings,
		settings,
		sizeof( *settings ) );
	return i;
}

static void ClearPulseBlendPalettesSettings(
		...
		pulseBlendPalette->pulseBlendSettings.restorePaletteOnUnload )
	{
		for( i = pulseBlendPalette->pulseBlendSettings.paletteOffset;
			...
			pulseBlendPalette->pulseBlendSettings.paletteOffset +
				pulseBlendPalette->pulseBlendSettings
					.numColors;
			i++ )
			gPlttBufferFaded[i] = gPlttBufferUnfaded[i];
	}

	memset( &pulseBlendPalette->pulseBlendSettings,
		0,
	...
	u8 multiSelection )
{
	...
	if( !multiSelection )
	{
		ClearPulseBlendPalettesSettings(
			&pulseBlend->pulseBlendPalettes
				 [pulseBlendPaletteSelector & 0xF] );
	}
	else
	{
		for( i = 0; i < 16; i++ )
	...
	u8 multiSelection )
{
	...
	if( !multiSelection )
	{
		pulseBlendPalette = &pulseBlend->pulseBlendPalettes
					     [pulseBlendPaletteSelector & 0xF];
		if( !pulseBlendPalette->available && pulseBlendPalette->inUse )
		{
			if( pulseBlendPalette->pulseBlendSettings
					.restorePaletteOnUnload )
			{
				for( i = pulseBlendPalette->pulseBlendSettings
						 .paletteOffset;
					i <
					pulseBlendPalette->pulseBlendSettings
							.paletteOffset +
	...
	else
	{
		for( j = 0; j < 16; j++ )
		{
			...
			else
			{
				...
				pulseBlendPalette->available = 1;
				pulseBlend->usedPulseBlendPalettes &=
					~( 1 << j );
			}
		}
	}
				...
				if( --pulseBlendPalette->delayCounter == 0xFF )
				{
					...
					BlendPalette(
						...
						pulseBlendPalette
							->pulseBlendSettings
							.blendColor );
					switch( pulseBlendPalette
							->pulseBlendSettings
							.fadeType )
					{
					case 0: // Fade all the way to the max
						// blend
						// amount, then wrap around
						// BUG: This comparison will
						// never be true for
						// maxBlendCoeff values that
						// are >= 8. This is because
						// maxBlendCoeff is a signed
						// 4-bit field, but blendCoeff
						// is an unsigned 4-bit field.
						// This code is never reached,
						// anyway, so the bug is not
						// observable in vanilla
						// gameplay.
