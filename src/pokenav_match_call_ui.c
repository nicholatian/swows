...
u32 sub_81C91AC( struct UnknownInnerStruct_81C81D4* a0,
	const struct BgTemplate* a1,
	struct MatchCallListTemplate* a2,
	s32 a3 );
void sub_81C9160(
	struct MatchCallWindowState* a0, struct MatchCallListTemplate* a1 );
...
void sub_81C8EF8( struct MatchCallWindowState* a0,
	struct UnknownInnerStruct_81C81D4* a1 );
void sub_81C8ED0( void );
void sub_81C8E54( struct MatchCallWindowState* a0,
	struct UnknownInnerStruct_81C81D4* a1,
	u32 a2 );
void PrintMatchCallFieldNames( struct UnknownInnerStruct_81C81D4* a0, u32 a1 );
void sub_81C8D4C( struct MatchCallWindowState* a0,
	struct UnknownInnerStruct_81C81D4* a1 );
void sub_81C8CB4( struct MatchCallWindowState* a0,
	struct UnknownInnerStruct_81C81D4* a1 );
...
void sub_81C83AC( u32 a0,
	u32 a1,
	u32 a2,
	u32 a3,
	u32 a4,
	struct UnknownInnerStruct_81C81D4* a5 );
void sub_81C837C( struct MatchCallWindowState* a0,
	struct UnknownInnerStruct_81C81D4* a1 );
...
static const u8 sPokenavColors_0861FBE4[] = {0, 2, 5};
...
static const u8 sMatchCallFieldColors[] = {1, 4, 5};

static const u8 sUnknown_0861FBF7[] = {2, 4, 6, 7, 0};
...
bool32 sub_81C81D4( const struct BgTemplate* arg0,
	struct MatchCallListTemplate* arg1,
	s32 arg2 )
{
	u32 v1;
	struct UnknownSubStruct_81C81D4* structPtr;

	structPtr = AllocSubstruct(
		0x11, sizeof( struct UnknownSubStruct_81C81D4 ) );
...
void sub_81C82E4( struct UnknownSubStruct_81C81D4* a0 )
{
	...
	sub_8199DF0(
		a0->unk0.unk0.bg, PIXEL_FILL( 1 ), a0->unk0.unk0.unk6, 1 );
	sub_8199DF0(
		a0->unk0.unk0.bg, PIXEL_FILL( 4 ), a0->unk0.unk0.unk6 + 1, 1 );
...
u32 LoopedTask_sub_81C83F0( s32 a0 )
{
	...
	structPtr =
		&( (struct UnknownSubStruct_81C81D4*)GetSubstructPtr( 0x11 ) )
			 ->unk0;
	switch( a0 )
	{
	case 0:
		v1 = ( structPtr->unk0.unkA + structPtr->unk0.unkC +
			     structPtr->unk10 ) &
			0xF;
		...
		if( structPtr->unk38 != NULL )
			...
			structPtr->unk38( *(u16*)( &structPtr->unk0.windowId ),
				structPtr->unk14,
				v1 );
		...
		if( ++structPtr->unk0.unkC >= structPtr->unk0.unkE )
		{
			if( structPtr->unk38 != NULL )
				CopyWindowToVram(
					structPtr->unk0.windowId, 3 );
			else
				CopyWindowToVram(
					structPtr->unk0.windowId, 2 );
			return 0;
		}
		else
		{
...
bool32 ShouldShowDownArrow( void )
{
	...
	return subPtr->windowTopIndex + subPtr->visibleEntries <
		subPtr->listLength;
}
...
void MatchCall_MoveWindow( s32 a0, bool32 a1_ )
{
	...
	register struct MatchCallWindowState* subPtr asm( "r5" ) =
		&structPtr->unk888;
	...
	{
		s32 temp = gUnknown_0203CF44 =
			subPtr->windowTopIndex + subPtr->visibleEntries;
		s32 listLength;
		...
		sub_81C83AC( subPtr->unk10,
			...
			&structPtr->unk0 );
		// Needed to prevent GCC from combining the two sub_81C83AC
		// calls.
		asm( "" );
	}
	else
...
u32 LoopedTask_sub_81C85A0( s32 a0 )
{
	...
	case 1:
		...
		{
			structPtr->unk0.unkA =
				( structPtr->unk0.unkA + structPtr->unk2C ) &
				0xF;
...
int MatchCall_MoveCursorDown( void )
{
	...
	if( structPtr->windowTopIndex + structPtr->selectedIndexOffset <
		structPtr->listLength - 1 )
	{
		if( structPtr->selectedIndexOffset <
			structPtr->visibleEntries - 1 )
		{
			structPtr->selectedIndexOffset++;
			return 1;
		}
...
int MatchCall_PageDown( void )
{
	...
	if( ShouldShowDownArrow( ) )
	{
		...
		s32 v3;
		windowBottomIndex =
			structPtr->windowTopIndex + structPtr->visibleEntries;
...
void sub_81C8838( void )
{
	...
	structPtr->unk0.unk38( structPtr->unk0.unk0.windowId,
		subStr->windowTopIndex + subStr->selectedIndexOffset,
		( structPtr->unk0.unk0.unkA + subStr->selectedIndexOffset ) &
			0xF );
...
u32 LoopedTask_sub_81C8870( s32 a0 )
{
	...
	case 1:
		if( structPtr->unk89C !=
			...
			sub_81C8B70(
				&structPtr->unk0.unk0, structPtr->unk89C, 1 );
...
u32 LoopedTask_sub_81C8A28( s32 a0 )
{
	...
	case 1: {
		...
		*v3 = 0;
		...
		else
		{
			...
			v2 = subPtr888->windowTopIndex +
				subPtr888->visibleEntries;
	...
	case 5:
		...
		v4 = ++structPtr->unk89C;
		if( v4 >= subPtr888->listLength ||
			v4 >= subPtr888->visibleEntries )
			return 1;
		return 9;
	case 6:
...
void sub_81C8C64( struct UnknownSubSubStruct_0203CF40* a0, u32 a1 )
{
	...
	v1 = (u16*)GetBgTilemapBuffer(
		GetWindowAttribute( a0->windowId, WINDOW_BG ) );
	...
	struct UnknownInnerStruct_81C81D4* a1 )
{
	...
	memcpy( colors,
		sPokenavColors_0861FBE4,
		ARRAY_COUNT( sPokenavColors_0861FBE4 ) );

	a1->unk34(
		a0->unk10 + a0->unkC * a0->windowTopIndex, a1->unkTextBuffer );
	a1->unk38( a1->unk0.windowId, a0->windowTopIndex, a1->unk0.unkA );
	FillWindowPixelRect( a1->unk0.windowId,
		PIXEL_FILL( 4 ),
		0,
		a1->unk0.unkA * 16,
		a1->unk0.unk4 * 8,
	...
	CopyWindowRectToVram(
		a1->unk0.windowId, 3, 0, a1->unk0.unkA * 2, a1->unk0.unk4, 2 );
}
...
void PrintMatchCallFieldNames(
	struct UnknownInnerStruct_81C81D4* a0, u32 fieldId )
{
	...
	memcpy( fieldNames,
		sMatchCallFieldNames,
		sizeof( sMatchCallFieldNames ) );
	memcpy( colors,
		sMatchCallFieldColors,
		sizeof( sMatchCallFieldColors ) );

	r4  = a0->unk0.unkA;
	tmp = fieldId * 2 + 1;
	r4 += tmp;
	r4 &= 0xF;
	FillWindowPixelRect( a0->unk0.windowId,
		PIXEL_FILL( 1 ),
		0,
		r4 << 4,
		a0->unk0.unk4,
	...
	// AddTextPrinterParameterized3(a0->unk0.windowId, 7, 2, r4 << 4 + 1,
	// colors, TEXT_SPEED_FF, fieldNames[fieldId]); But the original GCC
	// does some clever reuse of the `1` constant that the current GCC
	// doesn't.
	one = 1;
	AddTextPrinterParameterized3( a0->unk0.windowId,
		7,
		2,
		( r4 << 4 ) + one,
		colors,
		...
		fieldNames[fieldId] );
	CopyWindowRectToVram(
		a0->unk0.windowId, 2, 0, r4 << 1, a0->unk0.unk4, 2 );
}
	...
	u32 a2 )
{
	...
	{
		sub_81DB620(
			a1->unk0.windowId, 1, r6 * 2, a1->unk0.unk4 - 1, 2 );
		AddTextPrinterParameterized( a1->unk0.windowId,
			7,
			str,
			2,
			...
			NULL );
		CopyWindowRectToVram(
			a1->unk0.windowId, 2, 0, r6 * 2, a1->unk0.unk4, 2 );
	}
}
...
void sub_81C8ED0( void )
{
	...
	for( i = 0, ptr = sMatchcallArrowSpriteSheets;
		i < ARRAY_COUNT( sMatchcallArrowSpriteSheets );
		ptr++, i++ )
	{
	...
	struct UnknownInnerStruct_81C81D4* a1 )
{
	...
	spriteId       = (u8)CreateSprite( &sMatchCallRightArrowSprite,
                a1->unk0.unk2 * 8 + 3,
                ( a1->unk0.unk3 + 1 ) * 8,
                7 );
	...
	spriteId      = (u8)CreateSprite( &sMatchCallUpDownArrowSprite,
                temp,
                a1->unk0.unk3 * 8 + a0->visibleEntries * 16,
                7 );
	a1->downArrow = &gSprites[spriteId];
	a1->downArrow->oam.tileNum += 2;
	a1->downArrow->callback = SpriteCB_MatchCallDownArrow;

	spriteId = (u8)CreateSprite(
		&sMatchCallUpDownArrowSprite, temp, a1->unk0.unk3 * 8, 7 );
...
void ToggleMatchCallArrows(
	struct UnknownInnerStruct_81C81D4* a0, bool32 shouldHide )
{
	if( shouldHide )
	{
