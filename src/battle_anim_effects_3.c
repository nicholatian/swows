...
const u16 gUnknown_085CE55C[] =
	INCBIN_U16( "graphics/unknown/unknown_85CE55C.gbapal" );
...
{
	sub_807521C( GetBattlerSpriteCoord( gBattleAnimTarget, 2 ) + 8,
...
static void sub_815A3F0( struct Sprite* sprite )
{
	...
	if( sprite->data[4]++ > 15 )
	{
		...
		SetGpuReg( REG_OFFSET_BLDCNT,
			BLDCNT_TGT2_ALL | BLDCNT_EFFECT_BLEND );
		SetGpuReg( REG_OFFSET_BLDALPHA,
			BLDALPHA_BLEND( sprite->data[0], 0 ) );
...
static void sub_815A52C( u8 taskId )
{
	...
	if( ++gTasks[taskId].data[5] == 4 )
	{
		...
		for( i = 10; i > 0; i-- )
			gPlttBufferFaded[paletteIndex * 16 + i + 1] =
				gPlttBufferFaded[paletteIndex * 16 + i];
...
void sub_815AAA4( struct Sprite* sprite )
{
	...
	{
		sprite->pos1.x =
			GetBattlerSpriteCoord( gBattleAnimAttacker, 0 );
		sprite->pos1.y =
			GetBattlerSpriteCoord( gBattleAnimAttacker, 1 );
	}
...
static void sub_815AB5C( struct Sprite* sprite )
{
	if( sprite->data[2] == 0 )
	{
		...
		if( sprite->pos2.x == 0 )
		{
			...
			if( sprite->data[3] == 0 )
			{
				PlaySE1WithPanning( SE_W227,
					BattleAnimAdjustPanning( -64 ) );
			}
		}
	}
	else
...
void sub_815ACD0( struct Sprite* sprite )
{
	...
	{
		sprite->pos1.x =
			GetBattlerSpriteCoord( gBattleAnimTarget, 0 ) +
			gBattleAnimArgs[1];
...
static void sub_815B054( u8 taskId )
{
	...
	case 0:
		...
		spriteId = CreateSprite(
			&gUnknown_08593114, x, y, 6 - task->data[1] );
		...
		PrepareAffineAnimInTaskData(
			task, task->data[15], gUnknown_085CE2A0 );
		task->data[1]++;
		task->data[0] = 1;
		break;
	case 1:
...
void sub_815B394( struct Sprite* sprite )
{
	...
	case 0:
		sprite->pos1.x =
			GetBattlerSpriteCoord( gBattleAnimAttacker, 2 );
		sprite->pos1.y =
			GetBattlerSpriteCoord( gBattleAnimAttacker, 3 );
	...
	case 1:
		...
		sprite->data[2] -= 26;
		SetSpriteRotScale(
			spriteId, sprite->data[1], sprite->data[2], 0 );
...
void sub_815B7D0( u8 taskId )
{
	...
	case 1:
		...
		break;
	case 2:
		HandleSpeciesGfxDataChange( gBattleAnimAttacker,
			gBattleAnimTarget,
		...
		if( IsContest( ) )
		{
			...
			if( IsSpeciesNotUnown(
				    gContestResources->field_18->unk2 ) )
				gSprites[gBattlerSpriteIds
						 [gBattleAnimAttacker]]
					.affineAnims = gUnknown_082FF6C0;
			else
				gSprites[gBattlerSpriteIds
						 [gBattleAnimAttacker]]
...
void sub_815BB58( u8 taskId )
{
	HandleSpeciesGfxDataChange(
		gBattleAnimAttacker, gBattleAnimTarget, TRUE );
	DestroyAnimVisualTask( taskId );
}
...
void sub_815BB84( u8 taskId )
{
	...
	case 0:
		...
		AnimLoadCompressedBgGfx(
			animBg.bgId, gUnknown_08C2A634, animBg.tilesOffset );
		LoadCompressedPalette(
			gUnknown_08C2A6D4, animBg.paletteId * 16, 32 );
	...
	case 2:
		...
		{
			gBattle_BG1_X =
				gUnknown_085CE460[gTasks[taskId].data[2]] +
				gTasks[taskId].data[10];
	...
	case 3:
		if( ++gTasks[taskId].data[3] == 4 )
		{
			...
			PlaySE12WithPanning(
				SE_W234, BattleAnimAdjustPanning( -64 ) );
		}
		break;
	case 4:
		sub_80A6B30( &animBg );
...
void sub_815BE04( struct Sprite* sprite )
{
	...
	sprite->pos1.x =
		GetBattlerSpriteCoord( gBattleAnimAttacker, 0 ) + xOffset;
	sprite->pos1.y  = GetBattlerSpriteCoord( gBattleAnimAttacker, 1 ) + 32;
	sprite->data[1] = gBattleAnimArgs[0];
	sprite->data[2] = gBattleAnimArgs[1];

	spriteId1 = CreateSprite( &gUnknown_085CE48C,
		sprite->pos1.x,
		sprite->pos1.y,
		sprite->subpriority + 1 );
	spriteId2 = CreateSprite( &gUnknown_085CE48C,
		sprite->pos1.x,
		sprite->pos1.y,
		sprite->subpriority + 1 );
...
void sub_815C0A4( u8 taskId )
{
	...
	case 1:
		...
		if( GetBattlerSide( gBattleAnimTarget ) == B_SIDE_OPPONENT )
			gBattle_BG1_X = gTasks[taskId].data[10] +
				gUnknown_085CE4A4[gTasks[taskId].data[2]];
		else
			gBattle_BG1_X = gTasks[taskId].data[10] -
				gUnknown_085CE4A4[gTasks[taskId].data[2]];
	...
	case 2:
		...
		SetGpuReg( REG_OFFSET_BLDALPHA,
			BLDALPHA_BLEND( 3, gTasks[taskId].data[1] ) );
		if( gTasks[taskId].data[1] == 5 )
			gTasks[taskId].data[0]++;
		break;
	case 3:
		if( ++gTasks[taskId].data[3] >
			gUnknown_085CE4A8[gTasks[taskId].data[2]] )
			gTasks[taskId].data[0]++;
		break;
	case 4:
...
static void AnimTask_RockMonBackAndForthStep( u8 taskId )
{
	...
	case 0:
		...
		SetSpriteRotScale(
			task->data[15], 0x100, 0x100, task->data[2] );
...
static void AnimTask_FlailMovementStep( u8 taskId )
{
	...
	gSprites[task->data[15]].pos2.x =
		-( ( ( temp = task->data[2] ) >= 0 ? task->data[2]
						   : temp + 63 ) >>
			6 );
...
void AnimPainSplitProjectile( struct Sprite* sprite )
{
	if( !sprite->data[0] )
	{
		...
		{
			sprite->pos1.x = GetBattlerSpriteCoord(
				gBattleAnimAttacker, BATTLER_COORD_X_2 );
			sprite->pos1.y =
...
void AnimTask_PainSplitMovement( u8 taskId )
{
	...
	if( gTasks[taskId].data[0] == 0 )
	{
		...
		case 1:
			...
			SetBattlerSpriteYOffsetFromYScale( spriteId );
			if( IsContest( ) ||
				GetBattlerSide( gTasks[taskId].data[11] ) ==
					B_SIDE_PLAYER )
...
void AnimReversalOrb( struct Sprite* sprite )
{
	sprite->pos1.x = GetBattlerSpriteCoord(
		gBattleAnimAttacker, BATTLER_COORD_X_2 );
	sprite->pos1.y = GetBattlerSpriteCoord(
...
void AnimTask_RolePlaySilhouette( u8 taskId )
{
	...
	FillPalette( RGB_WHITE,
		( gSprites[spriteId].oam.paletteNum << 4 ) + 0x100,
		32 );
	gSprites[spriteId].oam.priority = priority;
...
static void AnimTask_RolePlaySilhouetteStep2( u8 taskId )
{
	...
	TrySetSpriteRotScale( &gSprites[spriteId],
		TRUE,
		gTasks[taskId].data[10],
...
void AnimTask_DeepInhale( u8 taskId )
{
	...
	task->data[15]    = GetAnimBattlerSpriteId( gBattleAnimArgs[0] );
	PrepareAffineAnimInTaskData(
		&gTasks[taskId], task->data[15], gDeepInhaleAffineAnimCmds );
...
static void InitYawnCloudPosition( struct Sprite* sprite,
	s16 startX,
	s16 startY,
	s16 destX,
	s16 destY,
...
void AnimYawnCloud( struct Sprite* sprite )
{
	...
	InitYawnCloudPosition(
		sprite, sprite->pos1.x, sprite->pos1.y, destX, destY, 64 );
	sprite->data[0]  = 0;
...
void AnimSmokeBallEscapeCloud( struct Sprite* sprite )
{
	...
	sprite->pos1.x = GetBattlerSpriteCoord(
				 gBattleAnimAttacker, BATTLER_COORD_X_2 ) +
		gBattleAnimArgs[1];
	sprite->pos1.y = GetBattlerSpriteCoord( gBattleAnimAttacker,
				 BATTLER_COORD_Y_PIC_OFFSET ) +
		gBattleAnimArgs[2];
...
static void sub_815D8D8( u8 taskId )
{
	...
	gTasks[taskId].data[0]--;
	if( ( gTasks[taskId].data[6] & 0x8000 ) &&
	...
	if( gTasks[taskId].data[2] & 0x8000 )
		gSprites[gTasks[taskId].data[15]].pos2.x =
			gTasks[taskId].data[9] - ( var0 >> 8 );
	else
		gSprites[gTasks[taskId].data[15]].pos2.x =
			gTasks[taskId].data[9] + ( var0 >> 8 );

	if( gTasks[taskId].data[3] & 0x8000 )
		gSprites[gTasks[taskId].data[15]].pos2.y =
...
void AnimTask_SquishAndSweatDroplets( u8 taskId )
{
	...
	task->data[15] = GetAnimBattlerSpriteId( gBattleAnimArgs[0] );
	PrepareAffineAnimInTaskData(
		task, task->data[15], gFacadeSquishAffineAnimCmds );
...
static void AnimTask_SquishAndSweatDropletsStep( u8 taskId )
{
	...
	case 0:
		...
		if( !RunAffineAnimFromTaskData( task ) )
		{
			...
			{
				task->data[1] = 0;
				PrepareAffineAnimInTaskData( task,
					task->data[15],
...
static void CreateSweatDroplets( u8 taskId, bool8 arg1 )
{
	...
	for( i = 0; i < 4; i++ )
	{
		u8 spriteId = CreateSprite( &gFacadeSweatDrop,
			xCoords[i],
			yCoords[i & 1],
			task->data[6] - 5 );
		if( spriteId != MAX_SPRITES )
...
void AnimTask_FacadeColorBlend( u8 taskId )
{
	...
	gTasks[taskId].data[2] =
		0x100 + gSprites[spriteId].oam.paletteNum * 16;
...
static void AnimTask_FacadeColorBlendStep( u8 taskId )
{
	if( gTasks[taskId].data[1] )
	{
		BlendPalette( gTasks[taskId].data[2],
			16,
			8,
...
void AnimTask_GlareEyeDots( u8 taskId )
{
	...
	if( GetBattlerSide( gBattleAnimAttacker ) == B_SIDE_PLAYER )
		task->data[11] = GetBattlerSpriteCoord( gBattleAnimAttacker,
					 BATTLER_COORD_X_2 ) +
	...
	task->data[12] = GetBattlerSpriteCoord( gBattleAnimAttacker,
				 BATTLER_COORD_Y_PIC_OFFSET ) -
		GetBattlerSpriteCoordAttr(
			gBattleAnimAttacker, BATTLER_COORD_ATTR_HEIGHT ) /
			4;
	task->data[13] =
		GetBattlerSpriteCoord( gBattleAnimTarget, BATTLER_COORD_X_2 );
	task->data[14] = GetBattlerSpriteCoord(
		gBattleAnimTarget, BATTLER_COORD_Y_PIC_OFFSET );
...
static void GetGlareEyeDotCoords( s16 arg0,
	s16 arg1,
	s16 arg2,
	s16 arg3,
	u8 arg4,
...
// Moves a ball in an arc twoards the target, and rotates the ball while
// arcing. No args.
void AnimTask_BarrageBall( u8 taskId )
{
	struct Task* task = &gTasks[taskId];

	task->data[11] = GetBattlerSpriteCoord(
		gBattleAnimAttacker, BATTLER_COORD_X_2 );
	task->data[12] = GetBattlerSpriteCoord(
		gBattleAnimAttacker, BATTLER_COORD_Y_PIC_OFFSET );
	task->data[13] =
		GetBattlerSpriteCoord( gBattleAnimTarget, BATTLER_COORD_X_2 );
	task->data[14] = GetBattlerSpriteCoord( gBattleAnimTarget,
				 BATTLER_COORD_Y_PIC_OFFSET ) +
		GetBattlerSpriteCoordAttr(
			gBattleAnimTarget, BATTLER_COORD_ATTR_HEIGHT ) /
			4;
...
static void AnimTask_BarrageBallStep( u8 taskId )
{
	...
	case 2:
		if( ++task->data[1] > 1 )
		{
			...
			if( task->data[2] == 16 )
			{
				FreeOamMatrix( gSprites[task->data[15]]
						       .oam.matrixNum );
...
void AnimSmellingSaltsHand( struct Sprite* sprite )
{
	...
	if( gBattleAnimArgs[1] == 0 )
	{
		...
		sprite->pos1.x = GetBattlerSpriteCoordAttr(
					 battler, BATTLER_COORD_ATTR_LEFT ) -
			8;
	}
	else
	{
		sprite->pos1.x = GetBattlerSpriteCoordAttr(
					 battler, BATTLER_COORD_ATTR_RIGHT ) +
			8;
	}
...
void AnimTask_SmellingSaltsSquish( u8 taskId )
{
	...
	else
	{
		...
		gTasks[taskId].data[15] =
			GetAnimBattlerSpriteId( gBattleAnimArgs[0] );
		PrepareAffineAnimInTaskData( &gTasks[taskId],
			gTasks[taskId].data[15],
...
void AnimSmellingSaltExclamation( struct Sprite* sprite )
{
	if( gBattleAnimArgs[0] == ANIM_ATTACKER )
	{
		sprite->pos1.x =
			GetBattlerSpriteCoord( gBattleAnimAttacker, 2 );
		sprite->pos1.y = GetBattlerSpriteCoordAttr(
			gBattleAnimAttacker, BATTLER_COORD_ATTR_TOP );
	}
	else
	{
		sprite->pos1.x = GetBattlerSpriteCoord( gBattleAnimTarget, 2 );
		sprite->pos1.y = GetBattlerSpriteCoordAttr(
			gBattleAnimTarget, BATTLER_COORD_ATTR_TOP );
	}
...
void AnimTask_HelpingHandAttackerMovement( u8 taskId )
{
	...
	if( !IsContest( ) )
	{
		if( IsDoubleBattle( ) == TRUE )
		{
			int x = GetBattlerSpriteCoord(
				gBattleAnimAttacker, 0 );
			int y = GetBattlerSpriteCoord(
				BATTLE_PARTNER( gBattleAnimAttacker ), 0 );
			if( x > y )
				task->data[14] = 1;
			else
				task->data[14] = -1;
...
static void AnimForesightMagnifyingGlassStep( struct Sprite* sprite )
{
	...
	case 0:
		...
		case 0:
		case 4:
			x = GetBattlerSpriteCoordAttr( sprite->data[7], 5 ) -
				4;
			y = GetBattlerSpriteCoordAttr( sprite->data[7], 3 ) -
				4;
			break;
		case 1:
			x = GetBattlerSpriteCoordAttr( sprite->data[7], 5 ) -
				4;
			y = GetBattlerSpriteCoordAttr( sprite->data[7], 2 ) +
				4;
			break;
		case 2:
			x = GetBattlerSpriteCoordAttr( sprite->data[7], 4 ) +
				4;
			y = GetBattlerSpriteCoordAttr( sprite->data[7], 3 ) -
				4;
			break;
...
{
	sprite->pos2.x =
		( ( sprite->data[2] - sprite->data[0] ) * sprite->data[5] ) /
		sprite->data[4];
	sprite->pos2.y =
		( ( sprite->data[3] - sprite->data[1] ) * sprite->data[5] ) /
		sprite->data[4];
	if( !( sprite->data[5] & 1 ) )
	{
...
{
	s16 y = GetBattlerSpriteCoord( gBattleAnimTarget,
		BATTLER_COORD_X_2 ); // unused local variable
	s16 x = GetBattlerSpriteCoord( gBattleAnimTarget,
		BATTLER_COORD_Y_PIC_OFFSET ); // unused local variable
...
static void AnimTask_MonToSubstituteDoll( u8 taskId )
{
	...
	case 1:
		...
		if( gSprites[spriteId].pos1.y + gSprites[spriteId].pos2.y >=
			-32 )
			gSprites[spriteId].pos2.x = 0;
		...
		{
			PlaySE12WithPanning(
				SE_W145B, BattleAnimAdjustPanning( -64 ) );
...
void AnimBlockX( struct Sprite* sprite )
{
	...
	{
		sprite->subpriority =
			GetBattlerSpriteSubpriority( gBattleAnimTarget ) - 2;
		y = -144;
	}
	else
	{
...
static void AnimBlockXStep( struct Sprite* sprite )
{
	...
	case 0:
		...
		{
			PlaySE12WithPanning(
				SE_W166, BattleAnimAdjustPanning( 63 ) );
			sprite->pos2.y = 0;
			sprite->data[0]++;
		}
		break;
	...
	case 3:
		...
		{
			PlaySE12WithPanning(
				SE_W043, BattleAnimAdjustPanning( 63 ) );
			sprite->data[1] = 0;
			sprite->data[0]++;
		}
		break;
...
// Makes the mon run out of screen, run past the opposing mon, and return to
// its original position. No args.
	...
	case 1:
		...
		else
		{
				...
				B_SIDE_PLAYER )
			{
				...
				subpriority = gSprites[GetAnimBattlerSpriteId(
							       ANIM_TARGET )]
						      .subpriority +
					1;
				isBackPic = 0;
		...
		spriteId2 = sub_80A8394( species,
			...
			GetBattlerSpriteCoord(
				gBattleAnimTarget, BATTLER_COORD_Y ),
			subpriority,
			personality,
			otId,
			gBattleAnimAttacker,
		...
		if( gBattleSpritesDataPtr->battlerData[gBattleAnimAttacker]
				.transformSpecies != SPECIES_NONE )
			BlendPalette(
				( gSprites[spriteId2].oam.paletteNum * 16 ) |
					0x100,
				16,
				6,
				RGB_WHITE );
...
void sub_815FE80( struct Sprite* sprite )
{
	...
	case 0:
		...
		{
			PlaySE12WithPanning(
				SE_W233, BattleAnimAdjustPanning( 63 ) );
			sprite->pos1.y =
				GetBattlerSpriteCoord(
					gBattleAnimTarget, BATTLER_COORD_Y ) +
				16;
			...
			if( GetBattlerSide( gBattleAnimAttacker ) ==
				...
				!IsContest( ) )
				sprite->subpriority =
					gSprites[GetAnimBattlerSpriteId(
							 ANIM_TARGET )]
						.subpriority -
					1;
		}
...
void AnimTask_SnatchPartnerMove( u8 taskId )
{
	...
	case 0:
		...
		targetX = GetBattlerSpriteCoord(
			gBattleAnimTarget, BATTLER_COORD_X );
...
void AnimTask_TeeterDanceMovement( u8 taskId )
{
	...
	task->data[4] = GetBattlerSide( gBattleAnimAttacker ) == B_SIDE_PLAYER
		? 1
		: -1;
...
static void AnimTask_TeeterDanceMovementStep( u8 taskId )
{
	...
	case 0:
		task->data[11] += 8;
		task->data[11] &= 0xFF;
		gSprites[task->data[3]].pos2.x =
			gSineTable[task->data[11]] >> 5;
		task->data[9] += 2;
		task->data[9] &= 0xFF;
		gSprites[task->data[3]].pos1.x =
			( gSineTable[task->data[9]] >> 3 ) * task->data[4] +
			task->data[5];
		if( task->data[9] == 0 )
		{
...
void AnimTask_SlackOffSquish( u8 taskId )
{
	...
	task->data[15]    = GetAnimBattlerSpriteId( gBattleAnimArgs[0] );
	PrepareAffineAnimInTaskData(
		task, task->data[15], gSlackOffSquishAffineAnimCmds );
