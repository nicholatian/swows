...
void DisplayItemMessage(
	u8 taskId, u8 a, const u8* str, void ( *callback )( u8 taskId ) );
...
static const u8 gUnknown_085920E4[] = {
...
void DisplayDadsAdviceCannotUseItemMessage(
	u8 taskId, bool8 isUsingRegisteredKeyItemOnField )
{
	DisplayCannotUseItemMessage(
		taskId, isUsingRegisteredKeyItemOnField, gText_DadsAdvice );
}

void DisplayCannotDismountBikeMessage(
	u8 taskId, bool8 isUsingRegisteredKeyItemOnField )
{
	DisplayCannotUseItemMessage( taskId,
...
u8 CheckIfItemIsTMHMOrEvolutionStone( u16 itemId )
{
	if( ItemId_GetFieldFunc( itemId ) == ItemUseOutOfBattle_TMHM )
		return 1;
	else if( ItemId_GetFieldFunc( itemId ) ==
...
void ItemUseOnFieldCB_Itemfinder( u8 taskId )
{
	if( ItemfinderCheckForHiddenItems( gMapHeader.events, taskId ) ==
		...
		DisplayItemMessageOnField(
			taskId, gText_ItemFinderNothing, sub_80FD5CC );
}
...
void sub_80FD504( u8 taskId )
{
	...
	if( !data[3] )
	{
		if( data[4] == 4 )
		{
			...
			{
				sub_80FDA24(
					gUnknown_085920E4[playerDirToItem -
						1] );
...
bool8 ItemfinderCheckForHiddenItems(
	const struct MapEvents* events, u8 taskId )
{
	...
	for( i = 0; i < events->bgEventCount; i++ )
	{
		if( events->bgEvents[i].kind == BG_EVENT_HIDDEN_ITEM &&
			!FlagGet( events->bgEvents[i]
					  .bgUnion.hiddenItem.hiddenItemId +
				0x1F4 ) )
		{
			...
			if( ( u16 )( newDistanceX + 7 ) < 15 &&
				( newDistanceY >= -5 ) &&
				( newDistanceY < 6 ) )
				sub_80FD8E0(
					taskId, newDistanceX, newDistanceY );
		}
	}
...
bool8 sub_80FD6D4( const struct MapEvents* events, s16 x, s16 y )
{
	...
	{
		if( bgEvent[i].kind == BG_EVENT_HIDDEN_ITEM &&
			x == (u16)bgEvent[i].x &&
			y == (u16)bgEvent[i].y ) // hidden item
						 // and
						 // coordinates
						 // matches x and
						 // y passed?
...
bool8 sub_80FD730( struct MapConnection* connection, int x, int y )
{
	...
	struct MapHeader const* const mapHeader =
		mapconnection_get_mapheader( connection );
	...
	case 2:
		...
		localY      = localLength +
			y; // additions are reversed for some reason
		break;
	case 1:
		localOffset = connection->offset + 7;
		localX      = x - localOffset;
	...
	case 3:
		...
		localX      = localLength +
			x; // additions are reversed for some reason
		localOffset = connection->offset + 7;
...
void sub_80FD7C8( u8 taskId )
{
	...
	for( curX = x - 7; curX <= x + 7; curX++ )
	{
		...
		{
			if( var1 > curX || curX >= width || var2 > curY ||
				curY >= height )
			{
				struct MapConnection* conn =
					sub_8088A8C( curX, curY );
				if( conn &&
					sub_80FD730( conn, curX, curY ) ==
						TRUE )
					sub_80FD8E0(
						taskId, curX - x, curY - y );
			}
		}
	}
}
...
u8 sub_80FD9B0( s16 itemX, s16 itemY )
{
	...
	else
	{
		...
		return DIR_NONE; // should never get here. return something so
				 // it doesnt crash.
	}
}
...
void sub_80FDA24( u8 direction )
{
	...
	EventObjectClearHeldMovement(
		&gEventObjects[GetEventObjectIdByLocalIdAndMap(
			EVENT_OBJ_ID_PLAYER, 0, 0 )] );
	UnfreezeEventObject( &gEventObjects[GetEventObjectIdByLocalIdAndMap(
		EVENT_OBJ_ID_PLAYER, 0, 0 )] );
...
void sub_80FDA94( u8 taskId )
{
	if( EventObjectCheckHeldMovementStatus(
		    &gEventObjects[GetEventObjectIdByLocalIdAndMap(
			    EVENT_OBJ_ID_PLAYER, 0, 0 )] ) == TRUE )
		DisplayItemMessageOnField(
			taskId, gText_ItemFinderNearby, sub_80FD5CC );
}
...
void sub_80FDADC( u8 taskId )
{
		...
		data[2] == FALSE )
	{
		...
		if( data[3] == 4 )
			DisplayItemMessageOnField(
				taskId, gText_ItemFinderOnTop, sub_80FD5CC );
	}
}
...
void sub_80FDBEC( void ) { OpenPokeblockCase( 0, bag_menu_mail_related ); }
...
void sub_80FDE7C( u8 taskId )
{
	...
	ScriptContext1_SetupScript(
		BattleFrontier_OutsideEast_EventScript_242CFC );
	DestroyTask( taskId );
}
...
void ItemUseOutOfBattle_TMHM( u8 taskId )
{
	if( gSpecialVar_ItemId >= ITEM_HM01_CUT )
		DisplayItemMessage(
			taskId, 1, gText_BootedUpHM, sub_80FDF90 ); // HM
	else
		DisplayItemMessage(
			taskId, 1, gText_BootedUpTM, sub_80FDF90 ); // TM
}
...
void task08_0809AD8C( u8 taskId )
{
	if( gMain.newKeys & ( A_BUTTON | B_BUTTON ) )
	{
		...
		StringExpandPlaceholders(
			gStringVar4, gText_TMHMContainedVar1 );
...
void sub_80FE058( void )
{
	...
	if( !InBattlePyramid( ) )
	{
		...
		SetInitialScrollAndCursorPositions(
			ItemId_GetPocket( gSpecialVar_ItemId ) );
	}
	else
	{
		sub_81C5924( );
...
void ItemUseOutOfBattle_Repel( u8 taskId )
{
	if( VarGet( VAR_REPEL_STEP_COUNT ) == 0 )
		...
		DisplayItemMessageInBattlePyramid(
			taskId, gText_RepelEffectsLingered, sub_81C6714 );
}
...
void sub_80FE164( u8 taskId )
{
	if( !IsSEPlaying( ) )
	{
		VarSet( VAR_REPEL_STEP_COUNT,
			ItemId_GetHoldEffectParam( gSpecialVar_ItemId ) );
		sub_80FE058( );
		if( !InBattlePyramid( ) )
			DisplayItemMessage( taskId,
				1,
			...
			DisplayItemMessageInBattlePyramid(
				taskId, gStringVar4, sub_81C6714 );
	}
}
...
void ItemUseOutOfBattle_BlackWhiteFlute( u8 taskId )
{
	...
	if( gSpecialVar_ItemId == ITEM_WHITE_FLUTE )
	{
		...
		StringExpandPlaceholders(
			gStringVar4, gText_UsedVar2WildLured );
	}
	else
	{
		...
		StringExpandPlaceholders(
			gStringVar4, gText_UsedVar2WildRepelled );
	}
...
void ItemUseInBattle_PokeBall( u8 taskId )
{
	...
	{
		DisplayItemMessage(
			taskId, 1, gText_BoxFull, BagMenu_InitListsMenu );
	}
	else
		DisplayItemMessageInBattlePyramid(
			taskId, gText_BoxFull, sub_81C6714 );
}
...
void sub_80FE440( u8 taskId )
{
	if( ++gTasks[taskId].data[8] > 7 )
	{
		...
		if( !InBattlePyramid( ) )
			DisplayItemMessage( taskId,
				1,
				sub_806CF78( gSpecialVar_ItemId ),
				sub_80FE408 );
		else
...
void ItemUseInBattle_StatIncrease( u8 taskId )
{
	...
	if( ExecuteTableBasedItemEffect(
		    &gPlayerParty[partyId], gSpecialVar_ItemId, partyId, 0 ) !=
		FALSE )
	{
		if( !InBattlePyramid( ) )
			DisplayItemMessage( taskId,
			...
			DisplayItemMessageInBattlePyramid(
				taskId, gText_WontHaveEffect, sub_81C6714 );
	}
	else
	{
...
void ItemUseInBattle_Escape( u8 taskId )
{
	if( ( gBattleTypeFlags & BATTLE_TYPE_TRAINER ) == FALSE )
	{
		...
		if( !InBattlePyramid( ) )
			...
			DisplayItemMessageInBattlePyramid(
				taskId, gStringVar4, sub_81C5B14 );
	}
	else
	{
		DisplayDadsAdviceCannotUseItemMessage(
