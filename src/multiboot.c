...
output_burst:
	if( mp->sendflag )
	{
		...
		i = REG_SIOCNT &
			( SIO_MULTI_BUSY | SIO_ERROR | SIO_ID | SIO_MULTI_SD |
				SIO_MULTI_SI );
		if( i != SIO_MULTI_SD )
		{
			MultiBootInit( mp );
	...
	output_master_info:
		return MultiBootSend(
			mp, ( MULTIBOOT_MASTER_INFO << 8 ) | mp->client_bit );
...
void MultiBootStartMaster( struct MultiBootParam* mp,
	const u8* srcp,
	int length,
	u8 palette_color,
	s8 palette_speed )
{
	...
	if( mp->probe_count != 0 || mp->client_bit == 0 ||
		mp->check_wait != 0 )
	{
		MultiBootInit( mp );
		return;
	...
	mp->boot_srcp = srcp;
	length        = ( length + 15 ) & ~15;
	if( length < MULTIBOOT_SEND_SIZE_MIN ||
		length > MULTIBOOT_SEND_SIZE_MAX )
	{
		MultiBootInit( mp );
		return;
	}
...
#define must_data ( mp->system_work[1] )
	...
	default:
		...
		{
			must_data =
				mp->masterp[0xac] | ( mp->masterp[0xad] << 8 );
