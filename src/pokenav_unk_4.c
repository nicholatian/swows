...
bool32 sub_81CB260( void )
{
	struct Pokenav4Struct* state =
		AllocSubstruct( 6, sizeof( struct Pokenav4Struct ) );
	if( !state )
		return FALSE;
...
static u32 sub_81CB324( int taskState )
{
	...
	case 0:
		InitBgTemplates(
			gUnknown_0862278C, ARRAY_COUNT( gUnknown_0862278C ) );
		ChangeBgX( 2, 0, 0 );
		ChangeBgY( 2, 0, 0 );
		decompress_and_copy_tile_data_to_vram(
			2, gUnknown_08622530, 0, 0, 0 );
...
static void sub_81CBC64( u8 taskId )
{
	...
	if( taskData[15] )
	{
		...
		if( !gPaletteFade.active )
			CpuCopy32( gPlttBufferUnfaded + 0x50,
				gPlttBufferFaded + 0x50,
				0x20 );
	}
}
...
static void sub_81CBE0C( u16 windowId )
{
	...
	ConvertIntToDecimalStringN(
		str, sub_81CAE38( ), STR_CONV_MODE_LEFT_ALIGN, 3 );
...
static void sub_81CBE50( u16 windowId )
{
	...
	ConvertIntToDecimalStringN(
		str, numTrainerBattles, STR_CONV_MODE_LEFT_ALIGN, 5 );
...
static void sub_81CBE88( u16 windowId, const u8* str, int top )
{
	...
	AddTextPrinterParameterized(
		windowId, 7, str, 2, y, TEXT_SPEED_FF, NULL );
}
...
static void sub_81CBEB4( u16 windowId, const u8* str, int top )
{
	...
	int y = top * 16 + 1;
	AddTextPrinterParameterized(
		windowId, 7, str, x, y, TEXT_SPEED_FF, NULL );
}
...
static void sub_81CBF60( struct Pokenav4Struct* state )
{
	...
	for( i = 0; i < 3; i++ )
	{
		...
		AddTextPrinterParameterized( state->unk12,
			7,
			gUnknown_086227E8[messageId],
			16,
			i * 16 + 1,
			TEXT_SPEED_FF,
			NULL );
...
{
	AddTextPrinterParameterized(
		state->unk14, 1, gUnknown_086227F4, 32, 1, 1, NULL );
}
...
{
	AddTextPrinterParameterized(
		state->unk14, 1, gText_TrainerCloseBy, 0, 1, 1, NULL );
}
...
static void sub_81CC158( struct Pokenav4Struct* state )
{
	...
	AddTextPrinterParameterized(
		state->unk14, 1, str, 32, 1, speed, NULL );
}
...
static void sub_81CC39C( struct Pokenav4Struct* state )
{
	...
	if( trainerId >= 0 )
	{
		DecompressPicFromTable( &gTrainerFrontPicTable[trainerId],
			state->unk1828,
			SPECIES_NONE );
		LZ77UnCompWram( gTrainerFrontPicPaletteTable[trainerId].data,
			state->unk2028 );
		cursor = RequestDma3Copy(
			state->unk1828, state->unk1824, 0x800, 1 );
