...
void unref_sub_8170478( u8 taskId )
{
	...
	AnimLoadCompressedBgGfx( unknownStruct.bgId,
		gUnknown_08C2EA50,
		unknownStruct.tilesOffset );
	LoadCompressedPalette(
		gCureBubblesPal, unknownStruct.paletteId << 4, 32 );
	...
	case 2:
		if( gTasks[taskId].data[11]++ > 1 )
		{
			...
			if( gTasks[taskId].data[12] == 0 )
			{
				...
				gBattle_WIN0V = 0;
				SetGpuReg( REG_OFFSET_WININ,
					WININ_WIN0_BG_ALL | WININ_WIN0_OBJ |
						WININ_WIN0_CLR |
						WININ_WIN1_BG_ALL |
						WININ_WIN1_OBJ |
						WININ_WIN1_CLR );
				...
				SetGpuReg( REG_OFFSET_BLDALPHA,
					BLDALPHA_BLEND( 0, 0 ) );
				DestroySprite(
					&gSprites[gTasks[taskId].data[0]] );
				DestroySprite(
					&gSprites[gTasks[taskId].data[2]] );
				SetAnimBgAttribute(
					1, BG_ANIM_AREA_OVERFLOW_MODE, 0 );
				spriteId1 =
					gSprites[gHealthboxSpriteIds[battler]]
						.oam.affineParam;
				spriteId2 =
					gSprites[gHealthboxSpriteIds[battler]]
						.data[5];
...
static void sub_8170834( u8* paletteId1, u8* paletteId2, u8 battler )
{
	...
	*paletteId2       = AllocSpritePalette( 0xD70A );
	...
	LoadPalette(
		&gPlttBufferUnfaded[offset1], *paletteId1 * 16 + 0x100, 0x20 );
	LoadPalette(
		&gPlttBufferUnfaded[offset2], *paletteId2 * 16 + 0x100, 0x20 );
...
static void sub_8170A38( u8 taskId )
{
	...
	if( gTasks[taskId].data[0]++ >= gTasks[taskId].data[11] )
	{
		...
		case 0:
			...
			BlendPalette( paletteOffset + colorOffset,
				1,
...
void sub_8170BB0( u8 taskId )
{
	...
	if( GetBattlerSide( gBattleAnimAttacker ) == B_SIDE_PLAYER )
		ball = GetMonData(
			&gPlayerParty
				[gBattlerPartyIndexes[gBattleAnimAttacker]],
			MON_DATA_POKEBALL );
	else
		ball = GetMonData(
			&gEnemyParty
	...
	case 0:
		...
		gTasks[taskId].data[10] = AnimateBallOpenParticles(
			x, y + 32, priority, subpriority, ballId );
		selectedPalettes        = sub_80A75AC( 1, 0, 0, 0, 0, 0, 0 );
		gTasks[taskId].data[11] = LaunchBallFadeMonTask(
			0, gBattleAnimAttacker, selectedPalettes, ballId );
		gTasks[taskId].data[0]++;
		break;
	case 1:
...
void sub_8170E04( u8 taskId )
{
	...
	gSprites[spriteId].data[2] =
		GetBattlerSpriteCoord( gBattleAnimTarget, 1 ) - 16;
...
void sub_8170F2C( u8 taskId )
{
	...
	ballId      = ItemIdToBallId( gLastUsedItem );
	subpriority = GetBattlerSpriteSubpriority( GetBattlerAtPosition(
			      B_POSITION_OPPONENT_LEFT ) ) +
		1;
	spriteId = CreateSprite(
		&gBallSpriteTemplates[ballId], x + 32, y | 80, subpriority );
	gSprites[spriteId].data[0] = 34;
	gSprites[spriteId].data[1] =
		GetBattlerSpriteCoord( gBattleAnimTarget, 0 );
	gSprites[spriteId].data[2] =
	...
	gSprites[gBattlerSpriteIds[GetBattlerAtPosition(
			 B_POSITION_PLAYER_LEFT )]]
		.callback      = sub_8039E84;
	gTasks[taskId].data[0] = spriteId;
...
{
	if( gSprites[gBattlerSpriteIds[GetBattlerAtPosition(
			     B_POSITION_PLAYER_LEFT )]]
			.animCmdIndex == 1 )
	{
	...
	if( TranslateAnimHorizontalArc( sprite ) )
	{
		...
		else
		{
			...
			case 0 ... POKEBALL_COUNT - 1:
				AnimateBallOpenParticles( sprite->pos1.x,
					sprite->pos1.y - 5,
					1,
					28,
					ballId );
				LaunchBallFadeMonTask(
					0, gBattleAnimTarget, 14, ballId );
				break;
			}
		}
	}
...
static void sub_8171240( struct Sprite* sprite )
{
	...
	case 0:
		...
		gUnknown_030062E4       = ( gSprites[spriteId].pos1.y +
...
static void sub_81713D0( struct Sprite* sprite )
{
	...
	case 0:
		...
		if( sprite->data[5] >= 64 )
		{
			...
			// Play a different sound effect for each pokeball
			// bounce.
			switch( bounceCount )
			{
			case 1:
...
static void sub_8171520( struct Sprite* sprite )
{
	...
	case 5:
		...
		{
			if( gBattleSpritesDataPtr->animationData
						->ballThrowCaseId ==
...
static void sub_81717F8( struct Sprite* sprite )
{
	...
	if( sprite->data[4] == 40 )
	{
		...
		BlendPalettes(
			0x10000 << sprite->oam.paletteNum, 6, RGB( 0, 0, 0 ) );
		sub_81719EC( sprite );
	}
	else if( sprite->data[4] == 60 )
	{
		BeginNormalPaletteFade( 0x10000 << sprite->oam.paletteNum,
			2,
			6,
			0,
			RGB( 0, 0, 0 ) );
	...
	{
		FreeOamMatrix(
			gSprites[gBattlerSpriteIds[*battler]].oam.matrixNum );
...
static void sub_81718D8( struct Sprite* sprite )
{
	...
	case 0:
		...
		SetGpuReg( REG_OFFSET_BLDCNT,
			BLDCNT_EFFECT_BLEND | BLDCNT_TGT2_ALL );
		SetGpuReg( REG_OFFSET_BLDALPHA, BLDALPHA_BLEND( 16, 0 ) );
		paletteIndex = IndexOfSpritePaletteTag(
			sprite->template->paletteTag );
		BeginNormalPaletteFade( 1 << ( paletteIndex + 0x10 ),
			0,
			0,
			16,
			RGB( 31, 31, 31 ) );
	...
	case 1:
		if( sprite->data[1]++ > 0 )
		{
			...
			sprite->data[2]++;
			SetGpuReg( REG_OFFSET_BLDALPHA,
				BLDALPHA_BLEND( 16 - sprite->data[2],
					sprite->data[2] ) );
			if( sprite->data[2] == 16 )
				sprite->data[0]++;
		}
...
static void sub_81719EC( struct Sprite* sprite )
{
	...
	for( i = 0; i < 3; i++ )
	{
		u8 spriteId = CreateSprite( &gBallParticlesSpriteTemplates[4],
			sprite->pos1.x,
			sprite->pos1.y,
			subpriority );
		if( spriteId != MAX_SPRITES )
		{
			...
			gSprites[spriteId].data[2] = sprite->pos1.x +
				sBallCaptureSuccessStarData[i].xOffset;
			gSprites[spriteId].data[4] = sprite->pos1.y +
				sBallCaptureSuccessStarData[i].yOffset;
			gSprites[spriteId].data[5] =
				sBallCaptureSuccessStarData[i].unk2;
			InitAnimArcTranslation( &gSprites[spriteId] );
			gSprites[spriteId].callback = sub_8171AAC;
			StartSpriteAnim( &gSprites[spriteId],
				gBallParticleAnimNums[4] );
		}
	}
}
...
static void sub_8171AE4( struct Sprite* sprite )
{
	...
	case 0 ... POKEBALL_COUNT - 1:
		AnimateBallOpenParticles(
			sprite->pos1.x, sprite->pos1.y - 5, 1, 28, ballId );
	...
	StartSpriteAffineAnim(
		&gSprites[gBattlerSpriteIds[gBattleAnimTarget]], 1 );
...
static void sub_8171BAC( struct Sprite* sprite )
{
	...
	{
		StartSpriteAffineAnim(
			&gSprites[gBattlerSpriteIds[gBattleAnimTarget]], 0 );
		next = TRUE;
	}
	else
	{
...
static void sub_8171D60( u8 ballId )
{
	...
	if( GetSpriteTileStartByTag( gBallParticleSpritesheets[ballId].tag ) ==
		0xFFFF )
	{
		LoadCompressedSpriteSheetUsingHeap(
			&gBallParticleSpritesheets[ballId] );
		LoadCompressedSpritePaletteUsingHeap(
			&gBallParticlePalettes[ballId] );
	}
}

u8 AnimateBallOpenParticles(
	u8 x, u8 y, u8 priority, u8 subpriority, u8 ballId )
{
	u8 taskId;
...
static void PokeBallOpenParticleAnimation( u8 taskId )
{
	...
	if( gTasks[taskId].data[0] < 16 )
	{
		...
		spriteId =
			CreateSprite( &gBallParticlesSpriteTemplates[ballId],
				x,
				y,
				subpriority );
		if( spriteId != MAX_SPRITES )
...
static void GreatBallOpenParticleAnimation( u8 taskId )
{
	...
	else
	{
		...
		{
			spriteId = CreateSprite(
				&gBallParticlesSpriteTemplates[ballId],
				x,
				y,
				subpriority );
			if( spriteId != MAX_SPRITES )
...
static void RepeatBallOpenParticleAnimation_Step1( struct Sprite* sprite )
{
	sprite->pos2.x = Sin( sprite->data[0], sprite->data[1] );
	sprite->pos2.y = Cos(
		sprite->data[0], Sin( sprite->data[0], sprite->data[2] ) );
...
{
	sprite->pos2.x  = Sin( sprite->data[0], sprite->data[1] );
	sprite->pos2.y  = Cos( sprite->data[0],
			...
			if( i == POKEBALL_COUNT )
			{
				for( j = 0; j < POKEBALL_COUNT; j++ )
				{
					FreeSpriteTilesByTag(
						gBallParticleSpritesheets[j]
							.tag );
					FreeSpritePaletteByTag(
						gBallParticlePalettes[j].tag );
				}
			}
...
u8 LaunchBallFadeMonTask(
	u8 unfadeLater, u8 battler, u32 selectedPalettes, u8 ballId )
{
	u8 taskId;
	...
	BeginNormalPaletteFade(
		selectedPalettes, 0, 0, 16, RGB( 31, 31, 31 ) );
	return taskId;
}
...
static void sub_8172AB0( u8 taskId )
{
	...
	{
		u32 selectedPalettes = (u16)gTasks[taskId].data[10] |
			( (u16)gTasks[taskId].data[11] << 16 );
		BeginNormalPaletteFade(
			selectedPalettes, 0, 16, 0, RGB( 31, 31, 31 ) );
		DestroyTask( taskId );
	}
}
...
void sub_8172BF0( u8 taskId )
{
	...
	case 0:
		...
		break;
	case 1:
		LoadBattleMonGfxAndAnimate( gBattleAnimAttacker,
...
void sub_8172D98( u8 taskId )
{
	...
	case 0:
		if( GetBattlerSpriteBGPriorityRank( gBattleAnimAttacker ) ==
			...
			SetGpuReg( REG_OFFSET_BLDCNT,
				BLDCNT_TGT1_BG2 | BLDCNT_EFFECT_BLEND |
					BLDCNT_TGT2_ALL );
	...
	case 1:
		if( gTasks[taskId].data[1]++ > 1 )
		{
			...
			gTasks[taskId].data[0]++;
			SetGpuReg( REG_OFFSET_BLDALPHA,
				BLDALPHA_BLEND( 16 - gTasks[taskId].data[0],
					gTasks[taskId].data[0] ) );
			if( gTasks[taskId].data[0] == 16 )
				gTasks[taskId].data[15]++;
		}
...
void sub_8172EF0( u8 battler, struct Pokemon* mon )
{
	...
	{
		shinyValue = HIHALF( otId ) ^ LOHALF( otId ) ^
			HIHALF( personality ) ^ LOHALF( personality );
		if( shinyValue < 8 )
			isShiny = TRUE;

		if( isShiny )
		{
			if( GetSpriteTileStartByTag( ANIM_TAG_GOLD_STARS ) ==
				0xFFFF )
			{
				LoadCompressedSpriteSheetUsingHeap(
					&gBattleAnimPicTable
...
static void sub_8172FEC( u8 taskId )
{
	...
	{
		spriteId = CreateSprite(
			&gMiniTwinklingStarSpriteTemplate, x, y, 5 );
...
void AnimTask_LoadPokeblockGfx( u8 taskId )
{
	...
	LoadCompressedSpriteSheetUsingHeap(
		&gBattleAnimPicTable[ANIM_TAG_POKEBLOCK -
			ANIM_SPRITES_START] );
	LoadCompressedSpritePaletteUsingHeap(
		&gBattleAnimPaletteTable[ANIM_TAG_POKEBLOCK -
			ANIM_SPRITES_START] );
...
static void sub_817330C( struct Sprite* sprite )
{
	...
	sprite->data[2] =
		GetBattlerSpriteCoord(
			GetBattlerAtPosition( B_POSITION_OPPONENT_LEFT ), 0 ) +
		gBattleAnimArgs[2];
	sprite->data[4] =
		GetBattlerSpriteCoord(
			GetBattlerAtPosition( B_POSITION_OPPONENT_LEFT ), 1 ) +
		gBattleAnimArgs[3];
	sprite->data[5] = -32;
	InitAnimArcTranslation( sprite );
...
{
	if( gSprites[gBattlerSpriteIds[gBattleAnimAttacker]].animCmdIndex ==
		1 )
...
void AnimTask_GetTrappedMoveAnimId( u8 taskId )
{
	...
	else if( gBattleSpritesDataPtr->animationData->animArg ==
		MOVE_WHIRLPOOL )
