...
bool32 sub_81521C0( u8 count )
{
	...
	if( sUnknown_0203ABB8 == NULL )
		return FALSE;
	sUnknown_0203ABB8->unk4 =
		AllocZeroed( count * sizeof( struct UnkStruct_81520A8 ) );
	...
	for( i = 0; i < count; i++ )
	{
		memcpy( &sUnknown_0203ABB8->unk4[i].oam,
			&gDummyOamData,
...
bool32 sub_8152254( void )
{
	...
	for( i = 0; i < sUnknown_0203ABB8->count; i++ )
		memcpy( &gMain.oamBuffer[i + 64],
			&gDummyOamData,
			sizeof( struct OamData ) );

	memset( sUnknown_0203ABB8->unk4,
		0,
		sUnknown_0203ABB8->count *
			sizeof( struct UnkStruct_81520A8 ) );
...
bool32 sub_81522D4( void )
{
	...
	for( i = 0; i < sUnknown_0203ABB8->count; i++ )
	{
		if( sUnknown_0203ABB8->unk4[i].unk19_0 &&
			sUnknown_0203ABB8->unk4[i].unk19_1 )
		{
			if( sUnknown_0203ABB8->unk4[i].callback != NULL )
				sUnknown_0203ABB8->unk4[i].callback(
					&sUnknown_0203ABB8->unk4[i] );
			...
			{
				sUnknown_0203ABB8->unk4[i].oam.y =
					sUnknown_0203ABB8->unk4[i].y +
					sUnknown_0203ABB8->unk4[i].yDelta;
				sUnknown_0203ABB8->unk4[i].oam.x =
					sUnknown_0203ABB8->unk4[i].x +
					sUnknown_0203ABB8->unk4[i].xDelta;
				sUnknown_0203ABB8->unk4[i].oam.priority =
					sUnknown_0203ABB8->unk4[i].priority;
				sUnknown_0203ABB8->unk4[i].oam.tileNum =
					sUnknown_0203ABB8->unk4[i].tileNum;
				memcpy( &gMain.oamBuffer[i + 64],
					&sUnknown_0203ABB8->unk4[i],
					sizeof( struct OamData ) );
			}
...
static bool32 sub_81523F4( struct UnkStruct_81520A8* structPtr, u8 arg1 )
{
	...
	structPtr->tileNum =
		( sub_80355F8( structPtr->oam.shape, structPtr->oam.size ) *
			arg1 ) +
		tileStart;
	return TRUE;
}
...
u8 sub_8152474( u8 id, u8 dataArrayId, s16 dataValue )
{
	if( sUnknown_0203ABB8 == NULL || id >= sUnknown_0203ABB8->count )
		...
		dataArrayId >=
			ARRAY_COUNT( sUnknown_0203ABB8->unk4[id].data ) )
		return 0xFF;
...
u8 sub_81524C4( const struct OamData* oam,
	u16 tileTag,
	u16 palTag,
	s16 x,
	s16 y,
