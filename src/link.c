...
ALIGNED( 4 )
const u16 gWirelessLinkDisplayPal[] =
	INCBIN_U16( "graphics/interface/wireless_link_display.gbapal" );
const u32 gWirelessLinkDisplayGfx[] =
	INCBIN_U32( "graphics/interface/wireless_link_display.4bpp.lz" );
const u32 gWirelessLinkDisplayTilemap[] =
	INCBIN_U32( "graphics/interface/wireless_link_display.bin.lz" );
const u16 gLinkTestDigitsPal[] =
	INCBIN_U16( "graphics/interface/link_test_digits.gbapal" );
const u16 gLinkTestDigitsGfx[] =
	INCBIN_U16( "graphics/interface/link_test_digits.4bpp" );
const u8 unkstring_82ed160[] = _( "{HIGHLIGHT TRANSPARENT}{COLOR WHITE}" );
const u16 g2BlankTilesGfx[] =
	INCBIN_U16( "graphics/interface/blank_1x2.4bpp" );
const struct BlockRequest gUnknown_082ED1A8[] = {{gBlockSendBuffer, 200},
	{gBlockSendBuffer, 200},
	{gBlockSendBuffer, 100},
	{gBlockSendBuffer, 220},
	{gBlockSendBuffer, 40}};
const u8 gBGControlRegs[]                     = {REG_OFFSET_BG0CNT,
        REG_OFFSET_BG1CNT,
        REG_OFFSET_BG2CNT,
        REG_OFFSET_BG3CNT};
...
const u8 gUnknown_082ED224[] = {
	0x00, 0x01, 0x02, 0x00, 0xff, 0xfe, 0xff, 0x00};
...
static void InitLinkTestBG(
	u8 paletteNum, u8 bgNum, u8 screenBaseBlock, u8 charBaseBlock, u16 a4 )
{
	LoadPalette( gLinkTestDigitsPal, paletteNum * 16, 0x20 );
	DmaCopy16( 3,
		gLinkTestDigitsGfx,
		(u16*)BG_CHAR_ADDR( charBaseBlock ) + ( 16 * a4 ),
		sizeof gLinkTestDigitsGfx );
	...
	case 1:
		SetGpuReg( REG_OFFSET_BG1CNT,
			BGCNT_SCREENBASE( screenBaseBlock ) |
				BGCNT_PRIORITY( 1 ) |
...
void sub_80094EC(
	u8 paletteNum, u8 bgNum, u8 screenBaseBlock, u8 charBaseBlock )
{
	LoadPalette( gLinkTestDigitsPal, paletteNum * 16, 0x20 );
	DmaCopy16( 3,
		gLinkTestDigitsGfx,
		(u16*)BG_CHAR_ADDR( charBaseBlock ),
		sizeof gLinkTestDigitsGfx );
	gLinkTestBGInfo.screenBaseBlock = screenBaseBlock;
	gLinkTestBGInfo.paletteNum      = paletteNum;
	gLinkTestBGInfo.dummy_8         = 0;
	SetGpuReg( gBGControlRegs[bgNum],
		BGCNT_SCREENBASE( screenBaseBlock ) |
			BGCNT_CHARBASE( charBaseBlock ) );
}
...
static void LinkTestProcessKeyInput( void )
{
	...
	{
		SetLinkDebugValues( gMain.vblankCounter2,
			gLinkCallback ? gLinkVSyncDisabled
...
static void ProcessRecvCmds( u8 unused )
{
	...
	for( i = 0; i < MAX_LINK_PLAYERS; i++ )
	{
		...
		case LINKCMD_SEND_LINK_TYPE: {
			...
			memcpy( block->magic1,
				gASCIIGameFreakInc,
				sizeof( block->magic1 ) - 1 );
			memcpy( block->magic2,
				gASCIIGameFreakInc,
				sizeof( block->magic2 ) - 1 );
		...
		case LINKCMD_CONT_BLOCK: {
			...
			if( sBlockRecv[i].pos >= sBlockRecv[i].size )
			{
				if( gRemoteLinkPlayersNotReceived[i] == TRUE )
				{
					...
					block = (struct
						LinkPlayerBlock*)&gBlockRecvBuffer
						[i];
					linkPlayer  = &gLinkPlayers[i];
					*linkPlayer = block->linkPlayer;
					if( ( linkPlayer->version & 0xFF ) ==
							VERSION_RUBY ||
						( linkPlayer->version &
							0xFF ) ==
							VERSION_SAPPHIRE )
		...
		case LINKCMD_0xCCCC:
			SendBlock( 0,
				gUnknown_082ED1A8[gRecvCmds[i][1]].address,
				gUnknown_082ED1A8[gRecvCmds[i][1]].size );
			break;
...
u32 sub_800A03C( void ) { return 2; }

bool32 sub_800A040( void )
{
	if( GetLinkPlayerCount( ) != 4 ||
		sub_8009FF8( VERSION_RUBY, VERSION_SAPPHIRE ) < 0 )
	{
		return FALSE;
	}
	return TRUE;
...
u8 GetLinkPlayerDataExchangeStatusTimed( int lower, int upper )
{
	...
	if( gReceivedRemoteLinkPlayers == TRUE )
	{
		...
		else
		{
			...
			for( i = 0, index = 0; i < GetLinkPlayerCount( );
				index++, i++ )
			{
				if( gLinkPlayers[index].linkType ==
					gLinkPlayers[0].linkType )
				{
					count++;
				}
			}
			...
			else
			{
				...
				linkType1 = gLinkPlayers[GetMultiplayerId( )]
						    .linkType;
				linkType2 =
...
static void LinkCB_BlockSend( void )
{
	...
	for( i = 0; i < 7; i++ )
	{
		gSendCmd[i + 1] = ( src[sBlockSend.pos + i * 2 + 1] << 8 ) |
			src[sBlockSend.pos + i * 2];
	}
	sBlockSend.pos += 14;
...
u32 sub_800A44C( void ) { return gUnknown_020223C0; }

void sub_800A458( void ) { BuildSendCmd( LINKCMD_0xAAAA ); }
...
static void LinkTest_prnthexchar( char a0, u8 a1, u8 a2 )
{
	...
	vAddr[a2 * 32 + a1] = ( gLinkTestBGInfo.paletteNum << 12 ) |
		( a0 + 1 + gLinkTestBGInfo.dummy_8 );
}
...
// The number of players when trading began. This is frequently compared
// against the current number of connected players to check if anyone dropped
// out.
u8 GetSavedPlayerCount( void ) { return gSavedLinkPlayerCount; }

u8 sub_800AA54( void ) { return gSavedMultiplayerId; }
...
bool8 sub_800AA60( void )
{
	...
	for( i = 0; i < gSavedLinkPlayerCount; i++ )
	{
		if( gLinkPlayers[i].trainerId ==
			gSavedLinkPlayers[i].trainerId )
		{
			if( gLinkType == 0x2288 )
			{
...
void sub_800AAF4( void )
{
	...
	for( i = 0; i < 4; i++ )
	{
		CpuSet( &gSavedLinkPlayers[i],
			NULL,
			sizeof( struct LinkPlayer ) );
	}
}
...
void sub_800AB18( void )
{
	...
	for( i = 0; i < gSavedLinkPlayerCount; i++ )
	{
		if( gSavedLinkPlayers[i].trainerId !=
				gLinkPlayers[i].trainerId ||
			StringCompare( gSavedLinkPlayers[i].name,
				gLinkPlayers[i].name ) != 0 )
		{
			gLinkErrorOccurred = TRUE;
			CloseLink( );
...
u8 sub_800ABE8( void ) { return gUnknown_03000D58; }
...
static void CheckErrorStatus( void )
{
	if( gLinkOpen && EXTRACT_LINK_ERRORS( gLinkStatus ) )
	{
		if( !gSuppressLinkErrorMessage )
		{
			...
			sLinkErrorBuffer.lastRecvQueueCount =
				gLastRecvQueueCount;
			sLinkErrorBuffer.lastSendQueueCount =
				gLastSendQueueCount;
			SetMainCallback2( CB2_LinkError );
		}
		gLinkErrorOccurred = TRUE;
...
void sub_800AF18(
	u32 status, u8 lastSendQueueCount, u8 lastRecvQueueCount, u8 unk_06 )
{
...
static void sub_800B080( void )
{
	...
	DecompressAndLoadBgGfxUsingHeap(
		1, gWirelessLinkDisplayGfx, FALSE, 0, 0 );
	...
	AddTextPrinterParameterized3(
		0, 3, 2, 6, gUnknown_082ED224, 0, gText_CommErrorEllipsis );
...
bool8 GetSioMultiSI( void ) { return ( REG_SIOCNT & 0x04 ) != 0; }
...
bool8 HandleLinkConnection( void )
{
	...
	if( gWirelessCommType == 0 )
	{
		gLinkStatus = LinkMain1(
			&gShouldAdvanceLinkState, gSendCmd, gRecvCmds );
...
u8 sub_800B518( void ) { return gWirelessCommType; }
...
u32 LinkMain1( u8* shouldAdvanceLinkState,
	u16* sendCmd,
	...
	*shouldAdvanceLinkState = 0;
	...
	}
	{
		...
		u32 link_field_F = gLink.link_field_F
			<< LINK_STAT_UNK_FLAG_9_SHIFT;
		u32 hardwareError = gLink.hardwareError
			<< LINK_STAT_ERROR_HARDWARE_SHIFT;
		u32 badChecksum = gLink.badChecksum
			<< LINK_STAT_ERROR_CHECKSUM_SHIFT;
		u32 queueFull = gLink.queueFull
			<< LINK_STAT_ERROR_QUEUE_FULL_SHIFT;
		u32 val;
...
static bool8 DoHandshake( void )
{
	...
	*(u64*)gLink.tempRecvBuffer = REG_SIOMLT_RECV;
	...
	for( i = 0; i < 4; i++ )
	{
		...
		{
			playerCount++;
			if( minRecv > gLink.tempRecvBuffer[i] &&
				gLink.tempRecvBuffer[i] != 0 )
			{
	...
	else
	{
		...
		if( gLink.recvQueue.count < QUEUE_CAPACITY )
		{
			for( i = 0; i < gLink.playerCount; i++ )
			{
				...
				gLink.recvQueue
					.data[i][gLink.recvCmdIndex][index] =
					recv[i];
			}
		}
		else
...
static void DoSend( void )
{
	...
	else
	{
		...
		{
			REG_SIOMLT_SEND =
				gLink.sendQueue.data[gLink.sendCmdIndex]
						    [gLink.sendQueue.pos];
		}
		gLink.sendCmdIndex++;
	}
