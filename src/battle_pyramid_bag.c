...
static void PyramidBagMoveCursorFunc(
...
void sub_81C4F84( void ) { sub_81C4F98( 4, gPyramidBagCursorData.callback ); }
...
void sub_81C4F98( u8 a0, void ( *callback )( void ) )
{
	...
	memset( gPyramidBagResources->itemsSpriteIds,
		0xFF,
		sizeof( gPyramidBagResources->itemsSpriteIds ) );
	memset( gPyramidBagResources->windowIds,
		0xFF,
		sizeof( gPyramidBagResources->windowIds ) );
...
static void sub_81C504C( void )
{
	while( sub_81221EC( ) != TRUE && sub_81C5078( ) != TRUE &&
		sub_81221AC( ) != TRUE )
		;
}
...
static void sub_81C51DC( void )
{
	...
	InitBgsFromTemplates(
		0, gUnknown_0861F2B4, ARRAY_COUNT( gUnknown_0861F2B4 ) );
...
static bool8 sub_81C5238( void )
{
	...
	case 1:
		...
		{
			LZDecompressWram( gBattleFrontierGfx_PyramidBagTileMap,
				gPyramidBagResources->tilemapBuffer );
			gPyramidBagResources->state++;
		}
		break;
	case 2:
...
{
	u16 i;
	u16* pyramidItems = gSaveBlock2Ptr->frontier.pyramidBag
				    .itemId[gSaveBlock2Ptr->frontier.lvlMode];

	for( i = 0; i < gPyramidBagResources->listMenuCount - 1; i++ )
	{
		PyramidBag_CopyItemName( gPyramidBagResources->itemStrings[i],
			pyramidItems[i] );
		gPyramidBagResources->bagListItems[i].name =
			gPyramidBagResources->itemStrings[i];
	...
	gMultiuseListMenuTemplate.maxShowed =
		gPyramidBagResources->listMenuMaxShown;
}
		...
		if( itemIndex != LIST_CANCEL )
			ShowItemImage(
				gSaveBlock2Ptr->frontier.pyramidBag
					.itemId[gSaveBlock2Ptr->frontier
							.lvlMode][itemIndex],
...
static void PrintItemQuantity( u8 windowId, s32 itemIndex, u8 y )
{
	...
	PrintOnWindow_Font7(
		windowId, gStringVar4, xAlign, y, 0, 0, TEXT_SPEED_FF, 0 );
}
...
static void PrintItemDescription( s32 listMenuId )
{
	...
	else
	{
		StringCopy( gStringVar1,
			gReturnToXStringsTable2[gPyramidBagCursorData.unk4] );
...
static void AddScrollArrow( void )
{
	if( gPyramidBagResources->scrollIndicatorsTaskId == 0xFF )
		gPyramidBagResources->scrollIndicatorsTaskId =
			AddScrollIndicatorArrowPairParameterized( 2,
				172,
				12,
				148,
				gPyramidBagResources->listMenuCount -
...
void sub_81C59BC( void )
{
	if( gPyramidBagCursorData.scrollPosition != 0 &&
		...
		gPyramidBagCursorData.scrollPosition =
			gPyramidBagResources->listMenuCount -
			gPyramidBagResources->listMenuMaxShown;
	if( gPyramidBagCursorData.scrollPosition +
			gPyramidBagCursorData.cursorPosition >=
		gPyramidBagResources->listMenuCount )
	{
...
static void sub_81C5A20( void )
{
	...
	{
		for( i = 0; i <= gPyramidBagCursorData.cursorPosition - 4;
			i++ )
		{
			if( gPyramidBagCursorData.scrollPosition +
...
static void sub_81C5AB8( u8 y, u8 arg1 )
{
	if( arg1 == 0xFF )
		...
		PrintOnWindow_Font1(
			0, gText_SelectorArrow2, 0, y, 0, 0, 0, arg1 );
}
	...
	if( sub_81221EC( ) != TRUE && !gPaletteFade.active )
	{
		...
		else
		{
			...
			default:
				...
				gSpecialVar_ItemId =
					gSaveBlock2Ptr->frontier.pyramidBag
						.itemId[gSaveBlock2Ptr
								->frontier
								.lvlMode]
						       [listId];
				data[1] = listId;
				data[2] = gSaveBlock2Ptr->frontier.pyramidBag
						  .quantity[gSaveBlock2Ptr
								    ->frontier
								    .lvlMode]
							   [listId];
				if( gPyramidBagCursorData.unk4 == 2 )
...
static void sub_81C5D20( u8 taskId )
{
	...
	default:
		...
		gPyramidBagResources->menuActionsCount =
			ARRAY_COUNT( sFieldMenuActionIds );
		break;
	case 1:
		if( ItemId_GetBattleUsage( gSpecialVar_ItemId ) )
		{
			...
			gPyramidBagResources->menuActionsCount =
				ARRAY_COUNT( sBattleMenuActionIds );
		}
		else
		{
			gPyramidBagResources->menuActionIds =
				gUnknown_0861F310;
			gPyramidBagResources->menuActionsCount =
				ARRAY_COUNT( gUnknown_0861F310 );
		}
		break;
	case 3:
		gPyramidBagResources->menuActionIds = gUnknown_0861F30C;
		gPyramidBagResources->menuActionsCount =
			ARRAY_COUNT( gUnknown_0861F30C );
		break;
	}
...
static void HandleFewMenuActionsInput( u8 taskId )
{
	if( sub_81221EC( ) != TRUE )
	{
		...
		default:
			PlaySE( SE_SELECT );
			if( sMenuActions[gPyramidBagResources
						 ->menuActionIds[id]]
					.func.void_u8 != NULL )
				sMenuActions[gPyramidBagResources
						     ->menuActionIds[id]]
					.func.void_u8( taskId );
			break;
		}
...
static void HandleMenuActionInput( u8 taskId )
{
	if( sub_81221EC( ) != TRUE )
	{
		...
		{
			if( id < gPyramidBagResources->menuActionsCount - 2 &&
...
static bool8 IsValidMenuAction( s8 actionTableId )
{
	if( actionTableId < 0 )
		...
		return FALSE;
	else if( gPyramidBagResources->menuActionIds[actionTableId] ==
		ACTION_DUMMY )
		return FALSE;
	else
		return TRUE;
...
static void BagAction_UseOnField( u8 taskId )
{
		...
		ItemIsMail( gSpecialVar_ItemId ) == TRUE )
	{
		...
		DisplayItemMessageInBattlePyramid(
			taskId, gText_DadsAdvice, sub_81C6714 );
	}
...
static void sub_81C6350( u8 taskId )
{
	...
	ConvertIntToDecimalStringN(
		gStringVar2, data[8], STR_CONV_MODE_LEFT_ALIGN, 2 );
...
static void sub_81C6404( void )
{
	...
	ConvertIntToDecimalStringN(
		gStringVar1, 1, STR_CONV_MODE_LEADING_ZEROS, 2 );
	StringExpandPlaceholders( gStringVar4, gText_xVar1 );
	sub_81C6CEC( 3 );
...
static void sub_81C645C( s16 value )
{
	...
	ConvertIntToDecimalStringN(
		gStringVar1, value, STR_CONV_MODE_LEADING_ZEROS, 2 );
	StringExpandPlaceholders( gStringVar4, gText_xVar1 );
	x = GetStringCenterAlignXOffset( 1, gStringVar4, 0x28 );
	AddTextPrinterParameterized( 3, 1, gStringVar4, x, 2, 0, NULL );
}
...
static void BagAction_Give( u8 taskId )
{
	...
	{
		DisplayItemMessageInBattlePyramid(
			taskId, gText_CantWriteMail, sub_81C66EC );
	}
...
static void Task_BeginItemSwap( u8 taskId )
{
	...
	data[1] = gPyramidBagCursorData.scrollPosition +
		gPyramidBagCursorData.cursorPosition;
	gPyramidBagResources->unk814 = data[1];
	ListMenuSetUnkIndicatorsStructField( data[0], 0x10, 1 );
	CopyItemName(
		gSaveBlock2Ptr->frontier.pyramidBag
			.itemId[gSaveBlock2Ptr->frontier.lvlMode][data[1]],
		gStringVar1 );
...
static void sub_81C6A14( u8 taskId )
{
	...
	SetBagItemsListTemplate( );
	data[0] = ListMenuInit(
		&gMultiuseListMenuTemplate, *scrollOffset, *selectedRow );
	SetTaskToMainPyramidBagInputHandler( taskId );
}
...
void sub_81C6A94( void )
{
	...
	for( i = 0; i < 3; i++ )
	{
		...
		{
			memcpy( gSaveBlock2Ptr->frontier.pyramidBag.itemId
					[gSaveBlock2Ptr->frontier.lvlMode],
				newItems,
				PYRAMID_BAG_ITEMS_COUNT * sizeof( u16 ) );
			memcpy( gSaveBlock2Ptr->frontier.pyramidBag.quantity
					[gSaveBlock2Ptr->frontier.lvlMode],
				newQuantities,
				PYRAMID_BAG_ITEMS_COUNT * sizeof( u8 ) );
			Free( newItems );
...
static void sub_81C6DAC( u8 taskId, const struct YesNoFuncTable* yesNoTable )
{
	CreateYesNoMenuWithCallbacks(
		taskId, &gUnknown_0861F350[4], 1, 0, 2, 1, 0xE, yesNoTable );
}

void DisplayItemMessageInBattlePyramid(
	u8 taskId, const u8* str, void ( *callback )( u8 taskId ) )
{
...
{
	u8* spriteId =
		&gPyramidBagResources->itemsSpriteIds[itemSpriteArrayId];
	if( *spriteId != 0xFF )
	{
...
static void sub_81C6E98( void )
{
	...
	spritePalette.data =
		palPtr + ( gSaveBlock2Ptr->frontier.lvlMode * 16 );
...
static void sub_81C6F20( void )
{
	struct Sprite* sprite =
		&gSprites[gPyramidBagResources->itemsSpriteIds[0]];
...
static void ShowItemImage( u16 itemId, u8 itemSpriteArrayId )
{
	u8 itemSpriteId;
	u8* spriteId =
		&gPyramidBagResources->itemsSpriteIds[itemSpriteArrayId + 1];
	if( *spriteId == 0xFF )
	{
		FreeSpriteTilesByTag( ITEM_IMAGE_TAG + 1 + itemSpriteArrayId );
		FreeSpritePaletteByTag(
			ITEM_IMAGE_TAG + 1 + itemSpriteArrayId );
...
static void sub_81C704C( u8 y )
{
	sub_8122448( &gPyramidBagResources->itemsSpriteIds[3],
		8 | 0x80,
		120,
		( y + 1 ) * 16 );
}
