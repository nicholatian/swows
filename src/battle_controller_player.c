...
// unknown unused data
static const u8 sUnknown_0831C5FC[] = {
...
static void PlayerBufferRunCommand( void )
{
	if( gBattleControllerExecFlags & gBitTable[gActiveBattler] )
	{
		if( gBattleBufferA[gActiveBattler][0] <
			ARRAY_COUNT( sPlayerBufferCommands ) )
			sPlayerBufferCommands[gBattleBufferA[gActiveBattler]
							    [0]]( );
		else
	...
	if( gMain.newKeys & A_BUTTON )
	{
		...
		case 0:
			BtlController_EmitTwoReturnValues(
				1, B_ACTION_USE_MOVE, 0 );
			break;
		case 1:
			BtlController_EmitTwoReturnValues(
				1, B_ACTION_USE_ITEM, 0 );
			break;
		case 2:
			BtlController_EmitTwoReturnValues(
				1, B_ACTION_SWITCH, 0 );
...
static void HandleInputChooseTarget( void )
{
	...
	else if( gMain.newKeys & ( DPAD_LEFT | DPAD_UP ) )
	{
		...
		{
			u8 currSelIdentity =
				GetBattlerPosition( gMultiUsePlayerCursor );
			...
			{
				if( --i < 0 )
					i = MAX_BATTLERS_COUNT; // UB: array
								// out of range
				gMultiUsePlayerCursor =
					GetBattlerAtPosition( identities[i] );
...
static void HandleInputChooseMove( void )
{
	...
	struct ChooseMoveStruct* moveInfo =
		(struct ChooseMoveStruct*)( &gBattleBufferA[gActiveBattler]
							   [4] );
	...
	if( gMain.newKeys & A_BUTTON )
	{
		...
		PlaySE( SE_SELECT );
		if( moveInfo->moves[gMoveSelectionCursor[gActiveBattler]] ==
			MOVE_CURSE )
		{
			if( moveInfo->monType1 != TYPE_GHOST &&
				moveInfo->monType2 != TYPE_GHOST )
		...
		else
		{
			moveTarget =
				gBattleMoves
					[moveInfo->moves[gMoveSelectionCursor
							 [gActiveBattler]]]
						.target;
		}
		...
		{
			if( !( moveTarget &
				    ( MOVE_TARGET_RANDOM | MOVE_TARGET_BOTH |
					    MOVE_TARGET_DEPENDS |
			...
			if( moveInfo->currentPp[gMoveSelectionCursor
					    [gActiveBattler]] == 0 )
			{
				...
				CountAliveMonsInBattle(
					BATTLE_ALIVE_EXCEPT_ACTIVE ) <= 1 )
			{
				gMultiUsePlayerCursor =
					GetDefaultMoveTarget( gActiveBattler );
				canSelectTarget = FALSE;
			}
		}
		...
		else
		{
			...
			gSprites[gBattlerSpriteIds[gMultiUsePlayerCursor]]
				.callback = sub_8039AD8;
		}
	}
	else if( gMain.newKeys & B_BUTTON || gPlayerDpadHoldFrames > 59 )
	{
	...
	else if( gMain.newKeys & SELECT_BUTTON )
	{
		...
		{
			MoveSelectionCreateCursorAt(
				gMoveSelectionCursor[gActiveBattler], 29 );
...
static void HandleMoveSwitching( void )
{
	...
	if( gMain.newKeys & ( A_BUTTON | SELECT_BUTTON ) )
	{
		...
		if( gMoveSelectionCursor[gActiveBattler] !=
			gMultiUsePlayerCursor )
		{
			struct ChooseMoveStruct* moveInfo = (struct
				ChooseMoveStruct*)( &gBattleBufferA
							    [gActiveBattler]
							    [4] );
			s32 i;
			...
			if( gDisableStructs[gActiveBattler].mimickedMoves &
				gBitTable[gMoveSelectionCursor
						[gActiveBattler]] )
			{
			...
			totalPPBonuses = perMovePPBonuses
				[gMoveSelectionCursor[gActiveBattler]];
			perMovePPBonuses[gMoveSelectionCursor
					[gActiveBattler]] =
			...
			totalPPBonuses = 0;
			for( i = 0; i < MAX_MON_MOVES; i++ )
				totalPPBonuses |= perMovePPBonuses[i]
					<< ( i * 2 );
				    ...
				    STATUS2_TRANSFORMED ) )
			{
				...
				i = moveStruct.moves[gMoveSelectionCursor
						[gActiveBattler]];
				moveStruct.moves[gMoveSelectionCursor
						[gActiveBattler]] =
					moveStruct
						.moves[gMultiUsePlayerCursor];
				moveStruct.moves[gMultiUsePlayerCursor] = i;

				i = moveStruct.currentPp[gMoveSelectionCursor
						[gActiveBattler]];
				moveStruct.currentPp[gMoveSelectionCursor
						[gActiveBattler]] =
					moveStruct.currentPp
						[gMultiUsePlayerCursor];
				...
				SetMonData( &gPlayerParty[gBattlerPartyIndexes
							    [gActiveBattler]],
					MON_DATA_PP_BONUSES,
					&totalPPBonuses );
			}
...
{
	if( --gBattleSpritesDataPtr->healthBoxesData[gActiveBattler].field_9 ==
		0xFF )
	{
...
static void sub_805896C( void )
{
	...
	else
	{
		if( gSprites[gHealthboxSpriteIds[gActiveBattler]].callback ==
				SpriteCallbackDummy &&
			gSprites[gHealthboxSpriteIds[gActiveBattler ^
					 BIT_FLANK]]
...
static void sub_8058B40( void )
{
	...
	if( !IsDoubleBattle( ) ||
		( IsDoubleBattle( ) &&
			( gBattleTypeFlags & BATTLE_TYPE_MULTI ) ) )
	{
		if( gSprites[gUnknown_03005D7C[gActiveBattler]].callback ==
				SpriteCallbackDummy &&
	...
	else
	{
		if( gSprites[gUnknown_03005D7C[gActiveBattler]].callback ==
			...
			gSprites[gBattlerSpriteIds[gActiveBattler]].callback ==
				SpriteCallbackDummy &&
			gSprites[gUnknown_03005D7C[gActiveBattler ^ BIT_FLANK]]
					.callback == SpriteCallbackDummy &&
	...
	if( r9 && r8 )
	{
		if( IsDoubleBattle( ) &&
			!( gBattleTypeFlags & BATTLE_TYPE_MULTI ) )
			DestroySprite(
				&gSprites[gUnknown_03005D7C[gActiveBattler ^
					BIT_FLANK]] );
...
static void sub_8058EDC( void )
{
			...
			SpriteCallbackDummy )
	{
		...
		if( gBattleSpritesDataPtr->battlerData[gActiveBattler]
				.behindSubstitute )
...
{
	s16 hpValue = MoveBattleBar( gActiveBattler,
		gHealthboxSpriteIds[gActiveBattler],
		HEALTH_BAR,
		0 );
...
static void Task_GiveExpToMon( u8 taskId )
{
	...
	if( IsDoubleBattle( ) == TRUE ||
		monId !=
			gBattlerPartyIndexes[battlerId] ) // Give exp without
							  // moving the expbar.
	{
		...
		u32 nextLvlExp =
			gExperienceTables[gBaseStats[species].growthRate]
					 [level + 1];
		...
		if( currExp + gainedExp >= nextLvlExp )
		{
			...
			BtlController_EmitTwoReturnValues(
				1, RET_VALUE_LEVELED_UP, gainedExp );
		...
		else
		{
			...
			gBattlerControllerFuncs[battlerId] =
...
static void Task_PrepareToGiveExpWithExpBar( u8 taskId )
{
	...
	exp -= currLvlExp;
	expToNextLvl =
		gExperienceTables[gBaseStats[species].growthRate][level + 1] -
		currLvlExp;
	SetBattleBarStruct( battlerId,
...
static void sub_8059400( u8 taskId )
{
	...
	else
	{
		...
		newExpPoints = MoveBattleBar( battlerId,
			gHealthboxSpriteIds[battlerId],
			EXP_BAR,
			0 );
		SetHealthboxSpriteVisible( gHealthboxSpriteIds[battlerId] );
		if( newExpPoints == -1 ) // The bar has been filled with given
					 // exp points.
		{
			u8 level;
			s32 currExp;
			...
			level = GetMonData(
				&gPlayerParty[monId], MON_DATA_LEVEL );
			currExp = GetMonData(
				&gPlayerParty[monId], MON_DATA_EXP );
			species = GetMonData(
				&gPlayerParty[monId], MON_DATA_SPECIES );
			expOnNextLvl = gExperienceTables
				[gBaseStats[species].growthRate][level + 1];
...
static void Task_LaunchLvlUpAnim( u8 taskId )
{
	...
	if( IsDoubleBattle( ) == TRUE &&
		monIndex == gBattlerPartyIndexes[battlerId ^ BIT_FLANK] )
		battlerId ^= BIT_FLANK;

	InitAndLaunchSpecialAnimation(
		battlerId, battlerId, battlerId, B_ANIM_LVL_UP );
...
static void Task_UpdateLvlInHealthbox( u8 taskId )
{
	...
	if( !gBattleSpritesDataPtr->healthBoxesData[battlerId]
			.specialAnimActive )
	{
		u8 monIndex = gTasks[taskId].tExpTask_monId;

		GetMonData(
			&gPlayerParty[monIndex], MON_DATA_LEVEL ); // Unused
								   // return
								   // value.

		if( IsDoubleBattle( ) == TRUE &&
			monIndex ==
				gBattlerPartyIndexes[battlerId ^ BIT_FLANK] )
			UpdateHealthboxAttribute(
				gHealthboxSpriteIds[battlerId ^ BIT_FLANK],
...
static void DestroyExpTaskAndCompleteOnInactiveTextPrinter( u8 taskId )
{
	...
	GetMonData( &gPlayerParty[monIndex], MON_DATA_LEVEL ); // Unused return
							       // value.
...
static void sub_80596A8( void )
{
		...
		DISPLAY_HEIGHT )
	{
		...
		nullsub_24( species );
		FreeOamMatrix( gSprites[gBattlerSpriteIds[gActiveBattler]]
				       .oam.matrixNum );
		DestroySprite( &gSprites[gBattlerSpriteIds[gActiveBattler]] );
	...
	if( gMain.callback2 == BattleMainCB2 && !gPaletteFade.active )
	{
		if( gUnknown_0203CEE8 == 1 )
			BtlController_EmitChosenMonReturnValue(
				1, gUnknown_0203CEE9, gUnknown_0203CF00 );
		else
...
static void MoveSelectionDisplayPpNumber( void )
{
	...
	if( gBattleBufferA[gActiveBattler][2] ==
		TRUE ) // check if we didn't want
		       // to display pp number
		return;

	SetPpNumbersPaletteInMoveSelection( );
	moveInfo  = (struct ChooseMoveStruct*)( &gBattleBufferA[gActiveBattler]
                                                              [4] );
...
static void MoveSelectionDisplayMoveType( void )
{
	...
	StringCopy( txtPtr,
		gTypeNames[gBattleMoves[moveInfo->moves[gMoveSelectionCursor
							[gActiveBattler]]]
				   .type] );
...
static void MoveSelectionCreateCursorAt( u8 cursorPosition, u8 arg1 )
{
	...
	CopyToBgTilemapBufferRect_ChangePalette( 0,
		src,
...
static void PlayerHandleGetMonData( void )
{
	u8 monData[sizeof( struct Pokemon ) * 2 + 56]; // this allows to get
						       // full data of two
						       // pokemon, trying to
						       // get more will result
						       // in overwriting data
	u32 size = 0;
	u8 monToCheck;
	s32 i;
	...
	if( gBattleBufferA[gActiveBattler][2] == 0 )
	{
		size += CopyPlayerMonData(
			gBattlerPartyIndexes[gActiveBattler], monData );
	}
	else
	{
...
static u32 CopyPlayerMonData( u8 monId, u8* dst )
{
	...
	case REQUEST_ALL_BATTLE:
		...
		battleMon.item =
			GetMonData( &gPlayerParty[monId], MON_DATA_HELD_ITEM );
		for( size = 0; size < MAX_MON_MOVES; size++ )
		{
			battleMon.moves[size] = GetMonData(
				&gPlayerParty[monId], MON_DATA_MOVE1 + size );
			battleMon.pp[size] = GetMonData(
				&gPlayerParty[monId], MON_DATA_PP1 + size );
		}
		battleMon.ppBonuses = GetMonData(
			&gPlayerParty[monId], MON_DATA_PP_BONUSES );
		battleMon.friendship = GetMonData(
			&gPlayerParty[monId], MON_DATA_FRIENDSHIP );
		battleMon.experience =
			GetMonData( &gPlayerParty[monId], MON_DATA_EXP );
		battleMon.hpIV =
			GetMonData( &gPlayerParty[monId], MON_DATA_HP_IV );
		battleMon.attackIV =
			GetMonData( &gPlayerParty[monId], MON_DATA_ATK_IV );
		battleMon.defenseIV =
			GetMonData( &gPlayerParty[monId], MON_DATA_DEF_IV );
		battleMon.speedIV =
			GetMonData( &gPlayerParty[monId], MON_DATA_SPEED_IV );
		battleMon.spAttackIV =
			GetMonData( &gPlayerParty[monId], MON_DATA_SPATK_IV );
		battleMon.spDefenseIV =
			GetMonData( &gPlayerParty[monId], MON_DATA_SPDEF_IV );
		battleMon.personality = GetMonData(
			&gPlayerParty[monId], MON_DATA_PERSONALITY );
		battleMon.status1 =
			GetMonData( &gPlayerParty[monId], MON_DATA_STATUS );
		battleMon.level =
			GetMonData( &gPlayerParty[monId], MON_DATA_LEVEL );
		battleMon.hp = GetMonData( &gPlayerParty[monId], MON_DATA_HP );
		battleMon.maxHP =
			GetMonData( &gPlayerParty[monId], MON_DATA_MAX_HP );
		battleMon.attack =
			GetMonData( &gPlayerParty[monId], MON_DATA_ATK );
		battleMon.defense =
			GetMonData( &gPlayerParty[monId], MON_DATA_DEF );
		battleMon.speed =
			GetMonData( &gPlayerParty[monId], MON_DATA_SPEED );
		battleMon.spAttack =
			GetMonData( &gPlayerParty[monId], MON_DATA_SPATK );
		battleMon.spDefense =
			GetMonData( &gPlayerParty[monId], MON_DATA_SPDEF );
		battleMon.isEgg =
			GetMonData( &gPlayerParty[monId], MON_DATA_IS_EGG );
		battleMon.abilityNum = GetMonData(
			&gPlayerParty[monId], MON_DATA_ABILITY_NUM );
		battleMon.otId =
			GetMonData( &gPlayerParty[monId], MON_DATA_OT_ID );
		GetMonData(
			&gPlayerParty[monId], MON_DATA_NICKNAME, nickname );
		StringCopy10( battleMon.nickname, nickname );
		GetMonData( &gPlayerParty[monId],
			MON_DATA_OT_NAME,
			battleMon.otName );
		src = (u8*)&battleMon;
	...
	case REQUEST_MOVES_PP_BATTLE:
		for( size = 0; size < MAX_MON_MOVES; size++ )
		{
			moveData.moves[size] = GetMonData(
				&gPlayerParty[monId], MON_DATA_MOVE1 + size );
			moveData.pp[size] = GetMonData(
				&gPlayerParty[monId], MON_DATA_PP1 + size );
		}
		moveData.ppBonuses = GetMonData(
			&gPlayerParty[monId], MON_DATA_PP_BONUSES );
		src = (u8*)( &moveData );
	...
	case REQUEST_MET_LOCATION_BATTLE:
		dst[0] = GetMonData(
			&gPlayerParty[monId], MON_DATA_MET_LOCATION );
		size = 1;
		break;
	case REQUEST_MET_LEVEL_BATTLE:
		dst[0] =
			GetMonData( &gPlayerParty[monId], MON_DATA_MET_LEVEL );
		size = 1;
		break;
	...
	case REQUEST_COOL_RIBBON_BATTLE:
		dst[0] = GetMonData(
			&gPlayerParty[monId], MON_DATA_COOL_RIBBON );
		size = 1;
		break;
	case REQUEST_BEAUTY_RIBBON_BATTLE:
		dst[0] = GetMonData(
			&gPlayerParty[monId], MON_DATA_BEAUTY_RIBBON );
		size = 1;
		break;
	case REQUEST_CUTE_RIBBON_BATTLE:
		dst[0] = GetMonData(
			&gPlayerParty[monId], MON_DATA_CUTE_RIBBON );
		size = 1;
		break;
	case REQUEST_SMART_RIBBON_BATTLE:
		dst[0] = GetMonData(
			&gPlayerParty[monId], MON_DATA_SMART_RIBBON );
		size = 1;
		break;
	case REQUEST_TOUGH_RIBBON_BATTLE:
		dst[0] = GetMonData(
			&gPlayerParty[monId], MON_DATA_TOUGH_RIBBON );
		size = 1;
		break;
	}
...
void PlayerHandleGetRawMonData( void )
{
	...
	BtlController_EmitDataTransfer(
		1, gBattleBufferA[gActiveBattler][2], dst );
	PlayerBufferExecCompleted( );
}
...
static void SetPlayerMonData( u8 monId )
{
	...
	case REQUEST_ALL_BATTLE: {
		...
		SetMonData( &gPlayerParty[monId],
			MON_DATA_ATK,
			&battlePokemon->attack );
		SetMonData( &gPlayerParty[monId],
			MON_DATA_DEF,
			&battlePokemon->defense );
		SetMonData( &gPlayerParty[monId],
			MON_DATA_SPEED,
			&battlePokemon->speed );
		SetMonData( &gPlayerParty[monId],
			MON_DATA_SPATK,
			&battlePokemon->spAttack );
		SetMonData( &gPlayerParty[monId],
			MON_DATA_SPDEF,
			&battlePokemon->spDefense );
	}
	break;
	case REQUEST_SPECIES_BATTLE:
		SetMonData( &gPlayerParty[monId],
...
static void PlayerHandleSwitchInAnim( void )
{
	ClearTemporarySpeciesSpriteData(
		gActiveBattler, gBattleBufferA[gActiveBattler][2] );
	gBattlerPartyIndexes[gActiveBattler] =
		gBattleBufferA[gActiveBattler][1];
	BattleLoadPlayerMonSpriteGfx(
		&gPlayerParty[gBattlerPartyIndexes[gActiveBattler]],
...
static void sub_805B258( u8 battlerId, bool8 dontClearSubstituteBit )
{
	...
	species = GetMonData( &gPlayerParty[gBattlerPartyIndexes[battlerId]],
		MON_DATA_SPECIES );
	gUnknown_03005D7C[battlerId] =
		CreateInvisibleSpriteWithCallback( sub_805D714 );
	SetMultiuseSpriteTemplateToPokemon(
		species, GetBattlerPosition( battlerId ) );
	...
	gSprites[gUnknown_03005D7C[battlerId]].data[1] =
...
static void DoSwitchOutAnimation( void )
{
	switch( gBattleSpritesDataPtr->healthBoxesData[gActiveBattler]
			.animationState )
	{
	case 0:
		if( gBattleSpritesDataPtr->battlerData[gActiveBattler]
...
static void PlayerHandleDrawTrainerPic( void )
{
	...
	if( gBattleTypeFlags & BATTLE_TYPE_LINK )
	{
		if( ( gLinkPlayers[GetMultiplayerId( )].version & 0xFF ) ==
				VERSION_FIRE_RED ||
			( gLinkPlayers[GetMultiplayerId( )].version & 0xFF ) ==
				VERSION_LEAF_GREEN )
		{
			trainerPicId =
				gLinkPlayers[GetMultiplayerId( )].gender +
				TRAINER_BACK_PIC_RED;
		}
		else if( ( gLinkPlayers[GetMultiplayerId( )].version &
				 0xFF ) == VERSION_RUBY ||
			( gLinkPlayers[GetMultiplayerId( )].version & 0xFF ) ==
				VERSION_SAPPHIRE )
		{
	...
	if( gBattleTypeFlags & BATTLE_TYPE_MULTI )
	{
			...
			gPartnerTrainerId != TRAINER_STEVEN_PARTNER )
		{
			xPos = 90;
			yPos = ( 8 -
				       gTrainerFrontPicCoords[trainerPicId]
					       .size ) *
					4 +
				80;
		}
		...
		{
			yPos = ( 8 -
				       gTrainerBackPicCoords[trainerPicId]
					       .size ) *
					4 +
				80;
		}
	...
	{
		xPos = 80;
		yPos = ( 8 - gTrainerBackPicCoords[trainerPicId].size ) * 4 +
			80;
	}

	// Use front pic table for any tag battles unless your partner is
	// Steven.
	if( gBattleTypeFlags & BATTLE_TYPE_INGAME_PARTNER &&
		gPartnerTrainerId != TRAINER_STEVEN_PARTNER )
	{
		...
		SetMultiuseSpriteTemplateToTrainerFront(
			trainerPicId, GetBattlerPosition( gActiveBattler ) );
		...
		gSprites[gBattlerSpriteIds[gActiveBattler]]
			.oam.paletteNum = IndexOfSpritePaletteTag(
			gTrainerFrontPicPaletteTable[trainerPicId].tag );
	...
	else
	{
				...
				.specialAnimActive )
		{
			...
			HandleLowHpMusicChange(
				...
				gActiveBattler );
			PlaySE12WithPanning(
				SE_POKE_DEAD, SOUND_PAN_ATTACKER );
			gSprites[gBattlerSpriteIds[gActiveBattler]].data[1] =
				0;
			gSprites[gBattlerSpriteIds[gActiveBattler]].data[2] =
				5;
			gSprites[gBattlerSpriteIds[gActiveBattler]].callback =
				sub_8039C00;
...
static void PlayerHandleSuccessBallThrowAnim( void )
{
	gBattleSpritesDataPtr->animationData->ballThrowCaseId =
...
static void PlayerHandleMoveAnimation( void )
{
	if( !mplay_80342A4( gActiveBattler ) )
	{
		...
		gAnimDisableStructPtr =
			(struct DisableStruct*)&gBattleBufferA[gActiveBattler]
							      [16];
		gTransformedPersonalities[gActiveBattler] =
			gAnimDisableStructPtr->transformedMonPersonality;
		if( IsMoveWithoutAnimation(
			    move, gAnimMoveTurn ) ) // Always returns
						    // FALSE.
		{
			PlayerBufferExecCompleted( );
		}
		else
		{
			...
			BattleTv_SetDataBasedOnMove( move,
				gWeatherMoveAnim,
				gAnimDisableStructPtr );
		}
	}
...
static void PlayerDoMoveAnimation( void )
{
	...
	case 0:
		if( gBattleSpritesDataPtr->battlerData[gActiveBattler]
				.behindSubstitute &&
			!gBattleSpritesDataPtr->battlerData[gActiveBattler]
				 .flag_x8 )
		{
			gBattleSpritesDataPtr->battlerData[gActiveBattler]
				.flag_x8 = 1;
...
static void PlayerChooseMoveInBattlePalace( void )
{
	if( --*( gBattleStruct->arenaMindPoints + gActiveBattler ) == 0 )
	{
		...
		BtlController_EmitTwoReturnValues(
			1, 10, ChooseMoveAndTargetInBattlePalace( ) );
		PlayerBufferExecCompleted( );
	}
}
...
static void PlayerHandleChoosePokemon( void )
{
	...
	if( gBattleTypeFlags & BATTLE_TYPE_ARENA &&
		( gBattleBufferA[gActiveBattler][1] & 0xF ) !=
			PARTY_CANT_SWITCH )
	{
		BtlController_EmitChosenMonReturnValue( 1,
			gBattlerPartyIndexes[gActiveBattler] + 1,
			gUnknown_0203CF00 );
		PlayerBufferExecCompleted( );
	}
	else
	{
		gUnknown_03005D7C[gActiveBattler] =
			CreateTask( TaskDummy, 0xFF );
		gTasks[gUnknown_03005D7C[gActiveBattler]].data[0] =
			gBattleBufferA[gActiveBattler][1] & 0xF;
		*( &gBattleStruct->battlerPreventingSwitchout ) =
			gBattleBufferA[gActiveBattler][1] >> 4;
		*( &gBattleStruct->field_8B ) =
			gBattleBufferA[gActiveBattler][2];
...
static void PlayerHandleHealthBarUpdate( void )
{
	...
	{
		u32 maxHP = GetMonData(
			&gPlayerParty[gBattlerPartyIndexes[gActiveBattler]],
			MON_DATA_MAX_HP );
		u32 curHP = GetMonData(
			&gPlayerParty[gBattlerPartyIndexes[gActiveBattler]],
			MON_DATA_HP );
	...
	else
	{
		...
		UpdateHpTextInHealthbox(
			gHealthboxSpriteIds[gActiveBattler], 0, HP_CURRENT );
	}
...
static void PlayerHandleExpUpdate( void )
{
	...
	else
	{
		...
		LoadBattleBarGfx( 1 );
		GetMonData( &gPlayerParty[monId],
			MON_DATA_SPECIES ); // Unused return
					    // value.
		expPointsToGive =
			T1_READ_16( &gBattleBufferA[gActiveBattler][2] );
...
static void PlayerHandleStatusIconUpdate( void )
{
	if( !mplay_80342A4( gActiveBattler ) )
	{
		...
		gBattleSpritesDataPtr->healthBoxesData[battlerId]
			.statusAnimActive = 0;
		gBattlerControllerFuncs[gActiveBattler] =
...
static void PlayerHandleCmd42( void ) { PlayerBufferExecCompleted( ); }
...
static void PlayerHandleIntroTrainerBallThrow( void )
{
	...
	StoreSpriteCallbackInData6(
		&gSprites[gBattlerSpriteIds[gActiveBattler]], sub_805CC00 );
	StartSpriteAnim( &gSprites[gBattlerSpriteIds[gActiveBattler]], 1 );
	...
	else
	{
		...
		gBattlerStatusSummaryTaskId[gActiveBattler] =
			CreatePartyStatusSummarySprites( gActiveBattler,
				(struct HpAndStatus*)&gBattleBufferA
					[gActiveBattler][4],
				gBattleBufferA[gActiveBattler][1],
				gBattleBufferA[gActiveBattler][2] );
		gBattleSpritesDataPtr->healthBoxesData[gActiveBattler]
...
static void sub_805CE38( void )
{
	if( gBattleSpritesDataPtr->healthBoxesData[gActiveBattler].field_5++ >
		0x5C )
	{
		gBattleSpritesDataPtr->healthBoxesData[gActiveBattler]
			.field_5 = 0;
...
static void PlayerHandleBattleAnimation( void )
{
	if( !mplay_80342A4( gActiveBattler ) )
	{
		...
		if( TryHandleLaunchBattleTableAnimation( gActiveBattler,
			    gActiveBattler,
			    gActiveBattler,
			    animationId,
			    argument ) )
...
static void PlayerHandleCmd55( void )
{
	...
	gSaveBlock2Ptr->frontier.field_CA9_b =
		gBattleBufferA[gActiveBattler][2];
	FadeOutMapMusic( 5 );
