...
#define subsprite_table( ptr ) \
	{ \
		.subsprites = ptr, \
		.subspriteCount = \
			( sizeof ptr ) / ( sizeof( struct Subsprite ) ) \
	}
...
static void escalatorAnimTask( u8 );
static bool8 gEscalator_Task0( struct Task* );
static bool8 gEscalator_Task1( struct Task* );
static bool8 gEscalator_Task2( struct Task* );
static bool8 gEscalator_Task3( struct Task* );
static bool8 gEscalator_Task4( struct Task* );
static bool8 gEscalator_Task5( struct Task* );

static void gEscalator_WarpUp( struct Task* );
static void gEscalator_WarpDown( struct Task* );
static void gEscalator_DoWarp( void );
static void gEscalator_FinishWarp( void );
static void SetupEscalator2Anim( void );
static void escalator2AnimTask( u8 );

static bool8 gEscalator2_Task0( struct Task* );
static bool8 gEscalator2_TaskWarpUp( struct Task* );
static bool8 gEscalator2_TaskWarpUp2( struct Task* );
static bool8 gEscalator2_TaskWarpDown( struct Task* );
static bool8 gEscalator2_TaskWarpDown2( struct Task* );
...
static bool8 waterfall_2_wait_anim_finish_probably(
	struct Task*, struct EventObject* );
...
extern struct CompressedSpritePalette gMonPaletteTable[]; // GF made a mistake
							  // and did not extern
							  // it as const.
...
// .rodata
const u32 gNewGameBirchPic[] =
	INCBIN_U32( "graphics/birch_speech/birch.4bpp" );
const u32 gUnusedBirchBeauty[] =
	INCBIN_U32( "graphics/unused/intro_birch_beauty.4bpp" );
const u16 gNewGameBirchPalette[16] =
	INCBIN_U16( "graphics/birch_speech/birch.gbapal" );
const u32 gSpriteImage_855A970[] =
	INCBIN_U32( "graphics/misc/pokeball_glow.4bpp" );
const u16 gFieldEffectObjectPalette4[16] = INCBIN_U16(
	"graphics/event_objects/palettes/field_effect_object_palette_04.gbapal" );
const u32 gSpriteImage_855A9B0[] =
	INCBIN_U32( "graphics/misc/pokecenter_monitor/0.4bpp" );
const u32 gSpriteImage_855AA70[] =
	INCBIN_U32( "graphics/misc/pokecenter_monitor/1.4bpp" );
const u32 gSpriteImage_855AB30[] =
	INCBIN_U32( "graphics/misc/big_hof_monitor.4bpp" );
const u8 gSpriteImage_855AD30[] =
	INCBIN_U8( "graphics/misc/small_hof_monitor.4bpp" );
const u16 gFieldEffectObjectPalette5[16] = INCBIN_U16(
	"graphics/event_objects/palettes/field_effect_object_palette_05.gbapal" );

// Graphics for the lights streaking past your Pokemon when it uses a field
// move.
const u32 gFieldMoveStreaksTiles[] =
	INCBIN_U32( "graphics/misc/field_move_streaks.4bpp" );
const u16 gFieldMoveStreaksPalette[16] =
	INCBIN_U16( "graphics/misc/field_move_streaks.gbapal" );
const u16 gFieldMoveStreaksTilemap[] =
	INCBIN_U16( "graphics/misc/field_move_streaks_map.bin" );

// The following light streaks effect is used when the map is dark (e.g. a
// cave).
const u32 gDarknessFieldMoveStreaksTiles[] =
	INCBIN_U32( "graphics/misc/darkness_field_move_streaks.4bpp" );
const u16 gDarknessFieldMoveStreaksPalette[16] =
	INCBIN_U16( "graphics/misc/darkness_field_move_streaks.gbapal" );
const u16 gDarknessFieldMoveStreaksTilemap[] =
	INCBIN_U16( "graphics/misc/darkness_field_move_streaks_map.bin" );

const u16 gUnknown_0855B610[16] =
	INCBIN_U16( "graphics/misc/spotlight.gbapal" );
const u8 gUnknown_0855B630[] = INCBIN_U8( "graphics/misc/spotlight.4bpp" );
const u8 gUnknown_0855C170[] =
	INCBIN_U8( "graphics/unknown/unknown_55C170.4bpp" );
const u8 gUnknown_0855C190[] =
	INCBIN_U8( "graphics/unknown/unknown_55C190.4bpp" );
const u8 gUnknown_0855C1B0[] =
	INCBIN_U8( "graphics/unknown/unknown_55C1B0.4bpp" );
const u8 gUnknown_0855C1D0[] =
	INCBIN_U8( "graphics/unknown/unknown_55C1D0.4bpp" );
...
const struct SpritePalette gNewGameBirchObjectPaletteInfo = {
	.data = gNewGameBirchPalette, .tag = 0x1006};
...
const struct SpriteTemplate gNewGameBirchObjectTemplate = {.tileTag = 0xFFFF,
	.paletteTag                                                 = 4102,
...
const struct SpritePalette gFieldEffectObjectPaletteInfo4 = {
	.data = gFieldEffectObjectPalette4, .tag = 0x1007};

const struct SpritePalette gFieldEffectObjectPaletteInfo5 = {
	.data = gFieldEffectObjectPalette5, .tag = 0x1010};
...
const struct SpriteFrameImage gSpriteImageTable_855C294[] = {
	{.data        = gSpriteImage_855AD30,
		.size = 0x200} // the macro breaks down here
};

const struct Subsprite gSubspriteTable_855C29C[] = {{.x                 = -12,
							    .y          = -8,
							    .priority   = 2,
							    .tileOffset = 0,
							    .shape      = 1,
...
const struct SubspriteTable gUnknown_0855C2AC =
	subsprite_table( gSubspriteTable_855C29C );

const struct Subsprite gSubspriteTable_855C2B4[] = {{.x                 = -32,
							    .y          = -8,
							    .priority   = 2,
							    .tileOffset = 0,
							    .shape      = 1,
...
const struct SubspriteTable gUnknown_0855C2C4 =
	subsprite_table( gSubspriteTable_855C2B4 );
...
const union AnimCmd* const gSpriteAnimTable_855C2F8[] = {
	gSpriteAnim_855C2CC, gSpriteAnim_855C2D4};

const union AnimCmd* const gSpriteAnimTable_855C300[] = {gSpriteAnim_855C2CC};

const struct SpriteTemplate gSpriteTemplate_855C304 = {.tileTag = 0xFFFF,
	.paletteTag                                             = 4103,
...
const struct SpriteTemplate gSpriteTemplate_855C31C = {.tileTag = 0xFFFF,
	.paletteTag                                             = 4100,
...
const struct SpriteTemplate gSpriteTemplate_855C334 = {.tileTag = 0xFFFF,
	.paletteTag                                             = 4112,
	.oam         = &gOamData_855C220,
...
const struct SpriteTemplate gSpriteTemplate_855C34C = {.tileTag = 0xFFFF,
	.paletteTag                                             = 4112,
...
void ( *const gUnknown_0855C364[] )( struct Task* ) = {PokecenterHealEffect_0,
...
void ( *const gUnknown_0855C384[] )( struct Sprite* ) = {PokeballGlowEffect_0,
...
const struct Coords16 gUnknown_0855C3A4[] = {{.x = 0, .y = 0},
	{.x = 6, .y = 0},
	{.x = 0, .y = 4},
	{.x = 6, .y = 4},
	{.x = 0, .y = 8},
...
bool8 ( *const gEscalator_TaskArr[] )( struct Task* ) = {
	gEscalator_Task0,
	gEscalator_Task1,
	gEscalator_Task2,
	gEscalator_Task3,
...
bool8 ( *const gEscalator2_TaskArr[] )( struct Task* ) = {
	gEscalator2_Task0,
	gEscalator2_TaskWarpUp,
	gEscalator2_TaskWarpUp2,
	gEscalator2_TaskWarpDown,
	gEscalator2_TaskWarpDown2,
	sub_80B72D0,
	sub_80B72F4,
};
...
bool8 ( *const gUnknown_0855C438[] )(
	struct Task*, struct EventObject*, struct Sprite* ) = {
	sub_80B764C,
	sub_80B7684,
	sub_80B76B8,
	sub_80B7704,
...
{
	return ( *script )[0] + ( ( *script )[1] << 8 ) +
		( ( *script )[2] << 16 ) + ( ( *script )[3] << 24 );
}
...
void FieldEffectScript_LoadFadedPalette( u8** script )
{
	...
	UpdateSpritePaletteWithWeather(
		IndexOfSpritePaletteTag( palette->tag ) );
	( *script ) += 4;
}
...
{
	u32 ( *func )( void ) =
		(u32( * )( void ))FieldEffectScript_ReadWord( script );
	*val = func( );
	( *script ) += 4;
}
	...
	u8 trainerSpriteID, s16 x, s16 y, u8 subpriority, u8* buffer )
{
	...
	spriteTemplate.paletteTag =
		gTrainerFrontPicPaletteTable[trainerSpriteID].tag;
...
void LoadTrainerGfx_TrainerCard( u8 gender, u16 palOffset, u8* dest )
{
	...
	LoadCompressedPalette(
		gTrainerFrontPicPaletteTable[gender].data, palOffset, 0x20 );
}
...
{
	s32 spriteId = CreateMonPicSprite_HandleDeoxys( species,
		0,
		0x8000,
		1,
		x,
	...
	PreservePaletteInWeather(
		IndexOfSpritePaletteTag( gMonPaletteTable[species].tag ) +
		0x10 );
	if( spriteId == 0xFFFF )
		return MAX_SPRITES;
	else
...
u8 CreateMonSprite_FieldMove(
	u16 species, u32 d, u32 g, s16 x, s16 y, u8 subpriority )
{
	const struct CompressedSpritePalette* spritePalette =
		GetMonSpritePalStructFromOtIdPersonality( species, d, g );
	u16 spriteId = CreateMonPicSprite_HandleDeoxys(
		species, d, g, 1, x, y, 0, spritePalette->tag );
	PreservePaletteInWeather(
		IndexOfSpritePaletteTag( spritePalette->tag ) + 0x10 );
	if( spriteId == 0xFFFF )
		return MAX_SPRITES;
	else
		return spriteId;
...
{
	task->data[0]++;
	task->data[6] = CreatePokeballGlowSprite(
		task->data[1], task->data[2], task->data[3], 1 );
	task->data[7] =
		PokecenterHealEffectHelper( task->data[4], task->data[5] );
}
...
static void HallOfFameRecordEffect_1( struct Task* task )
{
	if( gSprites[task->data[6]].data[0] > 1 )
	{
		task->data[15]++; // was this ever initialized? is this ever
				  // used?
		task->data[0]++;
	}
}
...
static void PokeballGlowEffect_0( struct Sprite* sprite )
{
	...
	{
		sprite->data[1] = 25;
		endSpriteId     = CreateSpriteAtEnd( &gSpriteTemplate_855C304,
                        gUnknown_0855C3A4[sprite->data[2]].x + sprite->pos2.x,
                        gUnknown_0855C3A4[sprite->data[2]].y + sprite->pos2.y,
                        0 );
...
static void PokeballGlowEffect_2( struct Sprite* sprite )
{
	...
	MultiplyInvertedPaletteRGBComponents(
		( IndexOfSpritePaletteTag( 0x1007 ) << 4 ) + 0x108,
		gUnknown_0855C3BC[phase],
		gUnknown_0855C3C0[phase],
		gUnknown_0855C3C4[phase] );
	phase = ( sprite->data[2] + 2 ) & 3;
	MultiplyInvertedPaletteRGBComponents(
		( IndexOfSpritePaletteTag( 0x1007 ) << 4 ) + 0x106,
		gUnknown_0855C3BC[phase],
		gUnknown_0855C3C0[phase],
		gUnknown_0855C3C4[phase] );
	phase = ( sprite->data[2] + 1 ) & 3;
	MultiplyInvertedPaletteRGBComponents(
		( IndexOfSpritePaletteTag( 0x1007 ) << 4 ) + 0x102,
		gUnknown_0855C3BC[phase],
		gUnknown_0855C3C0[phase],
		gUnknown_0855C3C4[phase] );
	phase = sprite->data[2];
	MultiplyInvertedPaletteRGBComponents(
		( IndexOfSpritePaletteTag( 0x1007 ) << 4 ) + 0x105,
		gUnknown_0855C3BC[phase],
		gUnknown_0855C3C0[phase],
		gUnknown_0855C3C4[phase] );
	MultiplyInvertedPaletteRGBComponents(
		( IndexOfSpritePaletteTag( 0x1007 ) << 4 ) + 0x103,
		gUnknown_0855C3BC[phase],
		gUnknown_0855C3C0[phase],
		gUnknown_0855C3C4[phase] );
	if( sprite->data[3] > 2 )
...
static void HallOfFameRecordEffectHelper( s16 a0, s16 a1, s16 a2, u8 a3 )
{
	...
	if( !a3 )
	{
		spriteIdAtEnd = CreateSpriteAtEnd(
			&gSpriteTemplate_855C334, a1, a2, 0 );
		SetSubspriteTables(
			&gSprites[spriteIdAtEnd], &gUnknown_0855C2C4 );
	}
	else
	{
		spriteIdAtEnd = CreateSpriteAtEnd(
			&gSpriteTemplate_855C34C, a1, a2, 0 );
	}
...
static void mapldr_08084390( void )
{
	...
	{
		EventObjectTurn( &gEventObjects[gPlayerAvatar.eventObjectId],
			DIR_WEST );
	}
	ScriptContext2_Enable( );
	FreezeEventObjects( );
...
static void sub_80B6B94( u8 taskId )
{
	...
	while( gUnknown_0855C3C8[task->data[0]]( task ) )
		; // return code signifies whether to continue blocking here
}
...
static bool8 sub_80B6BCC( struct Task* task ) // gUnknown_0855C3C8[0]
{
	...
	EventObjectSetHeldMovement( playerObject,
...
static bool8 sub_80B6C90( struct Task* task ) // gUnknown_0855C3C8[2]
{
	...
	sprite->pos2.y     = -( sprite->pos1.y + sprite->centerToCornerVecY +
...
void EscalatorAnimTaskSetup( u8 mb, u8 priority )
{
	u8 taskId;
	taskId                 = CreateTask( escalatorAnimTask, priority );
	gTasks[taskId].data[1] = 0; // MB_DOWN_ESCALATOR
	if( mb == MB_UP_ESCALATOR )
	{
		gTasks[taskId].data[1] = 1;
	}
	else if( mb == MB_DOWN2_ESCALATOR )
	{
		gTasks[taskId].data[1] = 2;
	}
	else if( mb == MB_UP2_ESCALATOR )
	{
		gTasks[taskId].data[1] = 3;
	}
}

static void escalatorAnimTask( u8 taskId )
{
	struct Task* task;
	task = &gTasks[taskId];
	while( gEscalator_TaskArr[task->data[0]]( task ) )
		;
}

static bool8 gEscalator_Task0( struct Task* task )
{
	FreezeEventObjects( );
	CameraObjectReset2( );
	InitEscalatorAnimTask( task->data[1] );
	task->data[0]++;
	return FALSE;
}

static bool8 gEscalator_Task1( struct Task* task )
{
	struct EventObject* eventObject;
	eventObject = &gEventObjects[gPlayerAvatar.eventObjectId];
	...
	{
		u8 data1 = (u8)task->data[1];
		EventObjectSetHeldMovement( eventObject,
			GetFaceDirectionMovementAction(
				GetPlayerFacingDirection( ) ) );
		task->data[0]++;
		task->data[2] = 0;
...
// up
static bool8 gEscalator_Task2( struct Task* task )
{
	gEscalator_WarpUp( task );
	if( task->data[2] > 3 )
	{
		gEscalator_DoWarp( );
		task->data[0]++;
...
static bool8 gEscalator_Task3( struct Task* task )
{
	gEscalator_WarpUp( task );
	gEscalator_FinishWarp( );
	return FALSE;
}

// down
static bool8 gEscalator_Task4( struct Task* task )
{
	gEscalator_WarpDown( task );
	if( task->data[2] > 3 )
	{
		gEscalator_DoWarp( );
		task->data[0]++;
...
static bool8 gEscalator_Task5( struct Task* task )
{
	gEscalator_WarpDown( task );
	gEscalator_FinishWarp( );
	return FALSE;
...
static void gEscalator_WarpUp( struct Task* task )
{
	struct Sprite* sprite;
	sprite = &gSprites[gPlayerAvatar.spriteId];
	if( task->data[1] < 2 )
	...
	{
		sprite->pos2.x = Cos( 0x94, task->data[2] ) * -1;
		sprite->pos2.y = Sin( 0x84, task->data[2] ) * -1;
	}
	task->data[3]++;
	if( task->data[3] & 1 )
	{
...
static void gEscalator_WarpDown( struct Task* task )
{
	struct Sprite* sprite;
	sprite = &gSprites[gPlayerAvatar.spriteId];
	if( task->data[1] < 2 )
	...
	{
		sprite->pos2.x = Cos( 0x76, task->data[2] ) * -1;
		sprite->pos2.y = Sin( 0x7C, task->data[2] ) * -1;
	}
	task->data[3]++;
	if( task->data[3] & 1 )
	{
...
static void gEscalator_FinishWarp( void )
{
	...
	{
		FiniEscalatorAnimTask( );
		WarpIntoMap( );
		gFieldCallback = SetupEscalator2Anim;
		SetMainCallback2( CB2_LoadMap );
		DestroyTask( FindTaskIdByFunc( escalatorAnimTask ) );
	}
}
...
static void SetupEscalator2Anim( void )
{
	...
	pal_fill_for_maplights( );
	ScriptContext2_Enable( );
	CreateTask( escalator2AnimTask, 0 );
	gFieldCallback = NULL;
}

static void escalator2AnimTask( u8 taskId )
{
	struct Task* task;
	task = &gTasks[taskId];
	while( gEscalator2_TaskArr[task->data[0]]( task ) )
		;
}

static bool8 gEscalator2_Task0( struct Task* task )
{
	struct EventObject* eventObject;
	s16 x;
	s16 y;
	...
	else if( behavior == MB_UP2_ESCALATOR )
	{
		behavior = 2;
	}
	else if( behavior == MB_DOWN2_ESCALATOR )
	{
		behavior      = 3;
		task->data[0] = 3;
	}
	...
	EventObjectSetHeldMovement( eventObject,
		GetFaceDirectionMovementAction(
			behavior < 2 ? DIR_EAST : DIR_WEST ) );
	task->data[3] = behavior;
	InitEscalatorAnimTask( behavior );
	return TRUE;
}

static bool8 gEscalator2_TaskWarpUp( struct Task* task )
{
	struct Sprite* sprite;
	sprite = &gSprites[gPlayerAvatar.spriteId];
	if( task->data[3] < 2 )
	...
	{
		sprite->pos2.x = Cos( 0x94, task->data[1] ) * -1;
		sprite->pos2.y = Sin( 0x84, task->data[1] ) * -1;
	}
	task->data[0]++;
	return FALSE;
}

static bool8 gEscalator2_TaskWarpUp2( struct Task* task )
{
	struct Sprite* sprite;
	sprite = &gSprites[gPlayerAvatar.spriteId];
	if( task->data[3] < 2 )
...
static bool8 gEscalator2_TaskWarpDown( struct Task* task )
{
	struct Sprite* sprite;
	sprite = &gSprites[gPlayerAvatar.spriteId];
	if( task->data[3] < 2 )
	...
	{
		sprite->pos2.x = Cos( 0x7C, task->data[1] ) * -1;
		sprite->pos2.y = Sin( 0x76, task->data[1] ) * -1;
	}
	task->data[0]++;
	return FALSE;
}

static bool8 gEscalator2_TaskWarpDown2( struct Task* task )
{
	struct Sprite* sprite;
	sprite = &gSprites[gPlayerAvatar.spriteId];
	if( task->data[3] < 2 )
	...
	if( EventObjectClearHeldMovementIfFinished( eventObject ) )
	{
		...
		EventObjectSetHeldMovement( eventObject,
			GetWalkNormalMovementAction(
				task->data[3] < 2 ? DIR_EAST : DIR_WEST ) );
		DestroyTask( FindTaskIdByFunc( escalator2AnimTask ) );
	}
	return FALSE;
}
...
{
	while( gUnknown_0855C418[gTasks[taskId].data[0]]( &gTasks[taskId],
...
static bool8 waterfall_1_do_anim_probably(
	struct Task* task, struct EventObject* eventObject )
{
	ScriptContext2_Enable( );
	if( !EventObjectIsMovementOverridden( eventObject ) )
	{
...
static bool8 sub_80B7450( struct Task* task, struct EventObject* eventObject )
{
	EventObjectSetHeldMovement(
		eventObject, GetWalkSlowMovementAction( DIR_NORTH ) );
	task->data[0]++;
	return FALSE;
}
...
void sub_80B75D8( u8 priority ) { CreateTask( sub_80B75F0, priority ); }

static void sub_80B75F0( u8 taskId )
{
	while( gUnknown_0855C438[gTasks[taskId].data[0]]( &gTasks[taskId],
		&gEventObjects[gPlayerAvatar.eventObjectId],
		&gSprites[gPlayerAvatar.spriteId] ) )
		;
}

static bool8 sub_80B764C( struct Task* task,
	struct EventObject* eventObject,
	struct Sprite* sprite )
{
	FreezeEventObjects( );
...
static bool8 sub_80B7684( struct Task* task,
	struct EventObject* eventObject,
	struct Sprite* sprite )
{
	SetCameraPanning( 0, task->data[1] );
...
static bool8 sub_80B76B8( struct Task* task,
	struct EventObject* eventObject,
	struct Sprite* sprite )
{
	sprite->pos2.y           = 0;
...
static bool8 sub_80B7704( struct Task* task,
	struct EventObject* eventObject,
	struct Sprite* sprite )
{
	s16 centerToCornerVecY;
...
static bool8 sub_80B77F8( struct Task* task,
	struct EventObject* eventObject,
	struct Sprite* sprite )
{
	TryFadeOutOldMapMusic( );
...
static bool8 sub_80B7814( struct Task* task,
	struct EventObject* eventObject,
	struct Sprite* sprite )
{
	if( !gPaletteFade.active && BGMusicStopped( ) == TRUE )
...
{
	while( gUnknown_0855C450[gTasks[taskId].data[0]]( &gTasks[taskId],
		&gEventObjects[gPlayerAvatar.eventObjectId],
		&gSprites[gPlayerAvatar.spriteId] ) )
		;
}

static bool8 sub_80B78EC( struct Task* task,
	struct EventObject* eventObject,
	struct Sprite* sprite )
{
	CameraObjectReset2( );
...
static bool8 sub_80B791C( struct Task* task,
	struct EventObject* eventObject,
	struct Sprite* sprite )
{
	if( IsWeatherNotFadingIn( ) )
...
static bool8 sub_80B7968( struct Task* task,
	struct EventObject* eventObject,
	struct Sprite* sprite )
{
	...
	if( sprite->animCmdIndex > 1 )
	{
		...
		PlaySE( SE_W091 );
		EventObjectSetHeldMovement(
			eventObject, GetJumpMovementAction( DIR_EAST ) );
	}
	return FALSE;
}

static bool8 sub_80B79BC( struct Task* task,
	struct EventObject* eventObject,
	struct Sprite* sprite )
{
	if( EventObjectClearHeldMovementIfFinished( eventObject ) )
...
{
	u8 spriteId;
	sub_80930E0( (s16*)&gFieldEffectArguments[0],
		(s16*)&gFieldEffectArguments[1],
		8,
		8 );
	spriteId = CreateSpriteAtEnd( gFieldEffectObjectTemplatePointers[33],
...
void sub_80B7A74( u8 priority ) { CreateTask( sub_80B7A8C, priority ); }

static void sub_80B7A8C( u8 taskId )
{
	while( gUnknown_0855C460[gTasks[taskId].data[0]]( &gTasks[taskId],
		&gEventObjects[gPlayerAvatar.eventObjectId],
		&gSprites[gPlayerAvatar.spriteId] ) )
		;
}

static bool8 sub_80B7AE8( struct Task* task,
	struct EventObject* eventObject,
	struct Sprite* sprite )
{
	FreezeEventObjects( );
...
static bool8 sub_80B7B18( struct Task* task,
	struct EventObject* eventObject,
	struct Sprite* sprite )
{
	if( EventObjectClearHeldMovementIfFinished( eventObject ) )
	{
		if( task->data[1] > 3 )
		{
			...
			gFieldEffectArguments[3] = sprite->oam.priority;
			task->data[1] =
				FieldEffectStart( FLDEFF_POP_OUT_OF_ASH );
			task->data[0]++;
		}
		else
		{
...
static bool8 sub_80B7B94( struct Task* task,
	struct EventObject* eventObject,
	struct Sprite* sprite )
{
...
static bool8 sub_80B7BCC( struct Task* task,
	struct EventObject* eventObject,
	struct Sprite* sprite )
{
...
static bool8 sub_80B7BF4( struct Task* task,
	struct EventObject* eventObject,
	struct Sprite* sprite )
{
	if( !gPaletteFade.active && BGMusicStopped( ) == TRUE )
...
u8 FldEff_PopOutOfAsh( void )
{
	...
	sub_80930E0( (s16*)&gFieldEffectArguments[0],
		...
		8,
		8 );
	spriteId = CreateSpriteAtEnd( gFieldEffectObjectTemplatePointers[32],
		gFieldEffectArguments[0],
		gFieldEffectArguments[1],
		gFieldEffectArguments[2] );
	gSprites[spriteId].oam.priority       = gFieldEffectArguments[3];
...
static void EscapeRopeFieldEffect_Step1( struct Task* task )
{
	...
	if( !EventObjectIsMovementOverridden( eventObject ) ||
		EventObjectClearHeldMovementIfFinished( eventObject ) )
	{
		if( task->data[14] == 0 && !gPaletteFade.active &&
			BGMusicStopped( ) == TRUE )
		{
			...
			SetMainCallback2( CB2_LoadMap );
			DestroyTask(
				FindTaskIdByFunc( DoEscapeRopeFieldEffect ) );
		}
		else if( task->data[1] == 0 || ( --task->data[1] ) == 0 )
		{
			EventObjectSetHeldMovement( eventObject,
				GetFaceDirectionMovementAction( spinDirections
						[eventObject->facingDirection] ) );
			if( task->data[2] < 12 )
			{
				task->data[2]++;
...
static void sub_80B7EE8( struct Task* task )
{
	...
	{
		if( EventObjectIsMovementOverridden( eventObject ) &&
			!EventObjectClearHeldMovementIfFinished(
				eventObject ) )
		{
			return;
		}
...
static void TeleportFieldEffectTask2( struct Task* task )
{
	...
	if( task->data[1] == 0 || ( --task->data[1] ) == 0 )
	{
		EventObjectTurn( eventObject,
			spinDirections[eventObject->facingDirection] );
		task->data[1] = 8;
		task->data[2]++;
	}
	if( task->data[2] > 7 &&
		task->data[15] == eventObject->facingDirection )
	{
		task->data[0]++;
		task->data[1] = 4;
		task->data[2] = 8;
...
static void TeleportFieldEffectTask3( struct Task* task )
{
	...
	if( ( --task->data[2] ) <= 0 &&
		( task->data[2] = 4, task->data[3] < 8 ) )
	{
		task->data[3] <<= 1;
	}
	if( task->data[4] > 8 &&
		( sprite->oam.priority = 1, sprite->subspriteMode != 0 ) )
	{
		sprite->subspriteMode = 2;
	}
	if( task->data[4] >= 0xa8 )
...
void ( *const gUnknown_0855C49C[] )( struct Task* ) = {
	sub_80B8280, sub_80B830C, sub_80B8410};
...
static void sub_80B830C( struct Task* task )
{
	...
	if( sprite->pos2.y >= -0x30 && task->data[1] > 1 &&
		!( sprite->pos2.y & 1 ) )
	{
		task->data[1]--;
	}
...
static void sub_80B8410( struct Task* task )
{
	...
	if( ( --task->data[1] ) == 0 )
	{
		...
		task->data[1] = 8;
		if( ( ++task->data[2] ) > 4 &&
			task->data[14] == eventObject->facingDirection )
		{
			ScriptContext2_Disable( );
			CameraObjectReset1( );
			UnfreezeEventObjects( );
...
bool8 FldEff_FieldMoveShowMon( void )
{
	...
	gTasks[taskId].data[15] = sub_80B8C60( gFieldEffectArguments[0],
		gFieldEffectArguments[1],
		gFieldEffectArguments[2] );
	return FALSE;
}
...
static void overworld_bg_setup_2( struct Task* task )
{
	...
	FreeResourcesAndDestroySprite(
		&gSprites[task->data[15]], task->data[15] );
...
static void sub_80B88E4( struct Task* task )
{
	...
	StoreWordInTwoHalfwords(
		(u16*)&task->data[13], (u32)gMain.vblankCallback );
...
static void sub_80B8920( struct Task* task )
{
	...
	task->data[12] = delta;
	CpuCopy16( gDarknessFieldMoveStreaksTiles,
		(void*)( VRAM + offset ),
		0x80 );
	CpuFill32( 0, (void*)( VRAM + delta ), 0x800 );
...
static bool8 sub_80B8B38( struct Task* task )
{
	...
	if( dstOffs >= task->data[4] )
	{
		...
		{
			dest[dstOffs + i * 32] =
				gDarknessFieldMoveStreaksTilemap[srcOffs +
					i * 32];
...
static void sub_80B8DB4( struct Task* task )
{
	...
	MoveCoords(
		gEventObjects[gPlayerAvatar.eventObjectId].movementDirection,
		&task->data[1],
		&task->data[2] );
	task->data[0]++;
}
...
static void sub_80B8EA8( struct Task* task )
{
	...
	if( !FieldEffectActiveListContains( FLDEFF_FIELD_MOVE_SHOW_MON ) )
	{
		...
		EventObjectSetGraphicsId(
			eventObject, GetPlayerAvatarGraphicsIdByStateId( 3 ) );
		EventObjectClearHeldMovementIfFinished( eventObject );
		EventObjectSetHeldMovement( eventObject,
		...
		eventObject->fieldEffectSpriteId =
			FieldEffectStart( FLDEFF_SURF_BLOB );
		task->data[0]++;
	}
}
...
u8 sub_80B8F98( void )
{
	...
	u8 spriteId = CreateSprite(
		gFieldEffectObjectTemplatePointers[36], 0x78, -0x18, 1 );
	...
	for( k = 0; k < 90; k++ )
	{
		for( i = 0; i < 8; i++ )
		{
			*(u16*)( BG_CHAR_ADDR( 2 ) + ( k + 1 ) * 32 + i * 4 ) =
				( gUnknown_0855B630[k * 32 + i * 4 + 1]
					<< 8 ) +
				gUnknown_0855B630[k * 32 + i * 4];
			*(u16*)( BG_CHAR_ADDR( 2 ) + ( k + 1 ) * 32 + i * 4 +
				2 ) = ( gUnknown_0855B630[k * 32 + i * 4 + 3]
					      << 8 ) +
				gUnknown_0855B630[k * 32 + i * 4 + 2];
		}
	}
	return spriteId;
}
...
u8 FldEff_NPCFlyOut( void )
{
	...
	spriteId = CreateSprite(
		gFieldEffectObjectTemplatePointers[26], 0x78, 0, 1 );
	sprite = &gSprites[spriteId];
...
static void sub_80B9390( struct Task* task )
{
	if( ( ++task->data[2] ) >= 8 )
	{
		...
		EventObjectSetGraphicsId( eventObject,
			GetPlayerAvatarGraphicsIdByStateId( 0x03 ) );
		StartSpriteAnim( &gSprites[eventObject->spriteId], 0x16 );
		eventObject->inanimate = 1;
		EventObjectSetHeldMovement(
			eventObject, MOVEMENT_ACTION_JUMP_IN_PLACE_LEFT );
		if( task->data[15] & 0x08 )
		{
			DestroySprite(
				&gSprites[eventObject->fieldEffectSpriteId] );
		}
		task->data[0]++;
		task->data[2] = 0;
	}
...
static u8 sub_80B94C4( void )
{
	...
	LoadFieldEffectPalette( 26 );
	spriteId = CreateSprite(
		gFieldEffectObjectTemplatePointers[26], 0xff, 0xb4, 0x1 );
	sprite               = &gSprites[spriteId];
	sprite->oam.priority = 1;
...
static u8 sub_80B9508( u8 spriteId ) { return gSprites[spriteId].data[7]; }
...
static void sub_80B9524( u8 spriteId )
{
	...
	memset( &sprite->data[0],
		0,
		8 * sizeof( u16 ) /* zero all data cells */ );
	sprite->data[6] = 0x40;
}

static void sub_80B9560( u8 a0, u8 a1 ) { gSprites[a0].data[6] = a1; }
...
const union AffineAnimCmd* const gSpriteAffineAnimTable_0855C548[] = {
	SpriteAffineAnim_855C518, SpriteAffineAnim_855C530};
...
static void sub_80B9804( struct Task* task )
{
		...
		EventObjectClearHeldMovementIfFinished( eventObject ) )
	{
		...
		EventObjectSetGraphicsId( eventObject,
			GetPlayerAvatarGraphicsIdByStateId( 0x3 ) );
		CameraObjectReset2( );
...
static void sub_80B9978( struct Task* task )
{
	...
	if( sub_80B9508( task->data[1] ) )
	{
		...
		sprite      = &gSprites[eventObject->spriteId];
		eventObject->inanimate = 0;
		MoveEventObjectToMapCoords( eventObject,
			eventObject->currentCoords.x,
			eventObject->currentCoords.y );
		sprite->pos2.x             = 0;
		sprite->pos2.y             = 0;
...
{
	if( EventObjectClearHeldMovementIfFinished(
		    &gEventObjects[gPlayerAvatar.eventObjectId] ) )
	{
		task->data[0]++;
...
bool8 sub_80B9ADC( void )
{
	...
	if( !TryGetEventObjectIdByLocalIdAndMap( gFieldEffectArguments[0],
		    gFieldEffectArguments[1],
		    gFieldEffectArguments[2],
		    &eventObjectIdBuffer ) )
	{
...
void sub_80B9BD0( u8 taskId ) { gTasks[taskId].data[7] = 1; }
...
void sub_80B9C54( s16* data, u8 taskId )
{
	...
	{
		struct Sprite* sprite =
			&gSprites[gEventObjects[data[2]].spriteId];
...
const struct SpriteTemplate gUnknown_0855C5EC = {.tileTag = 0xFFFF,
	.paletteTag                                       = 4378,
	.oam                                              = &gOamData_855C218,
...
{
	int i;
	int xPos = (s16)gTotalCameraPixelOffsetX + sprite->pos1.x +
		sprite->pos2.x;
	int yPos = (s16)gTotalCameraPixelOffsetY + sprite->pos1.y +
		sprite->pos2.y - 4;

	for( i = 0; i < 4; i++ )
	{
		u8 spriteId =
			CreateSprite( &gUnknown_0855C5EC, xPos, yPos, 0 );
...
static void sub_80B9DB8( struct Sprite* sprite )
{
	...
	if( ( u16 )( sprite->pos1.x + 4 ) > 0xF8 || sprite->pos1.y < -4 ||
		sprite->pos1.y > 0xA4 )
		DestroySprite( sprite );
}
...
bool8 Fldeff_MoveDeoxysRock( struct Sprite* sprite )
{
		    ...
		    &eventObjectIdBuffer ) )
	{
		...
		ShiftEventObjectCoords( object,
			gFieldEffectArguments[3] + 7,
			gFieldEffectArguments[4] + 7 );
		taskId = CreateTask( Fldeff_MoveDeoxysRock_Step, 0x50 );
		gTasks[taskId].data[1] = object->spriteId;
		gTasks[taskId].data[2] =
			gSprites[object->spriteId].pos1.x + xPos;
		gTasks[taskId].data[3] =
			gSprites[object->spriteId].pos1.y + yPos;
