...
struct PokedexView
{
	...
	s16 menuY; // Menu Y position (inverted because we use REG_BG0VOFS for
		   // this)
	u8 unk654[8];
	u8 unk65C[8];
};
...
static const struct OamData sOamData_855CFE4 = {.y = 160,
	.affineMode                                = 0,
	.objMode                                   = 0,
	.mosaic                                    = 0,
	.bpp                                       = 0,
...
static const struct OamData sOamData_855CFEC = {.y = 160,
	.affineMode                                = 0,
	.objMode                                   = 0,
	.mosaic                                    = 0,
	.bpp                                       = 0,
...
static const struct OamData sOamData_855CFF4 = {.y = 160,
	.affineMode                                = 0,
	.objMode                                   = 0,
	.mosaic                                    = 0,
	.bpp                                       = 0,
...
static const struct OamData sOamData_855CFFC = {.y = 160,
	.affineMode                                = 0,
	.objMode                                   = 2,
	.mosaic                                    = 0,
	.bpp                                       = 0,
...
static const struct OamData sOamData_855D004 = {.y = 160,
	.affineMode                                = 0,
	.objMode                                   = 0,
	.mosaic                                    = 0,
	.bpp                                       = 0,
...
static const struct OamData sOamData_855D00C = {.y = 160,
	.affineMode                                = 0,
	.objMode                                   = 0,
	.mosaic                                    = 0,
	.bpp                                       = 0,
...
static const union AnimCmd sSpriteAnim_855D014[] = {
	ANIMCMD_FRAME( 3, 30 ), ANIMCMD_END};
...
static const union AnimCmd sSpriteAnim_855D02C[] = {
	ANIMCMD_FRAME( 48, 30 ), ANIMCMD_END};

static const union AnimCmd sSpriteAnim_855D034[] = {
	ANIMCMD_FRAME( 40, 30 ), ANIMCMD_END};
...
static const union AnimCmd* const sSpriteAnimTable_855D130[] = {
	sSpriteAnim_855D04C, sSpriteAnim_855D054};

static const union AnimCmd* const sSpriteAnimTable_855D138[] = {
	sSpriteAnim_855D05C, sSpriteAnim_855D064};
...
static const u8 gUnknown_0855D2BE[] =
...
static const u16 gUnknown_0856E610[] =
	INCBIN_U16( "graphics/pokedex/black.gbapal" );
...
void sub_80BB7D4( u8 taskId )
{
	...
	else
	{
		if( ( gMain.newKeys & A_BUTTON ) &&
			sPokedexView
				->pokedexList[sPokedexView->selectedPokemon]
				.seen )
		{
			sub_80BD8D0( );
			BeginNormalPaletteFade(
				~( 1 << ( gSprites[sPokedexView
							   ->selectedMonSpriteId]
						   .oam.paletteNum +
					   16 ) ),
				0,
		...
		{
			// Handle D-pad
			sPokedexView->selectedPokemon = sub_80BD69C(
...
{
	if( sub_80BD404( sPokedexView->unk62F,
		    sPokedexView->unk634,
		    sPokedexView->unk636 ) )
		gTasks[taskId].func = sub_80BB7D4;
}
	...
	else
	{
		if( gMain.newKeys & A_BUTTON )
		{
			...
			case 2: // LIST BOTTOM
				...
				sPokedexView->unk62C =
					sPokedexView->pokemonListCount * 16 +
					0x30;
				sub_80BDA40( );
				CreateInitialPokemonSprites(
					sPokedexView->selectedPokemon, 0xE );
...
void sub_80BBBF4( u8 taskId )
{
		...
		gSprites[sPokedexView->selectedMonSpriteId].pos1.y == 56 )
	{
		...
		gTasks[taskId].data[0] = sub_80BE91C(
			&sPokedexView
				 ->pokedexList[sPokedexView->selectedPokemon],
...
void sub_80BBC74( u8 taskId )
{
	...
	{
		if( sPokedexView->unk64A == 1 &&
			!sub_80BE9C4( gTasks[taskId].data[0] ) &&
			sub_80BD930( ) )
			sub_80BE9F8( &sPokedexView->pokedexList
					      [sPokedexView->selectedPokemon],
...
bool8 sub_80BC514( u8 a )
{
	...
	case 3:
		if( a == 0 )
			sub_80BC8D4( sPokedexView->dexMode,
				sPokedexView->dexOrder );
		CreateInitialPokemonSprites(
			sPokedexView->selectedPokemon, 0xE );
		sPokedexView->menuIsOpen = 0;
		sPokedexView->menuY      = 0;
...
{
	u16 vars[3]; // I have no idea why three regular variables are stored
		     // in an array, but whatever.
...
#define temp_dexNum vars[2]
	...
	case 0:
		if( temp_isHoennDex )
		{
			for( i = 0; i < temp_dexCount; i++ )
			{
				...
				sPokedexView->pokedexList[i].dexNum =
					temp_dexNum;
				sPokedexView->pokedexList[i].seen =
					GetSetPokedexFlag(
						temp_dexNum, FLAG_GET_SEEN );
				sPokedexView->pokedexList[i].owned =
					GetSetPokedexFlag(
						temp_dexNum, FLAG_GET_CAUGHT );
		...
		else
		{
			...
			for( i = 0; i < temp_dexCount; i++ )
			{
				...
				if( r10 )
				{
					asm( "" ); // Needed to match for some
						   // reason
					sPokedexView->pokedexList[r5].dexNum =
						temp_dexNum;
					sPokedexView->pokedexList[r5].seen =
						GetSetPokedexFlag( temp_dexNum,
							FLAG_GET_SEEN );
					sPokedexView->pokedexList[r5].owned =
						GetSetPokedexFlag( temp_dexNum,
							FLAG_GET_CAUGHT );
					if( sPokedexView->pokedexList[r5]
							.seen )
						sPokedexView
							->pokemonListCount =
							r5 + 1;
	...
	case 1:
		for( i = 0; i < POKEMON_SLOTS_NUMBER - 1; i++ )
		{
			...
			if( NationalToHoennOrder( temp_dexNum ) <=
					temp_dexCount &&
				GetSetPokedexFlag(
					temp_dexNum, FLAG_GET_SEEN ) )
			{
...
static void PrintMonDexNumAndName(
	u8 windowId, u8 fontId, const u8* str, u8 left, u8 top )
{
	u8 color[3];
...
static void CreateMonListEntry( u8 direction, u16 b, u16 c )
{
	...
	default:
		...
		for( i = 0; i <= 10; i++ )
		{
			if( _b < 0 || _b >= NATIONAL_DEX_COUNT ||
				sPokedexView->pokedexList[_b].dexNum ==
					0xFFFF )
			{
	...
	case 1: // Up
		_b = b - 5;
		if( _b < 0 || _b >= NATIONAL_DEX_COUNT ||
			sPokedexView->pokedexList[_b].dexNum == 0xFFFF )
		{
		...
		else
		{
			...
			if( sPokedexView->pokedexList[_b].seen )
			{
				CreateMonDexNum( _b,
					0x12,
					sPokedexView->unk630 * 2,
					c );
				CreateCaughtBall(
					sPokedexView->pokedexList[_b].owned,
					0x11,
					sPokedexView->unk630 * 2,
					c );
				CreateMonName(
			...
			else
			{
				CreateMonDexNum( _b,
					...
					c );
				CreateCaughtBall(
					0, 0x11, sPokedexView->unk630 * 2, c );
				CreateMonName(
					0, 0x16, sPokedexView->unk630 * 2 );
			}
		}
		break;
	case 2: // Down
...
static void CreateCaughtBall( u16 a, u8 x, u8 y, u16 unused )
{
	if( a )
		BlitBitmapToWindow(
			0, gUnknown_0855D2BE, x * 8, y * 8, 8, 16 );
	else
...
u16 sub_80BD69C( u16 selectedMon, u16 b )
{
	...
	sub_80BD404( sPokedexView->unk62F,
		sPokedexView->unk634,
		sPokedexView->unk636 );
...
void sub_80BD8D0( void )
{
	...
	for( i = 0; i < 4; i++ )
	{
		...
		if( gSprites[spriteId].pos2.x == 0 &&
			gSprites[spriteId].pos2.y == 0 && spriteId != 0xFFFF )
...
u8 sub_80BD930( void )
{
	...
	if( ( gMain.newKeys & DPAD_UP ) && r4 )
	{
		...
		{
			r2 = sub_80C0E0C(
				1, r2, 0, sPokedexView->pokemonListCount - 1 );
		...
		r4 < sPokedexView->pokemonListCount - 1 )
	{
		r2 = r4;
		while( r2 < sPokedexView->pokemonListCount - 1 )
		{
			r2 = sub_80C0E0C(
				0, r2, 0, sPokedexView->pokemonListCount - 1 );
...
u32 sub_80BDACC( u16 num, s16 x, s16 y )
{
	...
	for( i = 0; i < 4; i++ )
	{
		if( sPokedexView->unk61E[i] == 0xFFFF )
		{
			u8 spriteId = CreateMonSpriteFromNationalDexNumber(
				num, x, y, i );
			...
			gSprites[spriteId].data[2] =
				NationalPokedexNumToSpecies( num );
	...
	if( a == 0 )
	{
		...
		if( !IsNationalPokedexEnabled( ) )
		{
			...
			spriteId = CreateSprite(
				&sSeenOwnTextSpriteTemplate, 32, 72, 1 );
			StartSpriteAnim( &gSprites[spriteId], 1 );
			_a = 0;
			// Seen value - 100s
			spriteId =
				CreateSprite( &gUnknown_0855D224, 24, 48, 1 );
			...
			// Seen value - 10s
			spriteId =
				CreateSprite( &gUnknown_0855D224, 32, 48, 1 );
			...
			// Seen value - 1s
			spriteId =
				CreateSprite( &gUnknown_0855D224, 40, 48, 1 );
			r5 = ( sPokedexView->seenCount % 100 ) % 10;
			StartSpriteAnim( &gSprites[spriteId], r5 );
			_a = 0;
			// Owned value - 100s
			spriteId =
				CreateSprite( &gUnknown_0855D224, 24, 80, 1 );
			...
			// Owned value - 10s
			spriteId =
				CreateSprite( &gUnknown_0855D224, 32, 80, 1 );
			...
			// Owned value -1s
			spriteId =
				CreateSprite( &gUnknown_0855D224, 40, 80, 1 );
			r5 = ( sPokedexView->ownCount % 100 ) % 10;
			StartSpriteAnim( &gSprites[spriteId], r5 );
		}
		else
		{
			...
			spriteId = CreateSprite(
				&sSeenOwnTextSpriteTemplate, 32, 76, 1 );
			StartSpriteAnim( &gSprites[spriteId], 1 );

			CreateSprite( &gUnknown_0855D20C, 17, 45, 1 );

			spriteId =
				CreateSprite( &gUnknown_0855D20C, 17, 55, 1 );
			StartSpriteAnim( &gSprites[spriteId], 1 );

			CreateSprite( &gUnknown_0855D20C, 17, 81, 1 );

			spriteId =
				CreateSprite( &gUnknown_0855D20C, 17, 91, 1 );
			StartSpriteAnim( &gSprites[spriteId], 1 );
			...
			spriteId =
				CreateSprite( &gUnknown_0855D23C, 40, 45, 1 );
			r5 = r6 / 100;
			StartSpriteAnim( &gSprites[spriteId], r5 );
			if( r5 != 0 )
				_a = 1;
			...
			spriteId =
				CreateSprite( &gUnknown_0855D23C, 48, 45, 1 );
			r5 = ( r6 % 100 ) / 10;
			if( r5 != 0 || _a != 0 )
				StartSpriteAnim( &gSprites[spriteId], r5 );
			else
			...
			spriteId =
				CreateSprite( &gUnknown_0855D23C, 56, 45, 1 );
			r5 = ( r6 % 100 ) % 10;
			StartSpriteAnim( &gSprites[spriteId], r5 );
			...
			spriteId =
				CreateSprite( &gUnknown_0855D23C, 40, 55, 1 );
			r5 = sPokedexView->seenCount / 100;
			StartSpriteAnim( &gSprites[spriteId], r5 );
			if( r5 != 0 )
				_a = 1;
			...
			spriteId =
				CreateSprite( &gUnknown_0855D23C, 48, 55, 1 );
			r5 = ( sPokedexView->seenCount % 100 ) / 10;
			if( r5 != 0 || _a != 0 )
				StartSpriteAnim( &gSprites[spriteId], r5 );
			else
			...
			spriteId =
				CreateSprite( &gUnknown_0855D23C, 56, 55, 1 );
			r5 = ( sPokedexView->seenCount % 100 ) % 10;
			StartSpriteAnim( &gSprites[spriteId], r5 );
			...
			spriteId =
				CreateSprite( &gUnknown_0855D23C, 40, 81, 1 );
			r5 = r6 / 100;
			StartSpriteAnim( &gSprites[spriteId], r5 );
			if( r5 != 0 )
				_a = 1;
			...
			spriteId =
				CreateSprite( &gUnknown_0855D23C, 48, 81, 1 );
			r5 = ( r6 % 100 ) / 10;
			if( r5 != 0 || _a != 0 )
				StartSpriteAnim( &gSprites[spriteId], r5 );
			else
			...
			spriteId =
				CreateSprite( &gUnknown_0855D23C, 56, 81, 1 );
			r5 = ( r6 % 100 ) % 10;
			StartSpriteAnim( &gSprites[spriteId], r5 );
			...
			spriteId =
				CreateSprite( &gUnknown_0855D23C, 40, 91, 1 );
			r5 = sPokedexView->ownCount / 100;
			StartSpriteAnim( &gSprites[spriteId], r5 );
			if( r5 != 0 )
				_a = 1;
			...
			spriteId =
				CreateSprite( &gUnknown_0855D23C, 48, 91, 1 );
			r5 = ( sPokedexView->ownCount % 100 ) / 10;
			if( r5 != 0 || _a != 0 )
				StartSpriteAnim( &gSprites[spriteId], r5 );
			else
			...
			spriteId =
				CreateSprite( &gUnknown_0855D23C, 56, 91, 1 );
			r5 = ( sPokedexView->ownCount % 100 ) % 10;
			StartSpriteAnim( &gSprites[spriteId], r5 );
		}
...
static void SpriteCB_Scrollbar( struct Sprite* sprite )
{
	if( sPokedexView->unk64A != 0 && sPokedexView->unk64A != 3 )
		...
		sprite->pos2.y = sPokedexView->selectedPokemon * 120 /
			( sPokedexView->pokemonListCount - 1 );
}
...
void sub_80BE658( struct Sprite* sprite )
{
	...
	else
	{
		...
		if( sprite->data[1] != 0 )
		{
			if( sPokedexView->selectedPokemon ==
				sPokedexView->pokemonListCount - 1 )
				sprite->invisible = TRUE;
			else
				sprite->invisible = FALSE;
		...
		if( sPokedexView->menuIsOpen == 0 &&
			sPokedexView->menuY == 0 && sprite->invisible == 0 )
			sprite->invisible = FALSE;
		else
			sprite->invisible = TRUE;
	}
...
void sub_80BEA24( u8 taskId )
{
	...
	case 1:
		DecompressAndLoadBgGfxUsingHeap(
			3, gPokedexMenu_Gfx, 0x2000, 0, 0 );
		CopyToBgTilemapBuffer(
			3, gPokedexTilemap_DescriptionScreen, 0, 0 );
	...
	case 3:
		...
		break;
	case 4:
		sub_80C020C( sPokedexListItem->dexNum,
			sPokedexView->dexMode == 0 ? 0 : 1,
	...
	case 6: {
		...
		if( gTasks[taskId].data[2] != 0 )
			r3 = 0x14;
		if( gTasks[taskId].data[1] != 0 )
			r3 |= ( 1 << ( gSprites[gTasks[taskId].data[4]]
						.oam.paletteNum +
					16 ) );
	...
	case 8:
		if( !gPaletteFade.active )
		{
			...
			if( gTasks[taskId].data[3] == 0 )
			{
				...
				PlayCry2( NationalPokedexNumToSpecies(
...
void sub_80BF0AC( u8 taskId )
{
	...
	case 1:
		...
		break;
	case 2:
		ShowPokedexAreaScreen( NationalPokedexNumToSpecies(
					       sPokedexListItem->dexNum ),
			&sPokedexView->unk64E );
		SetVBlankCallback( gUnknown_030060B4 );
		sPokedexView->unk64E = 0;
...
void sub_80BF5CC( u8 taskId )
{
	...
	{
		sub_80BF7FC( 1 );
		sub_8145534( NationalPokedexNumToSpecies(
			sPokedexListItem->dexNum ) );
		return;
	}
...
void sub_80BF82C( u8 taskId )
{
	...
	case 3: {
		...
		sub_80BE8DC( string,
			GetStringCenterAlignXOffset( 1, string, 0xF0 ),
			0x79 );
		gMain.state++;
	}
	break;
	case 4:
		...
		break;
	case 5:
		spriteId = sub_80C0EF8( PlayerGenderToFrontTrainerPicId(
		...
		gSprites[spriteId].pos2.y =
			gPokedexEntries[sPokedexListItem->dexNum]
				.trainerOffset;
		SetOamMatrix( 1,
	...
	case 6:
		spriteId = CreateMonSpriteFromNationalDexNumber(
			sPokedexListItem->dexNum, 88, 56, 1 );
		gSprites[spriteId].oam.affineMode = 1;
...
void sub_80BFD0C( u8 a, u16 unused )
{
	...
	for( i = 0; i < 4; i++ )
	{
		...
		for( j = 0; j < 7; j++ )
		{
			...
			ptr[row + j + 0x20] =
				( ptr[row + j + 0x20] % 0x1000 ) | newPalette;
		}
	}
	CopyBgTilemapBufferToVram( 1 );
}
#else
__attribute__( ( naked ) ) void sub_80BFD0C( u8 a, u16 unused )
{
	asm( ".syntax unified\n\
    push {r4-r7,lr}\n\
    mov r7, r8\n\
...
#else
__attribute__( ( naked ) ) void sub_80BFD7C( u8 a, u16 b )
{
	asm( ".syntax unified\n\
    push {r4-r7,lr}\n\
    mov r7, r8\n\
...
void blockset_load_palette_to_gpu( u8 taskId )
{
	if( !gPaletteFade.active )
	{
		...
		species =
			NationalPokedexNumToSpecies( gTasks[taskId].data[1] );
		otId = ( (u16)gTasks[taskId].data[13] << 16 ) |
			(u16)gTasks[taskId].data[12];
		personality = ( (u16)gTasks[taskId].data[15] << 16 ) |
			(u16)gTasks[taskId].data[14];
		paletteNum = gSprites[gTasks[taskId].data[3]].oam.paletteNum;
		lzPaletteData = GetFrontSpritePalFromSpeciesAndPersonality(
			species, otId, personality );
		LoadCompressedPalette(
			lzPaletteData, 0x100 | paletteNum * 16, 32 );
		DestroyTask( taskId );
	}
}
...
void sub_80C020C( u32 num, u32 value, u32 c, u32 d )
{
	...
	if( d )
		sub_80BE8DC( gText_PokedexRegistration,
			GetStringCenterAlignXOffset(
				1, gText_PokedexRegistration, 0xF0 ),
			0 );
	if( value == 0 )
	...
	ConvertIntToDecimalStringN(
		StringCopy( str, gText_UnkCtrlF908Clear01 ), value, 2, 3 );
	...
	sub_80BE8DC(
		text3, GetStringCenterAlignXOffset( 1, text3, 0xF0 ), 0x5F );
}
...
#else
__attribute__( ( naked ) ) void sub_80C0460( u16 weight, u8 left, u8 top )
{
	asm( ".syntax unified\n\
    push {r4-r7,lr}\n\
    mov r7, r10\n\
...
s8 GetSetPokedexFlag( u16 nationalDexNo, u8 caseID )
{
	...
	case FLAG_GET_SEEN:
		...
		{
			if( ( gSaveBlock2Ptr->pokedex.seen[index] & mask ) ==
					( gSaveBlock1Ptr->seen1[index] &
						mask ) &&
				( gSaveBlock2Ptr->pokedex.seen[index] &
					mask ) ==
					( gSaveBlock1Ptr->seen2[index] &
						mask ) )
				retVal = 1;
			else
			{
	...
	case FLAG_GET_CAUGHT:
		if( gSaveBlock2Ptr->pokedex.owned[index] & mask )
		{
			if( ( gSaveBlock2Ptr->pokedex.owned[index] & mask ) ==
					( gSaveBlock2Ptr->pokedex.seen[index] &
						mask ) &&
				( gSaveBlock2Ptr->pokedex.owned[index] &
					mask ) ==
					( gSaveBlock1Ptr->seen1[index] &
						mask ) &&
				( gSaveBlock2Ptr->pokedex.owned[index] &
...
u16 GetHoennPokedexCount( u8 caseID )
{
	...
	for( i = 0; i < HOENN_DEX_COUNT; i++ )
	{
		...
		{
		case FLAG_GET_SEEN:
			if( GetSetPokedexFlag( HoennToNationalOrder( i + 1 ),
				    FLAG_GET_SEEN ) )
				count++;
			break;
		case FLAG_GET_CAUGHT:
...
bool16 HasAllHoennMons( void )
{
	...
	{
		if( !GetSetPokedexFlag(
			    HoennToNationalOrder( i + 1 ), FLAG_GET_CAUGHT ) )
			return FALSE;
	}
	return TRUE;
}
...
void sub_80C0A88( u8 windowId, const u8* str, u8 left, u8 top )
{
	...
	AddTextPrinterParameterized4(
		windowId, 1, left, top, 0, 0, color, -1, str );
}
...
u16 CreateMonSpriteFromNationalDexNumber(
	u16 nationalNum, s16 x, s16 y, u16 paletteSlot )
{
	nationalNum = NationalPokedexNumToSpecies( nationalNum );
	return CreateMonPicSprite_HandleDeoxys( nationalNum,
		8,
		sub_80C0E68( nationalNum ),
		TRUE,
		x,
		y,
		paletteSlot,
...
{
	return CreateTrainerPicSprite(
		species, TRUE, x, y, paletteSlot, 0xFFFF );
}

int sub_80C0F30( u8 dexMode,
	u8 sortMode,
	u8 abcGroup,
	u8 bodyColor,
	u8 type1,
	u8 type2 )
{
	...
	if( abcGroup != 0xFF )
	{
			...
			i++ )
		{
			...
			if( ( r3 >= gUnknown_0856ED08[abcGroup][0] &&
				    r3 < gUnknown_0856ED08[abcGroup][0] +
						    gUnknown_0856ED08[abcGroup]
								     [1] ) ||
				( r3 >= gUnknown_0856ED08[abcGroup][2] &&
					r3 < gUnknown_0856ED08[abcGroup][2] +
							gUnknown_0856ED08
								[abcGroup]
								[3] ) )
	...
	{
		for( i = sPokedexView->pokemonListCount;
...
u8 sub_80C1258( void ) { return CreateTask( sub_80C12E0, 0 ); }
...
void sub_80C12E0( u8 taskId )
{
	...
	case 0:
		if( !gPaletteFade.active )
		{
			...
			PutWindowTilemap( 0 );
			DecompressAndLoadBgGfxUsingHeap(
				3, gPokedexSearchMenu_Gfx, 0x2000, 0, 0 );

			if( !IsNationalPokedexEnabled( ) )
				CopyToBgTilemapBuffer(
					3, gPokedexSearch2_Tilemap, 0, 0 );
			else
				CopyToBgTilemapBuffer(
					3, gPokedexSearch1_Tilemap, 0, 0 );
...
void sub_80C170C( u8 taskId )
{
	...
	if( ( gMain.newKeys & DPAD_LEFT ) &&
		r6[gTasks[taskId].data[1]][0] != 0xFF )
	{
		PlaySE( SE_SELECT );
	...
	if( ( gMain.newKeys & DPAD_RIGHT ) &&
		r6[gTasks[taskId].data[1]][1] != 0xFF )
	{
		PlaySE( SE_SELECT );
	...
	if( ( gMain.newKeys & DPAD_UP ) &&
		r6[gTasks[taskId].data[1]][2] != 0xFF )
	{
		PlaySE( SE_SELECT );
	...
	if( ( gMain.newKeys & DPAD_DOWN ) &&
		r6[gTasks[taskId].data[1]][3] != 0xFF )
	{
		PlaySE( SE_SELECT );
...
#else
__attribute__( ( naked ) ) void sub_80C1D98( u8 a, u8 b, u8 c, u8 d )
{
	asm( ".syntax unified\n\
    push {r4-r7,lr}\n\
    mov r7, r8\n\
...
void sub_80C1DFC( u8 a, u8 b, u8 c )
{
	...
	case 2:
		sub_80C1D98( r5,
			gUnknown_0856ED30[a].unk4,
			gUnknown_0856ED30[a].unk5,
			gUnknown_0856ED30[a].unk6 );
		break;
	case 3:
	case 4:
	case 7:
	case 8:
		sub_80C1D98( r5,
			gUnknown_0856ED48[a - 3].unk4,
			gUnknown_0856ED48[a - 3].unk5,
			gUnknown_0856ED48[a - 3].unk6 );
		// fall through
	case 5:
	case 6:
		sub_80C1D98( r5,
			gUnknown_0856ED48[a - 3].unk7,
			gUnknown_0856ED48[a - 3].unk8,
			gUnknown_0856ED48[a - 3].unk9 );
		break;
	case 10:
		sub_80C1D98( r5,
			gUnknown_0856ED48[2].unk4,
			gUnknown_0856ED48[2].unk5,
			gUnknown_0856ED48[2].unk6 );
		break;
	case 9:
		if( !IsNationalPokedexEnabled( ) )
			sub_80C1D98( r5,
				gUnknown_0856ED48[a - 3].unk4,
				gUnknown_0856ED48[a - 3].unk5 - 2,
				gUnknown_0856ED48[a - 3].unk6 );
		else
			sub_80C1D98( r5,
				gUnknown_0856ED48[a - 3].unk4,
...
{
	const struct PokedexOption* r6 =
		gUnknown_0856EFC8[gTasks[taskId].data[1]].pokedexList;
	const u16* r8 =
		&gTasks[taskId]
			 .data[gUnknown_0856EFC8[gTasks[taskId].data[1]].unk4];
	const u16* r7 =
		&gTasks[taskId]
			 .data[gUnknown_0856EFC8[gTasks[taskId].data[1]].unk5];
	u16 i;
	u16 j;
...
void sub_80C2638( u32 y ) { sub_80C12B0( 0x90, y * 16 + 8, 8, 16 ); }
...
void sub_80C267C( void ) { sub_80C12B0( 0x90, 8, 0x60, 0x60 ); }
