...
void sub_8158C58( u8 taskId )
{
	...
	panIncrement =
		CalculatePanIncrement( sourcePan, targetPan, panIncrement );
...
static void sub_8158D08( u8 taskId )
{
	if( gTasks[taskId].data[12]++ == gTasks[taskId].data[6] )
	{
		...
		PlaySE12WithPanning(
			gTasks[taskId].data[0], gTasks[taskId].data[11] );
	...
	if( gTasks[taskId].data[10]++ == gTasks[taskId].data[5] )
	{
		...
		gTasks[taskId].data[11] =
			KeepPanInRange( gTasks[taskId].data[11], oldPan );
	}
}
// task end
	...
	if( IsContest( ) )
	{
		if( gBattleAnimArgs[0] == ANIM_ATTACKER )
			...
			DestroyAnimVisualTask(
				taskId ); // UB: function should return upon
					  // destroying task.
	}
	else
	{
