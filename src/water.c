...
// what is this?
const u8 gUnknown_8593C80[] =
	INCBIN_U8( "graphics/unknown/unknown_593C80.4bpp" );
const u8 gUnknown_8593FFC[] =
	INCBIN_U8( "graphics/unknown/unknown_593FFC.bin" );
...
void sub_810721C( struct Sprite* sprite ) { sprite->callback = sub_8107228; }
...
void sub_8107380( struct Sprite* sprite )
{
	...
	sprite->pos2.x +=
		Sin( trigIndex >> 8, gSprites[otherSpriteId].data[0] );
	sprite->pos2.y +=
		Cos( trigIndex >> 8, gSprites[otherSpriteId].data[1] );
	gSprites[otherSpriteId].data[3] =
		trigIndex + gSprites[otherSpriteId].data[2];
	if( --timer != 0 )
	{
...
void sub_810744C( struct Sprite* sprite )
{
	...
	sprite->data[1] = sprite->pos1.x;
	sprite->data[2] =
		GetBattlerSpriteCoord( gBattleAnimTarget, 2 ) + unkArg;
	sprite->data[3] = sprite->pos1.y;
	sprite->data[4] = GetBattlerSpriteCoord( gBattleAnimTarget, 3 ) +
		gBattleAnimArgs[3];
	InitAnimLinearTranslation( sprite );
...
void sub_8107528( u8 taskId )
{
	...
	gTasks[taskId].data[2] =
		IndexOfSpritePaletteTag( ANIM_TAG_RAINBOW_RINGS ) * 16 + 256;
...
void sub_81076F4( u8 taskId )
{
	gBattleAnimArgs[ARG_RET_ID] =
		( gBattleAnimArgs[ARG_RET_ID] + 3 ) & 0xFF;
	if( --gTasks[taskId].data[0] == 0 )
		DestroyAnimVisualTask( taskId );
}
...
void AnimTask_CreateSurfWave( u8 taskId )
{
	...
	if( !IsContest( ) )
	{
		...
		if( GetBattlerSide( gBattleAnimAttacker ) == B_SIDE_OPPONENT )
			AnimLoadCompressedBgTilemap(
				animBg.bgId, gUnknown_08D95E00 );
		else
			AnimLoadCompressedBgTilemap(
				animBg.bgId, gUnknown_08D960D0 );
	}
	else
	{
	...
	AnimLoadCompressedBgGfx(
		animBg.bgId, gBattleAnimBgImage_Surf, animBg.tilesOffset );
	if( gBattleAnimArgs[0] == 0 )
		LoadCompressedPalette(
			gBattleAnimBgPalette_Surf, animBg.paletteId * 16, 32 );
	else
		LoadCompressedPalette(
			gBattleAnimBackgroundImageMuddyWater_Pal,
			animBg.paletteId * 16,
			32 );
	...
	*BGptrY += gTasks[taskId].data[1];
	...
	if( ++gTasks[taskId].data[5] == 4 )
	{
		...
		for( i = 6; i != 0; i-- )
		{
			...
			palOffset = 1 + i;
			gPlttBufferFaded[palNum + palOffset] =
				gPlttBufferFaded[palNum + palOffset - 1];
		}
	...
	{
		gTasks[taskId].data[0] =
			gTasks[gTasks[taskId].data[15]].data[1] & 0x1F;
...
void sub_8108140( u8 taskId )
{
	...
	case 0:
		sub_80A805C(
			task, task->data[15], 0x100, 0x100, 224, 0x200, 32 );
		task->data[0]++;
	case 1:
		if( ++task->data[3] > 1 )
		{
...
void sub_8108408( struct Task* task, u8 taskId )
{
	...
	{
		spriteId = CreateSprite( &gUnknown_08595268,
			attackerCoordX,
			attackerCoordY,
			subpriority );
...
void sub_810851C( struct Sprite* sprite )
{
	...
	case 1:
		...
		if( sprite->pos1.x < -8 || sprite->pos1.x > 248 ||
			sprite->pos1.y < -8 || sprite->pos1.y > 120 )
		{
...
{
	u16 yPosArg =
		( ( gSineTable[task->data[8]] + 3 ) >> 4 ) + task->data[6];
	...
	task->data[7] =
		( ( task->data[7] * 0x41c64e6d + 0x3039 ) % task->data[5] ) +
		task->data[4];
}
...
void sub_81087C0( struct Sprite* sprite )
{
	if( sprite->data[0] == 0 )
	{
		...
		if( sprite->pos1.y >= sprite->data[5] )
		{
			...
			sprite->data[1] = CreateSprite( &gUnknown_08597388,
				sprite->pos1.x,
				sprite->pos1.y,
				1 );
			if( sprite->data[1] != MAX_SPRITES )
			{
				...
				gSprites[sprite->data[1]].data[6] =
					sprite->data[6];
				gSprites[sprite->data[1]].data[7] =
					sprite->data[7];
				gSprites[sprite->data[1]].callback =
					sub_810886C;
			}
			DestroySprite( sprite );
		}
...
void sub_8108AC0( struct Task* task )
{
	...
	if( ++task->data[2] > 1 )
	{
		...
		spriteId      = CreateSprite(
                        &gUnknown_08595268, task->data[3], task->data[4], 10 );
		if( spriteId != MAX_SPRITES )
		{
...
void sub_8108D54( struct Sprite* sprite, int xDiff, int yDiff )
{
	...
	for( i = 0; i <= 0; i++ )
	{
		spriteId                   = CreateSprite( &gUnknown_08595310,
                        combinedX,
                        combinedY + something,
                        130 );
