...
static void ContestAI_DoAIProcessing( void )
{
	while( eContestAI.aiState != CONTESTAI_FINISHED )
	{
		...
		case CONTESTAI_SETTING_UP:
			gAIScriptPtr =
				gContestAIChecks[eContestAI.currentAICheck];

			if( gContestMons[eContestAI.contestantId]
					.moves[eContestAI.nextMoveIndex] == 0 )
				eContestAI.nextMove =
					0; // don't process a move that doesn't
					   // exist.
			else
				eContestAI.nextMove =
					gContestMons[eContestAI.contestantId]
						.moves[eContestAI
								.nextMoveIndex];
			eContestAI.aiState++;
			break;
		case CONTESTAI_PROCESSING:
			...
			{
				sContestAICmdTable
					[*gAIScriptPtr]( ); // run the command.
			}
			else
			{
				eContestAI.unk5[eContestAI.nextMoveIndex] =
					0; // don't
					   // consider
					   // a move
					   // that
...
static void ContestAICmd_score( void )
{
	s16 score = eContestAI.unk5[eContestAI.nextMoveIndex] +
		(s8)gAIScriptPtr[1];
...
static void ContestAICmd_get_user_order( void )
{
	eContestAI.scriptResult =
		eContestResources8.turnOrder[eContestAI.contestantId];
	gAIScriptPtr += 1;
}
...
{
	eContestAI.scriptResult =
		gContestMonConditions[eContestAI.contestantId];
	gAIScriptPtr += 1;
}
...
static void ContestAICmd_get_move_effect( void )
{
	u16 move = gContestMons[eContestAI.contestantId]
			   .moves[eContestAI.nextMoveIndex];
...
static void ContestAICmd_check_most_appealing_move( void )
{
	...
	for( i = 0; i < MAX_MON_MOVES; i++ )
	{
		...
		if( newMove != 0 &&
			appeal < gContestEffects[gContestMoves[newMove].effect]
					 .appeal )
			break;
	}
...
static void ContestAICmd_unk_2F( void )
{
	...
	for( i = 0; i < MAX_MON_MOVES; i++ )
	{
		u16 newMove = gContestMons[eContestAI.contestantId].moves[i];
		if( newMove != 0 &&
			jam < gContestEffects[gContestMoves[newMove].effect]
					.jam )
			break;
	}
...
static void ContestAICmd_get_move_used_count( void )
{
	...
	if( move != eContestantStatus[eContestAI.contestantId].prevMove )
		...
		result = eContestantStatus[eContestAI.contestantId]
				 .moveRepeatCount +
			1;
...
static void ContestAICmd_check_combo_starter( void )
{
	...
	for( i = 0; i < MAX_MON_MOVES; i++ )
	{
		...
		{
			result = AreMovesContestCombo( move,
				gContestMons[eContestAI.contestantId]
					.moves[i] );
			if( result )
			{
	...
	for( i = 0; i < MAX_MON_MOVES; i++ )
	{
		if( gContestMons[eContestAI.contestantId].moves[i] )
		{
			result = AreMovesContestCombo(
				gContestMons[eContestAI.contestantId].moves[i],
				move );
			if( result )
			{
				result = 1;
...
static void ContestAICmd_check_would_finish_combo( void )
{
	...
	if( eContestantStatus[eContestAI.contestantId].prevMove )
		result = AreMovesContestCombo(
			eContestantStatus[eContestAI.contestantId].prevMove,
			move );
...
static void ContestAICmd_get_used_combo_starter( void )
{
	...
	if( sub_80DE1E8( var ) )
		result = gContestMoves[eContestantStatus[var].prevMove]
				 .comboStarterId
			? 1
			: 0;
...
static void ContestAICmd_unk_59( void )
{
	...
	eContestAI.scriptResult = eContestantStatus[var].pointTotal -
		eContestantStatus[eContestAI.contestantId].pointTotal;
	gAIScriptPtr += 2;
}
...
static void ContestAICmd_unk_5E( void )
{
	...
	eContestAI.scriptResult = gContestMonConditions[var] -
		gContestMonConditions[eContestAI.contestantId];
	gAIScriptPtr += 2;
}
...
static void ContestAICmd_unk_72( void )
{
	...
	// gAIScriptPtr[2]?
	eContestAI.scriptArr[gAIScriptPtr[1]] +=
...
{
	eContestAI.scriptArr[gAIScriptPtr[1]] +=
		eContestAI.scriptArr[gAIScriptPtr[2]];
	gAIScriptPtr += 3;
}
...
static void ContestAICmd_unk_75( void )
{
	if( eContestAI.scriptArr[gAIScriptPtr[1]] <
		T1_READ_16( gAIScriptPtr + 2 ) )
