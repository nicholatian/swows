...
// The shaking alternates between the original position and the target
// position. arg 0: anim battler arg 1: x pixel offset arg 2: y pixel offset
...
void AnimTask_ShakeMonStep( u8 taskId )
{
	if( gTasks[taskId].data[3] == 0 )
	{
		...
		{
			gSprites[gTasks[taskId].data[0]].pos2.x =
		...
		{
			gSprites[gTasks[taskId].data[0]].pos2.y =
...
void AnimTask_ShakeMon2( u8 taskId )
{
	...
	else if( gBattleAnimArgs[0] != 8 )
	{
		...
		case 4:
			...
			break;
		case 5:
			battlerId = GetBattlerAtPosition(
...
void AnimTask_ShakeMon2Step( u8 taskId )
{
	if( gTasks[taskId].data[3] == 0 )
	{
		if( gSprites[gTasks[taskId].data[0]].pos2.x ==
			gTasks[taskId].data[4] )
			gSprites[gTasks[taskId].data[0]].pos2.x =
				-gTasks[taskId].data[4];
		else
		...
		if( gSprites[gTasks[taskId].data[0]].pos2.y ==
			gTasks[taskId].data[5] )
			gSprites[gTasks[taskId].data[0]].pos2.y =
				-gTasks[taskId].data[5];
		else
...
// picture. arg 0: battler arg 1: x offset arg 2: y offset arg 3: num shakes
// arg 4: delay
...
void sub_80D57B8( u8 taskId )
{
	...
	gSprites[spriteId].pos2.x =
		Sin( gTasks[taskId].data[5], gTasks[taskId].data[1] );
	gSprites[spriteId].pos2.y =
		-Cos( gTasks[taskId].data[5], gTasks[taskId].data[2] );
...
static void SlideMonToOriginalPos( struct Sprite* sprite )
{
	...
	sprite->data[1] =
		gSprites[monSpriteId].pos1.x + gSprites[monSpriteId].pos2.x;
	sprite->data[2] = gSprites[monSpriteId].pos1.x;
	sprite->data[3] =
		gSprites[monSpriteId].pos1.y + gSprites[monSpriteId].pos2.y;
...
static void SlideMonToOriginalPosStep( struct Sprite* sprite )
{
	...
	else
	{
		...
		monSprite->pos2.x =
			( s8 )( sprite->data[3] >> 8 ) + sprite->data[5];
		monSprite->pos2.y =
			( s8 )( sprite->data[4] >> 8 ) + sprite->data[6];
	}
}
...
// is first translated in an arc to one position.  Then, it "lunges" to a
// target x offset.  Used in TAKE_DOWN, for example. arg 0: anim bank arg 1:
// horizontal speed (subpixel) arg 2: wave amplitude arg 3: first duration arg
// 4: delay before starting lunge arg 5: target x offset for lunge arg 6: lunge
// duration
void AnimTask_WindUpLunge( u8 taskId )
{
	...
	gTasks[taskId].data[1] =
		( gBattleAnimArgs[1] << 8 ) / gBattleAnimArgs[3];
	gTasks[taskId].data[2] = gBattleAnimArgs[2];
	gTasks[taskId].data[3] = gBattleAnimArgs[3];
	gTasks[taskId].data[4] = gBattleAnimArgs[4];
	gTasks[taskId].data[5] =
		( gBattleAnimArgs[5] << 8 ) / gBattleAnimArgs[6];
...
void AnimTask_WindUpLungePart1( u8 taskId )
{
	...
	gSprites[spriteId].pos2.y =
		Sin( ( u8 )( gTasks[taskId].data[10] >> 8 ),
...
void AnimTask_WindUpLungePart2( u8 taskId )
{
	...
	else
	{
		...
		gSprites[spriteId].pos2.x = ( gTasks[taskId].data[12] >> 8 ) +
			( gTasks[taskId].data[11] >> 8 );
...
void sub_80D5DB0( u8 taskId )
{
	...
	case 2:
		...
		spriteId = gBattlerSpriteIds[BATTLE_PARTNER(
			gBattleAnimAttacker )];
		break;
	case 3:
		if( !IsBattlerSpriteVisible(
		...
		spriteId =
			gBattlerSpriteIds[BATTLE_PARTNER( gBattleAnimTarget )];
		break;
	default:
		DestroyAnimVisualTask( taskId );
		return;
...
void sub_80A8B3C( u8 taskId )
{
	...
	if( gSprites[spriteId].pos2.x + gSprites[spriteId].pos1.x + 0x20 >
		0x130u )
	{
		DestroyAnimVisualTask( taskId );
		return;
...
void AnimTask_SwayMonStep( u8 taskId )
{
	...
	else
	{
		...
		{
			gSprites[spriteId].pos2.y =
				( sineValue >= 0 ) ? sineValue : -sineValue;
		}
		else
		{
			gSprites[spriteId].pos2.y =
...
void sub_80D6134( u8 taskId )
{
	...
	{
		gTasks[taskId].data[3] =
			gBattleAnimArgs[0] * gBattleAnimArgs[1];
	}
	gTasks[taskId].data[4] = gBattleAnimArgs[1];
...
void sub_80D6308( u8 taskId )
{
	gTasks[taskId].data[3] += gTasks[taskId].data[4];
	SetSpriteRotScale(
		gTasks[taskId].data[5], 0x100, 0x100, gTasks[taskId].data[3] );
	if( gTasks[taskId].data[7] )
	{
...
void sub_80D646C( u8 taskId )
{
	...
	if( ++task->data[0] > task->data[1] )
	{
		...
		if( task->data[10] )
		{
			if( task->data[12] )
			{
				gSprites[task->data[7]].pos2.x =
					task->data[8] + task->data[13];
			}
			else
			{
				gSprites[task->data[7]].pos2.x =
					task->data[8] - task->data[14];
			}
		}
		if( task->data[11] )
		{
