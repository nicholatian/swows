	...
	for( i = 0, j = 0; i <= 6; i++ )
	{
		...
		if( spriteId != 0x40 )
		{
			...
			if( ++j >= 5 )
				j = 0;
			// gHeatedRockCoords
			sub_8109984( &gSprites[spriteId],
				gUnknown_085955CC[i][0] * sign,
				gUnknown_085955CC[i][1] );
...
static u16 sub_8109930( u8 spriteId )
{
	u16 var1 = gSprites[spriteId].pos1.y + gSprites[spriteId].pos2.y +
...
static void sub_8109C4C( struct Sprite* sprite )
{
	...
	if( !AnimTranslateLinear( sprite ) )
	{
		...
		if( ( initialData5 == 0 || initialData5 > 196 ) &&
			newData5 > 0 && sprite->data[7] == 0 )
		{
...
void sub_8109CB0( struct Sprite* sprite )
{
	...
	if( !IsContest( ) )
	{
		if( sprite->data[1] < 64 || sprite->data[1] > 195 )
			sprite->oam.priority = GetBattlerSpriteBGPriority(
				gBattleAnimTarget );
		else
			sprite->oam.priority = GetBattlerSpriteBGPriority(
						       gBattleAnimTarget ) +
...
static void sub_8109E2C( u8 taskId )
{
	...
	case 0:
		...
		for( task->data[3] = 0; task->data[3] < task->data[13];
			task->data[3]++ )
		{
			gSprites[task->data[task->data[3] + 14]].pos2.x =
...
void AnimTask_BlendBackground( u8 taskId )
{
	...
	BlendPalette( animBg.paletteId * 16,
		16,
		gBattleAnimArgs[0],
		gBattleAnimArgs[1] ); // u16 palOffset, u16 numEntries, u8
				      // coeff, u16 blendColor
	DestroyAnimVisualTask( taskId );
}
...
void sub_810A094( u8 taskId )
{
	...
	if( gTasks[taskId].data[3] == 1 )
		gSprites[spriteId].pos2.y = gBattleAnimArgs[1] * unk < 0
			? -( gBattleAnimArgs[1] * unk )
			: gBattleAnimArgs[1] * unk;
	else
