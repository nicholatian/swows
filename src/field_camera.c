...
static s32 MapPosToBgTilemapOffset(
	struct FieldCameraOffset* a, s32 x, s32 y );
static void DrawWholeMapViewInternal(
	int x, int y, const struct MapLayout* mapLayout );
...
// text
static void move_tilemap_camera_to_upper_left_corner_(
	struct FieldCameraOffset* cameraOffset )
{
...
static void tilemap_move_something(
	struct FieldCameraOffset* cameraOffset, u32 b, u32 c )
{
...
{
	DrawWholeMapViewInternal( gSaveBlock1Ptr->pos.x,
	...
	for( i = 0; i < 32; i += 2 )
	{
		...
		for( j = 0; j < 32; j += 2 )
		{
			...
			if( temp >= 32 )
				temp -= 32;
			DrawMetatileAt(
				mapLayout, r6 + temp, x + j / 2, y + i / 2 );
		}
	}
}

static void RedrawMapSlicesForCameraUpdate(
	struct FieldCameraOffset* cameraOffset, int x, int y )
{
	...
	const struct MapLayout* mapLayout )
{
	...
	for( i = 0; i < 32; i += 2 )
	{
		...
		if( temp >= 32 )
			temp -= 32;
		DrawMetatileAt( mapLayout,
			r7 + temp,
			gSaveBlock1Ptr->pos.x + i / 2,
...
static void DrawMetatileAt(
	const struct MapLayout* mapLayout, u16 offset, int x, int y )
{
...
static void DrawMetatile( s32 metatileLayerType, u16* metatiles, u16 offset )
{
	...
	case 0: // LAYER_TYPE_NORMAL
		...
		// Draw metatile's top layer to the top background layer, which
		// covers event object sprites.
		gBGTilemapBuffers2[offset]        = metatiles[4];
		gBGTilemapBuffers2[offset + 1]    = metatiles[5];
		gBGTilemapBuffers2[offset + 0x20] = metatiles[6];
		gBGTilemapBuffers2[offset + 0x21] = metatiles[7];
...
static s32 MapPosToBgTilemapOffset(
	struct FieldCameraOffset* cameraOffset, s32 x, s32 y )
{
...
static void CameraUpdateCallback( struct CameraObject* fieldCamera )
{
	...
	{
		fieldCamera->movementSpeedX =
			gSprites[fieldCamera->spriteId].data[2];
		fieldCamera->movementSpeedY =
			gSprites[fieldCamera->spriteId].data[3];
	}
}
...
void CameraUpdate( void )
{
	...
	if( deltaX != 0 || deltaY != 0 )
	{
		...
		tilemap_move_something(
			&sFieldCameraOffset, deltaX * 2, deltaY * 2 );
...
void UpdateCameraPanning( void )
{
	...
	gSpriteCoordOffsetY =
		gTotalCameraPixelOffsetY - sVerticalCameraPan - 8;
}
