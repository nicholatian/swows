	...
	else
	{
		...
		for( i = 0; i < MAX_BATTLERS_COUNT; i++ )
		{
			...
			if( i == multiplayerId )
			{
				...
				case 0:
				case 3:
					gBattlerPositions[gLinkPlayers[i].id] =
						0;
					gBattlerPartyIndexes[gLinkPlayers[i]
								     .id] = 0;
					break;
				case 1:
				case 2:
				...
				if( ( !( gLinkPlayers[i].id & 1 ) &&
					...
					( ( gLinkPlayers[i].id & 1 ) &&
						( gLinkPlayers[multiplayerId]
							...
							1 ) ) )
				{
					gBattlerControllerFuncs[gLinkPlayers[i]
									.id] =
						SetControllerToLinkPartner;
					switch( gLinkPlayers[i].id )
					{
						...
						if( GetMonData(
							...
							GetMonData(
								&gPlayerParty
									[j],
								MON_DATA_SPECIES ) !=
								SPECIES_NONE // Probably a typo by Game
									     // Freak. The rest use
									     // SPECIES2.
							&&
							GetMonData(
...
static void CreateTasksForSendRecvLinkBuffers( void )
{
	...
	sLinkReceiveTaskId =
		CreateTask( Task_HandleCopyReceivedLinkBuffersData, 0 );
...
void PrepareBufferDataTransferLink( u8 bufferId, u16 size, u8* data )
{
	...
	if( gTasks[sLinkSendTaskId].data[14] + alignedSize + LINK_BUFF_DATA +
			1 >
		BATTLE_BUFFER_LINK_SIZE )
	{
		gTasks[sLinkSendTaskId].data[12] =
	...
	gLinkBattleSendBuffer[gTasks[sLinkSendTaskId].data[14] +
		LINK_BUFF_ATTACKER]       = gBattlerAttacker;
	gLinkBattleSendBuffer[gTasks[sLinkSendTaskId].data[14] +
		LINK_BUFF_TARGET]         = gBattlerTarget;
	gLinkBattleSendBuffer[gTasks[sLinkSendTaskId].data[14] +
		LINK_BUFF_SIZE_LO]        = alignedSize;
	gLinkBattleSendBuffer[gTasks[sLinkSendTaskId].data[14] +
		LINK_BUFF_SIZE_HI]        = ( alignedSize & 0x0000FF00 ) >> 8;
	gLinkBattleSendBuffer[gTasks[sLinkSendTaskId].data[14] +
	...
	for( i = 0; i < size; i++ )
		gLinkBattleSendBuffer[gTasks[sLinkSendTaskId].data[14] +
			LINK_BUFF_DATA + i] = data[i];

	gTasks[sLinkSendTaskId].data[14] = gTasks[sLinkSendTaskId].data[14] +
		alignedSize + LINK_BUFF_DATA;
}
	...
	case 2:
		...
		else
		{
			if( gBattleTypeFlags & BATTLE_TYPE_BATTLE_TOWER )
				...
				var = ( gBattleTypeFlags & BATTLE_TYPE_MULTI )
					? 4
					: 2;
	...
	case 3:
		if( gTasks[taskId].data[15] != gTasks[taskId].data[14] )
		{
			if( gTasks[taskId].data[13] == 0 )
			{
				...
				blockSize =
					( gLinkBattleSendBuffer[gTasks[taskId].data
									[15] +
						  LINK_BUFF_SIZE_LO] |
		...
		if( IsLinkTaskFinished( ) )
		{
			blockSize =
				gLinkBattleSendBuffer[gTasks[taskId].data[15] +
					LINK_BUFF_SIZE_LO] |
				( gLinkBattleSendBuffer[gTasks[taskId]
								.data[15] +
					  LINK_BUFF_SIZE_HI]
					<< 8 );
			gTasks[taskId].data[13] = 1;
			gTasks[taskId].data[15] = gTasks[taskId].data[15] +
				blockSize + LINK_BUFF_DATA;
		...
		( gBattleTypeFlags & BATTLE_TYPE_20 ) )
	{
		...
		for( i = 0; i < GetLinkPlayerCount( ); i++ )
		{
			if( GetBlockReceivedStatus( ) & gBitTable[i] )
			{
				...
				if( gTasks[sLinkReceiveTaskId].data[14] + 9 +
						r6 >
					0x1000 )
				{
					gTasks[sLinkReceiveTaskId].data[12] =
						gTasks[sLinkReceiveTaskId]
							.data[14];
					gTasks[sLinkReceiveTaskId].data[14] =
						0;
				}

				dest = &gLinkBattleRecvBuffer
					       [gTasks[sLinkReceiveTaskId]
							       .data[14]];
				src = recvBuffer;
...
static void Task_HandleCopyReceivedLinkBuffersData( u8 taskId )
{
	...
	if( gTasks[taskId].data[15] != gTasks[taskId].data[14] )
	{
		...
		battlerId = gLinkBattleRecvBuffer[gTasks[taskId].data[15] +
			LINK_BUFF_ACTIVE_BATTLER];
		blockSize = gLinkBattleRecvBuffer[gTasks[taskId].data[15] +
				    LINK_BUFF_SIZE_LO] |
		...
		case 0:
			...
			memcpy( gBattleBufferA[battlerId],
				&gLinkBattleRecvBuffer[gTasks[taskId]
							       .data[15] +
					LINK_BUFF_DATA],
				blockSize );
		...
		case 1:
			...
			break;
		case 2:
			var = gLinkBattleRecvBuffer[gTasks[taskId].data[15] +
				LINK_BUFF_DATA];
			gBattleControllerExecFlags &=
				~( gBitTable[battlerId] << ( var * 4 ) );
			break;
		}
...
void BtlController_EmitSetMonData(
	u8 bufferId, u8 requestId, u8 monToCheck, u8 bytes, void* data )
{
	s32 i;
	...
	PrepareBufferDataTransfer(
		bufferId, sBattleBuffersTransferData, 3 + bytes );
}

void BtlController_EmitSetRawMonData(
	u8 bufferId, u8 monId, u8 bytes, void* data )
{
	s32 i;
	...
	for( i = 0; i < bytes; i++ )
		sBattleBuffersTransferData[3 + i] = *(u8*)( data++ );
	PrepareBufferDataTransfer(
		bufferId, sBattleBuffersTransferData, bytes + 3 );
}
...
void BtlController_EmitSwitchInAnim(
	u8 bufferId, u8 partyId, bool8 dontClearSubstituteBit )
{
...
void BtlController_EmitPause( u8 bufferId, u8 toWait, void* data )
{
	...
	PrepareBufferDataTransfer(
		bufferId, sBattleBuffersTransferData, toWait * 3 + 2 );
}
	...
	u8 multihit )
{
	...
	if( WEATHER_HAS_EFFECT2 )
	{
		...
		sBattleBuffersTransferData[13] =
			( gBattleWeather & 0xFF00 ) >> 8;
	}
	else
	{
	...
	memcpy( &sBattleBuffersTransferData[16],
		disableStructPtr,
		sizeof( struct DisableStruct ) );
	PrepareBufferDataTransfer( bufferId,
...
void BtlController_EmitChooseMove( u8 bufferId,
...
void BtlController_EmitChoosePokemon(
	u8 bufferId, u8 caseId, u8 arg2, u8 abilityId, u8* arg4 )
{
	s32 i;
...
void BtlController_EmitStatusIconUpdate(
	u8 bufferId, u32 status1, u32 status2 )
{
...
void BtlController_EmitStatusAnimation(
	u8 bufferId, bool8 status2, u32 status )
{
...
void BtlController_EmitDataTransfer( u8 bufferId, u16 size, void* data )
{
	...
	PrepareBufferDataTransfer(
		bufferId, sBattleBuffersTransferData, size + 4 );
}

void BtlController_EmitDMA3Transfer(
	u8 bufferId, void* dst, u16 size, void* data )
{
	s32 i;
	...
	PrepareBufferDataTransfer(
		bufferId, sBattleBuffersTransferData, size + 7 );
}

void BtlController_EmitPlayBGM(
	u8 bufferId, u16 songId, void* unusedDumbDataParameter )
{
	s32 i;
	...
	for( i = 0; i < songId; i++ ) // ????
		sBattleBuffersTransferData[3 + i] =
			*(u8*)( unusedDumbDataParameter++ );
	PrepareBufferDataTransfer(
		bufferId, sBattleBuffersTransferData, songId + 3 );
}
...
void BtlController_EmitCmd32( u8 bufferId, u16 size, void* data )
{
	...
	for( i = 0; i < size; i++ )
		sBattleBuffersTransferData[3 + i] = *(u8*)( data++ );
	PrepareBufferDataTransfer(
		bufferId, sBattleBuffersTransferData, size + 3 );
}
...
void BtlController_EmitPlayFanfareOrBGM(
	u8 bufferId, u16 songId, bool8 playBGM )
{
	...
	u8 bufferId, struct HpAndStatus* hpAndStatus, u8 arg2 )
{
	...
	for( i = 0; i < ( s32 )( sizeof( struct HpAndStatus ) * PARTY_SIZE );
		i++ )
		sBattleBuffersTransferData[4 + i] =
			*( i + (u8*)( hpAndStatus ) );
	PrepareBufferDataTransfer( bufferId,
...
void BtlController_EmitBattleAnimation(
	u8 bufferId, u8 animationId, u16 argument )
{
