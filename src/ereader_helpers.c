...
bool8 EReader_IsReceivedDataValid( struct EReaderTrainerHillSet* buffer )
{
	...
	checksum = CalcByteArraySum(
		(u8*)buffer->unk_8, var0 * sizeof( struct Unk81D38FC ) );
...
static bool32 TrainerHill_VerifyChecksum(
	struct EReaderTrainerHillSet* buffer )
{
	u32 checksum;
	int var0 = buffer->unk_0;
	if( var0 < 1 || var0 > 8 )
	...
	checksum = CalcByteArraySum( (u8*)buffer->unk_8,
		sizeof( struct EReaderTrainerHillSet ) -
			offsetof( struct EReaderTrainerHillSet, unk_8 ) );
	if( checksum != buffer->checksum )
		return FALSE;
...
static bool32 TryWriteTrainerHill_r(
	struct EReaderTrainerHillSet* arg0, struct Unk81D3998* buffer2 )
{
	int i;
	...
	for( i = 0; i < arg0->unk_0; i++ )
	{
		if( !( i & 1 ) )
		{
			buffer2->unk_008[i / 2].unk_000[0] =
				arg0->unk_8[i].unk0;
			memcpy( buffer2->unk_008[i / 2].unk_294,
				arg0->unk_8[i].unk14C,
				0x124 );
			memcpy( buffer2->unk_008[i / 2].unk_004,
				arg0->unk_8[i].unk4,
				0x148 );
		}
		else
		{
			buffer2->unk_008[i / 2].unk_000[1] =
				arg0->unk_8[i].unk0;
			memcpy( buffer2->unk_008[i / 2].unk_14C,
				arg0->unk_8[i].unk4,
				0x148 );
		}
	}
	...
	buffer2->checksum = CalcByteArraySum( (u8*)buffer2->unk_008,
		sizeof( struct Unk81D3998 ) -
			offsetof( struct Unk81D3998, unk_008 ) );
	if( TryWriteSpecialSaveSection(
		    SECTOR_ID_TRAINER_HILL, (u8*)buffer2 ) != 1 )
		return FALSE;
...
static bool32 TryReadTrainerHill_r(
	struct EReaderTrainerHillSet* arg0, u8* arg1 )
{
...
{
	u16 terminal =
		( *(vu32*)REG_ADDR_SIOCNT ) & ( SIO_MULTI_SI | SIO_MULTI_SD );
	...
	case 3:
		...
		if( gUnknown_030012C8.unk0[0] == 1 )
		{
			if( gUnknown_030012C8.unkC < gUnknown_030012C8.unk10 )
			{
				REG_SIODATA32 =
					gUnknown_030012C8
						.unk8[gUnknown_030012C8.unkC];
				gUnknown_030012C8.unk14 +=
					gUnknown_030012C8
						.unk8[gUnknown_030012C8.unkC];
			}
			else
		...
		else
		{
			if( gUnknown_030012C8.unkC > 0 &&
				gUnknown_030012C8.unkC <
					gUnknown_030012C8.unk10 + 1 )
			{
				gUnknown_030012C8
					.unk8[gUnknown_030012C8.unkC - 1] =
					value;
...
static void sub_81D413C( void ) { REG_SIOCNT |= SIO_ENABLE; }
