...
static void ItemStorage_MoveCursor(
	s32 id, bool8 b, struct ListMenu* thisMenu );
...
EWRAM_DATA struct PlayerPCItemPageStruct playerPCItemPageInfo = {
	0, 0, 0, 0, {0, 0, 0}, 0};
...
static const u8 gPlayerPC_OptionOrder[] = {PLAYERPC_MENU_ITEMSTORAGE,
	PLAYERPC_MENU_MAILBOX,
	PLAYERPC_MENU_TURNOFF};
...
void NewGameInitPCItems( void )
{
	...
	for( i = 0,
	     ClearItemSlots( gSaveBlock1Ptr->pcItems,
		     ARRAY_COUNT( gSaveBlock1Ptr->pcItems ) );
		NEW_GAME_PC_ITEMS( i, PC_ITEM_ID ) &&
		NEW_GAME_PC_ITEMS( i, PC_QUANTITY ) &&
		AddPCItem( NEW_GAME_PC_ITEMS( i, PC_ITEM_ID ),
			NEW_GAME_PC_ITEMS( i, PC_QUANTITY ) ) == TRUE;
		i++ )
		;
}
...
static void InitPlayerPCMenu( u8 taskId )
{
	...
	windowTemplate.width = sub_81DB3D8( sPlayerPCMenuActions,
	...
	InitMenuInUpperLeftCornerPlaySoundWhenAPressed(
		data[4], gPcItemMenuOptionsNum, 0 );
...
static void PlayerPCProcessMenuInput( u8 taskId )
{
	...
	default:
		...
		gTasks[taskId].func =
			sPlayerPCMenuActions
				[gPcItemMenuOptionOrder[inputOptionId]]
					.func.void_u8;
		break;
	}
}
...
void ReshowPlayerPC( u8 var )
{
	DisplayItemMessageOnField(
		var, gText_WhatWouldYouLike, InitPlayerPCMenu );
}
...
static void PlayerPC_Mailbox( u8 taskId )
{
	...
	if( playerPCItemPageInfo.count == 0 )
		DisplayItemMessageOnField(
			taskId, gText_NoMailHere, ReshowPlayerPC );
	else
	{
...
static void PlayerPC_TurnOff( u8 taskId )
{
	if( gPcItemMenuOptionsNum == 4 ) // if the option count is 4, we are at
					 // the bedroom PC and not player PC,
					 // so do gender specific handling.
	{
		if( gSaveBlock2Ptr->playerGender == MALE )
			ScriptContext1_SetupScript(
				LittlerootTown_BrendansHouse_2F_EventScript_1F863F );
		else
			ScriptContext1_SetupScript(
				LittlerootTown_MaysHouse_2F_EventScript_1F958F );
	}
	else
	{
...
static void InitItemStorageMenu( u8 taskId, u8 var )
{
	...
	windowTemplate.width =
		GetMaxWidthInMenuTable( gPCText_ItemPCOptionsText, 4 );
	data[4] = AddWindow( &windowTemplate );
	SetStandardWindowBorderStyle( data[4], 0 );
...
static void ItemStorageMenuProcessInput( u8 taskId )
{
	...
	default:
		PlaySE( SE_SELECT );
		gPCText_ItemPCOptionsText[inputOptionId].func.void_u8(
			taskId );
		break;
	}
}
...
void Mailbox_DoRedrawMailboxMenuAfterReturn( void )
{
	...
	InitItemStorageMenu(
		CreateTask( ItemStorage_HandleReturnToProcessInput, 0 ), 1 );
	pal_fill_black( );
}
...
static void ItemStorage_Withdraw( u8 taskId )
{
	...
	else
	{
		sub_816B4DC( taskId );
		DisplayItemMessageOnField(
			taskId, gText_NoItems, PlayerPC_ItemStorage );
	}
}
	...
	for( i = 6; i < 15; i++ )
	{
		for( j = i + 1; j < 16; j++ )
		{
			if( gSaveBlock1Ptr->mail[i].itemId == 0 )
			{
				...
				gSaveBlock1Ptr->mail[i] =
					gSaveBlock1Ptr->mail[j];
...
static void Mailbox_ProcessInput( u8 taskId )
{
	...
	if( !gPaletteFade.active )
	{
		...
		ListMenuGetScrollAndRow( data[5],
...
static void Mailbox_FadeAndReadMail( u8 taskId )
{
	if( !gPaletteFade.active )
	{
		sub_81D1EC0( );
		CleanupOverworldWindowsAndTilemaps( );
		ReadMail( &( gSaveBlock1Ptr->mail[playerPCItemPageInfo
							  .itemsAbove +
				  6 + playerPCItemPageInfo.cursorPos] ),
...
{
	DisplayItemMessageOnField(
		taskId, gText_MessageWillBeLost, Mailbox_DrawYesNoBeforeMove );
}
...
static void Mailbox_DoMailMoveToBag( u8 taskId )
{
	struct MailStruct* mailStruct =
		&( gSaveBlock1Ptr->mail[playerPCItemPageInfo.itemsAbove + 6 +
			playerPCItemPageInfo.cursorPos] );
	if( !AddBagItem( mailStruct->itemId, 1 ) )
	{
		DisplayItemMessageOnField(
			taskId, gText_BagIsFull, Mailbox_Cancel );
	}
	else
	{
		DisplayItemMessageOnField(
			taskId, gText_MailToBagMessageErased, Mailbox_Cancel );
...
void ItemStorage_RefreshListMenu( void )
{
	...
	{
		CopyItemName_PlayerPC( &( gUnknown_0203BCC4->unk198[i][0] ),
			gSaveBlock1Ptr->pcItems[i].itemId );
		gUnknown_0203BCC4->unk0[i].name =
			&( gUnknown_0203BCC4->unk198[i][0] );
...
static void sub_816BEF0( s32 id )
{
	...
	if( id != -2 )
		description = (u8*)ItemId_GetDescription(
			gSaveBlock1Ptr->pcItems[id].itemId );
	else
		description = ItemStorage_GetItemPcResponse(
...
static void ItemStorage_ProcessWithdrawTossInput( u8 taskId )
{
	...
	AddTextPrinterParameterized(
		gUnknown_0203BCC4->windowIds[3], 1, text, x, 1, 0, NULL );
...
static void ItemStorage_ProcessInput( u8 taskId )
{
	...
	if( gMain.newKeys & SELECT_BUTTON )
	{
		ListMenuGetScrollAndRow( data[5],
			&( playerPCItemPageInfo.itemsAbove ),
			&( playerPCItemPageInfo.cursorPos ) );
		if( ( playerPCItemPageInfo.itemsAbove +
			    playerPCItemPageInfo.cursorPos ) !=
			( playerPCItemPageInfo.count - 1 ) )
		{
			PlaySE( SE_SELECT );
...
static void ItemStorage_ItemSwapChoosePrompt( u8 taskId )
{
	...
	gUnknown_0203BCC4->unk666 = ( playerPCItemPageInfo.itemsAbove +
		playerPCItemPageInfo.cursorPos );
	sub_816BFB8( data[5], 0, 0 );
	sub_816C690( gUnknown_0203BCC4->unk666 );
	CopyItemName(
		gSaveBlock1Ptr->pcItems[gUnknown_0203BCC4->unk666].itemId,
		gStringVar1 );
	ItemStorage_PrintItemPcResponse(
		ItemStorage_GetItemPcResponse( ITEMPC_SWITCH_WHICH_ITEM ) );
...
static void ItemStorage_DoItemSwap( u8 taskId, bool8 a )
{
	...
	data = gTasks[taskId].data;
	b    = ( playerPCItemPageInfo.itemsAbove +
                playerPCItemPageInfo.cursorPos );
	PlaySE( SE_SELECT );
	DestroyListMenuTask( data[5],
		&( playerPCItemPageInfo.itemsAbove ),
...
{
	sub_8122448(
		gUnknown_0203BCC4->spriteIds, 7, 128, ( ( a + 1 ) * 16 ) );
}
...
static void sub_816C6BC( u8 windowId, u16 value, u32 mode, u8 x, u8 y, u8 n )
{
	...
	AddTextPrinterParameterized( windowId,
		1,
		gStringVar4,
		GetStringCenterAlignXOffset( 1, gStringVar4, 48 ),
		y,
		0,
		NULL );
}
...
static void ItemStorage_DoItemAction( u8 taskId )
{
	...
	data = gTasks[taskId].data;
	b    = ( playerPCItemPageInfo.cursorPos +
                playerPCItemPageInfo.itemsAbove );
	ItemStorage_RemoveScrollIndicator( );
	data[2] = 1;
	if( !data[3] )
...
static void ItemStorage_HandleQuantityRolling( u8 taskId )
{
	...
	b    = ( playerPCItemPageInfo.cursorPos +
                playerPCItemPageInfo.itemsAbove );
	if( AdjustQuantityAccordingToDPadInput( &( data[2] ),
		    gSaveBlock1Ptr->pcItems[b].quantity ) == TRUE )
		sub_816C6BC( sub_816BC7C( 4 ),
			data[2],
			STR_CONV_MODE_LEADING_ZEROS,
			8,
...
static void ItemStorage_DoItemWithdraw( u8 taskId )
{
	...
	if( AddBagItem( gSaveBlock1Ptr->pcItems[b].itemId, data[2] ) == TRUE )
	{
		...
		ConvertIntToDecimalStringN(
			gStringVar2, data[2], STR_CONV_MODE_LEFT_ALIGN, 3 );
		ItemStorage_PrintItemPcResponse( ItemStorage_GetItemPcResponse(
	...
	else
	{
		data[2] = 0;
		ItemStorage_PrintItemPcResponse(
			ItemStorage_GetItemPcResponse( ITEMPC_NO_MORE_ROOM ) );
		gTasks[taskId].func =
...
{
	ItemStorage_PrintItemPcResponse(
		ItemStorage_GetItemPcResponse( ITEMPC_THREW_AWAY_ITEM ) );
	gTasks[taskId].func = ItemStorage_HandleRemoveItem;
}
...
static void ItemStorage_ResumeInputFromNoToss( u8 taskId )
{
	ItemStorage_PrintItemPcResponse( ItemStorage_GetItemPcResponse(
		gSaveBlock1Ptr
			->pcItems[( playerPCItemPageInfo.itemsAbove +
				playerPCItemPageInfo.cursorPos )]
			.itemId ) );
...
static void ItemStorage_HandleRemoveItem( u8 taskId )
{
	...
	{
		RemovePCItem( ( playerPCItemPageInfo.cursorPos +
				      playerPCItemPageInfo.itemsAbove ),
			data[2] );
		DestroyListMenuTask( data[5],
			&( playerPCItemPageInfo.itemsAbove ),
