...
static const union AnimCmd* const sSpriteAnimTable_862A724[] = {
	sSpriteAnim_862A6FC, sSpriteAnim_862A710};
...
static const union AnimCmd* const sSpriteAnimTable_862A8BC[] = {
	sSpriteAnim_862A894, sSpriteAnim_862A8A8};
...
static const union AnimCmd* const sSpriteAnimTable_862AB70[] = {
	sSpriteAnim_862AB54, sSpriteAnim_862AB5C};
...
// code
void DoRayquazaScene(
	u8 animId, bool8 onlyOneAnim, void ( *callback )( void ) )
{
...
static void Task_SetNextAnim( u8 taskId )
{
	if( !gPaletteFade.active )
	{
		...
		else
		{
			...
			gTasks[taskId].func =
				sTasksForAnimations[sRayScene->animId];
		}
	}
}
...
static void sub_81D6FE0( void )
{
	...
	InitBgsFromTemplates(
		0, sUnknown_0862A888, ARRAY_COUNT( sUnknown_0862A888 ) );
...
static void sub_81D706C( void )
{
	...
	decompress_and_copy_tile_data_to_vram(
		0, gRaySceneClouds_Gfx, 0, 0, 0 );
	while( free_temp_tile_data_buffers_if_possible( ) )
		;

	LZDecompressWram(
		gRaySceneClouds2_Tilemap, sRayScene->tilemapBuffers[0] );
	LZDecompressWram(
		gRaySceneClouds1_Tilemap, sRayScene->tilemapBuffers[1] );
	LZDecompressWram(
		gRaySceneClouds3_Tilemap, sRayScene->tilemapBuffers[2] );
		...
		else
		{
			...
			case 380:
				SetGpuReg( REG_OFFSET_BLDCNT,
					BLDCNT_TGT1_BG2 | BLDCNT_TGT2_BG1 |
						BLDCNT_EFFECT_BLEND );
...
static void sub_81D752C( u8 taskId )
{
	...
	if( data[0] != 16 )
	{
		...
		SetGpuReg( REG_OFFSET_BLDALPHA,
			BLDALPHA_BLEND( 16 - data[0], data[0] ) );
	}
}
...
static void sub_81D7E10( void )
{
	...
	ResetBgsAndClearDma3BusyFlags( 0 );
	InitBgsFromTemplates(
		1, sUnknown_0862AA54, ARRAY_COUNT( sUnknown_0862AA54 ) );
	SetBgTilemapBuffer( 0, sRayScene->tilemapBuffers[0] );
	SetBgTilemapBuffer( 1, sRayScene->tilemapBuffers[1] );
	SetBgTilemapBuffer( 2, sRayScene->tilemapBuffers[2] );
	ResetAllBgsCoordinates( );
...
static void sub_81D7E9C( void )
{
	...
	decompress_and_copy_tile_data_to_vram(
		0, gRaySceneClouds_Gfx, 0, 0, 0 );
	decompress_and_copy_tile_data_to_vram(
		1, gRaySceneOvercast_Gfx, 0, 0, 0 );
	decompress_and_copy_tile_data_to_vram(
		2, gRaySceneRayquaza_Gfx, 0, 0, 0 );
	while( free_temp_tile_data_buffers_if_possible( ) )
		;
	...
	LZDecompressWram(
		gRaySceneClouds2_Tilemap, sRayScene->tilemapBuffers[0] );
	LZDecompressWram(
		gRaySceneOvercast_Tilemap, sRayScene->tilemapBuffers[1] );
	LZDecompressWram(
		gRaySceneRayquaza_Tilemap, sRayScene->tilemapBuffers[2] );
...
static void sub_81D82B0( void )
{
	...
	ResetBgsAndClearDma3BusyFlags( 0 );
	InitBgsFromTemplates(
		0, sUnknown_0862AACC, ARRAY_COUNT( sUnknown_0862AACC ) );
	SetBgTilemapBuffer( 0, sRayScene->tilemapBuffers[0] );
	SetBgTilemapBuffer( 1, sRayScene->tilemapBuffers[1] );
	SetBgTilemapBuffer( 2, sRayScene->tilemapBuffers[2] );
...
static void sub_81D8358( void )
{
	reset_temp_tile_data_buffers( );
	decompress_and_copy_tile_data_to_vram(
		0, gRaySceneRayquazaLight_Gfx, 0, 0, 0 );
	decompress_and_copy_tile_data_to_vram(
		1, gRaySceneOvercast2_Gfx, 0, 0, 0 );
	while( free_temp_tile_data_buffers_if_possible( ) )
		;

	LZDecompressWram(
		gRaySceneRayquazaLight_Tilemap, sRayScene->tilemapBuffers[0] );
	LZDecompressWram(
		gRaySceneOvercast2_Tilemap, sRayScene->tilemapBuffers[3] );
	CpuFastFill16( 0, sRayScene->tilemapBuffers[2], 0x800 );
	CpuFastCopy( sRayScene->tilemapBuffers[3],
...
static void sub_81D844C( void )
{
	...
	if( VCOUNT >= 24 && VCOUNT <= 135 &&
		VCOUNT - 24 <= sRayScene->field_2008 )
		REG_BLDALPHA = 0xD08;
	else
		REG_BLDALPHA = 0x1000;
...
static void Task_RayDescendsAnim( u8 taskId )
{
	...
	SetGpuRegBits( REG_OFFSET_BLDCNT,
		BLDCNT_TGT1_BG0 | BLDCNT_TGT2_BG1 | BLDCNT_TGT2_BG2 |
	...
	case 1:
		if( !gPaletteFade.active )
		{
			if( data[1] == 10 )
			{
				...
				EnableInterrupts(
					INTR_FLAG_HBLANK | INTR_FLAG_VBLANK );
			}
			else
			{
				data[1]++;
...
static void sub_81D8828( void )
{
	...
	ResetBgsAndClearDma3BusyFlags( 0 );
	InitBgsFromTemplates(
		0, sUnknown_0862AB44, ARRAY_COUNT( sUnknown_0862AB44 ) );
	SetBgTilemapBuffer( 0, sRayScene->tilemapBuffers[0] );
	SetBgTilemapBuffer( 1, sRayScene->tilemapBuffers[1] );
	SetBgTilemapBuffer( 2, sRayScene->tilemapBuffers[2] );
	SetBgTilemapBuffer( 3, sRayScene->tilemapBuffers[3] );
...
static void sub_81D88D0( void )
{
	reset_temp_tile_data_buffers( );
	decompress_and_copy_tile_data_to_vram(
		1, gRaySceneRayquazaChase_Gfx, 0, 0, 0 );
	decompress_and_copy_tile_data_to_vram(
		2, gRaySceneChaseStreaks_Gfx, 0, 0, 0 );
	decompress_and_copy_tile_data_to_vram(
		3, gRaySceneChaseBg_Gfx, 0, 0, 0 );
	while( free_temp_tile_data_buffers_if_possible( ) )
		;
	...
	LZDecompressWram(
		gRayChaseRayquazaChase_Tilemap, sRayScene->tilemapBuffers[1] );
	LZDecompressWram(
		gRaySceneChaseStreaks_Tilemap, sRayScene->tilemapBuffers[2] );
	LZDecompressWram(
		gRaySceneChaseBg_Tilemap, sRayScene->tilemapBuffers[3] );
...
static void sub_81D8C38( void )
{
	...
	ResetBgsAndClearDma3BusyFlags( 0 );
	InitBgsFromTemplates(
		1, sUnknown_0862AD08, ARRAY_COUNT( sUnknown_0862AD08 ) );
	SetBgTilemapBuffer( 0, sRayScene->tilemapBuffers[0] );
	SetBgTilemapBuffer( 1, sRayScene->tilemapBuffers[1] );
	SetBgTilemapBuffer( 2, sRayScene->tilemapBuffers[2] );
	ResetAllBgsCoordinates( );
...
static void sub_81D8CC4( void )
{
	reset_temp_tile_data_buffers( );
	decompress_and_copy_tile_data_to_vram(
		2, gRaySceneHushRing_Gfx, 0, 0, 0 );
	decompress_and_copy_tile_data_to_vram(
		0, gRaySceneHushBg_Gfx, 0, 0, 0 );
	while( free_temp_tile_data_buffers_if_possible( ) )
		;

	LZDecompressWram(
		gRaySceneHushRing_Tilemap, sRayScene->tilemapBuffers[1] );
	LZDecompressWram(
		gRaySceneHushBg_Tilemap, sRayScene->tilemapBuffers[0] );
	LZDecompressWram(
		gRaySceneHushRing_Map, sRayScene->tilemapBuffers[2] );
...
static void sub_81D9528( struct Sprite* sprite )
{
	...
	{
		sprite->data[7] =
			CreateSprite( &sUnknown_0862ACF0, 152, 132, 0 );
		gSprites[sprite->data[7]].oam.priority = 1;
		sprite->data[7] =
			CreateSprite( &sUnknown_0862ACF0, 224, 132, 0 );
		gSprites[sprite->data[7]].oam.priority = 1;
...
static void sub_81D961C( struct Sprite* sprite )
{
	...
	else
	{
		...
		case 352:
			sub_81D9274(
				FindTaskIdByFunc( Task_HandleRayChasesAway ) );
			break;
		}
	}
