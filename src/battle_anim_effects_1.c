...
static void AnimSolarbeamSmallOrbStep( struct Sprite* sprite )
{
	...
	else
	{
		if( sprite->data[5] > 0x7F )
			sprite->subpriority = GetBattlerSpriteSubpriority(
						      gBattleAnimTarget ) +
				1;
		else
			sprite->subpriority = GetBattlerSpriteSubpriority(
...
// Moves an orb from the target mon to the attacking mon in an arc-like
// fashion. arg 0: initial x pixel offset arg 1: initial y pixel offset arg 2:
// wave amplitude arg 3: wave period (lower means faster wave)
...
static void AnimSporeParticleStep( struct Sprite* sprite )
{
	...
	sprite->pos2.y = Cos( sprite->data[1], -3 ) +
		( ( sprite->data[2] += 24 ) >> 8 );
	...
	{
		u8 priority =
			GetBattlerSpriteBGPriority( gBattleAnimTarget ) + 1;
		if( priority > 3 )
			priority = 3;
...
static void AnimPetalDanceSmallFlowerStep( struct Sprite* sprite )
{
	if( !AnimTranslateLinear( sprite ) )
	{
		...
		if( ( u16 )( sprite->data[5] - 59 ) < 5 ||
			( u16 )( sprite->data[5] - 187 ) < 5 )
...
void AnimTranslateLinearSingleSineWave( struct Sprite* sprite )
{
	...
	{
		sprite->data[2] = GetBattlerSpriteCoord( gBattleAnimTarget,
					  BATTLER_COORD_X_2 ) +
			gBattleAnimArgs[2];
...
void sub_80FF458( u8 taskId )
{
	...
	else
	{
		...
		gSprites[spriteId].oam.priority = GetBattlerSpriteBGPriority(
			BATTLE_PARTNER( gBattleAnimTarget ) );
...
void AnimIngrainRoot( struct Sprite* sprite )
{
	if( !sprite->data[0] )
	{
		sprite->pos1.x = GetBattlerSpriteCoord(
			gBattleAnimAttacker, BATTLER_COORD_X_2 );
		sprite->pos1.y = GetBattlerSpriteCoord(
			gBattleAnimAttacker, BATTLER_COORD_Y );
		...
		if( sprite->pos1.y + sprite->pos2.y > 120 )
			sprite->pos1.y +=
				sprite->pos2.y + sprite->pos1.y - 120;
	}
...
{
	s16 attackerX = GetBattlerSpriteCoord(
		gBattleAnimAttacker, BATTLER_COORD_X_2 );
	s16 attackerY = GetBattlerSpriteCoord(
		gBattleAnimAttacker, BATTLER_COORD_Y_PIC_OFFSET );
	s16 targetX =
		GetBattlerSpriteCoord( gBattleAnimTarget, BATTLER_COORD_X_2 );
...
void AnimIngrainOrb( struct Sprite* sprite )
{
	...
	sprite->pos2.y =
		Sin( ( sprite->data[0] * 20 ) & 0xFF, sprite->data[2] );
...
static void sub_80FFAB4( struct Sprite* sprite )
{
	...
	sprite->pos2.y =
		Sin( sprite->data[0] + 128, 30 - sprite->data[1] * 8 );
...
static void sub_80FFB90( struct Sprite* sprite )
{
	...
	sprite->pos2.y =
		Sin( sprite->data[0] + 0x80, 30 - sprite->data[1] * 8 );
	if( sub_80FF9E0( sprite ) )
	{
		sprite->pos2.y  = zero;
		sprite->data[0] = zero;
...
static void AnimTrickBagStep2( struct Sprite* sprite )
{
	...
	else
	{
		sprite->data[2]++;
		sprite->data[1] =
			( gUnknown_08592700[sprite->data[0]][0] *
					gUnknown_08592700[sprite->data[0]][2] +
				sprite->data[1] ) &
			0xFF;
		if( !IsContest( ) )
		{
...
void sub_80FFFC0( u8 taskId )
{
	...
	task->data[10] = GetBattlerSpriteCoordAttr(
		gBattleAnimTarget, BATTLER_COORD_ATTR_WIDTH );
	task->data[11] = GetBattlerSpriteCoordAttr(
		gBattleAnimTarget, BATTLER_COORD_ATTR_HEIGHT );
	task->data[5] =
		( GetBattlerSide( gBattleAnimTarget ) == B_SIDE_OPPONENT )
		? 1
		: -1;
	task->data[9] = 56 - ( task->data[5] * 64 );
	task->data[8] = task->data[7] - task->data[9] + task->data[6];
	task->data[2] = CreateSprite( &gUnknown_08592778,
		task->data[8],
		task->data[9],
		task->data[4] );
	...
	gSprites[task->data[2]].data[5] =
		sub_8100504( &gSprites[task->data[2]] );
...
static void sub_8100128( u8 taskId )
{
	...
	case 3:
		...
		sprite->data[1] = sprite->pos1.x;
		sprite->data[2] = task->data[6] -
			( ( task->data[10] / 2 ) + 10 ) * task->data[5];
		sprite->data[3] = sprite->pos1.y;
		sprite->data[4] = task->data[7] -
			( ( task->data[11] / 2 ) + 10 ) * task->data[5];
		sprite->data[5]     = sub_8100504( sprite );
		task->data[3]       = 2;
		sprite->subpriority = task->data[4];
		StartSpriteAnim( sprite, task->data[3] );
...
static void sub_8100524( struct Task* task, u8 taskId )
{
	...
	if( task->data[14] > 0 )
	{
		...
		s16 spriteY;
		task->data[14] = 0;
		spriteX        = gSprites[task->data[2]].pos1.x +
			gSprites[task->data[2]].pos2.x;
		spriteY = gSprites[task->data[2]].pos1.y +
			gSprites[task->data[2]].pos2.y;
		spriteId = CreateSprite(
			&gUnknown_08592778, spriteX, spriteY, task->data[4] );
...
void sub_8100640( struct Sprite* sprite )
{
	...
	case 1:
		sprite->pos1.y = gBattleAnimArgs[0];
		sprite->oam.priority =
			GetBattlerSpriteBGPriority( battler ) + 1;
		break;
	case 2:
...
static void sub_810074C( struct Sprite* sprite )
{
	...
	sprite->data[7]++;
	sprite->pos2.y =
		( sprite->data[1] * gSineTable[sprite->data[0]] ) >> 8;
...
void sub_81007C4( u8 taskId )
{
	...
	case 0:
		task->data[8] =
			IndexOfSpritePaletteTag( ANIM_TAG_LEAF ) * 16 + 256;
		task->data[12] =
			IndexOfSpritePaletteTag( ANIM_TAG_RAZOR_LEAF ) * 16 +
			256;
		task->data[0]++;
		break;
	case 1:
		...
		{
			task->data[9] = 0;
			BlendPalette( task->data[8],
				16,
				task->data[10],
				gMagicalLeafBlendColors[task->data[11]] );
			BlendPalette( task->data[12],
				16,
				task->data[10],
				gMagicalLeafBlendColors[task->data[11]] );
			if( ++task->data[10] == 17 )
...
void sub_8100898( struct Sprite* sprite )
{
	...
	else
	{
		...
		y               = sprite->pos1.y;
		sprite->data[2] = y * 16;
		sprite->data[3] = ( sprite->data[5] - sprite->pos1.x ) * 16 /
			gBattleAnimArgs[4];
		sprite->data[4] = ( sprite->data[6] - sprite->pos1.y ) * 16 /
			gBattleAnimArgs[4];
...
void sub_8100B88( struct Sprite* sprite )
{
	...
	case 1:
		a = GetBattlerSpriteCoord(
			BATTLE_PARTNER( gBattleAnimTarget ), 0 );
		b = GetBattlerSpriteCoord(
			BATTLE_PARTNER( gBattleAnimTarget ), 1 );
		break;
	case 2:
...
void sub_8100EF0( struct Sprite* sprite )
{
	...
	if( GetBattlerSide( gBattleAnimAttacker ) == B_SIDE_PLAYER ||
		IsContest( ) )
		sprite->oam.priority =
			GetBattlerSpriteBGPriority( gBattleAnimAttacker ) + 1;
	else
		sprite->oam.priority =
			GetBattlerSpriteBGPriority( gBattleAnimAttacker );

	sprite->data[0] = gBattleAnimArgs[2];
	sprite->data[2] =
		( IndexOfSpritePaletteTag( ANIM_TAG_PROTECT ) << 4 ) + 0x100;
	sprite->data[7] = 16;
...
static void sub_8100FD4( struct Sprite* sprite )
{
	...
	if( sprite->data[7] > 6 && sprite->data[0] > 0 &&
		++sprite->data[6] > 1 )
	{
		sprite->data[6] = 0;
		sprite->data[7] -= 1;
		SetGpuReg( REG_OFFSET_BLDALPHA,
			BLDALPHA_BLEND(
				16 - sprite->data[7], sprite->data[7] ) );
	}
...
void sub_81010CC( struct Sprite* sprite )
{
	...
	sprite->pos1.y =
		GetBattlerSpriteCoord( gBattleAnimTarget, 3 ) + 0xFFE8;
	sprite->data[0] = 0;
	sprite->data[1] = 0;
	sprite->data[2] = 0;
	sprite->data[3] = 0;
...
void sub_810135C( struct Sprite* sprite )
{
	...
	{
		SetAverageBattlerPositions( battler,
			gBattleAnimArgs[6],
			&sprite->pos1.x,
			&sprite->pos1.y );
		SetAnimSpriteInitialXOffset( sprite, gBattleAnimArgs[0] );
	...
	else
	{
		if( !gBattleAnimArgs[6] )
		{
			...
			sprite->pos1.y = GetBattlerSpriteCoord( battler, 1 ) +
				gBattleAnimArgs[1];
		}
		else
		{
...
static void sub_81015D4( struct Sprite* sprite )
{
	...
	case 1:
		...
		sprite->data[0] = 8;
		sprite->data[2] = sprite->pos1.x +
			gUnknown_08592CD0[sprite->data[5] >> 8][0];
		sprite->data[4] = sprite->pos1.y +
			gUnknown_08592CD0[sprite->data[5] >> 8][1];
		sprite->callback = StartAnimLinearTranslation;
...
static void sub_81016B8( struct Sprite* sprite )
{
	...
	else
	{
		...
		sprite->data[0] = 6;
		sprite->data[2] =
			GetBattlerSpriteCoord( gBattleAnimTarget, 2 ) + a;
		sprite->data[4] =
			GetBattlerSpriteCoord( gBattleAnimTarget, 3 ) + b;
		sprite->callback = StartAnimLinearTranslation;
...
static void sub_8101774( struct Sprite* sprite )
{
	...
	BlendPalettes( sub_80A75AC( 1, 1, 1, 1, 1, 0, 0 ),
		sprite->data[1],
		RGB( 31, 31, 31 ) );
	if( sprite->data[1] == 16 )
	{
		...
		LoadPalette( &gPlttBufferUnfaded[0x108 + pal * 16],
			pal * 16 | 0x101,
			4 );
...
static void sub_81019E8( struct Sprite* sprite )
{
	if( sprite->data[0] == 0 )
	{
		...
		PrepareBattlerSpriteForRotScale(
			sprite->data[3], ST_OAM_OBJ_NORMAL );
		sprite->data[4] = ( sprite->data[6] = GetBattlerSide(
					    gBattleAnimAttacker ) )
			? 0x300
			: 0xFFFFFD00;
		sprite->data[5] = 0;
...
static void sub_8101BA0( struct Sprite* sprite )
{
	...
	case 0:
		...
		sprite->data[4] =
			( sprite->data[3] != B_SIDE_PLAYER ) ? 0x200 : -0x200;
		sprite->data[5] = 0;
		PrepareBattlerSpriteForRotScale(
			sprite->data[2], ST_OAM_OBJ_NORMAL );
		sprite->data[0]++;
		// fall through
	case 1:
		sprite->data[5] += sprite->data[4];
		SetSpriteRotScale(
			sprite->data[2], 0x100, 0x100, sprite->data[5] );
...
static void sub_8101D2C( u8 taskId )
{
	...
	case 1:
		if( task->data[3] )
		{
			task->data[4] += task->data[5];
			SetSpriteRotScale(
				task->data[0], 0x100, 0x100, task->data[4] );
...
{
	sprite->pos1.x =
		GetBattlerSpriteCoord( gBattleAnimTarget, 2 ) + 0xFFD0;
...
void sub_81020D8( struct Sprite* sprite )
{
	...
	else
	{
		sprite->pos1.x =
			...
			gBattleAnimArgs[1];
		sprite->pos1.y =
			GetBattlerSpriteCoord( gBattleAnimTarget, 1 ) +
			gBattleAnimArgs[2];
	}
...
void sub_81022D4( u8 taskId )
{
	...
	else
	{
		if( ++gTasks[taskId].data[0] == 4 )
		{
			...
			gTasks[taskId].data[1]++;
			SetGpuReg( REG_OFFSET_BLDALPHA,
				BLDALPHA_BLEND( 16 - gTasks[taskId].data[1],
					gTasks[taskId].data[1] ) );
...
void sub_81025C0( u8 taskId )
{
	...
	c                      = a | b;
	StorePointerInVars(
		&gTasks[taskId].data[14], &gTasks[taskId].data[15], (void*)c );
	b = b | ( 0x10000 << IndexOfSpritePaletteTag( ANIM_TAG_MOON ) );
	d = IndexOfSpritePaletteTag( ANIM_TAG_GREEN_SPARKLE );
	BeginNormalPaletteFade(
		( 0x10000 << d ) | b, 0, 0, 16, RGB( 27, 29, 31 ) );
...
void sub_8102BE8( u8 taskId )
{
	...
	for( j = 1; j < ARRAY_COUNT( gUnknown_08593020 ); j++ )
	{
		...
		{
			index = ( index << 4 ) + 0x100;
			for( i = 1; i < ARRAY_COUNT( gUnknown_08593020[0] );
				i++ )
				gPlttBufferFaded[index + i] =
					gUnknown_08593020[j][i];
		}
	}
	DestroyAnimVisualTask( taskId );
}
...
void sub_8102CD4( struct Sprite* sprite )
{
	...
	if( ( index = IndexOfSpritePaletteTag(
		      gUnknown_08593020[gBattleAnimArgs[1]][0] ) ) != 0xFF )
	...
	else
	{
		if( sprite->data[3] && ++sprite->data[2] > sprite->data[3] )
		{
			...
			index = IndexOfSpritePaletteTag(
				gUnknown_08593020[sprite->data[1]][0] );
			if( index != 0xFF )
				sprite->oam.paletteNum = index;
		}
	}
...
void sub_8103028( struct Sprite* sprite )
{
	...
	index = IndexOfSpritePaletteTag(
		gUnknown_08593020[gBattleAnimArgs[2]][0] );
	if( index != 0xFF )
		sprite->oam.paletteNum = index;
...
void sub_810310C( u8 battler, struct Sprite* sprite )
{
	...
	sprite->pos1.y = GetBattlerSpriteCoord( battler, 3 ) -
		(s16)GetBattlerSpriteCoordAttr(
			battler, BATTLER_COORD_ATTR_HEIGHT ) /
			4;
}
...
void sub_8103284( struct Sprite* sprite )
{
	...
	sprite->pos1.x = GetBattlerSpriteCoord( battler, 0 );
	sprite->pos1.y =
		GetBattlerSpriteCoordAttr( battler, BATTLER_COORD_ATTR_TOP );
	if( sprite->pos1.y <= 9 )
		sprite->pos1.y = 10;
