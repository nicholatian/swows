...
static void MoveLeftHeader(
	struct Sprite* sprite, int startX, int endX, int duration );
...
const u16 gSpinningPokenavPaletteData[] =
	INCBIN_U16( "graphics/pokenav/icon2.gbapal" );
const u32 gSpinningPokenavGfx[] =
	INCBIN_U32( "graphics/pokenav/icon2.4bpp.lz" );
const u32 gUnused_SpinningPokenavGfx2[] =
	INCBIN_U32( "graphics/pokenav/icon2_unused.4bpp.lz" );
...
const u8 gMenuButtonReminderColor[3] = {4, 1, 2};
...
bool32 InitPokenavMainMenu( void )
{
	...
	structPtr =
		AllocSubstruct( 0, sizeof( struct PokenavMainMenuResources ) );
	if( structPtr == NULL )
		return FALSE;

	ResetSpriteData( );
	FreeAllSpritePalettes( );
	structPtr->currentTaskId =
		CreateLoopedTask( LoopedTask_InitPokenavMenu, 1 );
	return TRUE;
}
...
void sub_81C7880( void )
{
	struct PokenavMainMenuResources* structPtr = GetSubstructPtr( 0 );
	structPtr->currentTaskId =
		CreateLoopedTask( LoopedTask_ScrollMenuHeaderDown, 4 );
}
...
void sub_81C78A0( void )
{
	struct PokenavMainMenuResources* structPtr = GetSubstructPtr( 0 );
	structPtr->currentTaskId =
		CreateLoopedTask( LoopedTask_ScrollMenuHeaderUp, 4 );
}
...
void CopyPaletteIntoBufferUnfaded(
	const u16* palette, u32 bufferOffset, u32 size )
{
...
{
	CpuFill16( a1,
		gPlttBufferFaded + 0x100 + ( a0 * 16 ),
		16 * sizeof( u16 ) );
}

NAKED
void sub_81C79BC(
	const u16* a0, const u16* a1, u32 a2, u32 a3, u32 a4, u16* a5 )
{
	asm( ".syntax unified\n\
    push {r4-r7,lr}\n\
    mov r7, r10\n\
...
void sub_81C7AC0( int a0 )
{
	...
	case 0:
		BeginNormalPaletteFade(
			structPtr->unk14, -2, 0, 16, RGB_BLACK );
		break;
	case 1:
		BeginNormalPaletteFade(
			structPtr->unk14, -2, 16, 0, RGB_BLACK );
		break;
	case 2:
...
void sub_81C7B40( void ) { BlendPalettes( 0xFFFEFFFE, 16, RGB_BLACK ); }
...
static void sub_81C7B74( void )
{
	...
	CopyWindowToVram( structPtr->unk10, 3 ); // TODO: Use a defined
						 // constant here.
}
...
void sub_81C7BA4( u32 helpBarIndex )
{
	...
	sub_81C7BF8( structPtr->unk10 );
	AddTextPrinterParameterized3( structPtr->unk10,
		1,
		0,
		1,
		gMenuButtonReminderColor,
		0,
		sHelpBarTexts[helpBarIndex] );
}
...
static void InitHoennMapHeaderSprites( void )
{
	...
	for( i = 0; i < (int)ARRAY_COUNT( structPtr->leftHeaderSprites ); i++ )
	{
		spriteId = CreateSprite(
			&sPokenavLeftHeaderHoennMapSpriteTemplate, 0, 0, 1 );
		...
		structPtr->submenuLeftHeaderSprites[i]->oam.tileNum +=
			( i * 8 ) + 64;
	}
}
...
void LoadLeftHeaderGfxForIndex( u32 arg0 )
{
	if( arg0 < POKENAV_GFX_SUBMENUS_START )
		...
		LoadLeftHeaderGfxForSubMenu(
			arg0 - POKENAV_GFX_SUBMENUS_START );
}
...
void sub_81C7E14( u32 arg0 )
{
	...
	if( arg0 == 4 )
		structPtr->leftHeaderSprites[1]->oam.tileNum =
			GetSpriteTileStartByTag( 2 ) + 32;
	else
		structPtr->leftHeaderSprites[1]->oam.tileNum =
			GetSpriteTileStartByTag( 2 ) + 64;
}
...
static void LoadLeftHeaderGfxForMenu( u32 index )
{
	...
	size      = GetDecompressedDataSize(
                sPokenavMenuLeftHeaderSpriteSheets[index].data );
	LoadPalette( &gPokenavLeftHeader_Pal[tag * 16],
		( IndexOfSpritePaletteTag( 1 ) * 16 ) + 0x100,
		0x20 );
	LZ77UnCompWram( sPokenavMenuLeftHeaderSpriteSheets[index].data,
		gDecompressionBuffer );
	RequestDma3Copy( gDecompressionBuffer,
		(void*)VRAM + 0x10000 + ( GetSpriteTileStartByTag( 2 ) * 32 ),
		size,
		1 );
	structPtr->leftHeaderSprites[1]->oam.tileNum =
		GetSpriteTileStartByTag( 2 ) +
		sPokenavMenuLeftHeaderSpriteSheets[index].size;

	if( index == POKENAV_GFX_MAP_MENU_UNK0 ||
		index == POKENAV_GFX_MAP_MENU_UNK1 )
...
static void LoadLeftHeaderGfxForSubMenu( u32 arg0 )
{
	...
	size = GetDecompressedDataSize(
		sPokenavSubMenuLeftHeaderSpriteSheets[arg0].data );
	LoadPalette( &gPokenavLeftHeader_Pal[tag * 16],
		( IndexOfSpritePaletteTag( 2 ) * 16 ) + 0x100,
		0x20 );
	LZ77UnCompWram( sPokenavSubMenuLeftHeaderSpriteSheets[arg0].data,
...
bool32 sub_81C8010( void )
{
	...
	if( structPtr->leftHeaderSprites[0]->callback == SpriteCallbackDummy &&
		structPtr->submenuLeftHeaderSprites[0]->callback ==
			SpriteCallbackDummy )
		return FALSE;
	else
		return TRUE;
...
static void ShowLeftHeaderSprites( u32 startY, bool32 isOnRightSide )
{
	...
	for( i = 0; i < (int)ARRAY_COUNT( structPtr->leftHeaderSprites ); i++ )
	{
		...
		MoveLeftHeader(
			structPtr->leftHeaderSprites[i], start, end, 12 );
	}
}
...
static void ShowLeftHeaderSubmenuSprites( u32 startY, bool32 isOnRightSide )
{
	...
	for( i = 0;
		i < (int)ARRAY_COUNT( structPtr->submenuLeftHeaderSprites );
		i++ )
	{
		structPtr->submenuLeftHeaderSprites[i]->pos1.y = startY;
		MoveLeftHeader( structPtr->submenuLeftHeaderSprites[i],
			start,
			end,
			12 );
	}
