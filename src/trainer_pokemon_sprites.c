...
static const struct OamData gUnknown_0860B06C = {.affineMode = 1,
...
static bool16 DecompressPic( u16 species,
	u32 personality,
	bool8 isFrontPic,
	u8* dest,
	bool8 isTrainer,
	...
	else
	{
		if( isFrontPic )
			DecompressPicFromTable(
				&gTrainerFrontPicTable[species],
				dest,
				species );
		else
			DecompressPicFromTable( &gTrainerBackPicTable[species],
				dest,
				species );
	}
	return FALSE;
...
static bool16 DecompressPic_HandleDeoxys( u16 species,
	u32 personality,
	bool8 isFrontPic,
	u8* dest,
	bool8 isTrainer )
{
	return DecompressPic(
		species, personality, isFrontPic, dest, isTrainer, FALSE );
}

static void LoadPicPaletteByTagOrSlot( u16 species,
	u32 otId,
	u32 personality,
	u8 paletteSlot,
	u16 paletteTag,
	bool8 isTrainer )
{
	...
	else
	{
		if( paletteTag == 0xFFFF )
		{
			sCreatingSpriteTemplate.paletteTag = 0xFFFF;
			LoadCompressedPalette(
				gTrainerFrontPicPaletteTable[species].data,
				0x100 + paletteSlot * 0x10,
				0x20 );
		}
		else
		{
			sCreatingSpriteTemplate.paletteTag = paletteTag;
			LoadCompressedSpritePalette(
				&gTrainerFrontPicPaletteTable[species] );
		}
	}
}

static void LoadPicPaletteBySlot( u16 species,
	u32 otId,
	u32 personality,
	u8 paletteSlot,
	bool8 isTrainer )
	...
	bool8 ignoreDeoxys )
{
	...
	LoadPicPaletteByTagOrSlot( species,
		otId,
		personality,
		paletteSlot,
		paletteTag,
...
static u16 CreatePicSprite_HandleDeoxys( u16 species,
	...
	bool8 isTrainer )
{
	return CreatePicSprite( species,
		otId,
		personality,
		isFrontPic,
		x,
...
u16 CreatePicSprite2( u16 species,
	u32 otId,
	u32 personality,
	u8 flags,
	s16 x,
	...
	u16 paletteTag )
{
	...
	if( DecompressPic(
		    species, personality, flags, framePics, FALSE, FALSE ) )
	{
		// debug trap?
		return 0xFFFF;
	}
	...
	{
		sCreatingSpriteTemplate.oam = &gUnknown_0860B064;
		sCreatingSpriteTemplate.affineAnims =
			gDummySpriteAffineAnimTable;
	}
	sCreatingSpriteTemplate.callback = DummyPicSpriteCallback;
	LoadPicPaletteByTagOrSlot(
		species, otId, personality, paletteSlot, paletteTag, FALSE );
	spriteId = CreateSprite( &sCreatingSpriteTemplate, x, y, 0 );
	if( paletteTag == 0xFFFF )
	{
		gSprites[spriteId].oam.paletteNum = paletteSlot;
	...
	bool8 isTrainer )
{
	...
	LoadPicPaletteBySlot(
		species, otId, personality, paletteSlot, isTrainer );
	return 0;
}

static u16 sub_818D6CC( u16 species,
	u32 otId,
	u32 personality,
	bool8 isFrontPic,
	u16 destX,
	...
	bool8 isTrainer )
{
	...
	framePics = Alloc( 4 * 0x800 );
	if( framePics &&
		!DecompressPic_HandleDeoxys( species,
			personality,
			isFrontPic,
			framePics,
			isTrainer ) )
...
u16 CreateMonPicSprite_HandleDeoxys( u16 species,
	...
	u16 paletteTag )
{
	return CreateMonPicSprite( species,
		otId,
		personality,
		isFrontPic,
		x,
...
u16 CreateTrainerPicSprite( u16 species,
	bool8 isFrontPic,
	s16 x,
	s16 y,
	u8 paletteSlot,
	u16 paletteTag )
{
	return CreatePicSprite_HandleDeoxys( species,
		0,
		0,
		isFrontPic,
		x,
...
{
	return sub_818D65C(
		species, 0, 0, isFrontPic, paletteSlot, windowId, TRUE );
}

u16 sub_818D938( u16 species,
	bool8 isFrontPic,
	u16 destX,
	u16 destY,
	u8 paletteSlot,
