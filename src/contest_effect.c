...
static void ContestEffect_UserMoreEasilyStartled( void )
{
	eContestantStatus[eContestResources8.contestant].moreEasilyStartled =
		TRUE;
	SetContestantEffectStringID(
		eContestResources8.contestant, CONTEST_STRING_MORE_CONSCIOUS );
}
...
static void ContestEffect_GreatAppealButNoMoreMoves( void )
{
	...
	SetContestantEffectStringID(
		eContestResources8.contestant, CONTEST_STRING_NO_APPEAL );
}

// Can be used repeatedly without boring the JUDGE.
static void ContestEffect_RepetitionNotBoring( void )
{
	eContestantStatus[eContestResources8.contestant].usedRepeatableMove =
		TRUE;
	eContestantStatus[eContestResources8.contestant].disappointedRepeat =
		FALSE;
...
static void ContestEffect_AvoidStartleOnce( void )
{
	...
	SetContestantEffectStringID(
		eContestResources8.contestant, CONTEST_STRING_SETTLE_DOWN );
}
...
static void ContestEffect_AvoidStartle( void )
{
	...
	SetContestantEffectStringID( eContestResources8.contestant,
...
static void ContestEffect_AvoidStartleSlightly( void )
{
	...
	SetContestantEffectStringID(
		eContestResources8.contestant, CONTEST_STRING_LESS_AWARE );
}
...
static void ContestEffect_UserLessEasilyStartled( void )
{
	...
	SetContestantEffectStringID(
		eContestResources8.contestant, CONTEST_STRING_STOPPED_CARING );
}
...
static void ContestEffect_StartleFrontMon( void )
{
	...
	if( eContestResources8.turnOrder[a] != 0 )
	{
		...
		for( i = 0; i < 4; i++ )
		{
			if( eContestResources8.turnOrder[a] - 1 ==
				eContestResources8.turnOrder[i] )
				break;
		}
	...
	if( idx == 0 )
		SetContestantEffectStringID2( eContestResources8.contestant,
...
static void ContestEffect_StartlePrevMons( void )
{
	...
	if( eContestResources8.turnOrder[a] != 0 )
	{
		...
		for( i = 0, j = 0; i < 4; i++ )
		{
			if( eContestResources8.turnOrder[a] >
				eContestResources8.turnOrder[i] )
...
static void ContestEffect_StartlePrevMons2( void )
{
	...
	if( turnOrder != 0 )
	{
		...
		for( i = 0; i < 4; i++ )
		{
			if( eContestResources8.turnOrder[contestant] >
				eContestResources8.turnOrder[i] )
			{
				u8 rval, jam;
		...
		for( i = 0; i < 4; i++ )
		{
				...
				CanUnnerveContestant( i ) )
			{
				eContestantStatus[i].hasJudgesAttention =
					FALSE;
				eContestantStatus[i]
					.judgesAttentionWasRemoved = TRUE;
				SetContestantEffectStringID(
					i, CONTEST_STRING_JUDGE_LOOK_AWAY2 );
				hitAny = TRUE;
			}
		}
	}
	SetContestantEffectStringID(
		eContestResources8.contestant, CONTEST_STRING_DAZZLE_ATTEMPT );
	if( !hitAny )
	{
		SetContestantEffectStringID2( eContestResources8.contestant,
			CONTEST_STRING_MESSED_UP2 );
...
static void ContestEffect_MakeFollowingMonNervous( void )
{
	...
	if( eContestResources8.turnOrder[eContestResources8.contestant] != 3 )
	{
		...
		for( i = 0; i < 4; i++ )
		{
			if( eContestResources8.turnOrder[eContestResources8
								 .contestant] +
					1 ==
				eContestResources8.turnOrder[i] )
			{
				if( CanUnnerveContestant( i ) )
				{
					...
					SetContestantEffectStringID(
						i, CONTEST_STRING_NERVOUS );
					hitAny = TRUE;
				}
				else
				{
					SetContestantEffectStringID(
						i, CONTEST_STRING_UNAFFECTED );
					hitAny = TRUE;
				}
			}
		}
...
static void ContestEffect_MakeFollowingMonsNervous( void )
{
		...
		if( eContestantStatus[i].hasJudgesAttention &&
			...
			oddsMod[i] =
				gComboStarterLookupTable
					[gContestMoves[eContestantStatus[i]
							       .prevMove]
							.comboStarterId] *
				10;
		else
	...
	if( odds[0] != 0 )
	{
		...
		{
			if( Random( ) % 100 <
				odds[i] + oddsMod[contestantIds[i]] )
			{
...
static void ContestEffect_WorsenConditionOfPrevMons( void )
{
	...
	SetContestantEffectStringID(
		eContestResources8.contestant, CONTEST_STRING_TAUNT_WELL );
	if( numHit == 0 )
		SetContestantEffectStringID2( eContestResources8.contestant,
...
static void ContestEffect_BadlyStartlesMonsInGoodCondition( void )
{
	...
	for( i = 0; i < 4; i++ )
	{
		if( eContestResources8
				.turnOrder[eContestResources8.contestant] >
			eContestResources8.turnOrder[i] )
		{
	...
	SetContestantEffectStringID(
		eContestResources8.contestant, CONTEST_STRING_JAM_WELL );
	if( numHit == 0 )
		SetContestantEffectStringID2( eContestResources8.contestant,
			CONTEST_STRING_IGNORED );
}
...
static void ContestEffect_BetterIfFirst( void )
{
	...
	{
		u16 move = eContestantStatus[eContestResources8.contestant]
				   .currMove;
		eContestantStatus[eContestResources8.contestant].appeal2 +=
...
static void ContestEffect_AppealAsGoodAsPrevOnes( void )
{
	...
	eContestantStatus[eContestResources8.contestant]
		.appeal2 = RoundTowardsZero(
		eContestantStatus[eContestResources8.contestant].appeal2 );
}
...
{
	u8 whichTurn =
		eContestResources8.turnOrder[eContestResources8.contestant];
	if( whichTurn == 0 )
...
static void ContestEffect_BetterIfSameType( void )
{
	...
	while( 1 )
	{
		...
		if( eContestantStatus[j].noMoreTurns ||
			eContestantStatus[j].nervous ||
			...
			if( eContestResources8.turnOrder[eContestResources8
				...
				gContestMoves[move].contestCategory !=
					gContestMoves[eContestantStatus[i]
						...
						.contestCategory )
			{
				eContestantStatus[eContestResources8
							  .contestant]
					.appeal2 +=
					gContestEffects[gContestMoves[move]
								.effect]
						.appeal *
					2;
				SetContestantEffectStringID(
					eContestResources8.contestant,
	...
	if( eContestResources8.turnOrder[eContestResources8.contestant] != 0 )
	{
		...
		for( i = 0; i < 4; i++ )
		{
			if( eContestResources8.turnOrder[eContestResources8
				...
				eContestResources8.turnOrder[i] )
			{
				if( eContestantStatus[eContestResources8
							      .contestant]
						.appeal2 >
				...
				else if( eContestantStatus[eContestResources8
								   .contestant]
						 .appeal2 <
					eContestantStatus[i].appeal2 )
				{
...
static void ContestEffect_ImproveConditionPreventNervousness( void )
{
	...
	{
		eContestantStatus[eContestResources8.contestant].condition +=
			10;
		eContestantStatus[eContestResources8.contestant].conditionMod =
			1;
		SetContestantEffectStringID( eContestResources8.contestant,
...
static void ContestEffect_NextAppealEarlier( void )
{
	...
	if( eContest.turnNumber != 4 )
	{
		...
		eContestantStatus[eContestResources8.contestant].turnOrderMod =
			1;
...
static void ContestEffect_BadlyStartleMonsWithGoodAppeals( void )
{
	...
	for( i = 0; i < 4; i++ )
	{
			...
			eContestResources8.turnOrder[i] )
		{
			...
			{
				eContestResources8.jam =
					eContestantStatus[i].appeal2 / 2;
				eContestResources8.jam =
					RoundUp( eContestResources8.jam );
			}
			else
				eContestResources8.jam = 10;
			eContestResources8.jamQueue[0] = i;
...
static void ContestEffect_DontExciteAudience( void )
{
	if( !eContestResources10.excitementFrozen )
	{
		...
		eContestResources10.excitementFreezer =
			eContestResources8.contestant;
		SetContestantEffectStringID( eContestResources8.contestant,
...
static void JamByMoveCategory( u8 category )
{
	...
	for( i = 0; i < 4; i++ )
	{
		...
		{
			if( category ==
				gContestMoves[eContestantStatus[i].currMove]
					.contestCategory )
				eContestResources8.jam = 40;
			else
				eContestResources8.jam = 10;
	...
	for( i = 0; eContestResources8.jamQueue[i] != 0xFF; i++ )
	{
		...
		if( CanUnnerveContestant( contestant ) )
		{
			...
			else
			{
				...
				{
					JamContestant( contestant,
						eContestResources8.jam2 );
					SetStartledString( contestant,
						eContestResources8.jam2 );
					jamBuffer[contestant] =
						eContestResources8.jam2;
				}
			}
		}
