...
static u8 CheckPathBetweenTrainerAndPlayer(
	struct EventObject* trainerObj, u8 approachDistance, u8 direction );
...
static void SetIconSpriteData(
	struct Sprite* sprite, u16 fldEffId, u8 spriteAnimNum );

static u8 GetTrainerApproachDistanceSouth(
	struct EventObject* trainerObj, s16 range, s16 x, s16 y );
static u8 GetTrainerApproachDistanceNorth(
	struct EventObject* trainerObj, s16 range, s16 x, s16 y );
static u8 GetTrainerApproachDistanceWest(
	struct EventObject* trainerObj, s16 range, s16 x, s16 y );
static u8 GetTrainerApproachDistanceEast(
	struct EventObject* trainerObj, s16 range, s16 x, s16 y );

static bool8 sub_80B4178(
	u8 taskId, struct Task* task, struct EventObject* trainerObj );
static bool8 TrainerExclamationMark(
	u8 taskId, struct Task* task, struct EventObject* trainerObj );
static bool8 WaitTrainerExclamationMark(
	u8 taskId, struct Task* task, struct EventObject* trainerObj );
static bool8 TrainerMoveToPlayer(
	u8 taskId, struct Task* task, struct EventObject* trainerObj );
static bool8 PlayerFaceApproachingTrainer(
	u8 taskId, struct Task* task, struct EventObject* trainerObj );
static bool8 WaitPlayerFaceApproachingTrainer(
	u8 taskId, struct Task* task, struct EventObject* trainerObj );
static bool8 RevealDisguisedTrainer(
	u8 taskId, struct Task* task, struct EventObject* trainerObj );
static bool8 WaitRevealDisguisedTrainer(
	u8 taskId, struct Task* task, struct EventObject* trainerObj );
static bool8 RevealHiddenTrainer(
	u8 taskId, struct Task* task, struct EventObject* trainerObj );
static bool8 PopOutOfAshHiddenTrainer(
	u8 taskId, struct Task* task, struct EventObject* trainerObj );
static bool8 JumpInPlaceHiddenTrainer(
	u8 taskId, struct Task* task, struct EventObject* trainerObj );
static bool8 WaitRevealHiddenTrainer(
	u8 taskId, struct Task* task, struct EventObject* trainerObj );
...
static const u8 sEmotion_ExclamationMarkGfx[] =
	INCBIN_U8( "graphics/misc/emotion_exclamation.4bpp" );
static const u8 sEmotion_QuestionMarkGfx[] =
	INCBIN_U8( "graphics/misc/emotion_question.4bpp" );
static const u8 sEmotion_HeartGfx[] =
	INCBIN_U8( "graphics/misc/emotion_heart.4bpp" );
...
bool8 CheckForTrainersWantingBattle( void )
{
	...
	for( i = 0; i < EVENT_OBJECTS_COUNT; i++ )
	{
		...
		if( !gEventObjects[i].active )
			continue;
		if( gEventObjects[i].trainerType != 1 &&
			gEventObjects[i].trainerType != 3 )
			continue;
		...
		if( GetMonsStateToDoubles_2( ) !=
			0 ) // one trainer found and cant
			    // have a double battle
			break;
	}
	...
	if( gNoOfApproachingTrainers == 1 )
	{
		...
		ConfigureAndSetUpOneTrainerBattle(
			gApproachingTrainers[gNoOfApproachingTrainers - 1]
				.eventObjectId,
			gApproachingTrainers[gNoOfApproachingTrainers - 1]
				.trainerScriptPtr );
		gUnknown_030060AC = 1;
	...
	{
		ResetTrainerOpponentIds( );
		for( i = 0; i < gNoOfApproachingTrainers;
			i++, gApproachingTrainerId++ )
		{
			ConfigureTwoTrainersBattle(
				gApproachingTrainers[i].eventObjectId,
...
static u8 CheckTrainer( u8 eventObjectId )
{
	...
	if( InTrainerHill( ) == TRUE )
		...
		scriptPtr = GetEventObjectScriptPointerByEventObjectId(
			eventObjectId );
	...
	approachDistance =
		GetTrainerApproachDistance( &gEventObjects[eventObjectId] );
	...
	if( approachDistance != 0 )
	{
		...
		gApproachingTrainers[gNoOfApproachingTrainers].eventObjectId =
			eventObjectId;
		gApproachingTrainers[gNoOfApproachingTrainers]
			.trainerScriptPtr = scriptPtr;
		gApproachingTrainers[gNoOfApproachingTrainers].radius =
			approachDistance;
		TrainerApproachPlayer(
			&gEventObjects[eventObjectId], approachDistance - 1 );
...
static u8 GetTrainerApproachDistance( struct EventObject* trainerObj )
{
	...
	if( trainerObj->trainerType == 1 ) // can only see in one direction
	{
		approachDistance = sDirectionalApproachDistanceFuncs
			[trainerObj->facingDirection - 1]( trainerObj,
				trainerObj->trainerRange_berryTreeId,
				x,
				y );
		return CheckPathBetweenTrainerAndPlayer( trainerObj,
			approachDistance,
	...
	else // can see in all directions
	{
		...
		{
			approachDistance =
				sDirectionalApproachDistanceFuncs[i](
					trainerObj,
					trainerObj->trainerRange_berryTreeId,
					x,
					y );
...
// Returns how far west the player is from trainer. 0 if out of trainer's
// sight.
static u8 GetTrainerApproachDistanceWest(
	struct EventObject* trainerObj, s16 range, s16 x, s16 y )
{
...
// Returns how far east the player is from trainer. 0 if out of trainer's
// sight.
static u8 GetTrainerApproachDistanceEast(
	struct EventObject* trainerObj, s16 range, s16 x, s16 y )
{
	...
	struct EventObject* trainerObj, u8 approachDistance, u8 direction )
{
	...
	for( i = 0; i < approachDistance - 1;
		i++, MoveCoords( direction, &x, &y ) )
	{
		collision = GetCollisionFlagsAtCoords(
			trainerObj, x, y, direction );
...
static void Task_RunTrainerSeeFuncList( u8 taskId )
{
	...
	struct EventObject* trainerObj =
		&gEventObjects[task->tTrainerEventObjectId];
	...
	{
		while( sTrainerSeeFuncList[task->tFuncId](
	...
	u8 taskId, struct Task* task, struct EventObject* trainerObj )
{
	...
	EventObjectGetLocalIdAndMap( trainerObj,
		&gFieldEffectArguments[0],
		&gFieldEffectArguments[1],
		&gFieldEffectArguments[2] );
	FieldEffectStart( FLDEFF_EXCLAMATION_MARK_ICON );
	direction =
		GetFaceDirectionMovementAction( trainerObj->facingDirection );
	...
	u8 taskId, struct Task* task, struct EventObject* trainerObj )
{
		...
		EventObjectClearHeldMovementIfFinished( trainerObj ) )
	{
		...
		{
			EventObjectSetHeldMovement( trainerObj,
		...
		else
		{
			EventObjectSetHeldMovement(
				trainerObj, MOVEMENT_ACTION_FACE_PLAYER );
			task->tFuncId++;
		}
	}
	return FALSE;
	...
	u8 taskId, struct Task* task, struct EventObject* trainerObj )
{
	...
	SetTrainerMovementType( trainerObj,
		GetTrainerFacingDirectionMovementType(
			trainerObj->facingDirection ) );
	TryOverrideTemplateCoordsForEventObject( trainerObj,
		GetTrainerFacingDirectionMovementType(
			trainerObj->facingDirection ) );
	...
	EventObjectSetHeldMovement(
		...
		GetFaceDirectionMovementAction( GetOppositeDirection(
			trainerObj->facingDirection ) ) );
	task->tFuncId++;
	return FALSE;
}
	...
	u8 taskId, struct Task* task, struct EventObject* trainerObj )
{
	if( EventObjectCheckHeldMovementStatus( trainerObj ) )
	{
		...
		gFieldEffectArguments[2] =
			gSprites[trainerObj->spriteId].subpriority - 1;
		gFieldEffectArguments[3] = 2;
		task->tOutOfAshSpriteId =
...
static void sub_80B44C8( u8 taskId )
{
	...
	if( task->data[0] == 3 &&
		!FieldEffectActiveListContains( FLDEFF_POP_OUT_OF_ASH ) )
	{
		SetTrainerMovementType( eventObj,
			GetTrainerFacingDirectionMovementType(
				eventObj->facingDirection ) );
		TryOverrideTemplateCoordsForEventObject( eventObj,
			GetTrainerFacingDirectionMovementType(
				eventObj->facingDirection ) );
		DestroyTask( taskId );
	}
...
void sub_80B4578( struct EventObject* var )
{
	StoreWordInTwoHalfwords(
		&gTasks[CreateTask( sub_80B44C8, 0 )].data[1], (u32)var );
}
	...
	if( gNoOfApproachingTrainers == 2 )
	{
		if( gApproachingTrainerId == 0 )
		{
			...
			FreezeEventObjectsExceptOne(
				gApproachingTrainers[1].eventObjectId );
		}
		else
		{
			gApproachingTrainerId = 0;
...
u8 FldEff_ExclamationMarkIcon( void )
{
	...
	spriteId = CreateSpriteAtEnd(
		&sSpriteTemplate_ExclamationQuestionMark, 0, 0, 0x53 );

	if( spriteId != MAX_SPRITES )
		SetIconSpriteData(
			&gSprites[spriteId], FLDEFF_EXCLAMATION_MARK_ICON, 0 );
...
u8 FldEff_QuestionMarkIcon( void )
{
	...
	LoadSpritePalette( &sEventObjectSpritePalettes[8] );
	spriteId = CreateSpriteAtEnd(
		&sSpriteTemplate_ExclamationQuestionMark, 0, 0, 0x52 );

	if( spriteId != MAX_SPRITES )
		SetIconSpriteData(
			&gSprites[spriteId], FLDEFF_QUESTION_MARK_ICON, 1 );
...
static void SpriteCB_TrainerIcons( struct Sprite* sprite )
{
	...
	if( TryGetEventObjectIdByLocalIdAndMap( sprite->sLocalId,
		    sprite->sMapNum,
		    sprite->sMapGroup,
		    &eventObjId ) ||
		sprite->animEnded )
	...
	{
		struct Sprite* eventObjSprite =
			&gSprites[gEventObjects[eventObjId].spriteId];
...
void sub_80B4808( void )
{
	...
	{
		trainerObj =
			&gEventObjects[gApproachingTrainers[gUnknown_03006080]
					       .eventObjectId];
		gUnknown_03006084[0] = GetFaceDirectionMovementAction(
			GetOppositeDirection( trainerObj->facingDirection ) );
		gUnknown_03006084[1] = 0xFE;
		ScriptMovement_StartObjectMovementScript( 0xFF,
