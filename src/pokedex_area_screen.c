...
struct PokeDexAreaScreen
{
	...
	/*0x00E*/ u16 species;
	/*0x010*/ struct PokeDexAreaScreenMapIdentity
		overworldAreasWithMons[0x40];
...
// Only some parts of this array are acutally used, because corner flags that
// overlap with edge flags are cancelled out before lookup. For example,
// GLOW_TILE_BOTTOM_RIGHT | GLOW_TILE_RIGHT will never be read.
//
// The rest of the bytes seem to be old data from before the cancellation was
// implemented. Most of them line up as you would expect ([BOTTOM_RIGHT |
// RIGHT] has the same value as [RIGHT]).
//
// Any unreachable entries are simply listed in order, without the fancy
// "[FLAGS] = 0xXX" notation.
static const u8 sAreaGlowTilemapMapping[] = {
	...
	[GLOW_TILE_BOTTOM | GLOW_TILE_TOP | GLOW_TILE_RIGHT | GLOW_TILE_LEFT] =
		0x0f,
	...
	0x23,
	0x03,
	[GLOW_TILE_BOTTOM_LEFT | GLOW_TILE_BOTTOM_RIGHT | GLOW_TILE_TOP] =
		0x28,
	0x2d,
	0x2c,
	0x07,
	...
	0x0e,
	0x0f,
	[GLOW_TILE_TOP_LEFT | GLOW_TILE_TOP_RIGHT | GLOW_TILE_BOTTOM_RIGHT] =
		0x1b,
	0x22,
	0x24,
	0x03,
	...
	0x0e,
	0x0f,
	[GLOW_TILE_TOP_LEFT | GLOW_TILE_BOTTOM_LEFT | GLOW_TILE_BOTTOM_RIGHT] =
		0x1d,
	0x20,
	0x25,
	0x03,
...
static bool8 DrawAreaGlow( void )
{
	...
	case 2:
		decompress_and_copy_tile_data_to_vram(
			2, sAreaGlow_Gfx, 0, 0, 0 );
		LoadBgTilemap(
			2, sPokedexAreaScreen->areaGlowTilemap, 0x500, 0 );
		break;
	case 3:
		if( !free_temp_tile_data_buffers_if_possible( ) )
		{
			CpuCopy32( sAreaGlow_Pal,
				gPlttBufferUnfaded + 0xA0,
				0x20 );
...
static void FindMapsWithMon( u16 species )
{
	...
	if( species != roamer->species )
	{
		...
		for( i = 0; i < ARRAY_COUNT( sSpeciesHiddenFromAreaScreen );
			i++ )
		{
		...
		for( i = 0; sFeebasData[i][0] != NUM_SPECIES; i++ )
		{
			if( species == sFeebasData[i][0] )
			{
				...
				case MAP_GROUP_OVERWORLD_MONS:
					SetAreaHasMon( sFeebasData[i][1],
						sFeebasData[i][2] );
					break;
				case MAP_GROUP_SPECIAL_MONS_1:
				case MAP_GROUP_SPECIAL_MONS_2:
					SetSpecialMapHasMon( sFeebasData[i][1],
						sFeebasData[i][2] );
					break;
				}
			}
	...
	else
	{
		...
		{
			GetRoamerLocation(
				&sPokedexAreaScreen->overworldAreasWithMons[0]
					 .mapGroup,
				&sPokedexAreaScreen->overworldAreasWithMons[0]
					 .mapNum );
			sPokedexAreaScreen->overworldAreasWithMons[0]
...
static void SetAreaHasMon( u16 mapGroup, u16 mapNum )
{
	...
	{
		sPokedexAreaScreen
			->overworldAreasWithMons[sPokedexAreaScreen
							 ->numOverworldAreas]
			.mapGroup = mapGroup;
		sPokedexAreaScreen
			->overworldAreasWithMons[sPokedexAreaScreen
...
static void SetSpecialMapHasMon( u16 mapGroup, u16 mapNum )
{
	...
	{
		u16 regionMapSectionId =
			GetRegionMapSectionId( mapGroup, mapNum );
		if( regionMapSectionId < MAPSEC_NONE )
		{
			for( i = 0;
				i < ARRAY_COUNT( sMovingRegionMapSections );
				i++ )
			{
				if( regionMapSectionId ==
					sMovingRegionMapSections[i] )
					return;
			}
			...
			for( i = 0; i < sPokedexAreaScreen->numSpecialAreas;
				i++ )
			{
			...
			{
				sPokedexAreaScreen
					->specialAreaRegionMapSectionIds[i] =
					regionMapSectionId;
...
static bool8 MapHasMon( const struct WildPokemonHeader* info, u16 species )
{
		...
		MAPSEC_ALTERING_CAVE_2 )
	{
		...
		if( sPokedexAreaScreen->unk6E2 !=
			sPokedexAreaScreen->unk6E4 + 1 )
			return FALSE;
	}
...
static bool8 MonListHasMon(
	const struct WildPokemonInfo* info, u16 species, u16 size )
{
	u16 i;
	if( info != NULL )
	{
...
static void BuildAreaGlowTilemap( void )
{
	...
	for( i = 0; i < ARRAY_COUNT( sPokedexAreaScreen->areaGlowTilemap );
		i++ )
	...
	for( i = 0; i < sPokedexAreaScreen->numOverworldAreas; i++ )
	{
		...
		for( y = 0; y < AREA_SCREEN_HEIGHT; y++ )
		{
			...
			{
				if( GetRegionMapSectionIdAt( x, y ) ==
					sPokedexAreaScreen
						->overworldAreasWithMons[i]
						.regionMapSectionId )
					sPokedexAreaScreen
						->areaGlowTilemap[j] =
						GLOW_TILE_FULL;
	...
	for( y = 0; y < AREA_SCREEN_HEIGHT; y++ )
	{
		for( x = 0; x < AREA_SCREEN_WIDTH; x++ )
		{
			if( sPokedexAreaScreen->areaGlowTilemap[j] ==
				GLOW_TILE_FULL )
			{
				// The "tile != GLOW_TILE_FULL" check is
				// pointless in all of these conditionals,
				// since there's no harm in OR'ing 0xFFFF with
				// anything else.

				// Edges
				if( x != 0 &&
					sPokedexAreaScreen->areaGlowTilemap[j -
						1] != GLOW_TILE_FULL )
					sPokedexAreaScreen
						->areaGlowTilemap[j - 1] |=
						GLOW_TILE_RIGHT;
		...
		i++ )
	{
		...
		else if( sPokedexAreaScreen->areaGlowTilemap[i] )
		{
			...
			if( sPokedexAreaScreen->areaGlowTilemap[i] &
				GLOW_TILE_RIGHT )
			...
			sPokedexAreaScreen
				->areaGlowTilemap[i] = sAreaGlowTilemapMapping
				[sPokedexAreaScreen->areaGlowTilemap[i]];
			sPokedexAreaScreen->areaGlowTilemap[i] |= 0xA000;
		}
	}
...
static void DoAreaGlow( void )
{
	...
	if( sPokedexAreaScreen->whichMarkersFlashing == 0 )
	{
		if( sPokedexAreaScreen->areaShadeOrMarkerFrameCounter == 0 )
		{
			...
			x = gSineTable[sPokedexAreaScreen
					       ->areaShadeBldArgLo] >>
				4;
			y = gSineTable[sPokedexAreaScreen
					       ->areaShadeBldArgHi] >>
				4;
			SetGpuReg(
				REG_OFFSET_BLDALPHA, BLDALPHA_BLEND( x, y ) );
			sPokedexAreaScreen->areaShadeOrMarkerFrameCounter = 0;
...
static void CreateAreaMarkerSprites( void )
{
	...
	for( i = 0; i < sPokedexAreaScreen->numSpecialAreas; i++ )
	{
		...
		if( spriteId != MAX_SPRITES )
		{
			...
			sPokedexAreaScreen->areaMarkerSprites[numSprites++] =
				&gSprites[spriteId];
		}
	}
...
static void DestroyAreaMarkerSprites( void )
{
	...
	for( i = 0; i < 3; i++ )
	{
		if( sPokedexAreaScreen->areaUnknownSprites[i] )
			DestroySprite(
				sPokedexAreaScreen->areaUnknownSprites[i] );
	}
}
...
static void CreateAreaUnknownSprites( void )
{
	...
	else
	{
		for( i = 0; i < 3; i++ )
		{
			spriteId = CreateSprite( &sAreaUnknownSpriteTemplate,
				i * 32 + 0xa0,
				0x8c,
				0 );
			if( spriteId != MAX_SPRITES )
