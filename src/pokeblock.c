...
struct PokeblockMenuStruct
{
	...
	u8 menuItemsStrings[POKEBLOCKS_COUNT + 1][0x20]; // + 1 because of STOW
							 // CASE item
...
static void MovePokeblockMenuCursor(
	s32 pkblId, bool8 arg1, struct ListMenu* arg2 );
...
static const u8 sActionsOnField[] = {
	PKBL_USE_ON_FIELD, PKBL_TOSS, PKBL_CANCEL};
static const u8 sActionsInBattle[] = {PKBL_USE_IN_BATTLE, PKBL_CANCEL};
static const u8 sActionsOnPokeblockFeeder[] = {
	PKBL_USE_ON_FEEDER, PKBL_CANCEL};
...
static const struct YesNoFuncTable sTossYesNoFuncTable = {
	TossPokeblockChoice_Yes, TossPokeblockChoice_No};

static const u8 sContestStatsMonData[] = {MON_DATA_COOL,
	MON_DATA_BEAUTY,
	MON_DATA_CUTE,
	MON_DATA_SMART,
	MON_DATA_TOUGH};
...
{
	OpenPokeblockCase(
		PBLOCK_CASE_BATTLE, SetCB2ToReshowScreenAfterMenu2 );
}
...
static bool8 InitPokeblockMenu( void )
{
	...
	case 11:
		HandlePokeblockMenuCursor(
			sSavedPokeblockData.lastItemPos, 0x1005 );
		gMain.state++;
		break;
	case 12:
		HandleInitWindows( );
...
static bool8 LoadPokeblockMenuGfx( void )
{
	...
	case 0:
		...
		decompress_and_copy_tile_data_to_vram(
			2, gMenuPokeblock_Gfx, 0, 0, 0 );
		sPokeblockMenu->gfxState++;
		break;
	case 1:
		if( free_temp_tile_data_buffers_if_possible( ) != TRUE )
		{
			LZDecompressWram( gMenuPokeblock_Tilemap,
				sPokeblockMenu->tilemap );
			sPokeblockMenu->gfxState++;
		}
		break;
	case 2:
...
static void HandleInitWindows( void )
{
	...
	for( i = 0; i < ARRAY_COUNT( sWindowTemplatesForPokeblockMenu ) - 1;
		i++ )
	{
...
static void PutPokeblockInfoText( void )
{
	...
	PrintOnPokeblockWindow( 0,
		itemName,
		GetStringCenterAlignXOffset( 1, itemName, 0x48 ) );
...
static void HandlePokeblockListMenuItems( void )
{
	...
	for( i = 0; i < sPokeblockMenu->itemsNo - 1; i++ )
	{
		PutPokeblockListMenuString(
			sPokeblockMenu->menuItemsStrings[i], i );
		sPokeblockMenu->items[i].name =
			sPokeblockMenu->menuItemsStrings[i];
...
static void sub_8135FCC( s32 pkblId )
{
	...
	if( pkblId != LIST_CANCEL )
	{
		...
		{
			if( GetPokeblockData( pokeblock, PBLOCK_SPICY + i ) >
				0 )
			{
...
static void HandlePokeblockMenuCursor( u16 cursorPos, u16 arg1 )
{
	FillBgTilemapBufferRect_Palette0(
		2, arg1, 0xF, ( cursorPos * 2 ) + 1, 0xE, 2 );
	schedule_bg_copy_tilemap_to_vram( 2 );
}
...
static void CompactPokeblockSlots( void )
{
	...
	for( i = 0; i < POKEBLOCKS_COUNT - 1; i++ )
	{
		for( j = i + 1; j < POKEBLOCKS_COUNT; j++ )
		{
			...
			{
				struct Pokeblock temp =
					gSaveBlock1Ptr->pokeblocks[i];
				gSaveBlock1Ptr->pokeblocks[i] =
					gSaveBlock1Ptr->pokeblocks[j];
...
static void sub_81362E0( void )
{
	...
	{
		if( sSavedPokeblockData.lastItemPage +
				sPokeblockMenu->maxShowed >
			sPokeblockMenu->itemsNo )
			sSavedPokeblockData.lastItemPage =
				sPokeblockMenu->itemsNo -
				sPokeblockMenu->maxShowed;
...
{
	return CreateSprite(
		&sSpriteTemplate_PokeblockCase, x, y, subpriority );
}
		...
		if( gMain.newKeys & SELECT_BUTTON )
		{
				...
				sPokeblockMenu->itemsNo - 1 )
			{
				...
				HandlePokeblockMenuCursor(
					sSavedPokeblockData.lastItemPos,
					0x2005 );
				data[2] = sSavedPokeblockData.lastItemPage +
					sSavedPokeblockData.lastItemPos;
...
static void Task_HandlePokeblocksSwapInput( u8 taskId )
{
	...
	else
	{
		...
		ListMenuGetScrollAndRow( data[0],
			&sSavedPokeblockData.lastItemPage,
			&sSavedPokeblockData.lastItemPos );
		if( i != sSavedPokeblockData.lastItemPage ||
			var != sSavedPokeblockData.lastItemPos )
		{
			for( i = 0; i < 9; i++ )
			{
...
static void HandlePokeblocksSwap( u8 taskId, bool8 noSwap )
{
	u8 i;
	s16* data         = gTasks[taskId].data;
	u16 swappedFromId = sSavedPokeblockData.lastItemPage +
		sSavedPokeblockData.lastItemPos;
	...
	if( !noSwap && data[2] != swappedFromId &&
		data[2] != swappedFromId - 1 )
	{
...
static void PutPokeblockOptionsWindow( u8 taskId )
{
	...
	InitMenuInUpperLeftCornerPlaySoundWhenAPressed(
		data[1], sPokeblockMenu->optionsNo, 0 );
...
static void Task_HandlePokeblockOptionsInput( u8 taskId )
{
	...
	{
		PlaySE( SE_SELECT );
		sPokeblockMenuActions[sPokeblockMenu
					      ->pokeblockOptionsIds[itemId]]
			.func.void_u8( taskId );
	}
}
...
static void PokeblockAction_Toss( u8 taskId )
{
	...
	StringCopy( gStringVar1,
		gPokeblockNames[gSaveBlock1Ptr->pokeblocks[gSpecialVar_ItemId]
					.color] );
...
static void HandleErasePokeblock( u8 taskId )
{
	if( gMain.newKeys & ( A_BUTTON | B_BUTTON ) )
	{
		...
		HandlePokeblockListMenuItems( );
		data[0] = ListMenuInit(
			&gMultiuseListMenuTemplate, *lastPage, *lastPos );
...
static void PokeblockAction_UseInBattle( u8 taskId )
{
	...
	s16 gain  = PokeblockGetGain(
                nature, &gSaveBlock1Ptr->pokeblocks[gSpecialVar_ItemId] );
	...
	gSpecialVar_ItemId =
		gSaveBlock1Ptr->pokeblocks[gSpecialVar_ItemId].color << 8;
	if( gain == 0 )
...
u8 GetHighestPokeblocksFlavorLevel( const struct Pokeblock* pokeblock )
{
	...
	{
		u8 currFlavor =
			GetPokeblockData( pokeblock, PBLOCK_SPICY + i );
...
s16 PokeblockGetGain( u8 nature, const struct Pokeblock* pokeblock )
{
	...
	for( flavor = 0; flavor < FLAVOR_COUNT; flavor++ )
	{
		...
		if( curGain > 0 )
			totalGain += curGain *
				gPokeblockFlavorCompatibilityTable[5 * nature +
					flavor];
	}
...
bool8 CopyMonFavoritePokeblockName( u8 nature, u8* dest )
{
	...
	for( i = 0; i < FLAVOR_COUNT; i++ )
	{
		if( PokeblockGetGain( nature, &sFavoritePokeblocksTable[i] ) >
			0 )
		{
