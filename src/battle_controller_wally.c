...
static void WallyBufferRunCommand( void )
{
	if( gBattleControllerExecFlags & gBitTable[gActiveBattler] )
	{
		if( gBattleBufferA[gActiveBattler][0] <
			ARRAY_COUNT( sWallyBufferCommands ) )
			sWallyBufferCommands[gBattleBufferA[gActiveBattler]
							   [0]]( );
		else
...
static void WallyHandleMoveAnimation( void )
{
	...
	if( IsMoveWithoutAnimation(
		    move, gAnimMoveTurn ) ) // always returns FALSE
	{
		WallyBufferExecCompleted( );
	}
	else
...
static void WallyHandleUnknownYesNoBox( void ) { WallyBufferExecCompleted( ); }
...
static void WallyHandleCmd37( void ) { WallyBufferExecCompleted( ); }

static void WallyHandleCmd38( void ) { WallyBufferExecCompleted( ); }

static void WallyHandleCmd39( void ) { WallyBufferExecCompleted( ); }

static void WallyHandleCmd40( void ) { WallyBufferExecCompleted( ); }
...
static void WallyHandleCmd42( void ) { WallyBufferExecCompleted( ); }
