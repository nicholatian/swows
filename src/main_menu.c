...
static void NewGameBirchSpeech_CreateDialogueWindowBorder(
	u8, u8, u8, u8, u8, u8 );
...
static const u16 sBirchSpeechPlatformBlackPal[] = {RGB_BLACK,
	RGB_BLACK,
	RGB_BLACK,
	RGB_BLACK,
	RGB_BLACK,
...
static const u16 sMainMenuBgPal[] =
	INCBIN_U16( "graphics/misc/main_menu_bg.gbapal" );
static const u16 sMainMenuTextPal[] =
	INCBIN_U16( "graphics/misc/main_menu_text.gbapal" );
...
enum
{
	...
	HAS_MYSTERY_EVENTS, // CONTINUE, NEW GAME, MYSTERY GIFT, MYSTERY
			    // EVENTS, OPTION
};
...
static u32 InitMainMenu( bool8 returningFromOptionsMenu )
{
	...
	if( returningFromOptionsMenu )
		BeginNormalPaletteFade(
			0xFFFFFFFF, 0, 0x10, 0, RGB_BLACK ); // fade to
							     // black
	else
		BeginNormalPaletteFade(
			0xFFFFFFFF, 0, 0x10, 0, RGB_WHITEALPHA ); // fade to
								  // white
	ResetBgsAndClearDma3BusyFlags( 0 );
	InitBgsFromTemplates(
		0, sMainMenuBgTemplates, ARRAY_COUNT( sMainMenuBgTemplates ) );
	ChangeBgX( 0, 0, 0 );
	ChangeBgY( 0, 0, 0 );
	ChangeBgX( 1, 0, 0 );
	ChangeBgY( 1, 0, 0 );
...
static void Task_MainMenuCheckSaveFile( u8 taskId )
{
	...
	if( !gPaletteFade.active )
	{
		...
		SetGpuReg( REG_OFFSET_WINOUT,
			WINOUT_WIN01_BG0 | WINOUT_WIN01_OBJ |
				WINOUT_WIN01_CLR );
		SetGpuReg( REG_OFFSET_BLDCNT,
			BLDCNT_EFFECT_DARKEN | BLDCNT_TGT1_BG0 );
		SetGpuReg( REG_OFFSET_BLDALPHA, 0 );
		...
		if( sCurrItemAndOptionMenuCheck &
			OPTION_MENU_FLAG ) // are we
					   // returning from
					   // the options
					   // menu?
		{
			switch( tMenuType ) // if so, highlight the OPTIONS
					    // item
			{
		...
		sCurrItemAndOptionMenuCheck &=
			CURRENT_ITEM_MASK; // turn off the
					   // "returning from
					   // options menu"
					   // flag
...
static void Task_DisplayMainMenu( u8 taskId )
{
	...
	if( !gPaletteFade.active )
	{
		...
		// Note: If there is no save file, the save block is zeroed
		// out, so the default gender is MALE.
		...
		default:
			...
			DrawMainMenuWindowBorder(
				&sWindowTemplates_MainMenu[0],
				MAIN_MENU_BORDER_TILE );
			DrawMainMenuWindowBorder(
				&sWindowTemplates_MainMenu[1],
				MAIN_MENU_BORDER_TILE );
			break;
		case HAS_SAVED_GAME:
			...
			AddTextPrinterParameterized3( 3,
				1,
				0,
				1,
				sTextColor_Headers,
				-1,
				gText_MainMenuNewGame );
			AddTextPrinterParameterized3( 4,
				1,
				0,
				1,
				sTextColor_Headers,
			...
			DrawMainMenuWindowBorder(
				&sWindowTemplates_MainMenu[2],
				MAIN_MENU_BORDER_TILE );
			DrawMainMenuWindowBorder(
				&sWindowTemplates_MainMenu[3],
				MAIN_MENU_BORDER_TILE );
			DrawMainMenuWindowBorder(
				&sWindowTemplates_MainMenu[4],
				MAIN_MENU_BORDER_TILE );
			break;
		case HAS_MYSTERY_GIFT:
			FillWindowPixelBuffer( 2, PIXEL_FILL( 0xA ) );
			...
			AddTextPrinterParameterized3( 5,
				1,
				0,
				1,
				sTextColor_Headers,
		...
		case HAS_MYSTERY_EVENTS:
			...
			AddTextPrinterParameterized3( 6,
				1,
				0,
				1,
				sTextColor_Headers,
			...
			if( sCurrItemAndOptionMenuCheck == 4 )
			{
				...
				tIsScrolled = TRUE;
				gTasks[tScrollArrowTaskId]
					.tArrowTaskIsScrolled = TRUE;
			}
			break;
		}
...
static bool8 HandleMainMenuInput( u8 taskId )
{
	...
	{
		PlaySE( SE_SELECT );
		IsWirelessAdapterConnected( ); // why bother calling this here?
					       // debug?
					       // Task_HandleMainMenuAPressed
					       // will check too
	...
	else if( ( gMain.newKeys & DPAD_UP ) && tCurrItem > 0 )
	{
			...
			tCurrItem == 1 )
		{
			...
			gTasks[tScrollArrowTaskId].tArrowTaskIsScrolled =
				tIsScrolled = FALSE;
		}
		tCurrItem--;
	...
	if( !gPaletteFade.active )
	{
		if( gTasks[taskId].tMenuType == HAS_MYSTERY_EVENTS )
			RemoveScrollIndicatorArrowPair(
				gTasks[taskId].tScrollArrowTaskId );
...
static void HighlightSelectedMainMenuItem(
	u8 menuType, u8 selectedMenuItem, s16 isScrolled )
{
	...
	case HAS_MYSTERY_EVENTS:
		...
		case 4:
			SetGpuReg( REG_OFFSET_WIN0V,
				MENU_WIN_VCOORDS( 6 ) - MENU_SCROLL_SHIFT );
			break;
		}
		break;
	}
	...
	if( gTasks[taskId].tIsDoneFadingSprites )
	{
		...
		else
		{
			...
			StringExpandPlaceholders(
				gStringVar4, gText_Birch_Welcome );
...
static void Task_NewGameBirchSpeech_MainSpeech( u8 taskId )
{
	...
	{
		StringExpandPlaceholders(
			gStringVar4, gText_Birch_MainSpeech );
		AddTextPrinterForMessage( 1 );
...
static void Task_NewGameBirchSpeech_ReshowBirchLotad( u8 taskId )
{
	...
	if( gTasks[taskId].tIsDoneFadingSprites )
	{
		...
		NewGameBirchSpeech_ClearWindow( 0 );
		StringExpandPlaceholders(
			gStringVar4, gText_Birch_YourePlayer );
		AddTextPrinterForMessage( 1 );
		gTasks[taskId].func =
			Task_NewGameBirchSpeech_WaitForSpriteFadeInAndTextPrinter;
	}
...
static void Task_NewGameBirchSpeech_WaitForSpriteFadeInAndTextPrinter(
	u8 taskId )
{
	if( gTasks[taskId].tIsDoneFadingSprites )
	{
		...
		if( !RunTextPrintersAndIsPrinter0Active( ) )
		{
			gSprites[gTasks[taskId].tBirchSpriteId].oam.objMode =
				1;
			gSprites[gTasks[taskId].tLotadSpriteId].oam.objMode =
				1;
...
static void Task_NewGameBirchSpeech_AreYouReady( u8 taskId )
{
	...
	if( gTasks[taskId].tIsDoneFadingSprites )
	{
		...
		NewGameBirchSpeech_StartFadePlatformOut( taskId, 1 );
		StringExpandPlaceholders(
			gStringVar4, gText_Birch_AreYouReady );
		AddTextPrinterForMessage( 1 );
...
static void CB2_NewGameBirchSpeech_ReturnFromNamingScreen( void )
{
	...
	ResetTasks( );
	taskId = CreateTask(
		Task_NewGameBirchSpeech_ReturnFromNamingScreenShowTextbox, 0 );
...
static void AddBirchSpeechObjects( u8 taskId )
{
	...
	brendanSpriteId                      = CreateTrainerSprite(
                FacilityClassToPicIndex( FACILITY_CLASS_BRENDAN ),
                120,
                60,
                0,
	...
	maySpriteId                            = CreateTrainerSprite(
                FacilityClassToPicIndex( FACILITY_CLASS_MAY ),
                120,
                60,
                0,
...
static void Task_NewGameBirchSpeech_FadeOutTarget1InTarget2( u8 taskId )
{
	...
	else
	{
		...
		SetGpuReg( REG_OFFSET_BLDALPHA,
			gTasks[taskId].tAlphaCoeff1 + alphaCoeff2 );
	}
}

static void NewGameBirchSpeech_StartFadeOutTarget1InTarget2(
	u8 taskId, u8 delay )
{
	u8 taskId2;
	...
	taskId2                             = CreateTask(
                Task_NewGameBirchSpeech_FadeOutTarget1InTarget2, 0 );
...
static void NewGameBirchSpeech_StartFadeInTarget1OutTarget2(
	u8 taskId, u8 delay )
{
	u8 taskId2;
	...
	gTasks[taskId].tIsDoneFadingSprites = 0;
	taskId2                             = CreateTask(
                Task_NewGameBirchSpeech_FadeInTarget1OutTarget2, 0 );
	gTasks[taskId2].tMainTask    = taskId;
...
static void Task_NewGameBirchSpeech_FadePlatformIn( u8 taskId )
{
	...
	else
	{
		...
		LoadPalette(
			&sBirchSpeechBgGradientPal[gTasks[taskId].tPalIndex],
			1,
			16 );
	}
}
...
static void CreateMainMenuErrorWindow( const u8* str )
{
	...
	DrawMainMenuWindowBorder(
		&sWindowTemplates_MainMenu[7], MAIN_MENU_BORDER_TILE );
...
static void MainMenu_FormatSavegamePlayer( void )
{
	...
	AddTextPrinterParameterized3(
		2, 1, 0, 17, sTextColor_PlayerGenderColor, -1, gStringVar4 );
	AddTextPrinterParameterized3( 2,
		1,
		GetStringRightAlignXOffset(
			1, gSaveBlock2Ptr->playerName, 100 ),
		17,
...
static void MainMenu_FormatSavegameTime( void )
{
	...
	ptr = ConvertIntToDecimalStringN(
		str, gSaveBlock2Ptr->playTimeHours, 0, 3 );
	*ptr = 0xF0;
	ConvertIntToDecimalStringN(
		ptr + 1, gSaveBlock2Ptr->playTimeMinutes, 2, 2 );
	AddTextPrinterParameterized3( 2,
		1,
		GetStringRightAlignXOffset( 1, str, 0xD0 ),
		17,
		sTextColor_PlayerGenderColor,
		-1,
		str );
...
static void MainMenu_FormatSavegamePokedex( void )
{
	...
	if( FlagGet( FLAG_SYS_POKEDEX_GET ) == TRUE )
	{
		...
		StringExpandPlaceholders(
			gStringVar4, gText_ContinueMenuPokedex );
		AddTextPrinterParameterized3( 2,
			1,
			0,
			33,
		...
		AddTextPrinterParameterized3( 2,
			1,
			GetStringRightAlignXOffset( 1, str, 100 ),
			33,
			sTextColor_PlayerGenderColor,
			-1,
			str );
...
static void LoadMainMenuWindowFrameTiles( u8 bgId, u16 tileOffset )
{
	LoadBgTiles( bgId,
		...
		0x120,
		tileOffset );
	LoadPalette( GetWindowFrameTilesPal(
			     gSaveBlock2Ptr->optionsWindowFrameType )
			     ->pal,
		32,
		32 );
...
static void DrawMainMenuWindowBorder(
	const struct WindowTemplate* template, u16 baseTileNum )
{
...
static void NewGameBirchSpeech_ClearGenderWindowTilemap(
	u8 a, u8 b, u8 c, u8 d, u8 e, u8 unused )
{
	FillBgTilemapBufferRect( a, 0, b + 0xFF, c + 0xFF, d + 2, e + 2, 2 );
}

static void NewGameBirchSpeech_ClearGenderWindow(
	u8 windowId, bool8 copyToVram )
{
	CallWindowFunction(
		windowId, NewGameBirchSpeech_ClearGenderWindowTilemap );
...
{
	CallWindowFunction(
		windowId, NewGameBirchSpeech_CreateDialogueWindowBorder );
	FillWindowPixelBuffer( windowId, PIXEL_FILL( 1 ) );
	...
	u8 a, u8 b, u8 c, u8 d, u8 e, u8 f )
{
	...
	FillBgTilemapBufferRect(
		a, BG_TILE_V_FLIP( 0xFD ), b - 2, c + e, 1, 1, f );
	FillBgTilemapBufferRect(
		a, BG_TILE_V_FLIP( 0xFF ), b - 1, c + e, 1, 1, f );
	FillBgTilemapBufferRect(
		a, BG_TILE_V_FLIP( 0x100 ), b, c + e, d - 1, 1, f );
	FillBgTilemapBufferRect(
		a, BG_TILE_V_FLIP( 0x101 ), b + d - 1, c + e, 1, 1, f );
	FillBgTilemapBufferRect(
		a, BG_TILE_V_FLIP( 0x102 ), b + d, c + e, 1, 1, f );
}
