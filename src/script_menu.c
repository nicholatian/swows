...
bool8 ScriptMenu_Multichoice(
	u8 left, u8 top, u8 multichoiceId, u8 ignoreBPress )
{
	...
	else
	{
		...
		DrawMultichoiceMenu(
			left, top, multichoiceId, ignoreBPress, 0 );
		return TRUE;
	}
}

bool8 ScriptMenu_MultichoiceWithDefault( u8 left,
	u8 top,
	u8 multichoiceId,
	bool8 ignoreBPress,
	u8 defaultChoice )
...
static void DrawMultichoiceMenu(
	u8 left, u8 top, u8 multichoiceId, u8 ignoreBPress, u8 cursorPos )
{
	int i;
	u8 windowId;
	u8 count = gMultichoiceLists[multichoiceId].count;
	const struct MenuAction* actions =
		gMultichoiceLists[multichoiceId].list;
	int width = 0;
	u8 newWidth;
...
static void sub_80E1FBC(
	u8 ignoreBPress, u8 count, u8 windowId, u8 multichoiceId )
{
	u8 i;
	u8 taskId;
	gUnknown_02039F90 = 2;
...
bool8 ScriptMenu_MultichoiceGrid(
	u8 left, u8 top, u8 multichoiceId, u8 ignoreBPress, u8 columnCount )
{
	...
	else
	{
		...
		{
			width = display_text_and_get_width(
				gMultichoiceLists[multichoiceId].list[i].text,
				width );
		}
		...
		rowCount =
			gMultichoiceLists[multichoiceId].count / columnCount;

		taskId = CreateTask( Task_HandleMultichoiceGridInput, 80 );

		gTasks[taskId].tIgnoreBPress = ignoreBPress;
		gTasks[taskId].tWindowId     = CreateWindowFromRect(
                        left, top, columnCount * newWidth, rowCount * 2 );
		...
		PrintMenuGridTable( gTasks[taskId].tWindowId,
			...
			rowCount,
			gMultichoiceLists[multichoiceId].list );
		sub_8199944( gTasks[taskId].tWindowId,
			newWidth * 8,
			columnCount,
			rowCount,
			0 );
...
static void CreatePCMenu( void )
{
	...
	{
		pixelWidth = display_text_and_get_width(
			sPCNameStrings[i], pixelWidth );
	}

	if( FlagGet( FLAG_SYS_GAME_CLEAR ) )
	{
		pixelWidth = display_text_and_get_width(
			gText_HallOfFame, pixelWidth );
	}
...
void ScriptMenu_DisplayPCStartupPrompt( void )
{
	...
	AddTextPrinterParameterized2(
		0, 1, gText_WhichPCShouldBeAccessed, 0, NULL, 2, 1, 3 );
}
...
static void sub_80E2578( void )
{
	...
	if( CheckBagHasItem( ITEM_EON_TICKET, 1 ) == TRUE &&
		FlagGet( FLAG_SYS_HAS_EON_TICKET ) == TRUE )
	{
		if( gSpecialVar_0x8004 == 0 )
		{
		...
		if( gSpecialVar_0x8004 == 1 &&
			FlagGet( FLAG_HAS_EON_TICKET ) == FALSE )
		{
			gUnknown_03001124[temp] = 2;
			temp++;
		...
		FlagGet( FLAG_ENABLE_SHIP_NAVEL_ROCK ) == TRUE )
	{
		...
		if( gSpecialVar_0x8004 == 1 &&
			FlagGet( FLAG_HAS_MYSTIC_TICKET ) == FALSE )
		{
			gUnknown_03001124[temp] = 3;
			temp++;
		...
		FlagGet( FLAG_ENABLE_SHIP_BIRTH_ISLAND ) == TRUE )
	{
		...
		if( gSpecialVar_0x8004 == 1 &&
			FlagGet( FLAG_HAS_AURORA_TICKET ) == FALSE )
		{
			gUnknown_03001124[temp] = 4;
			temp++;
		...
		FlagGet( FLAG_ENABLE_SHIP_FARAWAY_ISLAND ) == TRUE )
	{
		...
		if( gSpecialVar_0x8004 == 1 &&
			FlagGet( FLAG_HAS_OLD_SEA_MAP ) == FALSE )
		{
			gUnknown_03001124[temp] = 5;
			temp++;
	...
	if( gSpecialVar_0x8004 == 0 &&
		FlagGet( FLAG_MET_SCOTT_ON_SS_TIDAL ) == TRUE )
	{
		count = temp;
	}
	...
	else
	{
		...
		for( j = 0; j < ARRAY_COUNT( gUnknown_0858BB80 ); j++ )
		{
			...
			{
				pixelWidth = display_text_and_get_width(
					gUnknown_0858BB80[test], pixelWidth );
			}
		}

		width    = convert_pixel_width_to_tile_width( pixelWidth );
		windowId = CreateWindowFromRect(
			28 - width, ( 6 - count ) * 2, width, count * 2 );
		SetStandardWindowBorderStyle( windowId, 0 );

		for( temp = 0, i = 0; i < ARRAY_COUNT( gUnknown_0858BB80 );
			i++ )
		{
...
static void Task_PokemonPicWindow( u8 taskId )
{
	...
	case 2:
		FreeResourcesAndDestroySprite(
			&gSprites[task->tMonSpriteId], task->tMonSpriteId );
		task->tState++;
		break;
	case 3:
...
static void sub_80E2A94( u8 multichoiceId )
{
	...
	case 77:
		...
		AddTextPrinterParameterized2( 0,
			1,
			gUnknown_0858BBAC[Menu_GetCursorPos( )],
			0,
			NULL,
			2,
			1,
	...
	case 76:
		...
		AddTextPrinterParameterized2( 0,
			1,
			gUnknown_0858BB9C[Menu_GetCursorPos( )],
			0,
			NULL,
			2,
			1,
	...
	case 78:
		...
		AddTextPrinterParameterized2( 0,
			1,
			gUnknown_0858BBBC[Menu_GetCursorPos( )],
			0,
			NULL,
			2,
			1,
	...
	case 79:
		...
		AddTextPrinterParameterized2( 0,
			1,
			gUnknown_0858BBCC[Menu_GetCursorPos( )],
			0,
			NULL,
			2,
			1,
	...
	case 75:
		...
		AddTextPrinterParameterized2( 0,
			1,
			gUnknown_0858BBEC[Menu_GetCursorPos( )],
			0,
			NULL,
			2,
			1,
	...
	case 74:
		...
		AddTextPrinterParameterized2( 0,
			1,
			gUnknown_0858BBE0[Menu_GetCursorPos( )],
			0,
			NULL,
			2,
			1,
...
static void CreateStartMenu( void )
{
	...
	AddTextPrinterParameterized(
		windowId, 1, gText_MenuOptionBag, 8, 41, TEXT_SPEED_FF, NULL );
	AddTextPrinterParameterized( windowId,
		1,
...
static void sub_80E2CC4(
	bool8 ignoreBPress, u8 unused, u8 windowId, u8 multichoiceId )
{
	u8 taskId;
	gUnknown_02039F90 = 2;
	taskId            = CreateTask( Task_HandleMultichoiceInput, 80 );
