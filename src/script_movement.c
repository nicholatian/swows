	...
	u8 localId, u8 mapNum, u8 mapGroup, const u8* movementScript )
{
	...
	if( TryGetEventObjectIdByLocalIdAndMap(
		    localId, mapNum, mapGroup, &eventObjId ) )
		return TRUE;
...
bool8 ScriptMovement_IsObjectMovementFinished(
	u8 localId, u8 mapNum, u8 mapBank )
{
	...
	if( TryGetEventObjectIdByLocalIdAndMap(
		    localId, mapNum, mapBank, &eventObjId ) )
		return TRUE;
	r4 = sub_80D33F4( );
...
static u8 sub_80D33F4( void ) { return FindTaskIdByFunc( sub_80D3660 ); }
...
static void npc_obj_offscreen_culling_and_flag_update(
	u8 a, const u8* movementScript )
{
	gUnknown_02039D90[a] = movementScript;
}

static const u8* sub_80D35CC( u8 a ) { return gUnknown_02039D90[a]; }

static void sub_80D35DC(
	u8 taskId, u8 b, u8 eventObjId, const u8* movementScript )
{
...
static void sub_80A2490( u8 taskId, u8 b, u8 eventObjId, const u8* d )
{
	...
	{
		if( !EventObjectSetHeldMovement(
			    &gEventObjects[eventObjId], var ) )
		{
			d++;
