...
static void PrintMysteryMenuText(
	u8 windowId, const u8* text, u8 x, u8 y, s32 speed );
...
static void CB2_MysteryEventMenu( void )
{
	...
	case 3:
		if( ( gLinkStatus & 0x20 ) && ( gLinkStatus & 0x1C ) > 4 )
		{
			...
			PrintMysteryMenuText(
				0, gText_PressAToLoadEvent, 1, 2, 1 );
			gMain.state++;
		}
	...
	case 6:
		if( IsLinkConnectionEstablished( ) )
		{
			...
			{
				if( GetLinkPlayerDataExchangeStatusTimed(
					    2, 2 ) == 3 )
				{
					sub_800AC34( );
					GetEventLoadMessage( gStringVar4, 1 );
	...
	case 11:
		...
		{
			u16 unkVal =
				RunMysteryEventScript( gDecompressionBuffer );
