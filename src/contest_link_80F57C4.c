...
void sub_80F57C4( void )
{
	...
	InitBgsFromTemplates(
		0, gUnknown_0858D888, ARRAY_COUNT( gUnknown_0858D888 ) );
	for( i = 0; i < 4; i++ )
	...
	SetGpuRegBits( REG_OFFSET_DISPCNT,
		DISPCNT_BG_ALL_ON | DISPCNT_OBJ_ON | DISPCNT_WIN0_ON |
			DISPCNT_WIN1_ON | DISPCNT_OBJWIN_ON );
	gBattle_BG0_X = 0;
	gBattle_BG0_Y = 0;
	gBattle_BG1_X = 0;
	gBattle_BG1_Y = 0;
	...
	for( i = 0; i < 4; i++ )
	{
		...
		for( j = 0; j < 10; j++ )
		{
			...
			FillBgTilemapBufferRect_Palette0(
				1, tile1, j + 19, i * 3 + 5, 1, 1 );
			FillBgTilemapBufferRect_Palette0(
				1, tile2, j + 19, i * 3 + 6, 1, 1 );
		}
	}
...
static void sub_80F5A74( u8 monIndex )
{
	...
	if( monIndex == gContestPlayerMonIndex )
		str = StringCopy(
			gDisplayedStringBattle, gText_ColorDarkGrey );
...
void sub_80F5B00( void )
{
	...
	sub_80F5AE0( );
	memset( gUnknown_0203A034->unk0,
		0,
		sizeof( *gUnknown_0203A034->unk0 ) );
	memset( gUnknown_0203A034->unk4,
		0,
		sizeof( *gUnknown_0203A034->unk4 ) );
	sub_80F6DC0( );
	sub_80F6AE8( );
...
static void sub_80F5F74( u8 taskId )
{
	...
	if( gTasks[taskId].data[0] == 0 )
	{
		...
		var0 = sub_80F6B78( gText_AnnouncingResults,
			gUnknown_0203A034->unk0->unk0 );
	...
	{
		var0 = sub_80F6B78( gText_PreliminaryResults,
			gUnknown_0203A034->unk0->unk0 );
...
static void sub_80F60F0( u8 taskId )
{
	...
	if( gUnknown_0203A034->unk0->unk4 == 0 )
	{
		...
		{
			gTasks[taskId].data[1] = 0;
			var0 = sub_80F6B78( gText_Round2Results,
				gUnknown_0203A034->unk0->unk0 );
			sub_80F6E9C( var0, 144, -1, 1088 );
		}
	}
...
static void sub_80F6204( u8 taskId )
{
	...
	case 4:
		...
		{
			gTasks[taskId].data[1] = 0;
			for( i = 0; i < 4 && gContestFinalStandings[i] != 0;
				i++ )
				;
			...
			var0 = sub_80F6B78(
				buffer, gUnknown_0203A034->unk0->unk0 );
			sub_80F6E9C( var0, 144, -1, 1088 );
			gTasks[taskId].data[0]++;
		}
		break;
...
static void sub_80F6404( u8 taskId )
{
	...
	case 0:
		...
		SetMultiuseSpriteTemplateToPokemon(
			species, B_POSITION_OPPONENT_LEFT );
		gMultiuseSpriteTemplate.paletteTag = pokePal->tag;
		spriteId =
			CreateSprite( &gMultiuseSpriteTemplate, 272, 80, 10 );
	...
	case 3:
		...
		{
			gTasks[taskId].data[1] = 0;
			gSprites[gUnknown_0203A034->unk0->spriteId].callback =
				sub_80F7620;
			gTasks[taskId].data[0]++;
		}
		break;
...
static void sub_80F68F0( u8 taskId )
{
	if( ++gTasks[taskId].data[0] == 2 )
	{
		...
		BlendPalette(
			0x6B, 1, gTasks[taskId].data[1], RGB( 30, 22, 11 ) );
		BlendPalette( 0x68, 1, gTasks[taskId].data[1], RGB_WHITE );
		BlendPalette(
			0x6E, 1, gTasks[taskId].data[1], RGB( 30, 29, 29 ) );
	}
	...
	u16 species, u8 monIndex, u8 srcOffset, u8 useDmaNow, u32 personality )
{
	...
	if( useDmaNow )
	{
		RequestDma3Copy( iconPtr,
			(void*)BG_CHAR_ADDR( 1 ) + monIndex * 0x200,
			0x180,
			1 );
		var0 = ( ( monIndex + 10 ) << 12 );
		var1 = ( monIndex * 0x10 + 0x200 );
		WriteSequenceToBgTilemapBuffer(
			1, var1 | var0, 3, monIndex * 3 + 4, 4, 3, 17, 1 );
	}
	else
	{
		RequestDma3Copy( iconPtr,
...
static void LoadAllContestMonIcons( u8 srcOffset, u8 useDmaNow )
{
	...
	for( i = 0; i < 4; i++ )
		sub_80F69B8( gContestMons[i].species,
			i,
			srcOffset,
			useDmaNow,
...
static void sub_80F6A9C( void )
{
	...
	for( i = 0; i < 4; i++ )
	{
		...
		LoadPalette( gMonIconPalettes[gMonIconPaletteIndices[species]],
			i * 0x10 + 0xA0,
			0x20 );
	}
}
...
static void sub_80F6AE8( void )
{
	...
	if( gLinkContestFlags & LINK_CONTEST_FLAG_IS_WIRELESS )
	{
		...
		RequestDma3Fill( 0xFFFFFFFF,
			(void*)BG_CHAR_ADDR( 4 ) + sheet * 0x20,
			0x80,
			1 );
...
s32 sub_80F6B78( const u8* text, u8 spriteId )
{
	...
	for( i = 1; i < 4; i++ )
		spriteTilePtrs[i] =
			(void*)( gSprites[sprite->data[i - 1]].oam.tileNum *
					32 +
				VRAM + 0x10000 );
...
static void sub_80F6F68( struct Sprite* sprite )
{
	...
	for( i = 0; i < 3; i++ )
	{
		...
		sprite2->pos1.x =
			sprite->pos1.x + sprite->pos2.x + ( i + 1 ) * 64;
	}
...
static void sub_80F707C( const u8* text )
{
	...
	gBattle_WIN0H = 0x00F0;
	gBattle_WIN0V =
		( ( sprite->pos1.y - 16 ) << 8 ) | ( sprite->pos1.y + 16 );
	SetGpuReg( REG_OFFSET_WININ,
		WININ_WIN1_BG_ALL | WININ_WIN1_OBJ | WININ_WIN1_CLR |
			WININ_WIN0_BG1 | WININ_WIN0_BG2 | WININ_WIN0_BG3 |
...
static void sub_80F71C8( void )
{
	...
	CopyToBgTilemapBufferRect_ChangePalette(
		2, gUnknown_0203A034->unkC[2], 0, 0, 32, 4, palette );
}
...
static void sub_80F73DC( u8 taskId )
{
	...
	else if( gTasks[taskId].data[10] == 1 )
	{
		if( --gTasks[taskId].data[11] == -1 )
		{
			...
			WriteSequenceToBgTilemapBuffer( 2,
				firstTileNum,
				1,
				gTasks[taskId].data[1] * 3 + 5,
				2,
				1,
				17,
				1 );
...
static void sub_80F74BC( u8 taskId )
{
	...
	CopyToBgTilemapBufferRect_ChangePalette( 2,
		i * 0xC0 + 0x100 + gUnknown_0203A034->unkC[2],
		0,
		i * 3 + 4,
		32,
		3,
...
static void sub_80F753C( u8 taskId )
{
	if( ++gTasks[taskId].data[11] == 1 )
	{
		...
		BlendPalette(
			0x91, 1, gTasks[taskId].data[12], RGB( 13, 28, 27 ) );
...
static void sub_80F7670( u8 taskId )
{
	if( ++gTasks[taskId].data[0] == 5 )
	{
		...
		{
			u8 spriteId = CreateSprite( &gSpriteTemplate_858D860,
				( Random( ) % 240 ) - 20,
				44,
				5 );
...
static void sub_80F7824( u8 taskId )
{
	...
	{
		gTasks[taskId].data[10] = 0;
		sub_80F69B8( gTasks[taskId].data[2],
			monIndex,
...
//                 x -= arg1;
//                 x--;
//                 FillBgTilemapBufferRect_Palette0(1, 0x60B3, x, i * 3 + 5, 1,
//                 1); taskId = CreateTask(sub_80F7CA8, 10); var0 =
//                 ((gUnknown_0203A034->unk4->unk0[i].unk8 << 16) /
//                 gUnknown_0203A034->unk4->unk0[i].unk11) * (arg1 + 1); var0
//                 &= 0xFFFF; if (var0 > 0x7FFF)
//                     var0 += 0x10000;
...
//                 int x = var1 - thing;
//                 x--;
//                 FillBgTilemapBufferRect_Palette0(1, tile, x, i * 3 + 6, 1,
//                 1); taskId = CreateTask(sub_80F7CA8, 10); var0 =
//                 ((gUnknown_0203A034->unk4->unk0[i].unkC << 16) /
//                 gUnknown_0203A034->unk4->unk0[i].unk12) * (arg1 + 1); var0
//                 &= 0xFFFF; if (var0 > 0x7FFF)
//                     var0 += 0x10000;
...
//                 if (gUnknown_0203A034->unk4->unk0[i].unk10)
//                     gTasks[taskId].data[1] =
//                     gUnknown_0203A034->unk4->unk0[i].unk8 - (var0 >> 16);
//                 else
//                     gTasks[taskId].data[1] =
//                     gUnknown_0203A034->unk4->unk0[i].unk8 + (var0 >> 16);
...
void sub_80F7CA8( u8 taskId )
{
	...
	if( !endTask )
	{
		if( r4 )
			...
			gUnknown_0203A034->unk0->unkC[r6] =
				gUnknown_0203A034->unk0->unkC[r6] - 1;
		else
			gUnknown_0203A034->unk0->unkC[r6] =
				gUnknown_0203A034->unk0->unkC[r6] + 1;
	}
	...
	if( !r4 && !endTask )
	{
		for( i = 0; i < 11; i++ )
		{
			if( gUnknown_0203A034->unk0->unkC[r6] >=
				( i + 1 ) * 8 )
				var0 = 8;
...
static void sub_80F7DF4( void )
{
	...
	gUnknown_0203A034->unk0 =
		AllocZeroed( sizeof( *gUnknown_0203A034->unk0 ) );
	gUnknown_0203A034->unk4 =
		AllocZeroed( sizeof( *gUnknown_0203A034->unk4 ) );
...
u16 sub_80F7F7C( void )
{
	...
	case CONTEST_CATEGORY_COOL:
		if( GetMonData( mon, MON_DATA_COOL_RIBBON ) >
			gSpecialVar_ContestRank )
			result = 1;
		break;
	case CONTEST_CATEGORY_BEAUTY:
		if( GetMonData( mon, MON_DATA_BEAUTY_RIBBON ) >
			gSpecialVar_ContestRank )
			result = 1;
		break;
	case CONTEST_CATEGORY_CUTE:
		if( GetMonData( mon, MON_DATA_CUTE_RIBBON ) >
			gSpecialVar_ContestRank )
			result = 1;
		break;
	case CONTEST_CATEGORY_SMART:
		if( GetMonData( mon, MON_DATA_SMART_RIBBON ) >
			gSpecialVar_ContestRank )
			result = 1;
		break;
	case CONTEST_CATEGORY_TOUGH:
		if( GetMonData( mon, MON_DATA_TOUGH_RIBBON ) >
			gSpecialVar_ContestRank )
			result = 1;
		break;
	}
...
void sub_80F7FFC( void )
{
	...
	case CONTEST_CATEGORY_COOL:
		...
		{
			ribbonData++;
			SetMonData( &gPlayerParty[gContestMonPartyIndex],
				MON_DATA_COOL_RIBBON,
				&ribbonData );
			if( GetRibbonCount(
				    &gPlayerParty[gContestMonPartyIndex] ) >
				4 )
				sub_80EE4DC(
					&gPlayerParty[gContestMonPartyIndex],
					MON_DATA_COOL_RIBBON );
...
{
	StringCopy(
		gStringVar1, gContestMons[gSpecialVar_0x8006].trainerName );
	sub_81DB5AC( gStringVar1 );
}
...
void sub_80F82B4( void )
{
	...
	{
		if( gContestMonConditions[gSpecialVar_0x8006] <
...
void sub_80F83D0( void ) { SetMainCallback2( CB2_StartContest ); }
...
void sub_80F84B0( void ) { gSpecialVar_0x8004 = gContestPlayerMonIndex; }
...
static void sub_80F8508( u8 taskId )
{
	if( gLinkContestFlags & LINK_CONTEST_FLAG_HAS_RS_PLAYER )
	{
		...
		SetTaskFuncWithFollowupFunc(
			taskId, sub_80FC6BC, sub_80F8568 );
	}
	else
	{
		sub_80DA8C8( gContestMonPartyIndex );
