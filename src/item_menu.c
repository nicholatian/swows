...
void GoToBagMenu(
	u8 bagMenuType, u8 pocketId, void ( *postExitMenuMainCallback2 )( ) );
void CB2_Bag( void );
...
const TaskFunc gUnknown_08614054[] = {unknown_item_menu_type,
...
const struct YesNoFuncTable sYesNoTossFunctions = {
	BagMenu_ConfirmToss, BagMenu_CancelToss};

const struct YesNoFuncTable sYesNoSellItemFunctions = {
	BagMenu_ConfirmSell, BagMenu_CancelSell};
...
const u8 gUnknown_086140A4[] =
	INCBIN_U8( "graphics/interface/select_button.4bpp" );
...
{
	GoToBagMenu( RETURN_LOCATION_FIELD,
		POCKETS_COUNT,
...
{
	if( !InBattlePyramid( ) )
		GoToBagMenu( RETURN_LOCATION_BATTLE,
			POCKETS_COUNT,
...
{
	GoToBagMenu( RETURN_LOCATION_FIELD_2,
		BERRIES_POCKET,
...
{
	GoToBagMenu( RETURN_LOCATION_FIELD_4,
		POCKETS_COUNT,
		bag_menu_leave_maybe_2 );
	gSpecialVar_Result = 0;
}
...
{
	GoToBagMenu(
		RETURN_LOCATION_FIELD_5, POCKETS_COUNT, bag_menu_leave_maybe );
	gSpecialVar_Result = 0;
}
	...
	u8 bagMenuType, u8 pocketId, void ( *postExitMenuMainCallback2 )( ) )
{
	...
	else
	{
		...
		if( postExitMenuMainCallback2 )
			gBagPositionStruct.bagCallback =
				postExitMenuMainCallback2;
		...
		gBagMenu->unk81E        = -1;
		gBagMenu->unk81F        = -1;
		memset( gBagMenu->spriteId,
			0xFF,
			sizeof( gBagMenu->spriteId ) );
...
void CB2_Bag( void )
{
	while( sub_81221EC( ) != TRUE && SetupBagMenu( ) != TRUE &&
		sub_81221AC( ) != TRUE )
	{
	};
}
...
bool8 SetupBagMenu( void )
{
	...
	case 14:
		...
		gTasks[taskId].data[0] = ListMenuInit(
			...
			gBagPositionStruct
				.scrollPosition[gBagPositionStruct.pocket],
			gBagPositionStruct
				.cursorPosition[gBagPositionStruct.pocket] );
...
bool8 LoadBagMenu_Graphics( void )
{
	...
	case 0:
		...
		decompress_and_copy_tile_data_to_vram(
			2, gBagScreen_Gfx, 0, 0, 0 );
		gBagMenu->unk834++;
		break;
	case 1:
		if( free_temp_tile_data_buffers_if_possible( ) != TRUE )
		{
			LZDecompressWram( gBagScreen_GfxTileMap,
				gBagMenu->tilemapBuffer );
			gBagMenu->unk834++;
		}
		break;
	case 2:
...
void LoadBagItemListBuffers( u8 pocketId )
{
	...
	if( !gBagMenu->hideCloseBagText )
	{
		...
		{
			GetItemName( sListBuffer2->name[i],
	...
	case TMHM_POCKET:
		...
		if( itemId >= ITEM_HM01 )
		{
			ConvertIntToDecimalStringN(
				gStringVar1, itemId - ITEM_HM01 + 1, 2, 1 );
			StringExpandPlaceholders(
				dest, gText_ClearTo11Var1Clear5Var2 );
		}
		else
		{
			ConvertIntToDecimalStringN(
				gStringVar1, itemId - ITEM_TM01 + 1, 2, 2 );
			StringExpandPlaceholders(
				dest, gText_UnkF908Var1Clear7Var2 );
		}
		break;
	case BERRIES_POCKET:
		ConvertIntToDecimalStringN(
			gStringVar1, itemId - ITEM_CHERI_BERRY + 1, 2, 2 );
...
void PrintItemQuantityPlusGFX( u8 rboxId, int item_index_in_pocket, u8 a )
{
	...
	if( item_index_in_pocket != -2 )
	{
		...
		if( gBagPositionStruct.pocket == BERRIES_POCKET )
		{
			...
			( unique = ItemId_GetImportance( itemId ) ) == FALSE )
		{
			ConvertIntToDecimalStringN(
				gStringVar1, itemQuantity, 1, 2 );
			StringExpandPlaceholders( gStringVar4, gText_xVar1 );
			offset = GetStringRightAlignXOffset(
				7, gStringVar4, 0x77 );
			BagMenu_Print( rboxId,
...
void BagMenu_PrintDescription( int a )
{
	...
	if( a != -2 )
	{
		str = ItemId_GetDescription( BagGetItemIdByPocketPosition(
			gBagPositionStruct.pocket + 1, a ) );
	}
	else
	{
		StringCopy( gStringVar1,
			gReturnToXStringsTable[gBagPositionStruct.location] );
...
void bag_menu_add_pocket_scroll_arrow_indicators_maybe( void )
{
	if( gBagMenu->unk81E == 0xFF )
		gBagMenu->unk81E = AddScrollIndicatorArrowPairParameterized(
			SCROLL_ARROW_UP,
			0xAC,
			12,
			0x94,
			gBagMenu->numItemStacks[gBagPositionStruct.pocket] -
				gBagMenu->numShownItems[gBagPositionStruct
								.pocket],
			0x6E,
			0x6E,
			&gBagPositionStruct
...
void bag_menu_add_list_scroll_arrow_indicators_maybe( void )
{
	if( gBagMenu->unk81B != 1 && gBagMenu->unk81F == 0xFF )
		gBagMenu->unk81F = AddScrollIndicatorArrowPair(
			&gBagScrollArrowsTemplate, &gBagPositionStruct.unk6 );
}
...
void DisplayItemMessage(
	u8 taskId, u8 fontId, const u8* str, void ( *callback )( u8 taskId ) )
{
	s16* data = gTasks[taskId].data;
...
void BagMenu_InitListsMenu( u8 taskId )
{
	...
	LoadBagItemListBuffers( gBagPositionStruct.pocket );
	data[0] = ListMenuInit(
		&gMultiuseListMenuTemplate, *scrollPos, *cursorPos );
	schedule_bg_copy_tilemap_to_vram( 0 );
	set_callback3_to_bag( taskId );
}

void sub_81ABC3C( u8 a ) { sub_81ABC54( BagMenu_AddWindow( a ), 1 ); }
...
void sub_81ABC54( u8 a, s16 b )
{
	...
	StringExpandPlaceholders( gStringVar4, gText_xVar1 );
	AddTextPrinterParameterized( a,
		1,
		gStringVar4,
		GetStringCenterAlignXOffset( 1, gStringVar4, 0x28 ),
		2,
		0,
		0 );
}
	...
	if( sub_81221EC( ) != TRUE && !gPaletteFade.active )
	{
		...
		else
		{
			...
			default: // A_BUTTON
				...
				data[1] = listPosition;
				data[2] = BagGetQuantityByPocketPosition(
					gBagPositionStruct.pocket + 1,
					listPosition );
				gSpecialVar_ItemId =
					BagGetItemIdByPocketPosition(
						gBagPositionStruct.pocket + 1,
						listPosition );
				gUnknown_08614054[gBagPositionStruct.location](
					taskId );
				break;
			}
		}
...
void SwitchBagPocket( u8 taskId, s16 deltaBagPocketId, u16 a3 )
{
	...
	if( a3 == 0 )
	{
		...
		DestroyListMenuTask( data[0],
			...
			&gBagPositionStruct
				 .cursorPosition[gBagPositionStruct.pocket] );
		schedule_bg_copy_tilemap_to_vram( 0 );
		gSprites[gBagMenu->spriteId[2 + ( gBagMenu->unk81B_1 ^ 1 )]]
			.invisible = TRUE;
		sub_81AB824( );
	}
	...
	SetTaskFuncWithFollowupFunc(
		taskId, sub_81AC10C, gTasks[taskId].func );
}
...
void sub_81AC10C( u8 taskId )
{
	...
	if( !sub_81221AC( ) && !IsWallysBag( ) )
	{
		...
		case 1:
			ChangeBagPocketId(
				&gBagPositionStruct.pocket, data[11] );
...
void BagMenu_SwapItems( u8 taskId )
{
	...
	CopyItemName( BagGetItemIdByPocketPosition(
			      gBagPositionStruct.pocket + 1, data[1] ),
		gStringVar1 );
		...
		if( gMain.newKeys & SELECT_BUTTON )
		{
			...
			ListMenuGetScrollAndRow( data[0],
				&gBagPositionStruct.scrollPosition
					 [gBagPositionStruct.pocket],
				&gBagPositionStruct.cursorPosition
					 [gBagPositionStruct.pocket] );
			sub_81AC498( taskId );
		}
		else
		{
			...
			ListMenuGetScrollAndRow( data[0],
				...
				&gBagPositionStruct.cursorPosition
					 [gBagPositionStruct.pocket] );
			sub_80D4FC8( 0 );
			sub_80D4FEC( gBagPositionStruct.cursorPosition
					     [gBagPositionStruct.pocket] );
			switch( input )
			{
			case LIST_NOTHING_CHOSEN:
...
void sub_81AC498( u8 taskId )
{
	...
	{
		MoveItemSlotInList(
			gBagPockets[gBagPositionStruct.pocket].itemSlots,
			data[1],
			realPos );
		gBagMenu->unk81A = -1;
		DestroyListMenuTask( data[0], scrollPos, cursorPos );
...
{
	sub_8198DBC(
		a, 7, 8, 1, 0x38, b, c, sItemMenuActions, gBagMenu->unk820 );
...
void Task_HandleInBattleItemMenuInput( u8 taskId )
{
	if( sub_81221EC( ) != TRUE )
	{
		...
		default:
			PlaySE( SE_SELECT );
			sItemMenuActions[gBagMenu->unk820[r4]].func.void_u8(
				taskId );
			break;
		}
	}
...
void Task_HandleOutOfBattleItemMenuInput( u8 taskId )
{
	if( sub_81221EC( ) != TRUE )
	{
		...
		{
			if( cursorPos < ( gBagMenu->unk828 - 2 ) &&
				sub_81ACDFC( cursorPos + 2 ) )
			{
				PlaySE( SE_SELECT );
				sub_8199134( 0, 1 );
			}
		...
		{
			PlaySE( SE_SELECT );
			sItemMenuActions[gBagMenu->unk820[cursorPos]]
				.func.void_u8( taskId );
		}
...
void ItemMenu_UseOutOfBattle( u8 taskId )
{
	if( ItemId_GetFieldFunc( gSpecialVar_ItemId ) )
	{
		...
		if( CalculatePlayerPartyCount( ) == 0 &&
			ItemId_GetType( gSpecialVar_ItemId ) == 1 )
		...
		else
		{
			...
			schedule_bg_copy_tilemap_to_vram( 0 );
			if( gBagPositionStruct.pocket != BERRIES_POCKET )
				ItemId_GetFieldFunc( gSpecialVar_ItemId )(
					taskId );
			else
				sub_80FDD10( taskId );
		}
...
void ItemMenu_Toss( u8 taskId )
{
	...
	else
	{
		CopyItemName( gSpecialVar_ItemId, gStringVar1 );
		StringExpandPlaceholders(
			gStringVar4, gText_TossHowManyVar1s );
		FillWindowPixelBuffer( 1, PIXEL_FILL( 0 ) );
		BagMenu_Print( 1, 1, gStringVar4, 3, 1, 0, 0, 0, 0 );
		sub_81ABC3C( 7 );
...
void ItemMenu_Give( u8 taskId )
{
	...
	{
		DisplayItemMessage(
			taskId, 1, gText_CantWriteMail, sub_81AD350 );
	}
...
void item_menu_type_2( u8 taskId )
{
	...
	else if( !sub_8122148( gSpecialVar_ItemId ) )
	{
		CopyItemName( gSpecialVar_ItemId, gStringVar1 );
		StringExpandPlaceholders(
			gStringVar4, gText_Var1CantBeHeldHere );
		DisplayItemMessage( taskId, 1, gStringVar4, sub_81AD350 );
	}
	else if( gBagPositionStruct.pocket != KEYITEMS_POCKET &&
...
void DisplaySellItemAskString( u8 taskId )
{
	...
	if( ItemId_GetPrice( gSpecialVar_ItemId ) == 0 )
	{
		...
		DisplayItemMessage(
			taskId, 1, gStringVar4, BagMenu_InitListsMenu );
	}
	else
	{
		data[8] = 1;
		...
		else
		{
			CopyItemName( gSpecialVar_ItemId, gStringVar2 );
			StringExpandPlaceholders(
				gStringVar4, gText_HowManyToSell );
			DisplayItemMessage(
				taskId, 1, gStringVar4, sub_81AD730 );
		}
	}
}
...
void sub_81AD680( u8 taskId )
{
	...
	ConvertIntToDecimalStringN( gStringVar1,
		( ItemId_GetPrice( gSpecialVar_ItemId ) / 2 ) * data[8],
		0,
		6 );
...
void sub_81AD794( u8 taskId )
{
	...
	if( AdjustQuantityAccordingToDPadInput( &data[8], data[2] ) == TRUE )
	{
		sub_81ABCC0( gBagMenu->unk818,
			data[8],
			( ItemId_GetPrice( gSpecialVar_ItemId ) / 2 ) *
				data[8] );
	}
	else if( gMain.newKeys & A_BUTTON )
	{
...
void sub_81AD8C8( u8 taskId )
{
	...
	PrintMoneyAmountInMoneyBox(
		gBagMenu->unk819, GetMoney( &gSaveBlock1Ptr->money ), 0 );
...
void DisplayDepositItemAskString( u8 taskId )
{
	...
	else
	{
		CopyItemName( gSpecialVar_ItemId, gStringVar1 );
		StringExpandPlaceholders(
			gStringVar4, gText_DepositHowManyVar1 );
		FillWindowPixelBuffer( 1, PIXEL_FILL( 0 ) );
		BagMenu_Print( 1, 1, gStringVar4, 3, 1, 0, 0, 0, 0 );
		sub_81ABC3C( 7 );
...
void sub_81ADB14( u8 taskId )
{
	...
	else if( AddPCItem( gSpecialVar_ItemId, data[8] ) == TRUE )
	{
		...
		ConvertIntToDecimalStringN( gStringVar2, data[8], 0, 3 );
		StringExpandPlaceholders(
			gStringVar4, gText_DepositedVar2Var1s );
		BagMenu_Print( 1, 1, gStringVar4, 3, 1, 0, 0, 0, 0 );
		gTasks[taskId].func = Task_ActuallyToss;
	}
	else
...
void PrepareBagForWallyTutorial( void )
{
	...
	memcpy( gUnknown_0203CE80->bagPocket_Items,
		gSaveBlock1Ptr->bagPocket_Items,
		sizeof( gSaveBlock1Ptr->bagPocket_Items ) );
	memcpy( gUnknown_0203CE80->bagPocket_PokeBalls,
	...
	for( i = 0; i <= 4; i++ )
	{
		gUnknown_0203CE80->cursorPosition[i] =
			gBagPositionStruct.cursorPosition[i];
		gUnknown_0203CE80->scrollPosition[i] =
			gBagPositionStruct.scrollPosition[i];
	}
...
void RestoreBagAfterWallyTutorial( void )
{
	...
	memcpy( gSaveBlock1Ptr->bagPocket_Items,
		gUnknown_0203CE80->bagPocket_Items,
		sizeof( gUnknown_0203CE80->bagPocket_Items ) );
	memcpy( gSaveBlock1Ptr->bagPocket_PokeBalls,
		gUnknown_0203CE80->bagPocket_PokeBalls,
		sizeof( gUnknown_0203CE80->bagPocket_PokeBalls ) );
...
void DoWallyTutorialBagMenu( void )
{
	...
	GoToBagMenu( RETURN_LOCATION_BATTLE_2,
		ITEMS_POCKET,
		SetCB2ToReshowScreenAfterMenu2 );
}
...
u8 sub_81AE124( u8 a ) { return gBagMenu->windowPointers[a]; }
...
void BagMenu_YesNo( u8 a, u8 b, const struct YesNoFuncTable* funcTable )
{
	CreateYesNoMenuWithCallbacks(
		a, &gUnknown_086141AC[b], 1, 0, 2, 1, 14, funcTable );
}
...
void bag_menu_AddMoney_window( void )
{
	...
	PrintMoneyAmountInMoneyBoxWithBorder(
		windowId, 1, 14, GetMoney( &gSaveBlock1Ptr->money ) );
...
void PrintTMHMMoveData( u16 itemId )
{
	...
	else
	{
		...
		ConvertIntToDecimalStringN(
			gStringVar1, gBattleMoves[moveId].pp, 1, 3 );
