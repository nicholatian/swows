...
static void HallOfFame_PrintMonInfo(
	struct HallofFameMon* currMon, u8 unused1, u8 unused2 );
...
static const s16 sHallOfFame_MonFullTeamPositions[6][4] = {{120, 210, 120, 40},
	{326, 220, 56, 40},
	{-86, 220, 184, 40},
	{120, -62, 120, 88},
	{-70, -92, 200, 88},
...
static const union AnimCmd sSpriteAnim_85E5414[] = {
	ANIMCMD_FRAME( 2, 30 ), ANIMCMD_END};
...
static const struct SpriteTemplate sSpriteTemplate_85E54D0 = {.tileTag = 1001,
	.paletteTag                                                    = 1001,
...
static const struct HallofFameMon sDummyFameMon = {0x3EA03EA, 0, 0, 0, {0}};
...
static void Task_Hof_InitMonData( u8 taskId )
{
	...
	for( i = 0; i < PARTY_SIZE; i++ )
	{
		...
		{
			sHofMonPtr->mon[i].species = GetMonData(
				&gPlayerParty[i], MON_DATA_SPECIES2 );
			sHofMonPtr->mon[i].tid =
				GetMonData( &gPlayerParty[i], MON_DATA_OT_ID );
			sHofMonPtr->mon[i].personality = GetMonData(
				&gPlayerParty[i], MON_DATA_PERSONALITY );
			sHofMonPtr->mon[i].lvl =
				GetMonData( &gPlayerParty[i], MON_DATA_LEVEL );
			GetMonData(
				&gPlayerParty[i], MON_DATA_NICKNAME, nick );
...
static void Task_Hof_InitTeamSaveData( u8 taskId )
{
	...
	*lastSavedTeam = *sHofMonPtr;
	...
	AddTextPrinterParameterized2(
		0, 1, gText_SavingDontTurnOffPower, 0, NULL, 2, 1, 3 );
...
static void Task_Hof_TrySaveData( u8 taskId )
{
	...
	if( TrySavingData( SAVE_HALL_OF_FAME ) == 0xFF &&
...
static void Task_Hof_DisplayMon( u8 taskId )
{
	...
	spriteId                         = CreatePicSprite2( currMon->species,
                currMon->tid,
                currMon->personality,
                1,
                xPos,
...
static void Task_Hof_PrintMonInfoAfterAnimating( u8 taskId )
{
	...
	struct Sprite* monSprite =
		&gSprites[gTasks[taskId].tMonSpriteId( currMonId )];
...
static void Task_Hof_TryDisplayAnotherMon( u8 taskId )
{
	...
	{
		sUnknown_0203BCD4 |= ( 0x10000
			<< gSprites[gTasks[taskId].tMonSpriteId( currPokeID )]
				   .oam.paletteNum );
		if( gTasks[taskId].tDisplayedMonId <= 4 &&
			currMon[1].species !=
				SPECIES_NONE ) // there is another pokemon to
					       // display
		{
			gTasks[taskId].tDisplayedMonId++;
			BeginNormalPaletteFade( sUnknown_0203BCD4,
				0,
				12,
				12,
				RGB( 16, 29, 24 ) );
...
static void Task_Hof_PaletteFadeAndPrintWelcomeText( u8 taskId )
{
	...
	for( i = 0; i < PARTY_SIZE; i++ )
	{
		if( gTasks[taskId].tMonSpriteId( i ) != 0xFF )
			gSprites[gTasks[taskId].tMonSpriteId( i )]
				.oam.priority = 0;
	}
...
static void sub_8173DC0( u8 taskId )
{
	...
	else
	{
		...
		BeginNormalPaletteFade(
			sUnknown_0203BCD4, 0, 12, 12, RGB( 16, 29, 24 ) );
...
static void sub_8173EE4( u8 taskId )
{
	...
	gTasks[taskId].tPlayerSpriteID = CreateTrainerPicSprite(
		PlayerGenderToFrontTrainerPicId_Debug(
			gSaveBlock2Ptr->playerGender, TRUE ),
		1,
		120,
		72,
		6,
	...
	LoadWindowGfx(
		1, gSaveBlock2Ptr->optionsWindowFrameType, 0x21D, 0xD0 );
...
static void Task_Hof_WaitAndPrintPlayerInfo( u8 taskId )
{
	...
	else
	{
		...
		DrawDialogueFrame( 0, 0 );
		AddTextPrinterParameterized2(
			0, 1, gText_LeagueChamp, 0, NULL, 2, 1, 3 );
		CopyWindowToVram( 0, 3 );
...
void CB2_DoHallOfFamePC( void )
{
	...
	case 3:
		if( !sub_8175024( ) )
		{
			struct HallofFameTeam* fameTeam = (struct
				HallofFameTeam*)( gDecompressionBuffer );
	...
	else
	{
		...
		if( i < HALL_OF_FAME_MAX_TEAMS )
			...
			gTasks[taskId].tCurrTeamNo =
				HALL_OF_FAME_MAX_TEAMS - 1;

		gTasks[taskId].tCurrPageNo =
			GetGameStat( GAME_STAT_ENTERED_HOF );
...
static void Task_HofPC_PrintMonInfo( u8 taskId )
{
	...
	sUnknown_0203BCD4 =
		( 0x10000 << gSprites[currMonID].oam.paletteNum ) ^ 0xFFFF0000;
...
static void Task_HofPC_HandleInput( u8 taskId )
{
	...
	if( gMain.newKeys & A_BUTTON )
	{
		if( gTasks[taskId].tCurrTeamNo !=
			0 ) // prepare another team to view
		{
			gTasks[taskId].tCurrTeamNo--;
			for( i = 0; i < 6; i++ )
			{
	...
	else if( gMain.newKeys & B_BUTTON ) // turn off hall of fame PC
	{
		...
		gTasks[taskId].tCurrMonId != 0 ) // change
						 // mon
						 // -1
	{
...
static void Task_HofPC_PrintDataIsCorrupted( u8 taskId )
{
	...
	DrawDialogueFrame( 0, 0 );
	AddTextPrinterParameterized2(
		0, 1, gText_HOFCorrupted, 0, NULL, 2, 1, 3 );
	CopyWindowToVram( 0, 3 );
	...
	struct HallofFameMon* currMon, u8 unused1, u8 unused2 )
{
	...
	if( currMon->species != SPECIES_EGG )
	{
		...
		AddTextPrinterParameterized3(
			0, 1, 0x10, 1, sUnknown_085E5388, -1, text );
	}
	...
	if( currMon->species == SPECIES_EGG )
	{
		...
		AddTextPrinterParameterized3(
			0, 1, width, 1, sUnknown_085E5388, -1, text );
		CopyWindowToVram( 0, 3 );
	}
	else
	{
		...
		stringPtr = StringCopy(
			text + 1, gSpeciesNames[currMon->species] );

		if( currMon->species != SPECIES_NIDORAN_M &&
			currMon->species != SPECIES_NIDORAN_F )
		{
			switch( GetGenderFromSpeciesAndPersonality(
				currMon->species, currMon->personality ) )
			{
			case MON_MALE:
		...
		stringPtr[0] = EOS;
		AddTextPrinterParameterized3(
			0, 1, 0x80, 1, sUnknown_085E5388, -1, text );

		stringPtr = StringCopy( text, gText_Level );
		ConvertIntToDecimalStringN(
			stringPtr, currMon->lvl, STR_CONV_MODE_LEFT_ALIGN, 3 );
		AddTextPrinterParameterized3(
			0, 1, 0x24, 0x11, sUnknown_085E5388, -1, text );

		stringPtr = StringCopy( text, gText_IDNumber );
		ConvertIntToDecimalStringN( stringPtr,
			( u16 )( currMon->tid ),
			STR_CONV_MODE_LEADING_ZEROS,
			5 );
		AddTextPrinterParameterized3(
			0, 1, 0x68, 0x11, sUnknown_085E5388, -1, text );
...
static void HallOfFame_PrintPlayerInfo( u8 unused1, u8 unused2 )
{
	...
	AddTextPrinterParameterized3(
		1, 1, 0, 1, sUnknown_085E538C, -1, gText_Name );

	width = GetStringRightAlignXOffset(
		1, gSaveBlock2Ptr->playerName, 0x70 );
	...
	AddTextPrinterParameterized3(
		1, 1, 0, 0x11, sUnknown_085E538C, 0, gText_IDNumber );
	...
	AddTextPrinterParameterized3(
		1, 1, width, 0x11, sUnknown_085E538C, -1, text );

	AddTextPrinterParameterized3(
		1, 1, 0, 0x21, sUnknown_085E538C, -1, gText_MainMenuTime );
	...
	width = GetStringRightAlignXOffset( 1, text, 0x70 );
	AddTextPrinterParameterized3(
		1, 1, width, 0x21, sUnknown_085E538C, -1, text );
...
static void sub_8174FAC( void )
{
	...
	InitBgsFromTemplates(
		0, sHof_BgTemplates, ARRAY_COUNT( sHof_BgTemplates ) );
...
static void sub_8175364( u8 taskId )
{
	...
	case 1:
		...
		{
			var = sub_81524C4( &sOamData_85E53FC,
				0x3E9,
				0x3E9,
				Random( ) % 240,
				-( Random( ) % 8 ),
