...
void AllocateBattleResources( void )
{
	...
	gBattleResources->flags =
		AllocZeroed( sizeof( *gBattleResources->flags ) );
	gBattleResources->battleScriptsStack =
		AllocZeroed( sizeof( *gBattleResources->battleScriptsStack ) );
	gBattleResources->battleCallbackStack = AllocZeroed(
	...
	if( gBattleTypeFlags & BATTLE_TYPE_SECRET_BASE )
	{
		...
		CreateSecretBaseEnemyParty(
			&gSaveBlock1Ptr->secretBases[currSecretBaseId] );
	}
}
...
void AdjustFriendshipOnBattleFaint( u8 battlerId )
{
	...
	if( gBattleMons[opposingBattlerId].level >
	...
	{
		AdjustFriendship(
			&gPlayerParty[gBattlerPartyIndexes[battlerId]], 6 );
	}
}
...
void sub_80571DC( u8 battlerId, u8 arg1 )
{
	if( GetBattlerSide( battlerId ) != B_SIDE_OPPONENT )
	{
		...
		for( i = 0; i < 3; i++ )
			gUnknown_0203CF00[i] = *(
				0 * 3 + i + (u8*)( gBattleStruct->field_60 ) );

		sub_81B8FB0(
			pokemon_order_func( gBattlerPartyIndexes[battlerId] ),
			pokemon_order_func( arg1 ) );
		...
		case 0:
			if( gBattleMons[battlerId].status1 & STATUS1_SLEEP )
			{
				...
				else
				{
					...
					if( gBattleMons[battlerId].ability ==
						ABILITY_EARLY_BIRD )
						toSub = 2;
					else
						toSub = 1;
