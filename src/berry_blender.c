...
static bool32 Blender_PrintText(
	s16* textState, const u8* string, s32 textSpeed );
...
static void Blender_PrintMadePokeblockString(
	struct Pokeblock* pokeblock, u8* dst );
static bool32 TryAddContestLinkTvShow(
	struct Pokeblock* pokeblock, struct TvBlenderStruct* a1 );
...
static const u16 sBlenderCenterPal[] =
	INCBIN_U16( "graphics/berry_blender/center.gbapal" );
static const u8 sBlenderCenterMap[] =
	INCBIN_U8( "graphics/berry_blender/center_map.bin" );
static const u16 sBlenderOuterPal[] =
	INCBIN_U16( "graphics/berry_blender/outer.gbapal" );

// unreferenced pals?
static const u16 sUnknownPal_0[] =
	INCBIN_U16( "graphics/unknown/unknown_339514.gbapal" );
...
static const u8* const sBlenderOpponentsNames[] = {sText_Mister,
	sText_Laddie,
	sText_Lassie,
	sText_Master,
	sText_Dude,
...
static const union AnimCmd sSpriteAnim_821631C[] = {
	ANIMCMD_FRAME( 16, 5, 1, 1 ), ANIMCMD_END};
...
static const union AnimCmd sSpriteAnim_821638C[] = {
	ANIMCMD_FRAME( 0, 5, 1, 1 ), ANIMCMD_END};
...
static const union AnimCmd sSpriteAnim_82164D0[] = {
	ANIMCMD_FRAME( 5, 5, 1, 1 ), ANIMCMD_END};

static const union AnimCmd sSpriteAnim_82164D8[] = {
	ANIMCMD_FRAME( 6, 5, 1, 1 ), ANIMCMD_END};
...
static const union AnimCmd sSpriteAnim_821651C[] = {
	ANIMCMD_FRAME( 32, 30 ), ANIMCMD_END};
...
{
	m4aMPlayPitchControl( &gMPlayInfo_SE2,
		0xFFFF,
		2 * ( sBerryBlenderData->field_4C - 128 ) );
}
...
static bool8 LoadBerryBlenderGfx( void )
{
	...
	{
	case 0:
		sBerryBlenderData->tilesBuffer = AllocZeroed(
			GetDecompressedDataSize( sBlenderCenterGfx ) + 100 );
		LZDecompressWram(
			sBlenderCenterGfx, sBerryBlenderData->tilesBuffer );
	...
	case 2:
		LoadBgTiles( 2,
			...
			GetDecompressedDataSize( sBlenderCenterGfx ),
			0 );
		sBerryBlenderData->loadGfxState++;
		break;
	case 3:
		LZDecompressWram(
			gUnknown_08D91DB8, sBerryBlenderData->tilesBuffer );
		sBerryBlenderData->loadGfxState++;
		break;
	case 4:
		LoadBgTiles( 1,
			sBerryBlenderData->tilesBuffer,
			GetDecompressedDataSize( gUnknown_08D91DB8 ),
			0 );
		sBerryBlenderData->loadGfxState++;
		break;
	case 5:
		LZDecompressWram(
			gUnknown_08D927EC, sBerryBlenderData->tilesBuffer );
		sBerryBlenderData->loadGfxState++;
		break;
	case 6:
		CopyToBgTilemapBuffer( 1,
			sBerryBlenderData->tilesBuffer,
			GetDecompressedDataSize( gUnknown_08D927EC ),
			0 );
...
void DoBerryBlending( void )
{
	if( sBerryBlenderData == NULL )
		sBerryBlenderData =
			AllocZeroed( sizeof( *sBerryBlenderData ) );
...
static void sub_807FAC8( void )
{
	...
	case 0:
		...
		InitBgsFromTemplates( 1,
			sBerryBlenderBgTemplates,
			ARRAY_COUNT( sBerryBlenderBgTemplates ) );
...
static void sub_807FD64(
	struct Sprite* sprite, s16 a2, s16 a3, s16 a4, s16 a5, s16 a6 )
{
	sprite->data[0]  = a3;
	sprite->data[1]  = a2;
	sprite->data[2]  = a3;
...
static void Blender_CopyBerryData( struct BlenderBerry* berry, u16 itemId )
{
	const struct Berry* berryInfo =
		GetBerryInfo( ITEM_TO_BERRY( itemId ) );
...
static void Blender_SetPlayerNamesLocal( u8 opponentsNum )
{
	...
	case 1:
		...
		if( !FlagGet(
			...
			StringCopy( gLinkPlayers[1].name,
				sBlenderOpponentsNames[BLENDER_MASTER] );
		else
			StringCopy( gLinkPlayers[1].name,
				sBlenderOpponentsNames[BLENDER_MISTER] );
	...
	case 2:
		...
		StringCopy( gLinkPlayers[0].name, gSaveBlock2Ptr->playerName );
		StringCopy( gLinkPlayers[1].name,
			sBlenderOpponentsNames[BLENDER_DUDE] );
		StringCopy( gLinkPlayers[2].name,
			sBlenderOpponentsNames[BLENDER_LASSIE] );
	...
	case 3:
		...
		StringCopy( gLinkPlayers[0].name, gSaveBlock2Ptr->playerName );
		StringCopy( gLinkPlayers[1].name,
			sBlenderOpponentsNames[BLENDER_MISS] );
		StringCopy( gLinkPlayers[2].name,
			sBlenderOpponentsNames[BLENDER_LADDIE] );
...
static void sub_8080018( void )
{
	...
	case 4:
		...
		break;
	case 5:
		Blender_PrintText( &sBerryBlenderData->textState,
	...
	case 8:
		...
		Blender_CopyBerryData( &sBerryBlenderData->blendedBerries[0],
			gSpecialVar_ItemId );
	...
	case 11:
		...
		for( i = 0; i < BLENDER_MAX_PLAYERS; i++ )
		{
			if( sBerryBlenderData->field_114 ==
				sUnknown_083399D0
					[sBerryBlenderData->playersNo - 2][i] )
			{
				sub_807FD90(
					sBerryBlenderData->chosenItemId
		...
		if( ++sBerryBlenderData->framesToWait > 60 )
		{
				...
				sBerryBlenderData->playersNo )
			{
				...
				sBerryBlenderData->arrowPos =
					sUnknown_083399DC[sUnknown_083399E4
							[sBerryBlenderData
									->playersNo -
								2]] -
					22528;
	...
	Blender_DummiedOutFunc(
		sBerryBlenderData->bg_X, sBerryBlenderData->bg_Y );
	RunTasks( );
	AnimateSprites( );
	BuildOamBuffer( );
	RunTextPrinters( );
...
static void Blender_SetOpponentsBerryData( u16 playerBerryItemId,
	u8 playersNum,
	struct BlenderBerry* playerBerry )
{
	...
	if( playerBerryItemId == ITEM_ENIGMA_BERRY )
	{
		...
		{
			if( playerBerry->flavors[opponentSetId] >
				playerBerry->flavors[i] )
				opponentSetId = i;
		}
		opponentSetId += 5;
	}
	...
	for( i = 0; i < playersNum - 1; i++ )
	{
			...
			gSpecialVar_0x8004 == 1 )
		{
			opponentSetId %= 5;
			opponentBerryId =
				sSpecialOpponentBerrySets[opponentSetId];
			if( var <= 4 )
				opponentBerryId -= 5;
		}
		Blender_SetParticipantBerryData(
			i + 1, opponentBerryId + FIRST_BERRY_INDEX );
	}
}
...
static void Blender_PrintPlayerNames( void )
{
	...
	for( i = 0; i < BLENDER_MAX_PLAYERS; i++ )
	{
		...
		{
			sBerryBlenderData->syncArrowSpriteIds
				[sBerryBlenderData->field_8E[i]] =
				sBerryBlenderData->syncArrowSprite2Ids[i];
			StartSpriteAnim(
				&gSprites[sBerryBlenderData->syncArrowSpriteIds
						  [sBerryBlenderData->field_8E
								  [i]]],
				i );

			text[0] = EOS;
			StringCopy( text,
				gLinkPlayers[sBerryBlenderData->field_8E[i]]
					.name );
...
static void sub_80808D4( void )
{
	...
	case 21:
		...
		if( gSpecialVar_0x8004 == 1 )
		{
			if( !FlagGet(
				    FLAG_HIDE_LILYCOVE_CONTEST_HALL_BLEND_MASTER_ONLOOKERS ) )
				sBerryBlenderData->field_120[0] =
					CreateTask( sub_8081224, 10 );
			else
				sBerryBlenderData->field_120[0] =
					CreateTask( sUnknown_083399EC[0], 10 );
		}

		if( gSpecialVar_0x8004 > 1 )
		{
			for( i = 0; i < gSpecialVar_0x8004; i++ )
				sBerryBlenderData->field_120[i] = CreateTask(
					sUnknown_083399EC[i], 10 + i );
		}
...
static void sub_8080FD0( u8 taskId )
{
	...
	if( ( var1 >> 8 ) > sUnknown_083399E7[var2] + 20 &&
		( var1 >> 8 ) < sUnknown_083399E7[var2] + 40 )
	{
		if( gTasks[taskId].data[0] == 0 )
		{
			if( sBerryBlenderData->field_123 == 0 )
...
static void sub_8081288( u16 a0, u8 a1 )
{
	...
	spriteId = CreateSprite( &sUnknown_08339B40,
		sBlenderSyncArrowsPos[a1][0] -
			( 10 * sUnknown_083399C0[a1][0] ),
		sBlenderSyncArrowsPos[a1][1] -
			( 10 * sUnknown_083399C0[a1][1] ),
		1 );
	if( a0 == 0x4523 )
	{
...
static void sub_8081370( u16 a0 )
{
	...
	case 0x4523:
		if( sBerryBlenderData->field_4C < 1500 )
			sBerryBlenderData->field_4C += ( 0x180 /
				sUnknown_08339CC3[sBerryBlenderData
							  ->playersNo] );
		else
		{
			sBerryBlenderData->field_4C += ( 128 /
				sUnknown_08339CC3[sBerryBlenderData
							  ->playersNo] );
			sub_80832BC( &sBerryBlenderData->bg_X,
				( sBerryBlenderData->field_4C / 100 ) - 10 );
			sub_80832BC( &sBerryBlenderData->bg_Y,
				( sBerryBlenderData->field_4C / 100 ) - 10 );
		}
		break;
	case 0x5432:
		if( sBerryBlenderData->field_4C < 1500 )
			sBerryBlenderData->field_4C += ( 0x100 /
				sUnknown_08339CC3[sBerryBlenderData
							  ->playersNo] );
		break;
	case 0x2345:
		sBerryBlenderData->field_4C -= ( 0x100 /
			sUnknown_08339CC3[sBerryBlenderData->playersNo] );
...
static void sub_80814F4( void )
{
	...
	for( i = 0; i < sBerryBlenderData->playersNo; i++ )
	{
		if( sub_80814B0( gRecvCmds[i][0], 0x4444, 0x4400 ) )
		{
			...
			{
				sub_8081370( 0x4523 );
				sBerryBlenderData->field_116 +=
					( sBerryBlenderData->field_4C / 55 );
				if( sBerryBlenderData->field_116 >= 1000 )
					sBerryBlenderData->field_116 = 1000;
				sub_8081288( 0x4523, var );
				sBerryBlenderData
					->scores[i][BLENDER_SCORE_BEST]++;
			}
			else if( gRecvCmds[i][2] == 0x5432 )
			{
				sub_8081370( 0x5432 );
				sBerryBlenderData->field_116 +=
					( sBerryBlenderData->field_4C / 70 );
				sub_8081288( 0x5432, var );
				sBerryBlenderData
					->scores[i][BLENDER_SCORE_GOOD]++;
			}
			...
			if( gRecvCmds[i][2] == 0x2345 ||
				...
				gRecvCmds[2][i] ==
					0x5432 ) // could be a bug, 2 and i are
						 // reversed
			{
				if( sBerryBlenderData->field_4C > 1500 )
...
static void sub_8081744( void )
{
	...
	if( sBerryBlenderData->gameEndState == 0 )
	{
		...
		{
			if( ( gMain.heldKeysRaw & ( A_BUTTON | L_BUTTON ) ) !=
				( A_BUTTON | L_BUTTON ) )
				A_pressed = TRUE;
		}
		...
		if( A_pressed )
		{
			...
			StartSpriteAnim(
				...
				var2 + 4 );
			var3 = sub_8080624(
				sBerryBlenderData->arrowPos, playerId );
...
static void sub_8081898( void )
{
	...
	if( sBerryBlenderData->gameFrameTime < ( 99 * 60 * 60 ) +
			( 59 * 60 ) ) // game time can't be longer than 99
				      // minutes and 59 seconds, can't print 3
				      // digits
		sBerryBlenderData->gameFrameTime++;

	sub_8081744( );
	SetLinkDebugValues( ( u16 )( sBerryBlenderData->field_4C ),
		sBerryBlenderData->field_116 );
	sub_80814F4( );
	...
	sub_8082AD4( );
	if( sBerryBlenderData->gameEndState == 0 &&
		sBerryBlenderData->field_118 >= 1000 )
	{
		sBerryBlenderData->field_116    = 1000;
...
static bool8 sub_8081964( struct BlenderBerry* berries, u8 index1, u8 index2 )
{
	if( berries[index1].itemId != berries[index2].itemId ||
		( StringCompare(
			  berries[index1].name, berries[index2].name ) == 0 &&
			( berries[index1].flavors[FLAVOR_SPICY] ==
					berries[index2]
						.flavors[FLAVOR_SPICY] &&
				berries[index1].flavors[FLAVOR_DRY] ==
					berries[index2].flavors[FLAVOR_DRY] &&
				berries[index1].flavors[FLAVOR_SWEET] ==
					berries[index2]
						.flavors[FLAVOR_SWEET] &&
				berries[index1].flavors[FLAVOR_BITTER] ==
					berries[index2]
						.flavors[FLAVOR_BITTER] &&
				berries[index1].flavors[FLAVOR_SOUR] ==
					berries[index2].flavors[FLAVOR_SOUR] &&
				berries[index1].smoothness ==
					berries[index2].smoothness ) ) )
		return TRUE;
	else
		return FALSE;
}

static u32 Blender_GetPokeblockColor(
	struct BlenderBerry* berries, s16* a1, u8 playersNo, u8 a3 )
{
	s16 vars[6];
	s32 i;
	s32 r6;
	...
	for( i = 0; i < playersNo; i++ )
	{
		...
		{
			if( berries[i].itemId == berries[r6].itemId &&
				i != r6 &&
				( berries[i].itemId != ITEM_ENIGMA_BERRY ||
					sub_8081964( berries, i, r6 ) ) )
				return 12;
		}
	}
	r2 = 0;
...
static void sub_8081BB0( s16 value ) { sUnknown_03000E04 = value; }

static s16 sub_8081BBC( void ) { return sUnknown_03000E04; }

static void sub_8081BC8( s16 value ) { sUnknown_03000E06 = value; }

static s16 sub_8081BD4( void ) { return sUnknown_03000E06; }

static void Blender_CalculatePokeblock( struct BlenderBerry* berries,
	...
	u16 maxRPM )
{
	...
	pokeblock->color = Blender_GetPokeblockColor(
		berries, &sUnknown_03000DE8[0], playersNo, var3 );
	sUnknown_03000DE8[5] =
		( sUnknown_03000DE8[5] / playersNo ) - playersNo;
...
static void BlenderDebug_CalculatePokeblock( struct BlenderBerry* berries,
	struct Pokeblock* pokeblock,
	u8 playersNo,
	u8* flavors,
	u16 a4 )
{
	Blender_CalculatePokeblock(
		berries, pokeblock, playersNo, flavors, a4 );
}
	...
	case 3:
		...
		else if( IsLinkTaskFinished( ) )
		{
			if( gReceivedRemoteLinkPlayers != 0 &&
				gWirelessCommType )
			{
				sBerryBlenderData->gameBlock.timeRPM.time =
					sBerryBlenderData->gameFrameTime;
				sBerryBlenderData->gameBlock.timeRPM.max_RPM =
					sBerryBlenderData->max_RPM;

				for( i = 0; i < BLENDER_MAX_PLAYERS; i++ )
				{
					for( j = 0; j < BLENDER_SCORES_NO;
						j++ )
						sBerryBlenderData->gameBlock
							.scores[i][j] =
							sBerryBlenderData
								->scores[i][j];
				}
				...
				if( SendBlock( 0,
					    &sBerryBlenderData->gameBlock,
					    sizeof( sBerryBlenderData
							    ->gameBlock ) ) )
					sBerryBlenderData->gameEndState++;
			}
			else
			{
				sBerryBlenderData->smallBlock.time =
					sBerryBlenderData->gameFrameTime;
				sBerryBlenderData->smallBlock.max_RPM =
					sBerryBlenderData->max_RPM;
				if( SendBlock( 0,
	...
	case 4:
		if( GetBlockReceivedStatus( ) )
		{
			...
			if( gReceivedRemoteLinkPlayers != 0 &&
				gWirelessCommType )
			{
				struct BlenderGameBlock* receivedBlock = (struct
					BlenderGameBlock*)( &gBlockRecvBuffer );

				sBerryBlenderData->max_RPM =
					receivedBlock->timeRPM.max_RPM;
				sBerryBlenderData->gameFrameTime =
					receivedBlock->timeRPM.time;
			...
			{
				struct TimeAndRPM* receivedBlock = (struct
					TimeAndRPM*)( &gBlockRecvBuffer );

				sBerryBlenderData->max_RPM =
					receivedBlock->max_RPM;
				sBerryBlenderData->gameFrameTime =
					receivedBlock->time;
			}
		}
		...
		if( sBerryBlenderData->yesNoAnswer == 0 )
		{
			if( IsBagPocketNonEmpty( POCKET_BERRIES ) ==
				FALSE ) // no
					// berries
			{
				sBerryBlenderData->playAgainState =
					CANT_PLAY_NO_BERRIES;
				gSendCmd[1] = 0x9999;
			}
			else if( GetFirstFreePokeblockSlot( ) ==
				-1 ) // no space for
				     // pokeblocks
			{
				sBerryBlenderData->playAgainState =
...
static void CB2_HandlePlayerLinkPlayAgainChoice( void )
{
	...
	case 1:
		...
		StringCopy( gStringVar4,
			gLinkPlayers[sBerryBlenderData->field_6E].name );
		StringAppend(
			gStringVar4, sText_ApostropheSPokeblockCaseIsFull );
		break;
	case 2:
		sBerryBlenderData->gameEndState++;
		StringCopy( gStringVar4,
...
static void CB2_HandlePlayerPlayAgainChoice( void )
{
	...
	case 0:
		...
		if( sBerryBlenderData->playAgainState == CANT_PLAY_NO_BERRIES )
			sBerryBlenderData->gameEndState = 2;
		if( sBerryBlenderData->playAgainState ==
			CANT_PLAY_NO_PKBLCK_SPACE )
			sBerryBlenderData->gameEndState = 1;
		break;
	case 1:
	...
	if( gReceivedRemoteLinkPlayers )
	{
			...
			sBerryBlenderData->field_64[0] != 0x2222 )
		{
			...
			if( i == GetLinkPlayerCount( ) )
			{
				for( i = 0; i < GetLinkPlayerCount( ); i++ )
				{
					if( sBerryBlenderData->field_64[i] !=
						0x7779 )
						break;
				}
				sub_8081F94( &gSendCmd[0] );
...
u16 GetBlenderArrowPosition( void ) { return sBerryBlenderData->arrowPos; }
...
static void sub_8082E84( void )
{
	...
	for( i = 0; i < limit; i++ )
	{
		...
		spriteId =
			CreateSprite( &sUnknown_08339BE0, x + 120, y + 80, 1 );
...
static void Blender_SetParticipantBerryData( u8 participantId, u16 itemId )
{
	...
	Blender_CopyBerryData(
		&sBerryBlenderData->blendedBerries[participantId], itemId );
}
...
static u32 sub_8083210( u16 a0 ) { return 0x57E40 * a0 / 0x10000; }
...
static bool8 sub_8083380( void )
{
	...
	sub_8083334(
		&sBerryBlenderData->bg_X, sBerryBlenderData->framesToWait );
	sub_8083334(
		&sBerryBlenderData->bg_Y, sBerryBlenderData->framesToWait );
...
{
	if( gSaveBlock1Ptr->berryBlenderRecords[sBerryBlenderData->playersNo -
		    2] < sBerryBlenderData->max_RPM )
		gSaveBlock1Ptr
			->berryBlenderRecords[sBerryBlenderData->playersNo -
				2] = sBerryBlenderData->max_RPM;
}
...
static bool8 Blender_PrintBlendingResults( void )
{
	...
	case 3: {
		...
		xPos = GetStringCenterAlignXOffset(
			1, sText_BlendingResults, 0xA8 );
		Blender_AddTextPrinter(
			5, sText_BlendingResults, xPos, 1, TEXT_SPEED_FF, 0 );
		...
		for( i = 0; i < sBerryBlenderData->playersNo; yPos += 16, i++ )
		{
			...
			ConvertIntToDecimalStringN(
				...
				1 );
			StringAppend(
				sBerryBlenderData->stringVar, sText_Dot );
			StringAppend(
				sBerryBlenderData->stringVar, gText_Space );
			StringAppend( sBerryBlenderData->stringVar,
				gLinkPlayers[place].name );
			Blender_AddTextPrinter( 5,
				sBerryBlenderData->stringVar,
				8,
				yPos,
				TEXT_SPEED_FF,
			...
			StringCopy( sBerryBlenderData->stringVar,
				sBerryBlenderData->blendedBerries[place]
					.name );
		...
		Blender_AddTextPrinter(
			5, sText_MaximumSpeed, 0, 0x51, TEXT_SPEED_FF, 3 );
		...
		xPos = GetStringRightAlignXOffset(
			1, sBerryBlenderData->stringVar, 0xA8 );
		Blender_AddTextPrinter( 5,
			sBerryBlenderData->stringVar,
			xPos,
			0x51,
			...
			3 );
		Blender_AddTextPrinter(
			5, sText_Time, 0, 0x61, TEXT_SPEED_FF, 3 );
		...
		ConvertIntToDecimalStringN( sBerryBlenderData->stringVar,
			...
			2 );
		txtPtr = StringAppend(
			sBerryBlenderData->stringVar, sText_Min );

		ConvertIntToDecimalStringN(
			txtPtr, seconds, STR_CONV_MODE_LEADING_ZEROS, 2 );
	...
	case 5:
		...
		Blender_PrintMadePokeblockString(
			&pokeblock, sBerryBlenderData->stringVar );
		TryAddContestLinkTvShow(
			&pokeblock, &sBerryBlenderData->tvBlender );
	...
	struct Pokeblock* pokeblock, u8* dst )
{
	...
	ConvertIntToDecimalStringN(
		text, flavorLvl, STR_CONV_MODE_LEFT_ALIGN, 3 );
...
static void Blender_SortScores( void )
{
	...
	Blender_SortBasedOnPoints(
		places, sBerryBlenderData->playersNo, points );
	for( i = 0; i < sBerryBlenderData->playersNo; i++ )
...
static bool8 Blender_PrintBlendingRanking( void )
{
	...
	case 3:
		...
		Blender_AddTextPrinter(
			5, sText_Ranking, xPos, 1, TEXT_SPEED_FF, 0 );

		sBerryBlenderData->scoreIconIds[BLENDER_SCORE_BEST] =
			CreateSprite( &sUnknown_08339B40, 128, 52, 0 );
		StartSpriteAnim(
			&gSprites[sBerryBlenderData
					  ->scoreIconIds[BLENDER_SCORE_BEST]],
			3 );
		gSprites[sBerryBlenderData->scoreIconIds[BLENDER_SCORE_BEST]]
			.callback = SpriteCallbackDummy;

		sBerryBlenderData->scoreIconIds[BLENDER_SCORE_GOOD] =
			CreateSprite( &sUnknown_08339B40, 160, 52, 0 );
		gSprites[sBerryBlenderData->scoreIconIds[BLENDER_SCORE_GOOD]]
			.callback = SpriteCallbackDummy;
		...
		StartSpriteAnim(
			&gSprites[sBerryBlenderData
					  ->scoreIconIds[BLENDER_SCORE_MISS]],
			1 );
		gSprites[sBerryBlenderData->scoreIconIds[BLENDER_SCORE_MISS]]
			.callback = SpriteCallbackDummy;

		Blender_SortScores( );

		for( yPos = 0x29, i = 0; i < sBerryBlenderData->playersNo;
			yPos += 0x10, i++ )
		{
			u8 place = sBerryBlenderData->playerPlaces[i];
			...
			ConvertIntToDecimalStringN(
				sBerryBlenderData->stringVar,
				sBerryBlenderData
					->scores[place][BLENDER_SCORE_BEST],
				STR_CONV_MODE_RIGHT_ALIGN,
				3 );
			Blender_AddTextPrinter( 5,
				sBerryBlenderData->stringVar,
			...
			ConvertIntToDecimalStringN(
				sBerryBlenderData->stringVar,
				sBerryBlenderData
					->scores[place][BLENDER_SCORE_GOOD],
				STR_CONV_MODE_RIGHT_ALIGN,
				3 );
			Blender_AddTextPrinter( 5,
				sBerryBlenderData->stringVar,
			...
			ConvertIntToDecimalStringN(
				sBerryBlenderData->stringVar,
				sBerryBlenderData
					->scores[place][BLENDER_SCORE_MISS],
				STR_CONV_MODE_RIGHT_ALIGN,
				3 );
			Blender_AddTextPrinter( 5,
				sBerryBlenderData->stringVar,
...
void ShowBerryBlenderRecordWindow( void )
{
	...
	xPos = GetStringCenterAlignXOffset(
		1, gText_BlenderMaxSpeedRecord, 0x90 );
	...
	AddTextPrinterParameterized(
		gRecordsWindowId, 1, gText_234Players, 4, 0x29, 0, NULL );
	...
	for( i = 0, yPos = 0x29; i < BLENDER_SCORES_NO; i++ )
	{
		...
		txtPtr = ConvertIntToDecimalStringN(
			text, record / 100, STR_CONV_MODE_RIGHT_ALIGN, 3 );
		txtPtr = StringAppend( txtPtr, sText_Dot );
		txtPtr = ConvertIntToDecimalStringN(
			txtPtr, record % 100, STR_CONV_MODE_LEADING_ZEROS, 2 );
	...
	if( gReceivedRemoteLinkPlayers != 0 )
	{
		if( sBerryBlenderData->field_1A4 == 0 && sheen > 20 )
		{
			StringCopy( tvBlender->name,
				gLinkPlayers[sBerryBlenderData->playerPlaces
						     [sBerryBlenderData
								     ->playersNo -
							     1]]
					.name );
			tvBlender->pokeblockFlavor =
				GetPokeblocksFlavor( pokeblock );
		...
		else if( sBerryBlenderData->field_1A4 ==
				sBerryBlenderData->playersNo - 1 &&
			sheen <= 20 )
		{
			StringCopy( tvBlender->name,
				gLinkPlayers[sBerryBlenderData
						     ->playerPlaces[0]]
					.name );
			tvBlender->pokeblockFlavor =
				GetPokeblocksFlavor( pokeblock );
	...
	u8 windowId, const u8* string, u8 x, u8 y, s32 speed, s32 caseId )
{
	...
	AddTextPrinterParameterized4(
		windowId, 1, x, y, letterSpacing, 1, txtColor, speed, string );
}
