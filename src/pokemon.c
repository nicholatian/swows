...
static const u8 sUnreferencedData[] = {0x34,
	0x00,
	0x10,
	0x00,
	0x01,
...
const u16 gSpeciesToHoennPokedexNum[] = // Assigns all species to the Hoenn Dex
					// Index (Summary No. for Hoenn Dex)
	{
...
const u16 gSpeciesToNationalPokedexNum[] = // Assigns all species to the
					   // National Dex Index (Summary No.
					   // for National Dex)
	{
...
const u16 gHoennToNationalOrder[] = // Assigns Hoenn Dex Pokémon (Using
				    // National Dex Index)
	{
		...
		HOENN_TO_NATIONAL( BULBASAUR ), // Pokémon from here onwards
						// are UNSEEN in the HoennDex.
...
const u8 gPPUpGetMask[] = {0x03, 0x0c, 0x30, 0xc0}; // Masks for getting PP Up
						    // count, also PP Max
						    // values
const u8 gPPUpSetMask[] = {0xfc, 0xf3, 0xcf, 0x3f}; // Masks for setting PP Up
						    // count
const u8 gPPUpAddMask[] = {0x01, 0x04, 0x10, 0x40}; // Values added to PP Up
						    // count
...
const u16 gLinkPlayerFacilityClasses[] = {FACILITY_CLASS_COOLTRAINER_M,
	...
	u32 fixedOtId )
{
	...
	if( otIdType == OT_ID_RANDOM_NO_SHINY ) // Pokemon cannot be shiny
	{
		...
		{
			value      = Random32( );
			shinyValue = HIHALF( value ) ^ LOHALF( value ) ^
	...
	SetBoxMonData( boxMon,
		MON_DATA_EXP,
		&gExperienceTables[gBaseStats[species].growthRate][level] );
	SetBoxMonData(
		boxMon, MON_DATA_FRIENDSHIP, &gBaseStats[species].friendship );
	...
	SetBoxMonData(
		boxMon, MON_DATA_OT_GENDER, &gSaveBlock2Ptr->playerGender );
...
void CreateMaleMon( struct Pokemon* mon, u16 species, u8 level )
{
	...
	do
	{
		...
		MON_MALE );
	CreateMon(
		mon, species, level, 32, 1, personality, OT_ID_PRESET, otId );
}
...
{
	CreateMon(
		mon, species, level, 0, 1, personality, OT_ID_PLAYER_ID, 0 );
...
bool8 sub_80688F8( u8 caseId, u8 battlerId )
{
	...
	case 5:
		if( gBattleTypeFlags & BATTLE_TYPE_LINK )
		{
			...
			if( gBattleTypeFlags & BATTLE_TYPE_MULTI )
			{
				if( gLinkPlayers[GetMultiplayerId( )].id ==
					battlerId )
					return FALSE;
			}
			else
...
static s32 GetDeoxysStat( struct Pokemon* mon, s32 statId )
{
	...
	statValue = ( u16 )(
		( ( sDeoxysBaseStats[statId] * 2 + ivVal + evVal / 4 ) *
			mon->level ) /
			100 +
		5 );
...
#define CALC_STAT( base, iv, ev, statIndex, field ) \
	{ \
		...
		s32 n       = ( ( ( 2 * baseStat + iv + ev / 4 ) * level ) / \
                                100 ) + \
			5; \
...
void CalculateMonStats( struct Pokemon* mon )
{
	...
	CALC_STAT( baseSpAttack,
		spAttackIV,
		spAttackEV,
		STAT_SPATK,
		MON_DATA_SPATK )
	CALC_STAT( baseSpDefense,
		spDefenseIV,
		spDefenseEV,
		STAT_SPDEF,
		MON_DATA_SPDEF )
...
u16 GiveMoveToBoxMon( struct BoxPokemon* boxMon, u16 move )
{
	...
	{
		u16 existingMove =
			GetBoxMonData( boxMon, MON_DATA_MOVE1 + i, NULL );
		if( !existingMove )
		{
			SetBoxMonData( boxMon, MON_DATA_MOVE1 + i, &move );
			SetBoxMonData( boxMon,
				MON_DATA_PP1 + i,
				&gBattleMoves[move].pp );
			return move;
		}
...
u16 MonTryLearningNewMove( struct Pokemon* mon, bool8 firstMove )
{
	...
	if( firstMove )
	{
			       ...
			       0xFE00 ) != ( level << 9 ) )
		{
			...
			if( gLevelUpLearnsets[species][sLearningMoveTableID] ==
				LEVEL_UP_END )
				return 0;
		}
	}

	if( ( gLevelUpLearnsets[species][sLearningMoveTableID] & 0xFE00 ) ==
		( level << 9 ) )
	{
		gMoveToLearn =
			( gLevelUpLearnsets[species][sLearningMoveTableID] &
				0x1FF );
		sLearningMoveTableID++;
...
#define APPLY_STAT_MOD( var, mon, stat, statIndex ) \
	{ \
		( var ) = ( stat ) * \
			( gStatStageRatios )[( mon )->statStages[( \
				statIndex )]][0]; \
		( var ) /= ( gStatStageRatios )[( mon )->statStages[( \
			statIndex )]][1]; \
	}
	...
	u8 battlerIdDef )
{
	...
	if( attacker->item == ITEM_ENIGMA_BERRY )
	{
		...
		attackerHoldEffectParam =
			gEnigmaBerries[battlerIdAtk].holdEffectParam;
	}
	else
	{
		attackerHoldEffect = ItemId_GetHoldEffect( attacker->item );
		attackerHoldEffectParam =
			ItemId_GetHoldEffectParam( attacker->item );
	}
	...
	if( defender->item == ITEM_ENIGMA_BERRY )
	{
		...
		defenderHoldEffectParam =
			gEnigmaBerries[battlerIdDef].holdEffectParam;
	}
	else
	{
		defenderHoldEffect = ItemId_GetHoldEffect( defender->item );
		defenderHoldEffectParam =
			ItemId_GetHoldEffectParam( defender->item );
	}
	...
	if( type == TYPE_ELECTRIC &&
		AbilityBattleEffects(
			ABILITYEFFECT_FIELD_SPORT, 0, 0, 0xFD, 0 ) )
		gBattleMovePower /= 2;
	if( type == TYPE_FIRE &&
		AbilityBattleEffects(
			ABILITYEFFECT_FIELD_SPORT, 0, 0, 0xFE, 0 ) )
		gBattleMovePower /= 2;
	...
	if( IS_TYPE_PHYSICAL( type ) )
	{
		if( gCritMultiplier == 2 )
		{
			if( attacker->statStages[STAT_ATK] > 6 )
				APPLY_STAT_MOD(
					damage, attacker, attack, STAT_ATK )
			else
				damage = attack;
		}
		else
		...
		if( gCritMultiplier == 2 )
		{
			...
			APPLY_STAT_MOD(
				damageHelper, defender, defense, STAT_DEF )
	...
	if( IS_TYPE_SPECIAL( type ) )
	{
		if( gCritMultiplier == 2 )
		{
			...
			APPLY_STAT_MOD(
				damage, attacker, spAttack, STAT_SPATK )
		...
		if( gCritMultiplier == 2 )
		{
			...
			APPLY_STAT_MOD(
				damageHelper, defender, spDefense, STAT_SPDEF )
...
static bool8 ShouldGetStatBadgeBoost( u16 badgeFlag, u8 battlerId )
{
	if( gBattleTypeFlags &
		( BATTLE_TYPE_LINK | BATTLE_TYPE_EREADER_TRAINER |
			BATTLE_TYPE_x2000000 | BATTLE_TYPE_FRONTIER ) )
		return FALSE;
...
{
	u8 opposing =
		BATTLE_OPPOSITE( GetBattlerPosition( battlerId ) & BIT_SIDE );
	...
	else
	{
		if( ( gAbsentBattlerFlags & gBitTable[opposing] ) )
			return GetBattlerAtPosition(
				BATTLE_PARTNER( opposing ) );
		else
			return GetBattlerAtPosition( opposing );
	}
}
...
static const u8 gender_tbl[3] = {MON_GENDERLESS, MON_FEMALE, MON_MALE};

u8 GetBoxMonGender( struct BoxPokemon* boxMon )
{
	return gender_tbl[get_gender(
		GetBoxMonData( boxMon, MON_DATA_PERSONALITY, NULL ),
		GetBoxMonData( boxMon, MON_DATA_SPECIES, NULL ) )];
}

int get_gender( u32 pid, u16 species )
{
	return gBaseStats[species].genderRatio == 0xFF
		? GENDER_NONE
		: gBaseStats[species].genderRatio > ( pid & 0xFF ) ? GENDER_F
								   : GENDER_M;
}
...
void SetMultiuseSpriteTemplateToPokemon( u16 speciesTag, u8 battlerPosition )
{
	if( gMonSpritesGfxPtr != NULL )
		...
		gMultiuseSpriteTemplate =
			gUnknown_020249B4[1]->templates[battlerPosition];
	else
	...
	if( battlerPosition == B_POSITION_PLAYER_LEFT ||
		...
		gMultiuseSpriteTemplate.anims =
			gMonFrontAnimsPtrTable[speciesTag];
}
		...
		battlerPosition == B_POSITION_PLAYER_RIGHT )
	{
		gMultiuseSpriteTemplate =
			gSpriteTemplateTable_TrainerBackSprites
				[trainerSpriteId];
		gMultiuseSpriteTemplate.anims =
			gTrainerBackAnimsPtrTable[trainerSpriteId];
	}
	else
	{
		if( gMonSpritesGfxPtr != NULL )
			...
			gMultiuseSpriteTemplate =
				gUnknown_08329D98[battlerPosition];
		gMultiuseSpriteTemplate.anims =
			gTrainerFrontAnimsPtrTable[trainerSpriteId];
	}
}
...
#define SUBSTRUCT_CASE( n, v1, v2, v3, v4 ) \
	case n: { \
		union PokemonSubstruct* substructs0 = \
			boxMon->secure.substructs; \
		union PokemonSubstruct* substructs1 = \
			boxMon->secure.substructs; \
		union PokemonSubstruct* substructs2 = \
			boxMon->secure.substructs; \
		union PokemonSubstruct* substructs3 = \
			boxMon->secure.substructs; \
		union PokemonSubstruct* substructs4 = \
			boxMon->secure.substructs; \
		union PokemonSubstruct* substructs5 = \
			boxMon->secure.substructs; \
		union PokemonSubstruct* substructs6 = \
			boxMon->secure.substructs; \
		union PokemonSubstruct* substructs7 = \
			boxMon->secure.substructs; \
		union PokemonSubstruct* substructs8 = \
			boxMon->secure.substructs; \
		union PokemonSubstruct* substructs9 = \
			boxMon->secure.substructs; \
		union PokemonSubstruct* substructs10 = \
			boxMon->secure.substructs; \
		union PokemonSubstruct* substructs11 = \
			boxMon->secure.substructs; \
		union PokemonSubstruct* substructs12 = \
			boxMon->secure.substructs; \
		union PokemonSubstruct* substructs13 = \
			boxMon->secure.substructs; \
		union PokemonSubstruct* substructs14 = \
			boxMon->secure.substructs; \
		union PokemonSubstruct* substructs15 = \
			boxMon->secure.substructs; \
		union PokemonSubstruct* substructs16 = \
			boxMon->secure.substructs; \
		union PokemonSubstruct* substructs17 = \
			boxMon->secure.substructs; \
		union PokemonSubstruct* substructs18 = \
			boxMon->secure.substructs; \
		union PokemonSubstruct* substructs19 = \
			boxMon->secure.substructs; \
		union PokemonSubstruct* substructs20 = \
			boxMon->secure.substructs; \
		union PokemonSubstruct* substructs21 = \
			boxMon->secure.substructs; \
		union PokemonSubstruct* substructs22 = \
			boxMon->secure.substructs; \
		union PokemonSubstruct* substructs23 = \
			boxMon->secure.substructs; \
\
...
u32 GetBoxMonData( struct BoxPokemon* boxMon, s32 field, u8* data )
{
	...
	// Any field greater than MON_DATA_ENCRYPT_SEPARATOR is encrypted and
	// must be treated as such
	if( field > MON_DATA_ENCRYPT_SEPARATOR )
	{
		substruct0 = &( GetSubstruct( boxMon, boxMon->personality, 0 )
					->type0 );
		substruct1 = &( GetSubstruct( boxMon, boxMon->personality, 1 )
					->type1 );
		substruct2 = &( GetSubstruct( boxMon, boxMon->personality, 2 )
					->type2 );
		substruct3 = &( GetSubstruct( boxMon, boxMon->personality, 3 )
					->type3 );
	...
	case MON_DATA_SPECIES2:
		...
		if( substruct0->species &&
			( substruct3->isEgg || boxMon->isBadEgg ) )
			retVal = SPECIES_EGG;
		break;
	case MON_DATA_IVS:
...
void SetBoxMonData( struct BoxPokemon* boxMon, s32 field, const void* dataArg )
{
	...
	case MON_DATA_OBEDIENCE:
		...
		break;
	case MON_DATA_IVS: {
		u32 ivs = data[0] | ( data[1] << 8 ) | ( data[2] << 16 ) |
			( data[3] << 24 );
	...
	do
	{
		for( boxPos = 0; boxPos < IN_BOX_COUNT; boxPos++ )
		{
			struct BoxPokemon* checkingMon =
				GetBoxedMonPtr( boxNo, boxPos );
			if( GetBoxMonData( checkingMon,
				    MON_DATA_SPECIES,
				    NULL ) == SPECIES_NONE )
			{
				MonRestorePP( mon );
...
u8 CalculatePlayerPartyCount( void )
{
	...
	while( gPlayerPartyCount < 6 &&
		GetMonData( &gPlayerParty[gPlayerPartyCount],
			MON_DATA_SPECIES,
			NULL ) != SPECIES_NONE )
	{
		gPlayerPartyCount++;
...
u8 CalculateEnemyPartyCount( void )
{
	...
	while( gEnemyPartyCount < 6 &&
		GetMonData( &gEnemyParty[gEnemyPartyCount],
			MON_DATA_SPECIES,
			NULL ) != SPECIES_NONE )
	{
		gEnemyPartyCount++;
...
u8 GetMonsStateToDoubles_2( void )
{
	...
	for( i = 0; i < PARTY_SIZE; i++ )
	{
		u32 species = GetMonData(
			&gPlayerParty[i], MON_DATA_SPECIES2, NULL );
	...
	if( aliveCount == 1 )
		return PLAYER_HAS_ONE_MON; // may have more than one, but only
					   // one is alive
...
{
	u8 facilityClass = sSecretBaseFacilityClasses
...
u8 CalculatePPWithBonus( u16 move, u8 ppBonuses, u8 moveIndex )
{
	...
	return basePP +
		( ( basePP * 20 *
			  ( ( gPPUpGetMask[moveIndex] & ppBonuses ) >>
				  ( 2 * moveIndex ) ) ) /
			100 );
}
...
void CopyPlayerPartyMonToBattleData( u8 battlerId, u8 partyIndex )
{
	...
	gBattleMons[battlerId].species = GetMonData(
		&gPlayerParty[partyIndex], MON_DATA_SPECIES, NULL );
	gBattleMons[battlerId].item = GetMonData(
		&gPlayerParty[partyIndex], MON_DATA_HELD_ITEM, NULL );

	for( i = 0; i < MAX_MON_MOVES; i++ )
	{
		gBattleMons[battlerId].moves[i] = GetMonData(
			&gPlayerParty[partyIndex], MON_DATA_MOVE1 + i, NULL );
		gBattleMons[battlerId].pp[i] = GetMonData(
			&gPlayerParty[partyIndex], MON_DATA_PP1 + i, NULL );
	}

	gBattleMons[battlerId].ppBonuses = GetMonData(
		&gPlayerParty[partyIndex], MON_DATA_PP_BONUSES, NULL );
	gBattleMons[battlerId].friendship = GetMonData(
		&gPlayerParty[partyIndex], MON_DATA_FRIENDSHIP, NULL );
	...
	gBattleMons[battlerId].speedIV = GetMonData(
		&gPlayerParty[partyIndex], MON_DATA_SPEED_IV, NULL );
	gBattleMons[battlerId].spAttackIV = GetMonData(
		&gPlayerParty[partyIndex], MON_DATA_SPATK_IV, NULL );
	gBattleMons[battlerId].spDefenseIV = GetMonData(
		&gPlayerParty[partyIndex], MON_DATA_SPDEF_IV, NULL );
	gBattleMons[battlerId].personality = GetMonData(
		&gPlayerParty[partyIndex], MON_DATA_PERSONALITY, NULL );
	...
	hpSwitchout =
		&gBattleStruct->hpOnSwitchout[GetBattlerSide( battlerId )];
	...
	struct Pokemon* mon, u16 item, u8 partyIndex, u8 moveIndex, u8 e )
{
	...
	if( heldItem == ITEM_ENIGMA_BERRY )
	{
		if( gMain.inBattle )
			holdEffect =
				gEnigmaBerries[gBattlerInMenuId].holdEffect;
		else
	...
	for( cmdIndex = 0; cmdIndex < 6; cmdIndex++ )
	{
		...
		case 1:
			...
			if( ( itemEffect[cmdIndex] & ITEM1_X_SPEED ) &&
				gBattleMons[gActiveBattler]
						.statStages[STAT_SPEED] < 12 )
			{
				gBattleMons[gActiveBattler]
					.statStages[STAT_SPEED] +=
		...
		case 3:
					...
					MAX_LEVEL )
			{
				dataUnsigned = gExperienceTables
					[gBaseStats[GetMonData( mon,
							    MON_DATA_SPECIES,
							    NULL )]
							.growthRate]
					[GetMonData(
						 mon, MON_DATA_LEVEL, NULL ) +
						1];
			...
			if( ( itemEffect[cmdIndex] & ITEM3_PARALYSIS ) &&
				HealStatusConditions( mon,
					...
					battlerId ) == 0 )
				retVal = FALSE;
			if( ( itemEffect[cmdIndex] &
				    ITEM3_CONFUSION ) // heal confusion
				&& gMain.inBattle &&
				battlerId != MAX_BATTLERS_COUNT &&
				( gBattleMons[battlerId].status2 &
					STATUS2_CONFUSION ) )
		...
		case 4:
			...
			if( r10 & ITEM4_PP_UP )
			{
				...
				dataUnsigned =
					...
					( moveIndex * 2 );
				var_38 = CalculatePPWithBonus(
					GetMonData( mon,
						MON_DATA_MOVE1 + moveIndex,
						NULL ),
					GetMonData( mon,
						MON_DATA_PP_BONUSES,
						NULL ),
...
static void sub_806CF24( s32 arg0 )
{
	...
	StringCopy(
		gBattleTextBuff1, gStatNamesTable[gUnknown_08329EC8[arg0]] );
...
u8* sub_806CF78( u16 itemId )
{
	...
	if( itemId == ITEM_ENIGMA_BERRY )
	{
		if( gMain.inBattle )
			itemEffect =
				gEnigmaBerries[gBattlerInMenuId].itemEffect;
		else
			itemEffect = gSaveBlock1Ptr->enigmaBerry.itemEffect;
	}
	else
...
u8 GetNatureFromPersonality( u32 personality ) { return personality % 25; }
	...
	struct Pokemon* mon, u8 type, u16 evolutionItem )
{
	...
	case 0:
		...
		for( i = 0; i < EVOS_PER_MON; i++ )
		{
			...
			case EVO_FRIENDSHIP_NIGHT:
				...
				break;
			case EVO_LEVEL:
				if( gEvolutionTable[species][i].param <=
					level )
					targetSpecies =
						gEvolutionTable[species][i]
							.targetSpecies;
	...
	case 1:
		for( i = 0; i < EVOS_PER_MON; i++ )
		{
			...
			{
			case EVO_TRADE:
				targetSpecies = gEvolutionTable[species][i]
							.targetSpecies;
				break;
			case EVO_TRADE_ITEM:
				if( gEvolutionTable[species][i].param ==
...
u16 HoennPokedexNumToSpecies( u16 hoennNum )
{
	...
	while( species < ( NUM_SPECIES - 1 ) &&
		gSpeciesToHoennPokedexNum[species] != hoennNum )
		species++;
...
u16 NationalPokedexNumToSpecies( u16 nationalNum )
{
	...
	while( species < ( NUM_SPECIES - 1 ) &&
		gSpeciesToNationalPokedexNum[species] != nationalNum )
		species++;
...
u16 NationalToHoennOrder( u16 nationalNum )
{
	...
	while( hoennNum < ( NUM_SPECIES - 1 ) &&
		gHoennToNationalOrder[hoennNum] != nationalNum )
		hoennNum++;
...
void EvolutionRenameMon( struct Pokemon* mon, u16 oldSpecies, u16 newSpecies )
{
	...
	if( language == GAME_LANGUAGE &&
		...
		SetMonData(
			mon, MON_DATA_NICKNAME, gSpeciesNames[newSpecies] );
}
...
{
	if( InBattlePyramid( ) )
		return GetBattlePyramindTrainerEncounterMusicId(
			trainerOpponentId );
	else if( sub_81D5C18( ) )
		return GetTrainerEncounterMusicIdInTrainerHill(
			trainerOpponentId );
	else
...
u16 ModifyStatByNature( u8 nature, u16 n, u8 statIndex )
{
	...
	{
		// Should just be "return n", but it wouldn't match without
		// this.
		u16 retVal = n;
		retVal++;
		retVal--;
...
void AdjustFriendship( struct Pokemon* mon, u8 event )
{
	...
	if( species && species != SPECIES_EGG )
	{
							...
							TRAINER_CLASS_CHAMPION ) ) ) )
		{
			...
			if( mod > 0 )
			{
				if( GetMonData( mon, MON_DATA_POKEBALL, 0 ) ==
					ITEM_LUXURY_BALL )
					friendship++;
				if( GetMonData(
					    mon, MON_DATA_MET_LOCATION, 0 ) ==
					GetCurrentRegionMapSectionId( ) )
					friendship++;
			}
			if( friendship < 0 )
...
void MonGainEVs( struct Pokemon* mon, u16 defeatedSpecies )
{
	...
	for( i = 0; i < NUM_STATS; i++ )
	{
		...
		case 1:
			evIncrease =
				gBaseStats[defeatedSpecies].evYield_Attack *
				multiplier;
			break;
		case 2:
			evIncrease =
				gBaseStats[defeatedSpecies].evYield_Defense *
				multiplier;
			break;
		case 3:
			evIncrease =
				gBaseStats[defeatedSpecies].evYield_Speed *
				multiplier;
			break;
		case 4:
			evIncrease =
				gBaseStats[defeatedSpecies].evYield_SpAttack *
				multiplier;
			break;
		case 5:
			evIncrease =
				gBaseStats[defeatedSpecies].evYield_SpDefense *
				multiplier;
			break;
		}
		...
		if( totalEVs + (s16)evIncrease > MAX_TOTAL_EVS )
			evIncrease = ( (s16)evIncrease + MAX_TOTAL_EVS ) -
				( totalEVs + evIncrease );
...
void UpdatePartyPokerusTime( u16 days )
{
	...
	for( i = 0; i < PARTY_SIZE; i++ )
	{
		...
		{
			u8 pokerus = GetMonData(
				&gPlayerParty[i], MON_DATA_POKERUS, 0 );
			if( pokerus & 0xF )
			{
...
void PartySpreadPokerus( struct Pokemon* party )
{
	if( ( Random( ) % 3 ) == 0 )
	{
		...
		for( i = 0; i < PARTY_SIZE; i++ )
		{
			...
			{
				u8 pokerus = GetMonData(
					&party[i], MON_DATA_POKERUS, 0 );
				u8 curPokerus = pokerus;
				if( pokerus )
				{
					if( pokerus & 0xF )
					{
						// Spread to adjacent party
						// members.
						if( i != 0 &&
							!( GetMonData(
								   &party[i -
...
bool8 TryIncrementMonLevel( struct Pokemon* mon )
{
	...
	if( expPoints >
		gExperienceTables[gBaseStats[species].growthRate][MAX_LEVEL] )
	{
		expPoints = gExperienceTables[gBaseStats[species].growthRate]
					     [MAX_LEVEL];
...
u16 GetBattleBGM( void )
{
	...
	else if( gBattleTypeFlags & BATTLE_TYPE_TRAINER )
	{
		...
		if( gBattleTypeFlags & BATTLE_TYPE_FRONTIER )
			...
			trainerClass = gTrainers[gTrainerBattleOpponent_A]
					       .trainerClass;
		...
		case TRAINER_CLASS_PKMN_TRAINER_3:
			if( gBattleTypeFlags & BATTLE_TYPE_FRONTIER )
				return MUS_BATTLE35;
			if( !StringCompare( gTrainers[gTrainerBattleOpponent_A]
						    .trainerName,
...
const u32* GetMonFrontSpritePal( struct Pokemon* mon )
{
	...
	u32 personality = GetMonData( mon, MON_DATA_PERSONALITY, 0 );
	return GetFrontSpritePalFromSpeciesAndPersonality(
	...
	u16 species, u32 otId, u32 personality )
{
	...
	shinyValue = HIHALF( otId ) ^ LOHALF( otId ) ^ HIHALF( personality ) ^
		LOHALF( personality );
	if( shinyValue < 8 )
	...
	struct Pokemon* mon )
{
	...
	u32 otId        = GetMonData( mon, MON_DATA_OT_ID, 0 );
	u32 personality = GetMonData( mon, MON_DATA_PERSONALITY, 0 );
	return GetMonSpritePalStructFromOtIdPersonality(
		species, otId, personality );
}
...
void BoxMonRestorePP( struct BoxPokemon* boxMon )
{
	...
	for( i = 0; i < MAX_MON_MOVES; i++ )
	{
		...
		{
			u16 move =
				GetBoxMonData( boxMon, MON_DATA_MOVE1 + i, 0 );
			u16 bonus = GetBoxMonData(
				boxMon, MON_DATA_PP_BONUSES, 0 );
...
void SetMonPreventsSwitchingString( void )
{
	...
	if( GetBattlerSide( gBattleStruct->battlerPreventingSwitchout ) ==
		B_SIDE_PLAYER )
		gBattleTextBuff1[3] = pokemon_order_func( gBattlerPartyIndexes
				[gBattleStruct->battlerPreventingSwitchout] );
	else
		gBattleTextBuff1[3] = gBattlerPartyIndexes
			[gBattleStruct->battlerPreventingSwitchout];

	PREPARE_MON_NICK_WITH_PREFIX_BUFFER( gBattleTextBuff2,
		gBattlerInMenuId,
		pokemon_order_func( gBattlerPartyIndexes[gBattlerInMenuId] ) )

	BattleStringExpandPlaceholders(
		gText_PkmnsXPreventsSwitching, gStringVar4 );
}

static s32 GetWildMonTableIdInAlteringCave( u16 species )
{
	s32 i;
	for( i = 0; i < (s32)ARRAY_COUNT( sAlteringCaveWildMonHeldItems );
		i++ )
...
{
	if( !( gBattleTypeFlags &
		    ( BATTLE_TYPE_LEGENDARY | BATTLE_TYPE_TRAINER |
			    BATTLE_TYPE_PYRAMID | BATTLE_TYPE_PIKE ) ) )
	{
		u16 rnd = Random( ) % 100;
		u16 species =
			GetMonData( &gEnemyParty[0], MON_DATA_SPECIES, 0 );
		u16 var1 = 45;
		u16 var2 = 95;
		if( !GetMonData(
			    &gPlayerParty[0], MON_DATA_SANITY_IS_EGG, 0 ) &&
		...
		{
			s32 alteringCaveId =
				GetWildMonTableIdInAlteringCave( species );
			if( alteringCaveId != 0 )
			{
				if( rnd < var2 )
					return;
...
{
	bool8 retVal   = FALSE;
	u32 shinyValue = HIHALF( otId ) ^ LOHALF( otId ) ^
		HIHALF( personality ) ^ LOHALF( personality );
	if( shinyValue < 8 )
		retVal = TRUE;
	return retVal;
...
const u8* GetTrainerPartnerName( void )
{
	...
	else
	{
		...
		return gLinkPlayers[GetBattlerMultiplayerId(
					    gLinkPlayers[id].id ^ 2 )]
			.name;
	}
}
	...
	struct Sprite* sprite, u16 species, bool8 noCry, u8 arg3 )
{
	if( gHitMarker & HITMARKER_NO_ANIMATIONS &&
		...
		DoMonFrontSpriteAnimation(
			sprite, species, noCry, arg3 | 0x80 );
	else
	...
	else
	{
		...
		if( sMonAnimationDelayTable[species - 1] != 0 )
		{
			...
			gTasks[taskId].data[3] =
				sMonAnimationDelayTable[species - 1];
		}
		else
		{
			LaunchAnimationTaskForFrontSprite(
				sprite, sMonFrontAnimIdsTable[species - 1] );
		}
	...
	struct Sprite* sprite, u16 species, bool8 oneFrame )
{
	...
	{
		u8 taskId =
			CreateTask( Task_PokemonSummaryAnimateAfterDelay, 0 );
		STORE_PTR_IN_TASK( sprite, taskId, 0 );
...
{
	u8 delayTaskId =
		FindTaskIdByFunc( Task_PokemonSummaryAnimateAfterDelay );
	if( delayTaskId != 0xFF )
...
void BattleAnimateBackSprite( struct Sprite* sprite, u16 species )
{
	...
	else
	{
		LaunchAnimationTaskForBackSprite(
			sprite, GetSpeciesBackAnimSet( species ) );
		sprite->callback = SpriteCallbackDummy_2;
	}
}
...
void HandleSetPokedexFlag( u16 nationalNum, u8 caseId, u32 personality )
{
	...
	if( !GetSetPokedexFlag(
		    nationalNum, getFlagCaseId ) ) // don't set if it's
						   // already set
	{
		GetSetPokedexFlag( nationalNum, caseId );
		if( NationalPokedexNumToSpecies( nationalNum ) ==
			SPECIES_UNOWN )
			gSaveBlock2Ptr->pokedex.unownPersonality = personality;
		if( NationalPokedexNumToSpecies( nationalNum ) ==
			SPECIES_SPINDA )
			gSaveBlock2Ptr->pokedex.spindaPersonality =
				personality;
	}
}
...
static void sub_806F160( struct Unknown_806F160_Struct* structPtr )
{
	...
	for( i = 0; i < FORCE_SIGNED( structPtr->field_0_0 ); i++ )
	{
		...
		{
			asm( "" );
			structPtr->frameImages[i * structPtr->field_1 + j]
				.data = &structPtr->byteArrays[i][j * 0x800];
		}
		structPtr->templates[i].images =
			&structPtr->frameImages[i * structPtr->field_1];
	}
}
...
static void sub_806F1FC( struct Unknown_806F160_Struct* structPtr )
{
	...
	for( i = 0; i < FORCE_SIGNED( structPtr->field_0_0 ); i++ )
	{
		...
		for( j = 0; j < structPtr->field_1; j++ )
		{
			structPtr->frameImages[i * structPtr->field_0_0 + j]
				.data = &structPtr->byteArrays[i][j * 0x800];
		}
		structPtr->templates[i].images =
			&structPtr->frameImages[i * structPtr->field_0_0];
...
struct Unknown_806F160_Struct* sub_806F2AC( u8 id, u8 arg1 )
{
	...
	structPtr->bytes = AllocZeroed(
		structPtr->field_3_0 * 0x800 * 4 * structPtr->field_0_0 );
	...
	{
		for( i = 0; i < FORCE_SIGNED( structPtr->field_0_0 ); i++ )
			structPtr->byteArrays[i] = structPtr->bytes +
				( structPtr->field_3_0 * ( i << 0xD ) );
	}

	structPtr->templates = AllocZeroed(
		sizeof( struct SpriteTemplate ) * structPtr->field_0_0 );
	structPtr->frameImages =
		AllocZeroed( sizeof( struct SpriteFrameImage ) *
			structPtr->field_0_0 * structPtr->field_1 );
	...
	{
		for( i = 0; i < structPtr->field_1 * structPtr->field_0_0;
			i++ )
...
void sub_806F47C( u8 id )
{
	...
	if( structPtr->magic != 0xA3 )
	{
		memset( structPtr,
			0,
			sizeof( struct Unknown_806F160_Struct ) );
	}
	else
	{
		if( structPtr->frameImages != NULL )
