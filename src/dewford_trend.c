...
static bool8 sub_8122A58(
	struct EasyChatPair* a, struct EasyChatPair* b, u8 c );
...
static s16 GetEqualEasyChatPairIndex(
	struct EasyChatPair* s, struct EasyChatPair* a, u16 b );
...
void InitDewfordTrend( void )
{
	...
	for( i = 0; i < 5; i++ )
	{
		gSaveBlock1Ptr->easyChatPairs[i].words[0] =
			sub_811EE38( EC_GROUP_CONDITIONS );

		if( Random( ) & 1 )
			gSaveBlock1Ptr->easyChatPairs[i].words[1] =
				sub_811EE38( EC_GROUP_LIFESTYLE );
		else
			gSaveBlock1Ptr->easyChatPairs[i].words[1] =
				sub_811EE38( EC_GROUP_HOBBIES );
...
void UpdateDewfordTrendPerDay( u16 a )
{
	...
	if( a != 0 )
	{
		...
		for( i = 0; i < 5; i++ )
		{
			u32 r4;
			u32 r2 = sp0;
			struct EasyChatPair* r5 =
				&( gSaveBlock1Ptr->easyChatPairs[i] );
...
bool8 sub_81226D8( u16* a )
{
	...
	if( !SB1ContainsWords( a ) )
	{
		if( !FlagGet( FLAG_SYS_POPWORD_INPUT ) )
		{
			...
			{
				gSaveBlock1Ptr->easyChatPairs[0].words[0] =
					a[0];
				gSaveBlock1Ptr->easyChatPairs[0].words[1] =
					a[1];
				return TRUE;
			}
		}
		...
		for( i = 0; i < 5; i++ )
		{
				    ...
				    0 ) )
			{
				...
				while( r3 > i )
				{
					gSaveBlock1Ptr->easyChatPairs[r3] =
						gSaveBlock1Ptr
							->easyChatPairs[r3 -
								1];
					r3--;
	...
	if( buffer1 != NULL )
	{
		...
		else
		{
			...
			for( i = 0; i < players; i++ )
			{
				for( j = 0; j < 5; j++ )
				{
					s16 foo = GetEqualEasyChatPairIndex(
						buffer2, src, r3 );
					if( foo < 0 )
					{
						*( dst++ ) = *src;
					...
					{
						foo_of_buffer2 = (struct
							EasyChatPair*)( (u32)buffer2 +
							( foo * 8 ) ); // required
								       // to do
								       // this
								       // to
...
{
	struct EasyChatPair* s =
		&gSaveBlock1Ptr->easyChatPairs[gSpecialVar_0x8004];
...
void TrendyPhraseIsOld( void )
{
	...
	if( gSaveBlock1Ptr->easyChatPairs[0].unk0_0 -
			gSaveBlock1Ptr->easyChatPairs[1].unk0_0 <
		2 )
	{
		asm( "" ::
				: "r2" ); // Force the compiler to store
					  // address of gSaveBlock1 in r3
					  // instead of r2
		if( !gSaveBlock1Ptr->easyChatPairs[0].unk1_6 &&
			gSaveBlock1Ptr->easyChatPairs[1].unk1_6 )
			result = 1;
	}
...
static bool8 SB1ContainsWords( u16* a )
{
	...
	for( i = 0; i < 5; i++ )
	{
		if( IsEasyChatPairEqual(
			    a, gSaveBlock1Ptr->easyChatPairs[i].words ) != 0 )
			return TRUE;
	}
	return FALSE;
}
