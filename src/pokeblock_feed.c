...
static const struct WindowTemplate sWindowTemplates[] = {
	{0, 1, 0xF, 0x1C, 4, 0xF, 0xA}, DUMMY_WIN_TEMPLATE};
...
static bool8 LoadMonAndSceneGfx( struct Pokemon* mon )
{
	...
	case 1:
		...
		palette     = GetMonSpritePalStructFromOtIdPersonality(
	...
	case 6:
		...
		decompress_and_copy_tile_data_to_vram(
			1, gBattleTerrainTiles_Building, 0, 0, 0 );
		sPokeblockFeed->loadGfxState++;
		break;
	case 7:
		if( free_temp_tile_data_buffers_if_possible( ) != TRUE )
		{
			LZDecompressWram( gUnknown_08D9BA44,
				sPokeblockFeed->tilemapBuffer );
			sPokeblockFeed->loadGfxState++;
		}
		break;
	case 8:
...
static void SetPokeblockSpritePal( u8 pokeblockCaseId )
{
	u8 colorId = GetPokeblockData(
		&gSaveBlock1Ptr->pokeblocks[pokeblockCaseId], PBLOCK_COLOR );
	...
	if( !gPaletteFade.active )
	{
		...
		case 255:
			DoPokeblockCaseThrowEffect(
				sPokeblockFeed->pokeblockCaseSpriteId,
				gTasks[taskId].tData1 );
			break;
		case 269:
			sPokeblockFeed->pokeblockSpriteId =
		...
		if( sPokeblockFeed->field_1058 < sPokeblockFeed->field_1056 )
			sub_817A634( );
		else if( sPokeblockFeed->field_1058 ==
			sPokeblockFeed->field_1056 )
...
static void Task_HandleMonAtePokeblock( u8 taskId )
{
	...
	if( gPokeblockGain == 0 )
		...
		StringExpandPlaceholders(
			gStringVar4, gText_Var1HappilyAteVar2 );
	else
		StringExpandPlaceholders(
			gStringVar4, gText_Var1DisdainfullyAteVar2 );
...
{
	u8 spriteId =
		CreateSprite( &sThrownPokeblockSpriteTemplate, 174, 84, 1 );
...
static void sub_817A634( void )
{
	...
	case 0:
		pokeblockFeed->animId =
			sNatureToMonPokeblockAnim[pokeblockFeed->nature][0];
		pokeblockFeed->monSpritePtr =
			&gSprites[pokeblockFeed->monSpriteId_];
	...
	case 10:
		...
		if( sNatureToMonPokeblockAnim[pokeblockFeed->nature][1] != 0 )
		{
			...
			pokeblockFeed->monSpritePtr->affineAnims =
	...
	case 50:
		if( sNatureToMonPokeblockAnim[pokeblockFeed->nature][1] != 0 )
		{
			if( !pokeblockFeed->noMonFlip ) // double negation, so
							// mon's sprite is
							// flipped
				StartSpriteAffineAnim(
					pokeblockFeed->monSpritePtr,
					sNatureToMonPokeblockAnim
							[pokeblockFeed->nature]
							[1] +
						10 );
			else
				StartSpriteAffineAnim(
	...
	case 60:
		if( sub_817A9E4( ) == TRUE )
		{
			...
			{
				FreeOamMatrix( pokeblockFeed->monSpritePtr->oam
						       .matrixNum );
...
static bool8 sub_817A91C( void )
{
	...
	for( i = 0; i < 10; i++ )
		pokeblockFeed->field_1060[i] =
			sMonPokeblockAnims[pokeblockFeed->animId][i];
	...
	else
	{
		pokeblockFeed->field_1060[10] =
			Sin( pokeblockFeed->field_1060[0],
				pokeblockFeed->field_1060[2] );
		pokeblockFeed->field_1060[11] =
			Cos( pokeblockFeed->field_1060[0],
				pokeblockFeed->field_1060[3] );
		pokeblockFeed->field_1060[12] = pokeblockFeed->field_1060[4];
		pokeblockFeed->field_1060[13] =
			pokeblockFeed->monSpritePtr->pos2.x;
		pokeblockFeed->field_1060[14] =
			pokeblockFeed->monSpritePtr->pos2.y;
		sub_817AB68( );
...
{
	u16 var =
		sPokeblockFeed->field_1060[12] - sPokeblockFeed->field_1060[4];
...
static void sub_817AA54( void )
{
	...
	s16 var3 =
		pokeblockFeed->field_1060[13] + pokeblockFeed->field_1060[6];
...
static void sub_817AB68( void )
{
	...
	while( 1 )
	{
		...
		r4 = pokeblockFeed->field_1060[12] -
