...
static void RegistryMenu_OnCursorMove(
	int unused, bool8 flag, struct ListMenu* menu );
...
static const struct SecretBaseEntranceMetatiles
...
void TrySetCurSecretBaseIndex( void )
{
	...
	{
		if( sCurSecretBaseId ==
			gSaveBlock1Ptr->secretBases[i].secretBaseId )
		{
...
void ToggleSecretBaseEntranceMetatile( void )
{
	...
	{
		if( sSecretBaseEntranceMetatiles[i].closedMetatileId ==
			metatileId )
		{
			MapGridSetMetatileIdAt( x,
				y,
				sSecretBaseEntranceMetatiles[i]
						.openMetatileId |
	...
	for( i = 0; i < ARRAY_COUNT( sSecretBaseEntranceMetatiles ); i++ )
	{
		if( sSecretBaseEntranceMetatiles[i].openMetatileId ==
			metatileId )
		{
			MapGridSetMetatileIdAt( x,
				y,
...
void SetOccupiedSecretBaseEntranceMetatiles( struct MapEvents const* events )
{
	...
	for( bgId = 0; bgId < events->bgEventCount; bgId++ )
	{
		if( events->bgEvents[bgId].kind == BG_EVENT_SECRET_BASE )
		{
			...
			{
				if( gSaveBlock1Ptr->secretBases[j]
						.secretBaseId ==
					events->bgEvents[bgId]
						.bgUnion.secretBaseId )
				{
					...
					for( i = 0; i <
						...
						i++ )
					{
						if( sSecretBaseEntranceMetatiles[i]
								.closedMetatileId ==
							tile_id )
						{
...
static void Task_EnterSecretBase( u8 taskId )
{
	...
	case 1:
		...
		if( gSaveBlock1Ptr->secretBases[secretBaseId].numTimesEntered <
			255 )
			gSaveBlock1Ptr->secretBases[secretBaseId]
				.numTimesEntered++;
...
{
	EventObjectTurn(
		&gEventObjects[gPlayerAvatar.eventObjectId], DIR_NORTH );
...
bool8 CurMapIsSecretBase( void )
{
	if( gSaveBlock1Ptr->location.mapGroup ==
			MAP_GROUP( SECRET_BASE_RED_CAVE1 ) &&
...
void InitSecretBaseAppearance( bool8 hidePC )
{
	...
	if( CurMapIsSecretBase( ) )
	{
		secretBaseId = VarGet( VAR_CURRENT_SECRET_BASE );
		decorations =
			gSaveBlock1Ptr->secretBases[secretBaseId].decorations;
		decorPos = gSaveBlock1Ptr->secretBases[secretBaseId]
				   .decorationPositions;
		for( x = 0; x < 16; x++ )
		{
			if( decorations[x] > 0 &&
				decorations[x] <= NUM_DECORATIONS &&
				gDecorations[decorations[x]].permission !=
					DECORPERM_SPRITE )
				ShowDecorationOnMap( ( decorPos[x] >> 4 ) + 7,
					( decorPos[x] & 0xF ) + 7,
					decorations[x] );
		}
		...
		if( secretBaseId != 0 )
		{
			// Another player's secret base. Change PC type to the
			// "Register" PC.
			FindMetatileIdMapCoords( &x, &y, 0x220 );
			MapGridSetMetatileIdAt( x + 7,
				y + 7,
				0x221 | METATILE_COLLISION_MASK );
		}
		else if( hidePC == TRUE &&
...
void InitSecretBaseDecorationSprites( void )
{
	...
	else
	{
		...
		decorations =
			gSaveBlock1Ptr->secretBases[secretBaseId].decorations;
		decorationPositions = gSaveBlock1Ptr->secretBases[secretBaseId]
					      .decorationPositions;
		numDecorations = 16;
	}
	...
	for( i = 0; i < numDecorations; i++ )
	{
		...
		{
			for( eventObjectId = 0; eventObjectId <
				gMapHeader.events->eventObjectCount;
				eventObjectId++ )
			{
				if( gMapHeader.events
						->eventObjects[eventObjectId]
						.flagId ==
					FLAG_DECORATION_1 +
						gSpecialVar_0x8004 )
					break;
					...
					metatileBehavior ) == TRUE )
			{
				gSpecialVar_Result = VAR_OBJ_GFX_ID_0 +
					( gMapHeader.events
							...
							.graphicsId -
						EVENT_OBJ_GFX_VAR_0 );
				VarSet( gSpecialVar_Result,
					gDecorations[decorations[i]]
						.tiles[0] );
				gSpecialVar_Result =
					gMapHeader.events
						->eventObjects[eventObjectId]
				...
				FlagClear( FLAG_DECORATION_1 +
					gSpecialVar_0x8004 );
				TrySpawnEventObject( gSpecialVar_Result,
					gSaveBlock1Ptr->location.mapNum,
					gSaveBlock1Ptr->location.mapGroup );
				TryMoveEventObjectToMapCoords(
					gSpecialVar_Result,
					gSaveBlock1Ptr->location.mapNum,
					gSaveBlock1Ptr->location.mapGroup,
					gSpecialVar_0x8006,
...
void HideSecretBaseDecorationSprites( void )
{
	...
	for( eventObjectId = 0;
		eventObjectId < gMapHeader.events->eventObjectCount;
		eventObjectId++ )
	{
		...
		if( flag >= FLAG_DECORATION_1 && flag <= FLAG_DECORATION_14 )
		{
			RemoveEventObjectByLocalIdAndMap(
				gMapHeader.events->eventObjects[eventObjectId]
					.localId,
				gSaveBlock1Ptr->location.mapNum,
				gSaveBlock1Ptr->location.mapGroup );
			FlagSet( flag );
...
void SetSecretBaseOwnerGfxId( void )
{
	VarSet( VAR_OBJ_GFX_ID_F,
		sSecretBaseOwnerGfxIds[GetSecretBaseOwnerType(
			VarGet( VAR_CURRENT_SECRET_BASE ) )] );
}
...
static u8* GetSecretBaseName( u8* dest, u8 secretBaseId )
{
	*StringCopyN( dest,
		...
		GetNameLength( gSaveBlock1Ptr->secretBases[secretBaseId]
				       .trainerName ) ) = EOS;
	ConvertInternationalString(
		dest, gSaveBlock1Ptr->secretBases[secretBaseId].language );
...
void CopyCurSecretBaseOwnerName_StrVar1( void )
{
	...
	*StringCopyN( gStringVar1, name, GetNameLength( name ) ) = EOS;
	ConvertInternationalString( gStringVar1,
		gSaveBlock1Ptr->secretBases[secretBaseId].language );
}
	...
	if( gSaveBlock1Ptr->secretBases[0].secretBaseId )
	{
		for( i = 0; i < PARTY_SIZE; i++ )
		{
			for( moveIdx = 0; moveIdx < MAX_MON_MOVES; moveIdx++ )
				party->moves[i * MAX_MON_MOVES + moveIdx] =
					MOVE_NONE;
			...
			{
				for( moveIdx = 0; moveIdx < MAX_MON_MOVES;
					moveIdx++ )
					party->moves[partyId * MAX_MON_MOVES +
						moveIdx] =
						GetMonData( &gPlayerParty[i],
							MON_DATA_MOVE1 +
								moveIdx );

				party->species[partyId] = GetMonData(
					&gPlayerParty[i], MON_DATA_SPECIES );
				party->heldItems[partyId] = GetMonData(
					&gPlayerParty[i], MON_DATA_HELD_ITEM );
				party->levels[partyId] = GetMonData(
					&gPlayerParty[i], MON_DATA_LEVEL );
				party->personality[partyId] =
					GetMonData( &gPlayerParty[i],
						MON_DATA_PERSONALITY );
				party->EVs[partyId] =
					GetAverageEVs( &gPlayerParty[i] );
				partyId++;
			}
		}
	}
	...
	for( i = 0; i < events->bgEventCount; i++ )
	{
				...
				events->bgEvents[i].bgUnion.secretBaseId )
		{
			metatileId = MapGridGetMetatileIdAt(
				events->bgEvents[i].x + 7,
				events->bgEvents[i].y + 7 );
			for( j = 0; j < 7; j++ )
			{
				if( sSecretBaseEntranceMetatiles[j]
						.openMetatileId == metatileId )
				{
					MapGridSetMetatileIdAt(
						events->bgEvents[i].x + 7,
						events->bgEvents[i].y + 7,
...
static void Task_ShowSecretBaseRegistryMenu( u8 taskId )
{
	...
	else
	{
		DisplayItemMessageOnField(
			taskId, gText_NoRegistry, GoToSecretBasePCMainMenu );
	}
}
...
static void BuildRegistryMenuItems( u8 taskId )
{
	...
	for( i = 1; i < SECRET_BASES_COUNT; i++ )
	{
		if( IsSecretBaseRegistered( i ) )
		{
			...
			sRegistryMenu->items[count].name =
				sRegistryMenu->names[count];
...
{
	s16* data = gTasks[taskId].data;
	data[8]   = AddScrollIndicatorArrowPairParameterized( SCROLL_ARROW_UP,
                0xbc,
                0x0c,
                0x94,
                data[0] - data[3],
...
static void ShowRegistryMenuDeleteConfirmation( u8 taskId )
{
	...
	StringExpandPlaceholders(
		gStringVar4, gText_OkayToDeleteFromRegistry );
	DisplayItemMessageOnField(
		taskId, gStringVar4, ShowRegistryMenuDeleteYesNo );
}
...
{
	u8 ownerType =
		GetSecretBaseOwnerType( VarGet( VAR_CURRENT_SECRET_BASE ) );
	if( ownerType == 0 )
...
void GetSecretBaseOwnerInteractionState( void )
{
	...
	{
		for( i = 0; i < SECRET_BASES_COUNT; i++ )
			gSaveBlock1Ptr->secretBases[i].battledOwnerToday =
				FALSE;
	...
	gSpecialVar_Result =
		gSaveBlock1Ptr->secretBases[secretBaseId].battledOwnerToday;
}
...
void SecretBasePerStepCallback( u8 taskId )
{
	...
	case 1:
		...
		if( x != data[2] || y != data[3] )
		{
			...
			else if( tileId == 0x2b8 || tileId == 0x2b9 ||
				tileId == 0x2ba || tileId == 0x2c0 ||
				tileId == 0x2c1 || tileId == 0x2c2 ||
				tileId == 0x2c8 || tileId == 0x2c9 ||
				tileId == 0x2ca )
			{
				if( sInFriendSecretBase == TRUE )
			...
			else if( tileId == 0x239 || tileId == 0x241 ||
				tileId == 0x251 || tileId == 0x259 )
			{
				if( sInFriendSecretBase == TRUE )
					VarSet( VAR_SECRET_BASE_LOW_TV_FLAGS,
						VarGet( VAR_SECRET_BASE_LOW_TV_FLAGS ) |
			...
			else if( MetatileBehavior_IsSecretBaseGlitterMat(
					 behavior ) == TRUE )
			{
				if( sInFriendSecretBase == TRUE )
					VarSet( VAR_SECRET_BASE_HIGH_TV_FLAGS,
			...
			else if( MetatileBehavior_IsSecretBaseBalloon(
					 behavior ) == TRUE )
			{
				PopSecretBaseBalloon(
					MapGridGetMetatileIdAt( x, y ), x, y );
				if( sInFriendSecretBase == TRUE )
				{
					switch( (int)MapGridGetMetatileIdAt(
						x, y ) )
					{
					case 0x338:
					case 0x33c:
			...
			else if( MetatileBehavior_IsSecretBaseBreakableDoor(
					 behavior ) == TRUE )
			{
				if( sInFriendSecretBase == TRUE )
					VarSet( VAR_SECRET_BASE_HIGH_TV_FLAGS,
			...
			else if( MetatileBehavior_IsSecretBaseSoundMat(
					 behavior ) == TRUE )
			{
				if( sInFriendSecretBase == TRUE )
					VarSet( VAR_SECRET_BASE_LOW_TV_FLAGS,
			...
			else if( MetatileBehavior_IsSecretBaseJumpMat(
					 behavior ) == TRUE )
			{
				if( sInFriendSecretBase == TRUE )
					VarSet( VAR_SECRET_BASE_HIGH_TV_FLAGS,
			...
			else if( MetatileBehavior_IsSecretBaseSpinMat(
					 behavior ) == TRUE )
			{
				if( sInFriendSecretBase == TRUE )
					VarSet( VAR_SECRET_BASE_HIGH_TV_FLAGS,
...
static void SetSecretBaseDataAndLanguage( u8 secretBaseId,
...
static bool8 SecretBasesHaveSameTrainerName(
	struct SecretBase* sbr1, struct SecretBase* sbr2 )
{
	u8 i;
...
static s16 GetSecretBaseIndexFromId( u8 secretBaseId )
{
	...
	for( i = 0; i < SECRET_BASES_COUNT; i++ )
	{
		if( gSaveBlock1Ptr->secretBases[i].secretBaseId ==
			secretBaseId )
			return i;
	}
...
static u8 sub_80EAA64(
	struct SecretBase* secretBase, u32 version, u32 language )
{
	s16 index;
	...
	if( index != 0 )
	{
		if( index != -1 )
		{
			if( gSaveBlock1Ptr->secretBases[index].sbr_field_1_0 ==
				1 )
				return 0;

			if( gSaveBlock1Ptr->secretBases[index]
						.registryStatus != 2 ||
				secretBase->sbr_field_1_0 == 1 )
			{
				SetSecretBaseDataAndLanguage(
					index, secretBase, version, language );
				return index;
			}
		}
		else
...
static void SortSecretBasesByRegistryStatus( void )
{
	...
	for( i = 1; i < 19; i++ )
	{
		...
		{
			if( ( secretBases[i].registryStatus == 0 &&
				    secretBases[j].registryStatus == 1 ) ||
				( secretBases[i].registryStatus == 2 &&
					secretBases[j].registryStatus != 2 ) )
			{
...
void sub_80EABA4( struct SecretBaseRecordMixer* mixer, u8 registryStatus )
{
	...
	for( i = 1; i < SECRET_BASES_COUNT; i++ )
	{
		if( mixer->secretBases[i].registryStatus == registryStatus )
			sub_80EAA64( &mixer->secretBases[i],
				mixer->version,
				mixer->language );
	}
}
...
bool8 SecretBaseBelongsToPlayer( struct SecretBase* secretBase )
{
	...
	for( i = 0; i < PLAYER_NAME_LENGTH &&
		...
		i++ )
	{
		if( secretBase->trainerName[i] !=
			gSaveBlock2Ptr->playerName[i] )
			return FALSE;
	}
	...
	for( i = 0; i < SECRET_BASES_COUNT; i++ )
	{
		if( secretBases[i].secretBaseId != 0 )
		{
				    ...
				    secretBase, &secretBases[i] ) == TRUE )
			{
				...
				secretBases[i].sbr_field_1_0 =
					secretBase->sbr_field_1_0;
...
void sub_80EAEB4( struct SecretBaseRecordMixer* mixers )
{
	...
	for( i = 0; i < SECRET_BASES_COUNT; i++ )
	{
		sub_80EAE90( &mixers[0].secretBases[i],
			mixers[0].version,
			mixers[0].language );
		sub_80EAE90( &mixers[1].secretBases[i],
			mixers[1].version,
			mixers[1].language );
		sub_80EAE90( &mixers[2].secretBases[i],
			mixers[2].version,
			mixers[2].language );
	}
}
...
void sub_80EAEF4( struct SecretBaseRecordMixer* mixers )
{
	...
	sub_80EAEB4( mixers );
	sub_80EAA64(
		mixers[0].secretBases, mixers[0].version, mixers[0].language );
	sub_80EAA64(
		mixers[1].secretBases, mixers[1].version, mixers[1].language );
	sub_80EAA64(
		mixers[2].secretBases, mixers[2].version, mixers[2].language );
...
void ReceiveSecretBasesData( void* secretBases, size_t recordSize, u8 linkIdx )
{
	...
	if( FlagGet( FLAG_RECEIVED_SECRET_POWER ) )
	{
		...
		for( i = 1; i < SECRET_BASES_COUNT; i++ )
		{
			...
			{
				gSaveBlock1Ptr->secretBases[i].registryStatus =
					1;
				gSaveBlock1Ptr->secretBases[i].sbr_field_1_0 =
					0;
			}
		}
		...
		for( i = 1; i < SECRET_BASES_COUNT; i++ )
		{
			if( gSaveBlock1Ptr->secretBases[i].registryStatus ==
				2 )
			{
				gSaveBlock1Ptr->secretBases[i].registryStatus =
					0;
...
{
	if( VarGet( VAR_SECRET_BASE_IS_NOT_LOCAL ) &&
		sInFriendSecretBase == TRUE && !CurMapIsSecretBase( ) )
	{
		VarSet( VAR_SECRET_BASE_IS_NOT_LOCAL, FALSE );
		sInFriendSecretBase = FALSE;
		sub_80EEA70( );
...
void sub_80EB290( void )
{
	if( VarGet( VAR_CURRENT_SECRET_BASE ) != 0 )
		VarSet( VAR_SECRET_BASE_HIGH_TV_FLAGS,
			VarGet( VAR_SECRET_BASE_HIGH_TV_FLAGS ) | 0x800 );
}
...
void sub_80EB2C8( void )
{
	if( VarGet( VAR_CURRENT_SECRET_BASE ) != 0 )
		VarSet( VAR_SECRET_BASE_LOW_TV_FLAGS,
			VarGet( VAR_SECRET_BASE_LOW_TV_FLAGS ) | 0x400 );
}
...
void sub_80EB300( void )
{
	...
	{
		VarSet( VAR_SECRET_BASE_LOW_TV_FLAGS,
			VarGet( VAR_SECRET_BASE_LOW_TV_FLAGS ) & ~0x3800 );
		VarSet( VAR_SECRET_BASE_HIGH_TV_FLAGS,
			VarGet( VAR_SECRET_BASE_HIGH_TV_FLAGS ) & ~0x001 );
		VarSet( VAR_SECRET_BASE_LOW_TV_FLAGS,
			VarGet( VAR_SECRET_BASE_LOW_TV_FLAGS ) | 0x2000 );
	}
}
...
void sub_80EB368( void )
{
	if( VarGet( VAR_CURRENT_SECRET_BASE ) != 0 )
	{
		...
		VarSet( VAR_SECRET_BASE_HIGH_TV_FLAGS,
			VarGet( VAR_SECRET_BASE_HIGH_TV_FLAGS ) & ~0x001 );
		VarSet( VAR_SECRET_BASE_LOW_TV_FLAGS,
			VarGet( VAR_SECRET_BASE_LOW_TV_FLAGS ) | 0x800 );
	}
}
...
void sub_80EB3D0( void )
{
	if( VarGet( VAR_CURRENT_SECRET_BASE ) != 0 )
	{
		...
		VarSet( VAR_SECRET_BASE_HIGH_TV_FLAGS,
			VarGet( VAR_SECRET_BASE_HIGH_TV_FLAGS ) & ~0x001 );
		VarSet( VAR_SECRET_BASE_LOW_TV_FLAGS,
			VarGet( VAR_SECRET_BASE_LOW_TV_FLAGS ) | 0x1000 );
	}
}
...
void SetSecretBaseSecretsTvFlags_MiscFurnature( void )
{
	...
	case 0x2fb:
		...
		if( VarGet( VAR_CURRENT_SECRET_BASE ) != 0 )
			VarSet( VAR_SECRET_BASE_LOW_TV_FLAGS,
				VarGet( VAR_SECRET_BASE_LOW_TV_FLAGS ) | 0x8 );
		break;
	case 0x22c:
	case 0x233:
		// Fence
