...
// const rom data
static const u16 sUnknown_0855C604[] =
	INCBIN_U16( "graphics/misc/option_menu_text.gbapal" );
// note: this is only used in the Japanese release
static const u8 sEqualSignGfx[] =
	INCBIN_U8( "graphics/misc/option_menu_equals_sign.4bpp" );
...
void CB2_InitOptionMenu( void )
{
	...
	case 1:
		...
		SetGpuReg( REG_OFFSET_DISPCNT,
			DISPCNT_WIN0_ON | DISPCNT_OBJ_ON |
				DISPCNT_OBJ_1D_MAP );
		ShowBg( 0 );
		ShowBg( 1 );
		gMain.state++;
	...
	case 4:
		LoadPalette(
			sUnknown_0855C6A0, 0, sizeof( sUnknown_0855C6A0 ) );
	...
	case 5:
		LoadPalette(
			sUnknown_0855C604, 0x10, sizeof( sUnknown_0855C604 ) );
		gMain.state++;
		break;
	case 6:
		PutWindowTilemap( 0 );
...
static void Task_OptionMenuProcessInput( u8 taskId )
{
	...
	else
	{
		...
		case MENUITEM_SOUND:
			...
			gTasks[taskId].data[TD_SOUND] = Sound_ProcessInput(
...
static void DrawOptionMenuChoice( const u8* text, u8 x, u8 y, u8 style )
{
	...
	AddTextPrinterParameterized(
		WIN_OPTIONS, 1, dst, x, y + 1, TEXT_SPEED_FF, NULL );
}
...
static u8 FrameType_ProcessInput( u8 selection )
{
	if( gMain.newKeys & DPAD_RIGHT )
	{
		...
		LoadBgTiles( 1,
			GetWindowFrameTilesPal( selection )->tiles,
			0x120,
			0x1A2 );
		LoadPalette(
			GetWindowFrameTilesPal( selection )->pal, 0x70, 0x20 );
		sArrowPressed = TRUE;
	}
	if( gMain.newKeys & DPAD_LEFT )
	{
...
static void DrawTextOption( void )
{
	...
	AddTextPrinterParameterized(
		WIN_TEXT_OPTION, 1, gText_Option, 8, 1, TEXT_SPEED_FF, NULL );
