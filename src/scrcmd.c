...
bool8 ScrCmd_vgoto_if( struct ScriptContext* ctx )
{
	u8 condition = ScriptReadByte( ctx );
	const u8* ptr =
		(const u8*)( ScriptReadWord( ctx ) - gUnknown_020375C4 );
...
bool8 ScrCmd_fadescreenswapbuffers( struct ScriptContext* ctx )
{
	...
	case 0:
	case 2:
		CpuCopy32( gPaletteDecompressionBuffer,
			gPlttBufferUnfaded,
			PLTT_DECOMP_BUFFER_SIZE );
		FadeScreen( mode, 0 );
		break;
...
bool8 ScrCmd_applymovement_at( struct ScriptContext* ctx )
{
	...
	ScriptMovement_StartObjectMovementScript(
		localId, mapNum, mapGroup, movementScript );
	sMovingNpcId = localId;
	return FALSE;
}
...
bool8 ScrCmd_removeobject( struct ScriptContext* ctx )
{
	...
	RemoveEventObjectByLocalIdAndMap( localId,
		gSaveBlock1Ptr->location.mapNum,
		gSaveBlock1Ptr->location.mapGroup );
	return FALSE;
}
...
bool8 ScrCmd_setobjectxy( struct ScriptContext* ctx )
{
	...
	TryMoveEventObjectToMapCoords( localId,
		gSaveBlock1Ptr->location.mapNum,
		gSaveBlock1Ptr->location.mapGroup,
		x,
		y );
...
bool8 ScrCmd_moveobjectoffscreen( struct ScriptContext* ctx )
{
	...
	TryOverrideEventObjectTemplateCoords( localId,
		gSaveBlock1Ptr->location.mapNum,
		gSaveBlock1Ptr->location.mapGroup );
	return FALSE;
}
...
bool8 ScrCmd_faceplayer( struct ScriptContext* ctx )
{
	...
	{
		EventObjectFaceOppositeDirection(
...
bool8 ScrCmd_turnobject( struct ScriptContext* ctx )
{
	...
	EventObjectTurnByLocalIdAndMap( localId,
		gSaveBlock1Ptr->location.mapNum,
		gSaveBlock1Ptr->location.mapGroup,
		direction );
	return FALSE;
...
bool8 ScrCmd_releaseall( struct ScriptContext* ctx )
{
	...
	HideFieldMessageBox( );
	playerObjectId =
		GetEventObjectIdByLocalIdAndMap( EVENT_OBJ_ID_PLAYER, 0, 0 );
...
bool8 ScrCmd_multichoicegrid( struct ScriptContext* ctx )
{
	...
	if( ScriptMenu_MultichoiceGrid(
		    left, top, multichoiceId, ignoreBPress, numColumns ) ==
		TRUE )
	{
		ScriptContext1_Stop( );
		return TRUE;
...
bool8 ScrCmd_braillemessage( struct ScriptContext* ctx )
{
	...
	winTemplate = CreateWindowTemplate(
		0, xWindow, yWindow + 1, width, height, 0xF, 0x1 );
	...
	AddTextPrinterParameterized(
		gUnknown_03000F30, 6, gStringVar4, xText, yText, 0xFF, 0x0 );
...
bool8 ScrCmd_bufferspeciesname( struct ScriptContext* ctx )
{
	...
	StringCopy(
		sScriptStringVars[stringVarIndex], gSpeciesNames[species] );
	return FALSE;
}
...
bool8 ScrCmd_bufferleadmonspeciesname( struct ScriptContext* ctx )
{
	...
	u32 species   = GetMonData(
                &gPlayerParty[partyIndex], MON_DATA_SPECIES, NULL );
...
bool8 ScrCmd_bufferitemnameplural( struct ScriptContext* ctx )
{
	...
	CopyItemNameHandlePlural(
		itemId, sScriptStringVars[stringVarIndex], quantity );
	return FALSE;
}
...
bool8 ScrCmd_buffernumberstring( struct ScriptContext* ctx )
{
	...
	ConvertIntToDecimalStringN(
		sScriptStringVars[stringVarIndex], v1, 0, v2 );
	return FALSE;
}
...
bool8 ScrCmd_bufferstdstring( struct ScriptContext* ctx )
{
	...
	StringCopy(
		sScriptStringVars[stringVarIndex], gUnknown_0858BAF0[index] );
	return FALSE;
}
...
bool8 ScrCmd_bufferboxname( struct ScriptContext* ctx )
{
	...
	StringCopy(
		sScriptStringVars[stringVarIndex], GetBoxNamePtr( boxId ) );
	return FALSE;
}
...
bool8 ScrCmd_givemon( struct ScriptContext* ctx )
{
	...
	gSpecialVar_Result = ScriptGiveMon(
		species, level, item, unkParam1, unkParam2, unkParam3 );
	return FALSE;
}
...
bool8 ScrCmd_checkmoney( struct ScriptContext* ctx )
{
	...
	if( !ignore )
		gSpecialVar_Result =
			IsEnoughMoney( &gSaveBlock1Ptr->money, amount );
	return FALSE;
}
...
bool8 ScrCmd_playslotmachine( struct ScriptContext* ctx )
{
	...
	PlaySlotMachine( slotMachineIndex,
...
bool8 ScrCmd_setfieldeffectarg( struct ScriptContext* ctx )
{
	...
	gFieldEffectArguments[argNum] =
		(s16)VarGet( ScriptReadHalfword( ctx ) );
	return FALSE;
}
...
bool8 ScrCmd_setmetatile( struct ScriptContext* ctx )
{
	...
	if( !isImpassable )
		...
		MapGridSetMetatileIdAt(
			x, y, tileId | METATILE_COLLISION_MASK );
	return FALSE;
}
...
// This command will force the Pokémon to be obedient, you don't get to make it
// disobedient.
...
bool8 ScrCmd_checkmonobedience( struct ScriptContext* ctx )
{
	...
	gSpecialVar_Result = GetMonData(
		&gPlayerParty[partyIndex], MON_DATA_OBEDIENCE, NULL );
	return FALSE;
}
...
void sub_809BE48( u16 npcId ) { sMovingNpcId = npcId; }
