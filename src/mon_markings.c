...
// .rodata
static const u16 gUnknown_0859E65C[] =
	INCBIN_U16( "graphics/misc/mon_markings.gbapal" );
static const u8 gUnknown_0859E67C[] =
	INCBIN_U8( "graphics/misc/mon_markings.4bpp" );
...
bool8 sub_811F960( void )
{
	...
	case 0:
		...
		CpuFastCopy( sMenu->frameTiles + TILE_SIZE_4BPP * 2,
			dest + TILE_SIZE_4BPP * 7,
			TILE_SIZE_4BPP );
		sMenu->tileLoadState++;
		break;
	default:
		CpuFastCopy( sMenu->frameTiles + TILE_SIZE_4BPP * 3,
			dest,
			TILE_SIZE_4BPP );
		for( i = 0; i < 6; i++ )
		{
		...
		break;
	case 13:
		CpuFastCopy( sMenu->frameTiles + TILE_SIZE_4BPP * 6,
			dest,
			TILE_SIZE_4BPP );
		for( i = 0; i < 6; i++ )
		{
...
bool8 sub_811FBA4( void )
{
	...
	if( gMain.newKeys & A_BUTTON )
	{
		...
		case 4:
			sMenu->markings = 0;
			for( i = 0; i < 4; i++ )
				sMenu->markings |= sMenu->markingsArray[i]
					<< i;
			return FALSE;
		case 5:
			return FALSE;
...
static struct Sprite* sub_811FFD4(
	u16 tileTag, u16 paletteTag, const u16* palette, u16 size )
{
	u8 spriteId;
	struct SpriteTemplate sprTemplate;
...
{
	RequestDma3Copy(
		gUnknown_0859E67C + markings * 0x80, dest, 0x80, 0x10 );
}
