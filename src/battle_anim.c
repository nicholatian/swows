...
const struct CompressedSpriteSheet gBattleAnimPicTable[] = {
	...
	{gBattleAnimSpriteGfx_SpinningGreenOrbs,
		0x0800,
	...
	{gBattleAnimSpriteGfx_PurpleGreenUnk,
		0x0a00,
	...
	{gBattleAnimSpriteGfx_WhirlwindLines,
		0x0300,
	...
	{gBattleAnimSpriteGfx_JaggedMusicNote,
		0x0400,
	...
	{gBattleAnimSpriteGfx_TriForceTriangle,
		0x0800,
	...
	{gBattleAnimSpriteGfx_SmellingsaltEffect,
		0x0200,
	...
	{gBattleAnimSpriteGfx_MagnifyingGlass,
		0x0200,
...
const struct CompressedSpritePalette gBattleAnimPaletteTable[] = {
	...
	{gBattleAnimSpritePal_SmellingsaltEffect,
		ANIM_TAG_SMELLINGSALT_EFFECT},
	...
	{gBattleAnimSpritePal_WhiteCircleOfLight,
		ANIM_TAG_WHITE_CIRCLE_OF_LIGHT},
...
const struct BattleAnimBackground gBattleAnimBackgroundTable[] = {
	[BG_DARK_]               = {gBattleAnimBgImage_Dark,
                gBattleAnimBgPalette_Dark,
                gBattleAnimBgTilemap_Dark},
	[BG_DARK]                = {gBattleAnimBgImage_Dark,
                gBattleAnimBgPalette_Dark,
                gBattleAnimBgTilemap_Dark},
	...
	[BG_ICE]                 = {gBattleAnimBgImage_Ice,
                gBattleAnimBgPalette_Ice,
	...
	[BG_SKY]                 = {gBattleAnimBgImage_Drill,
...
void LaunchBattleAnimation(
	const u8* const animsTable[], u16 tableId, bool8 isMoveAnim )
{
	s32 i;
...
static void ScriptCmd_unloadspritegfx( void )
{
	...
	FreeSpriteTilesByTag(
		gBattleAnimPicTable[GET_TRUE_SPRITE_INDEX( index )].tag );
...
static void ScriptCmd_createsprite( void )
{
	...
	sBattleAnimScriptPtr++;
	template = (const struct SpriteTemplate*)( T2_READ_32(
		sBattleAnimScriptPtr ) );
	...
	if( argVar & 0x80 )
	{
		...
		subpriority =
			GetBattlerSpriteSubpriority( gBattleAnimTarget ) +
			( s8 )( argVar );
	}
	else
	{
		...
		subpriority =
			GetBattlerSpriteSubpriority( gBattleAnimAttacker ) +
			( s8 )( argVar );
	}
...
static void ScriptCmd_end( void )
{
	...
	if( IsSEPlaying( ) )
	{
		if( ++sSoundAnimFramesToWait <= 90 ) // Wait 90 frames, then
						     // halt the sound effect.
		{
			gAnimFramesToWait = 1;
			return;
		}
	...
	for( i = 0; i < ANIM_SPRITE_INDEX_COUNT; i++ )
	{
		...
		{
			FreeSpriteTilesByTag(
				gBattleAnimPicTable[sAnimSpriteIndexArray[i]]
					.tag );
			FreeSpritePaletteByTag(
				gBattleAnimPicTable[sAnimSpriteIndexArray[i]]
					.tag );
...
static void sub_80A40F4( u8 taskId )
{
	...
	gTasks[newTaskId].data[1] = gSprites[battlerSpriteId].pos1.x +
		gSprites[battlerSpriteId].pos2.x;
	gTasks[newTaskId].data[2] = gSprites[battlerSpriteId].pos1.y +
		gSprites[battlerSpriteId].pos2.y;
...
void MoveBattlerSpriteToBG(
	u8 battlerId, bool8 toBG_2, bool8 setSpriteInvisible )
{
	...
	if( !toBG_2 )
	{
		...
		{
			RequestDma3Fill( 0,
				(void*)( BG_SCREEN_ADDR( 16 ) ),
				0x2000,
				1 );
			RequestDma3Fill( 0xFF,
				(void*)( BG_SCREEN_ADDR( 30 ) ),
				0x1000,
				0 );
		}
		else
		{
			RequestDma3Fill(
				0, (void*)( BG_SCREEN_ADDR( 8 ) ), 0x2000, 1 );
			RequestDma3Fill( 0xFF,
		...
		gBattle_BG1_X = -( gSprites[battlerSpriteId].pos1.x +
					gSprites[battlerSpriteId].pos2.x ) +
			0x20;
		if( IsContest( ) &&
			IsSpeciesNotUnown(
		...
		gBattle_BG1_Y = -( gSprites[battlerSpriteId].pos1.y +
					gSprites[battlerSpriteId].pos2.y ) +
			0x20;
		...
		LoadPalette( &gPlttBufferUnfaded[0x100 + battlerId * 16],
			animBg.paletteId * 16,
			0x20 );
	...
	{
		RequestDma3Fill(
			0, (void*)( BG_SCREEN_ADDR( 12 ) ), 0x2000, 1 );
		RequestDma3Fill(
			0, (void*)( BG_SCREEN_ADDR( 30 ) ), 0x1000, 1 );
		...
		gBattle_BG2_X = -( gSprites[battlerSpriteId].pos1.x +
					gSprites[battlerSpriteId].pos2.x ) +
			0x20;
		gBattle_BG2_Y = -( gSprites[battlerSpriteId].pos1.y +
					gSprites[battlerSpriteId].pos2.y ) +
			0x20;
...
static void task_pA_ma0A_obj_to_bg_pal( u8 taskId )
{
	...
	x = gTasks[taskId].data[1] -
		( gSprites[spriteId].pos1.x + gSprites[spriteId].pos2.x );
	y = gTasks[taskId].data[2] -
		( gSprites[spriteId].pos1.y + gSprites[spriteId].pos2.y );
...
static void ScriptCmd_clearmonbg( void )
{
	...
	if( animBattlerId > 1 && sMonAnimTaskIdArray[1] != 0xFF )
		gSprites[gBattlerSpriteIds[battlerId ^ BIT_FLANK]].invisible =
			FALSE;
	else
		animBattlerId = 0;
...
static void ScriptCmd_clearmonbg_23( void )
{
	...
	if( IsBattlerSpriteVisible( battlerId ) )
		gSprites[gBattlerSpriteIds[battlerId]].invisible = FALSE;
	if( animBattlerId > 1 &&
		IsBattlerSpriteVisible( battlerId ^ BIT_FLANK ) )
		gSprites[gBattlerSpriteIds[battlerId ^ BIT_FLANK]].invisible =
			FALSE;
	else
		animBattlerId = 0;
...
static void LoadMoveBg( u16 bgId )
{
	...
	else
	{
		...
		LZDecompressVram( gBattleAnimBackgroundTable[bgId].image,
			(void*)( BG_CHAR_ADDR( 2 ) ) );
		LoadCompressedPalette(
			gBattleAnimBackgroundTable[bgId].palette, 32, 32 );
	}
}
...
s8 BattleAnimAdjustPanning( s8 pan )
{
	...
	else if( IsContest( ) )
	{
		if( gBattleAnimAttacker != gBattleAnimTarget ||
			gBattleAnimAttacker != 2 || pan != SOUND_PAN_TARGET )
			pan *= -1;
	}
...
static void ScriptCmd_panse_1B( void )
{
	...
	incrementPan = CalculatePanIncrement(
		currentPan, targetPan, incrementPanArg );
...
void Task_PanFromInitialToTarget( u8 taskId )
{
	...
	if( gTasks[taskId].tFrameCounter++ >= gTasks[taskId].tFramesToWait )
	{
		...
		if( incrementPan == 0 ) // If we're not incrementing, just
					// cancel the task immediately.
		{
			destroyTask = TRUE;
		}
		else if( initialPanning <
			targetPanning ) // Panning increasing.
		{
...
static void Task_WaitAndPlaySE( u8 taskId )
{
	...
	{
		PlaySE12WithPanning(
			gTasks[taskId].tSongId, gTasks[taskId].tPanning );
		DestroyTask( taskId );
		gAnimSoundTaskCount--;
	}
}
...
static void ScriptCmd_doublebattle_2D( void )
{
			...
			GetBattlerSide( gBattleAnimTarget ) )
	{
		if( wantedBattler == ANIM_ATTACKER )
		{
			r4 = GetBattlerSpriteBGPriorityRank(
				gBattleAnimAttacker );
