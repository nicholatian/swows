	...
	u8 wraparound )
{
	if( !IsInvalidBg( bg ) )
	{
		...
		{
			sGpuBgConfigs.configs[bg].charBaseIndex =
				charBaseIndex & 0x3;
		}
		...
		if( screenSize != 0xFF )
		{
			sGpuBgConfigs.configs[bg].screenSize =
				screenSize & 0x3;
		}
...
u8 LoadBgVram( u8 bg, const void* src, u16 size, u16 destOffset, u8 mode )
{
	...
	if( !IsInvalidBg( bg ) && sGpuBgConfigs.configs[bg].visible )
	{
		...
		{
		case 0x1:
			offset = sGpuBgConfigs.configs[bg].charBaseIndex *
				BG_CHAR_SIZE;
			break;
		case 0x2:
			offset = sGpuBgConfigs.configs[bg].mapBaseIndex *
				BG_SCREEN_SIZE;
			break;
		default:
			cursor = -1;
		...
		cursor = RequestDma3Copy(
			src, (void*)( offset + BG_VRAM ), size, 0 );
...
void InitBgsFromTemplates(
	u8 bgMode, const struct BgTemplate* templates, u8 numTemplates )
{
	int i;
	u8 bg;
...
u16 LoadBgTiles( u8 bg, const void* src, u16 size, u16 destOffset )
{
	...
	{
		tileOffset =
			( sGpuBgConfigs2[bg].baseTile + destOffset ) * 0x20;
	}
	else
	{
		tileOffset =
			( sGpuBgConfigs2[bg].baseTile + destOffset ) * 0x40;
	}
	...
	if( gUnneededFireRedVariable == 1 )
	{
		DummiedOutFireRedLeafGreenTileAllocFunc(
			bg, tileOffset / 0x20, size / 0x20, 1 );
	}
...
u16 Unused_LoadBgPalette( u8 bg, const void* src, u16 size, u16 destOffset )
{
	...
	if( !IsInvalidBg32( bg ) )
	{
		u16 paletteOffset = ( sGpuBgConfigs2[bg].basePalette * 0x20 ) +
			( destOffset * 2 );
		cursor = RequestDma3Copy(
			src, (void*)( paletteOffset + BG_PLTT ), size, 0 );
...
void SetBgAttribute( u8 bg, u8 attributeId, u8 value )
{
	...
	case BG_ATTR_CHARBASEINDEX:
		SetBgControlAttributes(
			bg, value, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF );
		break;
	case BG_ATTR_MAPBASEINDEX:
		SetBgControlAttributes(
			bg, 0xFF, value, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF );
		break;
	case BG_ATTR_SCREENSIZE:
		SetBgControlAttributes(
			bg, 0xFF, 0xFF, value, 0xFF, 0xFF, 0xFF, 0xFF );
		break;
	case BG_ATTR_PALETTEMODE:
		SetBgControlAttributes(
			bg, 0xFF, 0xFF, 0xFF, value, 0xFF, 0xFF, 0xFF );
		break;
	case BG_ATTR_PRIORITY:
		SetBgControlAttributes(
			bg, 0xFF, 0xFF, 0xFF, 0xFF, value, 0xFF, 0xFF );
		break;
	case BG_ATTR_MOSAIC:
		SetBgControlAttributes(
	...
	case BG_ATTR_WRAPAROUND:
		SetBgControlAttributes(
			bg, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, value );
		break;
	}
}
...
s32 ChangeBgX( u8 bg, s32 value, u8 op )
{
	...
	if( IsInvalidBg32( bg ) ||
		!GetBgControlAttribute( bg, BG_CTRL_ATTR_VISIBLE ) )
	{
		return -1;
	}
...
void CopyToBgTilemapBuffer( u8 bg, const void* src, u16 mode, u16 destOffset )
{
	if( !IsInvalidBg32( bg ) && !IsTileMapOutsideWram( bg ) )
	{
		if( mode != 0 )
			CpuCopy16( src,
				(void*)( sGpuBgConfigs2[bg].tilemap +
					( destOffset * 2 ) ),
				mode );
		else
			LZ77UnCompWram( src,
...
void CopyToBgTilemapBufferRect(
	u8 bg, const void* src, u8 destX, u8 destY, u8 width, u8 height )
{
	const void* srcCopy;
	u16 destX16;
	u16 destY16;
	...
	if( !IsInvalidBg32( bg ) && !IsTileMapOutsideWram( bg ) )
	{
		...
		case 0:
			srcCopy = src;
			for( destY16 = destY; destY16 < ( destY + height );
				destY16++ )
			{
				for( destX16 = destX;
					destX16 < ( destX + width );
...
void CopyToBgTilemapBufferRect_ChangePalette( u8 bg,
	const void* src,
	u8 destX,
	u8 destY,
	u8 rectWidth,
	...
	s16 tileOffset )
{
	...
	{
		screenSize =
			GetBgControlAttribute( bg, BG_CTRL_ATTR_SCREENSIZE );
		...
		case 0:
			...
			{
				for( j = srcHeight; j < ( srcHeight + destY );
					j++ )
				{
					u16 index =
						GetTileMapIndexFromCoords( j,
							i,
							screenSize,
							screenWidth,
							screenHeight );
					CopyTileMapEntry( srcPtr,
						sGpuBgConfigs2[bg].tilemap +
							( index * 2 ),
						rectHeight,
						palette1,
						tileOffset );
...
void FillBgTilemapBufferRect_Palette0(
	u8 bg, u16 tileNum, u8 x, u8 y, u8 width, u8 height )
{
	u16 x16;
	u16 y16;
	u16 mode;
...
void FillBgTilemapBufferRect(
	u8 bg, u16 tileNum, u8 x, u8 y, u8 width, u8 height, u8 palette )
{
	WriteSequenceToBgTilemapBuffer(
		bg, tileNum, x, y, width, height, palette, 0 );
}

void WriteSequenceToBgTilemapBuffer( u8 bg,
	u16 firstTileNum,
	u8 x,
	u8 y,
	u8 width,
...
u32 GetTileMapIndexFromCoords(
	s32 x, s32 y, s32 screenSize, u32 screenWidth, u32 screenHeight )
{
	x = x & ( screenWidth - 1 );
	y = y & ( screenHeight - 1 );
...
#ifdef NONMATCHING // This one has some weird switch statement cases that
		   // refuse to cooperate
void CopyTileMapEntry(
	const u16* src, u16* dest, s32 palette1, s32 tileOffset, s32 palette2 )
{
	u16 var;
	switch( palette1 )
	{
	case 0 ... 16:
		if( palette1 != 16 )
			var = ( ( *src + tileOffset ) & 0xFFF ) +
				( ( palette1 + palette2 ) << 12 );
		else
			var = ( ( *dest & 0xFC00 ) + ( palette2 << 12 ) ) |
				( ( *src + tileOffset ) & 0x3FF );
		break;
	default:
