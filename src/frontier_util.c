...
static const struct FrontierBrainMon sFrontierBrainsMons[][2][3] = {
	...
	[FRONTIER_FACILITY_FACTORY] =
		{
			// Because Factory's pokemon are random, this
			// facility's Brain also uses random pokemon.
			// What is interesting, this team is actually
			// the one Steven uses in the multi tag
...
static const struct WindowTemplate gUnknown_08611C74 = {.bg = 0,
	.tilemapLeft                                        = 1,
	.tilemapTop                                         = 1,
	.width                                              = 0x1c,
	.height                                             = 0x12,
...
static const struct WindowTemplate gUnknown_08611C7C = {.bg = 0,
	.tilemapLeft                                        = 2,
	.tilemapTop                                         = 2,
	.width                                              = 0x1a,
	.height                                             = 15,
...
static const struct WindowTemplate gUnknown_08611C84 = {.bg = 0,
	.tilemapLeft                                        = 2,
	.tilemapTop                                         = 1,
	.width                                              = 0x1a,
	.height                                             = 17,
...
static void sub_81A1830( void )
{
	...
	case 6:
		...
		break;
	case 7:
		gSpecialVar_Result = gSaveBlock2Ptr->frontier.field_D00 &
			gUnknown_08611BFC[facility][currSymbol];
		break;
	}
}
...
static void sub_81A1968( void )
{
	...
	case 6:
		...
		break;
	case 7:
		gSaveBlock2Ptr->frontier.field_D00 |=
			gUnknown_08611BFC[facility][currSymbol];
		break;
	}
}
...
static void sub_81A1B38( void )
{
	...
	for( i = 0; i < 4; i++ )
	{
		...
		if( monId < PARTY_SIZE )
			gSaveBlock1Ptr
				->playerParty[gSaveBlock2Ptr->frontier
						      .selectedPartyMons[i] -
					1] = gPlayerParty[i];
	}
}
...
static void PrintAligned( const u8* str, s32 y )
{
	...
	AddTextPrinterParameterized(
		gRecordsWindowId, 1, str, x, y, TEXT_SPEED_FF, NULL );
}
...
static void PrintHyphens( s32 y )
{
	...
	y = ( y * 8 ) + 1;
	AddTextPrinterParameterized(
		gRecordsWindowId, 1, text, 4, y, TEXT_SPEED_FF, NULL );
}
...
{
	AddTextPrinterParameterized(
		gRecordsWindowId, 1, str, x1, y, TEXT_SPEED_FF, NULL );
	if( num > 9999 )
		num = 9999;
	ConvertIntToDecimalStringN(
		gStringVar1, num, STR_CONV_MODE_RIGHT_ALIGN, 4 );
	StringExpandPlaceholders( gStringVar4, gText_WinStreak );
	AddTextPrinterParameterized(
		gRecordsWindowId, 1, gStringVar4, x2, y, TEXT_SPEED_FF, NULL );
}

static void TowerPrintRecordStreak(
	u8 battleMode, u8 lvlMode, u8 x1, u8 x2, u8 y )
{
	u16 num = gSaveBlock2Ptr->frontier
			  .towerRecordWinStreaks[battleMode][lvlMode];
...
static void TowerPrintPrevOrCurrentStreak(
	u8 battleMode, u8 lvlMode, u8 x1, u8 x2, u8 y )
{
	bool8 isCurrent;
...
static void ShowTowerResultsWindow( u8 battleMode )
{
	...
	if( battleMode == FRONTIER_MODE_SINGLES )
		StringExpandPlaceholders(
			gStringVar4, gText_SingleBattleRoomResults );
	else if( battleMode == FRONTIER_MODE_DOUBLES )
		StringExpandPlaceholders(
			gStringVar4, gText_DoubleBattleRoomResults );
	else if( battleMode == FRONTIER_MODE_MULTIS )
		StringExpandPlaceholders(
			gStringVar4, gText_MultiBattleRoomResults );
	else
		StringExpandPlaceholders(
			gStringVar4, gText_LinkMultiBattleRoomResults );
	...
	TowerPrintPrevOrCurrentStreak(
		battleMode, FRONTIER_LVL_50, 72, 132, 49 );
	TowerPrintRecordStreak( battleMode, FRONTIER_LVL_50, 72, 132, 65 );
	TowerPrintPrevOrCurrentStreak(
		battleMode, FRONTIER_LVL_OPEN, 72, 132, 97 );
...
static void PrintTwoStrings(
	const u8* str1, const u8* str2, u16 num, u8 x1, u8 x2, u8 y )
{
	AddTextPrinterParameterized(
		gRecordsWindowId, 1, str1, x1, y, TEXT_SPEED_FF, NULL );
	ConvertIntToDecimalStringN(
		gStringVar1, num, STR_CONV_MODE_RIGHT_ALIGN, 4 );
...
static void DomePrintPrevOrCurrentStreak(
	u8 battleMode, u8 lvlMode, u8 x1, u8 x2, u8 y )
{
	bool8 isCurrent;
	...
	if( isCurrent == TRUE )
		...
		PrintTwoStrings(
			gText_Prev, gText_ClearStreak, winStreak, x1, x2, y );
}
...
static void ShowDomeResultsWindow( u8 battleMode )
{
	...
	if( battleMode == FRONTIER_MODE_SINGLES )
		StringExpandPlaceholders(
			gStringVar4, gText_SingleBattleTourneyResults );
	else
		StringExpandPlaceholders(
			gStringVar4, gText_DoubleBattleTourneyResults );

	PrintAligned( gStringVar4, 0 );
	AddTextPrinterParameterized(
		gRecordsWindowId, 1, gText_Lv502, 8, 33, TEXT_SPEED_FF, NULL );
	AddTextPrinterParameterized( gRecordsWindowId,
		1,
		gText_OpenLv,
		8,
	...
	DomePrintPrevOrCurrentStreak(
		battleMode, FRONTIER_LVL_50, 64, 121, 33 );
	...
	PrintTwoStrings( gText_Total,
		...
		65 );
	DomePrintPrevOrCurrentStreak(
		battleMode, FRONTIER_LVL_OPEN, 64, 121, 97 );
	PrintTwoStrings( gText_Record,
		gText_ClearStreak,
		gSaveBlock2Ptr->frontier
...
static void PalacePrintRecordStreak(
	u8 battleMode, u8 lvlMode, u8 x1, u8 x2, u8 y )
{
	u16 num = gSaveBlock2Ptr->frontier
...
static void PalacePrintPrevOrCurrentStreak(
	u8 battleMode, u8 lvlMode, u8 x1, u8 x2, u8 y )
{
	bool8 isCurrent;
...
static void ShowPalaceResultsWindow( u8 battleMode )
{
	...
	if( battleMode == FRONTIER_MODE_SINGLES )
		StringExpandPlaceholders(
			gStringVar4, gText_SingleBattleHallResults );
	else
		StringExpandPlaceholders(
			gStringVar4, gText_DoubleBattleHallResults );
	...
	PalacePrintPrevOrCurrentStreak(
		battleMode, FRONTIER_LVL_50, 72, 131, 49 );
	PalacePrintRecordStreak( battleMode, FRONTIER_LVL_50, 72, 131, 65 );
	PalacePrintPrevOrCurrentStreak(
		battleMode, FRONTIER_LVL_OPEN, 72, 131, 97 );
...
static void PikePrintPrevOrCurrentStreak( u8 lvlMode, u8 x1, u8 x2, u8 y )
{
	...
	if( isCurrent == TRUE )
		...
		PrintTwoStrings(
			gText_Prev, gText_RoomsCleared, winStreak, x1, x2, y );
}
...
static void FactoryPrintStreak(
	const u8* str, u16 num1, u16 num2, u8 x1, u8 x2, u8 x3, u8 y )
{
	AddTextPrinterParameterized(
		gRecordsWindowId, 1, str, x1, y, TEXT_SPEED_FF, NULL );
	if( num1 > 9999 )
		num1 = 9999;
	ConvertIntToDecimalStringN(
		gStringVar1, num1, STR_CONV_MODE_RIGHT_ALIGN, 4 );
	StringExpandPlaceholders( gStringVar4, gText_WinStreak );
	AddTextPrinterParameterized(
		gRecordsWindowId, 1, gStringVar4, x2, y, TEXT_SPEED_FF, NULL );

	ConvertIntToDecimalStringN(
		gStringVar1, num2, STR_CONV_MODE_RIGHT_ALIGN, 4 );
	StringExpandPlaceholders( gStringVar4, gText_TimesVar1 );
	AddTextPrinterParameterized(
		gRecordsWindowId, 1, gStringVar4, x3, y, TEXT_SPEED_FF, NULL );
}

static void FactoryPrintRecordStreak(
	u8 battleMode, u8 lvlMode, u8 x1, u8 x2, u8 x3, u8 y )
{
	u16 num1 = gSaveBlock2Ptr->frontier
			   .factoryRecordWinStreaks[battleMode][lvlMode];
	u16 num2 = gSaveBlock2Ptr->frontier
			   .factoryRecordRentsCount[battleMode][lvlMode];
	FactoryPrintStreak( gText_Record, num1, num2, x1, x2, x3, y );
}

static u16 FactoryGetWinStreak( u8 battleMode, u8 lvlMode )
{
	u16 winStreak = gSaveBlock2Ptr->frontier
...
{
	u16 rents = gSaveBlock2Ptr->frontier
			    .factoryRentsCount[battleMode][lvlMode];
	if( rents > 9999 )
		return 9999;
	else
		return rents;
...
static void FactoryPrintPrevOrCurrentStreak(
	u8 battleMode, u8 lvlMode, u8 x1, u8 x2, u8 x3, u8 y )
{
	bool8 isCurrent;
	...
	if( isCurrent == TRUE )
		FactoryPrintStreak(
			gText_Current, winStreak, rents, x1, x2, x3, y );
	else
		FactoryPrintStreak(
			gText_Prev, winStreak, rents, x1, x2, x3, y );
}
...
static void ShowFactoryResultsWindow( u8 battleMode )
{
	...
	if( battleMode == FRONTIER_MODE_SINGLES )
		StringExpandPlaceholders(
			gStringVar4, gText_BattleSwapSingleResults );
	else
		StringExpandPlaceholders(
			gStringVar4, gText_BattleSwapDoubleResults );
	...
	FactoryPrintPrevOrCurrentStreak(
		battleMode, FRONTIER_LVL_50, 8, 64, 158, 49 );
	FactoryPrintRecordStreak(
		battleMode, FRONTIER_LVL_50, 8, 64, 158, 65 );
	FactoryPrintPrevOrCurrentStreak(
		battleMode, FRONTIER_LVL_OPEN, 8, 64, 158, 113 );
	FactoryPrintRecordStreak(
		battleMode, FRONTIER_LVL_OPEN, 8, 64, 158, 129 );
	PutWindowTilemap( gRecordsWindowId );
	CopyWindowToVram( gRecordsWindowId, 3 );
}
...
static void ShowPyramidResultsWindow( void )
{
	...
	PrintAligned( gStringVar4, 2 );
	AddTextPrinterParameterized(
		gRecordsWindowId, 1, gText_Lv502, 8, 49, TEXT_SPEED_FF, NULL );
	AddTextPrinterParameterized( gRecordsWindowId,
		1,
		gText_OpenLv,
		8,
...
static void ShowLinkContestResultsWindow( void )
{
	...
	AddTextPrinterParameterized(
		gRecordsWindowId, 1, gStringVar4, x, 1, TEXT_SPEED_FF, NULL );

	str = gText_1st;
	x   = GetStringRightAlignXOffset( 1, str, 38 ) + 50;
	AddTextPrinterParameterized(
		gRecordsWindowId, 1, str, x, 25, TEXT_SPEED_FF, NULL );
	...
	x = 6;
	AddTextPrinterParameterized(
		gRecordsWindowId, 1, gText_Cool, x, 41, TEXT_SPEED_FF, NULL );
	AddTextPrinterParameterized( gRecordsWindowId,
		1,
		gText_Beauty,
		x,
		...
		NULL );
	AddTextPrinterParameterized(
		gRecordsWindowId, 1, gText_Cute, x, 73, TEXT_SPEED_FF, NULL );
	AddTextPrinterParameterized(
		gRecordsWindowId, 1, gText_Smart, x, 89, TEXT_SPEED_FF, NULL );
	AddTextPrinterParameterized( gRecordsWindowId,
		1,
		gText_Tough,
		x,
...
static void sub_81A31FC( void )
{
	...
	case FRONTIER_FACILITY_TOWER:
				...
				.towerRecordWinStreaks[battleMode][lvlMode] )
		{
			...
			{
				StringCopy( name,
					gLinkPlayers[gBattleScripting
							     .multiplayerId ^
						1]
						.name );
				StripExtCtrlCodes( name );
				StringCopy( gSaveBlock2Ptr->frontier
						    .opponentName[lvlMode],
					name );
				SetTrainerId(
					gLinkPlayers[gBattleScripting
							     .multiplayerId ^
	...
	case FRONTIER_FACILITY_DOME:
				...
				.domeRecordWinStreaks[battleMode][lvlMode] )
		{
			gSaveBlock2Ptr->frontier
				...
				gSaveBlock2Ptr->frontier
					.domeWinStreaks[battleMode][lvlMode];
			if( gSaveBlock2Ptr->frontier.domeWinStreaks[battleMode]
								   [lvlMode] >
					1 &&
				sub_80EE818( ) )
			{
	...
	case FRONTIER_FACILITY_PALACE:
		if( gSaveBlock2Ptr->frontier
				.palaceWinStreaks[battleMode][lvlMode] >
			gSaveBlock2Ptr->frontier
				.palaceRecordWinStreaks[battleMode][lvlMode] )
		{
			gSaveBlock2Ptr->frontier
	...
	case FRONTIER_FACILITY_ARENA:
			...
			gSaveBlock2Ptr->frontier.arenaRecordStreaks[lvlMode] )
		{
			gSaveBlock2Ptr->frontier.arenaRecordStreaks[lvlMode] =
				gSaveBlock2Ptr->frontier
					.arenaWinStreaks[lvlMode];
			if( gSaveBlock2Ptr->frontier.arenaWinStreaks[lvlMode] >
					1 &&
				sub_80EE818( ) )
			{
				sub_80EE8C8( gSaveBlock2Ptr->frontier
						     .arenaWinStreaks[lvlMode],
					10 );
			}
		}
	...
	case FRONTIER_FACILITY_FACTORY:
				...
				.factoryRecordWinStreaks[battleMode][lvlMode] )
		{
			gSaveBlock2Ptr->frontier
				.factoryRecordWinStreaks[battleMode][lvlMode] =
				gSaveBlock2Ptr->frontier
					.factoryWinStreaks[battleMode]
							  [lvlMode];
			gSaveBlock2Ptr->frontier
				.factoryRecordRentsCount[battleMode][lvlMode] =
				gSaveBlock2Ptr->frontier
					.factoryRentsCount[battleMode]
							  [lvlMode];
			if( gSaveBlock2Ptr->frontier
						.factoryWinStreaks[battleMode]
								  [lvlMode] >
	...
	case FRONTIER_FACILITY_PYRAMID:
		if( gSaveBlock2Ptr->frontier.pyramidWinStreaks[lvlMode] >
			gSaveBlock2Ptr->frontier
				.pyramidRecordStreaks[lvlMode] )
		{
			gSaveBlock2Ptr->frontier
				.pyramidRecordStreaks[lvlMode] =
				gSaveBlock2Ptr->frontier
					.pyramidWinStreaks[lvlMode];
			if( gSaveBlock2Ptr->frontier
						.pyramidWinStreaks[lvlMode] >
					1 &&
				sub_80EE818( ) )
...
u8 sub_81A3610( void )
{
	...
	default:
		if( r5 == gUnknown_08611550[facility][0] )
			...
			( r5 - gUnknown_08611550[facility][1] ) %
					gUnknown_08611550[facility][2] ==
				0 )
			ret = 4;
		break;
	}
...
void CopyFrontierTrainerText( u8 whichText, u16 trainerId )
{
	...
	case FRONTIER_BEFORE_TEXT:
		if( trainerId == TRAINER_EREADER )
			...
			FrontierSpeechToString(
				gFacilityTrainers[trainerId].speechBefore );
		else if( trainerId < TRAINER_RECORD_MIXING_APPRENTICE )
			FrontierSpeechToString(
				gSaveBlock2Ptr->frontier
					.towerRecords[trainerId -
	...
	case FRONTIER_PLAYER_LOST_TEXT:
		...
		{
			FrontierSpeechToString(
				gFacilityTrainers[trainerId].speechWin );
		}
		else if( trainerId < TRAINER_RECORD_MIXING_APPRENTICE )
		{
	...
	case FRONTIER_PLAYER_WON_TEXT:
		...
		{
			FrontierSpeechToString(
				gFacilityTrainers[trainerId].speechLose );
		}
		else if( trainerId < TRAINER_RECORD_MIXING_APPRENTICE )
		{
			if( gBattleTypeFlags & BATTLE_TYPE_RECORDED )
...
void sub_81A3908( void )
{
	...
	for( battleMode = 0; battleMode < 4; battleMode++ )
	{
		...
		{
			gSaveBlock2Ptr->frontier
				.towerWinStreaks[battleMode][lvlMode] = 0;
			...
			{
				gSaveBlock2Ptr->frontier
					.arenaWinStreaks[lvlMode] = 0;
				gSaveBlock2Ptr->frontier
					.pikeWinStreaks[lvlMode] = 0;
				gSaveBlock2Ptr->frontier
					.pyramidWinStreaks[lvlMode] = 0;
			}
		}
	}
...
static void sub_81A3B64( void )
{
	...
	{
	case FRONTIER_FACILITY_TOWER:
		challengeNum = gSaveBlock2Ptr->frontier
				       .towerWinStreaks[battleMode][lvlMode] /
			7;
		break;
	case FRONTIER_FACILITY_DOME:
	...
	case FRONTIER_FACILITY_ARENA:
		challengeNum =
			gSaveBlock2Ptr->frontier.arenaWinStreaks[lvlMode] / 7;
		break;
	case FRONTIER_FACILITY_FACTORY:
		challengeNum =
			gSaveBlock2Ptr->frontier
	...
	case FRONTIER_FACILITY_PIKE:
		challengeNum =
			gSaveBlock2Ptr->frontier.pikeWinStreaks[lvlMode] / 14;
		break;
	case FRONTIER_FACILITY_PYRAMID:
		challengeNum =
	...
	ConvertIntToDecimalStringN(
		gStringVar1, points, STR_CONV_MODE_LEFT_ALIGN, 2 );
...
static void sub_81A3FD4( void )
{
	...
	case FRONTIER_MODE_LINK_MULTIS:
		...
		break;
	case FRONTIER_MODE_DOUBLES:
		if( VarGet( VAR_FRONTIER_FACILITY ) ==
			FRONTIER_FACILITY_TOWER )
			toChoose = 4;
		else
			toChoose = 3;
	...
	do
	{
		...
		do
		{
			u16 species = GetMonData(
				&gPlayerParty[monId], MON_DATA_SPECIES2 );
			u16 heldItem = GetMonData(
				&gPlayerParty[monId], MON_DATA_HELD_ITEM );
			u8 level = GetMonData(
				&gPlayerParty[monId], MON_DATA_LEVEL );
			u16 hp = GetMonData(
				&gPlayerParty[monId], MON_DATA_HP );
			if( VarGet( VAR_FRONTIER_FACILITY ) ==
				FRONTIER_FACILITY_PYRAMID )
			{
				if( heldItem == 0 )
	...
	if( count < toChoose )
	{
		...
		for( i = 0; species != 0xFFFF;
			i++, species = gFrontierBannedSpecies[i] )
		{
			if( GetSetPokedexFlag(
		...
		for( i = 0; gFrontierBannedSpecies[i] != 0xFFFF; i++ )
			count = sub_81A3DD0( gFrontierBannedSpecies[i],
				count,
				caughtBannedMons );
		...
		else
		{
			if( count & 1 )
				StringAppend(
					gStringVar1, gText_ScrollTextUp );
			else
				StringAppend( gStringVar1, gText_Space2 );
...
static void sub_81A4224( void ) { ValidateEReaderTrainer( ); }
...
static void sub_81A4230( void )
{
	...
	case FRONTIER_FACILITY_TOWER:
		if( gSaveBlock2Ptr->frontier
				.towerWinStreaks[battleMode][lvlMode] < 9999 )
		{
			gSaveBlock2Ptr->frontier
				.towerWinStreaks[battleMode][lvlMode]++;
			if( battleMode == FRONTIER_MODE_SINGLES )
			{
				SetGameStat(
					...
					gSaveBlock2Ptr->frontier
						.towerWinStreaks[battleMode]
								[lvlMode] );
				gSaveBlock2Ptr->frontier.field_D02 =
					gSaveBlock2Ptr->frontier
						.towerWinStreaks[battleMode]
								[lvlMode];
			}
	...
	case FRONTIER_FACILITY_DOME:
		if( gSaveBlock2Ptr->frontier
				.domeWinStreaks[battleMode][lvlMode] < 9999 )
			gSaveBlock2Ptr->frontier
				.domeWinStreaks[battleMode][lvlMode]++;
		if( gSaveBlock2Ptr->frontier
	...
	case FRONTIER_FACILITY_PALACE:
		if( gSaveBlock2Ptr->frontier
				.palaceWinStreaks[battleMode][lvlMode] < 9999 )
			gSaveBlock2Ptr->frontier
				.palaceWinStreaks[battleMode][lvlMode]++;
		break;
	case FRONTIER_FACILITY_ARENA:
	...
	for( i = 0; i < 4; i++ )
	{
		if( gSaveBlock2Ptr->frontier.selectedPartyMons[i] != 0 )
		{
			u16 item = GetMonData(
				...
				NULL );
			SetMonData(
				&gPlayerParty[i], MON_DATA_HELD_ITEM, &item );
		}
	}
}
...
static void sub_81A443C( void )
{
	...
	case 0:
		GetFrontierTrainerName(
			gStringVar1, gTrainerBattleOpponent_A );
		break;
	case 1:
		GetFrontierTrainerName(
			gStringVar2, gTrainerBattleOpponent_A );
		break;
	}
}
...
static void Print1PRecord( s32 position,
	s32 x,
	s32 y,
	...
	s32 hallFacilityId )
{
	...
	if( hallRecord->winStreak )
	{
		TVShowConvertInternationalString(
			text, hallRecord->name, hallRecord->language );
		AddTextPrinterParameterized( gRecordsWindowId,
			1,
			text,
			( x + 2 ) * 8,
		...
		if( winStreak > 9999 )
			winStreak = 9999;
		ConvertIntToDecimalStringN(
			gStringVar2, winStreak, STR_CONV_MODE_RIGHT_ALIGN, 4 );
		StringExpandPlaceholders( gStringVar4,
			gHallFacilityToRecordsText[hallFacilityId] );
		AddTextPrinterParameterized( gRecordsWindowId,
			1,
			gStringVar4,
			GetStringRightAlignXOffset( 1,
				gHallFacilityToRecordsText[hallFacilityId],
				0xC8 ),
			( 8 * ( y + 5 * position ) ) + 1,
			TEXT_SPEED_FF,
...
static void Print2PRecord(
	s32 position, s32 x, s32 y, struct RankingHall2P* hallRecord )
{
	u8 text[32];
	u16 winStreak;
	...
	if( hallRecord->winStreak )
	{
		...
		TVShowConvertInternationalString(
			text, hallRecord->name1, hallRecord->language );
		AddTextPrinterParameterized( gRecordsWindowId,
			1,
			text,
			( x + 2 ) * 8,
		...
		if( IsStringJapanese( hallRecord->name2 ) )
			TVShowConvertInternationalString(
				text, hallRecord->name2, LANGUAGE_JAPANESE );
		else
		...
		ConvertIntToDecimalStringN(
			gStringVar2, winStreak, STR_CONV_MODE_RIGHT_ALIGN, 4 );
		StringExpandPlaceholders(
			gStringVar4, gHallFacilityToRecordsText[9] );
		AddTextPrinterParameterized( gRecordsWindowId,
			1,
			gStringVar4,
			GetStringRightAlignXOffset(
				1, gHallFacilityToRecordsText[9], 0xC8 ),
			( 8 * ( y + 5 * position ) ) + 1,
			TEXT_SPEED_FF,
			NULL );
	}
...
static void Fill1PRecords(
	struct RankingHall1P* dst, s32 hallFacilityId, s32 lvlMode )
{
	s32 i, j;
	struct RankingHall1P record1P[4];
	struct PlayerHallRecords* playerHallRecords =
		calloc( 1, sizeof( struct PlayerHallRecords ) );
	GetPlayerHallRecords( playerHallRecords );

	for( i = 0; i < 3; i++ )
		record1P[i] =
			gSaveBlock2Ptr
				->hallRecords1P[hallFacilityId][lvlMode][i];
...
static void PrintHallRecords( s32 hallFacilityId, s32 lvlMode )
{
	...
	StringExpandPlaceholders(
		gStringVar4, gUnknown_08611CB0[hallFacilityId][1] );
	AddTextPrinterParameterized(
		gRecordsWindowId, 1, gStringVar4, 0, 1, TEXT_SPEED_FF, NULL );
	x = GetStringRightAlignXOffset( 1, gLevelModeText[lvlMode], 0xD0 );
	AddTextPrinterParameterized( gRecordsWindowId,
		1,
		gLevelModeText[lvlMode],
		x,
		1,
		TEXT_SPEED_FF,
		NULL );
	...
	{
		gSaveBlock2Ptr->frontier.opponentName[0][PLAYER_NAME_LENGTH] =
			EOS;
		gSaveBlock2Ptr->frontier.opponentName[1][PLAYER_NAME_LENGTH] =
			EOS;
	...
	else
	{
		...
		for( i = 0; i < 3; i++ )
			Print1PRecord(
				i, 1, 4, &records1P[i], hallFacilityId );
	}
}
...
void ClearRankingHallRecords( void )
{
	...
	for( i = 0; i < HALL_FACILITIES_COUNT; i++ )
	{
		for( j = 0; j < 2; j++ )
		{
			...
			{
				CopyTrainerId(
					gSaveBlock2Ptr->hallRecords1P[i][j][k]
						.id,
					0 ); // BUG: Passing 0 as a pointer
					     // instead of a pointer holding a
					     // value of 0.
				gSaveBlock2Ptr->hallRecords1P[i][j][k]
					.name[0] = EOS;
				gSaveBlock2Ptr->hallRecords1P[i][j][k]
	...
	for( j = 0; j < 2; j++ )
	{
		for( k = 0; k < 3; k++ )
		{
			CopyTrainerId( gSaveBlock2Ptr->hallRecords2P[j][k].id1,
				0 ); // BUG: Passing 0 as a pointer instead of
				     // a pointer holding a value of 0.
...
{
	s32 i;
	struct Pokemon* monsParty =
		calloc( PARTY_SIZE, sizeof( struct Pokemon ) );
...
void CopyFrontierBrainTrainerName( u8* dst )
{
	...
	for( i = 0; i < PLAYER_NAME_LENGTH; i++ )
		dst[i] = gTrainers[gFacilityToBrainTrainerId[facility]]
				 .trainerName[i];
...
void CreateFrontierBrainPokemon( void )
{
	...
	if( facility == FRONTIER_FACILITY_DOME )
		monCountInBits = GetDomeTrainerMonCountInBits(
	...
	for( i = 0; i < 3; monCountInBits >>= 1, i++ )
	{
		...
		for( j = 0; j < NUM_STATS; j++ )
			SetMonData( &gEnemyParty[monPartyId],
				MON_DATA_HP_EV + j,
				&sFrontierBrainsMons[facility][symbol][i]
					 .evs[j] );
		friendship = 0xFF;
		for( j = 0; j < MAX_MON_MOVES; j++ )
		{
			SetMonMoveSlot( &gEnemyParty[monPartyId],
				sFrontierBrainsMons[facility][symbol][i]
					.moves[j],
				j );
			if( sFrontierBrainsMons[facility][symbol][i]
					.moves[j] == MOVE_FRUSTRATION )
				friendship = 0;
		}
		SetMonData( &gEnemyParty[monPartyId],
			MON_DATA_FRIENDSHIP,
...
s32 GetFronterBrainSymbol( void )
{
	...
	if( symbol == 2 )
	{
		...
		if( winStreak + gUnknown_08611550[facility][3] ==
			gUnknown_08611550[facility][0] )
			symbol = 0;
		else if( winStreak + gUnknown_08611550[facility][3] ==
			gUnknown_08611550[facility][1] )
			symbol = 1;
		else if( winStreak + gUnknown_08611550[facility][3] >
				gUnknown_08611550[facility][1] &&
			( winStreak + gUnknown_08611550[facility][3] -
				gUnknown_08611550[facility][1] ) %
					gUnknown_08611550[facility][2] ==
				0 )
			symbol = 1;
