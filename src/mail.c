...
struct MailRead
{
	...
	/*0x0224*/ u8* ( *parserMultiple )(
		u8* dest, const u16* src, u16 length1, u16 length2 );
...
static const u8 sUnknown_0859F2AC[] = {0, 10, 11};
...
static const struct MailGraphics sUnknown_0859F2B8[] = {
	...
	{gMailPalette_Harbor,
		...
		0x7fff,
		0x4631},
	{gMailPalette_Glitter,
		gMailTiles_Glitter,
		gMailTilemap_Glitter,
		0x0400,
		0,
		0x294a,
		0x6739},
...
void ReadMail(
	struct MailStruct* mail, void ( *callback )( void ), bool8 flag )
{
	u16 buffer[2];
	u16 species;
...
static bool8 MailReadBuildGraphics( void )
{
	...
	case 7:
		...
		break;
	case 8:
		decompress_and_copy_tile_data_to_vram( 1,
			sUnknown_0859F2B8[sMailRead->mailType].tiles,
			0,
			0,
			0 );
		break;
	...
	case 10:
		...
		CopyToBgTilemapBuffer( 1,
			sUnknown_0859F2B8[sMailRead->mailType].tileMap,
			0,
			0 );
		break;
	case 11:
	...
	case 12:
		...
		gPlttBufferUnfaded[250] =
			sUnknown_0859F2B8[sMailRead->mailType].color10;
		gPlttBufferFaded[250] =
			sUnknown_0859F2B8[sMailRead->mailType].color10;
		gPlttBufferUnfaded[251] =
			sUnknown_0859F2B8[sMailRead->mailType].color12;
		gPlttBufferFaded[251] =
			sUnknown_0859F2B8[sMailRead->mailType].color12;
		LoadPalette( sUnknown_0859F2B8[sMailRead->mailType].palette,
			0,
			32 );
		gPlttBufferUnfaded[10] =
			sUnknown_0859F2B0[gSaveBlock2Ptr->playerGender][0];
		gPlttBufferFaded[10] =
			sUnknown_0859F2B0[gSaveBlock2Ptr->playerGender][0];
		gPlttBufferUnfaded[11] =
			sUnknown_0859F2B0[gSaveBlock2Ptr->playerGender][1];
		gPlttBufferFaded[11] =
			sUnknown_0859F2B0[gSaveBlock2Ptr->playerGender][1];
		break;
	case 13:
	...
	case 17:
		...
		case 1:
			...
			sMailRead->monIconSprite = sub_80D2D78(
				icon, SpriteCallbackDummy, 0x60, 0x80, 0, 0 );
			break;
		case 2:
			LoadMonIconPalette( icon );
			sMailRead->monIconSprite = sub_80D2D78(
				icon, SpriteCallbackDummy, 0x28, 0x80, 0, 0 );
			break;
		}
		break;
	case 18:
...
static void sub_8121A1C( void )
{
	...
	if( !sMailRead->playerIsSender )
	{
		...
		sMailRead->signatureWidth = sMailRead->layout->signatureWidth -
			( StringLength( sMailRead->playerName ) * 8 - 0x60 );
	}
	else
	{
...
static void sub_8121B1C( void )
{
	...
	for( i = 0; i < sMailRead->layout->numSubStructs; i++ )
	{
		if( sMailRead->strbuf[i][0] == EOS ||
			sMailRead->strbuf[i][0] == CHAR_SPACE )
		{
			continue;
		}
		AddTextPrinterParameterized3( 0,
			1,
			sMailRead->layout->var8[i].xOffset +
				sMailRead->layout->wordsYPos,
	...
	box_x = GetStringCenterAlignXOffset(
			1, strbuf, sMailRead->signatureWidth ) +
		0x68;
	box_y = sMailRead->layout->signatureYPos + 0x58;
	AddTextPrinterParameterized3(
		0, 1, box_x, box_y, sUnknown_0859F2AC, 0, strbuf );
...
static void CB2_ExitMailReadFreeVars( void )
{
	if( !UpdatePaletteFade( ) )
	{
		...
		case 2:
			FreeMonIconPalette(
				sub_80D2E84( sMailRead->mail->species ) );
