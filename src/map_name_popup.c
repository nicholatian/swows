...
static const u16 gUnknown_0857F444[16] =
	INCBIN_U16( "graphics/interface/map_popup/857F444.gbapal" );
...
static const u8 gRegionMapSectionId_To_PopUpThemeIdMapping[] = {
	...
	[MAPSEC_UNDERWATER_TERRA_CAVE - KANTO_MAPSEC_COUNT] =
		MAPPOPUP_THEME_STONE2,
...
void ShowMapNamePopup( void )
{
	if( FlagGet( FLAG_HIDE_MAP_NAME_POPUP ) != TRUE )
	{
		...
		{
			sPopupTaskId =
				CreateTask( Task_MapNamePopUpWindow, 90 );
...
static void ShowMapNamePopUpWindow( void )
{
	...
	if( InBattlePyramid( ) )
	{
			...
			LAYOUT_BATTLE_FRONTIER_BATTLE_PYRAMID_TOP )
		{
			...
			mapDisplayHeaderSource =
				gBattlePyramid_MapHeaderStrings[7];
		}
		else
		{
			withoutPrefixPtr       = &( mapDisplayHeader[3] );
			mapDisplayHeaderSource = gBattlePyramid_MapHeaderStrings
	...
	else
	{
		withoutPrefixPtr = &( mapDisplayHeader[3] );
		GetMapName(
			withoutPrefixPtr, gMapHeader.regionMapSectionId, 0 );
	}
	...
	AddTextPrinterParameterized( GetMapNamePopUpWindowId( ),
		7,
		mapDisplayHeader,
		x,
		3,
...
static void sub_80D4A78( u8 bg, u8 x, u8 y, u8 deltaX, u8 deltaY, u8 unused )
{
	...
	{
		FillBgTilemapBufferRect(
			bg, 0x22F + i, i - 1 + x, y + deltaY, 1, 1, 0xE );
	}
}
...
static void LoadMapNamePopUpWindowBg( void )
{
	...
	if( regionMapSectionId > MAPSEC_DYNAMIC )
	{
		if( regionMapSectionId > MAPSEC_SPECIAL_AREA )
			regionMapSectionId -=
				( MAPSEC_SPECIAL_AREA - MAPSEC_DYNAMIC );
		else
			regionMapSectionId =
				0; // Discard kanto region sections;
	}
	popUpThemeId =
		gRegionMapSectionId_To_PopUpThemeIdMapping[regionMapSectionId];
	...
	if( gMapHeader.weather == WEATHER_BUBBLES )
		...
		LoadPalette(
			gMapPopUp_Palette_Table[popUpThemeId], 0xE0, 0x20 );
	BlitBitmapToWindow(
		popupWindowId, gMapPopUp_Table[popUpThemeId], 0, 0, 80, 24 );
}
