...
// Select_ refers to the first Pokemon selection screen where you choose 3
// Pokemon. Swap_   refers to the consecutive selection screen where you can
// keep your Pokemon or swap one with beaten trainer's.
...
struct FactorySelecteableMon
{
	u16 monSetId;
	u16 spriteId;
	u8 selectedId; // 0 - not selected, 1 - first pokemon, 2 - second
		       // pokemon, 3
		       // - third pokemon
...
static void Select_SetWinRegs(
	s16 mWin0H, s16 nWin0H, s16 mWin0V, s16 nWin0V );
...
static void sub_819F3F8(
	struct UnkFactoryStruct arg0, bool8* arg1, bool8 swapScreen );
...
static const u16 gUnknown_0860F13C[] =
	INCBIN_U16( "graphics/unknown/unknown_60F13C.gbapal" );
static const u16 gUnknown_0860F15C[] =
	INCBIN_U16( "graphics/unknown/unknown_60F15C.gbapal" );
static const u16 gUnknown_0860F17C[] =
	INCBIN_U16( "graphics/unknown/unknown_60F17C.gbapal" );
static const u8 gUnknown_0860F1BC[] =
	INCBIN_U8( "graphics/unknown/unknown_60F1BC.4bpp" );
static const u8 gUnknown_0860F3BC[] =
	INCBIN_U8( "graphics/unknown/unknown_60F3BC.4bpp" );
static const u8 gUnknown_0860F43C[] =
	INCBIN_U8( "graphics/unknown/unknown_60F43C.4bpp" );
static const u8 gUnknown_0860F53C[] =
	INCBIN_U8( "graphics/unknown/unknown_60F53C.4bpp" );
static const u8 gUnknown_0860F63C[] =
	INCBIN_U8( "graphics/unknown/unknown_60F63C.4bpp" );
static const u8 gUnknown_0860F6BC[] =
	INCBIN_U8( "graphics/unknown/unknown_60F6BC.4bpp" );
static const u8 gUnknown_0860F7BC[] =
	INCBIN_U8( "graphics/unknown/unknown_60F7BC.4bpp" );
static const u8 gUnknown_0860F83C[] =
	INCBIN_U8( "graphics/unknown/unknown_60F83C.4bpp" );
static const u8 gUnknown_0860F93C[] =
	INCBIN_U8( "graphics/unknown/unknown_60F93C.4bpp" );
static const u8 gUnknown_0860FA3C[] =
	INCBIN_U8( "graphics/unknown/unknown_60FA3C.4bpp" );
static const u8 gUnknown_0861023C[] =
	INCBIN_U8( "graphics/unknown/unknown_61023C.bin" );
static const u8 gUnknown_0861033C[] =
	INCBIN_U8( "graphics/unknown/unknown_61033C.4bpp" );
static const u16 gUnknown_0861039C[] =
	INCBIN_U16( "graphics/unknown/unknown_61039C.gbapal" );
...
static const u16 gUnknown_0861046C[] =
	INCBIN_U16( "graphics/unknown/unknown_61046C.gbapal" );
...
static const u16 gUnknown_08610918[] = {
	RGB_BLACK, RGB_BLACK, RGB_WHITE, RGB_BLACK, RGB_RED}; // Palette.
...
static void CB2_InitSelectScreen( void )
{
	...
	case 2:
		...
		CpuCopy16( gFrontierFactorySelectMenu_Gfx,
			sSelectMenuTilesetBuffer,
			0x440 );
		CpuCopy16( gUnknown_0861033C,
		...
		CpuCopy16( gFrontierFactorySelectMenu_Tilemap,
	...
	case 4:
		...
		SetGpuReg( REG_OFFSET_DISPCNT,
			DISPCNT_OBJ_ON | DISPCNT_BG0_ON | DISPCNT_BG1_ON |
				DISPCNT_OBJ_1D_MAP );
		if( sFactorySelectScreen->fromSummaryScreen == TRUE )
		{
			...
			SetGpuReg( REG_OFFSET_BLDCNT,
				BLDCNT_TGT1_BG3 | BLDCNT_EFFECT_BLEND |
					BLDCNT_TGT2_BG1 | BLDCNT_TGT2_OBJ );
			SetGpuReg(
				REG_OFFSET_BLDALPHA, BLDALPHA_BLEND( 11, 4 ) );
		}
		else
		{
			HideBg( 3 );
	...
	case 10:
		sFactorySelectScreen->fadeSpeciesNameTaskId =
			CreateTask( Task_SelectFadeSpeciesName, 0 );
		if( !sFactorySelectScreen->fromSummaryScreen )
		{
			gTasks[sFactorySelectScreen->fadeSpeciesNameTaskId]
				.data[0] = 0;
			taskId           = CreateTask(
                                Task_HandleSelectionScreenChooseMons, 0 );
		...
		else
		{
			...
			taskId                       = CreateTask(
                                Task_HandleSelectionScreenMenu, 0 );
...
static void Select_InitAllSprites( void )
{
	...
	for( i = 0; i < SELECTABLE_MONS_COUNT; i++ )
	{
		sFactorySelectScreen->mons[i].spriteId = CreateSprite(
			&gUnknown_086105D8, ( 35 * i ) + 32, 64, 1 );
	...
	sFactorySelectScreen->cursorSpriteId =
		CreateSprite( &gUnknown_086105F0, x, 88, 0 );
	sFactorySelectScreen->menuCursor1SpriteId =
		CreateSprite( &gUnknown_08610608, 176, 112, 0 );
	sFactorySelectScreen->menuCursor2SpriteId =
		CreateSprite( &gUnknown_08610620, 176, 144, 0 );
	...
	gSprites[sFactorySelectScreen->menuCursor1SpriteId]
		.centerToCornerVecX = 0;
	gSprites[sFactorySelectScreen->menuCursor1SpriteId]
		.centerToCornerVecY = 0;
	gSprites[sFactorySelectScreen->menuCursor2SpriteId]
		.centerToCornerVecX = 0;
	gSprites[sFactorySelectScreen->menuCursor2SpriteId]
		.centerToCornerVecY = 0;
}
...
static void Select_DestroyAllSprites( void )
{
	...
	for( i = 0; i < SELECTABLE_MONS_COUNT; i++ )
		DestroySprite(
			&gSprites[sFactorySelectScreen->mons[i].spriteId] );
...
static void Select_UpdateBallCursorPosition( s8 direction )
{
	...
	{
		if( sFactorySelectScreen->cursorPos !=
			SELECTABLE_MONS_COUNT - 1 )
	...
	gSprites[sFactorySelectScreen->cursorSpriteId].pos1.x =
		gSprites[sFactorySelectScreen->mons[cursorPos].spriteId]
			.pos1.x;
}
...
static void Select_HandleMonSelectionChange( void )
{
	...
	if( sFactorySelectScreen->mons[cursorPos].selectedId ) // Deselect a
							       // mon.
	{
			...
			sFactorySelectScreen->mons[cursorPos].selectedId == 1 )
		{
			for( i = 0; i < SELECTABLE_MONS_COUNT; i++ )
			{
				if( sFactorySelectScreen->mons[i].selectedId ==
					2 )
					break;
			}
...
static void Select_SetBallSpritePaletteNum( u8 id )
{
	...
	gSprites[sFactorySelectScreen->mons[id].spriteId].oam.paletteNum =
		palNum;
}
...
static void Task_FromSelectScreenToSummaryScreen( u8 taskId )
{
	...
	case 7:
		if( !gPaletteFade.active )
		{
			DestroyTask(
				sFactorySelectScreen->fadeSpeciesNameTaskId );
			sub_819F444( sFactorySelectScreen->unk294[1],
				&sFactorySelectScreen->unk2A0 );
	...
	case 8:
		...
		sFactorySelectMons = AllocZeroed(
			sizeof( struct Pokemon ) * SELECTABLE_MONS_COUNT );
		for( i = 0; i < SELECTABLE_MONS_COUNT; i++ )
			sFactorySelectMons[i] =
				sFactorySelectScreen->mons[i].monData;
		ShowPokemonSummaryScreen( 1,
			sFactorySelectMons,
			currMonId,
			SELECTABLE_MONS_COUNT - 1,
	...
	if( sFactorySelectScreen->unk2A0 != TRUE )
	{
		...
		case 1:
			if( !UpdatePaletteFade( ) )
			{
				...
				DestroyTask( sFactorySelectScreen
...
static void Task_HandleSelectionScreenMenu( u8 taskId )
{
	...
	case 2:
		if( !sFactorySelectScreen->fromSummaryScreen )
			sub_819F2B4( &sFactorySelectScreen->unk294[1].field1,
				&sFactorySelectScreen->unk2A0,
				FALSE );
	...
	case 3:
		...
		{
			PlaySE( SE_SELECT );
			sub_819F3F8( sFactorySelectScreen->unk294[1],
				&sFactorySelectScreen->unk2A0,
				FALSE );
			sub_819B958( 3 );
			sFactorySelectScreen->unk2A2 = TRUE;
	...
	case 12:
		if( !gPaletteFade.active )
		{
			if( sFactorySelectScreen->fromSummaryScreen == TRUE )
			{
				gPlttBufferFaded[228] =
					sFactorySelectScreen->unk2A4;
				gPlttBufferUnfaded[228] =
					gPlttBufferUnfaded[244];
			}
			sFactorySelectScreen->fromSummaryScreen = FALSE;
			gTasks[taskId].data[0]                  = 3;
		}
...
static void CreateFrontierFactorySelectableMons( u8 firstMonId )
{
	...
	u8 challengeNum = gSaveBlock2Ptr->frontier
				  .factoryWinStreaks[battleMode][lvlMode] /
		7;
	u8 var_28 = 0;
	...
	for( i = 0; i < SELECTABLE_MONS_COUNT; i++ )
	{
		...
		SetMonData(
			...
			&gBattleFrontierHeldItems
				[gFacilityTrainerMons[monSetId].itemTableId] );
	}
}
...
static void Select_CopyMonsToPlayerParty( void )
{
	...
	for( i = 0; i < 3; i++ )
	{
		for( j = 0; j < SELECTABLE_MONS_COUNT; j++ )
		{
			...
			{
				gPlayerParty[i] =
					sFactorySelectScreen->mons[j].monData;
				gSaveBlock2Ptr->frontier.rentalMons[i].monId =
					sFactorySelectScreen->mons[j].monSetId;
				gSaveBlock2Ptr->frontier.rentalMons[i]
					.personality =
				...
				gSaveBlock2Ptr->frontier.rentalMons[i]
					.abilityNum =
					GetBoxMonData( &gPlayerParty[i].box,
...
static void Select_PrintMonSpecies( void )
{
	...
	AddTextPrinterParameterized3(
		1, 1, x, 1, gUnknown_08610479, 0, gStringVar4 );
...
static void Select_PrintCantSelectSameMon( void )
{
	...
	AddTextPrinterParameterized(
		2, 1, gText_CantSelectSamePkmn, 2, 5, 0, NULL );
	CopyWindowToVram( 2, 2 );
}
...
{
	u8 selectedId =
		sFactorySelectScreen->mons[sFactorySelectScreen->cursorPos]
			.selectedId;

	PutWindowTilemap( 3 );
	FillWindowPixelBuffer( 3, PIXEL_FILL( 0 ) );
	AddTextPrinterParameterized3(
		3, 1, 7, 1, gUnknown_08610476, 0, gText_Summary );
	if( selectedId != 0 )
		AddTextPrinterParameterized3(
			3, 1, 7, 17, gUnknown_08610476, 0, gText_Deselect );
	else
		AddTextPrinterParameterized3(
			3, 1, 7, 17, gUnknown_08610476, 0, gText_Rent );

	AddTextPrinterParameterized3(
		3, 1, 7, 33, gUnknown_08610476, 0, gText_Others2 );
...
static void Select_PrintYesNoOptions( void )
{
	...
	AddTextPrinterParameterized3(
		4, 1, 7, 1, gUnknown_08610476, 0, gText_Yes2 );
	AddTextPrinterParameterized3(
		4, 1, 7, 17, gUnknown_08610476, 0, gText_No2 );
...
static u8 Select_OptionSummary( void ) { return 0; }
...
static void Select_PrintMonCategory( void )
{
	...
	if( monId < SELECTABLE_MONS_COUNT )
	{
		...
		species =
			GetMonData( &sFactorySelectScreen->mons[monId].monData,
				MON_DATA_SPECIES,
				NULL );
		CopyMonCategoryText(
			SpeciesToNationalPokedexNum( species ), text );
...
static void Summary_ShowMonSprite( void )
{
	...
	sFactorySelectScreen->unk294[1].field0 =
		CreateMonPicSprite_HandleDeoxys(
			species, otId, personality, TRUE, 88, 32, 15, 0xFFFF );
	gSprites[sFactorySelectScreen->unk294[1].field0].centerToCornerVecX =
		0;
	gSprites[sFactorySelectScreen->unk294[1].field0].centerToCornerVecY =
		0;

	sFactorySelectScreen->unk2A0 = FALSE;
}

static void sub_819BE20( bool8 arg0 ) { sFactorySelectScreen->unk2A0 = arg0; }
...
static void Select_ShowSummaryMonSprite( void )
{
	...
	sFactorySelectScreen->unk294[1].field1 =
		CreateSprite( &gUnknown_08610638, 120, 64, 1 );
	StartSpriteAffineAnim(
		&gSprites[sFactorySelectScreen->unk294[1].field1], 2 );

	mon = &sFactorySelectScreen->mons[sFactorySelectScreen->cursorPos]
		       .monData;
		...
		for( j = 0; j < SELECTABLE_MONS_COUNT; j++ )
		{
			if( sFactorySelectScreen->mons[j].selectedId == i + 1 )
			{
				...
				u16 species = GetMonData(
					mon, MON_DATA_SPECIES, NULL );
				u32 personality = GetMonData(
					mon, MON_DATA_PERSONALITY, NULL );
				u32 otId = GetMonData(
					mon, MON_DATA_OT_ID, NULL );

				sFactorySelectScreen->unk294[i].field0 =
					CreateMonPicSprite_HandleDeoxys(
						species,
						otId,
						personality,
						...
						i + 13,
						0xFFFF );
				gSprites[sFactorySelectScreen->unk294[i]
						 .field0]
					.centerToCornerVecX = 0;
				gSprites[sFactorySelectScreen->unk294[i]
						 .field0]
...
static void sub_819C040( struct Sprite* sprite )
{
	...
	if( sprite->affineAnimEnded &&
		gSprites[sFactorySelectScreen->unk294[0].field1]
			.affineAnimEnded &&
		gSprites[sFactorySelectScreen->unk294[2].field1]
			.affineAnimEnded )
	{
		sprite->invisible = TRUE;
		gSprites[sFactorySelectScreen->unk294[0].field1].invisible =
			TRUE;
		gSprites[sFactorySelectScreen->unk294[2].field1].invisible =
			TRUE;
...
static void sub_819C100( struct Sprite* sprite )
{
			...
			.affineAnimEnded )
	{
		...
		FreeOamMatrix( gSprites[sFactorySelectScreen->unk294[0].field1]
				       .oam.matrixNum );
		FreeOamMatrix( gSprites[sFactorySelectScreen->unk294[2].field1]
				       .oam.matrixNum );
...
static void sub_819C2D4( u8 taskId )
{
	...
	default:
		HideBg( 3 );
		gSprites[sFactorySelectScreen->unk294[1].field1].invisible =
			FALSE;
		gSprites[sFactorySelectScreen->unk294[1].field1].callback =
			sub_819C100;
		gSprites[sFactorySelectScreen->unk294[0].field1].invisible =
			FALSE;
		gSprites[sFactorySelectScreen->unk294[0].field1].callback =
			SpriteCallbackDummy;
		gSprites[sFactorySelectScreen->unk294[2].field1].invisible =
			FALSE;
		gSprites[sFactorySelectScreen->unk294[2].field1].callback =
			SpriteCallbackDummy;
		StartSpriteAffineAnim(
			&gSprites[sFactorySelectScreen->unk294[1].field1], 1 );
		StartSpriteAffineAnim(
			&gSprites[sFactorySelectScreen->unk294[0].field1], 1 );
		StartSpriteAffineAnim(
			&gSprites[sFactorySelectScreen->unk294[2].field1], 1 );
		ClearGpuRegBits( REG_OFFSET_DISPCNT, DISPCNT_WIN0_ON );
		DestroyTask( taskId );
		break;
	case 0:
...
static void sub_819C4B4( void )
{
	sFactorySelectScreen->unk294[1].field1 =
		CreateSprite( &gUnknown_08610638, 120, 64, 1 );
	sFactorySelectScreen->unk294[0].field1 =
		CreateSprite( &gUnknown_08610638, 44, 64, 1 );
	sFactorySelectScreen->unk294[2].field1 =
		CreateSprite( &gUnknown_08610638, 196, 64, 1 );
...
static void Select_SetWinRegs( s16 mWin0H, s16 nWin0H, s16 mWin0V, s16 nWin0V )
{
	...
	SetGpuReg( REG_OFFSET_WINOUT,
		WINOUT_WIN01_BG0 | WINOUT_WIN01_BG1 | WINOUT_WIN01_BG2 |
			WINOUT_WIN01_CLR | WINOUT_WIN01_OBJ );
}
...
static bool32 Select_AreSpeciesValid( u16 monSetId )
{
	...
	for( i = 1; i < selectState; i++ )
	{
		for( j = 0; j < SELECTABLE_MONS_COUNT; j++ )
		{
			...
			{
				if( gFacilityTrainerMons[sFactorySelectScreen
								 ->mons[j]
								 .monSetId]
						.species == species )
					return FALSE;
...
static void CopySwappedMonData( void )
{
	...
	gSaveBlock2Ptr->frontier.rentalMons[sFactorySwapScreen->playerMonId]
		...
		gSaveBlock2Ptr->frontier
			.rentalMons[sFactorySwapScreen->enemyMonId + 3]
			.monId;
	gSaveBlock2Ptr->frontier.rentalMons[sFactorySwapScreen->playerMonId]
		.ivs = gSaveBlock2Ptr->frontier
			       .rentalMons[sFactorySwapScreen->enemyMonId + 3]
			       .ivs;
	gSaveBlock2Ptr->frontier.rentalMons[sFactorySwapScreen->playerMonId]
		.personality =
		GetMonData( &gEnemyParty[sFactorySwapScreen->enemyMonId],
			MON_DATA_PERSONALITY,
			NULL );
	gSaveBlock2Ptr->frontier.rentalMons[sFactorySwapScreen->playerMonId]
		.abilityNum = GetBoxMonData(
		&gEnemyParty[sFactorySwapScreen->enemyMonId].box,
		MON_DATA_ABILITY_NUM,
		NULL );
}
...
static void Task_FromSwapScreenToSummaryScreen( u8 taskId )
{
	...
	case 7:
		if( !gPaletteFade.active )
		{
			...
			sub_819F444( sFactorySwapScreen->unk2C,
				&sFactorySwapScreen->unk30 );
...
static void Task_CloseSwapScreen( u8 taskId )
{
	if( sFactorySwapScreen->unk30 != TRUE )
	{
		...
		case 3:
			if( !UpdatePaletteFade( ) )
			{
				DestroyTask( sFactorySwapScreen
						     ->fadeSpeciesNameTaskId );
				Swap_DestroyAllSprites( );
				FREE_AND_SET_NULL( sSwapMenuTilesetBuffer );
				FREE_AND_SET_NULL(
...
static void sub_819CBDC( u8 taskId )
{
	...
	else
	{
		...
		gTasks[taskId].data[6] =
			( u32 )( Task_HandleSwapScreenChooseMons ) >> 16;
		gTasks[taskId].data[7] =
			( u32 )( Task_HandleSwapScreenChooseMons );
...
static void sub_819CCD4( u8 taskId )
{
	if( gTasks[taskId].data[0] == 0 )
	{
		sub_819F2B4( &sFactorySwapScreen->unk2C.field1,
			&sFactorySwapScreen->unk30,
			TRUE );
...
static void Task_HandleSwapScreenMenu( u8 taskId )
{
	...
	case 3:
		if( sFactorySwapScreen->unk30 != TRUE )
		{
			...
			{
				PlaySE( SE_SELECT );
				sub_819F3F8( sFactorySwapScreen->unk2C,
					&sFactorySwapScreen->unk30,
					TRUE );
				sub_819EA64( 3 );
				gTasks[taskId].data[0] = 0;
	...
	case 1:
		if( sFactorySwapScreen->unk22 )
		{
			...
			else
			{
				...
				BlendPalettes(
					0x4000, sFactorySwapScreen->unk28, 0 );
...
static void sub_819D12C( u8 taskId )
{
	...
	case 1:
		...
		for( i = 2; i >= 0; i-- )
		{
			...
			{
				var_2C = gSprites[sFactorySwapScreen
							  ->ballSpriteIds[i]]
						 .pos1.x;
			...
			{
				if( gSprites[sFactorySwapScreen
						     ->ballSpriteIds[i]]
						.pos1.x > ( i * 48 ) + 72 )
				{
					gSprites[sFactorySwapScreen
				...
				else if( gSprites[sFactorySwapScreen
							  ->ballSpriteIds[i]]
						 .pos1.x == ( i * 48 ) + 72 )
				{
					destroyTask = TRUE;
			...
			if( gSprites[sFactorySwapScreen->ballSpriteIds[i]]
						.pos1.x -
					16 >
				240 )
			{
...
static void sub_819D324( u8 taskId )
{
	...
	case 0:
		...
		else
		{
			for( j = 0; j < 2; j++ )
			{
				gSprites[sFactorySwapScreen->unk8[j][0]]
					.pos1.x = posX;
				gSprites[sFactorySwapScreen->unk8[j][1]]
					.pos1.x = posX + 16;
				gSprites[sFactorySwapScreen->unk8[j][2]]
					.pos1.x = posX + 48;
			}
	...
	case 1:
		...
		else
		{
			for( j = 0; j < 2; j++ )
			{
				gSprites[sFactorySwapScreen->unkE[j][0]]
					.pos1.x = posX;
				gSprites[sFactorySwapScreen->unkE[j][1]]
					.pos1.x = posX + 16;
			}
			taskId2                 = gTasks[taskId].data[1];
...
static void sub_819D588( u8 taskId )
{
	...
	case 0:
		LoadPalette(
			gUnknown_08610918, 0xE0, sizeof( gUnknown_08610918 ) );
	...
	case 2:
		BeginNormalPaletteFade(
			0x4000, 0, 0, 0x10, gUnknown_0860F13C[37] );
		gTasks[taskId].data[0]++;
		break;
	case 3:
		if( !gPaletteFade.active )
...
static void sub_819D770( u8 taskId )
{
	...
	case 3:
		BeginNormalPaletteFade(
			0x4000, 0, 0x10, 0, gUnknown_0860F13C[37] );
		gTasks[taskId].data[0]++;
		break;
	case 4:
		if( !gPaletteFade.active )
	...
	case 7:
		...
		if( sFactorySwapScreen->cursorPos < 3 )
			gSprites[sFactorySwapScreen->cursorSpriteId]
				.invisible = FALSE;
		Swap_PrintMonCategory( );
		gTasks[taskId].data[0]++;
		break;
...
static void sub_819D9EC( u8 taskId )
{
	...
	case 3:
		if( !FuncIsActiveTask( sub_819D12C ) &&
			gTasks[sFactorySwapScreen->fadeSpeciesNameTaskId]
					.data[4] == 1 )
		{
			sub_819EAC0( );
			if( !sFactorySwapScreen->inEnemyScreen )
			...
			gSprites[sFactorySwapScreen->cursorSpriteId].pos1.x =
				gSprites[sFactorySwapScreen->ballSpriteIds
						 [sFactorySwapScreen
								 ->cursorPos]]
					.pos1.x;
			gTasks[sFactorySwapScreen->fadeSpeciesNameTaskId]
				.func             = Task_SwapFadeSpeciesName;
			sFactorySwapScreen->unk27 = 0;
	...
	case 4:
		gTasks[taskId].data[0] = 0;
		gTasks[taskId].data[6] =
			( u32 )( Task_HandleSwapScreenChooseMons ) >> 0x10;
		gTasks[taskId].data[7] =
			( u32 )( Task_HandleSwapScreenChooseMons );
		gTasks[taskId].data[5] = 1;
		gTasks[taskId].func    = sub_819D770;
...
static void Swap_InitStruct( void )
{
	...
	{
		sFactorySwapScreen =
			AllocZeroed( sizeof( *sFactorySwapScreen ) );
...
static void CB2_InitSwapScreen( void )
{
	...
	case 2:
		...
		CpuCopy16( gFrontierFactorySelectMenu_Gfx,
			...
			0x440 );
		CpuCopy16(
			gUnknown_0861033C, sSwapMonCardBgTilesetBuffer, 0x60 );
		...
		LoadPalette( gFrontierFactorySelectMenu_Pal, 0, 0x40 );
		LoadPalette(
			gUnknown_08610918, 0xF0, sizeof( gUnknown_08610918 ) );
		LoadPalette(
			gUnknown_08610918, 0xE0, sizeof( gUnknown_08610918 ) );
		LoadPalette( gUnknown_0861039C, 0x20, 4 );
		gMain.state++;
	...
	case 15:
		sFactorySwapScreen->fadeSpeciesNameTaskId =
			CreateTask( Task_SwapFadeSpeciesName, 0 );
		if( !sFactorySwapScreen->fromSummaryScreen )
		{
			gTasks[sFactorySwapScreen->fadeSpeciesNameTaskId]
				.data[0] = 0;
			taskId           = CreateTask(
                                Task_HandleSwapScreenChooseMons, 0 );
			gTasks[taskId].data[0] = 0;
		}
		else
		{
...
static void Swap_InitAllSprites( void )
{
	...
	for( i = 0; i < 3; i++ )
	{
		sFactorySwapScreen->ballSpriteIds[i] = CreateSprite(
			&spriteTemplate, ( 48 * i ) + 72, 64, 1 );
	...
	sFactorySwapScreen->menuCursor1SpriteId =
		CreateSprite( &gUnknown_08610864, 176, 112, 0 );
	sFactorySwapScreen->menuCursor2SpriteId =
		CreateSprite( &gUnknown_0861087C, 176, 144, 0 );
	...
	gSprites[sFactorySwapScreen->menuCursor1SpriteId].centerToCornerVecX =
		0;
	gSprites[sFactorySwapScreen->menuCursor1SpriteId].centerToCornerVecY =
		0;
	gSprites[sFactorySwapScreen->menuCursor2SpriteId].centerToCornerVecX =
		0;
	gSprites[sFactorySwapScreen->menuCursor2SpriteId].centerToCornerVecY =
		0;
	...
	sFactorySwapScreen->unk8[0][0] =
		CreateSprite( &spriteTemplate, 240, 120, 10 );

	spriteTemplate         = gUnknown_08610864;
	spriteTemplate.tileTag = TAG_TILE_69;
	sFactorySwapScreen->unk8[0][1] =
		CreateSprite( &spriteTemplate, 256, 120, 10 );
	sFactorySwapScreen->unk8[0][2] =
		CreateSprite( &spriteTemplate, 288, 120, 10 );

	spriteTemplate         = gUnknown_0861084C;
	spriteTemplate.tileTag = TAG_TILE_6A;
	sFactorySwapScreen->unk8[1][0] =
		CreateSprite( &spriteTemplate, 240, 120, 1 );

	spriteTemplate         = gUnknown_08610864;
	spriteTemplate.tileTag = TAG_TILE_6B;
	sFactorySwapScreen->unk8[1][1] =
		CreateSprite( &spriteTemplate, 256, 120, 1 );
	spriteTemplate.tileTag = TAG_TILE_6C;
	sFactorySwapScreen->unk8[1][2] =
		CreateSprite( &spriteTemplate, 288, 120, 1 );

	spriteTemplate         = gUnknown_0861084C;
	spriteTemplate.tileTag = TAG_TILE_68;
	sFactorySwapScreen->unkE[0][0] =
		CreateSprite( &spriteTemplate, x, 144, 10 );

	spriteTemplate         = gUnknown_08610864;
	spriteTemplate.tileTag = TAG_TILE_69;
	sFactorySwapScreen->unkE[0][1] =
		CreateSprite( &spriteTemplate, x + 16, 144, 10 );

	spriteTemplate         = gUnknown_0861084C;
	spriteTemplate.tileTag = TAG_TILE_6A;
	sFactorySwapScreen->unkE[1][0] =
		CreateSprite( &spriteTemplate, x, 144, 1 );

	spriteTemplate         = gUnknown_08610864;
	spriteTemplate.tileTag = TAG_TILE_6C;
	sFactorySwapScreen->unkE[1][1] =
		CreateSprite( &spriteTemplate, x + 16, 144, 1 );

	for( i = 0; i < 2; i++ )
	{
		gSprites[sFactorySwapScreen->unk8[i][0]].centerToCornerVecX =
			0;
		gSprites[sFactorySwapScreen->unk8[i][0]].centerToCornerVecY =
			0;
		gSprites[sFactorySwapScreen->unk8[i][1]].centerToCornerVecX =
			0;
		gSprites[sFactorySwapScreen->unk8[i][1]].centerToCornerVecY =
			0;
		gSprites[sFactorySwapScreen->unk8[i][2]].centerToCornerVecX =
			0;
		gSprites[sFactorySwapScreen->unk8[i][2]].centerToCornerVecY =
			0;
		gSprites[sFactorySwapScreen->unkE[i][0]].centerToCornerVecX =
			0;
		gSprites[sFactorySwapScreen->unkE[i][0]].centerToCornerVecY =
			0;
		gSprites[sFactorySwapScreen->unkE[i][1]].centerToCornerVecX =
			0;
		gSprites[sFactorySwapScreen->unkE[i][1]].centerToCornerVecY =
			0;
...
static void Swap_DestroyAllSprites( void )
{
	...
	for( i = 0; i < 2; i++ )
	{
		for( j = 0; j < 3; j++ )
			DestroySprite(
				&gSprites[sFactorySwapScreen->unk8[i][j]] );
	}
	for( i = 0; i < 2; i++ )
	{
		for( j = 0; j < 2; j++ )
			DestroySprite(
				&gSprites[sFactorySwapScreen->unkE[i][j]] );
	}
}
...
static void Swap_HandleActionCursorChange( u8 cursorId )
{
	if( cursorId < 3 )
	{
		...
		sub_819E8EC( );
		gSprites[sFactorySwapScreen->cursorSpriteId].pos1.x =
			gSprites[sFactorySwapScreen->ballSpriteIds[cursorId]]
				.pos1.x;
	}
	else
	{
...
static void Swap_UpdateBallCursorPosition( s8 direction )
{
	...
	if( direction > 0 ) // Move cursor right.
	{
		if( sFactorySwapScreen->cursorPos + 1 !=
	...
	else // Move cursor left.
	{
		if( sFactorySwapScreen->cursorPos != 0 )
			...
			sFactorySwapScreen->cursorPos =
				sFactorySwapScreen->actionsCount - 1;
	}
...
static void sub_819E838( u8 arg0 )
{
	...
	for( i = 0; i < 3; i++ )
	{
		if( arg0 == 2 )
		{
			gSprites[sFactorySwapScreen->unk8[1][i]].invisible =
				FALSE;
			if( i < 2 )
				gSprites[sFactorySwapScreen->unkE[1][i]]
					.invisible = TRUE;
		}
		else if( arg0 == 3 )
		{
...
static void sub_819E8EC( void )
{
	...
	for( i = 0; i < 3; i++ )
	{
		...
		if( i < 2 )
			gSprites[sFactorySwapScreen->unkE[1][i]].invisible =
				TRUE;
	}
}
...
static void Swap_PrintMonSpecies( void )
{
	...
	else
	{
		...
		if( !sFactorySwapScreen->inEnemyScreen )
			species = GetMonData(
				&gPlayerParty[monId], MON_DATA_SPECIES, NULL );
		else
			species = GetMonData(
				&gEnemyParty[monId], MON_DATA_SPECIES, NULL );
		StringCopy( gStringVar4, gSpeciesNames[species] );
		x = GetStringRightAlignXOffset( 1, gStringVar4, 86 );
		AddTextPrinterParameterized3(
			1, 1, x, 1, gUnknown_08610925, 0, gStringVar4 );
...
static void Swap_PrintMenuOptions( void )
{
	...
	FillWindowPixelBuffer( 3, PIXEL_FILL( 0 ) );
	AddTextPrinterParameterized3(
		3, 1, 15, 1, gUnknown_08610922, 0, gText_Summary2 );
	AddTextPrinterParameterized3(
		3, 1, 15, 17, gUnknown_08610922, 0, gText_Swap );
	AddTextPrinterParameterized3(
		3, 1, 15, 33, gUnknown_08610922, 0, gText_Rechoose );
	CopyWindowToVram( 3, 3 );
}
...
static void Swap_PrintYesNoOptions( void )
{
	...
	FillWindowPixelBuffer( 4, PIXEL_FILL( 0 ) );
	AddTextPrinterParameterized3(
		4, 1, 7, 1, gUnknown_08610922, 0, gText_Yes3 );
	AddTextPrinterParameterized3(
		4, 1, 7, 17, gUnknown_08610922, 0, gText_No3 );
	CopyWindowToVram( 4, 3 );
}
...
static void Swap_PrintActionString( const u8* str, u32 y, u32 windowId )
{
	...
	AddTextPrinterParameterized3(
		windowId, 0, x, y, gUnknown_08610922, 0, str );
}
...
static void Swap_PrintMonSpecies2( void )
{
	...
	else
	{
		...
		x = GetStringRightAlignXOffset( 1, gStringVar4, 86 );
		AddTextPrinterParameterized3(
			7, 1, x, 1, gUnknown_08610925, 0, gStringVar4 );
		CopyWindowToVram( 7, 3 );
	}
}
...
static void Swap_InitActions( u8 id )
{
	if( sFactorySwapScreen->fromSummaryScreen != TRUE )
	{
		...
		case ACTIONS_PLAYER_SCREEN:
			...
			sFactorySwapScreen->cursorPos     = 0;
			sFactorySwapScreen->actionsCount =
				ARRAY_COUNT( sSwap_PlayerScreenActions );
			sFactorySwapScreen->actionsData =
		...
		case ACTIONS_ENEMY_SCREEN:
			...
			sFactorySwapScreen->cursorPos     = 0;
			sFactorySwapScreen->actionsCount =
				ARRAY_COUNT( sSwap_EnemyScreenActions );
			sFactorySwapScreen->actionsData =
...
{
	sSwap_CurrentTableFunc =
		sFactorySwapScreen->actionsData[sFactorySwapScreen->cursorPos]
			.func;
	sSwap_CurrentTableFunc( taskId );
}
...
static void Swap_ActionMon( u8 taskId )
{
	...
	{
		gTasks[taskId].data[6] =
			( u32 )( Task_HandleSwapScreenMenu ) >> 0x10;
...
static void Swap_ShowSummaryMonSprite( void )
{
	...
	sFactorySwapScreen->unk2C.field1 =
		CreateSprite( &gUnknown_08610894, 120, 64, 1 );
	StartSpriteAffineAnim(
		&gSprites[sFactorySwapScreen->unk2C.field1], 2 );
...
static bool8 Swap_AlreadyHasSameSpecies( u8 monId )
{
	u8 i;
	u16 species =
		GetMonData( &gEnemyParty[monId], MON_DATA_SPECIES, NULL );

	for( i = 0; i < 3; i++ )
	{
		if( i != sFactorySwapScreen->playerMonId &&
			( u16 )( GetMonData(
				&gPlayerParty[i], MON_DATA_SPECIES, NULL ) ) ==
				species )
			return TRUE;
	}
	return FALSE;
...
static void sub_819F69C( u8 taskId )
{
	...
	case 0:
		task->data[3] = 88;
		task->data[24] =
			152; // BUG: writing outside the array's bounds.
		task->data[5] = 64;
		task->data[8] = 65;
		SetGpuRegBits( REG_OFFSET_DISPCNT, DISPCNT_WIN0_ON );
		SetGpuReg( REG_OFFSET_WIN0H,
