...
const u8 gCableCarMtChimneyTilemap[] =
	INCBIN_U8( "graphics/misc/cable_car_mt_chimney_map.bin.lz" );
const u8 gCableCarTreeTilemap[] =
	INCBIN_U8( "graphics/misc/cable_car_tree_map.bin.lz" );
const u8 gCableCarMountainTilemap[] =
	INCBIN_U8( "graphics/misc/cable_car_mountain_map.bin.lz" );
...
const u8 gCableCarPylonStemTilemap[] =
	INCBIN_U8( "graphics/misc/cable_car_pylon_stem_map.bin.lz" );
...
static void CableCarMainCallback_Setup( void )
{
	...
	case 1:
		...
		InitBgsFromTemplates( 0,
			...
			ARRAY_COUNT( gCableCarBgTemplates ) );
	...
	case 2:
		...
		sCableCar->mtChimneyTilemap = malloc_and_decompress(
			gCableCarMtChimneyTilemap, &sizeOut );
		sCableCar->treeTilemap = malloc_and_decompress(
			gCableCarTreeTilemap, &sizeOut );
		sCableCar->mountainTilemap = malloc_and_decompress(
			gCableCarMountainTilemap, &sizeOut );
		sCableCar->pylonStemTilemap = malloc_and_decompress(
			gCableCarPylonStemTilemap, &sizeOut );
		sCableCar->pylonHookTilemapEntries =
			gCableCarPylonHookTilemapEntries;
		decompress_and_copy_tile_data_to_vram(
			0, gUnknown_08DBA5B8, 0, 0, 0 );
		gMain.state++;
		break;
	case 3:
	...
	case 6:
		CopyToBgTilemapBufferRect_ChangePalette(
			1, sCableCar->treeTilemap, 0, 17, 32, 15, 17 );
		CopyToBgTilemapBufferRect_ChangePalette(
			2, sCableCar->mountainTilemap, 0, 0, 30, 20, 17 );
		CopyToBgTilemapBufferRect_ChangePalette( 3,
			sCableCar->pylonHookTilemapEntries,
			0,
			0,
			5,
	...
	case 7:
		...
		CopyToBgTilemapBufferRect_ChangePalette( 0,
			sCableCar->mtChimneyTilemap + 0x48,
			0,
			14,
			12,
			3,
			17 );
		CopyToBgTilemapBufferRect_ChangePalette( 0,
			sCableCar->mtChimneyTilemap + 0x6C,
			12,
			17,
			12,
			3,
			17 );
		CopyToBgTilemapBufferRect_ChangePalette( 0,
			sCableCar->mtChimneyTilemap + 0x90,
			24,
			20,
			12,
			3,
			17 );
		CopyToBgTilemapBufferRect_ChangePalette( 0,
			sCableCar->mtChimneyTilemap + 0x0,
			0,
			17,
			12,
			3,
			17 );
		CopyToBgTilemapBufferRect_ChangePalette( 0,
			sCableCar->mtChimneyTilemap + 0x24,
			0,
			20,
			12,
			3,
	...
	case 1:
		...
		case WEATHER_ASH:
			if( gWeatherPtr->sprites.s2.ashSprites[0] != NULL &&
				gWeatherPtr->sprites.s2.ashSprites[0]
						->oam.priority != 0 )
			{
				for( ; i < NUM_ASH_SPRITES; i++ )
				{
					if( gWeatherPtr->sprites.s2
							.ashSprites[i] )
...
static void sub_8150664( u8 taskId )
{
	if( sCableCar->state != 0xFF )
	{
		...
		case 32:
			FillBgTilemapBufferRect( 3,
				sCableCar->pylonHookTilemapEntries[2],
				2,
				0,
				1,
				1,
				17 );
			FillBgTilemapBufferRect( 3,
				sCableCar->pylonHookTilemapEntries[3],
				3,
				0,
				1,
				1,
				17 );
			FillBgTilemapBufferRect( 3,
				sCableCar->pylonHookTilemapEntries[7],
				2,
				1,
				1,
				1,
				17 );
			FillBgTilemapBufferRect( 3,
				sCableCar->pylonHookTilemapEntries[8],
				3,
				1,
				1,
				1,
		...
		case 40:
			FillBgTilemapBufferRect( 3,
				sCableCar->pylonHookTilemapEntries[4],
				4,
				0,
				1,
				1,
				17 );
			FillBgTilemapBufferRect( 3,
				sCableCar->pylonHookTilemapEntries[9],
				4,
				1,
				1,
				1,
	...
	if( sCableCar->timer < sCableCar->unk4 )
		...
		gWeatherPtr->ashBaseSpritesX =
			( gWeatherPtr->ashBaseSpritesX + 247 ) % 248;
}
...
static void sub_8150B6C( bool8 which )
{
	...
	case TRUE:
		...
		SetGpuReg(
			REG_OFFSET_BG3HOFS, sCableCar->bg3HorizontalOffset );
		SetGpuReg( REG_OFFSET_BG3VOFS, sCableCar->bg3VerticalOffset );
		SetGpuReg( REG_OFFSET_BG2HOFS, 0 );
		SetGpuReg( REG_OFFSET_BG2VOFS, 0 );
		SetGpuReg(
			REG_OFFSET_BG1HOFS, sCableCar->bg1HorizontalOffset );
		SetGpuReg( REG_OFFSET_BG1VOFS, sCableCar->bg1VerticalOffset );
		SetGpuReg(
			REG_OFFSET_BG0HOFS, sCableCar->bg0HorizontalOffset );
		SetGpuReg( REG_OFFSET_BG0VOFS, sCableCar->bg0VerticalOffset );
...
static void LoadCableCarSprites( void )
{
	...
	u16 playerGraphicsIds[2]      = {EVENT_OBJ_GFX_RIVAL_BRENDAN_NORMAL,
                EVENT_OBJ_GFX_RIVAL_MAY_NORMAL};
	u16 rval                      = Random( );
	u16 hikerGraphicsIds[4]       = {EVENT_OBJ_GFX_HIKER,
	...
	default:
		spriteId = AddPseudoEventObject(
			playerGraphicsIds[gSaveBlock2Ptr->playerGender],
			sub_8150948,
			200,
			73,
			102 );
	...
	for( i = 0; i < 9; i++ )
	{
		spriteId = CreateSprite( &gSpriteTemplate_85CDBCC,
			16 * i + 96,
			8 * i - 8,
			0x68 );
		gSprites[spriteId].pos2.x = 8;
	...
	if( ( rval % 64 ) == 0 )
	{
		spriteId = AddPseudoEventObject( hikerGraphicsIds[rval % 3],
		...
		if( spriteId != MAX_SPRITES )
		{
			...
			gSprites[spriteId].pos2.x =
				-gSprites[spriteId].centerToCornerVecX;
			...
			gSprites[spriteId].data[2] =
				hikerMovementDelayTable[rval % 4];
		}
	}
}
...
void sub_8151088( void )
{
	...
	for( i = 0, k = 0, offset = 0x24 * ( sCableCar->unk1B + 2 ); i < 3;
		i++ )
	{
		for( j = 0; j < 12; j++ )
		{
			sCableCar->unk22[i][j] =
				sCableCar->mtChimneyTilemap[offset++];
			sCableCar->unk22[i + 3][j] =
				sCableCar->mtChimneyTilemap[k];
			sCableCar->unk22[i + 6][j] =
				( sCableCar->mtChimneyTilemap + 0x24 )[k];
			k++;
		}
	}
...
static void sub_8151214( void )
{
	...
	if( sCableCar->unk18 == 0 )
	{
		...
		FillBgTilemapBufferRect(
			0, 0, 0, gUnknown_0203ABB0, 32, 9, 17 );
	}
}
...
static void sub_8151388( void )
{
	...
	FillBgTilemapBufferRect(
		0, 0, sCableCar->unk19, gUnknown_0203ABB4, 2, 9, 17 );
