...
static const u16 sFontPalette[] =
	INCBIN_U16( "graphics/interface/berry_tag_screen.gbapal" );
...
static void HandleInitBackgrounds( void )
{
	...
	InitBgsFromTemplates(
		0, sBackgroundTemplates, ARRAY_COUNT( sBackgroundTemplates ) );
...
static bool8 LoadBerryTagGfx( void )
{
	...
	case 1:
		...
		{
			LZDecompressWram(
				gBerryTag_Gfx, sBerryTag->tilemapBuffers[0] );
			sBerryTag->gfxState++;
		}
		break;
	case 2:
		LZDecompressWram(
			gBerryTag_Pal, sBerryTag->tilemapBuffers[2] );
		sBerryTag->gfxState++;
		break;
	case 3:
		...
		{
			for( i = 0; i <
				ARRAY_COUNT( sBerryTag->tilemapBuffers[1] );
				i++ )
...
static void AddBerryTagTextToBg0( void )
{
	memcpy( GetBgTilemapBuffer( 0 ),
		sBerryTag->tilemapBuffers[2],
...
static void PrintBerrySize( void )
{
	...
	AddTextPrinterParameterized(
		WIN_SIZE_FIRM, 1, gText_SizeSlash, 0, 1, TEXT_SPEED_FF, NULL );
	if( berry->size != 0 )
	{
		u32 inches, fraction;
		...
		AddTextPrinterParameterized(
			WIN_SIZE_FIRM, 1, gStringVar4, 0x28, 1, 0, NULL );
	}
	else
	{
		AddTextPrinterParameterized(
			WIN_SIZE_FIRM, 1, gText_ThreeMarks, 0x28, 1, 0, NULL );
	}
}
...
{
	const struct Berry* berry = GetBerryInfo( sBerryTag->berryId );
	AddTextPrinterParameterized( WIN_SIZE_FIRM,
		1,
		gText_FirmSlash,
		0,
		0x11,
...
static void PrintBerryDescription1( void )
{
	const struct Berry* berry = GetBerryInfo( sBerryTag->berryId );
	AddTextPrinterParameterized(
		WIN_DESC, 1, berry->description1, 0, 1, 0, NULL );
}
...
static void PrintBerryDescription2( void )
{
	const struct Berry* berry = GetBerryInfo( sBerryTag->berryId );
	AddTextPrinterParameterized(
		WIN_DESC, 1, berry->description2, 0, 0x11, 0, NULL );
}
...
static void CreateFlavorCircleSprites( void )
{
	sBerryTag->flavorCircleIds[FLAVOR_SPICY] =
		CreateBerryFlavorCircleSprite( 64 );
	sBerryTag->flavorCircleIds[FLAVOR_DRY] =
		CreateBerryFlavorCircleSprite( 104 );
	sBerryTag->flavorCircleIds[FLAVOR_SWEET] =
		CreateBerryFlavorCircleSprite( 144 );
	sBerryTag->flavorCircleIds[FLAVOR_BITTER] =
		CreateBerryFlavorCircleSprite( 184 );
	sBerryTag->flavorCircleIds[FLAVOR_SOUR] =
		CreateBerryFlavorCircleSprite( 224 );
}
...
static void SetFlavorCirclesVisiblity( void )
{
	...
	if( berry->spicy )
		gSprites[sBerryTag->flavorCircleIds[FLAVOR_SPICY]].invisible =
			FALSE;
	else
		gSprites[sBerryTag->flavorCircleIds[FLAVOR_SPICY]].invisible =
			TRUE;
	...
	if( berry->dry )
		gSprites[sBerryTag->flavorCircleIds[FLAVOR_DRY]].invisible =
			FALSE;
	else
		gSprites[sBerryTag->flavorCircleIds[FLAVOR_DRY]].invisible =
			TRUE;
	...
	if( berry->sweet )
		gSprites[sBerryTag->flavorCircleIds[FLAVOR_SWEET]].invisible =
			FALSE;
	else
		gSprites[sBerryTag->flavorCircleIds[FLAVOR_SWEET]].invisible =
			TRUE;
	...
	if( berry->bitter )
		gSprites[sBerryTag->flavorCircleIds[FLAVOR_BITTER]].invisible =
			FALSE;
	else
		gSprites[sBerryTag->flavorCircleIds[FLAVOR_BITTER]].invisible =
			TRUE;
	...
	if( berry->sour )
		gSprites[sBerryTag->flavorCircleIds[FLAVOR_SOUR]].invisible =
			FALSE;
	else
		gSprites[sBerryTag->flavorCircleIds[FLAVOR_SOUR]].invisible =
			TRUE;
...
static void TryChangeDisplayedBerry( u8 taskId, s8 toMove )
{
	...
	s16 currPocketPosition = gBagPositionStruct.scrollPosition[3] +
		gBagPositionStruct.cursorPosition[3];
	u32 newPocketPosition = currPocketPosition + toMove;
	if( newPocketPosition < 46 &&
		BagGetItemIdByPocketPosition(
			POCKET_BERRIES, newPocketPosition ) != 0 )
	{
		if( toMove < 0 )
			data[1] = 2;
		else
...
static void HandleBagCursorPositionChange( s8 toMove )
{
	...
	sBerryTag->berryId = ItemIdToBerryType( BagGetItemIdByPocketPosition(
		POCKET_BERRIES, *scrollPos + *cursorPos ) );
}
