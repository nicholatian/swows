...
u32 GetMewMoveDirection( void )
{
	...
	sPlayerToMewDeltaX =
		gEventObjects[gPlayerAvatar.eventObjectId].previousCoords.x -
		mew->currentCoords.x;
	sPlayerToMewDeltaY =
		gEventObjects[gPlayerAvatar.eventObjectId].previousCoords.y -
		mew->currentCoords.y;
	for( i = 0; i < ARRAY_COUNT( sMewDirectionCandidates ); i++ )
		sMewDirectionCandidates[i] = DIR_NONE;

	if( gEventObjects[gPlayerAvatar.eventObjectId].previousCoords.x ==
	...
	{
		if( gEventObjects[gPlayerAvatar.eventObjectId]
				.previousCoords.x ==
	...
	{
		if( gEventObjects[gPlayerAvatar.eventObjectId]
				.currentCoords.y > mew->currentCoords.y )
		{
			if( CanMewWalkToCoords( mew->currentCoords.x,
		...
		if( CanMewWalkToCoords(
			    mew->currentCoords.x, mew->currentCoords.y - 1 ) )
			return DIR_NORTH;
	...
	{
		if( gEventObjects[gPlayerAvatar.eventObjectId]
				.currentCoords.x > mew->currentCoords.x )
		{
			if( CanMewWalkToCoords( mew->currentCoords.x - 1,
				    mew->currentCoords.y ) )
				return DIR_WEST;
		}
		...
		if( CanMewWalkToCoords(
			    mew->currentCoords.x + 1, mew->currentCoords.y ) )
			return DIR_EAST;
...
static u8 sub_81D4890( u8 ignoredDir )
{
	...
	if( CanMewWalkToCoords(
		    mew->currentCoords.x, mew->currentCoords.y - 1 ) == TRUE &&
	...
	if( CanMewWalkToCoords(
		    mew->currentCoords.x + 1, mew->currentCoords.y ) == TRUE &&
		ignoredDir != DIR_EAST )
	{
	...
	if( count > 1 )
		return sMewDirectionCandidates
			[VarGet( VAR_FARAWAY_ISLAND_STEP_COUNTER ) % count];
	else
...
void sub_81D4A90( void )
{
	...
	{
		mew->fixedPriority = 1;
		gSprites[mew->spriteId].subspriteMode =
	...
	else
	{
		...
		UpdateSpritePaletteWithWeather( IndexOfSpritePaletteTag(
			gFieldEffectObjectPaletteInfo1.tag ) );
...
{
	return sMewDirectionCandidates
		[VarGet( VAR_FARAWAY_ISLAND_STEP_COUNTER ) % mod];
}
