...
#define TILE_ATTRIBUTES( unused, surfable, wildEncounter ) \
	( ( ( wildEncounter ) ? 1 : 0 ) | ( ( surfable ) ? 2 : 0 ) | \
		( ( unused ) ? 4 : 0 ) )
...
const u8 mt_attrs[] = {
	...
	[MB_UP2_ESCALATOR]           = TILE_ATTRIBUTES( TRUE, FALSE, FALSE ),
	[MB_DOWN2_ESCALATOR]         = TILE_ATTRIBUTES( TRUE, FALSE, FALSE ),
	...
	[MB_PACIFIDLOG_HORIZONTAL_LOG_1] =
		TILE_ATTRIBUTES( TRUE, FALSE, FALSE ),
	[MB_PACIFIDLOG_HORIZONTAL_LOG_2] =
		TILE_ATTRIBUTES( TRUE, FALSE, FALSE ),
	...
	//[MB_UNUSED_EB]                  = TILE_ATTRIBUTES( FALSE, FALSE,
	//FALSE ), [MB_UNUSED_EC]                  = TILE_ATTRIBUTES( FALSE,
	//FALSE, FALSE ),
...
bool8 MetatileBehavior_IsNonOceanSurfableWater( u8 mb )
{
	return mb == MB_POND_WATER ? TRUE : FALSE;
}
...
{
	if( ( mt_attrs[metatileBehavior] & TILE_FLAG_ENCOUNTER_TILE ) )
		return TRUE;
	else
		return FALSE;
}
...
bool8 MetatileBehavior_IsEscalator( u8 mb )
{
	return mb == MB_UP_ESCALATOR || mb == MB_DOWN_ESCALATOR ||
		mb == MB_UP2_ESCALATOR || mb == MB_DOWN2_ESCALATOR;
}
...
{
	if( ( mt_attrs[metatileBehavior] & TILE_FLAG_SURFABLE ) )
		return TRUE;
	else
		return FALSE;
}
...
bool8 MetatileBehavior_IsPlayerFacingTVScreen(
	u8 metatileBehavior, u8 playerDir )
{
...
bool8 MetatileBehavior_IsPlayerFacingWirelessBoxResults(
	u8 tile, u8 playerDir )
{
	if( playerDir != CONNECTION_NORTH ) // if the player isn't facing
					    // north, forget about it.
		return FALSE;
	else if( tile ==
		MB_WIRELESS_BOX_RESULTS ) // is the player's north tile the
...
bool8 MetatileBehavior_IsCableBoxResults2( u8 tile, u8 playerDir )
{
	if( playerDir != CONNECTION_NORTH ) // if the player isn't facing
					    // north, forget about it.
		return FALSE;
	else if( tile == MB_CABLE_BOX_RESULTS_2 ) // is the player's north tile
