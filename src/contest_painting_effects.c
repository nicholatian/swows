...
static void sub_81250B8( u8 a0 ) // it changes palette someway somehow... .__.
{
	...
	{
		u16* var2 =
			&( *gUnknown_03006168 )[0][( gUnknown_03006178 + i ) *
				gUnknown_03006180];
...
static void sub_81252E8( void )
{
	...
	{
		u16* var0    = &( *gUnknown_03006168 )[0][gUnknown_03006178 *
                        gUnknown_03006180];
...
static void sub_81254E0( void )
{
	...
	for( i = 0; i < gUnknown_0300616C; i++ )
	{
		...
		*palette = sub_8125C98( palette, palette + 1 );
		for( j = 1, palette = palette + 1; j < gUnknown_03006174 - 1;
			j++, palette++ )
		{
			*palette = sub_8125C98( palette, palette + 1 );
			*palette = sub_8125C98( palette, palette - 1 );
	...
	for( j = 0; j < gUnknown_03006174; j++ )
	{
		...
		*palette = sub_8125C98( palette, palette + gUnknown_03006180 );
		for( i = 1, palette = palette + gUnknown_03006180;
			i < gUnknown_0300616C - 1;
			i++, palette += gUnknown_03006180 )
		{
			*palette = sub_8125C98(
				palette, palette + gUnknown_03006180 );
			*palette = sub_8125C98(
				palette, palette - gUnknown_03006180 );
		}
...
static void sub_81257F8( void )
{
	...
	for( i = 0; i < gUnknown_0300616C; i++ )
	{
		...
		u16 color    = *palette;
		for( j = 1, palette++; j < gUnknown_03006174 - 1;
			j++, palette++ )
		{
			if( !( 0x8000 & *palette ) )
			{
...
static void sub_81258A0( void )
{
	...
	for( i = 0; i < gUnknown_03006174; i++ )
	{
		...
		u16* palette = &var0[gUnknown_03006164 + i];
		u16 color    = *palette;
		for( j = 1, palette += gUnknown_03006180;
			j < gUnknown_0300616C - 1;
			j++, palette += gUnknown_03006180 )
		{
			if( !( 0x8000 & *palette ) )
			{
				*palette = sub_8125CF4( &color, palette );
...
static void sub_8125954( u16 arg0 )
{
	...
	{
		u16* pal = &( *gUnknown_03006168 )[unkStruct[i].unk1 * 2]
						  [unkStruct[i].unk0];
		...
		if( !( 0x8000 & *pal ) )
		{
			...
			case 0:
			case 1:
				switch( ( ( gUnknown_085A1F94[arg0][2] >> 3 ) &
						7 ) %
					3 )
				{
				case 0:
...
static u16 ConvertColorToGrayscale( u16* color )
{
	...
	s32 g    = ( clr >> 5 ) & 0x1F;
	s32 b    = ( clr >> 10 ) & 0x1F;
	s32 gray = ( r * Q_8_8( 0.3 ) + g * Q_8_8( 0.59 ) +
			   b * Q_8_8( 0.1133 ) ) >>
		8;
...
static u16 sub_8125E18( u16* a0, u16* a1, u16* a2 )
{
	...
	avg0 = ( ( ( *a0 >> 0 ) & 0x1F ) + ( ( *a0 >> 5 ) & 0x1F ) +
		       ( ( *a0 >> 10 ) & 0x1F ) ) /
		3;
	avg1 = ( ( ( *a1 >> 0 ) & 0x1F ) + ( ( *a1 >> 5 ) & 0x1F ) +
		       ( ( *a1 >> 10 ) & 0x1F ) ) /
		3;
	avg2 = ( ( ( *a2 >> 0 ) & 0x1F ) + ( ( *a2 >> 5 ) & 0x1F ) +
		       ( ( *a2 >> 10 ) & 0x1F ) ) /
		3;
...
static void sub_8126334( void )
{
	...
	gUnknown_03006184[1] = RGB2( 0, 0, 0 );
	for( i = 0; i < 14; i++ )
		gUnknown_03006184[i + 2] =
			RGB2( 2 * ( i + 2 ), 2 * ( i + 2 ), 2 * ( i + 2 ) );
}
	...
	for( i = 0; i < gUnknown_0300616C; i++ )
	{
		...
		for( j = 0; j < gUnknown_03006174; j++, pal++ )
		{
			...
			else
			{
				...
				{
					if( gUnknown_03006184[curIndex] ==
						RGB_BLACK )
					{
						gUnknown_03006184[curIndex] =
							color;
						*pal = gUnknown_03006188 +
							curIndex;
					}
