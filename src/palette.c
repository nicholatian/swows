...
void LoadCompressedPalette( const u32* src, u16 offset, u16 size )
{
	...
	CpuCopy16( gPaletteDecompressionBuffer,
		gPlttBufferUnfaded + offset,
		size );
	CpuCopy16(
		gPaletteDecompressionBuffer, gPlttBufferFaded + offset, size );
}
...
bool8 BeginNormalPaletteFade(
	u32 selectedPalettes, s8 delay, u8 startY, u8 targetY, u16 blendColor )
{
	u8 temp;
...
static void unused_sub_80A1CDC( struct PaletteStruct* a1, u32* a2 )
{
	...
	if( !a1->base->pst_field_8_0 )
	{
		...
		{
			gPlttBufferUnfaded[a1->destOffset] =
				a1->base->src[srcOffset];
			gPlttBufferFaded[a1->destOffset] =
				a1->base->src[srcOffset];
			i++;
			a1->destOffset++;
			srcOffset++;
			...
			gPaletteFade_selectedPalettes ) )
	{
		...
		else
		{
			if( !gPaletteFade.delayCounter )
			{
				...
				{
					u32 srcOffset =
						a1->srcIndex * a1->base->size;
					u8 i;

					for( i = 0; i < a1->base->size; i++ )
						gPlttBufferFaded[a1->baseDestOffset +
							i] =
							a1->base->src
								[srcOffset +
									i];
...
static u8 UpdateNormalPaletteFade( void )
{
	...
	else
	{
		...
		// gPaletteFade.active cannot change since the last time it was
		// checked. So this is equivalent to `return
...
static u8 UpdateFastPaletteFade( void )
{
	...
	case FAST_FADE_OUT_TO_WHITE:
		...
		{
			struct PlttData* data =
				(struct PlttData*)&gPlttBufferFaded[i];
			r = data->r + 2;
			g = data->g + 2;
			b = data->b + 2;
...
void BeginHardwarePaletteFade(
	u8 blendCnt, u8 delay, u8 y, u8 targetY, u8 shouldResetBlendRegisters )
{
...
void TintPalette_CustomTone(
	u16* palette, u16 count, u16 rTone, u16 gTone, u16 bTone )
{
	s32 r, g, b, i;
	u32 gray;
