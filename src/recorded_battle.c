...
#define ILLEGAL_BATTLE_TYPES \
	( ( BATTLE_TYPE_LINK | BATTLE_TYPE_SAFARI | \
		...
		BATTLE_TYPE_KYOGRE_GROUDON | BATTLE_TYPE_LEGENDARY | \
		BATTLE_TYPE_REGI | BATTLE_TYPE_RECORDED | \
...
void RecordedBattle_SetBattlerAction( u8 battlerId, u8 action )
{
	if( sRecordedBytesNo[battlerId] < BATTLER_RECORD_SIZE &&
		sUnknown_0203C7AC != 2 )
	{
		sBattleRecords[battlerId][sRecordedBytesNo[battlerId]++] =
			action;
	}
}
...
u8 RecordedBattle_GetBattlerAction( u8 battlerId )
{
			...
			0xFF )
	{
		gSpecialVar_Result = gBattleOutcome =
			B_OUTCOME_PLAYER_TELEPORTED; // hah
	...
	{
		return sBattleRecords[battlerId]
				     [sRecordedBytesNo[battlerId]++];
	}
}

u8 sub_81850D0( void ) { return sUnknown_0203C7AC; }
...
u8 sub_81850DC( u8* arg0 )
{
	...
	for( i = 0; i < MAX_BATTLERS_COUNT; i++ )
	{
		if( sRecordedBytesNo[i] != sUnknown_0203C79C[i] )
		{
			arg0[ret++] = i;
			arg0[ret++] =
				sRecordedBytesNo[i] - sUnknown_0203C79C[i];

			for( j = 0;
				j < sRecordedBytesNo[i] - sUnknown_0203C79C[i];
				j++ )
			{
				arg0[ret++] =
					sBattleRecords[i]
...
void sub_81851A8( u8* arg0 )
{
	...
	if( !( gBattleTypeFlags & BATTLE_TYPE_IS_MASTER ) )
	{
		for( var2 = *arg0; var2 != 0; )
		{
			...
			for( i = 0; i < unkVar2; i++ )
			{
				sBattleRecords[unkVar]
					      [sUnknown_0203C7A4[unkVar]++] =
						      sub_8185278( arg0,
							      &var1,
							      &var2 );
			}
...
static bool32 IsRecordedBattleSaveValid( struct RecordedBattleSave* save )
{
	...
	if( save->battleFlags & ILLEGAL_BATTLE_TYPES )
		return FALSE;
	if( CalcByteArraySum( (void*)( save ), sizeof( *save ) - 4 ) !=
		save->checksum )
		return FALSE;
...
static bool32 RecordedBattleToSave( struct RecordedBattleSave* battleSave,
	struct RecordedBattleSave* saveSection )
{
	memset( saveSection, 0, 0x1000 );
	memcpy( saveSection, battleSave, sizeof( *battleSave ) );

	saveSection->checksum = CalcByteArraySum(
		(void*)( saveSection ), sizeof( *saveSection ) - 4 );

	if( TryWriteSpecialSaveSection(
		    SECTOR_ID_RECORDED_BATTLE, (void*)( saveSection ) ) != 1 )
		return FALSE;
	else
		return TRUE;
}
...
bool32 MoveRecordedBattleToSaveData( void )
{
	...
	if( sBattleFlags & BATTLE_TYPE_LINK )
	{
		battleSave->battleFlags =
			( sBattleFlags &
				~( BATTLE_TYPE_LINK | BATTLE_TYPE_20 ) ) |
		...
		else if( sBattleFlags & BATTLE_TYPE_MULTI )
		{
			...
			case 0:
			case 2:
				if( !( sPlayers[gUnknown_0203C7B4].battlerId &
					    1 ) )
					battleSave->battleFlags |=
		...
		gPartnerTrainerId < TRAINER_RECORD_MIXING_APPRENTICE )
	{
		for( i = 0; i < PLAYER_NAME_LENGTH + 1; i++ )
			battleSave->recordMixFriendName[i] =
				gSaveBlock2Ptr->frontier
					.towerRecords[gPartnerTrainerId -
						TRAINER_RECORD_MIXING_FRIEND]
					.name[i];
		battleSave->recordMixFriendClass =
			gSaveBlock2Ptr->frontier
...
{
	if( TryReadSpecialSaveSection(
		    SECTOR_ID_RECORDED_BATTLE, (void*)( saveBuffer ) ) != 1 )
		return FALSE;
...
static bool32 CopyRecordedBattleFromSave( struct RecordedBattleSave* dst )
{
	struct SaveSection* savBuffer =
		AllocZeroed( sizeof( struct SaveSection ) );
...
void sub_8185F84( void ) { sUnknown_0203C7B5 = 0; }

void sub_8185F90( u16 arg0 ) { sUnknown_0203C7B5 |= ( arg0 & 0x8000 ) >> 0xF; }

u8 sub_8185FAC( void ) { return sUnknown_0203C7B5; }
...
void sub_818603C( u8 arg0 )
{
	...
	for( battlerId = 0; battlerId < gBattlersCount; battlerId++ )
	{
					  ...
					  // only
		{
			if( arg0 == 1 )
			{
				for( j = 0; j < MAX_MON_MOVES; j++ )
				{
					if( gBattleMons[battlerId].moves[j] !=
						sPlayerMonMoves[battlerId / 2]
							       [j] )
						break;
				}
				if( j != MAX_MON_MOVES ) // player's mon's move
							 // has been changed
				{
			...
			{
				if( sBattleRecords[battlerId][sRecordedBytesNo
						    [battlerId]] ==
					ACTION_MOVE_CHANGE )
				{
					u8 ppBonuses[4];
...
void sub_8186444( void ) { sUnknown_0203CCD0 = 1; }

bool8 sub_8186450( void ) { return ( sUnknown_0203CCD0 == 0 ); }
