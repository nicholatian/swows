...
static void sub_8098400( u8 taskId )
{
	...
	if( !task->data[1] &&
		!gEventObjects[gSelectedEventObject].singleMovementActive )
	{
...
void sub_8098574( void )
{
	EventObjectFaceOppositeDirection(
		&gEventObjects[gSelectedEventObject], gSpecialVar_Facing );
}
...
static void sub_80985BC( u8 taskId )
{
	...
	if( !task->data[1] &&
		!gEventObjects[eventObjectId].singleMovementActive )
	{
