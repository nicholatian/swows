...
const u16 gUnknown_0860F074[] =
	INCBIN_U16( "graphics/interface/860F074.gbapal" );
...
const u16 gUnknown_0860F0B0[] =
	INCBIN_U16( "graphics/interface/860F0B0.gbapal" );
...
void AddTextPrinterForMessage_2( bool8 allowSkippingDelayWithButtonPress )
{
	...
	AddTextPrinterParameterized2(
		0, 1, gStringVar4, GetPlayerTextSpeedDelay( ), NULL, 2, 1, 3 );
}
	...
	bool8 allowSkippingDelayWithButtonPress, u8 speed )
{
	gTextFlags.canABSpeedUpPrint = allowSkippingDelayWithButtonPress;
	AddTextPrinterParameterized2(
		0, 1, gStringVar4, speed, NULL, 2, 1, 3 );
}

void sub_81973A4( void )
{
	LoadMessageBoxGfx(
		0, DLG_WINDOW_BASE_TILE_NUM, DLG_WINDOW_PALETTE_NUM * 0x10 );
	LoadUserWindowBorderGfx(
		0, STD_WINDOW_BASE_TILE_NUM, STD_WINDOW_PALETTE_NUM * 0x10 );
}
...
void WindowFunc_DrawStandardFrame( u8 bg,
	u8 tilemapLeft,
	u8 tilemapTop,
	u8 width,
	u8 height,
...
void WindowFunc_DrawDialogueFrame( u8 bg,
	u8 tilemapLeft,
	u8 tilemapTop,
	u8 width,
	u8 height,
...
void WindowFunc_ClearStdWindowAndFrame( u8 bg,
	u8 tilemapLeft,
	u8 tilemapTop,
	u8 width,
	u8 height,
...
void WindowFunc_ClearDialogWindowAndFrame( u8 bg,
	u8 tilemapLeft,
	u8 tilemapTop,
	u8 width,
	u8 height,
...
{
	DrawStdFrameWithCustomTileAndPalette( windowId,
		copyToVram,
...
{
	LoadMessageBoxGfx( windowId,
		DLG_WINDOW_BASE_TILE_NUM,
		DLG_WINDOW_PALETTE_NUM * 0x10 );
	DrawDialogFrameWithCustomTileAndPalette(
		windowId, copyToVram, DLG_WINDOW_BASE_TILE_NUM, 0xF );
}
...
const u16* sub_81978C8( void ) { return gUnknown_0860F074; }
...
void DisplayItemMessageOnField(
	u8 taskId, const u8* string, TaskFunc callback )
{
	sub_81973A4( );
...
u8 sub_81979C4( u8 a1 )
{
	if( sStartMenuWindowId == 0xFF )
		sStartMenuWindowId = sub_8198AA4(
			0, 0x16, 1, 7, ( a1 * 2 ) + 2, 0xF, 0x139 );
...
u16 sub_8197A30( void ) { return DLG_WINDOW_BASE_TILE_NUM; }

u16 sub_8197A38( void ) { return STD_WINDOW_BASE_TILE_NUM; }

u8 AddMapNamePopUpWindow( void )
{
	if( sMapNamePopupWindowId == 0xFF )
		sMapNamePopupWindowId =
			sub_8198AA4( 0, 1, 1, 10, 3, 14, 0x107 );
	...
	u8 windowId, bool8 copyToVram, u16 tileNum, u8 paletteNum )
{
	...
	CallWindowFunction(
		windowId, WindowFunc_DrawDialogFrameWithCustomTileAndPalette );
	FillWindowPixelBuffer( windowId, PIXEL_FILL( 1 ) );
	PutWindowTilemap( windowId );
	if( copyToVram == TRUE )
		CopyWindowToVram( windowId, 3 );
...
void DrawDialogFrameWithCustomTile(
	u8 windowId, bool8 copyToVram, u16 tileNum )
{
	sTileNum    = tileNum;
...
void WindowFunc_DrawDialogFrameWithCustomTileAndPalette( u8 bg,
	u8 tilemapLeft,
	u8 tilemapTop,
	u8 width,
	u8 height,
...
void ClearDialogWindowAndFrameToTransparent( u8 windowId, bool8 copyToVram )
{
	...
	CallWindowFunction(
		windowId, WindowFunc_ClearDialogWindowAndFrameNullPalette );
...
void WindowFunc_ClearDialogWindowAndFrameNullPalette( u8 bg,
	u8 tilemapLeft,
	u8 tilemapTop,
	u8 width,
	u8 height,
	...
	u8 windowId, bool8 copyToVram, u16 baseTileNum, u8 paletteNum )
{
	...
	sPaletteNum = paletteNum;
	CallWindowFunction(
		windowId, WindowFunc_DrawStdFrameWithCustomTileAndPalette );
	FillWindowPixelBuffer( windowId, PIXEL_FILL( 1 ) );
	PutWindowTilemap( windowId );
	if( copyToVram == TRUE )
		CopyWindowToVram( windowId, 3 );
...
void DrawStdFrameWithCustomTile(
	u8 windowId, bool8 copyToVram, u16 baseTileNum )
{
	sTileNum    = baseTileNum;
	sPaletteNum = GetWindowAttribute( windowId, WINDOW_PALETTE_NUM );
	CallWindowFunction(
...
void WindowFunc_DrawStdFrameWithCustomTileAndPalette( u8 bg,
	u8 tilemapLeft,
	u8 tilemapTop,
	u8 width,
	u8 height,
...
{
	CallWindowFunction(
		windowId, WindowFunc_ClearStdWindowAndFrameToTransparent );
	FillWindowPixelBuffer( windowId, PIXEL_FILL( 0 ) );
	ClearWindowTilemap( windowId );
	if( copyToVram == TRUE )
		CopyWindowToVram( windowId, 3 );
...
void WindowFunc_ClearStdWindowAndFrameToTransparent( u8 bg,
	u8 tilemapLeft,
	u8 tilemapTop,
	u8 width,
	u8 height,
...
void sub_8198204(
	const u8* string, const u8* string2, u8 a3, u8 a4, bool8 copyToVram )
{
	u8 color[3];
	u16 width = 0;
	...
	if( sWindowId != 0xFF )
	{
		...
		AddTextPrinterParameterized4(
			sWindowId, 1, 4, 1, 0, 0, color, 0, string );
		if( copyToVram )
			CopyWindowToVram( sWindowId, 3 );
	}
}
...
void sub_8198854( u8 windowId,
	u8 fontId,
	u8 lineHeight,
	u8 itemCount,
	const struct MenuAction* strs )
{
	PrintTextArray( windowId,
		fontId,
		GetFontAttribute( fontId, 0 ),
		1,
		lineHeight,
		itemCount,
		strs );
	...
	const u8* a8 )
{
	...
	printer.bgColor =
		GetFontAttribute( fontId, FONTATTR_COLOR_BACKGROUND );
	printer.shadowColor =
...
struct WindowTemplate CreateWindowTemplate( u8 bg,
	u8 left,
	u8 top,
	u8 width,
	u8 height,
	...
	u16 baseBlock )
{
	...
	SetWindowTemplateFields( &template,
		bg,
		left,
		top,
		width,
	...
	u8 initialCursorPos )
{
	...
	DrawStdFrameWithCustomTileAndPalette(
		sYesNoWindowId, TRUE, baseTileNum, paletteNum );

	printer.currentChar = gText_YesNo;
	printer.windowId    = sYesNoWindowId;
	printer.fontId      = fontId;
	printer.x =
		GetFontAttribute( fontId, FONTATTR_MAX_LETTER_WIDTH ) + left;
	printer.y        = top;
...
void sub_8198C34( const struct WindowTemplate* window,
	u8 fontId,
	u16 baseTileNum,
	u8 paletteNum )
{
	...
	const u8* a8 )
{
	...
	for( i = 0; i < itemCount2; i++ )
	{
		...
		{
			printer.currentChar =
				strs[a8[( itemCount * i ) + j]].text;
			printer.x = ( a4 * j ) + left;
			printer.y =
				( GetFontAttribute( fontId, 1 ) * i ) + top;
			printer.currentX = printer.x;
			printer.currentY = printer.y;
			AddTextPrinter( &printer, 0xFF, NULL );
		}
...
u8 sub_8198FD4(
	u8 windowId, u8 fontId, u8 left, u8 top, u8 a4, u8 a5, u8 a6, u8 a7 )
{
	u8 cursorHeight = GetMenuCursorDimensionByFont( fontId, 1 );
	u8 numChoices   = a5 * a6;
	return sub_8198F58( windowId,
		fontId,
		left,
		top,
		a4,
...
void sub_8199060( u8 oldCursorPos, u8 newCursorPos )
{
	...
	u8 xPos = ( oldCursorPos % sMenu.columns ) * sMenu.optionWidth +
		sMenu.left;
	u8 yPos = ( oldCursorPos / sMenu.columns ) * sMenu.optionHeight +
		sMenu.top;
	FillWindowPixelRect( sMenu.windowId,
		PIXEL_FILL( 1 ),
		xPos,
		...
		cursorWidth,
		cursorHeight );
	xPos = ( newCursorPos % sMenu.columns ) * sMenu.optionWidth +
		sMenu.left;
	yPos = ( newCursorPos / sMenu.columns ) * sMenu.optionHeight +
		sMenu.top;
	AddTextPrinterParameterized( sMenu.windowId,
		sMenu.fontId,
		gText_SelectorArrow3,
...
u8 sub_8199134( s8 deltaX, s8 deltaY )
{
	...
	if( deltaX != 0 )
	{
		...
		else if( ( sMenu.cursorPos % sMenu.columns ) + deltaX >=
			sMenu.columns )
		{
			sMenu.cursorPos = ( sMenu.cursorPos / sMenu.columns ) *
				sMenu.columns;
		}
		else
		{
	...
	if( deltaY != 0 )
	{
		...
		else if( ( sMenu.cursorPos / sMenu.columns ) + deltaY >=
			sMenu.rows )
		{
			sMenu.cursorPos -= sMenu.columns * ( sMenu.rows - 1 );
		}
...
u8 sub_81991F8( s8 deltaX, s8 deltaY )
{
	...
	{
		if( ( ( sMenu.cursorPos % sMenu.columns ) + deltaX >= 0 ) &&
			( ( sMenu.cursorPos % sMenu.columns ) + deltaX <
				sMenu.columns ) )
		{
			sMenu.cursorPos += deltaX;
		}
...
u8 InitMenuInUpperLeftCorner(
	u8 windowId, u8 itemCount, u8 initialCursorPos, bool8 APressMuted )
{
	s32 pos;
...
u8 InitMenuInUpperLeftCornerPlaySoundWhenAPressed(
	u8 windowId, u8 itemCount, u8 initialCursorPos )
{
	return InitMenuInUpperLeftCorner(
		windowId, itemCount, initialCursorPos, FALSE );
}
...
void CreateYesNoMenu( const struct WindowTemplate* window,
	u16 baseTileNum,
	u8 paletteNum,
	u8 initialCursorPos )
{
	...
	InitMenuInUpperLeftCornerPlaySoundWhenAPressed(
		sYesNoWindowId, 2, initialCursorPos );
}

void PrintMenuGridTable( u8 windowId,
	u8 optionWidth,
	u8 columns,
	u8 rows,
	const struct MenuAction* strs )
...
u8 sub_8199944(
	u8 windowId, u8 optionWidth, u8 columns, u8 rows, u8 initialCursorPos )
{
	s32 pos;
...
void* decompress_and_copy_tile_data_to_vram(
	u8 bgId, const void* src, u32 size, u16 offset, u8 mode )
{
	u32 sizeOut;
...
void DecompressAndLoadBgGfxUsingHeap(
	u8 bgId, const void* src, u32 size, u16 offset, u8 mode )
{
	u32 sizeOut;
	void* ptr = malloc_and_decompress( src, &sizeOut );
	...
	if( ptr )
	{
		u8 taskId = CreateTask(
			task_free_buf_after_copying_tile_data_to_vram, 0 );
		gTasks[taskId].data[0] = copy_decompressed_tile_data_to_vram(
			bgId, ptr, size, offset, mode );
...
u16 copy_decompressed_tile_data_to_vram(
	u8 bgId, const void* src, u16 size, u16 offset, u8 mode )
{
	switch( mode )
	{
	case 0:
...
void sub_8199D3C( void* ptr, int delta, int width, int height, bool32 is8BPP )
{
	...
	else
	{
		...
		for( i = 0; i < area; i++ )
		{
			as4BPP[i] = ( as4BPP[i] & 0xFC00 ) |
				( ( as4BPP[i] + delta ) & 0x3FF );
		}
	}
}
...
{
	int temp =
		( !GetBgAttribute( bg, BG_ATTR_PALETTEMODE ) ) ? 0x20 : 0x40;
	void* addr = (void*)( ( GetBgAttribute( bg, BG_ATTR_CHARBASEINDEX ) *
				      0x4000 ) +
		( GetBgAttribute( bg, BG_ATTR_BASETILE ) + a2 ) * temp );
	RequestDma3Fill( a1 << 24 | a1 << 16 | a1 << 8 | a1,
		addr + VRAM,
		a3 * temp,
		1 );
}

void AddTextPrinterParameterized3( u8 windowId,
	u8 fontId,
	u8 left,
	u8 top,
	const u8* color,
...
#ifdef NONMATCHING
void sub_819A080( struct UnkStruct_819A080* a0,
	struct UnkStruct_819A080* a1,
	u16 a2,
	u16 a3,
	u16 a4,
	u16 a5,
	...
	u16 a7 )
{
	...
	for( r12 = a3, r8 = a5; r12 < sp14; r12++, r8++ )
	{
		...
		{
			u8* r3 = a0->unk00 + ( ( r5 >> 1 ) & 0x3 ) +
				( ( r5 >> 3 ) << 5 ) +
				( ( ( r12 >> 3 ) * sp18 ) << 5 ) +
				( ( r12 & 0x7 ) << 2 );
			u8* r4 = a1->unk00 + ( ( r6 >> 1 ) & 0x3 ) +
				( ( r6 >> 3 ) << 5 ) +
				( ( ( r8 >> 3 ) * sp1c ) << 5 ) +
				( ( r8 & 0x7 ) << 2 );
			if( ( (uintptr_t)r4 ) & 0x1 )
...
void blit_move_info_icon( u8 windowId, u8 iconId, u16 x, u16 y )
{
	BlitBitmapRectToWindow( windowId,
		gFireRedMenuElements_Gfx +
			gMoveMenuInfoIcons[iconId].offset * 32,
		0,
		0,
		128,
		128,
	...
	case 1:
		if( IsNationalPokedexEnabled( ) )
			string = ConvertIntToDecimalStringN(
				string, GetNationalPokedexCount( 1 ), 0, 3 );
		else
			string = ConvertIntToDecimalStringN(
				string, GetHoennPokedexCount( 1 ), 0, 3 );
		*string = EOS;
		break;
	case 2:
		string = ConvertIntToDecimalStringN(
			string, gSaveBlock2Ptr->playTimeHours, 0, 3 );
		*( string++ ) = CHAR_COLON;
		ConvertIntToDecimalStringN(
			string, gSaveBlock2Ptr->playTimeMinutes, 2, 2 );
		break;
	case 3:
		sub_81245DC( string, gMapHeader.regionMapSectionId );
		break;
	case 4:
		for( curFlag     = FLAG_BADGE01_GET,
		     flagCount   = 0,
		     endOfString = string + 1;
			curFlag <= FLAG_BADGE08_GET;
			curFlag++ )
		{
			if( FlagGet( curFlag ) )
				flagCount++;
