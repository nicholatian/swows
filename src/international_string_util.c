...
{
	return GetStringCenterAlignXOffsetWithLetterSpacing(
		fontId, str, totalWidth, 0 );
}
...
{
	return GetStringWidthDifference(
		       fontId, str, totalWidth, letterSpacing ) /
		2;
}

int GetStringWidthDifference(
	int fontId, const u8* str, int totalWidth, int letterSpacing )
{
...
int Intl_GetListMenuWidth( const struct ListMenuTemplate* listMenu )
{
	...
	{
		int width =
			GetStringWidth( listMenu->fontId, items[i].name, 0 );
		if( width > maxWidth )
			maxWidth = width;
	}
