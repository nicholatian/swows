...
// counting from Id 1, because 0 in sSpeciesToBackAnimSet is used for mons with
// no back animation
...
static void sub_817F3F0(
	struct Sprite* sprite, u16 index, s16 amplitudeX, s16 amplitudeY )
{
	s16 xAdder, yAdder;
...
static void Task_HandleMonAnimation( u8 taskId )
{
	...
	struct Sprite* sprite =
		(struct Sprite*)( u32 )( ( gTasks[taskId].tPtrLO << 0x10 ) |
...
static void SetAffineData(
	struct Sprite* sprite, s16 xScale, s16 yScale, u16 rotation )
{
	u8 matrixNum;
...
static void HandleSetAffineData(
	struct Sprite* sprite, s16 xScale, s16 yScale, u16 rotation )
{
	if( !sprite->data[1] )
	{
...
static void pokemonanimfunc_01( struct Sprite* sprite )
{
	...
	else
	{
		...
		HandleSetAffineData(
			sprite, sprite->data[4], sprite->data[5], 0 );
	}
...
static void pokemonanimfunc_02( struct Sprite* sprite )
{
	...
	else
	{
		...
		sprite->pos2.x =
			Sin( ( sprite->data[2] * 128 / 40 ) % 256, 6 ) * sign;
	}
...
static void sub_817F8FC( struct Sprite* sprite )
{
	...
	{
		sprite->pos2.x = Sin(
			( sprite->data[2] * 384 / sprite->data[0] ) % 256, 6 );
	}
...
static void sub_817F978( struct Sprite* sprite )
{
	...
	{
		sprite->pos2.y = -(
			Sin( ( sprite->data[2] * 384 / sprite->data[0] ) % 256,
				6 ) );
	}
...
static void sub_817F9F4( struct Sprite* sprite )
{
	...
	else
	{
		...
		case 1:
			sprite->pos2.y =
				-( Sin( counter % 128, sprite->data[0] * 2 ) );
			break;
		case 2:
		case 3:
			sprite->pos2.y =
				-( Sin( counter - 256, sprite->data[0] * 3 ) );
			break;
		}
	}
...
static void sub_817FF3C( struct Sprite* sprite )
{
	...
	{
		sprite->data[6] = ( 65536 / sUnknown_03001240[id].field_8 ) *
			sprite->data[2];
...
static void sub_817FFF0( struct Sprite* sprite )
{
	...
	else
	{
		...
		sprite->pos2.x =
			-( Cos( index, sUnknown_03001240[id].field_8 * 2 ) );
		sprite->pos2.y = Sin( index, sUnknown_03001240[id].field_8 ) +
			sUnknown_03001240[id].field_8;
	}
...
static void pokemonanimfunc_15( struct Sprite* sprite )
{
	...
	else
	{
		...
		BlendPalette(
			sprite->data[7], 0x10, sprite->data[6], RGB_BLACK );
	}
...
static void pokemonanimfunc_16( struct Sprite* sprite )
{
	...
	else
	{
		...
		if( sprite->data[1] == 0 )
			sprite->data[4] = ( Sin( index2, 40 ) - 256 ) +
				Sin( index1, 16 );
		else
			sprite->data[4] = ( 256 - Sin( index2, 40 ) ) -
				Sin( index1, 16 );
...
static void pokemonanimfunc_17( struct Sprite* sprite )
{
	...
	else
	{
		...
		sprite->data[5] =
			( 256 - Sin( index2, 40 ) ) - Sin( index1, 8 );
...
static void pokemonanimfunc_1A( struct Sprite* sprite )
{
	...
	else
	{
		...
		if( counter < 10 )
			HandleSetAffineData(
				sprite, 256, 256, counter / 2 * 512 );
		else if( counter >= 10 && counter <= 29 )
			sprite->pos2.x = -( Sin( index, 5 ) );
		else
			HandleSetAffineData( sprite,
				256,
				256,
				( 35 - counter ) / 2 * 1024 );
	}
...
static void sub_8180714( struct Sprite* sprite )
{
	...
	else
	{
		...
		HandleSetAffineData(
			sprite, 256 - Sin( sprite->data[4], 32 ), yScale, 0 );
		sprite->data[2]++;
		sprite->data[4] =
			( sprite->data[4] + 128 / sprite->data[0] ) & 0xFF;
	}
...
static void sub_8180828( struct Sprite* sprite )
{
	...
	else
	{
		...
		sprite->pos2.y = -( posY );
		HandleSetAffineData(
			sprite, Sin( sprite->data[4], 48 ) + 256, yScale, 0 );
		sprite->data[2]++;
...
static void sub_8180900( struct Sprite* sprite )
{
	...
	r9       = sUnknown_0860AD8E[sUnknown_03001240[structId].field_8]
			      [sprite->data[4]][0];
	r10 = sUnknown_0860AD8E[sUnknown_03001240[structId].field_8]
			       [sprite->data[4]][1] -
		r9;
	...
	else
	{
		...
		sprite->pos2.y = -( Sin( r7 * 128 /
				sUnknown_0860AD8E[arrId][sprite->data[4]][2],
			10 ) );
		sprite->pos2.x =
			( r10 * r7 /
				sUnknown_0860AD8E[arrId][sprite->data[4]]
						 [2] ) +
			r9;
...
static void pokemonanimfunc_21( struct Sprite* sprite )
{
	...
	else
	{
		sprite->data[6] = Sin( sprite->data[2], 12 );
		BlendPalette(
			sprite->data[7], 0x10, sprite->data[6], RGB_RED );
	}
...
static void pokemonanimfunc_22( struct Sprite* sprite )
{
	...
	else
	{
		sprite->data[6] = Sin( sprite->data[2], 12 );
		BlendPalette(
			sprite->data[7], 0x10, sprite->data[6], RGB_BLUE );
	}
...
static void pokemonanimfunc_23( struct Sprite* sprite )
{
	...
	else
	{
		sprite->data[6] = Sin( sprite->data[2], 12 );
		BlendPalette(
			sprite->data[7], 0x10, sprite->data[6], RGB_YELLOW );
	}
...
static void sub_8181214( struct Sprite* sprite )
{
	...
	else
	{
		...
		if( sprite->data[2] == 0 )
		{
			...
			HandleStartAffineAnim( sprite );
			sprite->data[7] =
				sUnknown_03001240[sprite->data[0]].field_2;
			sprite->data[3] = -1;
			sprite->data[4] = -1;
			sprite->data[5] = 0;
			sprite->data[6] = 0;
	...
	else
	{
		if( sprite->data[4] == 1 )
		{
			if( sUnknown_0860AA64[sprite->data[6]][0] != 0 )
				...
				BlendPalette(
					sprite->data[7], 0x10, 0, RGB_YELLOW );
...
static void sub_81814D4( struct Sprite* sprite )
{
	...
	{
		s16 index = ( sprite->data[2] * 256 ) /
			sUnknown_03001240[sprite->data[0]].field_8;
...
static void sub_8181708( struct Sprite* sprite )
{
	...
	sprite->data[7]--;
	sprite->pos2.x =
		Cos( sprite->data[7], sprite->data[6] ) + sprite->data[6];

	sprite->pos2.y = -( Sin(
		sprite->data[7], sprite->data[6] += 0 ) ); // dummy += 0 is
							   // needed to match

	HandleSetAffineData(
		sprite, 256, 256, ( sprite->data[7] - 128 ) << 8 );
...
static void sub_8181810( struct Sprite* sprite )
{
	...
	else
	{
		...
		else if( sprite->data[5] == 1 )
		{
			...
			if( sprite->data[4] == 128 )
			{
				if( sUnknown_03001240[sprite->data[0]]
						.field_4 > 1 )
				{
					sUnknown_03001240[sprite->data[0]]
						.field_4--;
					sUnknown_03001240[sprite->data[0]]
						.field_0 = 10;
					sprite->data[4]  = 0;
...
static void sub_8181ABC( struct Sprite* sprite )
{
	...
	else
	{
		...
		{
			s32 var =
				8 * sUnknown_03001240[sprite->data[0]].field_8;
...
static void sub_8181C2C( struct Sprite* sprite )
{
	...
	{
		register s32 var asm( "r4" ) =
			sUnknown_03001240[sprite->data[0]].field_8;

		sprite->pos2.x = ( var << 3 ) * ( counter % 128 ) / 128 -
			( sUnknown_03001240[sprite->data[0]].field_8 * 8 );
		sprite->pos2.y = -( Sin( counter % 128, 8 ) );
	}
...
static void sub_8182648( struct Sprite* sprite )
{
	...
	else
	{
		...
		sprite->pos2.y =
			-( sprite->data[6] * 4 ) - Sin( sprite->data[7], 8 );
		if( sprite->data[7] > 63 )
		{
			sign  = -1;
...
static void sub_8183140( struct Sprite* sprite )
{
	...
	{
		sprite->pos2.y =
			Sin( ( counter + 192 ) % 256, sprite->data[7] ) +
			sprite->data[7];
	}
...
static void sub_81832C8( struct Sprite* sprite )
{
	...
	else
	{
		...
		yScale    = 256 - Sin( index2, amplitude ) -
			Sin( index1, amplitude / 5 );
...
static void sub_8183418( struct Sprite* sprite )
{
	...
	else
	{
		...
		if( sprite->data[1] == 0 )
			xScale = -256 + Sin( index2, amplitude ) +
				Sin( index1, amplitude / 5 * 2 );
		else
			xScale = 256 - Sin( index2, amplitude ) -
				Sin( index1, amplitude / 5 * 2 );
...
static void pokemonanimfunc_5F( struct Sprite* sprite )
{
	...
	else
	{
		...
		sprite->pos2.x =
			Sin( ( sprite->data[2] * 128 / 40 ) % 256, 9 ) * sign;
	}
...
static void pokemonanimfunc_60( struct Sprite* sprite )
{
	...
	else
	{
		...
		sprite->pos2.x =
			Sin( ( sprite->data[2] * 128 / 40 ) % 256, 12 ) * sign;
	}
...
static void sub_81837DC( struct Sprite* sprite )
{
	...
	else
	{
		...
		if( sprite->data[1] == 0 )
			xScale = Sin( index2, amplitude ) +
				( Sin( index1, amplitude / 5 * 2 ) - 256 );
		else
			xScale = 256 - Sin( index1, amplitude / 5 * 2 ) -
				Sin( index2, amplitude );

		yScale = 256 - Sin( index1, amplitude / 5 ) -
			Sin( index2, amplitude );
		SetAffineData( sprite, xScale, yScale, 0 );
		sprite->data[5]++;
	}
...
static void sub_8183B4C( struct Sprite* sprite )
{
	...
	if( sUnknown_0860ADBE[sprite->data[3]][2] / sprite->data[5] ==
		sprite->data[2] )
	{
		sprite->data[3]++;
		sprite->data[2] = 0;
	...
	else
	{
		...
		sprite->pos2.x +=
			( sUnknown_0860ADBE[sprite->data[3]][0] * amplitude );
		sprite->pos2.y += ( sUnknown_0860ADBE[sprite->data[3]][1] *
			sprite->data[5] ); // what's the point of the var if
...
static void sub_8183FA8( struct Sprite* sprite )
{
	...
	else
	{
		...
		sprite->pos2.y = Sin(
			( sprite->data[7] + 192 ) % 256, sprite->data[4] );
		if( sprite->pos2.y > 0 )
			sprite->pos2.y *= -1;
...
static void sub_81840C4( struct Sprite* sprite )
{
	...
	else
	{
		...
		{
			sprite->pos2.y = Sin( 96, 6 ) -
				Sin( ( sprite->data[7] - 96 ) * 2, 4 );
		}
		else
		{
...
{
	u16 index = Sin(
		( sprite->data[2] * 128 ) / sprite->data[7], sprite->data[5] );
...
static void sub_8184468( struct Sprite* sprite )
{
	...
	else
	{
		...
		s32 posY;
		s16 index =
			( u16 )( sprite->data[2] % sprite->data[6] * 256 ) /
			sprite->data[6] % 256;
		if( sprite->data[2] % 2 == 0 )
		{
			sprite->data[4] = Sin( index, 32 ) + 256;
...
{
	const struct YellowBlendStruct* array =
		sUnknown_0860AE7C[sprite->data[3]];
	sub_8184770( sprite );
...
static void pokemonanimfunc_8E( struct Sprite* sprite )
{
	...
	if( sprite->data[2] >=
		( 128 - sprite->data[0] * sprite->data[4] ) / 2 )
		sub_8184934( sprite );
