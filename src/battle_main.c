...
static u8 CreateNPCTrainerParty(
	struct Pokemon* party, u16 trainerNum, bool8 firstTrainer );
...
EWRAM_DATA struct UnknownPokemonStruct4 gUnknown_02022FF8[3] = {0}; // what is
								    // it used
								    // for?
EWRAM_DATA struct UnknownPokemonStruct4* gUnknown_02023058 = NULL; // what is
								   // it used
								   // for?
...
EWRAM_DATA const u8* gPalaceSelectionBattleScripts[MAX_BATTLERS_COUNT] = {
	NULL};
...
static const u16 gUnknown_0831ACC4[]      = {
        0xfff0, 0, 0x0400, 0, 0, 0, 0x3c00, 0, 0x7ffe, 1, 0, 0};
...
static void CB2_InitBattleInternal( void )
{
		    ...
		    ( BATTLE_TYPE_LINK | BATTLE_TYPE_RECORDED ) ) )
	{
		CreateNPCTrainerParty(
			&gEnemyParty[0], gTrainerBattleOpponent_A, TRUE );
		if( gBattleTypeFlags & BATTLE_TYPE_TWO_OPPONENTS )
			CreateNPCTrainerParty( &gEnemyParty[3],
...
static void SetPlayerBerryDataInBattleStruct( void )
{
	...
	struct BattleEnigmaBerry* battleBerry =
		&battleStruct->battleEnigmaBerry;

	if( IsEnigmaBerryValid( ) == TRUE )
	{
		for( i = 0; i < BERRY_NAME_LENGTH; i++ )
			battleBerry->name[i] =
				gSaveBlock1Ptr->enigmaBerry.berry.name[i];
		battleBerry->name[i] = EOS;

		for( i = 0; i < BERRY_ITEM_EFFECT_COUNT; i++ )
			battleBerry->itemEffect[i] =
				gSaveBlock1Ptr->enigmaBerry.itemEffect[i];

		battleBerry->holdEffect =
			gSaveBlock1Ptr->enigmaBerry.holdEffect;
		battleBerry->holdEffectParam =
...
static void SetAllPlayersBerryData( void )
{
	...
	if( !( gBattleTypeFlags & BATTLE_TYPE_LINK ) )
	{
		if( IsEnigmaBerryValid( ) == TRUE )
		{
			for( i = 0; i < BERRY_NAME_LENGTH; i++ )
			{
				gEnigmaBerries[0].name[i] =
					gSaveBlock1Ptr->enigmaBerry.berry
						.name[i];
				gEnigmaBerries[2].name[i] =
					gSaveBlock1Ptr->enigmaBerry.berry
						.name[i];
			}
			...
			gEnigmaBerries[0].holdEffect =
				gSaveBlock1Ptr->enigmaBerry.holdEffect;
			gEnigmaBerries[2].holdEffect =
				gSaveBlock1Ptr->enigmaBerry.holdEffect;
		...
		{
			const struct Berry* berryData = GetBerryInfo(
	...
	else
	{
		...
		if( gBattleTypeFlags & BATTLE_TYPE_MULTI )
		{
			...
			{
				src       = (struct
                                        BattleEnigmaBerry*)( gBlockRecvBuffer
                                                                     [i] +
                                        2 );
				battlerId = gLinkPlayers[i].id;

				for( j = 0; j < BERRY_NAME_LENGTH; j++ )
					gEnigmaBerries[battlerId].name[j] =
						src->name[j];
				gEnigmaBerries[battlerId].name[j] = EOS;

				for( j = 0; j < BERRY_ITEM_EFFECT_COUNT; j++ )
					gEnigmaBerries[battlerId]
						.itemEffect[j] =
						src->itemEffect[j];

				gEnigmaBerries[battlerId].holdEffect =
					src->holdEffect;
				gEnigmaBerries[battlerId].holdEffectParam =
					src->holdEffectParam;
			}
		}
		else
		{
			for( i = 0; i < 2; i++ )
			{
				...
				for( j = 0; j < BERRY_NAME_LENGTH; j++ )
				{
					gEnigmaBerries[i].name[j] =
						src->name[j];
					gEnigmaBerries[i + 2].name[j] =
						src->name[j];
				}
				...
				for( j = 0; j < BERRY_ITEM_EFFECT_COUNT; j++ )
				{
					gEnigmaBerries[i].itemEffect[j] =
						src->itemEffect[j];
					gEnigmaBerries[i + 2].itemEffect[j] =
						src->itemEffect[j];
				}

				gEnigmaBerries[i].holdEffect = src->holdEffect;
				gEnigmaBerries[i + 2].holdEffect =
					src->holdEffect;
				gEnigmaBerries[i].holdEffectParam =
					src->holdEffectParam;
				gEnigmaBerries[i + 2].holdEffectParam =
					src->holdEffectParam;
			}
		}
	}
...
static void sub_8036EB8( u8 arg0, u8 arg1 )
{
	...
	if( gBlockRecvBuffer[0][0] == 256 )
	{
		if( arg1 == 0 )
			gBattleTypeFlags |=
				BATTLE_TYPE_IS_MASTER | BATTLE_TYPE_TRAINER;
		else
...
static void CB2_HandleStartBattle( void )
{
			...
			if( gReceivedRemoteLinkPlayers != 0 )
			{
				if( IsLinkTaskFinished( ) )
				{
					...
					*( &gBattleStruct->field_181 ) = 3;
					...
					if( gTrainerBattleOpponent_A ==
						TRAINER_OPPONENT_C00 )
					{
						gLinkPlayers[0].id = 0;
						gLinkPlayers[1].id = 1;
					}
	...
	case 2:
		if( ( GetBlockReceivedStatus( ) & 3 ) == 3 )
		{
			...
			gTasks[taskId].data[3] = gBattleStruct->field_182 |
				( gBattleStruct->field_183 << 8 );
	...
	case 1:
		if( gBattleTypeFlags & BATTLE_TYPE_LINK )
		{
			if( gReceivedRemoteLinkPlayers != 0 )
			{
				...
				gLinkPlayers[2].id = 1;
				gLinkPlayers[3].id = 3;
				GetFrontierTrainerName( gLinkPlayers[2].name,
					gTrainerBattleOpponent_A );
				GetFrontierTrainerName( gLinkPlayers[3].name,
					gTrainerBattleOpponent_B );
				GetBattleTowerTrainerLanguage(
					&language, gTrainerBattleOpponent_A );
				gLinkPlayers[2].language = language;
				GetBattleTowerTrainerLanguage(
					&language, gTrainerBattleOpponent_B );
...
static void sub_80379F8( u8 arrayIdPlus )
{
	...
	for( i = 0; i < 3; i++ )
	{
		gUnknown_02022FF8[i].species = GetMonData(
			&gPlayerParty[arrayIdPlus + i], MON_DATA_SPECIES );
		gUnknown_02022FF8[i].heldItem = GetMonData(
			&gPlayerParty[arrayIdPlus + i], MON_DATA_HELD_ITEM );
		GetMonData( &gPlayerParty[arrayIdPlus + i],
			MON_DATA_NICKNAME,
			gUnknown_02022FF8[i].nickname );
		gUnknown_02022FF8[i].level = GetMonData(
			&gPlayerParty[arrayIdPlus + i], MON_DATA_LEVEL );
		gUnknown_02022FF8[i].hp = GetMonData(
			&gPlayerParty[arrayIdPlus + i], MON_DATA_HP );
		gUnknown_02022FF8[i].maxhp = GetMonData(
			&gPlayerParty[arrayIdPlus + i], MON_DATA_MAX_HP );
		gUnknown_02022FF8[i].status = GetMonData(
			&gPlayerParty[arrayIdPlus + i], MON_DATA_STATUS );
		gUnknown_02022FF8[i].personality = GetMonData(
			&gPlayerParty[arrayIdPlus + i], MON_DATA_PERSONALITY );
		gUnknown_02022FF8[i].gender =
			GetMonGender( &gPlayerParty[arrayIdPlus + i] );
		StripExtCtrlCodes( gUnknown_02022FF8[i].nickname );
		if( GetMonData( &gPlayerParty[arrayIdPlus + i],
			    MON_DATA_LANGUAGE ) != LANGUAGE_JAPANESE )
			PadNameString(
				gUnknown_02022FF8[i].nickname, CHAR_SPACE );
	}
	memcpy( gUnknown_02023058,
		gUnknown_02022FF8,
		sizeof( gUnknown_02022FF8 ) );
}
...
static void CB2_PreInitMultiBattle( void )
{
	...
	case 0:
		...
		{
			gUnknown_02023058 = Alloc(
				sizeof( struct UnknownPokemonStruct4 ) * 3 );
			sub_80379F8( 0 );
			SendBlock( bitmask_all_link_players_but_self( ),
				gUnknown_02023058,
				sizeof( struct UnknownPokemonStruct4 ) * 3 );
			...
			for( i = 0; i < numPlayers; i++ )
			{
				...
				if( numPlayers == 4 )
				{
					if( ( !( gLinkPlayers[i].id & 1 ) &&
						    !( gLinkPlayers[playerMultiplierId]
								    .id &
							    1 ) ) ||
						( gLinkPlayers[i].id & 1 &&
							gLinkPlayers[playerMultiplierId]
								...
								1 ) )
					{
						memcpy( gUnknown_02022FF8,
							gBlockRecvBuffer[i],
							sizeof( struct
								UnknownPokemonStruct4 ) *
								3 );
					}
				}
				else
...
static void CB2_PreInitIngamePlayerPartnerBattle( void )
{
	...
	case 0:
		gUnknown_02023058 =
			Alloc( sizeof( struct UnknownPokemonStruct4 ) * 3 );
		sub_80379F8( 3 );
		gBattleCommunication[MULTIUSE_STATE]++;
		*savedCallback        = gMain.savedCallback;
		*savedBattleTypeFlags = gBattleTypeFlags;
...
static void CB2_HandleStartMultiBattle( void )
{
			...
			for( id = 0; id < MAX_LINK_PLAYERS; id++ )
			{
				...
				else
				{
					if( ( !( gLinkPlayers[id].id & 1 ) &&
						    !( gLinkPlayers[playerMultiplayerId]
								    .id &
							    1 ) ) ||
						( ( gLinkPlayers[id].id &
							  1 ) &&
	...
	case 7:
		...
		{
			for( id = 0; id < 4 &&
				( gLinkPlayers[id].version & 0xFF ) == 3;
				id++ )
				;
	...
	case 8:
		if( IsLinkTaskFinished( ) )
		{
			...
			ptr[1]   = gRecordedBattleRngSeed; // UB: overwrites
							 // berry data
			SendBlock(
				bitmask_all_link_players_but_self( ), ptr, 8 );
			gBattleCommunication[MULTIUSE_STATE]++;
		}
		break;
	case 9:
...
void BattleMainCB2( void )
{
	...
	if( gMain.heldKeys & B_BUTTON &&
		gBattleTypeFlags & BATTLE_TYPE_RECORDED && sub_8186450( ) )
	{
	...
	struct Pokemon* party, u16 trainerNum, bool8 firstTrainer )
{
				...
				BATTLE_TYPE_TRAINER_HILL ) ) )
	{
		...
		for( i = 0; i < monsCount; i++ )
		{
			...
			else if( gTrainers[trainerNum].encounterMusic_gender &
				0x80 )
			...
			for( j = 0;
				gTrainers[trainerNum].trainerName[j] != EOS;
				j++ )
				nameHash +=
					gTrainers[trainerNum].trainerName[j];
			...
			case 0: {
				...
				for( j = 0; gSpeciesNames[partyData[i].species]
							 [j] != EOS;
					j++ )
					nameHash += gSpeciesNames
						[partyData[i].species][j];
			...
			case F_TRAINER_PARTY_CUSTOM_MOVESET: {
				const struct TrainerMonNoItemCustomMoves*
					partyData = gTrainers[trainerNum]
							    .party
							    .NoItemCustomMoves;
...
void VBlankCB_Battle( void )
{
	...
	if( !( gBattleTypeFlags &
		    ( BATTLE_TYPE_LINK | BATTLE_TYPE_FRONTIER |
...
static void sub_8038F34( void )
{
	...
	case 2:
		if( !gPaletteFade.active )
		{
			...
			if( !gSaveBlock2Ptr->frontier.field_CA9_b &&
				i == monsCount )
			{
	...
	case 7:
		if( !IsTextPrinterActive( 0 ) )
		{
			if( gMain.field_439_x4 )
			{
				if( IsLinkTaskFinished( ) == TRUE )
				{
					BeginNormalPaletteFade( 0xFFFFFFFF,
						0,
						0,
						0x10,
						RGB_BLACK );
...
static void TryCorrectShedinjaLanguage( struct Pokemon* mon )
{
		...
		GetMonData( mon, MON_DATA_LANGUAGE ) != language )
	{
		...
		if( StringCompareWithoutExtCtrlCodes(
			    nickname, sText_ShedinjaJpnName ) == 0 )
...
static void SpriteCb_WildMonShowHealthbox( struct Sprite* sprite )
{
	if( sprite->animEnded )
	{
		...
		SetHealthboxSpriteVisible(
			gHealthboxSpriteIds[sprite->sBattler] );
...
static void SpriteCb_WildMonAnimate( struct Sprite* sprite )
{
	if( !gPaletteFade.active )
	{
		BattleAnimateFrontSprite(
			sprite, sprite->sSpeciesId, FALSE, 1 );
	}
}
...
void SpriteCB_FaintOpponentMon( struct Sprite* sprite )
{
	...
	if( gBattleSpritesDataPtr->battlerData[battler].transformSpecies != 0 )
		species = gBattleSpritesDataPtr->battlerData[battler]
				  .transformSpecies;
	else
	...
	if( species == SPECIES_UNOWN )
	{
		...
		u16 unownForm =
			( ( ( ( personalityValue & 0x3000000 ) >> 18 ) |
				  ( ( personalityValue & 0x30000 ) >> 12 ) |
				  ( ( personalityValue & 0x300 ) >> 6 ) |
				  ( personalityValue & 3 ) ) %
				0x1C );
		u16 unownSpecies;
		...
		if( unownForm == 0 )
			...
			unownSpecies = NUM_SPECIES +
				unownForm; // Use one of the other
					   // Unown letters.
	...
	{
		yOffset = gCastformFrontSpriteCoords[gBattleMonForms[battler]]
				  .y_offset;
	}
...
static void SpriteCB_AnimFaintOpponent( struct Sprite* sprite )
{
	...
	if( --sprite->data[4] == 0 )
	{
		...
		else // Erase bottom part of the sprite to create a smooth
		     // illusion of mon falling down.
		{
			u8* dst = (u8*)gMonSpritesGfxPtr
					  ->sprites[GetBattlerPosition(
						  sprite->sBattler )] +
			...
			StartSpriteAnim(
				sprite, gBattleMonForms[sprite->sBattler] );
		}
	}
}
...
void sub_8039B58( struct Sprite* sprite )
{
	if( sprite->affineAnimEnded )
	{
		...
		BattleAnimateFrontSprite(
			sprite, sprite->sSpeciesId, TRUE, 1 );
	}
}
...
void DoBounceEffect( u8 battler, u8 which, s8 delta, s8 amplitude )
{
	...
	default:
		if( gBattleSpritesDataPtr->healthBoxesData[battler]
				.healthboxIsBouncing )
			return;
		break;
	case BOUNCE_MON:
	...
	invisibleSpriteId =
		CreateInvisibleSpriteWithCallback( SpriteCB_BounceEffect );
	if( which == BOUNCE_HEALTHBOX )
	{
		bouncerSpriteId = gHealthboxSpriteIds[battler];
		gBattleSpritesDataPtr->healthBoxesData[battler]
			.healthboxBounceSpriteId = invisibleSpriteId;
		gBattleSpritesDataPtr->healthBoxesData[battler]
			.healthboxIsBouncing          = 1;
	...
	else
	{
		...
		gBattleSpritesDataPtr->healthBoxesData[battler]
			.battlerBounceSpriteId = invisibleSpriteId;
		gBattleSpritesDataPtr->healthBoxesData[battler]
			.battlerIsBouncing            = 1;
...
void EndBounceEffect( u8 battler, u8 which )
{
	...
	if( which == BOUNCE_HEALTHBOX )
	{
		...
		bouncerSpriteId = gSprites[gBattleSpritesDataPtr
						   ->healthBoxesData[battler]
						   .healthboxBounceSpriteId]
					  .sBouncerSpriteId;
		DestroySprite( &gSprites[gBattleSpritesDataPtr
						 ->healthBoxesData[battler]
						 .healthboxBounceSpriteId] );
		gBattleSpritesDataPtr->healthBoxesData[battler]
			.healthboxIsBouncing = 0;
	}
	else
	{
		if( !gBattleSpritesDataPtr->healthBoxesData[battler]
...
static void SpriteCB_BounceEffect( struct Sprite* sprite )
{
	...
	gSprites[bouncerSpriteId].pos2.y =
...
void sub_8039E9C( struct Sprite* sprite )
{
	if( sprite->animDelayCounter == 0 )
		sprite->centerToCornerVecX =
			gUnknown_0831ACE0[sprite->animCmdIndex];
}
...
static void BattleStartClearSetData( void )
{
	...
	*( &gBattleStruct->safariCatchFactor ) =
		gBaseStats[GetMonData( &gEnemyParty[0], MON_DATA_SPECIES )]
			.catchRate *
		100 / 1275;
	...
	for( i = 0; i < 8; i++ )
	{
		...
		*( i + 0 * 8 + (u8*)( gBattleStruct->lastTakenMoveFrom ) +
			0 )                                = 0;
		*( i + 1 * 8 + (u8*)( gBattleStruct->lastTakenMoveFrom ) +
			0 )                                = 0;
		*( i + 2 * 8 + (u8*)( gBattleStruct->lastTakenMoveFrom ) +
			0 )                                = 0;
		*( i + 3 * 8 + (u8*)( gBattleStruct->lastTakenMoveFrom ) +
			0 )                                = 0;
	}
...
{
	struct DisableStruct disableStructCopy =
	...
	if( gBattleMoves[gCurrentMove].effect != EFFECT_BATON_PASS )
	{
		...
		for( i = 0; i < gBattlersCount; i++ )
		{
			if( ( gBattleMons[i].status2 &
				...
				gDisableStructs[i].battlerPreventingEscape ==
					gActiveBattler )
				gBattleMons[i].status2 &=
					~STATUS2_ESCAPE_PREVENTION;
			if( ( gStatuses3[i] & STATUS3_ALWAYS_HITS ) &&
				gDisableStructs[i].battlerWithSureHit ==
					gActiveBattler )
			{
	...
	{
		if( gBattleMons[i].status2 &
			STATUS2_INFATUATED_WITH( gActiveBattler ) )
			gBattleMons[i].status2 &=
				~( STATUS2_INFATUATED_WITH( gActiveBattler ) );
	...
	*( 0 * 2 + gActiveBattler * 8 +
		(u8*)( gBattleStruct->lastTakenMoveFrom ) + 0 )    = 0;
	*( 0 * 2 + gActiveBattler * 8 +
		(u8*)( gBattleStruct->lastTakenMoveFrom ) + 1 )    = 0;
	*( 1 * 2 + gActiveBattler * 8 +
		(u8*)( gBattleStruct->lastTakenMoveFrom ) + 0 )    = 0;
	*( 1 * 2 + gActiveBattler * 8 +
		(u8*)( gBattleStruct->lastTakenMoveFrom ) + 1 )    = 0;
	*( 2 * 2 + gActiveBattler * 8 +
		(u8*)( gBattleStruct->lastTakenMoveFrom ) + 0 )    = 0;
	*( 2 * 2 + gActiveBattler * 8 +
		(u8*)( gBattleStruct->lastTakenMoveFrom ) + 1 )    = 0;
	*( 3 * 2 + gActiveBattler * 8 +
		(u8*)( gBattleStruct->lastTakenMoveFrom ) + 0 )    = 0;
	*( 3 * 2 + gActiveBattler * 8 +
		(u8*)( gBattleStruct->lastTakenMoveFrom ) + 1 )    = 0;
...
static void BattleIntroDrawTrainersOrMonsSprites( void )
{
		...
		gActiveBattler++ )
	{
		...
		else
		{
			...
			gBattleMons[gActiveBattler].type1 =
				gBaseStats[gBattleMons[gActiveBattler].species]
					.type1;
			gBattleMons[gActiveBattler].type2 =
				gBaseStats[gBattleMons[gActiveBattler].species]
					.type2;
			...
			hpOnSwitchout =
				&gBattleStruct->hpOnSwitchout[GetBattlerSide(
					gActiveBattler )];
		...
		if( GetBattlerPosition( gActiveBattler ) ==
...
static void BattleIntroDrawPartySummaryScreens( void )
{
	...
	if( gBattleTypeFlags & BATTLE_TYPE_TRAINER )
	{
		for( i = 0; i < PARTY_SIZE; i++ )
		{
			if( GetMonData( &gEnemyParty[i], MON_DATA_SPECIES2 ) ==
					SPECIES_NONE ||
				GetMonData( &gEnemyParty[i],
			...
			{
				hpStatus[i].hp = GetMonData(
					&gEnemyParty[i], MON_DATA_HP );
				hpStatus[i].status = GetMonData(
					&gEnemyParty[i], MON_DATA_STATUS );
			}
		}
		gActiveBattler =
		...
		for( i = 0; i < PARTY_SIZE; i++ )
		{
			if( GetMonData( &gPlayerParty[i],
				    MON_DATA_SPECIES2 ) == SPECIES_NONE ||
	...
	{
		// The struct gets set here, but nothing is ever done with it
		// since wild battles don't show the party summary. Still,
		// there's no point in having dead code.
...
static void BattleIntroPrintOpponentSendsOut( void )
{
	...
	PrepareStringBattle(
		STRINGID_INTROSENDOUT, GetBattlerAtPosition( position ) );
...
static void TryDoEventsBeforeFirstTurn( void )
{
	...
	if( gBattleStruct->switchInAbilitiesCounter == 0 )
	{
		...
		for( i = 0; i < gBattlersCount - 1; i++ )
		{
			...
			{
				if( GetWhoStrikesFirst( gBattlerByTurnOrder[i],
					    gBattlerByTurnOrder[j],
					    TRUE ) != 0 )
					SwapTurnOrder( i, j );
			}
		}
	...
	if( !gBattleStruct->overworldWeatherDone &&
		AbilityBattleEffects(
			0, 0, 0, ABILITYEFFECT_SWITCH_IN_WEATHER, 0 ) != 0 )
	{
		gBattleStruct->overworldWeatherDone = TRUE;
		return;
	}
	// Check all switch in abilities happening from the fastest mon to
	// slowest.
	...
	if( AbilityBattleEffects( ABILITYEFFECT_TRACE, 0, 0, 0, 0 ) != 0 )
		return;
	// Check all switch in items having effect from the fastest mon to
	// slowest.
...
void sub_803BDA0( u8 battler )
{
	...
	for( i = 0; i < 3; i++ )
		gUnknown_0203CF00[i] = *(
			battler * 3 + i + (u8*)( gBattleStruct->field_60 ) );

	r4 = pokemon_order_func( gBattlerPartyIndexes[battler] );
	r1 = pokemon_order_func(
...
static void HandleTurnActionSelectionState( void )
{
		...
		gActiveBattler++ )
	{
		...
		{
		case STATE_TURN_START_RECORD: // Recorded battle related action
					      // on start of every turn.
			...
			*( gBattleStruct->monToSwitchIntoId +
			...
			if( gBattleTypeFlags & BATTLE_TYPE_MULTI ||
				...
				gBattleStruct->field_91 &
					gBitTable[GetBattlerAtPosition(
						BATTLE_PARTNER(
							position ) )] ||
				gBattleCommunication[GetBattlerAtPosition(
					BATTLE_PARTNER( position ) )] ==
					STATE_WAIT_ACTION_CONFIRMED )
			{
				if( gBattleStruct->field_91 &
		...
		case STATE_WAIT_ACTION_CHOSEN: // Try to perform an action.
						    ...
						    << 0xC ) ) ) )
			{
				...
				case B_ACTION_USE_MOVE:
					if( AreAllMovesUnusable( ) )
					{
						gBattleCommunication
							[gActiveBattler] =
								STATE_SELECTION_SCRIPT;
						*( gBattleStruct->selectionScriptFinished +
							gActiveBattler ) =
							FALSE;
				...
				case B_ACTION_SAFARI_POKEBLOCK:
					BtlController_EmitChooseItem( 0,
						gBattleStruct->field_60
							[gActiveBattler] );
					MarkBattlerForControllerExec(
						gActiveBattler );
				...
				case B_ACTION_CANCEL_PARTNER:
					...
					{
						RecordedBattle_ClearBattlerAction(
							GetBattlerAtPosition( BATTLE_PARTNER(
								GetBattlerPosition(
									gActiveBattler ) ) ),
							2 );
					}
			...
			if( !( gBattleControllerExecFlags &
				    ( ( gBitTable[gActiveBattler] ) |
					    ...
					    ( gBitTable[gActiveBattler]
						    << 0xC ) ) ) )
			{
				switch( gChosenActionByBattler
						[gActiveBattler] )
				{
				case B_ACTION_USE_MOVE:
					switch( gBattleBufferB[gActiveBattler]
							      [1] )
					{
					case 3:
					case 4:
					...
					default:
						sub_818603C( 2 );
						if( ( gBattleBufferB[gActiveBattler]
								    [2] |
							    ( gBattleBufferB[gActiveBattler]
									    [3]
								    << 8 ) ) ==
							0xFFFF )
						{
		...
		case STATE_SELECTION_SCRIPT:
			if( *( gBattleStruct->selectionScriptFinished +
				    gActiveBattler ) )
			{
				gBattleCommunication[gActiveBattler] = *(
					gBattleStruct->stateIdAfterSelScript +
...
static void sub_803CDF8( void )
{
	*( gBattleStruct->monToSwitchIntoId + gActiveBattler ) =
	...
	{
		*( gActiveBattler * 3 + (u8*)( gBattleStruct->field_60 ) +
			0 ) &= 0xF;
		*( gActiveBattler * 3 + (u8*)( gBattleStruct->field_60 ) +
			0 ) |= ( gBattleBufferB[gActiveBattler][2] & 0xF0 );
		*( gActiveBattler * 3 + (u8*)( gBattleStruct->field_60 ) +
			1 ) = gBattleBufferB[gActiveBattler][3];

		*( ( gActiveBattler ^ BIT_FLANK ) * 3 +
			(u8*)( gBattleStruct->field_60 ) + 0 ) &= ( 0xF0 );
		*( ( gActiveBattler ^ BIT_FLANK ) * 3 +
...
u8 GetWhoStrikesFirst( u8 battler1, u8 battler2, bool8 ignoreChosenMoves )
{
	...
	speedBattler1 =
		...
		( gStatStageRatios[gBattleMons[battler1]
					   .statStages[STAT_SPEED]][0] ) /
		( gStatStageRatios[gBattleMons[battler1]
					   .statStages[STAT_SPEED]][1] );
	...
	{
		holdEffect =
			ItemId_GetHoldEffect( gBattleMons[battler1].item );
		holdEffectParam = ItemId_GetHoldEffectParam(
			gBattleMons[battler1].item );
	}
	...
	speedBattler2 =
		...
		( gStatStageRatios[gBattleMons[battler2]
					   .statStages[STAT_SPEED]][0] ) /
		( gStatStageRatios[gBattleMons[battler2]
					   .statStages[STAT_SPEED]][1] );
	...
	{
		holdEffect =
			ItemId_GetHoldEffect( gBattleMons[battler2].item );
		holdEffectParam = ItemId_GetHoldEffectParam(
			gBattleMons[battler2].item );
	}
	...
	else
	{
		if( gChosenActionByBattler[battler1] == B_ACTION_USE_MOVE )
		{
			if( gProtectStructs[battler1].noValidMoves )
				...
				moveBattler1 = gBattleMons[battler1].moves[*(
					gBattleStruct->chosenMovePositions +
					battler1 )];
		}
		else
		...
		if( gChosenActionByBattler[battler2] == B_ACTION_USE_MOVE )
		{
			if( gProtectStructs[battler2].noValidMoves )
				...
				moveBattler2 = gBattleMons[battler2].moves[*(
					gBattleStruct->chosenMovePositions +
					battler2 )];
		}
		else
		...
		gBattleMoves[moveBattler2].priority != 0 )
	{
			...
			gBattleMoves[moveBattler2].priority )
		{
			...
			gBattleMoves[moveBattler2].priority )
			strikesFirst =
				1; // battler2's move has greater priority
	...
	else
	{
		if( gBattleTypeFlags & BATTLE_TYPE_LINK )
		{
			for( gActiveBattler = 0;
				...
				gActiveBattler++ )
			{
				if( gChosenActionByBattler[gActiveBattler] ==
					B_ACTION_RUN )
				{
					turnOrderId = 5;
					break;
		...
		else
		{
				...
				gActiveBattler++ )
			{
						...
						B_ACTION_SWITCH )
				{
					gActionsByTurnOrder[turnOrderId] =
						gChosenActionByBattler
							[gActiveBattler];
					gBattlerByTurnOrder[turnOrderId] =
						gActiveBattler;
					turnOrderId++;
				}
			}
			...
			for( i = 0; i < gBattlersCount - 1; i++ )
			{
				for( j = i + 1; j < gBattlersCount; j++ )
				{
					...
					if( gActionsByTurnOrder[i] !=
							B_ACTION_USE_ITEM &&
						gActionsByTurnOrder[j] !=
							B_ACTION_USE_ITEM &&
		...
		gActiveBattler++ )
	{
		...
		else
		{
			...
			if( gDisableStructs[gActiveBattler].rechargeTimer )
			{
				gDisableStructs[gActiveBattler]
					.rechargeTimer--;
				if( gDisableStructs[gActiveBattler]
						.rechargeTimer == 0 )
...
static void CheckFocusPunch_ClearVarsBeforeTurnStarts( void )
{
	if( !( gHitMarker & HITMARKER_RUN ) )
	{
		while( gBattleStruct->focusPunchBattlerId < gBattlersCount )
		{
			...
			gBattleStruct->focusPunchBattlerId++;
			if( gChosenMoveByBattler[gActiveBattler] ==
					MOVE_FOCUS_PUNCH &&
...
static void RunTurnActionsFunctions( void )
{
	...
	*( &gBattleStruct->savedTurnActionNumber ) = gCurrentTurnActionNumber;
	...
	if( gCurrentTurnActionNumber >=
		gBattlersCount ) // everyone did their
				 // actions, turn finished
	{
		gHitMarker &= ~( HITMARKER_x100000 );
	...
	else
	{
		if( gBattleStruct->savedTurnActionNumber !=
			gCurrentTurnActionNumber ) // action turn has been
						   // done, clear hitmarker
						   // bits for another
						   // battlerId
		{
			gHitMarker &= ~( HITMARKER_NO_ATTACKSTRING );
		...
		for( i = 0; i < PARTY_SIZE; i++ )
		{
			if( gLeveledUpInBattle & gBitTable[i] )
			{
				...
				species = GetEvolutionTargetSpecies(
					&gPlayerParty[i], 0, levelUpBits );
				if( species != SPECIES_NONE )
				{
					FreeAllWindowBuffers( );
					gBattleMainFunc =
						WaitForEvoSceneToFinish;
					EvolutionScene( &gPlayerParty[i],
						species,
						TRUE,
						i );
					return;
...
static void HandleAction_UseMove( void )
{
	...
	else if( gBattleMons[gBattlerAttacker].status2 &
			STATUS2_MULTIPLETURNS ||
	...
	// encore forces you to use the same move
		...
		gDisableStructs[gBattlerAttacker].encoredMove ==
			gBattleMons[gBattlerAttacker]
				.moves[gDisableStructs[gBattlerAttacker]
						.encoredMovePos] )
	{
		...
		gBattleMoves[gCurrentMove].type == TYPE_ELECTRIC )
	{
			...
			gActiveBattler++ )
		{
			if( side != GetBattlerSide( gActiveBattler ) &&
				*( gBattleStruct->moveTarget +
				...
				GetBattlerTurnOrderNum( gActiveBattler ) <
					var )
			{
		...
		if( var == 4 )
		{
			if( gBattleMoves[gChosenMove].target &
				MOVE_TARGET_RANDOM )
			{
			...
			{
				gBattlerTarget = *( gBattleStruct->moveTarget +
					gBattlerAttacker );
			}
		...
		gBattleMoves[gChosenMove].target & MOVE_TARGET_RANDOM )
	{
		...
		{
			gBattlerTarget = GetBattlerAtPosition(
				GetBattlerPosition( gBattlerTarget ) ^
				BIT_FLANK );
		}
	}
	else
		...
		gProtectStructs[gBattlerAttacker].palaceUnableToUseMove )
	{
		...
		else if( gPalaceSelectionBattleScripts[gBattlerAttacker] !=
			NULL )
		{
			gBattleCommunication[MULTISTRING_CHOOSER] = 4;
			gBattlescriptCurrInstr = gPalaceSelectionBattleScripts
				[gBattlerAttacker];
...
static void HandleAction_Switch( void )
{
	...
	PREPARE_MON_NICK_BUFFER( gBattleTextBuff1,
		gBattlerAttacker,
		*( gBattleStruct->field_58 + gBattlerAttacker ) )
...
static void HandleAction_UseItem( void )
{
	...
	{
		gBattlescriptCurrInstr =
			gBattlescriptsForBallThrow[gLastUsedItem];
	}
	...
	else
	{
		...
		switch( *( gBattleStruct->AI_itemType +
			( gBattlerAttacker >> 1 ) ) )
		{
		...
		case AI_ITEM_CURE_CONDITION:
				...
				1 )
			{
				if( *( gBattleStruct->AI_itemFlags +
					...
					0x3E )
					gBattleCommunication
						[MULTISTRING_CHOOSER] = 5;
			}
			else
			{
				while( !( *( gBattleStruct->AI_itemFlags +
						  gBattlerAttacker / 2 ) &
					1 ) )
				{
...
bool8 TryRunFromBattle( u8 battler )
{
	...
	else if( gBattleMons[battler].ability == ABILITY_RUN_AWAY )
	{
		if( InBattlePyramid( ) )
		{
			...
			speedVar          = ( gBattleMons[battler].speed *
                                           pyramidMultiplier ) /
					( gBattleMons[BATTLE_OPPOSITE(
							      battler )]
							.speed ) +
...
static void HandleAction_Run( void )
{
	...
	else
	{
		...
		{
			if( !TryRunFromBattle(
				    gBattlerAttacker ) ) // failed to run
							 // away
			{
...
static void HandleAction_ThrowPokeblock( void )
{
	...
	if( gBattleStruct->safariEscapeFactor > 1 )
	{
		if( gBattleStruct->safariEscapeFactor <
			...
			gBattleStruct
				->safariEscapeFactor -= sPkblToEscapeFactor
				[gBattleStruct->safariPkblThrowCounter]
				[gBattleCommunication[MULTISTRING_CHOOSER]];
	}
...
static void HandleAction_GoNear( void )
{
	...
	gBattleStruct->safariCatchFactor +=
		sGoNearCounterToCatchFactor[gBattleStruct
						    ->safariGoNearCounter];
	if( gBattleStruct->safariCatchFactor > 20 )
		gBattleStruct->safariCatchFactor = 20;

	gBattleStruct->safariEscapeFactor +=
		sGoNearCounterToEscapeFactor[gBattleStruct
						     ->safariGoNearCounter];
...
static void HandleAction_NothingIsFainted( void )
{
	...
	gHitMarker &= ~( HITMARKER_DESTINYBOND | HITMARKER_IGNORE_SUBSTITUTE |
		...
		HITMARKER_IGNORE_SAFEGUARD | HITMARKER_IGNORE_ON_AIR |
		HITMARKER_IGNORE_UNDERGROUND | HITMARKER_IGNORE_UNDERWATER |
		HITMARKER_x100000 | HITMARKER_OBEYS | HITMARKER_x10 |
		HITMARKER_SYNCHRONISE_EFFECT | HITMARKER_CHARGING |
		HITMARKER_x4000000 );
}
...
{
	*( gBattleStruct->monToSwitchIntoId +
		gBattlerByTurnOrder[gCurrentTurnActionNumber] ) = 6;
	gCurrentTurnActionNumber++;
	gCurrentActionFuncId = gActionsByTurnOrder[gCurrentTurnActionNumber];
	SpecialStatusesClear( );
	gHitMarker &= ~( HITMARKER_DESTINYBOND | HITMARKER_IGNORE_SUBSTITUTE |
