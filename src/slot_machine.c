...
struct SlotMachineEwramStruct
{
	...
	/*0x0C*/ s16 coins;
	/*0x0E*/ s16 payout;
	/*0x10*/ s16 netCoinLoss; // coins lost to machine (but never goes
				  // below 0)
	/*0x12*/ s16 bet;
...
static u8 sub_8105BF8( u8 templateIdx,
...
const struct WindowTemplate gUnknown_085A7434[] = {
	{0, 2, 15, 0x1B, 4, 15, 0x194}, DUMMY_WIN_TEMPLATE};

const struct WindowTemplate gUnknown_085A7444 = {0, 1, 3, 20, 13, 13, 1};
...
// also modifies data in sSlotMachine reel arrays to indicate how to get to the
// matching state
...
void ( *const gUnknown_083ECBA0[] )( struct Task* task, u8 taskId ) = {
	sub_8103C78, sub_8103CAC, sub_8103CC8};

const s16 gUnknown_083ECBAC[] = {5, 10, 15};

void ( *const gUnknown_083ECBB4[] )( struct Task* task ) = {
	nullsub_68, sub_810411C, sub_8104144, sub_81041AC};
...
void ( *const ReeltimeActions[] )( struct Task* task ) = {ReeltimeAction0,
	ReeltimeAction1,
	ReeltimeAction2,
	ReeltimeAction3,
	ReeltimeAction4,
...
static void sub_81019EC( void )
{
	...
	LoadWordFromTwoHalfwords(
		(u16*)( task->data + 1 ), (u32*)&sSlotMachine->prevMainCb );
}
...
static void SlotMachineSetup_0_0( void )
{
	...
	InitBgsFromTemplates(
		0, gUnknown_085A7424, ARRAY_COUNT( gUnknown_085A7424 ) );
...
static void SlotMachineSetup_0_1( void )
{
	...
	for( i = 0; i < NUM_REELS; i++ )
	{
		...
		sSlotMachine->reelTagOffsets[i] =
			gInitialReelPositions[i][sSlotMachine->luckyGame] %
			REEL_NUM_TAGS;
		sSlotMachine->reelPixelOffsets[i] =
			REEL_NUM_TAGS * REEL_TAG_HEIGHT -
			sSlotMachine->reelTagOffsets[i] * REEL_TAG_HEIGHT;
		sSlotMachine->reelPixelOffsets[i] %=
			REEL_NUM_TAGS * REEL_TAG_HEIGHT;
	}
...
static bool8 SlotAction_PrintYouDontHaveThreeCoins( struct Task* task )
{
	...
	AddTextPrinterParameterized(
		0, 1, gText_YouDontHaveThreeCoins, 0, 1, 0, 0 );
...
static bool8 SlotAction_CheckMatches( struct Task* task )
{
	...
	if( sSlotMachine->matchedSymbols )
	{
		...
		else if( sSlotMachine->matchedSymbols &
		...
		if( sSlotMachine->matchedSymbols &
			( ( 1 << SLOT_MACHINE_MATCHED_777_MIXED ) |
				...
				( 1 << SLOT_MACHINE_MATCHED_777_RED ) ) )
		{
			sSlotMachine->luckyFlags &=
				~( LUCKY_BIAS_777 | LUCKY_BIAS_MIXED_777 );
			if( sSlotMachine->matchedSymbols &
				( ( 1 << SLOT_MACHINE_MATCHED_777_BLUE ) |
					( 1 << SLOT_MACHINE_MATCHED_777_RED ) ) )
			{
				...
				if( sSlotMachine->matchedSymbols &
					( 1 << SLOT_MACHINE_MATCHED_777_BLUE ) )
					// this may be an error, but if you get
					// blue 777, the game becomes lucky
					sSlotMachine->luckyGame = TRUE;
			}
		}
		if( sSlotMachine->matchedSymbols &
				( 1 << SLOT_MACHINE_MATCHED_POWER ) &&
			sSlotMachine->pikaPower < 16 )
		{
...
static bool8 SlotAction_PrintMessage_9999Coins( struct Task* task )
{
	DrawDialogueFrame( 0, 0 );
	AddTextPrinterParameterized(
		0, 1, gText_YouveGot9999Coins, 0, 1, 0, 0 );
	CopyWindowToVram( 0, 3 );
...
static bool8 SlotAction_PrintMessage_NoMoreCoins( struct Task* task )
{
	DrawDialogueFrame( 0, 0 );
	AddTextPrinterParameterized(
		0, 1, gText_YouveRunOutOfCoins, 0, 1, 0, 0 );
	CopyWindowToVram( 0, 3 );
...
static void DrawLuckyFlags( void )
{
	...
	if( sSlotMachine->fairRollsLeft == 0 )
	{
			    ...
			    ( LUCKY_BIAS_777 | LUCKY_BIAS_MIXED_777 ) ) )
		{
			...
			// if it's not a lucky round or you got reel time, roll
			// for the lower lucky flags
			attempts = AttemptsAtLuckyFlags_NotTop3( );
			if( attempts != 5 ) // if you found a lucky number
			{
				sSlotMachine->luckyFlags |=
					gLuckyFlagSettings_NotTop3[attempts];
			}
		}
	}
}
...
{
	u8 rval = Random( );
	if( gLuckyRoundProbabilities[sSlotMachine->machineId]
				    [sSlotMachine->bet - 1] > rval )
		return TRUE;
	return FALSE;
}
...
static u8 AttemptsAtLuckyFlags_Top3( void )
{
	...
	for( count = 0; count < 3; count++ )
	{
		...
		s16 value =
			gLuckyFlagProbabilities_Top3[count]
...
static u8 AttemptsAtLuckyFlags_NotTop3( void )
{
	...
	for( count = 0; count < 5; count++ )
	{
		...
		s16 value = gLuckyFlagProbabilities_NotTop3
			[count][sSlotMachine->machineId];
...
static u8 GetReelTimeProbability( u8 reelTimeDraw )
{
	if( sSlotMachine->luckyGame == FALSE )
		return gReeltimeProbabilities_UnluckyGame
			[reelTimeDraw][sSlotMachine->pikaPower];
	else
		return gReelTimeProbabilities_LuckyGame
			[reelTimeDraw][sSlotMachine->pikaPower];
}
...
static u16 SlowReelSpeed( void )
{
	...
	if( rval < value )
		return 4;
	rval  = Random( ) % 100;
	value = gReelIncrementTable[i][1] +
		gReelTimeBonusIncrementTable[sSlotMachine->fairRollsUsed];
	if( rval < value )
		return 2;
	return 8;
}
...
static u8 GetMatchFromSymbolsInRow( u8 c1, u8 c2, u8 c3 )
{
	...
	if( c1 == SLOT_MACHINE_TAG_7_RED && c2 == SLOT_MACHINE_TAG_7_RED &&
		c3 == SLOT_MACHINE_TAG_7_BLUE )
		return SLOT_MACHINE_MATCHED_777_MIXED;
	if( c1 == SLOT_MACHINE_TAG_7_BLUE && c2 == SLOT_MACHINE_TAG_7_BLUE &&
		c3 == SLOT_MACHINE_TAG_7_RED )
		return SLOT_MACHINE_MATCHED_777_MIXED;
...
// Returns the tag that is posOffset below the tag at the top of reelIndex's
// tape
static u8 GetNearbyTag_Quantized( u8 reelIndex, s16 posOffset )
{
	s16 tagIndex =
		( sSlotMachine->reelTagOffsets[reelIndex] + posOffset ) %
		REEL_NUM_TAGS;
	if( tagIndex < 0 )
...
// Calculates GetNearbyTag_Quantized as if the reel was snapped downwards into
// place
...
static void AdvanceSlotReel( u8 reelIndex, s16 value )
{
	...
	sSlotMachine->reelTagOffsets[reelIndex] =
		REEL_NUM_TAGS - sSlotMachine->reelPixelOffsets[reelIndex] / 24;
}
...
static void AdvanceReeltimeReel( s16 value )
{
	...
	sSlotMachine->reeltimePosition =
		6 - sSlotMachine->reeltimePixelOffset / 20;
}
...
static bool8 SlotReelAction_StayStill( struct Task* task ) { return FALSE; }
...
// As in previous generations, the slot machine often doesn't stop exactly when
// you press stop
static bool8 SlotReelAction_DecideWhereToStop( struct Task* task )
{
	task->data[0]++;
	// initialize data for that reel --> these will be changed if gBiasTags
	// can be lined up
	...
	if( sSlotMachine->fairRollsLeft == 0 &&
		( sSlotMachine->luckyFlags == 0 ||
			sSlotMachine->luckySpinsLeft == 0 ||
			!DecideReelTurns_BiasTag[task->data[15]]( ) ) )
	{
		sSlotMachine->luckySpinsLeft = 0;
...
static bool8 SlotReelAction_MoveToStop( struct Task* task )
{
	...
	if( reelPixelPos != 0 )
		reelPixelPos = AdvanceSlotReelToNextTag(
			task->data[15], sSlotMachine->reelIncrement );
	else if( sSlotMachine->reelExtraTurns[task->data[15]] )
	{
		sSlotMachine->reelExtraTurns[task->data[15]]--;
		AdvanceSlotReel( task->data[15], sSlotMachine->reelIncrement );
		reelPixelPos =
			sSlotMachine->reelPixelOffsets[task->data[15]] % 24;
	}
	if( reelPixelPos == 0 &&
		sSlotMachine->reelExtraTurns[task->data[15]] == 0 )
	{
...
static bool8 SlotReelAction_OscillatingStop( struct Task* task )
{
	sSlotMachine->reelPixelOffsetsWhileStopping[task->data[15]] =
		task->data[1];
...
static bool8 DecideReelTurns_BiasTag_Reel1( void )
{
	...
	return DecideReelTurns_BiasTag_Reel1_Bets[sSlotMachine->bet - 1](
		tag1, tag2 );
}
...
{
	if( GetNearbyTag( 0, 1 - offsetFromCenter ) ==
			SLOT_MACHINE_TAG_CHERRY ||
		GetNearbyTag( 0, 2 - offsetFromCenter ) ==
			SLOT_MACHINE_TAG_CHERRY ||
		GetNearbyTag( 0, 3 - offsetFromCenter ) ==
			SLOT_MACHINE_TAG_CHERRY )
		return TRUE;
	else
		return FALSE;
...
{
	if( sSlotMachine->luckyFlags &
		( LUCKY_BIAS_777 | LUCKY_BIAS_MIXED_777 | LUCKY_BIAS_CHERRY ) )
		return TRUE;
	else
		return FALSE;
}
...
static bool8 DecideReelTurns_BiasTag_Reel1_Bet2or3( u8 tag1, u8 tag2 )
{
	...
	for( i = 1; i < 5; i++ )
	{
		...
		// if biased or if in the next 4 turns there is a screen with
		// no cherries...
		if( biasedCopy || !AreCherriesOnScreen_Reel1( i ) )
		{
			...
			{
				//...and if it only took 1 turn and the lucky
				//tag could
				// also be the
				// bottom row of a screen with no cherries...
				if( i == 1 &&
					( biasedCopy ||
				...
				//...or if it isn't the last turn and the lucky
				//tag could
				// be in the
				// center row of a screen with no cherries...
				if( i < 4 &&
					( biasedCopy ||
						!AreCherriesOnScreen_Reel1(
							i + 1 ) ) )
...
static bool8 DecideReelTurns_BiasTag_Reel2_Bet1or2( void )
{
	...
	for( i = 0; i < 5; i++ )
	{
		...
		if( GetNearbyTag( 1, biasTagLocation_Reel1 - i ) ==
			sSlotMachine->biasTag )
		{
...
static bool8 DecideReelTurns_BiasTag_Reel2_Bet3( void )
{
	...
	{
		//...and if the biasTag is not in middle row of reel 1 and if
		//biasTag
		// appears in middle row of reel 2 in 2 or 3 turns...
			...
			sSlotMachine->reelExtraTurns[1] != 4 )
		{
			for( i = 0; i < 5; i++ )
			{
				//...and if the bias tag will appear in the
				//middle row
				// within 4
				// turns
				if( GetNearbyTag( 1, 2 - i ) ==
					sSlotMachine->biasTag )
				{
	...
	if( sSlotMachine->winnerRows[0] != 2 )
	{
		for( i = 0; i < 5; i++ )
		{
			//...and if the biasTag will appear in the center row
			//of reel 2
			// within
			// 4 turns
...
static bool8 DecideReelTurns_BiasTag_Reel3( void )
{
	...
	return DecideReelTurns_BiasTag_Reel3_Bets[sSlotMachine->bet - 1](
		biasTag );
}
...
static bool8 DecideReelTurns_BiasTag_Reel3_Bet1or2( u8 biasTag )
{
	...
	for( i = 0; i < 5; i++ )
	{
		// if the biasTag appears in the same row as in reel 2 within 4
		// turns
...
static void DecideReelTurns_NoBiasTag_Reel2_Bet1( void )
{
	...
	{
		u8 biasTag =
			GetNearbyTag( 0, 2 - sSlotMachine->reelExtraTurns[0] );
...
static void DecideReelTurns_NoBiasTag_Reel2_Bet2( void )
{
	if( sSlotMachine->winnerRows[0] != 0 &&
		sSlotMachine->luckyFlags & LUCKY_BIAS_777 )
	{
		u8 biasTag = GetNearbyTag( 0,
			sSlotMachine->winnerRows[0] -
				sSlotMachine->reelExtraTurns[0] );
		//...and if biasTag is one of the 7's...
		if( IsBiasTag777_SwitchColor( &biasTag ) )
		//...swap color of biasTag...
		{
			...
			for( i = 0; i < 5; i++ )
			{
				//...and if the biasTag appears in same row in
				//reel 2
				// within 4
				// turns
				if( biasTag ==
		...
		else
		{
			...
			//...swap the color of the 7...
			{
				...
				for( j = 1; j < 5; j++ )
				{
					if( biasTag ==
						GetNearbyTag( 1,
							sSlotMachine->winnerRows
									[0] -
								j ) )
					{
						if( sSlotMachine->winnerRows
								[0] == 1 )
						{
							if( j < 3 )
							{
...
{
	if( ( tag1 == SLOT_MACHINE_TAG_7_RED &&
		    tag2 == SLOT_MACHINE_TAG_7_BLUE ) ||
		( tag1 == SLOT_MACHINE_TAG_7_BLUE &&
			tag2 == SLOT_MACHINE_TAG_7_RED ) )
		return TRUE;
	else
		return FALSE;
}
...
static bool8 AreTagsMixed777( u8 tag1, u8 tag2, u8 tag3 )
{
	if( ( tag1 == SLOT_MACHINE_TAG_7_RED &&
		    tag2 == SLOT_MACHINE_TAG_7_BLUE &&
		    tag3 == SLOT_MACHINE_TAG_7_RED ) ||
		( tag1 == SLOT_MACHINE_TAG_7_BLUE &&
			tag2 == SLOT_MACHINE_TAG_7_RED &&
			tag3 == SLOT_MACHINE_TAG_7_BLUE ) )
		return TRUE;
	else
		return FALSE;
}
...
static void DecideReelTurns_NoBiasTag_Reel3_Bet1( void )
{
	...
	if( tag1 == tag2 )
	{
		...
		{
			u8 tag3;
			if( !( tag1 == ( tag3 = GetNearbyTag( 2, 2 - i ) ) ||
				    ( tag1 == SLOT_MACHINE_TAG_7_RED &&
					    tag3 == SLOT_MACHINE_TAG_7_BLUE ) ||
				    ( tag1 == SLOT_MACHINE_TAG_7_BLUE &&
					    tag3 == SLOT_MACHINE_TAG_7_RED ) ) )
...
static void DecideReelTurns_NoBiasTag_Reel3_Bet2( void )
{
	...
	// GUESS: spin until there's no possible match within 4 turns of you
	// stopping
	while( 1 )
	{
		s16 loopExit;
				...
				i -
					sSlotMachine->reelExtraTurns
						[0] ); // why
						       ...
						       // i
			tag2 = GetNearbyTag(
				1, i - sSlotMachine->reelExtraTurns[1] );
...
{
	sub_81065A8(
		gUnknown_083ECBAC[task->data[15]], 0x62, 0x63, 0x72, 0x73 );
	task->data[0]++;
}
...
{
	sub_81065A8(
		gUnknown_083ECBAC[task->data[15]], 0x42, 0x43, 0x52, 0x53 );
	DestroyTask( taskId );
}
...
static void sub_8103EE4( struct Sprite* sprite )
{
	...
	if( sprite->data[1] )
	{
		...
		MultiplyPaletteRGBComponents(
			gUnknown_083EDD30[sprite->data[0]],
			sprite->data[4],
			sprite->data[4],
			sprite->data[4] );
	}
...
static void ReeltimeAction3( struct Task* task )
{
	...
	StartSpriteAnimIfDifferent(
		gSprites + sSlotMachine->reelTimeSprite3F, sp0[r5] );
	// once speed goes below 256, go to next ReelTimeAction and keep the
	// speed level
...
static void ReeltimeAction4( struct Task* task )
{
	...
	if( ++task->data[5] >= 80 )
	{
		...
		sub_81057E8( 2 );
		StartSpriteAnimIfDifferent(
			gSprites + sSlotMachine->reelTimeSprite3F, 3 );
	}
}
...
static void ReelTimeAction_LandOnOutcome( struct Task* task )
{
	...
	{
		reeltimePixelOffset =
			AdvanceReeltimeReelToNextTag( task->data[4] >> 8 );
		task->data[4] = (u8)task->data[4] + 0x40;
	}
	...
	if( reeltimePixelOffset == 0 &&
		GetNearbyReelTimeTag( 1 ) == sSlotMachine->reelTimeDraw )
	{
...
static void ReeltimeAction8( struct Task* task )
{
	if( ++task->data[4] >= 60 )
	{
		...
		if( sSlotMachine->reelTimeDraw == 0 )
		{
			task->data[4] = 0xa0;
			StartSpriteAnimIfDifferent(
				gSprites + sSlotMachine->reelTimeSprite3F, 5 );
		...
		else
		{
			task->data[4] = 0xc0;
			StartSpriteAnimIfDifferent(
				gSprites + sSlotMachine->reelTimeSprite3F, 4 );
			gSprites[sSlotMachine->reelTimeSprite3F].animCmdIndex =
				0;
...
static void sub_8104A40( s16 a0, s16 a1 )
{
	...
	{
		LoadBgTilemap( 1,
			&gReelTimeWindowTilemap[a1 + ( i - 4 ) * 20],
			2,
			32 * i + a0 );
	}
}
...
static void InfoBox_AddText( struct Task* task )
{
	AddTextPrinterParameterized3(
		1, 1, 2, 5, gColors_ReeltimeHelp, 0, gText_ReelTimeHelp );
...
static void sub_8104CAC( u8 arg0 )
{
	...
	{
		u8 spriteId;
		spriteId = sub_8105BB4( gUnknown_083ED048[arg0][i].unk00,
...
static void sub_8104EA8( void )
{
	...
	for( i = 0, x = 0x30; i < 3; i++, x += 0x28 )
	{
		for( j = 0; j < 120; j += 24 )
		{
			struct Sprite* sprite = gSprites +
				CreateSprite(
					&gSpriteTemplate_83ED414, x, 0, 14 );
...
{
	sprite->data[2] = sSlotMachine->reelPixelOffsets[sprite->data[0]] +
		sprite->data[1];
	sprite->data[2] %= 120;
	sprite->pos1.y =
		sSlotMachine->reelPixelOffsetsWhileStopping[sprite->data[0]] +
		28 + sprite->data[2];
	sprite->sheetTileStart =
		GetSpriteTileStartByTag( GetNearbyTag_Quantized(
			sprite->data[0], sprite->data[2] / 24 ) );
...
static void sub_8104FF4( s16 x, s16 y, u8 a2, s16 a3 )
{
	struct Sprite* sprite =
		gSprites + CreateSprite( &gSpriteTemplate_83ED42C, x, y, 13 );
...
static void sub_8105284_( void )
{
	...
	spriteId                  = CreateSprite(
                &spriteTemplate, 0xa8 - gSpriteCoordOffsetX, 0x50, 7 );
	sprite                     = &gSprites[spriteId];
	sprite->oam.priority       = 1;
	sprite->coordOffsetEnabled = TRUE;
...
static void sub_81052EC( void )
{
	...
	{
		u8 spriteId =
			CreateSprite( &gSpriteTemplate_83ED4BC, 0x170, 0, 10 );
		struct Sprite* sprite      = &gSprites[spriteId];
		sprite->oam.priority       = 1;
		sprite->coordOffsetEnabled = TRUE;
		sprite->data[7]            = r5;
...
{
	s16 r0 =
		( u16 )( sSlotMachine->reeltimePixelOffset + sprite->data[7] );
	r0 %= 40;
	sprite->pos1.y = r0 + 59;
...
{
	u8 spriteId =
		CreateSprite( &gSpriteTemplate_83ED4EC, 0x170, 0x4c, 11 );
	struct Sprite* sprite      = &gSprites[spriteId];
	sprite->coordOffsetEnabled = TRUE;
	sprite->oam.priority       = 1;
...
/*static */ void sub_8105784( struct Sprite* sprite )
{
	...
	if( sprite->data[0] && --sprite->data[6] <= 0 )
	{
		MultiplyInvertedPaletteRGBComponents(
			( IndexOfSpritePaletteTag( 7 ) << 4 ) + 0x103,
			sp[sprite->data[5]],
			sp[sprite->data[5]],
			sp[sprite->data[5]] );
		++sprite->data[5];
...
static void sub_8105804( void )
{
	u8 i;
	MultiplyInvertedPaletteRGBComponents(
		( IndexOfSpritePaletteTag( 7 ) << 4 ) + 0x103, 0, 0, 0 );
	for( i = 0; i < 2; i++ )
...
static void sub_81058C4( void )
{
	...
	for( i = 0; i < 4; i++ )
	{
		u8 spriteId           = CreateSprite( &gSpriteTemplate_83ED54C,
                        0x50 - gSpriteCoordOffsetX,
                        0x44,
                        0 );
		struct Sprite* sprite = &gSprites[spriteId];
		sprite->oam.priority  = 1;
...
static u8 sub_8105ACC( void ) { return gSprites[sSlotMachine->unk43].data[7]; }
...
static u8 sub_8105BB4( u8 templateIdx, u8 cbAndCoordsIdx, s16 a2 )
{
	return sub_8105BF8( templateIdx,
		gUnknown_083ECF0C[cbAndCoordsIdx],
		gUnknown_083ECE7E[cbAndCoordsIdx][0],
		gUnknown_083ECE7E[cbAndCoordsIdx][1],
		a2 );
}

static u8 sub_8105BF8(
	u8 templateIdx, SpriteCallback callback, s16 x, s16 y, s16 a4 )
{
	struct SpriteTemplate spriteTemplate;
	u8 spriteId;
	struct Sprite* sprite;
...
static void sub_8105C64( struct Sprite* sprite ) { sprite->data[7] = 0; }
...
static void sub_8105F9C( struct Sprite* sprite )
{
	...
	case 1:
		...
		if( sprite->data[1] < 0 )
			sprite->data[1] = 0;
		SetGpuReg( REG_OFFSET_MOSAIC,
			( ( sprite->data[1] << 4 ) | sprite->data[1] ) << 8 );
		sprite->data[2] &= 0xff;
		sprite->data[2] += 0x80;
		if( sprite->data[1] == 0 )
		{
...
static void sub_8106058( struct Sprite* sprite )
{
	if( sprite->data[1] < 3 )
	{
		LoadPalette( gUnknown_083EDE10[sprite->data[1]],
			( IndexOfSpritePaletteTag( 6 ) << 4 ) + 0x100,
			0x20 );
		if( ++sprite->data[2] >= 4 )
		{
			sprite->data[1]++;
...
static void sub_8106364( void ) { SetGpuReg( REG_OFFSET_MOSAIC, 0 ); }

static void sub_8106370( void )
{
	LoadPalette( gUnknown_083EDE20,
		( IndexOfSpritePaletteTag( 6 ) << 4 ) + 0x100,
		0x20 );
}
...
static void LoadSlotMachineWheelOverlay( void )
{
	...
	for( x = 4; x < 18; x += 5 )
	{
		...
		{
			LoadBgTilemap(
				3, sUnknown_0203AAD0, 2, x + dx + 5 * 32 );
			LoadBgTilemap( 3,
				sUnknown_0203AAD0 + 1,
				2,
				x + dx + 13 * 32 );
			LoadBgTilemap(
				3, sUnknown_0203AAD0 + 2, 2, x + dx + 6 * 32 );
			LoadBgTilemap( 3,
				sUnknown_0203AAD0 + 3,
				2,
				x + dx + 12 * 32 );
		...
		for( y = 7; y <= 11; y++ )
			LoadBgTilemap(
				3, sUnknown_0203AAD0 + 6, 2, x + y * 32 );
	}
}
...
const u8 gReelTimeTags[] = {1, 0, 5, 4, 3, 2};

const s16 gInitialReelPositions[][2] = {{0, 6}, {0, 10}, {0, 2}};

const u8 gLuckyRoundProbabilities[][3] = {{1, 1, 12},
	{1, 1, 14},
	{2, 2, 14},
	{2, 2, 14},
	{2, 3, 16},
...
const u8 gLuckyFlagProbabilities_Top3[][6] = {{25, 25, 30, 40, 40, 50},
	{25, 25, 30, 30, 35, 35},
	{25, 25, 30, 25, 25, 30}};

const u8 gLuckyFlagProbabilities_NotTop3[][6] = {{20, 25, 25, 20, 25, 25},
	{12, 15, 15, 18, 19, 22},
	{25, 25, 25, 30, 30, 40},
	{25, 25, 20, 20, 15, 15},
	{40, 40, 35, 35, 40, 40}};
...
const u8 gReelTimeProbabilities_LuckyGame[][17] = {{243,
							   243,
							   243,
							   200,
							   200,
...
const u16 gReelTimeBonusIncrementTable[] = {0, 5, 10, 15, 20};

// tentative name
const u8 gBiasTags[] = {SLOT_MACHINE_TAG_REPLAY,
	SLOT_MACHINE_TAG_CHERRY,
	SLOT_MACHINE_TAG_LOTAD,
	SLOT_MACHINE_TAG_AZURILL,
	SLOT_MACHINE_TAG_POWER,
...
const u16 gLuckyFlagSettings_NotTop3[] = {LUCKY_BIAS_POWER,
	LUCKY_BIAS_AZURILL,
	LUCKY_BIAS_LOTAD,
	LUCKY_BIAS_CHERRY,
	LUCKY_BIAS_REPLAY};

const u8 gSym2Match[] = {SLOT_MACHINE_MATCHED_777_RED,
	SLOT_MACHINE_MATCHED_777_BLUE,
	SLOT_MACHINE_MATCHED_AZURILL,
	SLOT_MACHINE_MATCHED_LOTAD,
	SLOT_MACHINE_MATCHED_1CHERRY,
...
const u16 gSlotMatchFlags[] = {1 << SLOT_MACHINE_MATCHED_1CHERRY,
...
const u16 gSlotPayouts[] = {2, 4, 0, 6, 12, 3, 90, 300, 300};

const s16 gUnknown_083ECE7E[][2] = {{0xd0, 0x38},
	{0xb8, 0x00},
	{0xc8, 0x08},
	{0xd8, 0x10},
	{0xe8, 0x18},
...
const SpriteCallback gUnknown_083ECF0C[] = {sub_8105C64,
	sub_8105F54,
	sub_8105F54,
	sub_8105F54,
	sub_8105F54,
...
const struct UnkStruct1 Unknown_83ECF98[] = {
	{25, 34, 0}, {2, 0, 0}, {9, 16, 0}, {255, 0, 0}};

const struct UnkStruct1 Unknown_83ECFA8[] = {{10, 1, 0},
	{11, 2, 0},
	{12, 3, 0},
	{13, 4, 0},
	{5, 5, 0},
...
const struct UnkStruct1 Unknown_83ECFC4[] = {
	{3, 7, 0}, {8, 17, 0}, {255, 0, 0}};

const struct UnkStruct1 Unknown_83ECFD0[] = {
	{4, 8, 0}, {6, 9, 0}, {6, 10, 1}, {6, 11, 2}, {6, 12, 3}, {255, 0, 0}};

const struct UnkStruct1 Unknown_83ECFE8[] = {
	{0, 13, 0}, {1, 14, 0}, {7, 15, 0}, {255, 0, 0}};

const struct UnkStruct1 Unknown_83ECFF8[] = {{19, 26, 0},
	{20, 27, 1},
	{21, 28, 2},
	{14, 29, 3},
	{15, 30, 4},
...
const struct UnkStruct1 Unknown_83ED020[] = {{22, 18, 0},
	{23, 19, 1},
	{24, 20, 2},
	{14, 21, 3},
	{15, 22, 4},
...
void ( *const gUnknown_083ED064[] )( void ) = {sub_810639C,
	sub_8106364,
	sub_8106370,
	nullsub_70,
	nullsub_70,
