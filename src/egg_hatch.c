...
static void EggHatchPrintMessage(
	u8 windowId, u8* string, u8 x, u8 y, u8 speed );
static void CreateRandomEggShardSprite( void );
static void CreateEggShardSprite(
	u8 x, u8 y, s16 data1, s16 data2, s16 data3, u8 spriteAnimIndex );
...
static u8 EggHatchCreateMonSprite(
	u8 a0, u8 switchID, u8 pokeID, u16* speciesLoc )
{
	u8 r5               = 0;
	u8 spriteID         = 0;
	...
	case 1:
		SetMultiuseSpriteTemplateToPokemon(
			GetMonSpritePalStruct( mon )->tag, r5 );
		spriteID =
			CreateSprite( &gMultiuseSpriteTemplate, 120, 75, 6 );
...
static void CB2_EggHatch_0( void )
{
	...
	case 5:
		...
		break;
	case 6:
		sEggHatchData->pokeSpriteID = EggHatchCreateMonSprite( 0,
			1,
			sEggHatchData->eggPartyID,
			&sEggHatchData->species );
		gMain.state++;
...
static void CB2_EggHatch_1( void )
{
	...
	case 0:
		...
		sEggHatchData->eggSpriteID =
			CreateSprite( &sSpriteTemplate_EggHatch, 120, 75, 5 );
		ShowBg( 0 );
		ShowBg( 1 );
	...
	case 1:
		if( !gPaletteFade.active )
		{
			FillWindowPixelBuffer(
				sEggHatchData->windowId, PIXEL_FILL( 0 ) );
	...
	case 2:
		...
		{
			sEggHatchData->CB2_state++;
			gSprites[sEggHatchData->eggSpriteID].callback =
				SpriteCB_Egg_0;
		}
		break;
	case 3:
			...
			SpriteCallbackDummy )
		{
			...
			DoMonFrontSpriteAnimation(
				&gSprites[sEggHatchData->pokeSpriteID],
				species,
				FALSE,
				1 );
			sEggHatchData->CB2_state++;
		...
		break;
	case 4:
		if( gSprites[sEggHatchData->pokeSpriteID].callback ==
			SpriteCallbackDummy )
		{
			sEggHatchData->CB2_state++;
		}
		break;
	case 5:
		GetMonNick( &gPlayerParty[sEggHatchData->eggPartyID],
			gStringVar1 );
		StringExpandPlaceholders( gStringVar4, gText_HatchedFromEgg );
		EggHatchPrintMessage(
			sEggHatchData->windowId, gStringVar4, 0, 3, 0xFF );
	...
	case 8:
		...
		EggHatchPrintMessage(
			sEggHatchData->windowId, gStringVar4, 0, 2, 1 );
		sEggHatchData->CB2_state++;
		break;
	case 9:
		if( !IsTextPrinterActive( sEggHatchData->windowId ) )
		{
			LoadUserWindowBorderGfx(
				sEggHatchData->windowId, 0x140, 0xE0 );
...
static void SpriteCB_Egg_4( struct Sprite* sprite )
{
	...
	if( sprite->data[0] == 0 )
		BeginNormalPaletteFade(
			0xFFFFFFFF, -1, 0, 0x10, RGB_WHITEALPHA );
	if( sprite->data[0] < 4u )
	{
		for( i = 0; i <= 3; i++ )
			CreateRandomEggShardSprite( );
...
static void SpriteCB_Egg_5( struct Sprite* sprite )
{
	...
	if( sprite->data[0] == 8 )
		BeginNormalPaletteFade(
			0xFFFFFFFF, -1, 0x10, 0, RGB_WHITEALPHA );
	if( sprite->data[0] <= 9 )
...
static void SpriteCB_EggShard( struct Sprite* sprite )
{
	...
	if( sprite->pos1.y + sprite->pos2.y > sprite->pos1.y + 20 &&
		sprite->data[2] > 0 )
...
static void CreateRandomEggShardSprite( void )
{
	...
	s16 velocity1 =
		sEggShardVelocities[sEggHatchData->eggShardVelocityID][0];
	s16 velocity2 =
		sEggShardVelocities[sEggHatchData->eggShardVelocityID][1];
	sEggHatchData->eggShardVelocityID++;
	spriteAnimIndex = Random( ) % 4;
	CreateEggShardSprite(
		120, 60, velocity1, velocity2, 100, spriteAnimIndex );
}
