...
{
	struct Pokenav3Struct* state =
		AllocSubstruct( 5, sizeof( struct Pokenav3Struct ) );
	if( !state )
		return FALSE;
...
void sub_81CAB38( void ) { FreePokenavSubstruct( 5 ); }
...
static u32 sub_81CAB44( struct Pokenav3Struct* state )
{
	...
	if( gMain.newKeys & A_BUTTON )
	{
		...
		if( !state->unk1C[selectedMatchCall].unk0 ||
			sub_81D17E8( state->unk1C[selectedMatchCall].unk2 ) )
		{
...
static u32 sub_81CAD20( int taskState )
{
	...
	case 1:
		for( i = 0, j = state->unk8; i < 30; i++, j++ )
		{
			if( MatchCallFlagGetByIndex( j ) )
			{
				...
				state->unk1C[state->unkA].unk1 =
					sub_81D16DC( j );
				state->unkA++;
			}

			if( ++state->unk8 >= 21 ) // TODO: This is the size of
						  // sMatchCallHeaders
			{
	...
	case 2:
		for( i = 0, j = state->unk8; i < 30; i++, j++ )
		{
			if( !sub_81D1BF8( state->unk8 ) &&
				sub_81CAE08( state->unk8 ) )
			{
	...
	*arg1                        = 0;
	...
	if( !state->unk1C[index].unk0 )
		*arg1 = SelectMatchCallMessage(
			GetTrainerIdxByRematchIdx( state->unk1C[index].unk2 ),
			gStringVar4 );
	else
...
const u8* sub_81CAFD8( int index, int textType )
{
	...
	if( state->unk1C[index].unk0 )
	{
		var0 = MatchCall_GetRematchTableIdx(
			state->unk1C[index].unk2 );
		if( var0 == REMATCH_TABLE_ENTRIES )
			return sub_81D1B40(
				state->unk1C[index].unk2, textType );
	}
	else
	{
		var0 = state->unk1C[index].unk2;
...
bool32 unref_sub_81CB16C( void )
{
	...
	for( i = 0; i < 21;
		i++ ) // TODO: This is the size of sMatchCallHeaders
	{
...
static bool32 sub_81CB1D0( void )
{
	...
	if( !state->unk1C[index].unk0 )
	{
		...
		{
			if( !gSaveBlock1Ptr->trainerRematches
					[state->unk1C[index].unk2] )
				return TRUE;
		}
	}
