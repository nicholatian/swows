...
EWRAM_DATA static u32 sFiller_02037344 = 0; // without this, the next file
...
static bool8 SkipCopyingMetatileFromSavedMap(
	u16* mapMetatilePtr, u16 mapWidth, u8 yMode );

struct MapHeader const* const mapconnection_get_mapheader(
	struct MapConnection* connection )
{
	return Overworld_GetMapHeaderByGroupAndId(
		connection->mapGroup, connection->mapNum );
}
...
static void InitMapLayoutData( struct MapHeader* mapHeader )
{
	...
	if( width * height <= MAX_MAP_DATA_SIZE )
	{
		InitBackupMapLayoutData(
			mapLayout->map, mapLayout->width, mapLayout->height );
...
u8 MapGridGetZCoordAt( int x, int y )
{
	...
	if( x >= 0 && x < gBackupMapLayout.width && y >= 0 &&
...
u8 MapGridGetMetatileLayerTypeAt( int x, int y )
{
	u16 metatile;
	metatile = MapGridGetMetatileIdAt( x, y );
	return ( GetBehaviorByMetatileId( metatile ) &
...
void MapGridSetMetatileIdAt( int x, int y, u16 metatile )
{
		...
		y < gBackupMapLayout.height )
	{
		...
		gBackupMapLayout.map[i] =
			( gBackupMapLayout.map[i] & METATILE_ELEVATION_MASK ) |
...
u16 GetBehaviorByMetatileId( u16 metatile )
{
	...
	{
		attributes = gMapHeader.mapLayout->primaryTileset
				     ->metatileAttributes;
		return attributes[metatile];
	}
	else if( metatile < NUM_METATILES_TOTAL )
	{
		attributes = gMapHeader.mapLayout->secondaryTileset
				     ->metatileAttributes;
...
static bool32 SavedMapViewIsEmpty( void )
{
	...
	// BUG: This loop extends past the bounds of the mapView array. Its
	// size is only 0x100.
	for( i = 0; i < 0x200; i++ )
...
{
	CpuFill16( 0,
		gSaveBlock1Ptr->mapView,
		sizeof( gSaveBlock1Ptr->mapView ) );
}
...
int GetPostCameraMoveMapBorderId( int x, int y )
{
	return GetMapBorderIdAt(
		gSaveBlock1Ptr->pos.x + 7 + x, gSaveBlock1Ptr->pos.y + 7 + y );
}
...
void sub_80887F8(
	struct MapConnection* connection, int direction, int x, int y )
{
...
bool8 CameraMove( int x, int y )
{
	...
	else
	{
		...
		connection = sub_8088950( direction,
...
struct MapConnection* sub_8088950( u8 direction, int x, int y )
{
	...
	for( i = 0; i < count; i++, connection++ )
	{
		if( connection->direction == direction &&
			sub_80889A8( direction, x, y, connection ) == TRUE )
			return connection;
	}
	return NULL;
}
...
bool8 sub_80889A8(
	u8 direction, int x, int y, struct MapConnection* connection )
{
	struct MapHeader const* mapHeader;
	mapHeader = mapconnection_get_mapheader( connection );
	switch( direction )
...
int sub_8088A4C( struct MapConnection* connection, int x, int y )
{
	...
	case CONNECTION_NORTH:
		return sub_8088A38(
			x - connection->offset, mapHeader->mapLayout->width );
	case CONNECTION_WEST:
	case CONNECTION_EAST:
		return sub_8088A38(
			y - connection->offset, mapHeader->mapLayout->height );
	}
	return FALSE;
}
		...
		for( i = 0; i < count; i++, connection++ )
		{
			...
			if( ( direction == CONNECTION_DIVE ||
				...
				( direction == CONNECTION_EAST &&
					x < gMapHeader.mapLayout->width + 7 ) )
			{
				continue;
			}
...
void copy_tileset_patterns_to_vram(
	struct Tileset const* tileset, u16 numTiles, u16 offset )
{
	if( tileset )
	{
		if( !tileset->isCompressed )
			...
			decompress_and_copy_tile_data_to_vram(
				2, tileset->tiles, numTiles * 32, offset, 0 );
	}
}

void copy_tileset_patterns_to_vram2(
	struct Tileset const* tileset, u16 numTiles, u16 offset )
{
	if( tileset )
	{
...
void apply_map_tileset_palette(
	struct Tileset const* tileset, u16 destOffset, u16 size )
{
	u16 black = RGB_BLACK;
	...
	if( tileset )
	{
		if( tileset->isSecondary == FALSE )
		{
			...
			LoadPalette( ( (u16*)tileset->palettes ) + 1,
				destOffset + 1,
				size - 2 );
		...
		{
			LoadPalette( ( (u16*)tileset->palettes ) +
		...
		else
		{
			LoadCompressedPalette(
				(u32*)tileset->palettes, destOffset, size );
			nullsub_3( destOffset, size >> 1 );
		}
	}
}
...
void copy_map_tileset1_to_vram( struct MapLayout const* mapLayout )
{
	copy_tileset_patterns_to_vram(
		mapLayout->primaryTileset, NUM_TILES_IN_PRIMARY, 0 );
}

void copy_map_tileset2_to_vram( struct MapLayout const* mapLayout )
{
	copy_tileset_patterns_to_vram( mapLayout->secondaryTileset,
...
{
	copy_tileset_patterns_to_vram2( mapLayout->secondaryTileset,
		NUM_TILES_TOTAL - NUM_TILES_IN_PRIMARY,
		NUM_TILES_IN_PRIMARY );
}
...
void apply_map_tileset1_palette( struct MapLayout const* mapLayout )
{
	apply_map_tileset_palette(
		mapLayout->primaryTileset, 0, NUM_PALS_IN_PRIMARY * 16 * 2 );
}

void apply_map_tileset2_palette( struct MapLayout const* mapLayout )
{
	apply_map_tileset_palette( mapLayout->secondaryTileset,
		NUM_PALS_IN_PRIMARY * 16,
