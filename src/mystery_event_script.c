...
{
	bool32 ret =
		RunMysteryEventScriptCommand( &sMysteryEventScriptContext );
...
static bool32 IsRecordMixingGiftValid( void )
{
	struct RecordMixingGiftData* data =
		&gSaveBlock1Ptr->recordMixingGift.data;
...
{
	CpuFill16( 0,
		&gSaveBlock1Ptr->recordMixingGift,
...
{
	u8* script =
		(u8*)( ScriptReadWord( ctx ) - ctx->data[1] + ctx->data[0] );
...
bool8 MEScrCmd_setenigmaberry( struct ScriptContext* ctx )
{
	...
	u8* berry =
		(u8*)( ScriptReadWord( ctx ) - ctx->data[1] + ctx->data[0] );
	StringCopyN( gStringVar1,
		gSaveBlock1Ptr->enigmaBerry.berry.name,
		BERRY_NAME_LENGTH + 1 );
	SetEnigmaBerry( berry );
	StringCopyN( gStringVar2,
		gSaveBlock1Ptr->enigmaBerry.berry.name,
		BERRY_NAME_LENGTH + 1 );
...
bool8 MEScrCmd_giveribbon( struct ScriptContext* ctx )
{
	...
	StringExpandPlaceholders(
		gStringVar4, gText_MysteryGiftSpecialRibbon );
	ctx->data[2] = 2;
	return FALSE;
}
...
bool8 MEScrCmd_initramscript( struct ScriptContext* ctx )
{
	...
	u8* scriptEnd =
		(u8*)( ScriptReadWord( ctx ) - ctx->data[1] + ctx->data[0] );
	InitRamScript(
		script, scriptEnd - script, mapGroup, mapNum, objectId );
	return FALSE;
}
...
bool8 MEScrCmd_givepokemon( struct ScriptContext* ctx )
{
	...
	if( species == SPECIES_EGG )
		...
		StringCopyN(
			gStringVar1, gText_Pokemon, POKEMON_NAME_LENGTH + 1 );

	if( gPlayerPartyCount == PARTY_SIZE )
	{
		StringExpandPlaceholders(
			gStringVar4, gText_MysteryGiftFullParty );
		ctx->data[2] = 3;
	}
	else
	{
		...
		StringExpandPlaceholders(
			gStringVar4, gText_MysteryGiftSentOver );
		ctx->data[2] = 2;
	}
