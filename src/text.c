...
const u8 gDownArrowTiles[] =
	INCBIN_U8( "data/graphics/fonts/down_arrow.4bpp" );
const u8 gDarkDownArrowTiles[] =
	INCBIN_U8( "data/graphics/fonts/down_arrow_RS.4bpp" );
const u8 gUnusedFRLGBlankedDownArrow[] =
	INCBIN_U8( "data/graphics/fonts/unused_frlg_blanked_down_arrow.4bpp" );
const u8 gUnusedFRLGDownArrow[] =
	INCBIN_U8( "data/graphics/fonts/unused_frlg_down_arrow.4bpp" );
...
const struct KeypadIcon gKeypadIcons[] = {{0x0, 0x8, 0xC},
	{0x1, 0x8, 0xC},
	{0x2, 0x10, 0xC},
	{0x4, 0x10, 0xC},
	{0x6, 0x18, 0xC},
...
const u8 gKeypadIconTiles[] =
	INCBIN_U8( "data/graphics/fonts/keypad_icons.4bpp" );
...
const u16 gFont9JapaneseGlyphs[] =
	INCBIN_U16( "data/graphics/fonts/font9.hwjpnfont" );
...
void SetFontsPointer( const struct FontInfo* fonts ) { gFonts = fonts; }
	...
	void ( *callback )( struct TextPrinterTemplate*, u16 ) )
{
	...
	else
	{
		...
		if( speed != TEXT_SPEED_FF )
			CopyWindowToVram(
				gTempTextPrinter.printerTemplate.windowId, 2 );
...
bool16 IsTextPrinterActive( u8 id ) { return gTextPrinters[id].active; }
...
u32 RenderFont( struct TextPrinter* textPrinter )
{
	...
	while( TRUE )
	{
		ret = gFonts[textPrinter->printerTemplate.fontId].fontFunction(
			textPrinter );
		if( ret != 2 )
			return ret;
	}
...
void DecompressGlyphTile( const void* src_, void* dest_ )
{
	...
	temp = *( src++ );
	*( dest )++ =
		( ( gFontHalfRowLookupTable[gFontHalfRowOffsets[temp & 0xFF]] )
			<< 16 ) |
	...
	if( gTextFlags.autoScroll == 0 )
	{
		if( subStruct->downArrowDelay != 0 )
		{
			subStruct->downArrowDelay =
				( ( *(u32*)&textPrinter->subUnion.sub )
						<< 19 >>
					27 ) -
				1; // convoluted way of getting downArrowDelay,
				   // necessary to match
		}
		else
		{
			...
			BlitBitmapRectToWindow(
				...
				arrowTiles,
				0,
				gDownArrowYCoords[*(u32*)subStruct << 17 >>
				...
				0x10 );
			CopyWindowToVram(
				textPrinter->printerTemplate.windowId, 0x2 );

			subStruct->downArrowDelay = 0x8;
			subStruct->downArrowYPosIdx =
				( *(u32*)subStruct << 17 >> 30 ) + 1;
		}
	}
}
...
void DrawDownArrow( u8 windowId,
	u16 x,
	u16 y,
	u8 bgColor,
	bool8 drawArrow,
...
u16 RenderText( struct TextPrinter* textPrinter )
{
	...
	case 0:
		...
		case CHAR_NEWLINE:
			...
			textPrinter->printerTemplate.currentY +=
				( gFonts[textPrinter->printerTemplate.fontId]
						.maxLetterHeight +
					textPrinter->printerTemplate
						.lineSpacing );
			return 2;
		...
		case EXT_CTRL_CODE_BEGIN:
			...
			case 10:
				...
				return 3;
			case 11:
				currChar = *textPrinter->printerTemplate
						    .currentChar;
				textPrinter->printerTemplate.currentChar++;
				currChar |= *textPrinter->printerTemplate
						     .currentChar
			...
			case EXT_CTRL_CODE_CLEAR:
				width = *textPrinter->printerTemplate
						 .currentChar;
				textPrinter->printerTemplate.currentChar++;
				if( width > 0 )
				{
			...
			case EXT_CTRL_CODE_CLEAR_TO: {
				widthHelper = *textPrinter->printerTemplate
						       .currentChar;
		...
		case CHAR_SPECIAL_F8:
			currChar = *textPrinter->printerTemplate.currentChar++;
			gUnknown_03002F90.unk80 = DrawKeypadIcon(
				textPrinter->printerTemplate.windowId,
				currChar,
				textPrinter->printerTemplate.currentX,
				textPrinter->printerTemplate.currentY );
			textPrinter->printerTemplate.currentX +=
				...
				textPrinter->printerTemplate.letterSpacing;
			return 0;
		case EOS:
			return 1;
		}
	...
	case 2:
		if( TextPrinterWaitWithDownArrow( textPrinter ) )
		{
			FillWindowPixelBuffer(
				textPrinter->printerTemplate.windowId,
				PIXEL_FILL( textPrinter->printerTemplate
						    .bgColor ) );
			textPrinter->printerTemplate.currentX =
				textPrinter->printerTemplate.x;
			textPrinter->printerTemplate.currentY =
	...
	case 4:
		if( textPrinter->scrollDistance )
		{
			...
			CopyWindowToVram(
				textPrinter->printerTemplate.windowId, 2 );
		}
		else
		{
			textPrinter->state = 0;
		...
		case EXT_CTRL_CODE_BEGIN:
			...
			case 0x6:
				...
				if( func == NULL )
					return 0;
				if( letterSpacing == -1 )
					localLetterSpacing = GetFontAttribute(
						*str,
...
u8 RenderTextFont9( u8* pixels, u8 fontId, u8* str )
{
	...
	do
	{
		...
		default:
			...
			CpuCopy32(
				gUnknown_03002F90.unk40, pixels + 0x20, 0x20 );
			pixels += 0x40;
			break;
		}
	} while( temp != EOS );
...
u8 DrawKeypadIcon( u8 windowId, u8 keypadIconId, u16 x, u16 y )
{
	BlitBitmapRectToWindow( windowId,
		gKeypadIconTiles +
			( gKeypadIcons[keypadIconId].tileOffset * 0x20 ),
		0,
		0,
		0x80,
		0x80,
...
void SetDefaultFontsPointer( void ) { SetFontsPointer( &gFontInfos[0] ); }
...
void DecompressGlyphFont0( u16 glyphId, bool32 isJapanese )
{
	...
	else
	{
		...
		if( gUnknown_03002F90.unk80 <= 8 )
		{
			DecompressGlyphTile( glyphs, gUnknown_03002F90.unk0 );
			DecompressGlyphTile(
				glyphs + 0x10, gUnknown_03002F90.unk40 );
		}
		else
		{
			DecompressGlyphTile( glyphs, gUnknown_03002F90.unk0 );
			DecompressGlyphTile(
				glyphs + 0x8, gUnknown_03002F90.unk20 );
			DecompressGlyphTile(
				glyphs + 0x10, gUnknown_03002F90.unk40 );
			DecompressGlyphTile(
				glyphs + 0x18, gUnknown_03002F90.unk60 );
		}
...
void DecompressGlyphFont7( u16 glyphId, bool32 isJapanese )
{
	...
	{
		int eff;
		glyphs = gFont1JapaneseGlyphs +
			( 0x100 * ( glyphId >> 0x4 ) ) +
			( 0x8 * ( glyphId & ( eff = 0xF ) ) ); // shh, no
							       // questions,
							       // only matching
							       // now
		DecompressGlyphTile( glyphs, gUnknown_03002F90.unk0 );
...
void DecompressGlyphFont2( u16 glyphId, bool32 isJapanese )
{
	...
	if( isJapanese == TRUE )
	{
		glyphs = gFont2JapaneseGlyphs +
			( 0x100 * ( glyphId >> 0x3 ) ) +
			( 0x10 * ( glyphId & 0x7 ) );
		DecompressGlyphTile( glyphs, gUnknown_03002F90.unk0 );
		DecompressGlyphTile( glyphs + 0x8,
			gUnknown_03002F90.unk20 ); // gUnknown_03002F90
						   // + 0x40
		DecompressGlyphTile( glyphs + 0x80,
			gUnknown_03002F90.unk40 ); // gUnknown_03002F90
						   // + 0x20
		DecompressGlyphTile( glyphs + 0x88,
			gUnknown_03002F90.unk60 ); // gUnknown_03002F90
						   // + 0x60
		gUnknown_03002F90.unk80 =
			gFont2JapaneseGlyphWidths[glyphId]; // gGlyphWidth
...
void DecompressGlyphFont9( u16 glyphId )
{
	...
	glyphs = gFont9JapaneseGlyphs + ( 0x100 * ( glyphId >> 4 ) ) +
		( 0x8 * ( glyphId & 0xF ) );
	DecompressGlyphTile( glyphs, gUnknown_03002F90.unk0 );
