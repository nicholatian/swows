...
static const u16 sMoves_DependsOnTheBattlesFlow[] = {MOVE_SANDSTORM,
	MOVE_RAIN_DANCE,
	MOVE_SUNNY_DAY,
	MOVE_HAIL,
	MOVE_WEATHER_BALL,
...
static void InitFactoryChallenge( void )
{
	...
	if( !( gSaveBlock2Ptr->frontier.field_CDC &
		    gUnknown_08612164[battleMode][lvlMode] ) )
	{
		gSaveBlock2Ptr->frontier
			.factoryWinStreaks[battleMode][lvlMode] = 0;
		gSaveBlock2Ptr->frontier
			.factoryRentsCount[battleMode][lvlMode] = 0;
	}
...
static void SetBattleFactoryData( void )
{
	...
	case 2:
		if( gSpecialVar_0x8006 )
			...
			gSaveBlock2Ptr->frontier.field_CDC &=
				gUnknown_08612174[battleMode][lvlMode];
		break;
	case 3:
...
static void GenerateOpponentMons( void )
{
	...
	u32 lvlMode    = gSaveBlock2Ptr->frontier.lvlMode;
	u32 battleMode = VarGet( VAR_FRONTIER_BATTLE_MODE );
	u32 winStreak  = gSaveBlock2Ptr->frontier
				.factoryWinStreaks[battleMode][lvlMode];
	...
	do
	{
		trainerId = sub_8162548( challengeNum,
	...
	while( i != 3 )
	{
		...
		for( j = 0; j < 6; j++ )
		{
			if( gFacilityTrainerMons[monSetId].species ==
...
static void SetRentalsToOpponentParty( void )
{
	...
	for( i = 0; i < 3; i++ )
	{
		gSaveBlock2Ptr->frontier.rentalMons[i + 3].monId =
			gUnknown_03006298[i];
		gSaveBlock2Ptr->frontier.rentalMons[i + 3].ivs = GetBoxMonData(
			&gEnemyParty[i].box, MON_DATA_ATK_IV, NULL );
		gSaveBlock2Ptr->frontier.rentalMons[i + 3].personality =
			GetMonData(
				&gEnemyParty[i], MON_DATA_PERSONALITY, NULL );
		gSaveBlock2Ptr->frontier.rentalMons[i + 3].abilityNum =
			GetBoxMonData( &gEnemyParty[i].box,
				MON_DATA_ABILITY_NUM,
				NULL );
		SetMonData( &gEnemyParty[i],
			MON_DATA_HELD_ITEM,
			&gBattleFrontierHeldItems
				[gFacilityTrainerMons[gUnknown_03006298[i]]
						.itemTableId] );
	}
}
...
static void SetPlayerAndOpponentParties( void )
{
	...
	if( gSpecialVar_0x8005 < 2 )
	{
		...
		for( i = 0; i < 3; i++ )
		{
			...
			for( j = 0; j < NUM_STATS; bits <<= 1, j++ )
			{
				if( gFacilityTrainerMons[monSetId].evSpread &
					bits )
	...
	case 2:
		for( i = 0; i < 3; i++ )
		{
			monSetId = gSaveBlock2Ptr->frontier.rentalMons[i + 3]
					   .monId;
			...
			CreateMon( &gEnemyParty[i],
				...
				ivs,
				TRUE,
				gSaveBlock2Ptr->frontier.rentalMons[i + 3]
					.personality,
				0,
				0 );
...
static void GenerateInitialRentalMons( void )
{
	...
	{
		if( i < var_40 )
			monSetId = GetMonSetId(
				factoryLvlMode, challengeNum, TRUE );
		else
			monSetId = GetMonSetId(
				factoryLvlMode, challengeNum, FALSE );
...
static void GetOpponentMostCommonMonType( void )
{
	...
	{
		u32 species =
			gFacilityTrainerMons[gUnknown_03006298[i]].species;
	...
	gSpecialVar_Result = gSpecialVar_Result; // Needed to match. Don't ask
						 // me why.
	if( typesCount[usedType[0]] != 0 &&
		( typesCount[usedType[0]] > typesCount[usedType[1]] ||
			usedType[0] == usedType[1] ) )
...
static void GetOpponentBattleStyle( void )
{
	...
	for( i = 0; i < 3; i++ )
	{
		...
		{
			u8 battleStyle = GetMoveBattleStyle(
...
void FillFactoryBrainParty( void )
{
	...
	while( i != 3 )
	{
		...
		{
			if( monSetId ==
				gSaveBlock2Ptr->frontier.rentalMons[j].monId )
				break;
		}
		if( j != 6 )
			continue;
		...
		for( k = 0; k < MAX_MON_MOVES; k++ )
			SetMonMoveAvoidReturn( &gEnemyParty[i],
				gFacilityTrainerMons[monSetId].moves[k],
				k );
		SetMonData(
			&gEnemyParty[i], MON_DATA_FRIENDSHIP, &friendship );
		SetMonData( &gEnemyParty[i],
			MON_DATA_HELD_ITEM,
			&gBattleFrontierHeldItems
...
static u16 GetMonSetId( u8 lvlMode, u8 challengeNum, bool8 arg2 )
{
	...
	if( challengeNum < 7 )
	{
		if( arg2 )
		{
			range = ( sMonsToChooseFrom[adder + challengeNum + 1]
						   [1] -
					sMonsToChooseFrom[adder +
						challengeNum + 1][0] ) +
				1;
			monSetId = Random( ) % range;
			monSetId +=
				sMonsToChooseFrom[adder + challengeNum + 1][0];
		}
		else
		{
			range = ( sMonsToChooseFrom[adder + challengeNum][1] -
					sMonsToChooseFrom[adder + challengeNum]
							 [0] ) +
				1;
			monSetId = Random( ) % range;
	...
	else
	{
		...
		if( num != 7 )
			num = 7;
		range = ( sMonsToChooseFrom[adder + num][1] -
				sMonsToChooseFrom[adder + num][0] ) +
			1;
		monSetId = Random( ) % range;
...
{
	u8 ret;
	u8 rents = gSaveBlock2Ptr->frontier
			   .factoryRentsCount[battleMode][lvlMode];
