...
void sub_8110368( struct Sprite* sprite )
{
	...
	sprite->pos1.x = GetBattlerSpriteCoord2( gBattleAnimTarget, 2 ) +
		gBattleAnimArgs[0];
	sprite->pos1.y = GetBattlerSpriteCoord2( gBattleAnimTarget, 3 ) +
		gBattleAnimArgs[1];
...
void sub_81105B4( struct Sprite* sprite )
{
	SetAverageBattlerPositions(
		gBattleAnimTarget, 0, &sprite->pos1.x, &sprite->pos1.y );
...
// Translates a stinger sprite linearly to a destination location. The sprite
// is initially rotated so that it appears to be traveling in a straight line.
...
void AnimTranslateStinger( struct Sprite* sprite )
{
	...
	{
		if( GetBattlerPosition( gBattleAnimTarget ) ==
				B_POSITION_PLAYER_LEFT ||
			GetBattlerPosition( gBattleAnimTarget ) ==
	...
	lVarX = GetBattlerSpriteCoord( gBattleAnimTarget, 2 ) +
		gBattleAnimArgs[2];
	lVarY = GetBattlerSpriteCoord( gBattleAnimTarget, 3 ) +
		gBattleAnimArgs[3];
...
static void AnimMissileArcStep( struct Sprite* sprite )
{
	...
	else
	{
		...
		if( !TranslateAnimHorizontalArc( sprite ) )
		{
			u16 rotation = ArcTan2Neg(
				sprite->pos1.x + sprite->pos2.x - x2,
				sprite->pos1.y + sprite->pos2.y - y2 );
			rotation += 0xC000;
			TrySetSpriteRotScale(
				sprite, FALSE, 0x100, 0x100, rotation );
...
void sub_8110994( struct Sprite* sprite )
{
	if( gBattleAnimArgs[0] == 0 )
	{
		...
		sprite->pos1.y =
			GetBattlerSpriteCoord( gBattleAnimAttacker, 3 ) + 18;
	}
	else
	{
		sprite->pos1.x = GetBattlerSpriteCoord( gBattleAnimTarget, 2 );
		sprite->pos1.y =
			GetBattlerSpriteCoord( gBattleAnimTarget, 3 ) + 18;
	}
