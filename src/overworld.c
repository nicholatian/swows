...
static void InitLinkPlayerEventObjectPos(
	struct EventObject* eventObj, s16 x, s16 y );
...
static void ZeroLinkPlayerEventObject(
	struct LinkPlayerEventObject* linkPlayerEventObj );
...
static void LoadTradeRoomPlayer(
	s32 linkPlayerId, s32 a2, struct TradeRoomPlayer* a3 );
...
EWRAM_DATA static struct InitialPlayerAvatarState gInitialPlayerAvatarState = {
	0};
...
static u8 ( *const gLinkPlayerMovementModes[] )( struct LinkPlayerEventObject*,
	struct EventObject*,
	u8 ) = {
...
static u8 FacingHandler_DoNothing(
	struct LinkPlayerEventObject*, struct EventObject*, u8 );
static u8 FacingHandler_DpadMovement(
	struct LinkPlayerEventObject*, struct EventObject*, u8 );
...
static void MovementStatusHandler_EnterFreeMode(
	struct LinkPlayerEventObject*, struct EventObject* );
...
void SetGameStat( u8 index, u32 value )
{
	if( index < NUM_USED_GAME_STATS )
		gSaveBlock1Ptr->gameStats[index] =
			value ^ gSaveBlock2Ptr->encryptionKey;
}
...
void ApplyNewEncryptionKeyToGameStats( u32 newKey )
{
	...
	for( i = 0; i < NUM_GAME_STATS; i++ )
		ApplyNewEncryptionKeyToWord(
			&gSaveBlock1Ptr->gameStats[i], newKey );
}
...
void LoadEventObjTemplatesFromHeader( void )
{
	...
	CpuCopy32( gMapHeader.events->eventObjects,
		...
		gMapHeader.events->eventObjectCount *
			sizeof( struct EventObjectTemplate ) );
}

void LoadSaveblockEventObjScripts( void )
{
	struct EventObjectTemplate* mapHeaderObjTemplates =
		gMapHeader.events->eventObjects;
...
static void SetWarpData(
	struct WarpData* warp, s8 mapGroup, s8 mapNum, s8 warpId, s8 x, s8 y )
{
...
struct MapHeader const* const Overworld_GetMapHeaderByGroupAndId(
...
static void SetPlayerCoordsFromWarp( void )
{
	if( gSaveBlock1Ptr->location.warpId >= 0 &&
		gSaveBlock1Ptr->location.warpId <
			gMapHeader.events->warpCount )
	{
		gSaveBlock1Ptr->pos.x =
			gMapHeader.events
				->warps[gSaveBlock1Ptr->location.warpId]
				.x;
...
void SetDynamicWarpWithCoords(
	s32 unused, s8 mapGroup, s8 mapNum, s8 warpId, s8 x, s8 y )
{
	SetWarpData(
		&gSaveBlock1Ptr->dynamicWarp, mapGroup, mapNum, warpId, x, y );
}
...
void SetWarpDestinationToHealLocation( u8 healLocationId )
{
	...
	if( warp )
		SetWarpDestination(
			warp->group, warp->map, -1, warp->x, warp->y );
}
...
void SetLastHealLocationWarp( u8 healLocationId )
{
	const struct HealLocation* healLocation =
		GetHealLocation( healLocationId );
	if( healLocation )
...
void UpdateEscapeWarp( s16 x, s16 y )
{
	...
	u8 destMapType = GetMapTypeByGroupAndId(
		sWarpDestination.mapGroup, sWarpDestination.mapNum );
	if( IsMapTypeOutdoors( currMapType ) &&
		IsMapTypeOutdoors( destMapType ) != TRUE )
...
{
	SetWarpData(
		&gSaveBlock1Ptr->escapeWarp, mapGroup, mapNum, warpId, x, y );
}
...
void SetWarpDestinationToFixedHoleWarp( s16 x, s16 y )
{
	if( IsDummyWarp( &gFixedHoleWarp ) == TRUE )
		...
		SetWarpDestination( gFixedHoleWarp.mapGroup,
			gFixedHoleWarp.mapNum,
			-1,
			x,
			y );
}
...
{
	SetWarpData( &gSaveBlock1Ptr->continueGameWarp,
		mapGroup,
		mapNum,
		warpId,
		x,
...
static bool8 SetDiveWarp( u8 dir, u16 x, u16 y )
{
	...
	{
		SetWarpDestination(
			connection->mapGroup, connection->mapNum, -1, x, y );
	}
	else
	{
...
static void mli0_load_map( u32 a1 )
{
	...
	if( a1 != 1 && isIndoors )
	{
		UpdateTVScreensOnMap(
			gBackupMapLayout.width, gBackupMapLayout.height );
...
void StoreInitialPlayerAvatarState( void )
{
	...
	gInitialPlayerAvatarState.transitionFlags =
...
static struct InitialPlayerAvatarState* GetInitialPlayerAvatarState( void )
{
	...
	u8 transitionFlags   = GetAdjustedInitialTransitionFlags(
                &gInitialPlayerAvatarState, metatileBehavior, mapType );
	playerStruct.transitionFlags = transitionFlags;
	playerStruct.direction =
		GetAdjustedInitialDirection( &gInitialPlayerAvatarState,
			transitionFlags,
			metatileBehavior,
			mapType );
	...
	u8 mapType )
{
	...
	else if( MetatileBehavior_IsSurfableWaterOrUnderwater(
			 metatileBehavior ) == TRUE )
		return PLAYER_AVATAR_FLAG_SURFING;
	else if( Overworld_IsBikingAllowed( ) != TRUE )
		return PLAYER_AVATAR_FLAG_ON_FOOT;
	else if( playerStruct->transitionFlags ==
		PLAYER_AVATAR_FLAG_MACH_BIKE )
...
static u16 GetCenterScreenMetatileBehavior( void )
{
	return MapGridGetMetatileBehaviorAt(
		gSaveBlock1Ptr->pos.x + 7, gSaveBlock1Ptr->pos.y + 7 );
}
...
u8 Overworld_GetFlashLevel( void ) { return gSaveBlock1Ptr->flashLevel; }
...
void sub_8085540( u8 var ) { sUnknown_020322D8 = var; }

u8 sub_808554C( void ) { return sUnknown_020322D8; }
...
u16 GetCurrLocationDefaultMusic( void )
{
	...
	// Play the desert music only when the sandstorm is active on Route
	// 111.
...
void Overworld_ClearSavedMusic( void ) { gSaveBlock1Ptr->savedMusic = 0; }
...
static void sub_8085810( void )
{
	if( FlagGet( FLAG_SPECIAL_FLAG_0x4001 ) != TRUE )
	{
		...
		{
			if( currentMusic == MUS_DEEPDEEP ||
				currentMusic == MUS_NAMINORI )
				return;
...
static void DoCB1_Overworld( u16 newKeys, u16 heldKeys )
{
	...
	if( !ScriptContext2_IsEnabled( ) )
	{
		...
		{
			player_step(
				inputStruct.dpadDirection, newKeys, heldKeys );
		}
	}
}
...
static void FieldClearVBlankHBlankCallbacks( void )
{
	...
	if( gWirelessCommType != 0 )
	{
		EnableInterrupts( INTR_FLAG_VBLANK | INTR_FLAG_VCOUNT |
...
static void InitOverworldGraphicsRegisters( void )
{
	...
	SetGpuReg( REG_OFFSET_WININ,
		WININ_WIN0_BG_ALL | WININ_WIN0_OBJ | WININ_WIN1_BG_ALL |
			WININ_WIN1_OBJ );
	...
	SetGpuReg( REG_OFFSET_BLDCNT,
		gOverworldBackgroundLayerFlags[1] |
			...
			gOverworldBackgroundLayerFlags[3] | BLDCNT_TGT2_OBJ |
			BLDCNT_EFFECT_BLEND );
...
static void mli4_mapscripts_and_other( void )
{
	...
	InitPlayerAvatar(
		x, y, player->direction, gSaveBlock2Ptr->playerGender );
...
static void SetCameraToTrackGuestPlayer( void )
{
	InitCameraUpdateCallback(
		GetSpriteForLinkedPlayer( gLocalLinkPlayerId ) );
}
...
static void sub_8086AE4( void )
{
	...
	// This is a hack of some kind; it's undone in sub_8086B14, which is
...
static void CB1_UpdateLinkState( void )
{
		...
		!IsSendingKeysToLink( ) )
	{
		...
		// Note: Because guestId is between 0 and 4, while the smallest
		// key code is LINK_KEY_CODE_EMPTY, this is functionally
		// equivalent to `sPlayerKeyInterceptCallback(0)`. It is
		// expecting the callback to be KeyInterCB_SelfIdle, and that
		// will completely ignore any input parameters.
		//
		// UpdateHeldKeyCode performs a sanity check on its input; if
		// sPlayerKeyInterceptCallback echoes back the argument, which
		// is selfId, then it'll use LINK_KEY_CODE_EMPTY instead.
		//
		// Note 2: There are some key intercept callbacks that treat
		// the key as a player ID. It's so hacky.
...
// Once every ~60 frames, if the link state hasn't changed (timer reset by
// calls to SetKeyInterceptCallback), it does a bunch of sanity checks on the
...
static void HandleLinkPlayerKeyInput( u32 playerId,
	u16 key,
	...
	u16* forceFacing )
{
	...
	if( sPlayerTradingStates[playerId] == PLAYER_TRADING_STATE_IDLE )
	{
		...
		if( script )
		{
			*forceFacing = GetDirectionForEventScript( script );
			sPlayerTradingStates[playerId] =
				PLAYER_TRADING_STATE_BUSY;
		...
		if( IsAnyPlayerInTradingState(
			    PLAYER_TRADING_STATE_EXITING_ROOM ) == TRUE )
		{
			sPlayerTradingStates[playerId] =
				PLAYER_TRADING_STATE_BUSY;
			if( trainer->isLocalPlayer )
	...
	case LINK_KEY_CODE_UNK_7:
		if( sPlayerTradingStates[playerId] ==
			PLAYER_TRADING_STATE_UNK_2 )
			sPlayerTradingStates[playerId] =
				PLAYER_TRADING_STATE_BUSY;
		break;
	}
...
{
	// keyOrPlayerId could be any keycode. This callback does no sanity
	// checking on the size of the key. It's assuming that it is being
	// called from CB1_UpdateLinkState.
	if( sPlayerTradingStates[keyOrPlayerId] !=
		PLAYER_TRADING_STATE_EXITING_ROOM )
		CheckRfuKeepAliveTimer( );
...
// Duplicate function.
static u16 KeyInterCB_SendNothing_2( u32 key ) { return LINK_KEY_CODE_EMPTY; }
...
u32 sub_8087214( void )
{
	...
	if( sPlayerKeyInterceptCallback == sub_8087170 &&
		sPlayerTradingStates[gLocalLinkPlayerId] !=
			PLAYER_TRADING_STATE_UNK_2 )
		return 0;
	if( sPlayerKeyInterceptCallback == KeyInterCB_DoNothingAndKeepAlive &&
		sPlayerTradingStates[gLocalLinkPlayerId] ==
			PLAYER_TRADING_STATE_BUSY )
		return 2;
	if( AreAllPlayersInTradingState( PLAYER_TRADING_STATE_UNK_2 ) !=
		FALSE )
		return 1;
	return 0;
}
...
static void LoadTradeRoomPlayer(
	s32 linkPlayerId, s32 myPlayerId, struct TradeRoomPlayer* trainer )
{
	s16 x, y;
...
static const u8* TryInteractWithPlayer( struct TradeRoomPlayer* player )
{
	...
	if( player->c != MOVEMENT_MODE_FREE &&
		player->c != MOVEMENT_MODE_SCRIPTED )
		return FACING_NONE;
	...
	if( linkPlayerId != 4 )
	{
		...
		else if( sPlayerTradingStates[linkPlayerId] !=
			PLAYER_TRADING_STATE_IDLE )
			return gEventScript_TradeRoom_TooBusyToNotice;
	...
	return GetInteractedLinkPlayerScript(
		&otherPlayerPos, player->field_C, player->facing );
}
...
static void sub_8087510( void ) { ScriptContext2_Enable( ); }
...
static void ZeroLinkPlayerEventObject(
	struct LinkPlayerEventObject* linkPlayerEventObj )
{
	memset( linkPlayerEventObj,
		0,
		sizeof( struct LinkPlayerEventObject ) );
}

void ClearLinkPlayerEventObjects( void )
{
	memset( gLinkPlayerEventObjects,
		0,
		sizeof( gLinkPlayerEventObjects ) );
}
	...
	struct EventObject* eventObj, s16 x, s16 y )
{
	...
	SetSpritePosToMapCoords(
		x, y, &eventObj->initialCoords.x, &eventObj->initialCoords.y );
...
static void sub_80877DC( u8 linkPlayerId, u8 a2 )
{
	...
	{
		u8 eventObjId =
			gLinkPlayerEventObjects[linkPlayerId].eventObjId;
		struct EventObject* eventObj = &gEventObjects[eventObjId];
...
static u8 GetLinkPlayerIdAt( s16 x, s16 y )
{
	...
	for( i = 0; i < 4; i++ )
	{
		if( gLinkPlayerEventObjects[i].active &&
			( gLinkPlayerEventObjects[i].movementMode == 0 ||
				gLinkPlayerEventObjects[i].movementMode ==
					2 ) )
		{
			struct EventObject* eventObj =
				&gEventObjects[gLinkPlayerEventObjects[i]
						       .eventObjId];
			if( eventObj->currentCoords.x == x &&
				eventObj->currentCoords.y == y )
				return i;
		}
	}
...
#define TEMP \
	gLinkPlayerMovementModes[linkPlayerEventObj->movementMode]( \
		linkPlayerEventObj, eventObj, facing )

			gMovementStatusHandler[TEMP](
				linkPlayerEventObj, eventObj );
...
static u8 MovementEventModeCB_Normal(
	...
	u8 a3 )
{
	return gLinkPlayerFacingHandlers[a3](
		linkPlayerEventObj, eventObj, a3 );
}
	...
	u8 a3 )
{
	...
	eventObj->range.as_byte =
		FlipVerticalAndClearForced( a3, eventObj->range.as_byte );
	EventObjectMoveDestCoords( eventObj, eventObj->range.as_byte, &x, &y );

	if( LinkPlayerDetectCollision( linkPlayerEventObj->eventObjId,
		    eventObj->range.as_byte,
		    x,
		    y ) )
	{
	...
	struct EventObject* eventObj )
{
	...
	MoveCoords( eventObj->range.as_byte,
		&eventObj->initialCoords.x,
		&eventObj->initialCoords.y );
	...
	for( i = 0; i < 16; i++ )
	{
		if( i != selfEventObjId )
		{
			if( ( gEventObjects[i].currentCoords.x == x &&
				...
				( gEventObjects[i].previousCoords.x == x &&
					gEventObjects[i].previousCoords.y ==
						y ) )
			{
				return 1;
			}
...
static void CreateLinkPlayerSprite( u8 linkPlayerId, u8 gameVersion )
{
	...
	if( linkPlayerEventObj->active )
	{
		...
		case VERSION_LEAF_GREEN:
			eventObj->spriteId = AddPseudoEventObject(
				GetFRLGAvatarGraphicsIdByGender(
					eventObj->singleMovementActive ),
				SpriteCB_LinkPlayer,
				0,
				0,
				0 );
...
{
	struct LinkPlayerEventObject* linkPlayerEventObj =
		&gLinkPlayerEventObjects[sprite->data[0]];
	struct EventObject* eventObj =
		&gEventObjects[linkPlayerEventObj->eventObjId];
	...
	if( !linkPlayerEventObj->movementMode != MOVEMENT_MODE_FREE )
		StartSpriteAnim( sprite,
			GetFaceDirectionAnimNum( eventObj->range.as_byte ) );
	else
		StartSpriteAnimIfDifferent( sprite,
			GetMoveDirectionAnimNum( eventObj->range.as_byte ) );
