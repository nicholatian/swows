...
static void SaveFailedScreenTextPrint( const u8* text, u8 var1, u8 var2 )
{
	...
	color[1] = 15;
	color[2] = 3;
	AddTextPrinterParameterized4( gSaveFailedWindowIds[TEXT_WIN_ID],
		1,
		var1 * 8,
		var2 * 8 + 1,
		0,
...
static void CB2_SaveFailedScreen( void )
{
	...
	default:
		...
		LZ77UnCompVram(
			gBirchBagTilemap, (void*)( BG_SCREEN_ADDR( 14 ) ) );
		LZ77UnCompVram(
			gBirchGrassTilemap, (void*)( BG_SCREEN_ADDR( 15 ) ) );
		LZ77UnCompVram(
			sSaveFailedClockGfx, (void*)( VRAM + 0x10020 ) );
		...
		gSaveFailedWindowIds[TEXT_WIN_ID] =
			AddWindowWithoutTileMap( gUnknown_085EFD9C );
		SetWindowAttribute( gSaveFailedWindowIds[TEXT_WIN_ID],
			7,
			(u32)&gDecompressionBuffer[0x2800] );
		gSaveFailedWindowIds[CLOCK_WIN_ID] =
			AddWindowWithoutTileMap( gUnknown_085EFDA4 );
		SetWindowAttribute( gSaveFailedWindowIds[CLOCK_WIN_ID],
			7,
		...
		DrawStdFrameWithCustomTileAndPalette(
			...
			0x214,
			0xE );
		FillWindowPixelBuffer( gSaveFailedWindowIds[CLOCK_WIN_ID],
			PIXEL_FILL( 1 ) ); // backwards?
		FillWindowPixelBuffer(
			gSaveFailedWindowIds[TEXT_WIN_ID], PIXEL_FILL( 1 ) );
		CopyWindowToVram(
			gSaveFailedWindowIds[CLOCK_WIN_ID], 2 ); // again?
		CopyWindowToVram( gSaveFailedWindowIds[TEXT_WIN_ID], 1 );
		SaveFailedScreenTextPrint(
			gText_SaveFailedCheckingBackup, 1, 0 );
...
static void CB2_WipeSave( void )
{
	...
	else
	{
		...
		if( gGameContinueCallback == NULL )
			SaveFailedScreenTextPrint(
				gText_SaveCompleteGameCannotContinue, 1, 0 );
		else
			SaveFailedScreenTextPrint(
				gText_SaveCompletePressA, 1, 0 );
	}
...
static void CB2_GameplayCannotBeContinued( void )
{
	...
	if( gMain.newKeys & A_BUTTON )
	{
		FillWindowPixelBuffer(
			gSaveFailedWindowIds[TEXT_WIN_ID], PIXEL_FILL( 1 ) );
		SaveFailedScreenTextPrint(
			gText_GamePlayCannotBeContinued, 1, 0 );
		SetVBlankCallback( VBlankCB );
		SetMainCallback2( CB2_FadeAndReturnToTitleScreen );
	}
}
...
static void CB2_ReturnToTitleScreen( void )
{
	if( !UpdatePaletteFade( ) )
	{
		if( gGameContinueCallback == NULL ) // no callback exists, so
						    // do a soft reset.
		{
			DoSoftReset( );
		}
		...
		{
			SetMainCallback2(
				(MainCallback)gGameContinueCallback );
...
static void VBlankCB_UpdateClockGraphics( void )
{
	...
	if( gSaveFailedClockInfo[CLOCK_RUNNING] != FALSE )
	{
		...
		gMain.oamBuffer[0].matrixNum = ( sClockFrames[n][2] << 4 ) |
			( sClockFrames[n][1] << 3 );
	}
	else
	{
