...
static u8 CreateSpriteAt( u8 index,
...
#define DUMMY_OAM_DATA \
	{ \
		.y = 160, .affineMode = 0, .objMode = 0, .mosaic = 0, \
		.bpp = 0, .shape = SPRITE_SHAPE( 8x8 ), .x = 304, \
		.matrixNum = 0, .size = SPRITE_SIZE( 8x8 ), .tileNum = 0, \
		.priority = 3, /* lowest \
				  priority */ \
...
void UpdateOamCoords( void )
{
	...
	for( i = 0; i < MAX_SPRITES; i++ )
	{
		...
		if( sprite->inUse && !sprite->invisible )
		{
			...
			{
				sprite->oam.x = sprite->pos1.x +
					sprite->pos2.x +
					sprite->centerToCornerVecX +
					gSpriteCoordOffsetX;
				sprite->oam.y = sprite->pos1.y +
					sprite->pos2.y +
...
void SortSprites( void )
{
	...
	for( i = 1; i < MAX_SPRITES; i++ )
	{
					...
					sprite1Y < sprite2Y ) ) )
		{
			...
			sprite1Priority =
				sSpritePriorities[sSpriteOrder[j - 1]];
...
u8 CreateSprite(
	const struct SpriteTemplate* template, s16 x, s16 y, u8 subpriority )
{
	u8 i;

	for( i = 0; i < MAX_SPRITES; i++ )
		if( !gSprites[i].inUse )
			return CreateSpriteAt(
				i, template, x, y, subpriority );

	return MAX_SPRITES;
}

u8 CreateSpriteAtEnd(
	const struct SpriteTemplate* template, s16 x, s16 y, u8 subpriority )
{
	s16 i;
	...
	u8 subpriority )
{
	...
	if( template->tileTag == 0xFFFF )
	{
		...
		tileNum        = AllocSpriteTiles(
                        ( u8 )( sprite->images->size / TILE_SIZE_4BPP ) );
		if( tileNum == -1 )
		{
			ResetSprite( sprite );
			return MAX_SPRITES;
	...
	else
	{
		sprite->sheetTileStart =
			GetSpriteTileStartByTag( template->tileTag );
	...
	if( template->paletteTag != 0xFFFF )
		sprite->oam.paletteNum =
			IndexOfSpritePaletteTag( template->paletteTag );

	return index;
}

u8 CreateSpriteAndAnimate(
	const struct SpriteTemplate* template, s16 x, s16 y, u8 subpriority )
{
	u8 i;
...
void LoadOam( void )
{
	if( !gMain.oamLoadDisabled )
		CpuCopy32( gMain.oamBuffer,
			(void*)OAM,
			sizeof( gMain.oamBuffer ) );
}
...
s16 AllocSpriteTiles( u16 tileCount )
{
	...
	if( tileCount == 0 )
	{
		...
		for( i = gReservedSpriteTileCount; i < TOTAL_OBJ_TILE_COUNT;
			i++ )
			FREE_SPRITE_TILE( i );
...
void BeginAnim( struct Sprite* sprite )
{
	...
	imageValue = sprite->anims[sprite->animNum][sprite->animCmdIndex]
			     .frame.imageValue;

	if( imageValue != -1 )
	{
		sprite->animBeginning = FALSE;
		duration = sprite->anims[sprite->animNum][sprite->animCmdIndex]
				   .frame.duration;
		hFlip = sprite->anims[sprite->animNum][sprite->animCmdIndex]
				.frame.hFlip;
		vFlip = sprite->anims[sprite->animNum][sprite->animCmdIndex]
				.frame.vFlip;
		...
		if( sprite->usingSheet )
			sprite->oam.tileNum =
				sprite->sheetTileStart + imageValue;
		else
			RequestSpriteFrameImageCopy( imageValue,
				sprite->oam.tileNum,
				sprite->images );
	}
}
...
void ContinueAnim( struct Sprite* sprite )
{
	...
	else if( !sprite->animPaused )
	{
		...
		s16 funcIndex;
		sprite->animCmdIndex++;
		type = sprite->anims[sprite->animNum][sprite->animCmdIndex]
			       .type;
		funcIndex = 3;
		if( type < 0 )
			funcIndex = type + 3;
...
void AnimCmd_frame( struct Sprite* sprite )
{
	...
	if( sprite->usingSheet )
		...
		RequestSpriteFrameImageCopy(
			imageValue, sprite->oam.tileNum, sprite->images );
}
...
void AnimCmd_jump( struct Sprite* sprite )
{
	...
	sprite->animCmdIndex =
		sprite->anims[sprite->animNum][sprite->animCmdIndex]
			.jump.target;
...
void JumpToTopOfAnimLoop( struct Sprite* sprite )
{
	if( sprite->animLoopCounter )
	{
		...
		while( sprite->anims[sprite->animNum][sprite->animCmdIndex - 1]
				.type != -3 )
		{
...
void BeginAffineAnim( struct Sprite* sprite )
{
		...
		sprite->affineAnims[0][0].type != 32767 )
	{
		...
		if( sprite->flags_f )
			obj_update_pos2(
				sprite, sprite->data[6], sprite->data[7] );
	}
}
...
void ContinueAffineAnim( struct Sprite* sprite )
{
	if( sprite->oam.affineMode & ST_OAM_AFFINE_ON_MASK )
	{
		...
		else
		{
			...
			type = sprite->affineAnims
				       [sAffineAnimStates[matrixNum].animNum]
				       [sAffineAnimStates[matrixNum]
						       .animCmdIndex]
					       .type;
			funcIndex = 3;
			if( type >= 32765 )
...
void BeginAffineAnimLoop( u8 matrixNum, struct Sprite* sprite )
{
	sAffineAnimStates[matrixNum].loopCounter =
		sprite->affineAnims[sAffineAnimStates[matrixNum].animNum]
				   [sAffineAnimStates[matrixNum].animCmdIndex]
					   .loop.count;
...
void obj_update_pos2( struct Sprite* sprite, s32 a1, s32 a2 )
{
	...
	if( a1 != 0x800 )
	{
		var0 = sUnknown_082EC6F4[sprite->oam.shape][sprite->oam.size]
					[0];
		var1           = var0 << 8;
...
void ApplyAffineAnimFrameAbsolute(
	u8 matrixNum, struct AffineAnimFrameCmd* frameCmd )
{
	...
	u8 matrixNum, struct AffineAnimFrameCmd* frameCmd )
{
	...
	sAffineAnimStates[matrixNum].rotation =
		...
		~0xFF;
	srcData.xScale =
		ConvertScaleParam( sAffineAnimStates[matrixNum].xScale );
	srcData.yScale =
		ConvertScaleParam( sAffineAnimStates[matrixNum].yScale );
...
void GetAffineAnimFrame( u8 matrixNum,
	struct Sprite* sprite,
	struct AffineAnimFrameCmd* frameCmd )
{
	frameCmd->xScale =
...
void SetOamMatrixRotationScaling(
	u8 matrixNum, s16 xScale, s16 yScale, u16 rotation )
{
	struct ObjAffineSrcData srcData;
	struct OamMatrix matrix;
...
u16 LoadSpriteSheet( const struct SpriteSheet* sheet )
{
	...
	{
		AllocSpriteTileRange( sheet->tag,
			(u16)tileStart,
			sheet->size / TILE_SIZE_4BPP );
		CpuCopy16( sheet->data,
...
u16 GetSpriteTileTagByTileStart( u16 start )
{
	...
	for( i = 0; i < MAX_SPRITES; i++ )
	{
		if( sSpriteTileRangeTags[i] != 0xFFFF &&
...
void SetSubspriteTables(
	struct Sprite* sprite, const struct SubspriteTable* subspriteTables )
{
...
bool8 AddSpriteToOamBuffer( struct Sprite* sprite, u8* oamIndex )
{
	...
	{
		return AddSubspritesToOamBuffer(
			sprite, &gMain.oamBuffer[*oamIndex], oamIndex );
	}
}

bool8 AddSubspritesToOamBuffer(
	struct Sprite* sprite, struct OamData* destOam, u8* oamIndex )
{
	...
	else
	{
		...
		for( i = 0; i < subspriteCount; i++, ( *oamIndex )++ )
		{
			...
			destOam[i].y     = baseY + y;
			destOam[i].tileNum = tileNum +
				subspriteTable->subsprites[i].tileOffset;

			if( sprite->subspriteMode !=
				SUBSPRITES_IGNORE_PRIORITY )
				destOam[i].priority =
					subspriteTable->subsprites[i].priority;
		}
	}
