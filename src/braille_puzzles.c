	...
	if( !FlagGet( FLAG_SYS_BRAILLE_DIG ) &&
		( gSaveBlock1Ptr->location.mapGroup ==
			...
			gSaveBlock1Ptr->location.mapNum ==
				MAP_NUM( SEALED_CHAMBER_OUTER_ROOM ) ) )
	{
...
bool8 CheckRelicanthWailord( void )
{
	...
	// First comes Wailord
	if( GetMonData( &gPlayerParty[0], MON_DATA_SPECIES2, 0 ) ==
		SPECIES_WAILORD )
	{
...
// functions but leaving the call to it itself, which creates the nullsub. the
// code is shown below to show what this might look like.
...
(gSaveBlock1.location.mapGroup == MAP_GROUP_ANCIENT_TOMB &&
...
bool8 ShouldDoBrailleRegirockEffect( void )
{
		...
		gSaveBlock1Ptr->location.mapNum == MAP_NUM( DESERT_RUINS ) )
	{
		...
		else if( gSaveBlock1Ptr->pos.x == 5 &&
			gSaveBlock1Ptr->pos.y == 23 )
		{
			sBraillePuzzleCallbackFlag = REGIROCK_PUZZLE;
			return TRUE;
		}
		else if( gSaveBlock1Ptr->pos.x == 7 &&
			gSaveBlock1Ptr->pos.y == 23 )
		{
			sBraillePuzzleCallbackFlag = REGIROCK_PUZZLE;
			return TRUE;
...
bool8 ShouldDoBrailleRegisteelEffect( void )
{
	if( !FlagGet( FLAG_SYS_REGISTEEL_PUZZLE_COMPLETED ) &&
		( gSaveBlock1Ptr->location.mapGroup ==
				MAP_GROUP( ANCIENT_TOMB ) &&
			gSaveBlock1Ptr->location.mapNum ==
				MAP_NUM( ANCIENT_TOMB ) ) )
	{
...
{
	u16 keyMask = A_BUTTON | B_BUTTON | START_BUTTON | SELECT_BUTTON |
DPAD_ANY;
...
bool8 ShouldDoBrailleRegicePuzzle( void )
{
		...
		gSaveBlock1Ptr->location.mapNum == MAP_NUM( ISLAND_CAVE ) )
	{
		...
		for( i = 0; i < 36; i++ )
		{
			...
			if( gSaveBlock1Ptr->pos.x == xPos &&
				gSaveBlock1Ptr->pos.y == yPos )
			{
				u16 varValue;
