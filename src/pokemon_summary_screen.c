...
static EWRAM_DATA struct PokemonSummaryScreenData
{
	...
	u8 minPageIndex;
	u8 maxPageIndex;
	bool8 lockMonFlag; // This is used to prevent the player from changing
			   // pokemon in the move deleter select, etc, but it
			   // is not needed because the input is handled
			   // differently there
	u16 newMove;
	u8 firstMoveIndex;
	u8 secondMoveIndex;
	bool8 unk40C8;
	u8 bgDisplayOrder; // Determines the order page backgrounds are loaded
	...
	s16 switchCounter; // Used for various switch statement cases that
			   // decompress/load graphics or pokemon data
	u8 unk_filler4[6];
...
static void SwapBoxMonMoves(
	struct BoxPokemon* mon, u8 moveIndex1, u8 moveIndex2 );
...
static const u16 sUnknown_0861CBC4[] =
	INCBIN_U16( "graphics/interface/unk_tilemap2.bin" );
...
static const struct WindowTemplate sPageMovesTemplate[] = // This is used for
							  // both battle and
							  // contest moves
	{
...
static const u8 sTextColors_861CD2C[][3] = {{0, 1, 2},
	{0, 3, 4},
	{0, 5, 6},
	{0, 7, 8},
	{0, 9, 10},
...
static const u8 sSummaryAButtonBitmap[] =
	INCBIN_U8( "graphics/interface/summary_a_button.4bpp" );
static const u8 sSummaryBButtonBitmap[] =
	INCBIN_U8( "graphics/interface/summary_b_button.4bpp" );

static void ( *const sTextPrinterFunctions[] )( void ) = {PrintInfoPageText,
	PrintSkillsPageText,
	PrintBattleMoves,
	PrintContestMoves};

static void ( *const sTextPrinterTasks[] )( u8 taskId ) = {Task_PrintInfoPage,
	Task_PrintSkillsPage,
	Task_PrintBattleMoves,
	Task_PrintContestMoves};

static const u8 sMemoNatureTextColor[] =
	_( "{COLOR LIGHT_RED}{SHADOW GREEN}" );
static const u8 sMemoMiscTextColor[] =
	_( "{COLOR WHITE}{SHADOW DARK_GREY}" ); // This is also affected by
						// palettes, apparently
static const u8 sStatsLeftColumnLayout[] = _(
	"{SPECIAL_F7 0x00}/{SPECIAL_F7 0x01}\n{SPECIAL_F7 0x02}\n{SPECIAL_F7 0x03}" );
static const u8 sStatsRightColumnLayout[] =
	_( "{SPECIAL_F7 0x00}\n{SPECIAL_F7 0x01}\n{SPECIAL_F7 0x02}" );
static const u8 sMovesPPLayout[] =
	_( "{PP}{SPECIAL_F7 0x00}/{SPECIAL_F7 0x01}" );
...
static const union AnimCmd sSpriteAnim_CategoryCool[] = {
	ANIMCMD_FRAME( ( CONTEST_CATEGORY_COOL + NUMBER_OF_MON_TYPES ) * 8,
		0,
		FALSE,
		FALSE ),
	ANIMCMD_END};
static const union AnimCmd sSpriteAnim_CategoryBeauty[] = {
	ANIMCMD_FRAME( ( CONTEST_CATEGORY_BEAUTY + NUMBER_OF_MON_TYPES ) * 8,
		0,
		FALSE,
		FALSE ),
	ANIMCMD_END};
static const union AnimCmd sSpriteAnim_CategoryCute[] = {
	ANIMCMD_FRAME( ( CONTEST_CATEGORY_CUTE + NUMBER_OF_MON_TYPES ) * 8,
		0,
		FALSE,
		FALSE ),
	ANIMCMD_END};
static const union AnimCmd sSpriteAnim_CategorySmart[] = {
	ANIMCMD_FRAME( ( CONTEST_CATEGORY_SMART + NUMBER_OF_MON_TYPES ) * 8,
		0,
		FALSE,
		FALSE ),
	ANIMCMD_END};
static const union AnimCmd sSpriteAnim_CategoryTough[] = {
	ANIMCMD_FRAME( ( CONTEST_CATEGORY_TOUGH + NUMBER_OF_MON_TYPES ) * 8,
		0,
		FALSE,
		FALSE ),
	ANIMCMD_END};
static const union AnimCmd* const
	sSpriteAnimTable_MoveTypes[NUMBER_OF_MON_TYPES +
...
static const struct CompressedSpriteSheet sMoveSelectorSpriteSheet = {
	.data = gMoveSelectorBitmap, .size = 0x400, .tag = 30000};
static const struct CompressedSpritePalette gUnknown_0861D07C = {
	.data = gUnknown_08D97CF4, .tag = 30000};
static const struct SpriteTemplate gUnknown_0861D084 = {.tileTag = 30000,
	.paletteTag                                              = 30000,
...
static const struct CompressedSpriteSheet sStatusIconsSpriteSheet = {
	.data = gStatusGfx_Icons, .size = 0x380, .tag = 30001};
static const struct CompressedSpritePalette sStatusIconsSpritePalette = {
	.data = gStatusPal_Icons, .tag = 30001};
...
// code
void ShowPokemonSummaryScreen( u8 mode,
	void* mons,
	u8 monIndex,
	u8 maxMonIndex,
	void ( *callback )( void ) )
...
void ShowSelectMovePokemonSummaryScreen( struct Pokemon* mons,
	u8 monIndex,
	u8 maxMonIndex,
	void ( *callback )( void ),
	u16 newMove )
{
	ShowPokemonSummaryScreen(
		PSS_MODE_SELECT_MOVE, mons, monIndex, maxMonIndex, callback );
	sMonSummaryScreen->newMove = newMove;
}

void ShowPokemonSummaryScreenSet40EF( u8 mode,
...
{
	ShowPokemonSummaryScreen(
		mode, mons, monIndex, maxMonIndex, callback );
...
static void SummaryScreen_LoadingCB2( void )
{
	while( sub_81221EC( ) != TRUE &&
		SummaryScreen_LoadGraphics( ) != TRUE &&
		sub_81221AC( ) != TRUE )
		;
}
...
static bool8 SummaryScreen_LoadGraphics( void )
{
	...
	case 10:
		if( ExtractMonDataToSummaryStruct(
			    &sMonSummaryScreen->currentMon ) != 0 )
			gMain.state++;
		break;
	case 11:
		sub_81C25E8( );
	...
	case 17:
		sMonSummaryScreen->spriteIds[0] =
			CreatePokemonSprite( &sMonSummaryScreen->currentMon,
...
static void InitBGs( void )
{
	...
	InitBgsFromTemplates(
		0, sUnknown_0861CBB4, ARRAY_COUNT( sUnknown_0861CBB4 ) );
	SetBgTilemapBuffer( 1,
		sMonSummaryScreen
			->bgTilemapBuffers[PSS_PAGE_BATTLE_MOVES][0] );
	SetBgTilemapBuffer(
		2, sMonSummaryScreen->bgTilemapBuffers[PSS_PAGE_SKILLS][0] );
	SetBgTilemapBuffer(
		3, sMonSummaryScreen->bgTilemapBuffers[PSS_PAGE_INFO][0] );
...
static bool8 SummaryScreen_DecompressGraphics( void )
{
	...
	case 0:
		...
		decompress_and_copy_tile_data_to_vram(
			1, &gStatusScreenBitmap, 0, 0, 0 );
		sMonSummaryScreen->switchCounter++;
		break;
	case 1:
		if( free_temp_tile_data_buffers_if_possible( ) != 1 )
		{
			LZDecompressWram( gPageInfoTilemap,
				sMonSummaryScreen
					->bgTilemapBuffers[PSS_PAGE_INFO][0] );
			sMonSummaryScreen->switchCounter++;
		}
		break;
	case 2:
		LZDecompressWram( gUnknown_08D98CC8,
			sMonSummaryScreen
				->bgTilemapBuffers[PSS_PAGE_INFO][1] );
		sMonSummaryScreen->switchCounter++;
		break;
	case 3:
		LZDecompressWram( gPageSkillsTilemap,
			sMonSummaryScreen
				->bgTilemapBuffers[PSS_PAGE_SKILLS][1] );
		sMonSummaryScreen->switchCounter++;
		break;
	case 4:
		LZDecompressWram( gPageBattleMovesTilemap,
			sMonSummaryScreen
				->bgTilemapBuffers[PSS_PAGE_BATTLE_MOVES][1] );
		sMonSummaryScreen->switchCounter++;
		break;
	case 5:
		LZDecompressWram( gPageContestMovesTilemap,
			sMonSummaryScreen
				->bgTilemapBuffers[PSS_PAGE_CONTEST_MOVES]
						  [1] );
		sMonSummaryScreen->switchCounter++;
		break;
	case 6:
...
static bool8 ExtractMonDataToSummaryStruct( struct Pokemon* mon )
{
	...
	case 3:
		...
		ConvertInternationalString( (u8*)&sum->OTName,
...
static void sub_81C0348( void )
{
	...
	else
	{
		DrawContestMoveHearts(
			sMonSummaryScreen->summary
				.moves[sMonSummaryScreen->firstMoveIndex] );
		sub_81C2194(
			sMonSummaryScreen
				->bgTilemapBuffers[PSS_PAGE_BATTLE_MOVES][0],
			3,
		...
		sub_81C2194(
			sMonSummaryScreen
				->bgTilemapBuffers[PSS_PAGE_CONTEST_MOVES][0],
			1,
			0 );
		SetBgTilemapBuffer( 1,
			sMonSummaryScreen
...
static void ChangeSummaryPokemon( u8 taskId, s8 delta )
{
	...
	if( !sMonSummaryScreen->lockMonFlag )
	{
		if( sMonSummaryScreen->isBoxMon == TRUE )
		{
			...
			v1 = sub_80D214C( sMonSummaryScreen->monList.boxMons,
		...
		{
			PlaySE( SE_SELECT );
			if( sMonSummaryScreen->summary.ailment !=
				AILMENT_NONE )
			{
...
static void sub_81C0704( u8 taskId )
{
	...
	case 1:
		...
		DestroySpriteAndFreeResources(
			&gSprites[sMonSummaryScreen->spriteIds[0]] );
		break;
	case 2:
		DestroySpriteAndFreeResources(
	...
	case 4:
		if( ExtractMonDataToSummaryStruct(
			    &sMonSummaryScreen->currentMon ) == FALSE )
			return;
		break;
	case 5:
	...
	case 7:
		...
		break;
	case 8:
		sMonSummaryScreen->spriteIds[0] = CreatePokemonSprite(
			&sMonSummaryScreen->currentMon, &data[1] );
		if( sMonSummaryScreen->spriteIds[0] == 0xFF )
			return;
	...
	default:
		if( sub_81221EC( ) == 0 &&
			FuncIsActiveTask( sub_81C20F0 ) == 0 )
		{
			data[0]             = 0;
...
static void ChangePage( u8 taskId, s8 delta )
{
	...
	if( summary->isEgg )
		...
		sMonSummaryScreen->currPageIndex ==
			sMonSummaryScreen->minPageIndex )
		return;
	else if( delta == 1 &&
		sMonSummaryScreen->currPageIndex ==
	...
	if( delta == 1 )
		SetTaskFuncWithFollowupFunc(
			taskId, PssScrollRight, gTasks[taskId].func );
	else
		SetTaskFuncWithFollowupFunc(
			taskId, PssScrollLeft, gTasks[taskId].func );
...
static void PssScrollRight( u8 taskId ) // Scroll right
{
	...
	if( data[0] == 0 )
	{
		...
		SetBgTilemapBuffer( data[1],
			sMonSummaryScreen->bgTilemapBuffers
...
static void PssScrollLeftEnd( u8 taskId ) // display left
{
	...
	{
		SetBgTilemapBuffer( data[1],
			(u8*)sMonSummaryScreen +
				( ( sMonSummaryScreen->currPageIndex << 12 ) +
					0xFFFFF0BC ) );
		ChangeBgX( data[1], 0x10000, 0 );
	}
	ShowBg( 1 );
...
static void sub_81C0E48( u8 taskId )
{
	...
	move                              = sMonSummaryScreen->summary
		       .moves[sMonSummaryScreen->firstMoveIndex];
	ClearWindowTilemap( PSS_LABEL_WINDOW_PORTRAIT_SPECIES );
...
static void HandleInput_MoveSelect( u8 taskId )
{
	...
	if( sub_81221EC( ) != 1 )
	{
		if( gMain.newKeys & DPAD_UP )
		{
			data[0] = 4;
			sub_81C1070(
				data, -1, &sMonSummaryScreen->firstMoveIndex );
		}
		else if( gMain.newKeys & DPAD_DOWN )
		{
			data[0] = 4;
...
static void sub_81C1070( s16* a, s8 b, u8* c )
{
	...
	schedule_bg_copy_tilemap_to_vram( 2 );
	PrintMoveDetails( move );
	if( ( *c == 4 && sMonSummaryScreen->newMove == MOVE_NONE ) ||
		a[1] == 1 )
	{
		ClearWindowTilemap( PSS_LABEL_WINDOW_PORTRAIT_SPECIES );
...
static void sub_81C13B0( u8 taskId, bool8 b )
{
	...
	if( b == TRUE )
	{
		if( !sMonSummaryScreen->isBoxMon )
		{
			...
			SwapMonMoves( &why[sMonSummaryScreen->curMonIndex],
				sMonSummaryScreen->firstMoveIndex,
				sMonSummaryScreen->secondMoveIndex );
		}
		else
		{
			struct BoxPokemon* why =
				sMonSummaryScreen->monList.boxMons;
			SwapBoxMonMoves( &why[sMonSummaryScreen->curMonIndex],
				sMonSummaryScreen->firstMoveIndex,
				sMonSummaryScreen->secondMoveIndex );
		}
		CopyMonToSummaryStruct( &sMonSummaryScreen->currentMon );
		sub_81C40A0( sMonSummaryScreen->firstMoveIndex,
			sMonSummaryScreen->secondMoveIndex );
...
static void SwapMonMoves( struct Pokemon* mon, u8 moveIndex1, u8 moveIndex2 )
{
	...
	ppBonuses &= ~ppUpMask1;
	ppBonuses &= ~ppUpMask2;
	ppBonuses |= ( ppBonusMove1 << ( moveIndex2 * 2 ) ) +
		( ppBonusMove2 << ( moveIndex1 * 2 ) );
...
// This redraws the power/accuracy window when the player scrolls out of the
// "HM Moves can't be forgotten" message
static void HandleHMMovesCantBeForgottenInput( u8 taskId )
{
	s16* data = gTasks[taskId].data;
	u16 move;
	if( FuncIsActiveTask( sub_81C1E20 ) != 1 )
	{
		...
		else if( gMain.newKeys & DPAD_LEFT || GetLRKeysState( ) == 1 )
		{
			if( sMonSummaryScreen->currPageIndex != 2 )
			{
				ClearWindowTilemap(
					PSS_LABEL_WINDOW_PORTRAIT_SPECIES );
				if( !gSprites[sMonSummaryScreen->spriteIds[2]]
						.invisible )
					ClearWindowTilemap(
						PSS_LABEL_WINDOW_POKEMON_SKILLS_STATUS );
				move = sMonSummaryScreen->summary
					       .moves[sMonSummaryScreen
							       ->firstMoveIndex];
				gTasks[taskId].func = HandleReplaceMoveInput;
				ChangePage( taskId, -1 );
				sub_81C1DA4( 9, -2 );
		...
		else if( gMain.newKeys & ( A_BUTTON | B_BUTTON ) )
		{
			...
			if( !gSprites[sMonSummaryScreen->spriteIds[2]]
				...
				ClearWindowTilemap(
					PSS_LABEL_WINDOW_POKEMON_SKILLS_STATUS );
			move = sMonSummaryScreen->summary.moves
				       [sMonSummaryScreen->firstMoveIndex];
			PrintMoveDetails( move );
			schedule_bg_copy_tilemap_to_vram( 0 );
			sub_81C1DA4( 9, -3 );
...
u8 sub_81C1B94( void ) { return sUnknown_0203CF21; }

static void DrawPagination( void ) // Updates the pagination dots at the top of
				   // the summary screen
{
...
static void sub_81C1CB0(
	const struct UnkStruct_61CC04* unkStruct, u16* dest, u8 c, bool8 d )
{
	u16 i;
	u16* alloced = Alloc( unkStruct->field_6 * 2 * unkStruct->field_7 );
	CpuFill16( unkStruct->field_4,
		alloced,
...
static void sub_81C2074( u16 a, s16 b )
{
	...
	{
		sub_81C1CB0( &sUnknown_0861CBEC,
			sMonSummaryScreen->bgTilemapBuffers[PSS_PAGE_INFO][0],
			b,
			0 );
		sub_81C1CB0( &sUnknown_0861CBF8,
			sMonSummaryScreen->bgTilemapBuffers[PSS_PAGE_INFO][0],
			b,
			0 );
	}
...
static void sub_81C2194( u16* output, u16 palette, bool8 c )
{
	...
	if( c == 0 )
	{
		...
		{
			output[var + i] =
				gSummaryScreenWindow_Tilemap[i] + palette;
			output[var + i + 0x20] =
				gSummaryScreenWindow_Tilemap[i] + palette;
			output[var + i + 0x40] =
				gSummaryScreenWindow_Tilemap[i + 20] + palette;
		}
	}
	else
	{
		for( i = 0; i < 20; i++ )
		{
			output[var + i] =
				gSummaryScreenWindow_Tilemap[i + 20] + palette;
			output[var + i + 0x20] =
				gSummaryScreenWindow_Tilemap[i + 40] + palette;
			output[var + i + 0x40] =
				gSummaryScreenWindow_Tilemap[i + 40] + palette;
		}
	}
...
static void DrawPokerusCuredSymbol( struct Pokemon* mon ) // This checks if the
							  // mon has been cured
							  // of pokerus
{
	if( !CheckPartyPokerus( mon, 0 ) &&
		CheckPartyHasHadPokerus(
			mon, 0 ) ) // If yes it draws the cured symbol
	{
		sMonSummaryScreen->bgTilemapBuffers[PSS_PAGE_INFO][0][0x223] =
			0x2C;
		sMonSummaryScreen->bgTilemapBuffers[PSS_PAGE_INFO][1][0x223] =
			0x2C;
	}
	else
	{
...
static void DrawExperienceProgressBar( struct Pokemon* unused )
{
	...
	if( summary->level < MAX_LEVEL )
	{
		u32 expBetweenLevels =
			gExperienceTables[gBaseStats[summary->species]
						  .growthRate]
					 [summary->level + 1] -
			gExperienceTables[gBaseStats[summary->species]
						  .growthRate][summary->level];
		u32 expSinceLastLevel = summary->exp -
			gExperienceTables[gBaseStats[summary->species]
						  .growthRate][summary->level];

		// Calculate the number of 1-pixel "ticks" to illuminate in the
		// experience progress bar. There are 8 tiles that make up the
		// bar, and each tile has 8 "ticks". Hence, the numerator is
		// multiplied by 64.
		numExpProgressBarTicks =
			expSinceLastLevel * 64 / expBetweenLevels;
...
static void DrawContestMoveHearts( u16 move )
{
	...
	{
		effectValue =
			gContestEffects[gContestMoves[move].effect].appeal;
...
static void LimitEggSummaryPageDisplay( void ) // If the pokemon is an egg,
...
static void SummaryScreen_PrintTextOnWindow(
	u8 windowId, const u8* string, u8 x, u8 y, u8 lineSpacing, u8 colorId )
{
...
static void sub_81C25E8( void )
{
	FillWindowPixelBuffer(
		PSS_LABEL_WINDOW_PORTRAIT_DEX_NUMBER, PIXEL_FILL( 0 ) );
	FillWindowPixelBuffer(
		PSS_LABEL_WINDOW_PORTRAIT_NICKNAME, PIXEL_FILL( 0 ) );
	FillWindowPixelBuffer(
		PSS_LABEL_WINDOW_PORTRAIT_SPECIES, PIXEL_FILL( 0 ) );
	if( !sMonSummaryScreen->summary.isEgg )
		sub_81C2628( );
	else
		sub_81C2794( );
...
static void sub_81C2628( void )
{
	...
	StringAppend( gStringVar1, gStringVar2 );
	SummaryScreen_PrintTextOnWindow( PSS_LABEL_WINDOW_PORTRAIT_SPECIES,
		gStringVar1,
		0x18,
		17,
		0,
	...
	SummaryScreen_PrintTextOnWindow(
		PSS_LABEL_WINDOW_PORTRAIT_NICKNAME, gStringVar1, 0, 1, 0, 1 );
	strArray[0] = CHAR_SLASH;
	StringCopy( &strArray[1], &gSpeciesNames[summary->species2][0] );
	SummaryScreen_PrintTextOnWindow(
		PSS_LABEL_WINDOW_PORTRAIT_SPECIES, &strArray[0], 0, 1, 0, 1 );
...
static void PrintAOrBButtonIcon( u8 windowId, bool8 bButton, u32 x )
{
	...
	BlitBitmapToWindow( windowId,
		( bButton ) ? sSummaryBButtonBitmap
			    : sSummaryBButtonBitmap - 0x80,
		x,
		0,
		16,
		16 );
...
static void PrintPageNamesAndStatsPageToWindows( void )
{
	...
	SummaryScreen_PrintTextOnWindow( PSS_LABEL_WINDOW_POKEMON_INFO_TITLE,
		gText_PkmnInfo,
		2,
		1,
		0,
		1 );
	SummaryScreen_PrintTextOnWindow( PSS_LABEL_WINDOW_POKEMON_SKILLS_TITLE,
		gText_PkmnSkills,
		2,
		1,
		0,
		1 );
	SummaryScreen_PrintTextOnWindow( PSS_LABEL_WINDOW_BATTLE_MOVES_TITLE,
		gText_BattleMoves,
		2,
		1,
		0,
		1 );
	SummaryScreen_PrintTextOnWindow( PSS_LABEL_WINDOW_CONTEST_MOVES_TITLE,
		gText_ContestMoves,
		2,
		1,
		0,
	...
	SummaryScreen_PrintTextOnWindow( PSS_LABEL_WINDOW_PROMPT_CANCEL,
		gText_Cancel2,
		stringXPos,
		1,
		0,
	...
	SummaryScreen_PrintTextOnWindow( PSS_LABEL_WINDOW_PROMPT_INFO,
		gText_Info,
		stringXPos,
		1,
		0,
	...
	SummaryScreen_PrintTextOnWindow( PSS_LABEL_WINDOW_PROMPT_SWITCH,
		gText_Switch,
		stringXPos,
		1,
		0,
	...
	SummaryScreen_PrintTextOnWindow( PSS_LABEL_WINDOW_POKEMON_INFO_RENTAL,
		gText_RentalPkmn,
		0,
		1,
		0,
		1 );
	SummaryScreen_PrintTextOnWindow( PSS_LABEL_WINDOW_POKEMON_INFO_TYPE,
		gText_TypeSlash,
		0,
		1,
		0,
	...
	SummaryScreen_PrintTextOnWindow(
		...
		0,
		1 );
	SummaryScreen_PrintTextOnWindow( PSS_LABEL_WINDOW_POKEMON_SKILLS_EXP,
		gText_ExpPoints,
		6,
		1,
		0,
	...
	SummaryScreen_PrintTextOnWindow(
		...
		1 );
	SummaryScreen_PrintTextOnWindow(
		PSS_LABEL_WINDOW_MOVES_POWER_ACC, gText_Power, 0, 1, 0, 1 );
	SummaryScreen_PrintTextOnWindow( PSS_LABEL_WINDOW_MOVES_POWER_ACC,
		gText_Accuracy2,
		0,
		17,
		0,
		1 );
	SummaryScreen_PrintTextOnWindow(
		PSS_LABEL_WINDOW_MOVES_APPEAL_JAM, gText_Appeal, 0, 1, 0, 1 );
	SummaryScreen_PrintTextOnWindow(
		PSS_LABEL_WINDOW_MOVES_APPEAL_JAM, gText_Jam, 0, 17, 0, 1 );
}
...
static u8 AddWindowFromTemplateList(
	const struct WindowTemplate* template, u8 templateId )
{
...
static void PrintPageSpecificText( u8 pageIndex )
{
	...
	for( i = 0; i < ARRAY_COUNT( sMonSummaryScreen->windowIds ); i++ )
	{
		if( sMonSummaryScreen->windowIds[i] != 0xFF )
			FillWindowPixelBuffer( sMonSummaryScreen->windowIds[i],
				PIXEL_FILL( 0 ) );
	}
...
static void PrintMonOTName( void )
{
	...
	if( InBattleFactory( ) != TRUE && InSlateportBattleTent( ) != TRUE )
	{
		windowId = AddWindowFromTemplateList( sPageInfoTemplate,
			PSS_DATA_WINDOW_INFO_ORIGINAL_TRAINER );
		SummaryScreen_PrintTextOnWindow(
			windowId, gText_OTSlash, 0, 1, 0, 1 );
		x = GetStringWidth( 1, gText_OTSlash, 0 );
		if( sMonSummaryScreen->summary.OTGender == 0 )
			SummaryScreen_PrintTextOnWindow( windowId,
				sMonSummaryScreen->summary.OTName,
				x,
				1,
				0,
				5 );
	...
	if( InBattleFactory( ) != TRUE && InSlateportBattleTent( ) != TRUE )
	{
		...
		SummaryScreen_PrintTextOnWindow(
			AddWindowFromTemplateList(
				sPageInfoTemplate, PSS_DATA_WINDOW_INFO_ID ),
			gStringVar1,
			xPos,
			1,
			0,
...
static void PrintMonAbilityName( void )
{
	...
	SummaryScreen_PrintTextOnWindow(
		AddWindowFromTemplateList(
			sPageInfoTemplate, PSS_DATA_WINDOW_INFO_ABILITY ),
...
static void PrintMonTrainerMemo( void )
{
	SummaryScreen_PrintTextOnWindow(
		AddWindowFromTemplateList(
			sPageInfoTemplate, PSS_DATA_WINDOW_INFO_MEMO ),
		gStringVar4,
		0,
		1,
		0,
...
static void PrintEggOTName( void )
{
	...
	SummaryScreen_PrintTextOnWindow(
		windowId, gText_FiveMarks, width, 1, 0, 1 );
}
...
static void BufferLeftColumnStats( void )
{
	...
	ConvertIntToDecimalStringN(
		currentHPString, sMonSummaryScreen->summary.currentHP, 1, 3 );
	ConvertIntToDecimalStringN(
		maxHPString, sMonSummaryScreen->summary.maxHP, 1, 3 );
	ConvertIntToDecimalStringN(
		attackString, sMonSummaryScreen->summary.atk, 1, 7 );
	ConvertIntToDecimalStringN(
		defenseString, sMonSummaryScreen->summary.def, 1, 7 );
	...
	DynamicPlaceholderTextUtil_ExpandPlaceholders(
		gStringVar4, sStatsLeftColumnLayout );
...
{
	ConvertIntToDecimalStringN(
		gStringVar1, sMonSummaryScreen->summary.spatk, 1, 3 );
	ConvertIntToDecimalStringN(
		gStringVar2, sMonSummaryScreen->summary.spdef, 1, 3 );
	ConvertIntToDecimalStringN(
		gStringVar3, sMonSummaryScreen->summary.speed, 1, 3 );
	...
	DynamicPlaceholderTextUtil_ExpandPlaceholders(
		gStringVar4, sStatsRightColumnLayout );
}
...
static void PrintExpPointsNextLevel( void )
{
	...
	u8 windowId             = AddWindowFromTemplateList(
                sPageSkillsTemplate, PSS_DATA_WINDOW_EXP );
	int offset;
	u32 expToNextLevel;
	...
	SummaryScreen_PrintTextOnWindow(
		windowId, gStringVar1, offset, 1, 0, 0 );

	if( sum->level < MAX_LEVEL )
		expToNextLevel =
			gExperienceTables[gBaseStats[sum->species].growthRate]
					 [sum->level + 1] -
			sum->exp;
	else
		expToNextLevel = 0;
	...
	offset = GetStringRightAlignXOffset( 1, gStringVar1, 42 ) + 2;
	SummaryScreen_PrintTextOnWindow(
		windowId, gStringVar1, offset, 17, 0, 0 );
}
...
static void PrintBattleMoves( void )
{
	...
	if( sMonSummaryScreen->mode == PSS_MODE_SELECT_MOVE )
	{
		...
		{
			PrintMoveDetails(
				sMonSummaryScreen->summary.moves
					[sMonSummaryScreen->firstMoveIndex] );
		}
	}
}
...
static void PrintMoveNameAndPP( u8 moveIndex )
{
	...
	u8 moveNameWindowId = AddWindowFromTemplateList(
		sPageMovesTemplate, PSS_DATA_WINDOW_MOVE_NAMES );
	u8 ppValueWindowId = AddWindowFromTemplateList(
		sPageMovesTemplate, PSS_DATA_WINDOW_MOVE_PP );
	u16 move = summaryStruct->summary.moves[moveIndex];

	if( move != 0 )
	{
		pp = CalculatePPWithBonus(
			move, summaryStruct->summary.ppBonuses, moveIndex );
		SummaryScreen_PrintTextOnWindow( moveNameWindowId,
			gMoveNames[move],
			0,
			moveIndex * 16 + 1,
			0,
			1 );
		ConvertIntToDecimalStringN( gStringVar1,
			summaryStruct->summary.pp[moveIndex],
			1,
			2 );
		...
		DynamicPlaceholderTextUtil_SetPlaceholderPtr( 1, gStringVar2 );
		DynamicPlaceholderTextUtil_ExpandPlaceholders(
			gStringVar4, sMovesPPLayout );
		text    = gStringVar4;
		ppState = GetCurrentPpToMaxPpState(
				  summaryStruct->summary.pp[moveIndex], pp ) +
			9;
	...
	SummaryScreen_PrintTextOnWindow( ppValueWindowId,
		text,
		offset,
		moveIndex * 16 + 1,
		0,
...
static void PrintMovePowerAndAccuracy( u16 moveIndex )
{
	...
	if( moveIndex != 0 )
	{
		FillWindowPixelRect( PSS_LABEL_WINDOW_MOVES_POWER_ACC,
			PIXEL_FILL( 0 ),
			53,
			0,
			19,
		...
		SummaryScreen_PrintTextOnWindow(
			PSS_LABEL_WINDOW_MOVES_POWER_ACC, text, 53, 1, 0, 0 );
		...
		{
			ConvertIntToDecimalStringN( gStringVar1,
				gBattleMoves[moveIndex].accuracy,
				1,
				3 );
			text = gStringVar1;
		}
		...
		SummaryScreen_PrintTextOnWindow(
			PSS_LABEL_WINDOW_MOVES_POWER_ACC, text, 53, 17, 0, 0 );
	}
}
...
static void PrintContestMoveDescription( u8 moveSlot )
{
	...
	{
		u8 windowId = AddWindowFromTemplateList(
			sPageMovesTemplate, PSS_DATA_WINDOW_MOVE_DESCRIPTION );
		SummaryScreen_PrintTextOnWindow( windowId,
			gContestEffectDescriptionPointers[gContestMoves[move]
								  .effect],
			6,
			1,
			0,
...
static void PrintNewMoveDetailsOrCancelText( void )
{
	u8 windowId1 = AddWindowFromTemplateList(
		sPageMovesTemplate, PSS_DATA_WINDOW_MOVE_NAMES );
	u8 windowId2 = AddWindowFromTemplateList(
		sPageMovesTemplate, PSS_DATA_WINDOW_MOVE_PP );

	if( sMonSummaryScreen->newMove == MOVE_NONE )
	{
		SummaryScreen_PrintTextOnWindow(
			windowId1, gText_Cancel, 0, 65, 0, 1 );
	}
	else
	{
		...
		if( sMonSummaryScreen->currPageIndex == 2 )
			SummaryScreen_PrintTextOnWindow(
				windowId1, gMoveNames[move], 0, 65, 0, 6 );
		else
			SummaryScreen_PrintTextOnWindow(
				windowId1, gMoveNames[move], 0, 65, 0, 5 );

		ConvertIntToDecimalStringN(
			gStringVar1, gBattleMoves[move].pp, 1, 2 );
		DynamicPlaceholderTextUtil_Reset( );
		DynamicPlaceholderTextUtil_SetPlaceholderPtr( 0, gStringVar1 );
		DynamicPlaceholderTextUtil_SetPlaceholderPtr( 1, gStringVar1 );
		DynamicPlaceholderTextUtil_ExpandPlaceholders(
			gStringVar4, sMovesPPLayout );
		SummaryScreen_PrintTextOnWindow( windowId2,
			gStringVar4,
			GetStringRightAlignXOffset( 1, gStringVar4, 0x2C ),
			0x41,
			0,
			12 );
	}
...
static void sub_81C40A0( u8 moveIndex1, u8 moveIndex2 )
{
	...
	FillWindowPixelRect(
		windowId1, PIXEL_FILL( 0 ), 0, moveIndex1 * 16, 0x48, 0x10 );
	FillWindowPixelRect(
		windowId1, PIXEL_FILL( 0 ), 0, moveIndex2 * 16, 0x48, 0x10 );

	FillWindowPixelRect(
		windowId2, PIXEL_FILL( 0 ), 0, moveIndex1 * 16, 0x30, 0x10 );
	FillWindowPixelRect(
		windowId2, PIXEL_FILL( 0 ), 0, moveIndex2 * 16, 0x30, 0x10 );
...
static void PrintHMMovesCantBeForgotten( void )
{
	...
	FillWindowPixelBuffer( windowId, PIXEL_FILL( 0 ) );
	SummaryScreen_PrintTextOnWindow(
		windowId, gText_HMMovesCantBeForgotten2, 6, 1, 0, 0 );
}
...
static void DestroySpriteInArray( u8 spriteArrayId )
{
	...
	{
		DestroySprite( &gSprites[sMonSummaryScreen
						 ->spriteIds[spriteArrayId]] );
...
static void SetSpriteInvisibility( u8 spriteArrayId, bool8 invisible )
{
	gSprites[sMonSummaryScreen->spriteIds[spriteArrayId]].invisible =
		invisible;
}
...
static void CreateMoveTypeIcons( void )
{
	...
	{
		if( sMonSummaryScreen->spriteIds[i] == 0xFF )
			sMonSummaryScreen->spriteIds[i] = CreateSprite(
				&sSpriteTemplate_MoveTypes, 0, 0, 2 );

		SetSpriteInvisibility( i, TRUE );
	}
}

static void SetMoveTypeSpritePosAndType(
	u8 typeId, u8 x, u8 y, u8 spriteArrayId )
{
...
static void SetMonTypeIcons( void )
{
	...
	else
	{
		SetMoveTypeSpritePosAndType(
			gBaseStats[summary->species].type1, 0x78, 0x30, 3 );
		if( gBaseStats[summary->species].type1 !=
...
static void SetMoveTypeIcons( void )
{
	...
	for( i = 0; i < MAX_MON_MOVES; i++ )
	{
		if( summary->moves[i] != MOVE_NONE )
			SetMoveTypeSpritePosAndType(
				gBattleMoves[summary->moves[i]].type,
				0x55,
				0x20 + ( i * 0x10 ),
				i + 3 );
		else
...
static void SetContestMoveTypeIcons( void )
{
	...
	{
		if( summary->moves[i] != MOVE_NONE )
			SetMoveTypeSpritePosAndType( NUMBER_OF_MON_TYPES +
					gContestMoves[summary->moves[i]]
						.contestCategory,
				0x55,
				0x20 + ( i * 0x10 ),
				i + 3 );
...
static void SetNewMoveTypeIcon( void )
{
	...
	else
	{
		if( sMonSummaryScreen->currPageIndex == 2 )
			SetMoveTypeSpritePosAndType(
				gBattleMoves[sMonSummaryScreen->newMove].type,
				85,
				96,
				7 );
		else
			SetMoveTypeSpritePosAndType( NUMBER_OF_MON_TYPES +
					gContestMoves[sMonSummaryScreen
							      ->newMove]
						.contestCategory,
				85,
				96,
...
static void sub_81C4568( u8 a0, u8 a1 )
{
	struct Sprite* sprite1 =
		&gSprites[sMonSummaryScreen->spriteIds[a0 + 3]];
	struct Sprite* sprite2 =
		&gSprites[sMonSummaryScreen->spriteIds[a1 + 3]];
...
static u8 CreatePokemonSprite( struct Pokemon* mon, s16* a1 )
{
	...
	case 0:
		...
		else
		{
			...
			{
				if( sMonSummaryScreen->monList.mons ==
						gPlayerParty ||
...
static void SpriteCB_Pokemon( struct Sprite* sprite )
{
	...
	if( !gPaletteFade.active && sprite->data[2] != 1 )
	{
		...
		PokemonSummaryDoMonAnimation(
			sprite, sprite->data[0], summary->isEgg );
	}
}

void SummaryScreen_SetUnknownTaskId( u8 a0 ) { sUnknownTaskId = a0; }
...
static void StopPokemonAnimations( void ) // A subtle effect, this function
					  // stops pokemon animations when
					  // leaving the PSS
{
	u16 i;
	u16 paletteIndex;
	...
	gSprites[sMonSummaryScreen->spriteIds[0]].callback =
		SpriteCallbackDummy;
	StopPokemonAnimationDelayTask( );

	paletteIndex =
		( gSprites[sMonSummaryScreen->spriteIds[0]].oam.paletteNum *
			16 ) |
		0x100;

	for( i = 0; i < 16; i++ )
	{
		gPlttBufferUnfaded[( u16 )( i + paletteIndex )] =
			gPlttBufferFaded[( u16 )( i + paletteIndex )];
	}
}
...
{
	struct Sprite* sprite =
		sub_811FF94( 30003, 30003, sSummaryMarkingsPalette );
	...
	{
		StartSpriteAnim(
			sprite, GetMonData( mon, MON_DATA_MARKINGS ) );
...
static void CreateCaughtBallSprite( struct Pokemon* mon )
{
	...
	LoadBallGfx( ball );
	sMonSummaryScreen->spriteIds[1] =
		CreateSprite( &gBallSpriteTemplates[ball], 16, 136, 0 );
	gSprites[sMonSummaryScreen->spriteIds[1]].callback =
		SpriteCallbackDummy;
...
static void CreateSetStatusSprite( void )
{
	...
	if( *spriteId == 0xFF )
	{
		*spriteId = CreateSprite(
			&sSpriteTemplate_StatusCondition, 64, 152, 0 );
	}
...
static void sub_81C4AF8( u8 a0 )
{
	...
	if( sMonSummaryScreen->currPageIndex > 1 )
	{
		...
		for( i = 0; i < 10; i++ )
		{
			spriteIds[i] = CreateSprite( &gUnknown_0861D084,
				i * 16 + 89,
				40,
				subsprite );
			if( i == 0 )
