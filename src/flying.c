...
static void sub_810DF18( u8 taskId )
{
	...
	if( gTasks[taskId].data[10]++ == gTasks[taskId].data[1] )
	{
		...
		do
		{
			gPlttBufferFaded[base + 0x101 + i] =
				gPlttBufferFaded[base + 0x100 + i];
			i--;
		} while( i > 0 );
...
static void sub_810E24C( struct Sprite* sprite )
{
	...
	{
		gSprites[GetAnimBattlerSpriteId( ANIM_ATTACKER )].invisible =
			0;
		DestroyAnimSprite( sprite );
	}
}
...
void sub_810E314( struct Sprite* sprite )
{
	...
	sprite->pos1.x =
		GetBattlerSpriteCoord( battler, BATTLER_COORD_ATTR_HEIGHT ) +
		gBattleAnimArgs[0];
	spriteCoord =
		GetBattlerSpriteCoord( battler, BATTLER_COORD_ATTR_WIDTH );
	...
	*(u16*)( data->unkC ) = gBattleAnimArgs[5];
	...
	if( data->unk2 >= 64 && data->unk2 <= 191 )
	{
		if( !IsContest( ) )
			...
			sprite->oam.priority =
				GetBattlerSpriteBGPriority( battler );
	...
	// The comparison below is completely pointless. 'sprite' is sure to be
	// a valid pointer and both the 'if' and 'else' parts are exactly the
	// same. The only reason for this is making sure the compiler generates
	// the exact ASM.
	if( sprite )
	{
...
2. sprite->oam.priority--; sprite->oam.priority++; Nonmatching version
tail-merges these, original does not.
3. sprite->pos2.x = (gSineTable[data->unk2] * var) >> 8; data->unk2 is very
weirdly read at seemingly random moments inside the switch.
...
void sub_810E520( struct Sprite* sprite )
{
	...
	else
	{
		...
		if( data->unk4 & 0x8000 )
			data->unk2 = ( data->unk2 - ( data->unk4 & 0x7FFF ) ) &
				0xFF;
		else
			data->unk2 = ( data->unk2 + ( data->unk4 & 0x7FFF ) ) &
				0xFF;
...
void sub_810EEF8( struct Sprite* sprite )
{
	...
	if( gBattleAnimArgs[1] == 0 )
	{
		...
		sprite->pos1.y =
			GetBattlerSpriteCoord( gBattleAnimAttacker, 1 ) + 32;
	}
	else
	{
		sprite->pos1.x = GetBattlerSpriteCoord( gBattleAnimTarget, 0 );
		sprite->pos1.y =
			GetBattlerSpriteCoord( gBattleAnimTarget, 1 ) + 32;
	}
