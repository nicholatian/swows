...
void pal_fill_for_maplights( void )
{
	...
	switch( GetMapPairFadeFromType(
		previousMapType, GetCurrentMapType( ) ) )
	{
	case 0:
...
void WarpFadeScreen( void )
{
	...
	switch( GetMapPairFadeToType(
		currentMapType, GetDestinationWarpMapHeader( )->mapType ) )
	{
	case 0:
...
static void sub_80AF0F4( u8 arg ) { sub_808C0A8( !arg ); }
...
static void sub_80AF438( u8 taskId )
{
	...
	case 1:
		if( WaitForWeatherFadeIn( ) )
		{
			...
			sub_80AF0F4( 1 );
			eventObjId =
				GetEventObjectIdByLocalIdAndMap( 0xFF, 0, 0 );
	...
	case 3:
		if( task->data[1] < 0 ||
			gTasks[task->data[1]].isActive != TRUE )
		{
...
{
	ScriptContext2_Enable( );
	EscalatorAnimTaskSetup( metatileBehavior, 10 );
}
...
void sub_80AF9F8( void ) { CreateTask( Task_ReturnToWorldFromLinkRoom, 10 ); }
...
static void SetFlashScanlineEffectWindowBoundary(
	u16* dest, u32 y, s32 left, s32 right )
{
	if( y <= 160 )
	{
		if( left < 0 )
...
static void SetFlashScanlineEffectWindowBoundaries(
	u16* dest, s32 centerX, s32 centerY, s32 radius )
{
	...
	while( r >= v3 )
	{
		SetFlashScanlineEffectWindowBoundary(
			dest, centerY - v3, centerX - r, centerX + r );
		SetFlashScanlineEffectWindowBoundary(
			dest, centerY + v3, centerX - r, centerX + r );
		SetFlashScanlineEffectWindowBoundary(
			dest, centerY - r, centerX - v3, centerX + v3 );
		SetFlashScanlineEffectWindowBoundary(
			dest, centerY + r, centerX - v3, centerX + v3 );
		v2 -= ( v3 * 2 ) - 1;
		v3++;
...
static void SetFlash2ScanlineEffectWindowBoundary(
	u16* dest, u32 y, s32 left, s32 right )
{
	if( y <= 160 )
	{
...
static void SetFlash2ScanlineEffectWindowBoundaries(
	u16* dest, s32 centerX, s32 centerY, s32 radius )
{
	s32 r  = radius;
	s32 v2 = radius;
...
void WriteFlashScanlineEffectBuffer( u8 flashLevel )
{
	...
	{
		SetFlashScanlineEffectWindowBoundaries(
			&gScanlineEffectRegBuffers[0][0],
			120,
			80,
			sFlashLevelPixelRadii[flashLevel] );
		CpuFastSet( &gScanlineEffectRegBuffers[0],
