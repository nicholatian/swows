...
static const u16 sEvoSparklePalette[] =
	INCBIN_U16( "graphics/misc/evo_sparkle.gbapal" );
static const u32 sEvoSparkleTiles[] =
...
static const struct SpritePalette sEvoSparkleSpritePals[] = {
	{sEvoSparklePalette, 1001}, {NULL, 0}};
...
static void SpriteCB_PreEvoSparkleSet1( struct Sprite* sprite )
{
	if( sprite->pos1.y > 8 )
	{
		...
		sprite->pos1.y =
			88 - ( sprite->data[7] * sprite->data[7] ) / 80;
		sprite->pos2.y =
			Sin( ( u8 )( sprite->data[6] ), sprite->data[5] ) / 4;
		sprite->pos2.x =
			Cos( ( u8 )( sprite->data[6] ), sprite->data[5] );
		sprite->data[6] += 4;
		if( sprite->data[7] & 1 )
			sprite->data[5]--;
		sprite->data[7]++;
...
static void SpriteCB_PostEvoSparkleSet1( struct Sprite* sprite )
{
	...
	{
		sprite->pos2.y =
			Sin( ( u8 )( sprite->data[6] ), sprite->data[5] );
		sprite->pos2.x =
			Cos( ( u8 )( sprite->data[6] ), sprite->data[5] );
...
static void SpriteCB_PostEvoSparkleSet2( struct Sprite* sprite )
{
	...
	if( sprite->data[6] < 128 )
	{
		...
		sprite->pos2.y =
			-Sin( ( u8 )( sprite->data[6] ), sprite->data[5] );
		sprite->pos1.x =
			120 + ( sprite->data[3] * sprite->data[7] ) / 3;
		sprite->data[6]++;
...
static void EvoTask_BeginPreSet1_FadeAndPlaySE( u8 taskID )
{
	...
	BeginNormalPaletteFade(
		3 << gTasks[taskID].data[1], 0xA, 0, 0x10, RGB_WHITE );
...
static void EvoTask_DestroyPreSet2Task( u8 taskID ) { DestroyTask( taskID ); }
...
static void EvoTask_DestroyPostSet1Task( u8 taskID ) { DestroyTask( taskID ); }
...
static void EvoTask_BeginPostSparklesSet2_AndFlash( u8 taskID )
{
	...
	BeginNormalPaletteFade(
		0xFFF9041C, 0, 0, 0x10, RGB_WHITE ); // was
						     // 0xFFF9001C
						     // in R/S
...
static void EvoTask_CreatePostEvoSparklesSet2_AndFlash( u8 taskID )
{
	if( gTasks[taskID].tFrameCounter < 128 )
	{
		...
		case 0:
			...
			break;
		case 32:
			BeginNormalPaletteFade( 0xFFFF041C,
				0x10,
				0x10,
				0,
				RGB_WHITE ); // was
...
{
	u8 taskID =
		CreateTask( EvoTask_BeginPostSparklesSet2_AndFlash_Trade, 0 );
	gTasks[taskID].data[2] = species;
	return taskID;
}
...
static void EvoTask_BeginPostSparklesSet2_AndFlash_Trade( u8 taskID )
{
	...
	CpuSet( &gPlttBufferFaded[0x20], &gPlttBufferUnfaded[0x20], 0x30 );
	BeginNormalPaletteFade(
		0xFFF90400, 0, 0, 0x10, RGB_WHITE ); // was
						     // 0xFFFF0001
						     // in R/S
...
static void EvoTask_CreatePostEvoSparklesSet2_AndFlash_Trade( u8 taskID )
{
	if( gTasks[taskID].tFrameCounter < 128 )
	{
		...
		case 0:
			...
			break;
		case 32:
			BeginNormalPaletteFade( 0xFFFF0400,
				0x10,
				0x10,
				0,
				RGB_WHITE ); // was
...
u8 sub_817C3A0( u8 preEvoSpriteID, u8 postEvoSpriteID )
{
	...
	CpuSet( stack,
		&gPlttBufferFaded[0x100 +
			( gSprites[preEvoSpriteID].oam.paletteNum * 16 )],
		16 );
	...
	CpuSet( stack,
		&gPlttBufferFaded[0x100 +
			( gSprites[postEvoSpriteID].oam.paletteNum * 16 )],
		16 );
