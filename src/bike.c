...
static u8 Bike_CheckCollisionTryAdvanceCollisionCount(
	struct EventObject*, s16, s16, u8, u8 );
...
static void ( *const sMachBikeTransitions[] )( u8 ) = {
	MachBikeTransition_FaceDirection, // Face vs Turn: Face has no anim
					  // while Turn does. Turn checks for
					  // collision because if you turn
					  // right as opposed to face right, if
					  // there is a wall there, turn will
					  // make a bonk sound effect while
					  // face will not.
...
// 0 is normal speed (1 speed), 1 is fast speed (2 speed), 2 is fastest speed
// (4 speed)
...
// this is a list of history inputs to do in order to do the check to retrieve
// a jump direction for acro bike. it seems to be an extensible list, so its
...
static const struct BikeHistoryInputInfo sAcroBikeTricksList[] = {
	...
	// perform the check on only the last entry of the history list,
	// otherwise
...
{
	// if the dir updated before this function, get the relevent new
	// direction to check later.
	u8 direction = GetPlayerMovementDirection( );

	// is the player standing still?
	if( *dirTraveling == 0 )
	{
		*dirTraveling = direction; // update the direction, since below
					   // we either faced a direction or we
					   // started moving.
	...
	// direction as well as ensuring that we dont update the state while
	// the player is moving: see the else check.
	if( *dirTraveling != direction &&
		gPlayerAvatar.runningState != MOVING )
	{
		if( gPlayerAvatar.bikeSpeed != SPEED_STANDING )
		{
			*dirTraveling =
				direction; // implement the new direction
			gPlayerAvatar.runningState = MOVING;
			return MACH_TRANS_START_MOVING;
		}
		// if you didnt start moving but your dir was different, do a
		// turn direction instead.
...
static void MachBikeTransition_TurnDirection( u8 direction )
{
	...
	if( CanBikeFaceDirOnMetatile(
		    direction, playerEventObj->currentMetatileBehavior ) )
	{
...
static void MachBikeTransition_TrySpeedUp( u8 direction )
{
	...
	if( CanBikeFaceDirOnMetatile( direction,
		    playerEventObj->currentMetatileBehavior ) == FALSE )
	{
		// we cannot go forward, so either slow down or, if we are
		// stopped, idle face direction.
	...
	else
	{
		...
		{
			// we hit a solid object, but check to see if its a
			// ledge and then jump.
			...
			{
				// we hit a solid object that is not a ledge,
				// so perform the collision.
				Bike_SetBikeStill( );
				if( collision == 4 &&
		...
		{
			// we did not hit anything that can slow us down, so
			// perform the advancement callback depending on the
			// bikeFrameCounter and try to increase the mach bike's
			// speed.
			sMachBikeSpeedCallbacks[gPlayerAvatar
							.bikeFrameCounter](
				direction );
...
static void MachBikeTransition_TrySlowDown( u8 direction )
{
	...
	{
		sMachBikeSpeedCallbacks[gPlayerAvatar.bikeFrameCounter](
			direction );
	}
}

// the acro bike requires the input handler to be executed before the
// transition can.
static void MovePlayerOnAcroBike( u8 newDirection, u16 newKeys, u16 heldKeys )
{
	sAcroBikeTransitions[CheckMovementInputAcroBike(
		&newDirection, newKeys, heldKeys )]( newDirection );
}

static u8 CheckMovementInputAcroBike(
	u8* newDirection, u16 newKeys, u16 heldKeys )
{
	return sAcroBikeInputHandlers[gPlayerAvatar.acroBikeState](
		newDirection, newKeys, heldKeys );
}

static u8 AcroBikeHandleInputNormal(
	u8* newDirection, u16 newKeys, u16 heldKeys )
{
	u8 direction = GetPlayerMovementDirection( );
		...
		gPlayerAvatar.runningState != MOVING )
	{
		...
		gPlayerAvatar.runningState  = NOT_MOVING;
		return CheckMovementInputAcroBike(
			newDirection, newKeys, heldKeys );
	}
	gPlayerAvatar.runningState = MOVING;
...
static u8 AcroBikeHandleInputTurning(
	u8* newDirection, u16 newKeys, u16 heldKeys )
{
	u8 direction;
	...
	*newDirection = gPlayerAvatar.newDirBackup;
	...
	if( gPlayerAvatar.bikeFrameCounter > 6 ) // ... because it takes 6
						 // frames to advance 1 tile.
	{
		gPlayerAvatar.runningState  = TURN_DIRECTION;
	...
	{
		Bike_SetBikeStill( ); // Bike_SetBikeStill sets speed to
				      // standing, but the next line
				      // immediately overrides it. could have
				      // just reset acroBikeState to 0 here
				      // instead of wasting a jump.
...
static u8 AcroBikeHandleInputWheelieStanding(
	u8* newDirection, u16 newKeys, u16 heldKeys )
{
	u8 direction;
	struct EventObject* playerEventObj;
	...
	else
	{
		...
		if( !MetatileBehavior_IsBumpySlope(
			    playerEventObj->currentMetatileBehavior ) )
		{
...
static u8 AcroBikeHandleInputBunnyHop(
	u8* newDirection, u16 newKeys, u16 heldKeys )
{
	u8 direction;
	struct EventObject* playerEventObj;
	...
	if( !( heldKeys & B_BUTTON ) )
	{
		...
		if( MetatileBehavior_IsBumpySlope(
			    playerEventObj->currentMetatileBehavior ) )
		{
			// even though B was released, dont undo the wheelie on
			// the bumpy slope.
			gPlayerAvatar.acroBikeState =
				ACRO_STATE_WHEELIE_STANDING;
			return CheckMovementInputAcroBike(
...
static u8 AcroBikeHandleInputWheelieMoving(
	u8* newDirection, u16 newKeys, u16 heldKeys )
{
	u8 direction;
	struct EventObject* playerEventObj;
	...
	if( !( heldKeys & B_BUTTON ) )
	{
		// we were moving on a wheelie, but we let go while moving.
		// reset bike still status
		Bike_SetBikeStill( );
		if( !MetatileBehavior_IsBumpySlope(
			    playerEventObj->currentMetatileBehavior ) )
		{
			// we let go of B and arent on a bumpy slope, set state
			// to normal because now we need to handle this
			gPlayerAvatar.acroBikeState = ACRO_STATE_NORMAL;
			if( *newDirection == DIR_NONE )
			{
				// we stopped moving but are turning, still try
				// to lower the wheelie in place.
				*newDirection              = direction;
				gPlayerAvatar.runningState = NOT_MOVING;
				return ACRO_TRANS_WHEELIE_TO_NORMAL;
			}
			if( *newDirection != direction &&
				gPlayerAvatar.runningState != MOVING )
			{
				// we did not turn while lowering wheelie, so
				// do so without turning.
				gPlayerAvatar.runningState = NOT_MOVING;
				return ACRO_TRANS_WHEELIE_TO_NORMAL;
			}
			// if we are moving while lowering wheelie, put the
			// acro into a lowering state while moving.
			gPlayerAvatar.runningState = MOVING;
	...
	if( *newDirection == DIR_NONE )
	{
		// idle the wheelie in place because we're holding B without
		// moving.
		*newDirection               = direction;
		gPlayerAvatar.acroBikeState = ACRO_STATE_WHEELIE_STANDING;
		gPlayerAvatar.runningState  = NOT_MOVING;
...
static void AcroBikeTransition_TurnDirection( u8 direction )
{
	...
	if( CanBikeFaceDirOnMetatile(
		    direction, playerEventObj->currentMetatileBehavior ) == 0 )
...
static void AcroBikeTransition_SideJump( u8 direction )
{
	...
	if( collision != 0 )
	{
		...
		if( WillPlayerCollideWithCollision( collision, direction ) ==
			FALSE )
		{
			AcroBikeTransition_TurnDirection( direction );
			return;
...
static void AcroBike_TryHistoryUpdate( u16 newKeys, u16 heldKeys ) // newKeys
								   // is unused
{
	...
	const u8* dirTimerList, const u8* abStartSelectTimerList )
{
	...
	{
		if( gPlayerAvatar.abStartSelectTimerHistory[i] >
			abStartSelectTimerList[i] )
			return FALSE;
	}
	return TRUE;
}
...
static u8 AcroBike_GetJumpDirection( void )
{
	...
	for( i = 0; i < ARRAY_COUNT( sAcroBikeTricksList ); i++ )
	{
		...
		abStartSelectHistory &=
			historyInputInfo->abStartSelectHistoryMask;
		if( dirHistory == historyInputInfo->dirHistoryMatch &&
			abStartSelectHistory ==
				historyInputInfo->abStartSelectHistoryMatch &&
...
static void Bike_UpdateDirTimerHistory( u8 dir )
{
	...
	for( i = 7; i != 0; i-- )
		gPlayerAvatar.dirTimerHistory[i] =
			gPlayerAvatar.dirTimerHistory[i - 1];
...
static u8 get_some_collision( u8 direction )
{
	...
	return Bike_CheckCollisionTryAdvanceCollisionCount(
		playerEventObj, x, y, direction, metatitleBehavior );
}
...
static u8 Bike_CheckCollisionTryAdvanceCollisionCount(
	...
	u8 metatitleBehavior )
{
	u8 collision = CheckForEventObjectCollision(
		eventObject, x, y, direction, metatitleBehavior );

	if( collision > 4 )
		return collision;

	if( collision == 0 &&
		IsRunningDisallowedByMetatile( metatitleBehavior ) )
		collision = 2;
...
static bool8 IsRunningDisallowedByMetatile( u8 tile )
{
	if( MetatileBehavior_IsRunningDisallowed( tile ) )
		return TRUE;
	if( MetatileBehavior_IsFortreeBridge( tile ) &&
		( PlayerGetZCoord( ) & 1 ) == 0 )
		return TRUE;
	return FALSE;
}
...
static bool8 WillPlayerCollideWithCollision(
	u8 newTileCollision, u8 direction )
{
...
void Bike_UpdateBikeCounterSpeed( u8 counter )
{
	...
	gPlayerAvatar.bikeSpeed        = gPlayerAvatar.bikeFrameCounter +
		( gPlayerAvatar.bikeFrameCounter >> 1 ); // lazy way of
							 // multiplying by 1.5.
}
...
{
	// because the player pressed a direction, it won't ever return a speed
	// of 0 since this function returns the player's current speed.
	s16 machSpeeds[3];
