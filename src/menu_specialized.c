...
static void MoveRelearnerCursorCallback(
	s32 itemIndex, bool8 onInit, struct ListMenu* list );
...
static const u8 sUnknown_08625400[] = {1, 2, 3};
...
bool8 sub_81D1C44( u8 count )
{
	...
	sUnknown_0203CF4C = Alloc( count * sizeof( *sUnknown_0203CF4C ) +
	...
	if( sUnknown_0203CF48[a0] == 0xFF )
	{
		if( a0 == 2 )
		{
			...
			template.width = GetMaxWidthInMenuTable(
				&gMailboxMailOptions[0], 4 );
			sUnknown_0203CF48[2] = AddWindow( &template );
		}
		else
		{
			sUnknown_0203CF48[a0] =
				AddWindow( &sUnknown_086253E8[a0] );
		}
...
static u8 sub_81D1D34( u8 a0 ) { return sUnknown_0203CF48[a0]; }
...
static void sub_81D1D44( u8 windowId, s32 itemId, u8 y )
{
	...
	AddTextPrinterParameterized4(
		windowId, 1, 8, y, 0, 0, sUnknown_08625400, -1, buffer );
}
...
{
	page->scrollIndicatorId = AddScrollIndicatorArrowPairParameterized( 2,
		0xC8,
		12,
		0x94,
		page->count - page->pageItems + 1,
		0x6E,
		0x6E,
		&page->itemsAbove );
}

void sub_81D1EC0( void ) { Free( sUnknown_0203CF4C ); }
...
void sub_81D1F84( struct UnknownStruct_81D1ED4* arg0,
...
void sub_81D20AC( struct UnknownStruct_81D1ED4* a0 ) { a0->unk355 = 0; }
...
void sub_81D2108( struct UnknownStruct_81D1ED4* arg0 )
{
	...
	for( i = 0; i < 66; i++ )
	{
		gScanlineEffectRegBuffers[1][( i + 55 ) * 2] =
			gScanlineEffectRegBuffers[0][( i + 55 ) * 2] =
				( arg0->unk140[i][0] << 8 ) |
			( arg0->unk140[i][1] );
...
/* TODO
static void sub_81D2278(struct UnknownStruct_81D1ED4 *arg0, u16 arg1[66][2],
struct UnknownSubStruct_81D1ED4 *arg2, struct UnknownSubStruct_81D1ED4 *arg3,
u8 arg4, u16 arg5[66][2])
{
    s32 var_2C = 0;
    u16 r8;
...
*/
NAKED
static void sub_81D2278( struct UnknownStruct_81D1ED4* arg0,
	u16 arg1[66][2],
	struct UnknownSubStruct_81D1ED4* arg2,
...
u8 LoadMoveRelearnerMovesList(
	const struct ListMenuItem* items, u16 numChoices )
{
...
static void MoveRelearnerLoadBattleMoveDescription( u32 chosenMove )
{
	...
	AddTextPrinterParameterized(
		0, 1, buffer, x, 0x29, TEXT_SPEED_FF, NULL );
	...
	AddTextPrinterParameterized(
		0, 1, str, 0x6A, 0x19, TEXT_SPEED_FF, NULL );
	...
	AddTextPrinterParameterized(
		0, 1, str, 0x6A, 0x29, TEXT_SPEED_FF, NULL );
...
void MoveRelearnerPrintText( u8* str )
{
	...
	AddTextPrinterParameterized2( 3,
		1,
		str,
		speed,
		NULL,
...
s32 GetBoxOrPartyMonData( u16 boxId, u16 monId, s32 request, u8* dst )
{
	...
	else
	{
		if( request == MON_DATA_NICKNAME ||
			request == MON_DATA_OT_NAME )
			ret = GetAndCopyBoxMonDataAt(
				boxId, monId, request, dst );
		else
	...
	*( dst++ ) = 9;
	...
	else
	{
		...
		species = GetBoxOrPartyMonData(
			boxId, monId, MON_DATA_SPECIES, NULL );
		if( boxId == TOTAL_BOXES_COUNT ) // Party mon.
		{
			level = GetMonData(
		...
		*( str++ ) = 5;
		str        = ConvertIntToDecimalStringN(
                        str, level, STR_CONV_MODE_LEFT_ALIGN, 3 );
		*( str++ ) = CHAR_SPACE;
		*str       = EOS;
...
void sub_81D2F78( struct UnknownStruct_81D1ED4* arg0,
	u8* sheen,
	u16 boxId,
	u16 monId,
	u16 arg5,
	...
	bool8 arg8 )
{
	...
	if( arg5 != arg7 )
	{
		arg0->unk0[id][0] = GetBoxOrPartyMonData(
			boxId, monId, MON_DATA_COOL, NULL );
		arg0->unk0[id][1] = GetBoxOrPartyMonData(
			boxId, monId, MON_DATA_TOUGH, NULL );
		arg0->unk0[id][2] = GetBoxOrPartyMonData(
			boxId, monId, MON_DATA_SMART, NULL );
		arg0->unk0[id][3] = GetBoxOrPartyMonData(
			boxId, monId, MON_DATA_CUTE, NULL );
		arg0->unk0[id][4] = GetBoxOrPartyMonData(
			boxId, monId, MON_DATA_BEAUTY, NULL );

		sheen[id] = ( GetBoxOrPartyMonData(
				      boxId, monId, MON_DATA_SHEEN, NULL ) !=
				    0xFF )
...
void sub_81D3094( void* tilesDst,
	void* palDst,
	u16 boxId,
	u16 monId,
	u16 arg5,
	...
	bool8 arg7 )
{
	...
	if( arg5 != arg6 )
	{
		u16 species = GetBoxOrPartyMonData(
			boxId, monId, MON_DATA_SPECIES2, NULL );
		u32 trainerId = GetBoxOrPartyMonData(
			boxId, monId, MON_DATA_OT_ID, NULL );
		u32 personality = GetBoxOrPartyMonData(
			boxId, monId, MON_DATA_PERSONALITY, NULL );
		...
		LoadSpecialPokePic( &gMonFrontPicTable[species],
			...
			personality,
			TRUE );
		LZ77UnCompWram( GetFrontSpritePalFromSpeciesAndPersonality(
...
static const u32 gUnknown_08625560[] =
	INCBIN_U32( "graphics/pokenav/pokeball.4bpp" );
static const u32 gUnknown_08625660[] =
	INCBIN_U32( "graphics/pokenav/pokeball_placeholder.4bpp" );
static const u16 gUnknown_08625680[] =
	INCBIN_U16( "graphics/pokenav/sparkle.gbapal" );
static const u32 gUnknown_086256A0[] =
	INCBIN_U32( "graphics/pokenav/sparkle.4bpp" );

static const struct OamData sOamData_8625A20 = {.y = 0,
	.affineMode                                = 0,
	.objMode                                   = 0,
	.mosaic                                    = 0,
	.bpp                                       = 0,
...
static const struct OamData sOamData_8625A28 = {.y = 0,
	.affineMode                                = 0,
	.objMode                                   = 0,
	.mosaic                                    = 0,
	.bpp                                       = 0,
...
static const union AnimCmd* const sSpriteAnimTable_8625A40[] = {
	sSpriteAnim_8625A30, sSpriteAnim_8625A38};

void sub_81D31D0( struct SpriteSheet* sheet,
...
void sub_81D321C( struct SpriteSheet* sheets,
	struct SpriteTemplate* template,
...
static const union AnimCmd sSpriteAnim_8625AD8[] = {ANIMCMD_FRAME( 0, 5 ),
	ANIMCMD_FRAME( 4, 5 ),
	ANIMCMD_FRAME( 8, 5 ),
	ANIMCMD_FRAME( 12, 5 ),
	ANIMCMD_FRAME( 16, 5 ),
...
// unused
static const union AnimCmd* const* const sUnknown_08625B10 =
...
void sub_81D3314( struct Sprite* sprite )
{
	...
	if( sprite2 != NULL )
	{
		sprite->pos1.x = sprite2->pos1.x + sprite2->pos2.x +
			gUnknown_08625B2C[sprite->data[0]][0];
		sprite->pos1.y = sprite2->pos1.y + sprite2->pos2.y +
			gUnknown_08625B2C[sprite->data[0]][1];
	}
	else
	{
...
static const u8* const sLvlUpStatStrings[] = {gUnknown_085EEA46,
	gUnknown_085EEA4E,
	gUnknown_085EEA55,
	gUnknown_085EEA63,
	gUnknown_085EEA6B,
...
void DrawLevelUpWindowPg1( u16 windowId,
	u16* statsBefore,
	u16* statsAfter,
	u8 bgClr,
	u8 fgClr,
	u8 shadowClr )
{
	...
	for( i = 0; i < NUM_STATS; i++ )
	{
		...
		StringCopy( text,
			( statsDiff[i] >= 0 ) ? gText_UnkCtrlF904
					      : gText_Dash );
		AddTextPrinterParameterized3(
			windowId, 1, 56, 15 * i, color, -1, text );
		...
		ConvertIntToDecimalStringN( text,
			...
			2 );
		AddTextPrinterParameterized3(
			windowId, 1, 56 + x, 15 * i, color, -1, text );
	}
}

void DrawLevelUpWindowPg2(
	u16 windowId, u16* currStats, u8 bgClr, u8 fgClr, u8 shadowClr )
{
	u16 i, numDigits, x;
	s16 stats[NUM_STATS];
	u8 text[12];
	...
	for( i = 0; i < NUM_STATS; i++ )
	{
		...
		ConvertIntToDecimalStringN(
			text, stats[i], STR_CONV_MODE_LEFT_ALIGN, numDigits );
		x = 6 * ( 4 - numDigits );
