...
void PickLotteryCornerTicket( void )
{
	...
	for( i = 0; i < PARTY_SIZE; i++ )
	{
		...
		if( GetMonData( mon, MON_DATA_SPECIES ) != SPECIES_NONE )
		{
			...
			if( !GetMonData( mon, MON_DATA_IS_EGG ) )
			{
				...
				u8 numMatchingDigits = GetMatchingDigits(
					gSpecialVar_Result, otId );
		...
		else // pokemon are always arranged from populated spots first
		     // to unpopulated, so the moment a NONE species is found,
	...
	if( gSpecialVar_0x8004 != 0 )
	{
		...
		else
		{
			...
			GetBoxMonData( &gPokemonStoragePtr->boxes[box][slot],
				MON_DATA_NICKNAME,
				gStringVar1 );
		}
...
// lottery numbers go from 0 to 99999, not 65535 (0xFFFF). interestingly
// enough, the function that calls GetLotteryNumber shifts to u16, so it cant
// be anything above 65535 anyway.
...
// but GF tried to change it to 32-bit later but didnt finish changing all
// calls as one GetLotteryNumber still shifts to u16.
