		...
		++i, bgLayer = templates[i].bg )
	{
		...
		if( gUnknown_03002F70[bgLayer] == NULL )
		{
			...
			if( attrib != 0xFFFF )
			{
				...
				gUnknown_03002F70[bgLayer] =
					allocatedTilemapBuffer;
				SetBgTilemapBuffer(
					bgLayer, allocatedTilemapBuffer );
			}
		}

		allocatedTilemapBuffer = AllocZeroed( ( u16 )( 0x20 *
...
u16 AddWindow( const struct WindowTemplate* template )
{
	...
	allocatedTilemapBuffer = AllocZeroed(
		( u16 )( 0x20 * ( template->width * template->height ) ) );
	...
	if( gUnneededFireRedVariable == 1 )
	{
		...
		DummiedOutFireRedLeafGreenTileAllocFunc( bgLayer,
			allocatedBaseBlock,
			gWindows[win].window.width *
				gWindows[win].window.height,
			1 );
	}
...
void RemoveWindow( u8 windowId )
{
	...
	if( gUnneededFireRedVariable == 1 )
	{
		DummiedOutFireRedLeafGreenTileAllocFunc( bgLayer,
			...
			gWindows[windowId].window.width *
...
void CopyWindowToVram( u8 windowId, u8 mode )
{
	...
	u16 windowSize =
		32 * ( windowLocal.window.width * windowLocal.window.height );
...
void PutWindowRectTilemapOverridePalette(
	u8 windowId, u8 x, u8 y, u8 width, u8 height, u8 palette )
{
	struct Window windowLocal = gWindows[windowId];
...
void BlitBitmapToWindow(
	u8 windowId, const u8* pixels, u16 x, u16 y, u16 width, u16 height )
{
	BlitBitmapRectToWindow(
		windowId, pixels, 0, 0, width, height, x, y, width, height );
}
...
void FillWindowPixelRect(
	u8 windowId, u8 fillValue, u16 x, u16 y, u16 width, u16 height )
{
...
void CopyToWindowPixelBuffer(
	u8 windowId, const void* src, u16 size, u16 tileOffset )
{
	if( size != 0 )
		CpuCopy16( src,
			gWindows[windowId].tileData + ( 0x20 * tileOffset ),
			size );
	else
		LZ77UnCompWram( src,
			gWindows[windowId].tileData + ( 0x20 * tileOffset ) );
...
{
	int fillSize = gWindows[windowId].window.width *
		gWindows[windowId].window.height;
	CpuFastFill8(
		fillValue, gWindows[windowId].tileData, 0x20 * fillSize );
}
...
#define MOVE_TILES_DOWN( a ) \
	{ \
		destOffset = i + ( a ); \
		srcOffset  = i + \
			( ( ( width * ( distanceLoop & ~7 ) ) | \
				  ( distanceLoop & 7 ) ) * \
				4 ); \
		if( srcOffset < size ) \
			*(u32*)( tileData + destOffset ) = \
				*(u32*)( tileData + srcOffset ); \
		else \
...
void ScrollWindow( u8 windowId, u8 direction, u8 distance, u8 fillValue )
{
	...
	u32 fillValue32 = ( fillValue << 24 ) | ( fillValue << 16 ) |
		( fillValue << 8 ) | fillValue;
...
void CallWindowFunction(
	u8 windowId, void ( *func )( u8, u8, u8, u8, u8, u8 ) )
{
	struct WindowTemplate window = gWindows[windowId].window;
	func( window.bg,
		window.tilemapLeft,
...
u16 AddWindow8Bit( const struct WindowTemplate* template )
{
	...
	if( gUnknown_03002F70[bgLayer] == 0 )
	{
		...
		if( attribute != 0xFFFF )
		{
			...
			if( memAddress == NULL )
				return 0xFF;
			for( i = 0; i < attribute;
				i++ ) // if we're going to zero out
				      // the memory anyway, why not
				      // call AllocZeroed?
				memAddress[i] = 0;
	...
	memAddress = Alloc(
		( u16 )( 0x40 * ( template->width * template->height ) ) );
	if( memAddress == NULL )
	{
		if( GetNumActiveWindowsOnBg8Bit( bgLayer ) == 0 &&
			gUnknown_03002F70[bgLayer] != nullsub_9 )
		{
			Free( gUnknown_03002F70[bgLayer] );
	...
	u8 paletteNum )
{
	...
	BlitBitmapRect4BitTo8Bit( &sourceRect,
		&destRect,
		srcX,
		srcY,
		destX,
...
void CopyWindowToVram8Bit( u8 windowId, u8 mode )
{
	...
	sWindowSize = 0x40 *
		( sWindowPtr->window.width * sWindowPtr->window.height );
