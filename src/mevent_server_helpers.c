...
void mevent_srv_sub_init(
	struct mevent_srv_sub* svr, u32 sendPlayerNo, u32 recvPlayerNo )
{
...
void mevent_srv_sub_init_send(
	struct mevent_srv_sub* svr, u32 ident, const void* src, u32 size )
{
	svr->seqno       = 0;
	svr->sendIdent   = ident;
	svr->sendCounter = 0;
...
void mevent_srv_sub_init_recv(
	struct mevent_srv_sub* svr, u32 ident, void* dest )
{
	svr->seqno       = 0;
	svr->recvIdent   = ident;
	svr->recvCounter = 0;
...
static bool32 mevent_receive_func( struct mevent_srv_sub* svr )
{
	...
	case 1:
		if( mevent_has_received( svr->recvPlayerNo ) )
		{
			...
			{
				mevent_recv_block( svr->recvPlayerNo,
					svr->recvBfr + blocksiz,
					svr->recvSize - blocksiz );
				++svr->recvCounter;
				++svr->seqno;
			}
			else
		...
		break;
	case 2:
		if( CalcCRC16WithTable( svr->recvBfr, svr->recvSize ) !=
			svr->recvCRC )
		{
			LinkRfu_FatalError( );
			return FALSE;
...
static bool32 mevent_send_func( struct mevent_srv_sub* svr )
{
	...
	case 0:
		if( IsLinkTaskFinished( ) )
		{
			...
			header.crc   = CalcCRC16WithTable(
                                svr->sendBfr, svr->sendSize );
	...
	case 1:
		if( IsLinkTaskFinished( ) )
		{
			if( mevent_has_received( svr->sendPlayerNo ) )
			{
				...
				{
					SendBlock( 0,
						svr->sendBfr + blocksiz,
						svr->sendSize - blocksiz );
					++svr->sendCounter;
					++svr->seqno;
				}
				else
	...
	case 2:
		if( IsLinkTaskFinished( ) )
		{
			if( CalcCRC16WithTable( svr->sendBfr,
				    svr->sendSize ) != svr->sendCRC )
				LinkRfu_FatalError( );
			else
				++svr->seqno;
		}
