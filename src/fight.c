...
void sub_810CF30( struct Sprite* sprite )
{
	...
	xMod = GetBattlerSpriteCoordAttr( battler, BATTLER_COORD_ATTR_WIDTH ) /
		2;
	yMod = GetBattlerSpriteCoordAttr(
	...
	sprite->data[7] = CreateSprite( &gBasicHitSplatSpriteTemplate,
		sprite->pos1.x,
		sprite->pos1.y,
...
static void sub_810D0B8( struct Sprite* sprite )
{
	if( sprite->data[0] == 0 )
	{
		...
		{
			FreeOamMatrix(
				gSprites[sprite->data[7]].oam.matrixNum );
...
void sub_810D714( struct Sprite* sprite )
{
	...
	sprite->data[0] = gBattleAnimArgs[3];
	StorePointerInVars( &sprite->data[4],
		&sprite->data[5],
		(void*)( sprite->pos1.y << 8 ) );
