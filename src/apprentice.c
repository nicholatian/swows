...
static void CreateChooseAnswerTask(
	bool8 noBButton, u8 itemsCount, u8 windowId );
...
const struct ApprenticeTrainer gApprentices[] = {
	...
	},
	{
		...
		.id            = 3,
		.easyChatWords =
			{0x100B, 0x1E0F, 0x1039, 0x1421, 0xC03, 0xFFFF},
	},
	{
		.name          = {_( "ヨウカ" ),
                        _( "MACY" ),
		...
		.id            = 4,
		.easyChatWords =
			{0x1E0F, 0x1014, 0x1006, 0x280F, 0x1C1C, 0x1C13},
	},
	{
		.name          = {_( "ヤスシ" ),
                        _( "DONTE" ),
	...
	},
	{
		...
		.id            = 11,
		.easyChatWords =
			{0x280E, 0x103D, 0x240F, 0xA14, 0x1E23, 0x1024},
	},
	{
		.name          = {_( "リュウジ" ),
                        _( "LAMONT" ),
		...
		.id            = 12,
		.easyChatWords =
			{0x1020, 0x62E, 0x100B, 0x1E22, 0x1E0F, 0x100B},
	},
	{
		.name          = {_( "カツアキ" ),
                        _( "TYRESE" ),
	...
	},
	{
		...
		.id            = 14,
		.easyChatWords =
			{0xA01, 0x1017, 0x1243, 0x1E22, 0x100B, 0x280F},
	},
	{
		.name          = {_( "ローウェン" ),
                        _( "ARTURO" ),
		...
		.id            = 15,
		.easyChatWords =
			{0x1E0F, 0x1404, 0x102F, 0x1006, 0x1020, 0xE03},
	},
};
...
static const u8 gUnknown_086114D3[] = {0x01,
	0x01,
	0x01,
	0x02,
	0x02,
...
static const u8 gUnknown_08611548[8] = {
...
void CopyFriendsApprenticeChallengeText( u8 saveblockApprenticeId )
{
	...
	StringCopy7( gStringVar1,
		gSaveBlock2Ptr->apprentices[saveblockApprenticeId]
			.playerName );
	ConvertInternationalString( gStringVar1,
		gSaveBlock2Ptr->apprentices[saveblockApprenticeId].language );
	ConvertIntToDecimalStringN( gStringVar2,
		gSaveBlock2Ptr->apprentices[saveblockApprenticeId].number,
		STR_CONV_MODE_RIGHT_ALIGN,
		i );
	str = gUnknown_08611330
		[gSaveBlock2Ptr->apprentices[saveblockApprenticeId].id];
...
void ResetAllApprenticeData( void )
{
	...
	{
		for( j = 0; j < 6; j++ )
			gSaveBlock2Ptr->apprentices[i].easyChatWords[j] =
				0xFFFF;
...
static void sub_819FBC8( void )
{
	if( gSaveBlock2Ptr->apprentices[0].number == 0 )
	{
		do
		{
			PLAYER_APPRENTICE.id = gUnknown_08611548[Random( ) %
				ARRAY_COUNT( gUnknown_08611548 )];
		} while( PLAYER_APPRENTICE.id ==
...
static void sub_819FC60( void )
{
	...
	for( i = 0; i < 3; i++ )
		PLAYER_APPRENTICE.monIds[i] = ( ( array[i * 2] & 0xF ) << 4 ) |
			( ( array[i * 2 + 1] ) & 0xF );
}
...
static void sub_819FD64( void )
{
	...
	for( i = 0; i < 9; i++ )
	{
		...
		if( sp_0[i] != 3 )
		{
			PLAYER_APPRENTICE.field_B8[i].unk0_1 =
				sub_819FCF8( sp_0[i], sp_C, &sp_10 );
			...
			if( sp_0[i] == 2 )
			{
				do
				{
					rand1 = Random( ) % 4;
					for( j = 0; j <
						gUnknown_030062EC->unk0 + 1;
						j++ )
					{
						if( gUnknown_030062EC
								->unk20[id]
								       [j] ==
							rand1 )
							break;
				...
				gUnknown_030062EC
					->unk20[id][gUnknown_030062EC->unk0] =
					rand1;
				PLAYER_APPRENTICE.field_B8[i].unk0_2 = rand1;
				PLAYER_APPRENTICE.field_B8[i]
					.unk2 = sub_819FF98(
					PLAYER_APPRENTICE.field_B8[i].unk0_1 );
			}
		}
...
#define APPRENTICE_SPECIES_ID( speciesArrId, monId ) \
	speciesArrId = \
		( PLAYER_APPRENTICE.monIds[monId] >> \
			( ( ( PLAYER_APPRENTICE.field_B2_0 >> monId ) & 1 ) \
				<< 2 ) ) & \
		0xF; \
	do \
	{ \
...
#define APPRENTICE_SPECIES_ID_2( speciesArrId, monId ) \
	{ \
		u8 a0 = ( ( PLAYER_APPRENTICE.field_B2_0 >> monId ) & 1 ); \
	...
	while( i <= MAX_MON_MOVES )
	{
				...
				do
				{
					id = Random( ) %
						( NUM_TECHNICAL_MACHINES +
							NUM_HIDDEN_MACHINES );
					valid = CanSpeciesLearnTMHM(
						species, id );
				} while( !valid );

				moveId =
					ItemIdToBattleMoveId( ITEM_TM01 + id );
				valid = TRUE;
		...
		else
		{
			...
			else
			{
				do
				{
					u8 learnsetId = Random( ) %
						( knownMovesCount -
							MAX_MON_MOVES );
					moveId = learnset[learnsetId] & 0x1FF;
					valid  = TRUE;
					for( j = knownMovesCount -
							MAX_MON_MOVES;
						j < knownMovesCount;
						j++ )
					{
...
static u16 sub_81A0284( u8 arg0, u8 speciesTableId, u8 arg2 )
{
	...
	for( i = 0; i < count && i < PLAYER_APPRENTICE.field_B1_1 - 3; i++ )
	{
		...
		{
			moves[PLAYER_APPRENTICE.field_B8[i].unk0_2] =
				PLAYER_APPRENTICE.field_B8[i].unk2;
		}
	}
...
static void sub_81A0390( u8 arg0 )
{
	...
	for( i = 0; i < 3; i++ )
	{
		...
		apprenticeMons[i]->species = gApprentices[PLAYER_APPRENTICE.id]
						     .species[speciesTableId];
		GetLatestLearnedMoves(
			apprenticeMons[i]->species, apprenticeMons[i]->moves );
	}
	...
	for( i = 0; i < arg0; i++ )
	{
		...
		else if( var1 == 2 )
		{
			...
			{
				u32 moveSlot =
					PLAYER_APPRENTICE.field_B8[i].unk0_2;
...
static void CreateMenuWithAnswers( u8 arg0 )
{
	...
	case APPRENTICE_ASK_3SPECIES:
		...
		for( i = 0; i < 3; i++ )
		{
			...
			APPRENTICE_SPECIES_ID( speciesTableId, i );
			species = gApprentices[PLAYER_APPRENTICE.id]
					  .species[speciesTableId];
...
static u8 CreateAndShowWindow( u8 left, u8 top, u8 width, u8 height )
{
	...
	struct WindowTemplate winTemplate = CreateWindowTemplate(
		0, left + 1, top + 1, width, height, 15, 100 );
...
static void sub_81A0978( void ) { sub_819FBC8( ); }

static void sub_81A0984( void ) { sub_819FD64( ); }

static void sub_81A0990( void ) { PLAYER_APPRENTICE.field_B1_1++; }
...
static void sub_81A0DD4( void )
{
	...
	for( i = 0; i < 9 && PLAYER_APPRENTICE.field_B8[i].unk0_0;
		count++, i++ )
		;
	...
	if( gSpecialVar_0x8005 == 2 )
	{
		if( PLAYER_APPRENTICE.field_B1_1 < 3 )
		{
			id1 = PLAYER_APPRENTICE.monIds[PLAYER_APPRENTICE
							       .field_B1_1] >>
				4;
			gUnknown_030062F0->unk2 =
				gApprentices[PLAYER_APPRENTICE.id]
					.species[id1];

			id2 = PLAYER_APPRENTICE
					.monIds[PLAYER_APPRENTICE.field_B1_1] &
				0xF;
			gUnknown_030062F0->unk0 =
				gApprentices[PLAYER_APPRENTICE.id]
					.species[id2];
		}
	}
	else if( gSpecialVar_0x8005 == 3 )
	{
		if( PLAYER_APPRENTICE.field_B1_1 >= 3 &&
			...
			PLAYER_APPRENTICE
					.field_B8[PLAYER_APPRENTICE
							  .field_B1_1 -
						3]
					.unk0_0 == 2 )
		{
			count = PLAYER_APPRENTICE
					.field_B8[PLAYER_APPRENTICE
							  .field_B1_1 -
						3]
					.unk0_1;
			...
			gUnknown_030062F0->unk0 =
				gApprentices[PLAYER_APPRENTICE.id]
					.species[id1];
			gUnknown_030062F0->unk4 = sub_81A0284( count,
				id1,
				PLAYER_APPRENTICE
					.field_B8[PLAYER_APPRENTICE
							  .field_B1_1 -
...
static void sub_81A0FE4( void ) { FREE_AND_SET_NULL( gUnknown_030062F0 ); }
...
static void sub_81A0FFC( void )
{
	...
	case APPRENTICE_BUFF_SPECIES1:
		StringCopy(
			stringDst, gSpeciesNames[gUnknown_030062F0->unk0] );
		break;
	case APPRENTICE_BUFF_SPECIES2:
		StringCopy(
			stringDst, gSpeciesNames[gUnknown_030062F0->unk2] );
		break;
	...
	case APPRENTICE_BUFF_ITEM:
		StringCopy( stringDst,
			ItemId_GetName( PLAYER_APPRENTICE
						.field_B8[PLAYER_APPRENTICE
								  .field_B1_1 -
							3]
						.unk2 ) );
	...
	case APPRENTICE_BUFF_NAME:
		TVShowConvertInternationalString( text,
			GetApprenticeNameInLanguage(
				PLAYER_APPRENTICE.id, LANGUAGE_ENGLISH ),
			LANGUAGE_ENGLISH );
	...
	case APPRENTICE_BUFF_EASY_CHAT:
		FrontierSpeechToString(
			gSaveBlock2Ptr->apprentices[0].easyChatWords );
		StringCopy( stringDst, gStringVar4 );
		break;
	case APPRENTICE_BUFF_SPECIES4:
		if( PLAYER_APPRENTICE.field_B1_2 < 3 )
		{
			APPRENTICE_SPECIES_ID(
				speciesArrayId, PLAYER_APPRENTICE.field_B1_2 );
		}
		else
		{
			speciesArrayId = 0;
		...
		StringCopy( stringDst,
			gSpeciesNames[gApprentices[PLAYER_APPRENTICE.id]
...
static void sub_81A1218( void ) { sub_81AAC28( ); }
...
static void sub_81A1224( void )
{
	...
	for( count = 0, j = 0; j < 9 && PLAYER_APPRENTICE.field_B8[j].unk0_0;
		count++, j++ )
		;
	...
	for( i = 0; i < count && i < PLAYER_APPRENTICE.field_B1_1 - 3; i++ )
	{
		...
		{
			PLAYER_APPRENTICE
				.field_B8[PLAYER_APPRENTICE.field_B1_1 - 3]
				.unk0_3 = 0;
			PLAYER_APPRENTICE
				.field_B8[PLAYER_APPRENTICE.field_B1_1 - 3]
				.unk2      = gSpecialVar_0x8005;
			gSpecialVar_Result = 0;
			return;
		}
	}

	PLAYER_APPRENTICE.field_B8[PLAYER_APPRENTICE.field_B1_1 - 3].unk0_3 =
		1;
	PLAYER_APPRENTICE.field_B8[PLAYER_APPRENTICE.field_B1_1 - 3].unk2 =
		gSpecialVar_0x8005;
	gSpecialVar_Result = 1;
}
...
static void sub_81A1370( void )
{
	...
	for( i = 0; i < 3; i++ )
	{
		...
		{
			gSaveBlock2Ptr->apprentices[i + 1] =
...
static void sub_81A1438( void )
{
	...
	gSaveBlock2Ptr->apprentices[0].lvlMode =
		PLAYER_APPRENTICE.activeLvlMode;
...
static void sub_81A150C( void )
{
	...
	u8 mapObjectGfxId;
	u8 class =
		gApprentices[gSaveBlock2Ptr->apprentices[0].id].facilityClass;
...
static void sub_81A1638( void ) { gSpecialVar_0x8004 = 1; }

static void sub_81A1644( void ) { gSpecialVar_0x8004 = 1; }

const u8* GetApprenticeNameInLanguage( u32 apprenticeId, s32 language )
{
	const struct ApprenticeTrainer* apprentice =
		&gApprentices[apprenticeId];
...
static void Task_ExecuteFuncAfterButtonPress( u8 taskId )
{
	...
	{
		gUnknown_030062F4 =
			(void*)( u32 )( ( (u16)gTasks[taskId].data[0] |
