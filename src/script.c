...
void InitScriptContext(
	struct ScriptContext* ctx, void* cmdTable, void* cmdTableEnd )
{
	s32 i;
...
u32 ScriptReadWord( struct ScriptContext* ctx )
{
	...
	return ( ( ( ( ( value3 << 8 ) + value2 ) << 8 ) + value1 ) << 8 ) +
		value0;
}

void ScriptContext2_Enable( void ) { sScriptContext2Enabled = TRUE; }

void ScriptContext2_Disable( void ) { sScriptContext2Enabled = FALSE; }

bool8 ScriptContext2_IsEnabled( void ) { return sScriptContext2Enabled; }
...
void ScriptContext1_Stop( void ) { sScriptContext1Status = 1; }
...
u8* MapHeaderGetScriptTable( u8 tag )
{
	...
	while( 1 )
	{
		...
		{
			mapScripts++;
			return (u8*)( mapScripts[0] + ( mapScripts[1] << 8 ) +
				( mapScripts[2] << 16 ) +
				( mapScripts[3] << 24 ) );
		}
		mapScripts += 5;
...
{
	u8* ptr =
		MapHeaderCheckScriptTable( MAP_SCRIPT_ON_WARP_INTO_MAP_TABLE );
	if( ptr )
		ScriptContext2_RunNewScript( ptr );
}
...
bool8 InitRamScript(
	const u8* script, u16 scriptSize, u8 mapGroup, u8 mapNum, u8 objectId )
{
...
const u8* GetRamScript( u8 objectId, const u8* script )
{
	...
	if( scriptData->objectId != objectId )
		return script;
	if( CalculateRamScriptChecksum( ) !=
		gSaveBlock1Ptr->ramScript.checksum )
	{
		ClearRamScript( );
		return script;
	}
