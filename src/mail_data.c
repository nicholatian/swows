...
bool8 MonHasMail( struct Pokemon* mon )
{
	...
	if( ItemIsMail( heldItem ) &&
		GetMonData( mon, MON_DATA_MAIL ) != 0xFF )
		return TRUE;
	else
		return FALSE;
}
	...
	for( id = 0; id < PARTY_SIZE; id++ )
	{
		if( gSaveBlock1Ptr->mail[id].itemId == 0 )
		{
			...
			for( i = 0; i < PLAYER_NAME_LENGTH + 1 - 1; i++ )
				gSaveBlock1Ptr->mail[id].playerName[i] =
					gSaveBlock2Ptr->playerName[i];
			gSaveBlock1Ptr->mail[id].playerName[i] = EOS;
			PadNameString( gSaveBlock1Ptr->mail[id].playerName,
				CHAR_SPACE );
...
u16 SpeciesToMailSpecies( u16 species, u32 personality )
{
	...
	{
		u32 species =
			GetUnownLetterByPersonality( personality ) + 30000;
		return species;
	}
...
u16 MailSpeciesToSpecies( u16 mailSpecies, u16* buffer )
{
	...
	if( mailSpecies >= 30000 &&
		mailSpecies < ( 30000 + UNOWN_FORM_COUNT ) )
	{
...
static bool32 DummyMailFunc( void ) { return FALSE; }
	...
	for( i = PARTY_SIZE; i < MAIL_COUNT; i++ )
	{
		if( gSaveBlock1Ptr->mail[i].itemId == ITEM_NONE )
		{
			memcpy( &gSaveBlock1Ptr->mail[i],
				&gSaveBlock1Ptr->mail[GetMonData(
					mon, MON_DATA_MAIL )],
