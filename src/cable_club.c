...
static void sub_80B2688( u8 taskId )
{
	...
	if( sub_80B252C( taskId ) == TRUE || sub_80B2578( taskId ) == TRUE ||
		playerCount < 2 )
		return;
...
static void sub_80B270C( u8 taskId )
{
	if( sub_80B252C( taskId ) == TRUE || sub_80B25CC( taskId ) == TRUE ||
...
static void sub_80B275C( u8 taskId )
{
	...
	sub_80B241C( data[5] );
	ConvertIntToDecimalStringN(
		gStringVar1, linkPlayerCount, STR_CONV_MODE_LEFT_ALIGN, 1 );
...
static void sub_80B28A8( u8 taskId )
{
	...
	if( sub_80B24F8( taskId ) == TRUE || sub_80B2D6C( taskId ) == TRUE )
		return;
...
static void sub_80B2918( u8 taskId )
{
	...
	if( sub_80B252C( taskId ) == TRUE || sub_80B24F8( taskId ) == TRUE )
		return;
...
static void task_map_chg_seq_0807EC34( u16* a0, u32 taskId )
{
	...
	if( *a0 == 1 )
	{
		...
		{
			if( sub_80B2AF4( trainerCards[0].monSpecies,
...
void sub_80B3254( void ) { SaveGame( ); }
...
{
	if( gSpecialVar_0x8004 == 1 || gSpecialVar_0x8004 == 2 ||
		gSpecialVar_0x8004 == 5 || gSpecialVar_0x8004 == 9 )
	{
		LoadPlayerParty( );
		SavePlayerBag( );
	}
...
static void sub_80B3950( void ) { CreateTask( sub_80B37FC, 80 ); }
...
bool32 GetLinkTrainerCardColor( u8 linkPlayerIndex )
{
	...
	StringCopy( gStringVar2,
		gTrainerCardColorNames[trainerCardColorIndex - 1] );
	return TRUE;
}
