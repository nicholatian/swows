...
{
	memset( &sPokeblockFeeders[index],
		0,
		sizeof( struct PokeblockFeeder ) );
}
...
void GetPokeblockFeederWithinRange( void )
{
	...
	for( i = 0; i < NUM_POKEBLOCK_FEEDERS; i++ )
	{
		if( gSaveBlock1Ptr->location.mapNum ==
			sPokeblockFeeders[i].mapNum )
		{
			// Get absolute value of x and y distance from
			// Pokeblock feeder on current map.
...
void SafariZoneActivatePokeblockFeeder( u8 pkblId )
{
	...
	for( i = 0; i < NUM_POKEBLOCK_FEEDERS; i++ )
	{
			...
			sPokeblockFeeders[i].y == 0 )
		{
			...
			sPokeblockFeeders[i].pokeblock =
				gSaveBlock1Ptr->pokeblocks[pkblId];
