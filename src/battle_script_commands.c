...
#define DEFENDER_IS_PROTECTED \
	( ( gProtectStructs[gBattlerTarget].protected ) && \
		( gBattleMoves[gCurrentMove].flags & \
			FLAG_PROTECT_AFFECTED ) )

// this file's functions
static bool8 IsTwoTurnsMove( u16 move );
static void TrySetDestinyBondToHappen( void );
static u8 AttacksThisTurn( u8 battlerId, u16 move ); // Note: returns 1 if it's
						     // a charging turn,
						     // otherwise 2.
...
static const u32 sStatusFlagsForMoveEffects[] = {0x00000000,
	STATUS1_SLEEP,
	STATUS1_POISON,
	STATUS1_BURN,
	STATUS1_FREEZE,
...
static const u16 sUnknown_0831C2C8[] =
	INCBIN_U16( "graphics/battle_interface/unk_battlebox.gbapal" );
static const u32 sUnknown_0831C2E8[] =
	INCBIN_U32( "graphics/battle_interface/unk_battlebox.4bpp.lz" );

// unused
static const u8 sRubyLevelUpStatBoxStats[] = {MON_DATA_MAX_HP,
	MON_DATA_SPATK,
	MON_DATA_ATK,
	MON_DATA_SPDEF,
	MON_DATA_DEF,
...
static const u16 sNaturePowerMoves[] = {MOVE_STUN_SPORE,
	MOVE_RAZOR_LEAF,
	MOVE_EARTHQUAKE,
	MOVE_HYDRO_PUMP,
	MOVE_SURF,
...
static const u8 sBallCatchBonuses[] = {
	...
	15,
	10,
	15 // Ultra, Great, Poke, Safari
};

const ALIGNED( 4 ) u8 gUnknown_0831C494[][4] = {{0x3d, 0x44, 0x3d, 0x44},
	{0x14, 0x2d, 0x54, 0x5c},
	{0x46, 0x55, 0x20, 0x5c},
	{0x26, 0x45, 0x46, 0x55},
	{0x14, 0x5a, 0x46, 0x5c},
...
static const u8 sUnknown_0831C4F8[] = {0x03,
	0x00,
	0x01,
	0x00,
	0x00,
...
static void atk00_attackcanceler( void )
{
	...
	if( AbilityBattleEffects(
		    ABILITYEFFECT_MOVES_BLOCK, gBattlerTarget, 0, 0, 0 ) )
		return;
	if( !gBattleMons[gBattlerAttacker].pp[gCurrMovePos] &&
		gCurrentMove != MOVE_STRUGGLE &&
		!( gHitMarker &
			( HITMARKER_x800000 | HITMARKER_NO_ATTACKSTRING ) ) &&
	...
	{
		PressurePPLose(
			gBattlerAttacker, gBattlerTarget, MOVE_MAGIC_COAT );
...
static void JumpIfMoveFailed( u8 adder, u16 move )
{
	...
	else
	{
		...
		if( AbilityBattleEffects( ABILITYEFFECT_ABSORBING,
			    gBattlerTarget,
			    0,
			    0,
			    move ) )
...
static bool8 AccuracyCalcHelper( u16 move )
{
	if( gStatuses3[gBattlerTarget] & STATUS3_ALWAYS_HITS &&
		gDisableStructs[gBattlerTarget].battlerWithSureHit ==
			gBattlerAttacker )
	{
		JumpIfMoveFailed( 7, move );
		return TRUE;
...
static void atk01_accuracycheck( void )
{
	...
	else
	{
		...
		{
			u8 acc = gBattleMons[gBattlerAttacker]
					 .statStages[STAT_ACC];
			buff = acc;
		}
		else
		...
		if( gBattleMons[gBattlerAttacker].ability ==
			ABILITY_COMPOUND_EYES )
			calc = ( calc * 130 ) / 100; // 1.3 compound eyes boost
		if( WEATHER_HAS_EFFECT &&
			gBattleMons[gBattlerTarget].ability ==
				ABILITY_SAND_VEIL &&
		...
		{
			holdEffect = ItemId_GetHoldEffect(
				gBattleMons[gBattlerTarget].item );
			param = ItemId_GetHoldEffectParam(
				gBattleMons[gBattlerTarget].item );
		}
...
static void atk03_ppreduce( void )
{
	...
	if( !( gHitMarker &
		    ( HITMARKER_NO_PPDEDUCT | HITMARKER_NO_ATTACKSTRING ) ) &&
...
static void atk06_typecalc( void )
{
	...
	else
	{
		while( TYPE_EFFECT_ATK_TYPE( i ) != TYPE_ENDTABLE )
		{
			...
			{
				if( gBattleMons[gBattlerTarget].status2 &
					STATUS2_FORESIGHT )
					break;
				i += 3;
				continue;
			...
			{
				// check type1
				if( TYPE_EFFECT_DEF_TYPE( i ) ==
					gBattleMons[gBattlerTarget].type1 )
					ModulateDmgByType(
						TYPE_EFFECT_MULTIPLIER( i ) );
				// check type2
				if( TYPE_EFFECT_DEF_TYPE( i ) ==
						gBattleMons[gBattlerTarget]
							.type2 &&
...
static void CheckWonderGuardAndLevitate( void )
{
		...
		AttacksThisTurn( gBattlerAttacker, gCurrentMove ) == 2 )
	{
			...
			gBattleMoves[gCurrentMove].power )
		{
			...
			gBattleCommunication[6] = 3;
			RecordAbilityBattle(
				gBattlerTarget, ABILITY_WONDER_GUARD );
		}
	}
}

static void ModulateDmgByType2( u8 multiplier,
	u16 move,
...
u8 TypeCalc( u16 move, u8 attacker, u8 defender )
{
	...
	else
	{
		while( TYPE_EFFECT_ATK_TYPE( i ) != TYPE_ENDTABLE )
		{
			if( TYPE_EFFECT_ATK_TYPE( i ) == TYPE_FORESIGHT )
			{
				if( gBattleMons[defender].status2 &
					STATUS2_FORESIGHT )
					break;
				i += 3;
				continue;
...
u8 AI_TypeCalc( u16 move, u16 targetSpecies, u8 targetAbility )
{
	s32 i    = 0;
	u8 flags = 0;
	u8 type1 = gBaseStats[targetSpecies].type1,
...
static void atk08_adjustnormaldamage2( void ) // The same as 0x7 except it
					      // doesn't check for false swipe
					      // move effect.
{
	u8 holdEffect, param;
...
static void atk09_attackanimation( void )
{
	...
	else
	{
		if( ( gBattleMoves[gCurrentMove].target & MOVE_TARGET_BOTH ||
			    gBattleMoves[gCurrentMove].target &
				    MOVE_TARGET_FOES_AND_ALLY ||
			    gBattleMoves[gCurrentMove].target &
				    MOVE_TARGET_DEPENDS ) &&
			gBattleScripting.animTargetsHit )
...
static void atk0B_healthbarupdate( void )
{
	...
	{
		gActiveBattler =
			GetBattlerForBattleScript( gBattlescriptCurrInstr[1] );
		...
		{
			PrepareStringBattle(
				STRINGID_SUBSTITUTEDAMAGED, gActiveBattler );
		}
		else
		{
			s16 healthValue;
			...
			s32 maxPossibleDmgValue =
				10000; // not present in R/S, ensures
				       // that huge damage values
				       // don't change sign
	...
	if( !( gMoveResultFlags & MOVE_RESULT_NO_EFFECT ) )
	{
				...
				if( gSpecialStatuses[gActiveBattler].dmg == 0 )
					gSpecialStatuses[gActiveBattler].dmg =
						gDisableStructs[gActiveBattler]
							.substituteHP;
				gHpDealt = gDisableStructs[gActiveBattler]
						   .substituteHP;
				gDisableStructs[gActiveBattler].substituteHP =
					0;
			}
		...
		else
		{
			...
			else // hp goes down
			{
				...
				if( !gSpecialStatuses[gActiveBattler].dmg &&
				...
				{
					gProtectStructs[gActiveBattler]
...
static void atk0D_critmessage( void )
{
	if( gBattleControllerExecFlags == 0 )
	{
		...
		{
			PrepareStringBattle(
				STRINGID_CRITICALHIT, gBattlerAttacker );
...
static void atk11_printselectionstring( void )
{
	...
	BtlController_EmitPrintSelectionString(
		0, T2_READ_16( gBattlescriptCurrInstr + 1 ) );
	MarkBattlerForControllerExec( gActiveBattler );
...
static void atk13_printfromtable( void )
{
	...
	{
		const u16* ptr =
			(const u16*)T1_READ_PTR( gBattlescriptCurrInstr + 1 );
...
void SetMoveEffect( bool8 primary, u8 certain )
{
	...
	{
		gEffectBattler = gBattlerAttacker; // battlerId that effects
						   // get applied on
		gBattleCommunication[MOVE_EFFECT_BYTE] &=
			~( MOVE_EFFECT_AFFECTS_USER );
		affectsUser              = MOVE_EFFECT_AFFECTS_USER;
		gBattleScripting.battler = gBattlerTarget; // theoretically the
							   // attacker
	}
	else
	{
	...
	if( gSideStatuses[GET_BATTLER_SIDE( gEffectBattler )] &
	...
	{
		switch( sStatusFlagsForMoveEffects
				[gBattleCommunication[MOVE_EFFECT_BYTE]] )
		{
		case STATUS1_SLEEP:
			// check active uproar
			if( gBattleMons[gEffectBattler].ability !=
				ABILITY_SOUNDPROOF )
			{
		...
		case STATUS1_POISON:
					...
					certain == MOVE_EFFECT_CERTAIN ) )
			{
				...
				RecordAbilityBattle(
					gEffectBattler, ABILITY_IMMUNITY );
				...
				{
					gBattleCommunication
						[MULTISTRING_CHOOSER] = 0;
				}
				RESET_RETURN
			}
			if( ( IS_BATTLER_OF_TYPE(
				      gEffectBattler, TYPE_POISON ) ||
				    IS_BATTLER_OF_TYPE(
					    gEffectBattler, TYPE_STEEL ) ) &&
		...
		case STATUS1_BURN:
					...
					certain == MOVE_EFFECT_CERTAIN ) )
			{
				...
				RecordAbilityBattle(
					gEffectBattler, ABILITY_WATER_VEIL );
		...
		case STATUS1_TOXIC_POISON:
					...
					gEffectBattler, TYPE_STEEL ) )
			{
				...
				// It's redundant, because at this point we
				// know the status1 value is 0.
		...
		if( statusChanged == TRUE )
		{
			...
			if( sStatusFlagsForMoveEffects[gBattleCommunication
					    [MOVE_EFFECT_BYTE]] ==
				STATUS1_SLEEP )
				gBattleMons[gEffectBattler].status1 |=
					( ( Random( ) & 3 ) + 2 );
			else
				gBattleMons[gEffectBattler]
					.status1 |= sStatusFlagsForMoveEffects
					[gBattleCommunication
							[MOVE_EFFECT_BYTE]];

			gBattlescriptCurrInstr = sMoveEffectBS_Ptrs
				[gBattleCommunication[MOVE_EFFECT_BYTE]];
			...
			if( gBattleCommunication[MOVE_EFFECT_BYTE] ==
					MOVE_EFFECT_POISON ||
				gBattleCommunication[MOVE_EFFECT_BYTE] ==
					MOVE_EFFECT_TOXIC ||
				gBattleCommunication[MOVE_EFFECT_BYTE] ==
	...
	else
	{
		...
		else
		{
			...
			case MOVE_EFFECT_FLINCH:
				...
				else
				{
					if( GetBattlerTurnOrderNum(
						...
						gBattleMons[gEffectBattler]
							.status2 |= sStatusFlagsForMoveEffects
							[gBattleCommunication
									[MOVE_EFFECT_BYTE]];
					gBattlescriptCurrInstr++;
				}
			...
			case MOVE_EFFECT_PAYDAY:
				...
				BattleScriptPush( gBattlescriptCurrInstr + 1 );
				gBattlescriptCurrInstr =
					sMoveEffectBS_Ptrs[gBattleCommunication
							[MOVE_EFFECT_BYTE]];
				break;
			...
			case MOVE_EFFECT_CHARGING:
				...
				gProtectStructs[gEffectBattler].chargingTurn =
					1;
				gBattlescriptCurrInstr++;
				break;
					...
					*( gBattleStruct->wrappedBy +
					...
					for( gBattleCommunication
						...
						gBattleCommunication
							[MULTISTRING_CHOOSER]++ )
					{
						if( gBattleCommunication
								[MULTISTRING_CHOOSER] >
							4 )
							break;
						if( gTrappingMoves[gBattleCommunication
								    [MULTISTRING_CHOOSER]] ==
							gCurrentMove )
							break;
					}
			...
			case MOVE_EFFECT_RECHARGE:
				gBattleMons[gEffectBattler].status2 |=
					STATUS2_RECHARGE;
				gDisableStructs[gEffectBattler].rechargeTimer =
					2;
				gLockedMoves[gEffectBattler] = gCurrentMove;
				gBattlescriptCurrInstr++;
				break;
			...
			case MOVE_EFFECT_STEAL_ITEM: {
							...
							BATTLE_TYPE_SECRET_BASE ) ) )
				{
					...
					( gWishFutureKnock
							.knockedOffMons[side] &
						gBitTable[gBattlerPartyIndexes
								[gBattlerAttacker]] ) )
				{
					gBattlescriptCurrInstr++;
				}
						...
						ABILITY_STICKY_HOLD )
				{
					...
					gLastUsedAbility =
						gBattleMons[gBattlerTarget]
							.ability;
					RecordAbilityBattle( gBattlerTarget,
						gLastUsedAbility );
				}
				...
				{
					u16* changedItem =
						&gBattleStruct->changedItems
							 [gBattlerAttacker];
					gLastUsedItem = *changedItem =
						gBattleMons[gBattlerTarget]
							.item;
					gBattleMons[gBattlerTarget].item = 0;
					...
					*(u8*)( (u8*)( &gBattleStruct->choicedMove
								[gBattlerTarget] ) +
						0 ) = 0;
					*(u8*)( (u8*)( &gBattleStruct->choicedMove
								[gBattlerTarget] ) +
...
static void atk15_seteffectwithchance( void )
{
	...
	if( gBattleMons[gBattlerAttacker].ability == ABILITY_SERENE_GRACE )
		percentChance =
			gBattleMoves[gCurrentMove].secondaryEffectChance * 2;
	else
		percentChance =
...
static void atk16_seteffectprimary( void ) { SetMoveEffect( TRUE, 0 ); }

static void atk17_seteffectsecondary( void ) { SetMoveEffect( FALSE, 0 ); }
...
static void atk18_clearstatusfromeffect( void )
{
	...
	if( gBattleCommunication[MOVE_EFFECT_BYTE] <= MOVE_EFFECT_TOXIC )
		gBattleMons[gActiveBattler].status1 &=
			( ~sStatusFlagsForMoveEffects[gBattleCommunication
					[MOVE_EFFECT_BYTE]] );
	else
...
static void atk19_tryfaintmon( void )
{
	...
	else
	{
			...
			gBattleMons[gActiveBattler].hp == 0 )
		{
			...
			{
				if( gBattleResults.opponentFaintCounter <
					0xFF )
					gBattleResults.opponentFaintCounter++;
				gBattleResults
					.lastOpponentSpecies = GetMonData(
...
static void atk1B_cleareffectsonfaint( void )
{
	if( gBattleControllerExecFlags == 0 )
	{
		...
		FaintClearSetData( ); // Effects like attractions, trapping,
				      // etc.
		gBattlescriptCurrInstr += 2;
	}
}
...
static void atk1C_jumpifstatus( void )
{
	...
	if( gBattleMons[battlerId].status1 & flags &&
...
static void atk1D_jumpifstatus2( void )
{
	...
	if( gBattleMons[battlerId].status2 & flags &&
		gBattleMons[battlerId].hp )
		gBattlescriptCurrInstr = jumpPtr;
	else
		gBattlescriptCurrInstr += 10;
...
static void atk1E_jumpifability( void )
{
	...
	{
		battlerId =
			AbilityBattleEffects( ABILITYEFFECT_CHECK_BATTLER_SIDE,
				gBattlerAttacker,
				ability,
				0,
				0 );
	...
	{
		battlerId =
			AbilityBattleEffects( ABILITYEFFECT_CHECK_OTHER_SIDE,
				gBattlerAttacker,
				ability,
				0,
				0 );
...
static void atk20_jumpifstat( void )
{
	...
	u8 battlerId = GetBattlerForBattleScript( gBattlescriptCurrInstr[1] );
	u8 value =
		gBattleMons[battlerId].statStages[gBattlescriptCurrInstr[3]];
	...
	if( ret )
		gBattlescriptCurrInstr =
			T2_READ_PTR( gBattlescriptCurrInstr + 5 );
	else
...
static void atk23_getexp( void )
{
	...
	case 0: // check if should receive exp at all
		...
		else
		{
			...
			gBattleStruct->givenExpMons |= gBitTable
				[gBattlerPartyIndexes[gBattlerFainted]];
		}
		break;
	case 1: // calculate experience points to redistribute
	{
		...
		for( viaSentIn = 0, i = 0; i < PARTY_SIZE; i++ )
		{
			...
			if( item == ITEM_ENIGMA_BERRY )
				holdEffect =
					gSaveBlock1Ptr->enigmaBerry.holdEffect;
			else
				holdEffect = ItemId_GetHoldEffect( item );
		...
		calculatedExp =
			gBaseStats[gBattleMons[gBattlerFainted].species]
				.expYield *
			gBattleMons[gBattlerFainted].level / 7;

		if( viaExpShare ) // at least one mon is getting exp via exp
				  // share
		{
	...
	case 2: // set exp value to the poke in expgetter_id and print message
		if( gBattleControllerExecFlags == 0 )
		{
			...
			{
				// music change in wild battle after fainting a
				// poke
				if( !( gBattleTypeFlags &
					    BATTLE_TYPE_TRAINER ) &&
					gBattleMons[0].hp &&
	...
	case 3: // Set stats and give exp
		if( gBattleControllerExecFlags == 0 )
		{
			gBattleBufferB[gBattleStruct->expGetterBattlerId][0] =
				0;
			if( GetMonData(
				    &gPlayerParty[gBattleStruct
							  ->expGetterMonId],
					...
					MON_DATA_LEVEL ) != MAX_LEVEL )
			{
				gBattleResources->beforeLvlUp
					->stats[STAT_HP] = GetMonData(
					&gPlayerParty[gBattleStruct
							      ->expGetterMonId],
					MON_DATA_MAX_HP );
				gBattleResources->beforeLvlUp
					->stats[STAT_ATK] = GetMonData(
					&gPlayerParty[gBattleStruct
							      ->expGetterMonId],
					MON_DATA_ATK );
				gBattleResources->beforeLvlUp
					->stats[STAT_DEF] = GetMonData(
					&gPlayerParty[gBattleStruct
							      ->expGetterMonId],
					MON_DATA_DEF );
				gBattleResources->beforeLvlUp
					->stats[STAT_SPEED] = GetMonData(
					&gPlayerParty[gBattleStruct
							      ->expGetterMonId],
					MON_DATA_SPEED );
				gBattleResources->beforeLvlUp
					->stats[STAT_SPATK] = GetMonData(
					&gPlayerParty[gBattleStruct
							      ->expGetterMonId],
					MON_DATA_SPATK );
				gBattleResources->beforeLvlUp
					->stats[STAT_SPDEF] = GetMonData(
					&gPlayerParty[gBattleStruct
							      ->expGetterMonId],
					MON_DATA_SPDEF );
	...
	case 4: // lvl up if necessary
		if( gBattleControllerExecFlags == 0 )
		{
			...
			if( gBattleBufferB[gActiveBattler][0] ==
					...
					RET_VALUE_LEVELED_UP )
			{
				...
				gBattleMoveDamage      = ( gBattleBufferB
                                                              [gActiveBattler]
                                                              [2] |
						...
						BATTLE_TYPE_DOUBLE ) )
				{
					...
					gBattleMons[2].defense = GetMonData(
						&gPlayerParty[gBattleStruct
								      ->expGetterMonId],
						MON_DATA_DEF );
					// Duplicated again, but this time
	...
	case 5: // looper increment
		if( gBattleMoveDamage ) // there is exp to give, goto case 3
					// that gives exp
		{
...
static void atk24( void )
{
		...
		for( i = 0; i < PARTY_SIZE; i++ )
		{
			if( GetMonData( &gPlayerParty[i], MON_DATA_SPECIES ) &&
				...
				( !( gBattleTypeFlags & BATTLE_TYPE_ARENA ) ||
					!( gBattleStruct->arenaLostPlayerMons &
						gBitTable[i] ) ) )
			{
				HP_count += GetMonData(
					&gPlayerParty[i], MON_DATA_HP );
	...
	for( HP_count = 0, i = 0; i < PARTY_SIZE; i++ )
	{
		if( GetMonData( &gEnemyParty[i], MON_DATA_SPECIES ) &&
			...
			( !( gBattleTypeFlags & BATTLE_TYPE_ARENA ) ||
				!( gBattleStruct->arenaLostOpponentMons &
					gBitTable[i] ) ) )
		{
			...
			( BATTLE_TYPE_LINK | BATTLE_TYPE_x2000000 ) ) )
	{
		...
		{
			if( HITMARKER_UNK( i ) & gHitMarker &&
				!gSpecialStatuses[i].flag40 )
				foundPlayer++;
		}
		...
		if( gBattleTypeFlags & BATTLE_TYPE_MULTI )
		{
			if( foundOpponent + foundPlayer > 1 )
				gBattlescriptCurrInstr = T2_READ_PTR(
					gBattlescriptCurrInstr + 1 );
			else
				gBattlescriptCurrInstr += 5;
		}
...
static void atk27_decrementmultihit( void )
{
	if( --gMultiHitCounter == 0 )
		...
		gBattlescriptCurrInstr =
			T2_READ_PTR( gBattlescriptCurrInstr + 1 );
}
...
static void atk3C_return( void ) { BattleScriptPop( ); }
...
static void atk43_jumpifabilitypresent( void )
{
	if( AbilityBattleEffects( ABILITYEFFECT_CHECK_ON_FIELD,
		    0,
		    gBattlescriptCurrInstr[1],
		    0,
		    0 ) )
		gBattlescriptCurrInstr =
			T1_READ_PTR( gBattlescriptCurrInstr + 2 );
	else
		gBattlescriptCurrInstr += 6;
}
...
static void atk45_playanimation( void )
{
		...
		gBattlescriptCurrInstr[2] == B_ANIM_SUBSTITUTE_FADE )
	{
		BtlController_EmitBattleAnimation(
			0, gBattlescriptCurrInstr[2], *argumentPtr );
		MarkBattlerForControllerExec( gActiveBattler );
		gBattlescriptCurrInstr += 7;
	}
...
static void atk46_playanimation2( void ) // animation Id is stored in the first
					 // pointer
{
	const u16* argumentPtr;
...
static void atk48_playstatchangeanimation( void )
{
	...
	if( gBattlescriptCurrInstr[3] & ATK48_STAT_NEGATIVE ) // goes down
	{
		...
		if( changeableStatsCount > 1 ) // more than one stat, so the
					       // color is gray
		{
			if( gBattlescriptCurrInstr[3] & ATK48_STAT_BY_TWO )
	...
	if( gBattlescriptCurrInstr[3] & ATK48_ONLY_MULTIPLE &&
	...
	{
		BtlController_EmitBattleAnimation(
			0, B_ANIM_STATS_CHANGE, statAnimId );
		MarkBattlerForControllerExec( gActiveBattler );
		if( gBattlescriptCurrInstr[3] & ATK48_ONLY_MULTIPLE &&
...
static void atk49_moveend( void )
{
	...
	if( gBattleMons[gBattlerAttacker].item == ITEM_ENIGMA_BERRY )
		...
		holdEffectAtk = ItemId_GetHoldEffect(
	...
	do
	{
		...
		case ATK49_SYNCHRONIZE_TARGET: // target synchronize
			if( AbilityBattleEffects( ABILITYEFFECT_SYNCHRONIZE,
				    gBattlerTarget,
				    0,
				    0,
				    0 ) )
			...
			gBattleScripting.atk49_state++;
			break;
		case ATK49_MOVE_END_ABILITIES: // Such as abilities activating
					       // on contact(Poison Spore,
					       // Rough Skin, etc.).
			if( AbilityBattleEffects( ABILITYEFFECT_MOVE_END,
				    gBattlerTarget,
				    0,
				    0,
				    0 ) )
		...
		case ATK49_STATUS_IMMUNITY_ABILITIES: // status immunities
			if( AbilityBattleEffects(
				    ABILITYEFFECT_IMMUNITY, 0, 0, 0, 0 ) )
				effect =
					TRUE; // it loops through all battlers,
					      // so we increment after its done
					      // with all battlers
			else
				gBattleScripting.atk49_state++;
			break;
		...
		case ATK49_ITEM_EFFECTS_ALL: // item effects for all battlers
			if( ItemBattleEffects(
				    ITEMEFFECT_MOVE_END, 0, FALSE ) )
				effect = TRUE;
			else
				gBattleScripting.atk49_state++;
			break;
		case ATK49_KINGSROCK_SHELLBELL: // king's rock and shell bell
			if( ItemBattleEffects( ITEMEFFECT_KINGSROCK_SHELLBELL,
				    0,
				    FALSE ) )
				effect = TRUE;
			gBattleScripting.atk49_state++;
			break;
		case ATK49_ATTACKER_INVISIBLE: // make attacker sprite
					       // invisible
			if( gStatuses3[gBattlerAttacker] &
		...
		case ATK49_TARGET_VISIBLE: // make target sprite visible
			if( !gSpecialStatuses[gBattlerTarget]
					.restoredBattlerSprite &&
				gBattlerTarget < gBattlersCount &&
				!( gStatuses3[gBattlerTarget] &
					STATUS3_SEMI_INVULNERABLE ) )
			{
				gActiveBattler = gBattlerTarget;
				BtlController_EmitSpriteInvisibility(
		...
		case ATK49_UPDATE_LAST_MOVES:
			...
			{
				gLastPrintedMoves[gBattlerAttacker] =
					gChosenMove;
			}
			if( !( gAbsentBattlerFlags &
				    gBitTable[gBattlerAttacker] ) &&
				!( gBattleStruct->field_91 &
					gBitTable[gBattlerAttacker] ) &&
				gBattleMoves[originallyUsedMove].effect !=
					EFFECT_BATON_PASS )
			{
				if( gHitMarker & HITMARKER_OBEYS )
				{
					gLastMoves[gBattlerAttacker] =
						gChosenMove;
					gLastResultingMoves[gBattlerAttacker] =
						gCurrentMove;
				}
				else
				{
				...
				{
					gLastLandedMoves[gBattlerTarget] =
						0xFFFF;
				}
			}
			gBattleScripting.atk49_state++;
		...
		case ATK49_MIRROR_MOVE: // mirror move
			if( !( gAbsentBattlerFlags &
				...
				!( gBattleStruct->field_91 &
					gBitTable[gBattlerAttacker] ) &&
				gBattleMoves[originallyUsedMove].flags &
				...
				!( gMoveResultFlags & MOVE_RESULT_NO_EFFECT ) )
			{
				...
				*( gBattleStruct->lastTakenMove +
					gBattlerTarget * 2 + 0 ) = gChosenMove;
				*( gBattleStruct->lastTakenMove +
					gBattlerTarget * 2 + 1 ) =
					gChosenMove >> 8;
			...
			gBattleScripting.atk49_state++;
			break;
		case ATK49_NEXT_TARGET: // For moves hitting two opposing
					// Pokemon.
		...
		if( arg1 == 2 && arg2 == gBattleScripting.atk49_state )
			gBattleScripting.atk49_state = ATK49_COUNT;
	} while( gBattleScripting.atk49_state != ATK49_COUNT &&
		effect == FALSE );
...
static void atk4C_getswitchedmondata( void )
{
	...
	BtlController_EmitGetMonData( 0,
		REQUEST_ALL_BATTLE,
...
static void atk4D_switchindataupdate( void )
{
	...
	PREPARE_MON_NICK_BUFFER( gBattleTextBuff1,
		gActiveBattler,
		gBattlerPartyIndexes[gActiveBattler] );
...
static void atk4E_switchinanim( void )
{
	...
	if( GetBattlerSide( gActiveBattler ) == B_SIDE_OPPONENT &&
		!( gBattleTypeFlags &
			( BATTLE_TYPE_LINK | BATTLE_TYPE_EREADER_TRAINER |
				BATTLE_TYPE_x2000000 |
				BATTLE_TYPE_TRAINER_HILL |
				BATTLE_TYPE_FRONTIER ) ) )
	...
	BtlController_EmitSwitchInAnim( 0,
		gBattlerPartyIndexes[gActiveBattler],
		gBattlescriptCurrInstr[2] );
	MarkBattlerForControllerExec( gActiveBattler );
...
static void atk4F_jumpifcantswitch( void )
{
	...
	gActiveBattler = GetBattlerForBattleScript(
		gBattlescriptCurrInstr[1] & ~( ATK4F_DONT_CHECK_STATUSES ) );
	...
	{
#if !defined( NONMATCHING ) && !defined( __VSCODE_INTELLISENSE__ )
		asm( "" ::: "r5" );
#endif // NONMATCHING
		if( GetBattlerSide( gActiveBattler ) == B_SIDE_OPPONENT )
			party = gEnemyParty;
	...
	else if( gBattleTypeFlags & BATTLE_TYPE_MULTI )
	{
		...
		else
		{
			...
			i = 0;
			if( GetLinkTrainerFlankId( GetBattlerMultiplayerId(
				    gActiveBattler ) ) == TRUE )
				i = 3;
		}
	...
	else
	{
		...
		if( GetBattlerSide( gActiveBattler ) == B_SIDE_OPPONENT )
		{
			battlerIn1 = GetBattlerAtPosition(
...
static void atk50_openpartyscreen( void )
{
	...
	{
		if( ( gBattleTypeFlags &
			    ( BATTLE_TYPE_DOUBLE | BATTLE_TYPE_MULTI ) ) !=
			BATTLE_TYPE_DOUBLE )
		{
			for( gActiveBattler = 0;
				gActiveBattler < gBattlersCount;
			...
			{
				if( gHitMarker &
					HITMARKER_FAINTED( gActiveBattler ) )
				{
					if( HasNoMonsToSwitch(
						    gActiveBattler, 6, 6 ) )
					{
						gAbsentBattlerFlags |= gBitTable
							[gActiveBattler];
						gHitMarker &= ~( HITMARKER_FAINTED(
							gActiveBattler ) );
		...
		else if( gBattleTypeFlags & BATTLE_TYPE_DOUBLE )
		{
			...
			if( gBitTable[0] & hitmarkerFaintBits )
			{
				...
				else if( !gSpecialStatuses[gActiveBattler]
						  .flag40 )
				{
					sub_804CF10(
						gBattleStruct
	...
	else
	{
		if( gBattlescriptCurrInstr[1] & 0x80 )
			hitmarkerFaintBits =
				PARTY_CHOOSE_MON; // Used here as the caseId
						  // for the EmitChoose
						  // function.
		else
			hitmarkerFaintBits = PARTY_MUST_CHOOSE_MON;

		battlerId = GetBattlerForBattleScript(
			gBattlescriptCurrInstr[1] & ~( 0x80 ) );
...
static void atk51_switchhandleorder( void )
{
		...
		for( i = 0; i < gBattlersCount; i++ )
		{
			if( gBattleBufferB[i][0] == 0x22 )
			{
				*( gBattleStruct->monToSwitchIntoId + i ) =
					gBattleBufferB[i][1];
				if( !( gBattleStruct->field_93 &
					    gBitTable[i] ) )
				{
					RecordedBattle_SetBattlerAction(
						i, gBattleBufferB[i][1] );
					gBattleStruct->field_93 |=
						gBitTable[i];
				}
			}
	...
	case 3:
		...
		*( gBattleStruct->monToSwitchIntoId + gActiveBattler ) =
		...
		{
			*( gActiveBattler * 3 +
				(u8*)( gBattleStruct->field_60 ) + 0 ) &= 0xF;
			*( gActiveBattler * 3 +
				(u8*)( gBattleStruct->field_60 ) + 0 ) |=
				( gBattleBufferB[gActiveBattler][2] & 0xF0 );
			*( gActiveBattler * 3 +
				(u8*)( gBattleStruct->field_60 ) + 1 ) =
				gBattleBufferB[gActiveBattler][3];

			*( ( gActiveBattler ^ BIT_FLANK ) * 3 +
				(u8*)( gBattleStruct->field_60 ) + 0 ) &=
				( 0xF0 );
			*( ( gActiveBattler ^ BIT_FLANK ) * 3 +
				(u8*)( gBattleStruct->field_60 ) + 0 ) |=
				...
				4;
			*( ( gActiveBattler ^ BIT_FLANK ) * 3 +
				(u8*)( gBattleStruct->field_60 ) + 2 ) =
				gBattleBufferB[gActiveBattler][3];
		}
		else if( gBattleTypeFlags & BATTLE_TYPE_INGAME_PARTNER )
		{
			sub_80571DC( gActiveBattler,
				*( gBattleStruct->monToSwitchIntoId +
					gActiveBattler ) );
		}
		else
		...
		PREPARE_SPECIES_BUFFER( gBattleTextBuff1,
			gBattleMons[gBattlerAttacker].species )
		PREPARE_MON_NICK_BUFFER( gBattleTextBuff2,
			gActiveBattler,
			gBattleBufferB[gActiveBattler][1] )
...
static void atk52_switchineffects( void )
{
		...
		gBattleMons[gActiveBattler].ability != ABILITY_LEVITATE )
	{
		...
		spikesDmg =
			( 5 -
				gSideTimers[GetBattlerSide( gActiveBattler )]
					.spikesAmount ) *
			2;
		gBattleMoveDamage =
			gBattleMons[gActiveBattler].maxHP / ( spikesDmg );
		if( gBattleMoveDamage == 0 )
			gBattleMoveDamage = 1;
	...
	{
		// There is a hack here to ensure the truant counter will be 0
		// when the battler's next turn starts. The truant counter is
		// not updated in the case where a mon switches in after a lost
		// judgement in the battle arena.
		...
		if( !AbilityBattleEffects( ABILITYEFFECT_ON_SWITCHIN,
			    ...
			    0,
			    0 ) &&
			!ItemBattleEffects( ITEMEFFECT_ON_SWITCH_IN,
				gActiveBattler,
				FALSE ) )
		{
			gSideStatuses[GetBattlerSide( gActiveBattler )] &=
...
static void atk55_fanfare( void )
{
	...
	BtlController_EmitPlayFanfareOrBGM(
		0, T2_READ_16( gBattlescriptCurrInstr + 1 ), FALSE );
	MarkBattlerForControllerExec( gActiveBattler );
...
static void atk59_handlelearnnewmove( void )
{
	...
	u16 ret = MonTryLearningNewMove(
		&gPlayerParty[gBattleStruct->expGetterMonId],
		gBattlescriptCurrInstr[9] );
	while( ret == 0xFFFE )
		ret = MonTryLearningNewMove(
			&gPlayerParty[gBattleStruct->expGetterMonId], 0 );
	...
	else
	{
		...
		{
			GiveMoveToBattleMon(
				&gBattleMons[gActiveBattler], ret );
		}
		if( gBattleTypeFlags & BATTLE_TYPE_DOUBLE )
		{
			gActiveBattler = GetBattlerAtPosition(
...
static void atk5A_yesnoboxlearnmove( void )
{
	...
	case 1:
		...
		if( gMain.newKeys & A_BUTTON )
		{
			...
			{
				HandleBattleWindow(
					0x18, 0x8, 0x1D, 0xD, WINDOW_CLEAR );
				BeginNormalPaletteFade(
					0xFFFFFFFF, 0, 0, 0x10, RGB_BLACK );
				gBattleScripting.learnMoveState++;
			}
			else
			{
	...
	case 4:
		if( !gPaletteFade.active && gMain.callback2 == BattleMainCB2 )
		{
			...
			else
			{
				...
				{
					gBattlescriptCurrInstr = T1_READ_PTR(
						gBattlescriptCurrInstr + 1 );

					PREPARE_MOVE_BUFFER(
						gBattleTextBuff2, moveId )
...
static u32 GetTrainerMoneyToGive( u16 trainerId )
{
	...
	{
		moneyReward = 20 *
			gBattleResources->secretBase->party.levels[0] *
	...
	else
	{
		...
		case 0: {
			...
			lastMonLevel =
				party[gTrainers[trainerId].partySize - 1].lvl;
		}
		break;
		...
		{
			if( gTrainerMoneyTable[i].classId ==
				gTrainers[trainerId].trainerClass )
				break;
		}
...
static void atk5D_getmoneyreward( void )
{
	...
	if( gBattleTypeFlags & BATTLE_TYPE_TWO_OPPONENTS )
		moneyReward +=
			GetTrainerMoneyToGive( gTrainerBattleOpponent_B );
	...
	case 1:
		if( gBattleControllerExecFlags == 0 )
		{
			...
			struct BattlePokemon* bufferPoke =
				(struct BattlePokemon*)&gBattleBufferB
					[gActiveBattler][4];
			for( i = 0; i < MAX_MON_MOVES; i++ )
			{
...
static void atk63_jumptocalledmove( void )
{
	...
	gBattlescriptCurrInstr =
		gBattleScriptsForMoveEffects[gBattleMoves[gCurrentMove]
						     .effect];
}
...
static void atk64_statusanimation( void )
{
	if( gBattleControllerExecFlags == 0 )
	{
		gActiveBattler =
			GetBattlerForBattleScript( gBattlescriptCurrInstr[1] );
		if( !( gStatuses3[gActiveBattler] &
			    STATUS3_SEMI_INVULNERABLE ) &&
			gDisableStructs[gActiveBattler].substituteHP == 0 &&
			!( gHitMarker & HITMARKER_NO_ANIMATIONS ) )
		{
			BtlController_EmitStatusAnimation( 0,
				FALSE,
				gBattleMons[gActiveBattler].status1 );
			MarkBattlerForControllerExec( gActiveBattler );
		}
	...
	if( gBattleControllerExecFlags == 0 )
	{
			...
			!( gHitMarker & HITMARKER_NO_ANIMATIONS ) )
		{
			BtlController_EmitStatusAnimation(
				0, gBattlescriptCurrInstr[2], wantedStatus );
			MarkBattlerForControllerExec( gActiveBattler );
		}
		gBattlescriptCurrInstr += 7;
	}
...
static void atk69_adjustsetdamage( void ) // The same as 0x7, except there's no
...
static void atk6C_drawlvlupbox( void )
{
	...
	case 8:
		...
		{
			PlaySE( SE_SELECT );
			HandleBattleWindow( 0x12,
				7,
				0x1D,
				0x13,
				WINDOW_x80 | WINDOW_CLEAR );
			gBattleScripting.atk6C_state++;
		}
		break;
	case 9:
...
static void DrawLevelUpWindow1( void )
{
	...
	GetMonLevelUpWindowStats(
		&gPlayerParty[gBattleStruct->expGetterMonId], currStats );
...
static void PutLevelAndGenderOnLvlUpBox( void )
{
	...
	monGender =
		GetMonGender( &gPlayerParty[gBattleStruct->expGetterMonId] );
	GetMonNickname(
		&gPlayerParty[gBattleStruct->expGetterMonId], gStringVar4 );
	...
	txtPtr = ConvertIntToDecimalStringN(
		txtPtr, monLevel, STR_CONV_MODE_LEFT_ALIGN, 3 );
	var    = ( u32 )(txtPtr)-var;
...
static void PutMonIconOnLvlUpBox( void )
{
	...
	spriteId =
		CreateSprite( &sSpriteTemplate_MonIconOnLvlUpBox, 256, 10, 0 );
...
static void atk70_recordlastability( void )
{
	...
	gBattlescriptCurrInstr += 1; // UB: Should be + 2, one byte for command
				     // and one byte for battlerId argument.
}
...
static void atk73_hpthresholds( void )
{
	...
	if( !( gBattleTypeFlags & BATTLE_TYPE_DOUBLE ) )
	{
		...
		result = gBattleMons[opposingBank].hp * 100 /
			gBattleMons[opposingBank].maxHP;
		if( result == 0 )
			result = 1;
...
static void atk74_hpthresholds2( void )
{
	...
	if( !( gBattleTypeFlags & BATTLE_TYPE_DOUBLE ) )
	{
		gActiveBattler =
			GetBattlerForBattleScript( gBattlescriptCurrInstr[1] );
		opposingBank = gActiveBattler ^ BIT_SIDE;
		hpSwitchout  = *( gBattleStruct->hpOnSwitchout +
                        GetBattlerSide( opposingBank ) );
		result = ( hpSwitchout - gBattleMons[opposingBank].hp ) * 100 /
			hpSwitchout;
...
static void atk75_useitemonopponent( void )
{
	...
	PokemonUseItemEffects(
		&gEnemyParty[gBattlerPartyIndexes[gBattlerAttacker]],
		gLastUsedItem,
		gBattlerPartyIndexes[gBattlerAttacker],
		0,
		1 );
...
static void atk76_various( void )
{
	...
	case VARIOUS_UPDATE_CHOICE_MOVE_ON_LVL_UP:
				...
				gBattleStruct->expGetterMonId )
		{
			...
			choicedMove =
				&gBattleStruct->choicedMove[gActiveBattler];

			for( i = 0; i < MAX_MON_MOVES; i++ )
			{
				if( gBattleMons[gActiveBattler].moves[i] ==
					*choicedMove )
					break;
			}
			if( i == MAX_MON_MOVES )
	...
	case 7:
		if( !( gBattleTypeFlags &
			    ( BATTLE_TYPE_LINK | BATTLE_TYPE_DOUBLE ) ) &&
	...
	case 8:
				...
				STATUS1_SLEEP ) )
		{
			...
			gBattleCommunication[MULTISTRING_CHOOSER] =
				sUnknown_0831C4F8[GetNatureFromPersonality(
					gBattleMons[gActiveBattler]
						.personality )];
		}
		break;
	...
	case VARIOUS_ARENA_OPPONENT_MON_LOST:
		...
		gBattleStruct->arenaLostOpponentMons |=
			gBitTable[gBattlerPartyIndexes[1]];
	...
	case VARIOUS_ARENA_PLAYER_MON_LOST:
		...
		gHitMarker |= HITMARKER_x400000;
		gBattleStruct->arenaLostPlayerMons |=
			gBitTable[gBattlerPartyIndexes[0]];
	...
	case VARIOUS_SET_ALREADY_STATUS_MOVE_ATTEMPT:
		gBattleStruct->alreadyStatusedMoveAttempt |=
			gBitTable[gActiveBattler];
		break;
	case 24:
...
static void atk77_setprotectlike( void ) // protect and endure
{
	...
	if( sProtectSuccessRates[gDisableStructs[gBattlerAttacker]
					 .protectUses] >= Random( ) &&
		notLastTurn )
	{
...
static void atk78_faintifabilitynotdamp( void )
{
	...
	else
	{
		...
		RecordAbilityBattle(
			gBattlerTarget, gBattleMons[gBattlerTarget].ability );
...
static void atk7C_trymirrormove( void )
{
	...
	for( validMovesCount = 0, i = 0; i < gBattlersCount; i++ )
	{
		...
		{
			move = *( i * 2 + gBattlerAttacker * 8 +
				       (u8*)( gBattleStruct
						       ->lastTakenMoveFrom ) +
				       0 ) |
				( *( i * 2 + gBattlerAttacker * 8 +
					  (u8*)( gBattleStruct
							  ->lastTakenMoveFrom ) +
					  1 )
					<< 8 );
	...
	move = *( gBattleStruct->lastTakenMove + gBattlerAttacker * 2 + 0 ) |
		( *( gBattleStruct->lastTakenMove + gBattlerAttacker * 2 + 1 )
			<< 8 );
...
static void atk7E_setreflect( void )
{
	...
	else
	{
		gSideStatuses[GET_BATTLER_SIDE( gBattlerAttacker )] |=
			SIDE_STATUS_REFLECT;
		gSideTimers[GET_BATTLER_SIDE( gBattlerAttacker )]
			.reflectTimer = 5;
		gSideTimers[GET_BATTLER_SIDE( gBattlerAttacker )]
			.reflectBattlerId = gBattlerAttacker;
...
static void atk80_manipulatedamage( void )
{
	...
	case ATK80_DMG_HALF_BY_TWO_NOT_MORE_THAN_HALF_MAX_HP:
		...
		if( gBattleMoveDamage == 0 )
			gBattleMoveDamage = 1;
		if( ( gBattleMons[gBattlerTarget].maxHP / 2 ) <
			gBattleMoveDamage )
			gBattleMoveDamage =
				gBattleMons[gBattlerTarget].maxHP / 2;
		break;
...
static void atk83_nop( void ) { gBattlescriptCurrInstr++; }
...
static void atk85_stockpile( void )
{
	...
	else
	{
		...
		PREPARE_BYTE_NUMBER_BUFFER( gBattleTextBuff1,
			1,
			gDisableStructs[gBattlerAttacker].stockpileCounter )
		...
		if( gBattleCommunication[6] != 1 )
		{
			gBattleMoveDamage =
				CalculateBaseDamage(
					...
					&gBattleMons[gBattlerTarget],
					gCurrentMove,
					gSideStatuses[GET_BATTLER_SIDE(
						gBattlerTarget )],
					0,
					0,
					gBattlerAttacker,
...
static u8 ChangeStatBuffs(
	s8 statValue, u8 statId, u8 flags, const u8* BS_ptr )
{
	bool8 certain            = FALSE;
	...
	{
		if( gSideTimers[GET_BATTLER_SIDE( gActiveBattler )]
				.mistTimer &&
		...
		else if( ( gBattleMons[gActiveBattler].ability ==
					 ABILITY_CLEAR_BODY ||
				 gBattleMons[gActiveBattler].ability ==
					 ABILITY_WHITE_SMOKE ) &&
			!certain && gCurrentMove != MOVE_CURSE )
			...
			!certain && statId == STAT_ACC )
		{
			if( flags == STAT_CHANGE_BS_PTR )
			{
				...
				gLastUsedAbility =
					gBattleMons[gActiveBattler].ability;
				RecordAbilityBattle(
					gActiveBattler, gLastUsedAbility );
			}
			return STAT_CHANGE_DIDNT_WORK;
		}
		else if( gBattleMons[gActiveBattler].ability ==
		...
		else // try to decrease
		{
			...
			if( gBattleMons[gActiveBattler].statStages[statId] ==
				0 )
				gBattleCommunication[MULTISTRING_CHOOSER] = 2;
			else
				gBattleCommunication[MULTISTRING_CHOOSER] =
...
static bool8 TryDoForceSwitchOut( void )
{
	...
	{
		*( gBattleStruct->field_58 + gBattlerTarget ) =
	...
	if( ( gBattleTypeFlags & BATTLE_TYPE_TRAINER ) )
	{
		...
		else if( gBattleTypeFlags & BATTLE_TYPE_TWO_OPPONENTS )
		{
			if( GetBattlerSide( gBattlerTarget ) == B_SIDE_PLAYER )
			{
				...
				monsCount  = 6;
				minNeeded =
					2; // since there are two opponents, it
					   // has to be a double battle
			}
			else
			{
				if( ( gBattlerTarget & BIT_FLANK ) != 0 )
		...
		else
		{
			...
			monsCount       = 6;
			minNeeded       = 1;
			battler2PartyId = gBattlerPartyIndexes
				[gBattlerTarget]; // there is
						  // only one
						  // pokemon out
						  // in single
						  // battles
...
static void atk90_tryconversiontypechange( void ) // randomly changes user's
						  // type to one of its moves'
						  // type
{
	u8 validMoves = 0;
	u8 moveChecked;
	...
	{
		moveType = gBattleMoves[gBattleMons[gBattlerAttacker]
						.moves[moveChecked]]
				   .type;
...
static void atk92_setlightscreen( void )
{
	...
	else
	{
		...
		gSideTimers[GET_BATTLER_SIDE( gBattlerAttacker )]
			.lightscreenTimer = 5;
		gSideTimers[GET_BATTLER_SIDE( gBattlerAttacker )]
			.lightscreenBattlerId = gBattlerAttacker;
...
static void atk93_tryKO( void )
{
		...
		( Random( ) % 100 ) < param )
	{
		RecordItemEffectBattle(
			gBattlerTarget, HOLD_EFFECT_FOCUS_BAND );
		gSpecialStatuses[gBattlerTarget].focusBanded = 1;
	}
...
static void atk97_tryinfatuating( void )
{
	...
	if( GetBattlerSide( gBattlerAttacker ) == B_SIDE_PLAYER )
		monAttacker =
			&gPlayerParty[gBattlerPartyIndexes[gBattlerAttacker]];
	else
		monAttacker =
			&gEnemyParty[gBattlerPartyIndexes[gBattlerAttacker]];
	...
	if( GetBattlerSide( gBattlerTarget ) == B_SIDE_PLAYER )
		monTarget =
			&gPlayerParty[gBattlerPartyIndexes[gBattlerTarget]];
	else
	...
	{
		if( GetGenderFromSpeciesAndPersonality(
			    speciesAttacker, personalityAttacker ) ==
				GetGenderFromSpeciesAndPersonality(
					speciesTarget, personalityTarget ) ||
			gBattleMons[gBattlerTarget].status2 &
				STATUS2_INFATUATION ||
			GetGenderFromSpeciesAndPersonality( speciesAttacker,
				personalityAttacker ) == MON_GENDERLESS ||
			GetGenderFromSpeciesAndPersonality( speciesTarget,
				personalityTarget ) == MON_GENDERLESS )
		{
			gBattlescriptCurrInstr =
				T1_READ_PTR( gBattlescriptCurrInstr + 1 );
		}
...
static void atk98_updatestatusicon( void )
{
	...
	if( gBattlescriptCurrInstr[1] != BS_ATTACKER_WITH_PARTNER )
	{
		gActiveBattler =
			GetBattlerForBattleScript( gBattlescriptCurrInstr[1] );
		BtlController_EmitStatusIconUpdate( 0,
			gBattleMons[gActiveBattler].status1,
			gBattleMons[gActiveBattler].status2 );
		MarkBattlerForControllerExec( gActiveBattler );
		gBattlescriptCurrInstr += 2;
	...
	else
	{
		...
		{
			gActiveBattler = GetBattlerAtPosition(
				GetBattlerPosition( gBattlerAttacker ) ^
				BIT_FLANK );
			if( !( gAbsentBattlerFlags &
				    gBitTable[gActiveBattler] ) )
			{
...
static void atk99_setmist( void )
{
	...
	else
	{
		gSideTimers[GET_BATTLER_SIDE( gBattlerAttacker )].mistTimer =
			5;
		gSideTimers[GET_BATTLER_SIDE( gBattlerAttacker )]
			.mistBattlerId = gBattlerAttacker;
		gSideStatuses[GET_BATTLER_SIDE( gBattlerAttacker )] |=
			SIDE_STATUS_MIST;
		gBattleCommunication[MULTISTRING_CHOOSER] = 0;
	}
...
static void atk9B_transformdataexecution( void )
{
	...
	else
	{
		...
		PREPARE_SPECIES_BUFFER(
			gBattleTextBuff1, gBattleMons[gBattlerTarget].species )
		...
		for( i = 0; i < MAX_MON_MOVES; i++ )
		{
			if( gBattleMoves[gBattleMons[gBattlerAttacker]
						 .moves[i]]
					.pp < 5 )
...
static void atk9C_setsubstitute( void )
{
	...
	{
		gBattleMoveDamage = gBattleMons[gBattlerAttacker].maxHP /
			4; // one bit value will only work for pokemon which
			   // max hp can go to 1020(which is more than possible
			   // in games)
		if( gBattleMoveDamage == 0 )
			gBattleMoveDamage = 1;
		...
		gDisableStructs[gBattlerAttacker].substituteHP =
			gBattleMoveDamage;
		gBattleCommunication[MULTISTRING_CHOOSER] = 0;
		gHitMarker |= HITMARKER_IGNORE_SUBSTITUTE;
	}
	...
	while( 1 )
	{
		...
		if( sMovesForbiddenToCopy[i] == METRONOME_FORBIDDEN_END )
		{
			...
			gBattlescriptCurrInstr = gBattleScriptsForMoveEffects
				[gBattleMoves[gCurrentMove].effect];
			gBattlerTarget = GetMoveTarget( gCurrentMove, 0 );
			return;
		}
	}
...
static void atkA1_counterdamagecalculator( void )
{
	...
	u8 sideTarget   = GetBattlerSide(
                gProtectStructs[gBattlerAttacker].physicalBattlerId );

	if( gProtectStructs[gBattlerAttacker].physicalDmg &&
		sideAttacker != sideTarget &&
		gBattleMons[gProtectStructs[gBattlerAttacker]
				    .physicalBattlerId]
			.hp )
	{
		gBattleMoveDamage =
			gProtectStructs[gBattlerAttacker].physicalDmg * 2;

		if( gSideTimers[sideTarget].followmeTimer &&
			gBattleMons[gSideTimers[sideTarget].followmeTarget]
				.hp )
			gBattlerTarget =
				gSideTimers[sideTarget].followmeTarget;
		else
...
static void atkA2_mirrorcoatdamagecalculator( void ) // a copy of atkA1 with
						     // the physical -> special
						     // field changes
{
	u8 sideAttacker = GetBattlerSide( gBattlerAttacker );
	u8 sideTarget   = GetBattlerSide(
                gProtectStructs[gBattlerAttacker].specialBattlerId );

	if( gProtectStructs[gBattlerAttacker].specialDmg &&
		sideAttacker != sideTarget &&
		gBattleMons[gProtectStructs[gBattlerAttacker].specialBattlerId]
			.hp )
	{
		gBattleMoveDamage =
			gProtectStructs[gBattlerAttacker].specialDmg * 2;
...
static void atkA3_disablelastusedattack( void )
{
	...
	for( i = 0; i < MAX_MON_MOVES; i++ )
	{
		if( gBattleMons[gBattlerTarget].moves[i] ==
			gLastMoves[gBattlerTarget] )
			break;
	}
...
static void atkA5_painsplitdmgcalc( void )
{
	if( !( gBattleMons[gBattlerTarget].status2 & STATUS2_SUBSTITUTE ) )
	{
		s32 hpDiff = ( gBattleMons[gBattlerAttacker].hp +
				     gBattleMons[gBattlerTarget].hp ) /
			2;
...
static void atkA6_settypetorandomresistance( void ) // conversion 2
{
	...
	else
	{
		...
		{
			while( ( ( i = ( Random( ) & 0x7F ) ) >
				sizeof( gTypeEffectiveness ) / 3 ) )
				;

			i *= 3;

			if( TYPE_EFFECT_ATK_TYPE( i ) ==
					gLastHitByType[gBattlerAttacker] &&
				TYPE_EFFECT_MULTIPLIER( i ) <=
					TYPE_MUL_NOT_EFFECTIVE &&
				!IS_BATTLER_OF_TYPE( gBattlerAttacker,
					TYPE_EFFECT_DEF_TYPE( i ) ) )
			{
				SET_BATTLER_TYPE( gBattlerAttacker,
					TYPE_EFFECT_DEF_TYPE( i ) );
				PREPARE_TYPE_BUFFER( gBattleTextBuff1,
					TYPE_EFFECT_DEF_TYPE( i ) );
		...
		gLastPrintedMoves[gBattlerTarget] != MOVE_SKETCH )
	{
		...
		else // sketch worked
		{
			...
			gBattleMons[gBattlerAttacker].moves[gCurrMovePos] =
				gLastPrintedMoves[gBattlerTarget];
			gBattleMons[gBattlerAttacker].pp[gCurrMovePos] =
				gBattleMoves[gLastPrintedMoves[gBattlerTarget]]
					.pp;
			gActiveBattler = gBattlerAttacker;
			...
			movePpData.ppBonuses =
				gBattleMons[gBattlerAttacker].ppBonuses;

			BtlController_EmitSetMonData( 0,
				REQUEST_MOVES_PP_BATTLE,
				0,
				sizeof( struct MovePpInfo ),
				&movePpData );
			MarkBattlerForControllerExec( gActiveBattler );
...
static void atkA9_trychoosesleeptalkmove( void )
{
	...
	for( i = 0; i < MAX_MON_MOVES; i++ )
	{
		if( IsInvalidForSleepTalkOrAssist(
			    gBattleMons[gBattlerAttacker].moves[i] ) ||
			gBattleMons[gBattlerAttacker].moves[i] ==
				MOVE_FOCUS_PUNCH ||
			gBattleMons[gBattlerAttacker].moves[i] ==
	...
	unusableMovesBits = CheckMoveLimitations(
		gBattlerAttacker, unusableMovesBits, ~( MOVE_LIMITATION_PP ) );
	...
	else // at least one move can be chosen
	{
		...
		gCalledMove =
			gBattleMons[gBattlerAttacker].moves[movePosition];
		...
		gLastMoves[gBattlerTarget] != 0xFFFF )
	{
			...
			gBattleMons[gBattlerTarget].pp[i] > 1 )
		{
			...
			ConvertIntToDecimalStringN(
				gBattleTextBuff2, ppToDeduct, 0, 1 );

			PREPARE_BYTE_NUMBER_BUFFER(
				gBattleTextBuff2, 1, ppToDeduct )
...
static void atkAE_healpartystatus( void )
{
	...
	if( gCurrentMove == MOVE_HEAL_BELL )
	{
		...
		if( gBattleTypeFlags & BATTLE_TYPE_DOUBLE &&
			!( gAbsentBattlerFlags & gBitTable[gActiveBattler] ) )
		{
			if( gBattleMons[gActiveBattler].ability !=
				ABILITY_SOUNDPROOF )
			{
				gBattleMons[gActiveBattler].status1 = 0;
				gBattleMons[gActiveBattler].status2 &=
		...
		for( i = 0; i < PARTY_SIZE; i++ )
		{
			...
			if( species != SPECIES_NONE && species != SPECIES_EGG )
			{
				...
				if( gBattlerPartyIndexes[gBattlerAttacker] ==
					i )
					ability = gBattleMons[gBattlerAttacker]
							  .ability;
					...
					!( gAbsentBattlerFlags &
						gBitTable[gActiveBattler] ) )
					ability = gBattleMons[gActiveBattler]
							  .ability;
				else
					ability = GetAbilityBySpecies(
						species, abilityNum );
	...
	else // Aromatherapy
	{
		...
		gActiveBattler = GetBattlerAtPosition(
			GetBattlerPosition( gBattlerAttacker ) ^ BIT_FLANK );
		if( gBattleTypeFlags & BATTLE_TYPE_DOUBLE &&
			!( gAbsentBattlerFlags & gBitTable[gActiveBattler] ) )
		{
			gBattleMons[gActiveBattler].status1 = 0;
	...
	if( toHeal )
	{
		gActiveBattler = gBattlerAttacker;
		BtlController_EmitSetMonData(
			0, REQUEST_STATUS_BATTLE, toHeal, 4, &zero );
		MarkBattlerForControllerExec( gActiveBattler );
	}
...
static void atkB3_rolloutdamagecalculation( void )
{
	...
	else
	{
		...
		if( --gDisableStructs[gBattlerAttacker].rolloutTimer ==
			0 ) // last
			    // hit
		{
			gBattleMons[gBattlerAttacker].status2 &=
		...
		for( i = 1; i <
			( 5 - gDisableStructs[gBattlerAttacker].rolloutTimer );
			i++ )
			gDynamicBasePower *= 2;
...
static void atkB4_jumpifconfusedandstatmaxed( void )
{
	if( gBattleMons[gBattlerTarget].status2 & STATUS2_CONFUSION &&
		gBattleMons[gBattlerTarget]
				.statStages[gBattlescriptCurrInstr[1]] == 0xC )
		gBattlescriptCurrInstr =
			T1_READ_PTR( gBattlescriptCurrInstr + 2 );
	else
		gBattlescriptCurrInstr += 6;
...
static void atkB6_happinesstodamagecalculation( void )
{
	if( gBattleMoves[gCurrentMove].effect == EFFECT_RETURN )
		gDynamicBasePower =
			10 * ( gBattleMons[gBattlerAttacker].friendship ) / 25;
	else // EFFECT_FRUSTRATION
		gDynamicBasePower = 10 *
			( 255 - gBattleMons[gBattlerAttacker].friendship ) /
			25;
...
static void atkB7_presentdamagecalculation( void )
{
	...
	else if( gBattleMons[gBattlerTarget].maxHP ==
		gBattleMons[gBattlerTarget].hp )
...
static void atkB8_setsafeguard( void )
{
	...
	else
	{
		...
		gSideTimers[GET_BATTLER_SIDE( gBattlerAttacker )]
			.safeguardTimer = 5;
		gSideTimers[GET_BATTLER_SIDE( gBattlerAttacker )]
			.safeguardBattlerId               = gBattlerAttacker;
...
static void atkB9_magnitudedamagecalculation( void )
{
		...
		gBattlerTarget++ )
	{
		if( gBattlerTarget == gBattlerAttacker )
			continue;
		if( !( gAbsentBattlerFlags &
			    gBitTable[gBattlerTarget] ) ) // a valid
							  // target
							  // was
							  // found
			break;
...
static void atkBA_jumpifnopursuitswitchdmg( void )
{
	...
	if( gChosenActionByBattler[gBattlerTarget] == B_ACTION_USE_MOVE &&
		gBattlerAttacker ==
			*( gBattleStruct->moveTarget + gBattlerTarget ) &&
		!( gBattleMons[gBattlerTarget].status1 &
...
static void atkBD_copyfoestats( void ) // psych up
{
	...
	gBattlescriptCurrInstr += 5; // Has an unused jump ptr(possibly for a
				     // failed attempt) parameter.
}
...
static void atkBE_rapidspinfree( void )
{
	if( gBattleMons[gBattlerAttacker].status2 & STATUS2_WRAPPED )
	{
		...
		gBattleMons[gBattlerAttacker].status2 &= ~( STATUS2_WRAPPED );
		gBattlerTarget =
			*( gBattleStruct->wrappedBy + gBattlerAttacker );

		gBattleTextBuff1[0] = B_BUFF_PLACEHOLDER_BEGIN;
		gBattleTextBuff1[1] = B_BUFF_MOVE;
		gBattleTextBuff1[2] = *( gBattleStruct->wrappedMove +
			gBattlerAttacker * 2 + 0 );
		gBattleTextBuff1[3] = *( gBattleStruct->wrappedMove +
	...
	else if( gStatuses3[gBattlerAttacker] & STATUS3_LEECHSEED )
	{
		...
		SIDE_STATUS_SPIKES )
	{
		gSideStatuses[GetBattlerSide( gBattlerAttacker )] &=
			~( SIDE_STATUS_SPIKES );
...
static void atkC0_recoverbasedonsunlight( void )
{
		...
		gBattleMons[gBattlerAttacker].maxHP )
	{
		if( gBattleWeather == 0 || !WEATHER_HAS_EFFECT )
			gBattleMoveDamage =
				gBattleMons[gBattlerAttacker].maxHP / 2;
		else if( gBattleWeather & WEATHER_SUN_ANY )
			gBattleMoveDamage =
				20 * gBattleMons[gBattlerAttacker].maxHP / 30;
		else // not sunny weather
			gBattleMoveDamage =
				gBattleMons[gBattlerAttacker].maxHP / 4;
...
static void atkC3_trysetfutureattack( void )
{
	...
	else
	{
		...
		gWishFutureKnock.futureSightMove[gBattlerTarget] =
			gCurrentMove;
		gWishFutureKnock.futureSightAttacker[gBattlerTarget] =
			gBattlerAttacker;
		gWishFutureKnock.futureSightCounter[gBattlerTarget] = 3;
		gWishFutureKnock
			.futureSightDmg[gBattlerTarget] = CalculateBaseDamage(
			&gBattleMons[gBattlerAttacker],
			&gBattleMons[gBattlerTarget],
			gCurrentMove,
			gSideStatuses[GET_BATTLER_SIDE( gBattlerTarget )],
...
static void atkC4_trydobeatup( void )
{
	...
	else
	{
		...
		{
			if( GetMonData( &party[gBattleCommunication[0]],
				    MON_DATA_HP ) &&
				GetMonData( &party[gBattleCommunication[0]],
					MON_DATA_SPECIES2 ) &&
				GetMonData( &party[gBattleCommunication[0]],
					MON_DATA_SPECIES2 ) != SPECIES_EGG &&
				!GetMonData( &party[gBattleCommunication[0]],
					MON_DATA_STATUS ) )
				break;
		}
		if( gBattleCommunication[0] < 6 )
		{
			...
			gBattleMoveDamage *=
				( GetMonData( &party[gBattleCommunication[0]],
					...
					2 );
			gBattleMoveDamage /=
				gBaseStats[gBattleMons[gBattlerTarget].species]
					.baseDefense;
			...
			gBattlescriptCurrInstr =
				T1_READ_PTR( gBattlescriptCurrInstr + 5 );
	}
}
...
static void atkD2_tryswapitems( void ) // trick
{
	...
	else
	{
		...
		// you can't swap items if they were knocked off in regular
		// battles
		if( !( gBattleTypeFlags &
			    ( BATTLE_TYPE_LINK | BATTLE_TYPE_EREADER_TRAINER |
				    BATTLE_TYPE_FRONTIER |
		...
		else if( gBattleMons[gBattlerTarget].ability ==
			ABILITY_STICKY_HOLD )
		{
			gBattlescriptCurrInstr =
				BattleScript_StickyHoldActivates;
			gLastUsedAbility = gBattleMons[gBattlerTarget].ability;
			RecordAbilityBattle(
				gBattlerTarget, gLastUsedAbility );
		}
		// took a while, but all checks passed and items can be safely
		// swapped
		else
		{
			...
			newItemAtk =
				&gBattleStruct->changedItems[gBattlerAttacker];
			...
			*newItemAtk = gBattleMons[gBattlerTarget].item;
			...
			gActiveBattler = gBattlerAttacker;
			BtlController_EmitSetMonData(
				0, REQUEST_HELDITEM_BATTLE, 0, 2, newItemAtk );
			MarkBattlerForControllerExec( gBattlerAttacker );
...
static void atkDB_tryimprison( void )
{
	...
	else
	{
		...
		for( battlerId = 0; battlerId < gBattlersCount; battlerId++ )
		{
			...
			{
				s32 attackerMoveId;
				for( attackerMoveId = 0;
					attackerMoveId < MAX_MON_MOVES;
					attackerMoveId++ )
				{
					s32 i;
					for( i = 0; i < MAX_MON_MOVES; i++ )
					{
						if( gBattleMons[gBattlerAttacker]
									.moves[attackerMoveId] ==
		...
		if( battlerId ==
			gBattlersCount ) // In Generation 3 games, Imprison
					 // fails if the user doesn't share
					 // any moves with any of the foes
			gBattlescriptCurrInstr =
				T1_READ_PTR( gBattlescriptCurrInstr + 1 );
	}
}
...
static void atkDE_assistattackselect( void )
{
	...
	for( monId = 0; monId < 6; monId++ )
	{
		if( monId == gBattlerPartyIndexes[gBattlerAttacker] )
			continue;
		if( GetMonData( &party[monId], MON_DATA_SPECIES2 ) ==
			SPECIES_NONE )
			continue;
		if( GetMonData( &party[monId], MON_DATA_SPECIES2 ) ==
			SPECIES_EGG )
		...
		for( moveId = 0; moveId < MAX_MON_MOVES; moveId++ )
		{
			s32 i    = 0;
			u16 move = GetMonData(
				&party[monId], MON_DATA_MOVE1 + moveId );

			if( IsInvalidForSleepTalkOrAssist( move ) )
				continue;

			for( ; sMovesForbiddenToCopy[i] !=
	...
	{
		gHitMarker &= ~( HITMARKER_ATTACKSTRING_PRINTED );
		gCalledMove    = movesArray[( ( Random( ) & 0xFF ) *
                                                 chooseableMovesNo ) >>
                        8];
		gBattlerTarget = GetMoveTarget( gCalledMove, 0 );
		gBattlescriptCurrInstr += 5;
...
static void atkE5_pickup( void )
{
	...
	else if( InBattlePyramid( ) )
	{
		...
		{
			species = GetMonData(
				&gPlayerParty[i], MON_DATA_SPECIES2 );
			heldItem = GetMonData(
				&gPlayerParty[i], MON_DATA_HELD_ITEM );

			if( GetMonData(
				    &gPlayerParty[i], MON_DATA_ABILITY_NUM ) )
				ability = gBaseStats[species].abilities[1];
			else
				ability = gBaseStats[species].abilities[0];

			if( ability == ABILITY_PICKUP && species != 0 &&
				species != SPECIES_EGG &&
				heldItem == ITEM_NONE &&
				( Random( ) % 10 ) == 0 )
			{
	...
	else
	{
		for( i = 0; i < PARTY_SIZE; i++ )
		{
				...
				( Random( ) % 10 ) == 0 )
			{
				s32 j;
				s32 rand      = Random( ) % 100;
				u8 lvlDivBy10 = ( GetMonData( &gPlayerParty[i],
							  MON_DATA_LEVEL ) -
							1 ) /
					10;
				if( lvlDivBy10 > 9 )
...
static void atkE6_docastformchangeanimation( void )
{
	...
	BtlController_EmitBattleAnimation(
		0, B_ANIM_CASTFORM_CHANGE, gBattleStruct->formToChangeInto );
	MarkBattlerForControllerExec( gActiveBattler );
...
{
	if( !IS_BATTLER_OF_TYPE(
		    gBattlerAttacker, sTerrainToType[gBattleTerrain] ) )
	{
		SET_BATTLER_TYPE(
			gBattlerAttacker, sTerrainToType[gBattleTerrain] );
		PREPARE_TYPE_BUFFER(
			gBattleTextBuff1, sTerrainToType[gBattleTerrain] );
...
enum
{
	MAX_CONST_FAC_BALL = BALL_FAST
};

static const u16 ball_map[MAX_BALL] = {ITEM_POKE_BALL,
	ITEM_GREAT_BALL,
	ITEM_ULTRA_BALL,
	ITEM_VINTAGE_BALL,
	ITEM_SAFARI_BALL,
...
static const u8 ball_rate_tbl[MAX_CONST_FAC_BALL] = {
	10, 15, 20, 7, 15, 15, 10, 10, 10, 10, 10, 10, 10};

typedef u8 ( *PFN_fac_calc )( void );

static u8 fac_fast( void )
{
	return gBaseStats[gBattleMons[gBattlerTarget].species].baseSpeed >= 100
		? 40
		: 10;
}

static u8 fac_heavy( void )
{
	u16 weight =
	...
	return weight >= 4096 ? 80
			      : weight >= 3072
			? 40
			: weight >= 2048 ? 20 : weight >= 1024 ? 10 : 5;
}

static u8 fac_level( void )
{
	return gBattleMons[gActiveBattler].level <=
			gBattleMons[gBattlerTarget].level
		? 10
		: gBattleMons[gActiveBattler].level >=
				gBattleMons[gBattlerTarget].level << 2
			? 80
			: gBattleMons[gActiveBattler].level >=
					gBattleMons[gBattlerTarget].level << 1
				? 40
				: 20;
}

static u8 fac_lure( void ) { return ( battle_bits & 1 ) ? 40 : 10; }

static u8 fac_love( void )
{
	const int self_gend =
		get_gender( gBattleMons[gActiveBattler].personality,
			gBattleMons[gActiveBattler].species );
	const int foe_gend =
		get_gender( gBattleMons[gBattlerTarget].personality,
			gBattleMons[gBattlerTarget].species );

	if( self_gend != GENDER_NONE && foe_gend != GENDER_NONE &&
		self_gend != foe_gend )
	{
		return 40;
	}
...
static u8 fac_moon( void )
{
	const u16 sp = gBattleMons[gBattlerTarget].species;

	return ( sp == SPECIES_NIDORINA || sp == SPECIES_NIDORINO ||
		       sp == SPECIES_CLEFAIRY || sp == SPECIES_JIGGLYPUFF ||
		       sp == SPECIES_UMBREON || sp == SPECIES_SKITTY )
		? 40
		: 10;
}

static u8 fac_net( void )
{
	return ( IS_BATTLER_OF_TYPE( gBattlerTarget, TYPE_WATER ) ||
		       IS_BATTLER_OF_TYPE( gBattlerTarget, TYPE_BUG ) )
		? 40
		: 10;
}

static u8 fac_dive( void )
{
	return GetCurrentMapType( ) == MAP_TYPE_UNDERWATER ? 40 : 10;
}

static u8 fac_nest( void )
{
	const u8 fac = 40 - gBattleMons[gBattlerTarget].level;

	return fac < 10 ? 10 : fac;
}

static u8 fac_repeat( void )
{
	return GetSetPokedexFlag(
		       SpeciesToNationalPokedexNum(
			       gBattleMons[gBattlerTarget].species ),
		       FLAG_GET_CAUGHT )
		? 40
...
static u8 fac_timer( void )
{
	const u8 count = gBattleResults.battleTurnCounter;

	return count <= 245 ? count + 10 : 255;
}

static u8 fac_dusk( void ) { return 10; }

static u8 fac_quick( void )
{
	return gBattleResults.battleTurnCounter == 0 ? 40 : 10;
}

static u8 fac_surf( void )
{
	s16 x, y;
	int mb;
	...
	return ( TILE_IS_SURF( mb ) &&
		       !MetatileBehavior_IsNonOceanSurfableWater( mb ) )
		? 40
		: 10;
}

static u8 fac_lake( void )
{
	s16 x, y;

	PlayerGetDestCoords( &x, &y );

	return MetatileBehavior_IsNonOceanSurfableWater(
		       MapGridGetMetatileBehaviorAt( x, y ) )
		? 40
		: 10;
}

static u8 fac_gs( void ) { return 10; }

static u8 fac_dark( void ) { return 10; }

static const PFN_fac_calc fac_calc[MAX_BALL - MAX_CONST_FAC_BALL] = {fac_fast,
	fac_heavy,
	fac_level,
	fac_lure,
	fac_love,
...
static u8 get_fac( void )
{
	unsigned i;
	unsigned item = MAX_BALL;

	for( i = 0, item = ITEM_NONE; i < MAX_BALL; ++i )
	{
		if( gLastUsedItem == ball_map[i] )
		{
			item = i;
	...
	/* default to Poké Ball factor */
	item = item == MAX_BALL ? BALL_POKE : item;

	/* return a constant factor or call a function to calculate it */
	return item < MAX_CONST_FAC_BALL
		? ball_rate_tbl[item]
		: fac_calc[item - MAX_CONST_FAC_BALL]( );
}

static u32 get_odds( u8 rate, u8 fac )
{
	u32 ret = ( rate * fac / 10 ) *
	...
	{
		u32 status = gBattleMons[gBattlerTarget].status1;

		ret = ( status & ( STATUS1_SLEEP | STATUS1_FREEZE ) )
			? ret * 2
			: ( status &
				  ( STATUS1_POISON | STATUS1_BURN |
					  STATUS1_PARALYSIS |
					  STATUS1_TOXIC_POISON ) )
				? ( ret * 15 ) / 10
				: ret;
	...
	{
		gBattleResults
			.catchAttempts[gLastUsedItem - ITEM_ULTRA_BALL]++;
	}

	return ret;
}

static u8 get_rate( void )
{
	return gLastUsedItem == ITEM_SAFARI_BALL
		? gBattleStruct->safariCatchFactor * 1275 / 100
		: gBaseStats[gBattleMons[gBattlerTarget].species].catchRate;
}
...
static void atkEF_handleballthrow( void )
{
	...
	/* block balls in trainer battles */
	if( gBattleTypeFlags & BATTLE_TYPE_TRAINER )
	{
	...
	/* auto-caught for wally tutorial */
	...
	{
		const u32 odds = get_odds( get_rate( ), get_fac( ) );
		u8 shakes;

		if( odds < 255 && gLastUsedItem != ITEM_MASTER_BALL )
		{
			const u32 nuodds =
			...
			for( shakes = 0; shakes < 4 && Random( ) < nuodds;
				shakes++ )
				;
		}
		else
		...
		if( shakes == BALL_3_SHAKES_SUCCESS )
		{
			...
			gBattleCommunication[MULTISTRING_CHOOSER] =
				CalculatePlayerPartyCount( ) == 6 ? 0 : 1;
...
static void atkEF_handleballthrow( void )
{
	...
	else
	{
		...
		if( gLastUsedItem != ITEM_SAFARI_BALL )
		{
			...
			{
				if( gBattleResults.catchAttempts[gLastUsedItem - ITEM_ULTRA_BALL] <
				   0xFF )
					gBattleResults.catchAttempts[gLastUsedItem - ITEM_ULTRA_BALL]++;
			}
		}
		...
		else // mon may be caught, calculate shakes
		{
			...
			if( gLastUsedItem == ITEM_MASTER_BALL )
				shakes = BALL_3_SHAKES_SUCCESS; // why calculate the shakes
				                                // before that check?

			BtlController_EmitBallThrowAnim( 0, shakes );
			MarkBattlerForControllerExec( gActiveBattler );

			if( shakes == BALL_3_SHAKES_SUCCESS ) // mon caught, copy of the
			                                      // code above
			{
				gBattlescriptCurrInstr = BattleScript_SuccessBallThrow;
				SetMonData( &gEnemyParty[gBattlerPartyIndexes[gBattlerTarget]],
...
static void atkF0_givecaughtmon( void )
{
			    ...
			    BIT_SIDE]] ) != MON_GIVEN_TO_PARTY )
	{
		if( !sub_813B21C( ) )
		{
			...
			StringCopy( gStringVar1,
				GetBoxNamePtr(
					VarGet( VAR_STORAGE_UNKNOWN ) ) );
			GetMonData(
				&gEnemyParty[gBattlerPartyIndexes
						[gBattlerAttacker ^ BIT_SIDE]],
				MON_DATA_NICKNAME,
				gStringVar2 );
		}
		else
		{
			...
			StringCopy( gStringVar3,
				GetBoxNamePtr( get_unknown_box_id( ) ) );
			gBattleCommunication[MULTISTRING_CHOOSER] = 2;
		}
	...
	gBattleResults.caughtMonSpecies = GetMonData(
		...
		MON_DATA_SPECIES,
		NULL );
	GetMonData( &gEnemyParty[gBattlerPartyIndexes[gBattlerAttacker ^
			    BIT_SIDE]],
		MON_DATA_NICKNAME,
...
static void atkF1_trysetcaughtmondexflags( void )
{
	...
	u32 personality =
		GetMonData( &gEnemyParty[0], MON_DATA_PERSONALITY, NULL );

	if( GetSetPokedexFlag(
		    SpeciesToNationalPokedexNum( species ), FLAG_GET_CAUGHT ) )
	{
		gBattlescriptCurrInstr =
			T1_READ_PTR( gBattlescriptCurrInstr + 1 );
	}
		...
		for( destX = xStart; destX <= xEnd; destX++ )
		{
			...
			if( flags & WINDOW_x80 )
				CopyToBgTilemapBufferRect_ChangePalette(
					1, &var, destX, destY, 1, 1, 0x11 );
			else
				CopyToBgTilemapBufferRect_ChangePalette(
					0, &var, destX, destY, 1, 1, 0x11 );
		}
	}
}
...
void BattleCreateYesNoCursorAt( u8 cursorPosition )
{
	...
	CopyToBgTilemapBufferRect_ChangePalette(
		0, src, 0x19, 9 + ( 2 * cursorPosition ), 1, 2, 0x11 );
	...
	case 2:
		if( !gPaletteFade.active )
		{
			...
			DoNamingScreen( NAMING_SCREEN_CAUGHT_MON,
				gBattleStruct->caughtMonNick,
				GetMonData( &gEnemyParty[gBattlerPartyIndexes
							    [gBattlerAttacker ^
								    BIT_SIDE]],
					MON_DATA_SPECIES ),
				GetMonGender( &gEnemyParty[gBattlerPartyIndexes
						[gBattlerAttacker ^
							BIT_SIDE]] ),
				GetMonData( &gEnemyParty[gBattlerPartyIndexes
							    [gBattlerAttacker ^
