...
const u8 DoorAnimPalettes_849700C[] = {11, 11, 11, 11, 11, 11, 11, 11}; // door
									// 06
const u8 DoorAnimPalettes_8497014[] = {10, 10, 10, 10, 10, 10, 10, 10}; // door
									// 07
...
const u8 DoorAnimPalettes_849712C[] = {1, 1, 1, 1, 1, 1, 1, 1}; // doors 42,
								// 43, and 44
...
const u8 DoorAnimPalettes_849714C[] = {1, 1, 1, 1, 1, 1, 1, 1}; // doors 47 and
								// 48
...
static void CopyDoorTilesToVram(
	const struct DoorGraphics* gfx, const struct DoorAnimFrame* frame )
{
	if( gfx->size == 2 )
		CpuFastSet( gfx->tiles + frame->offset,
			(void*)( VRAM + 0x7E00 ),
			0x80 );
	else
		CpuFastSet( gfx->tiles + frame->offset,
...
static void DrawCurrentDoorAnimFrame(
	const struct DoorGraphics* gfx, u32 x, u32 y, const u8* pal )
{
	u16 arr[24];
...
static bool32 sub_808A5F0(
	struct DoorGraphics* gfx, struct DoorAnimFrame* frames, s16* taskData )
{
	if( taskData[TD_COUNTER] == 0 )
		DrawDoor( gfx,
			&frames[taskData[TD_FRAME]],
			taskData[TD_X],
			taskData[TD_Y] );
...
static void Task_AnimateDoor( u8 taskId )
{
	...
	struct DoorAnimFrame* frames =
		(struct DoorAnimFrame*)( taskData[TD_FRAMELIST] << 16 |
			taskData[TD_FRAMELIST + 1] );
	struct DoorGraphics* gfx =
		(struct DoorGraphics*)( taskData[TD_GFX] << 16 |
			taskData[TD_GFX + 1] );
...
static const struct DoorGraphics* GetDoorGraphics(
	const struct DoorGraphics* gfx, u16 metatileNum )
{
...
static void DrawOpenedDoor( const struct DoorGraphics* gfx, u32 x, u32 y )
{
	...
	if( gfx != NULL )
		DrawDoor( gfx,
			GetLastDoorFrame(
				gDoorOpenAnimFrames, gDoorOpenAnimFrames ),
			x,
			y );
}

static s8 StartDoorOpenAnimation(
	const struct DoorGraphics* gfx, u32 x, u32 y )
{
	gfx = GetDoorGraphics( gfx, MapGridGetMetatileIdAt( x, y ) );
	if( gfx == NULL )
	{
	...
	else
	{
		if( gfx->size == 2 )
			return StartDoorAnimationTask(
				gfx, gBigDoorOpenAnimFrames, x, y );
		else
			return StartDoorAnimationTask(
				gfx, gDoorOpenAnimFrames, x, y );
	}
}

static s8 StartDoorCloseAnimation(
	const struct DoorGraphics* gfx, u32 x, u32 y )
{
	gfx = GetDoorGraphics( gfx, MapGridGetMetatileIdAt( x, y ) );
	if( gfx == NULL )
		...
		return StartDoorAnimationTask(
			gfx, gDoorCloseAnimFrames, x, y );
}

static s8 cur_mapdata_get_door_x2_at(
	const struct DoorGraphics* gfx, u32 x, u32 y )
{
	gfx = GetDoorGraphics( gfx, MapGridGetMetatileIdAt( x, y ) );
	if( gfx == NULL )
