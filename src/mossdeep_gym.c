...
void InitMossdeepGymTiles( bool8 arg0 )
{
	if( gUnknown_0203CE50 == NULL )
		gUnknown_0203CE50 =
			AllocZeroed( sizeof( *gUnknown_0203CE50 ) );
...
u16 MossdeepGym_MoveEvents( u8 arg0 )
{
	...
	for( i = 0; i < EVENT_OBJECT_TEMPLATES_COUNT; i++ )
	{
		...
		if( r5 < 4 )
		{
			...
			events[i].x += x;
			events[i].y += y;
			if( GetEventObjectIdByLocalIdAndMap( events[i].localId,
...
void MossdeepGym_TurnEvents( void )
{
	...
	for( i = 0; i < gUnknown_0203CE50->count; i++ )
	{
		...
		s8 r0;
		u8 eventObjectId;
		s16 x = events[gUnknown_0203CE50->objects[i].eventTemplateId]
				.x +
			7;
		s16 y = events[gUnknown_0203CE50->objects[i].eventTemplateId]
				.y +
			7;
		u16 metatile = MapGridGetMetatileIdAt( x, y );
		...
		eventObjectId = GetEventObjectIdByLocalIdAndMap(
			events[gUnknown_0203CE50->objects[i].eventTemplateId]
				.localId,
			gSaveBlock1Ptr->location.mapNum,
			gSaveBlock1Ptr->location.mapGroup );
		if( eventObjectId != EVENT_OBJECTS_COUNT )
		{
			const u8* movementScript;
			u8 direction =
				gEventObjects[eventObjectId].facingDirection;
			if( r6 == 0 )
			{
				switch( direction )
				{
				case DIR_EAST:
					...
					events[gUnknown_0203CE50->objects[i]
							.eventTemplateId]
						.movementType =
...
static void AddEventObject( u8 eventTemplateId, u8 arg1 )
{
	gUnknown_0203CE50->objects[gUnknown_0203CE50->count].eventTemplateId =
		eventTemplateId;
...
static void sub_81A8D94( u8 eventTemplateId, u8 arg1 )
{
	...
	if( r6 == 0 )
	{
		...
		case MOVEMENT_TYPE_FACE_RIGHT:
			events[eventTemplateId].movementType =
				MOVEMENT_TYPE_FACE_UP;
			break;
