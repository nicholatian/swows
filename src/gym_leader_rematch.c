...
static void UpdateGymLeaderRematchFromArray(
	const u16* data, size_t size, u32 maxRematch );
	...
	const u16* data, size_t size, u32 maxRematch )
{
	...
	if( whichLeader != 0 && lowestRematchIndex <= maxRematch )
	{
		...
		if( whichLeader != 0 )
		{
			...
			{
				if( !gSaveBlock1Ptr
						->trainerRematches[data[i]] )
				{
					rematchIndex =
						GetRematchIndex( data[i] );
					if( rematchIndex ==
						lowestRematchIndex )
					{
						if( whichLeader == 0 )
						{
							gSaveBlock1Ptr->trainerRematches
								[data[i]] =
								lowestRematchIndex;
							break;
						}
...
static s32 GetRematchIndex( u32 trainerIdx )
{
	...
	for( i = 0; i < 5; i++ )
	{
		if( !HasTrainerBeenFought(
			    gRematchTable[trainerIdx].trainerIds[i] ) )
		{
			return i;
		}
	}
