...
static void BuyMenuPrint(
	u8 windowId, const u8* text, u8 x, u8 y, s8 speed, u8 colorSet );
...
static void BuyMenuDrawMapMetatileLayer(
	u16* dest, s16 offset1, s16 offset2, const u16* src );
...
static void BuyMenuPrintItemDescriptionAndShowItemIcon(
	int item, bool8 onInit, struct ListMenu* list );
...
static u8 CreateShopMenu( u8 martType )
{
	...
	else
	{
		...
		winTemplate.width =
			GetMaxWidthInMenuTable( sShopMenuActions_BuyQuit,
	...
	PrintMenuTable(
		gMartInfo.windowId, numMenuItems, gMartInfo.menuActions );
	InitMenuInUpperLeftCornerPlaySoundWhenAPressed(
		gMartInfo.windowId, numMenuItems, 0 );
...
static void BuyMenuBuildListMenuTemplate( void )
{
	...
	gUnknown_02039F74 = Alloc(
		( gMartInfo.itemCount + 1 ) * sizeof( *gUnknown_02039F74 ) );
	gUnknown_02039F78 = Alloc(
		( gMartInfo.itemCount + 1 ) * sizeof( *gUnknown_02039F78 ) );
	for( i = 0; i < gMartInfo.itemCount; i++ )
		BuyMenuSetListEntry( &gUnknown_02039F74[i],
			gMartInfo.itemList[i],
			gUnknown_02039F78[i] );
...
static void BuyMenuSetListEntry(
	struct ListMenuItem* menuItem, u16 item, u8* name )
{
...
{
	u8 y = ListMenuGetYCoordForPrintingArrowCursor(
		scrollIndicatorsTaskId );
...
static void BuyMenuAddItemIcon( u16 item, u8 iconSlot )
{
	...
	if( gMartInfo.martType == MART_TYPE_NORMAL || item == 0xFFFF )
	{
		spriteId = AddItemIconSprite(
			iconSlot + 2110, iconSlot + 2110, item );
	...
	else
	{
		spriteId = AddDecorationIconObject(
			item, 20, 84, 1, iconSlot + 2110, iconSlot + 2110 );
		if( spriteId != MAX_SPRITES )
			*spriteIdPtr = spriteId;
	}
}
...
static void BuyMenuDecompressBgGraphics( void )
{
	decompress_and_copy_tile_data_to_vram(
		1, gBuyMenuFrame_Gfx, 0x3A0, 0x3E3, 0 );
	LZDecompressWram(
		gBuyMenuFrame_Tilemap, gShopDataPtr->tilemapBuffers[0] );
	...
	u8 windowId, const u8* text, u8 x, u8 y, s8 speed, u8 colorSet )
{
	AddTextPrinterParameterized4( windowId,
		...
		0,
		0,
		sShopBuyMenuTextColors[colorSet],
		speed,
		text );
}

static void BuyMenuDisplayMessage(
	u8 taskId, const u8* text, TaskFunc callback )
{
...
static void BuyMenuDrawGraphics( void )
{
	...
	PrintMoneyAmountInMoneyBoxWithBorder(
		0, 1, 13, GetMoney( &gSaveBlock1Ptr->money ) );
	schedule_bg_copy_tilemap_to_vram( 0 );
	...
	for( j = 0; j < 10; j++ )
	{
		for( i = 0; i < 15; i++ )
		{
			...
			else
			{
				BuyMenuDrawMapMetatile( i,
					j,
					(u16*)mapLayout->secondaryTileset
							->metatiles +
						( ( metatile -
...
static void BuyMenuDrawMapMetatile(
	s16 x, s16 y, const u16* src, u8 metatileLayerType )
{
	u16 offset1 = x * 2;
	u16 offset2 = y * 64;
	...
	{
	case 0:
		BuyMenuDrawMapMetatileLayer( gShopDataPtr->tilemapBuffers[3],
			offset1,
			offset2,
			src );
		BuyMenuDrawMapMetatileLayer( gShopDataPtr->tilemapBuffers[1],
			offset1,
			offset2,
			src + 4 );
		break;
	case 1:
		BuyMenuDrawMapMetatileLayer( gShopDataPtr->tilemapBuffers[2],
			offset1,
			offset2,
			src );
		BuyMenuDrawMapMetatileLayer( gShopDataPtr->tilemapBuffers[3],
...
static void BuyMenuCollectEventObjectData( void )
{
	...
	for( y = 0; y < 5; y++ )
	{
		for( x = 0; x < 7; x++ )
		{
			u8 eventObjId = GetEventObjectIdByXY(
				facingX - 4 + x, facingY - 2 + y );

			if( eventObjId != 16 )
			{
				gShopDataPtr
					->viewportObjects[r8][EVENT_OBJ_ID] =
					eventObjId;
				...
				switch( gEventObjects[eventObjId]
						.facingDirection )
				{
				case DIR_SOUTH:
					gShopDataPtr
...
static void BuyMenuDrawEventObjects( void )
{
	...
	for( i = 0; i < 16; i++ ) // max objects?
	{
		...
		graphicsInfo = GetEventObjectGraphicsInfo(
			gEventObjects[gShopDataPtr->viewportObjects
					      [i][EVENT_OBJ_ID]]
				.graphicsId );
		...
		spriteId = AddPseudoEventObject(
			gEventObjects[gShopDataPtr->viewportObjects
				...
				.graphicsId,
			SpriteCallbackDummy,
			(u16)gShopDataPtr->viewportObjects[i][X_COORD] * 16 +
				8,
			(u16)gShopDataPtr->viewportObjects[i][Y_COORD] * 16 +
				48 - graphicsInfo->height / 2,
			2 );

		if( BuyMenuCheckIfEventObjectOverlapsMenuBg(
			    gShopDataPtr->viewportObjects[i] ) == TRUE )
		{
		...
		StartSpriteAnim( &gSprites[spriteId],
			gShopDataPtr->viewportObjects[i][ANIM_NUM] );
	}
}
...
static bool8 BuyMenuCheckIfEventObjectOverlapsMenuBg( s16* object )
{
	if( !BuyMenuCheckForOverlapWithMenuBg(
		    object[X_COORD], object[Y_COORD] + 2 ) &&
...
static void Task_BuyMenu( u8 taskId )
{
	...
	if( !gPaletteFade.active )
	{
		...
		ListMenuGetScrollAndRow( tListTaskId,
			&gShopDataPtr->scrollOffset,
			&gShopDataPtr->selectedRow );
		...
		default:
			...
			if( !IsEnoughMoney( &gSaveBlock1Ptr->money,
				    gShopDataPtr->totalCost ) )
			{
...
static void Task_BuyHowManyDialogueInit( u8 taskId )
{
	...
	ConvertIntToDecimalStringN(
		gStringVar1, quantityInBag, STR_CONV_MODE_RIGHT_ALIGN, 4 );
	...
	maxQuantity =
		GetMoney( &gSaveBlock1Ptr->money ) / gShopDataPtr->totalCost;
...
static void Task_BuyHowManyDialogueHandleInput( u8 taskId )
{
	...
	if( AdjustQuantityAccordingToDPadInput(
		    &tItemCount, gShopDataPtr->maxQuantity ) == TRUE )
	{
		gShopDataPtr->totalCost = ( ItemId_GetPrice( tItemId ) >>
						  GetPriceReduction( 1 ) ) *
			tItemCount;
		BuyMenuPrintItemQuantityAndPrice( taskId );
	}
...
static void RecordItemPurchase( u8 taskId )
{
	...
	for( i = 0; i < 3; i++ )
	{
		...
		{
			if( gMartPurchaseHistory[i].quantity + tItemCount >
				255 )
			{
	...
	if( gMartPurchaseHistoryId < 3 )
	{
		...
		gMartPurchaseHistory[gMartPurchaseHistoryId].quantity =
			tItemCount;
