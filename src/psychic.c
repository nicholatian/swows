...
void sub_810F1EC( struct Sprite* sprite )
{
	...
	sprite->data[0] =
		256 + IndexOfSpritePaletteTag( gBattleAnimArgs[2] ) * 16;
...
static void sub_810F400( struct Sprite* sprite )
{
	...
	if( ++sprite->data[1] == 2 )
	{
		...
		for( i = 8; i > 0; i-- )
			gPlttBufferFaded[startOffset + i] =
				gPlttBufferFaded[startOffset + i - 1];
...
static void sub_810F46C( struct Sprite* sprite )
{
	...
	if( --sprite->data[3] == -1 )
	{
		...
		{
			u8 battlerCopy;
			u8 battler = battlerCopy = GetBattlerAtPosition(
...
{
	s16 x = GetBattlerSpriteCoordAttr(
			gBattleAnimAttacker, BATTLER_COORD_ATTR_WIDTH ) /
		2;
	s16 y = GetBattlerSpriteCoordAttr(
...
void sub_810F83C( u8 taskId )
{
	...
	task->data[2]     = 0;
	task->data[3] =
		GetBattlerSide( gBattleAnimAttacker ) != B_SIDE_PLAYER ? 4 : 8;
...
void sub_810F940( u8 taskId )
{
	...
	task->data[3]  = 16;
	task->data[4]  = 0;
	task->data[13] = GetBattlerSpriteCoord(
		gBattleAnimAttacker, BATTLER_COORD_X_2 );
	task->data[14] = GetBattlerSpriteCoord(
		gBattleAnimAttacker, BATTLER_COORD_Y_PIC_OFFSET );

	var0 = GetBattlerSpriteCoordAttr(
		       gBattleAnimAttacker, BATTLER_COORD_ATTR_WIDTH ) /
		3;
	var1 = GetBattlerSpriteCoordAttr(
		       gBattleAnimAttacker, BATTLER_COORD_ATTR_HEIGHT ) /
		3;
...
static void sub_810F9D4( u8 taskId )
{
	...
	case 0:
		...
		{
			task->data[1] = 0;
			spriteId      = CreateSprite( &gUnknown_08596794,
                                task->data[13],
                                task->data[14],
                                0 );
...
void sub_810FBF0( u8 taskId )
{
	...
	if( IsContest( ) )
	{
		if( gBattleAnimArgs[0] == 1 )
		{
			task->data[10] = -10;
			task->data[11] =
				GetBattlerSpriteCoordAttr( gBattleAnimTarget,
...
static void sub_810FD3C( u8 taskId )
{
	...
	case 0:
		...
		{
			task->data[1] = 0;
			spriteId      = CreateSprite( &gUnknown_08596864,
                                task->data[11],
                                task->data[12],
                                0 );
			if( spriteId != 64 )
			{
				...
				StartSpriteAffineAnim( &gSprites[spriteId],
					task->data[2] & 3 );
			}
...
static void sub_8110134( u8 taskId )
{
	...
	case 0:
		...
		SetSpriteRotScale(
			task->data[15], task->data[2], task->data[2], 0 );
		SetBattlerSpriteYOffsetFromOtherYScale(
			task->data[15], task->data[13] );
		if( task->data[1] == 48 )
			task->data[0]++;
		break;
	case 1:
...
void sub_8110240( struct Sprite* sprite )
{
	...
	case 0:
		...
		SetGpuReg( REG_OFFSET_BLDALPHA,
			BLDALPHA_BLEND(
				sprite->data[1], 16 - sprite->data[1] ) );
		sprite->data[0]++;
		break;
	case 1:
		if( sprite->affineAnimEnded )
		{
			PlaySE12WithPanning(
				SE_W100, BattleAnimAdjustPanning( -64 ) );
