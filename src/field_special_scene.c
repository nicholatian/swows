...
void Task_HandleTruckSequence( u8 taskId )
{
	...
	{
		/*
		Each case has a timer which is handled with data[1],
		incrementing until it reaches the if function's condition,
		which sets the next task up.
		*/
	case 0:
		data[1]++;
	...
	case 2:
		...
		break;
	case 3:
		if( !gTasks[data[3]].isActive ) // is Truck2 no longer active
						// (is Truck3 active?)
		{
...
void Task_HandlePorthole( u8 taskId )
{
	...
	case INIT_PORTHOLE: // finish fading before making porthole finish.
		...
		{
			data[1] = 0;
			data[0] = EXECUTE_MOVEMENT; // execute movement before
						    // checking if should be
						    // exited. strange?
		}
		break;
	case IDLE_CHECK: // idle and move.
		...
		if( !ScriptMovement_IsObjectMovementFinished(
			    0xFF, location->mapNum, location->mapGroup ) )
			return;
...
{
	u8 spriteId = AddPseudoEventObject(
		EVENT_OBJ_GFX_SS_TIDAL, SpriteCallbackDummy, 112, 80, 0 );
	...
	{
		StartSpriteAnim(
			&gSprites[spriteId], GetFaceDirectionAnimNum( 4 ) );
	}
	else
	{
		StartSpriteAnim(
			&gSprites[spriteId], GetFaceDirectionAnimNum( 3 ) );
	}
}
