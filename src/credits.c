...
enum
{
	...
	TDA_11 = 11, // Gets set depending on whether the bike or the grass
		     // scene should be shown
	TDA_12        = 12,
	TDA_13        = 13,
	TDA_14        = 14,
	TDA_TASK_B_ID = 15,
...
static const u8 gCreditsText_PkmnEmeraldVersion[] =
...
static const u8 gCreditsText_BattleSystemPgrms[] =
	_( "Battle System Programmers" );
static const u8 gCreditsText_FieldSystemPgrms[] =
	_( "Field System Programmer" );
static const u8 gCreditsText_Programmers[] = _( "Programmers" );
static const u8 gCreditsText_MainGraphicDesigner[] =
	_( "Main Graphic Designer" );
...
static const u8 gCreditsText_SoundEffectsAndPkmnVoices[] =
...
static const u8 gCreditsText_ParametricDesigners[] =
	_( "Parametric Designers" );
static const u8 gCreditsText_PokedexText[] = _( "POKéDEX Text" );
static const u8 gCreditsText_EnvAndToolPgrms[] =
...
static const u8 gCreditsText_InfoSupervisors[] =
	_( "Information Supervisors" );
...
static const u8 gCreditsText_BattleFrontierData[] =
	_( "Battle Frontier Data" );
...
static const u8 gCreditsText_TeruyukiShimoyamada[] =
	_( "Teruyuki Shimoyamada" );
...
static const u8 gCreditsText_JapanBrailleLibrary[] =
	_( "Japan Braille Library" );
...
static const u8 gCreditsText_PackageAndManual[] =
	_( "Package & Manual Illustration" );
static const u8 gCreditsText_EnglishVersion[] =
...
static const u8 gCreditsText_NationalFederationBlind[] =
	_( "National Federation of the Blind" );
static const u8 gCreditsText_PatriciaAMaurer[] = _( "Patricia A. Maurer" );
static const u8 gCreditsText_EuropeanBlindUnion[] =
	_( "European Blind Union" );
static const u8 gCreditsText_AustralianBrailleAuthority[] =
...
static const struct CreditsEntry gCreditsEntry_MapDesigners[] = {
	11, 1, gCreditsText_MapDesigners};
static const struct CreditsEntry gCreditsEntry_MapDataDesigners[] = {
	9, 1, gCreditsText_MapDataDesigners};
...
static void sub_81758E4( u8 taskIdA )
{
	...
	case 0: {
		...
		LZ77UnCompVram(
			gBirchGrassTilemap, (void*)( BG_SCREEN_ADDR( 7 ) ) );
	...
	case 1:
		gTasks[taskIdA].data[TDA_TASK_D_ID] =
			CreateTask( sub_81760FC, 0 );
		gTasks[gTasks[taskIdA].data[TDA_TASK_D_ID]].data[TDD_STATE] =
			1;
		gTasks[gTasks[taskIdA].data[TDA_TASK_D_ID]]
			.data[TDD_TASK_A_ID] = taskIdA;
		gTasks[gTasks[taskIdA].data[TDA_TASK_D_ID]].data[TDD_2] =
			gTasks[taskIdA].data[TDA_7];

		BeginNormalPaletteFade( 0xFFFFFFFF, 0, 16, 0, RGB_BLACK );
		SetGpuReg( REG_OFFSET_BG3HOFS, 0 );
		SetGpuReg( REG_OFFSET_BG3VOFS, 32 );
		SetGpuReg( REG_OFFSET_BG3CNT,
			BGCNT_PRIORITY( 3 ) | BGCNT_CHARBASE( 0 ) |
	...
	if( !gPaletteFade.active )
	{
		if( gTasks[taskIdA].data[TDA_0] == 0 || gMain.newKeys )
		{
			...
			BeginNormalPaletteFade(
				0xFFFFFFFF, 8, 0, 16, RGB_WHITEALPHA );
...
static void sub_8175DA0( u8 taskIdB )
{
	...
	default:
		if( !gPaletteFade.active )
		{
			...
			gTasks[gTasks[taskIdB].data[TDB_TASK_A_ID]]
				.data[TDA_14] = 0;
			gUnknown_0203BCE0     = 0;
		}
		return;
	case 1:
		...
		return;
	case 2:
		if( gTasks[gTasks[taskIdB].data[TDB_TASK_A_ID]].func ==
			sub_8175774 )
		{
			if( gTasks[taskIdB].data[TDB_CURRENT_PAGE] <
				PAGE_COUNT )
			{
				for( i = 0; i < 5; i++ )
					sub_81755BC(
				...
				if( gTasks[gTasks[taskIdB].data[TDB_TASK_A_ID]]
						.data[TDA_13] == 1 )
					BeginNormalPaletteFade( 0x00000300,
						0,
						16,
						0,
						COLOR_LIGHT_GREEN );
	...
	case 4:
		...
		if( sub_817603C( (u8)gTasks[taskIdB].data[TDB_CURRENT_PAGE],
		...
		gTasks[taskIdB].data[TDB_0] += 1;
		if( gTasks[gTasks[taskIdB].data[TDB_TASK_A_ID]].data[TDA_13] ==
			1 )
			BeginNormalPaletteFade(
				0x00000300, 0, 0, 16, COLOR_LIGHT_GREEN );
		else
			BeginNormalPaletteFade(
				0x00000300, 0, 0, 16, COLOR_DARK_GREEN );
		return;
	case 5:
		if( !gPaletteFade.active )
		{
...
static void sub_81760FC( u8 taskIdD )
{
	...
	case 0:
		break;
	case 1:
		if( gUnknown_0203BCE8->unk90 == 0 &&
			gTasks[gTasks[taskIdD].data[TDD_TASK_A_ID]]
					.data[TDA_14] == 0 )
			break;
		gTasks[taskIdD].data[TDD_STATE]++;
		break;
	case 2:
		if( gUnknown_0203BCE8->unk8E == 71 ||
			gTasks[gTasks[taskIdD].data[TDD_TASK_A_ID]].func !=
				sub_8175774 )
			break;
		r2 = sub_8177224(
			gUnknown_0203BCE8->unk0[gUnknown_0203BCE8->unk92],
			gUnknown_085E6F7C[gUnknown_0203BCE8->unk90][0],
			gUnknown_085E6F7C[gUnknown_0203BCE8->unk90][1],
...
static void sub_817624C( u8 taskIdC )
{
	...
	case 0:
		gUnknown_0203BD26 =
			Sin( ( gTasks[taskIdC].data[TDC_5] >> 1 ) & 0x7F, 12 );
		gTasks[taskIdC].data[TDC_5]++;
		break;
	case 1:
		if( gUnknown_0203BD26 != 0 )
		{
			gUnknown_0203BD26 = Sin(
				( gTasks[taskIdC].data[TDC_5] >> 1 ) & 0x7F,
				12 );
			gTasks[taskIdC].data[TDC_5]++;
		}
		else
	...
	case 2:
		if( gTasks[taskIdC].data[TDC_5] < 64 )
		{
			gTasks[taskIdC].data[TDC_5]++;
			gUnknown_0203BD26 =
				Sin( gTasks[taskIdC].data[TDC_5] & 0x7F, 20 );
		}
		else
		{
			gTasks[taskIdC].data[TDC_0]++;
...
static void sub_817651C( u8 taskIdE )
{
	...
	case 0:
		...
		{
			if( gTasks[gTasks[gTasks[taskIdE].data[TDE_TASK_A_ID]]
					    .data[TDA_TASK_B_ID]]
					.data[TDB_CURRENT_PAGE] == 2 )
			{
				gTasks[gTasks[gTasks[taskIdE]
						       .data[TDE_TASK_A_ID]]
						.data[TDA_TASK_C_ID]]
					.data[TDC_0]        = 20;
	...
	case 2:
		if( gTasks[taskIdE].data[TDE_1] != 0x7FFF )
		{
			taskIdC = gTasks[gTasks[taskIdE].data[TDE_TASK_A_ID]]
					  .data[TDA_TASK_C_ID];
...
static void sub_817664C( u8 data, u8 taskIdA )
{
	...
	{
	case 0:
		gSprites[gTasks[taskIdA].data[TDA_PLAYER_CYCLIST]].invisible =
			FALSE;
		gSprites[gTasks[taskIdA].data[TDA_RIVAL_CYCLIST]].invisible =
			FALSE;
		gSprites[gTasks[taskIdA].data[TDA_PLAYER_CYCLIST]].pos1.x =
			272;
		...
		gTasks[taskIdA].data[TDA_0] =
			CreateBicycleAnimationTask( 0, 0x2000, 0x20, 8 );
		break;
	case 1:
		gSprites[gTasks[taskIdA].data[TDA_PLAYER_CYCLIST]].invisible =
			FALSE;
	...
	case 2:
		...
		gSprites[gTasks[taskIdA].data[TDA_RIVAL_CYCLIST]].data[0]  = 0;
		gTasks[taskIdA].data[TDA_0] =
			CreateBicycleAnimationTask( 1, 0x2000, 0x200, 8 );
		break;
	case 3:
		gSprites[gTasks[taskIdA].data[TDA_PLAYER_CYCLIST]].invisible =
			FALSE;
	...
	case 4:
		...
		gSprites[gTasks[taskIdA].data[TDA_RIVAL_CYCLIST]].data[0]  = 0;
		gTasks[taskIdA].data[TDA_0] =
			CreateBicycleAnimationTask( 2, 0x2000, 0x200, 8 );
		break;
	}
	...
	gTasks[gTasks[taskIdA].data[TDA_TASK_E_ID]].data[TDE_TASK_A_ID] =
		taskIdA;
...
static void sub_8176D1C( u16 arg0, u16 arg1, u16 arg2 )
{
	...
	LoadPalette(
		gIntroCopyright_Pal, arg2, sizeof( gIntroCopyright_Pal ) );
...
static void sub_8176DBC(
	const u8 arg0[], u8 baseX, u8 baseY, u16 arg3, u16 palette )
{
	u8 y, x;
	...
	case 2:
		...
		else
		{
			SetGpuReg( REG_OFFSET_BLDCNT,
				BLDCNT_EFFECT_BLEND | BLDCNT_TGT2_BG0 |
					BLDCNT_TGT2_BG1 | BLDCNT_TGT2_BG2 |
					BLDCNT_TGT2_BG3 );
			SetGpuReg(
				REG_OFFSET_BLDALPHA, BLDALPHA_BLEND( 16, 0 ) );
...
static u8 sub_8177224( u16 nationalDexNum, s16 x, s16 y, u16 position )
{
	...
	spriteId2                   = CreateSprite( &gUnknown_085E7068,
                gSprites[spriteId].pos1.x,
                gSprites[spriteId].pos1.y,
                1 );
...
static void sub_8177388( void )
{
	u16 starter = SpeciesToNationalPokedexNum(
		GetStarterPokemon( VarGet( VAR_STARTER_MON ) ) );
	u16 page;
	u16 dexNum;
	u16 j;
	...
	do
	{
		...
		{
			gUnknown_0203BCE8->unk96[page] =
				gUnknown_0203BCE8
					->unk96[gUnknown_0203BCE8->unk39A];
			gUnknown_0203BCE8->unk96[gUnknown_0203BCE8->unk39A] =
				0;
		}
	...
	if( gUnknown_0203BCE8->unk94 < 71 )
	{
		...
		{
			gUnknown_0203BCE8->unk0[j] =
				gUnknown_0203BCE8->unk0[page];
	...
	{
		for( dexNum = 0; gUnknown_0203BCE8->unk0[dexNum] != starter &&
			dexNum < 71;
			dexNum++ )
			;
		...
		{
			gUnknown_0203BCE8->unk0[dexNum] =
				gUnknown_0203BCE8->unk0[70];
			gUnknown_0203BCE8->unk0[70] = starter;
		}
		else
		{
