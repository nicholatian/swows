...
u16 sub_80D2E84( u16 species )
{
	...
	else
	{
		if( species > ( SPECIES_UNOWN_B - 1 ) )
			species = SPECIES_OLD_UNOWN_J; // That's an oddly
						       // specific species.
...
void sub_80D2EF8( struct Sprite* sprite ) { sub_80D328C( sprite ); }
...
void SafeLoadMonIconPalette( u16 species )
{
	...
	if( IndexOfSpritePaletteTag( gMonIconPaletteTable[palIndex].tag ) ==
		0xFF )
...
void sub_80D3014( struct Sprite* sprite ) { UpdateMonIconFrame( sprite ); }
...
u8 UpdateMonIconFrame( struct Sprite* sprite )
{
	...
	if( sprite->animDelayCounter == 0 )
	{
		...
		default:
			RequestSpriteCopy(
				// pointer arithmetic is needed to get the
				// correct pointer to perform the sprite copy
				// on. because sprite->images is a struct def,
				// it has to be casted to (u8 *) before any
				// arithmetic can be performed.
				(u8*)sprite->images +
					( sSpriteImageSizes[sprite->oam.shape]
							   [sprite->oam.size] *
						frame ),
				(u8*)( OBJ_VRAM0 +
					sprite->oam.tileNum * TILE_SIZE_4BPP ),
				sSpriteImageSizes[sprite->oam.shape]
						 [sprite->oam.size] );
			sprite->animDelayCounter =
				sprite->anims[sprite->animNum]
					     [sprite->animCmdIndex]
						     .frame.duration &
				0xFF;
			sprite->animCmdIndex++;
...
static u8 CreateMonIconSprite( struct MonIconSpriteTemplate* iconTemplate,
	s16 x,
	s16 y,
	u8 subpriority )
{
	...
	struct SpriteFrameImage image = {NULL,
		sSpriteImageSizes[iconTemplate->oam->shape]
				 [iconTemplate->oam->size]};
