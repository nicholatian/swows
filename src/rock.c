...
void AnimTask_LoadSandstormBackground( u8 taskId )
{
	...
	AnimLoadCompressedBgGfx(
		animBg.bgId, gUnknown_08D8D58C, animBg.tilesOffset );
	sub_80A6D60( &animBg, gUnknown_08D8D410, 0 );
	LoadCompressedPalette(
		gBattleAnimSpritePal_FlyingDirt, animBg.paletteId * 16, 32 );
...
static void sub_8111214( struct Task* task )
{
	...
	if( spriteId != MAX_SPRITES )
	{
		...
		gSprites[spriteId].data[2] = ( ( task->data[12] * 20 ) + x ) +
			( task->data[1] * 3 );
...
void sub_8111590( u8 taskId )
{
	...
	gBattle_BG3_Y =
		gTasks[taskId].data[2] + Cos( 4, gTasks[taskId].data[1] );
