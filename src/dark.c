...
void sub_81138D4( struct Sprite* sprite )
{
	...
	sprite->data[3] = GetBattlerSpriteCoord(
...
void sub_8113A90( struct Sprite* sprite )
{
	...
	case 0:
		...
		sprite->pos1.y = GetBattlerSpriteCoordAttr(
					 battler, BATTLER_COORD_ATTR_TOP ) +
			8;
		break;
	case 1:
		sprite->pos1.x = GetBattlerSpriteCoordAttr(
...
void sub_8113BAC( u8 taskId )
{
	...
	task->data[7] =
		...
		31;
	task->data[6] = GetBattlerSpriteCoordAttr(
...
void sub_8113E78( u8 taskId )
{
	...
	case 0:
		...
		{
			task->data[3] = GetBattlerSpriteBGPriorityRank(
				gBattleAnimTarget );
			if( task->data[3] == 1 )
			{
				SetGpuReg( REG_OFFSET_BLDCNT,
	...
	case 1:
		...
		break;
	case 2:
		task->data[7] = GetBattlerSpriteCoord(
					gBattleAnimTarget, BATTLER_COORD_Y ) +
			31;
		task->data[6] = GetBattlerSpriteCoordAttr( gBattleAnimTarget,
...
static void sub_8114244( struct Task* task )
{
	...
	if( var2 != 0 )
	{
		...
		for( i = i; i < task->data[7]; i++ )
		{
			...
			{
				gScanlineEffectRegBuffers
					[gScanlineEffect.srcBuffer][i] = var4;
				var4--;
			}
		}
	}
...
{
	u8 toBG2 = GetBattlerSpriteBGPriorityRank( gBattleAnimAttacker ) ^ 1
		? 1
		: 0;
	...
	if( IsBattlerSpriteVisible( BATTLE_PARTNER( gBattleAnimAttacker ) ) )
	{
		MoveBattlerSpriteToBG(
			gBattleAnimAttacker ^ 2, toBG2 ^ 1, TRUE );
		gSprites[gBattlerSpriteIds[gBattleAnimAttacker ^ 2]]
			.invisible = 0;
	}
...
void AnimTask_MetallicShine( u8 taskId )
{
	...
	SetGpuReg( REG_OFFSET_WINOUT,
		WINOUT_WINOBJ_BG_ALL | WINOUT_WINOBJ_OBJ | WINOUT_WINOBJ_CLR |
			WINOUT_WIN01_BG0 | WINOUT_WIN01_BG2 |
	...
	AnimLoadCompressedBgGfx(
		animBg.bgId, gMetalShineGfx, animBg.tilesOffset );
...
void AnimTask_SetGreyscaleOrOriginalPal( u8 taskId )
{
	...
	if( spriteId != 0xFF )
		SetGreyscaleOrOriginalPalette(
			gSprites[spriteId].oam.paletteNum + 16,
			gBattleAnimArgs[1] );
