...
const struct SpritePalette gFieldEffectObjectPaletteInfo6 = {
	gFieldEffectObjectPalette6, 0x1000};
...
bool8 SetUpFieldMove_Cut( void )
{
	...
	if( CheckObjectGraphicsInFrontOfPlayer(
		    EVENT_OBJ_GFX_CUTTABLE_TREE ) == TRUE )
	{
	...
	else
	{
		...
		for( i = 0; i < CUT_NORMAL_SIDE; i++ )
		{
			...
			for( j = 0; j < CUT_NORMAL_SIDE; j++ )
			{
				...
				if( MapGridGetZCoordAt( x, y ) ==
					gPlayerFacingPosition.height )
				{
					tileBehavior =
						MapGridGetMetatileBehaviorAt(
							x, y );
					if( MetatileBehavior_IsPokeGrass(
						    tileBehavior ) == TRUE ||
					...
					{
						cutTiles[i * 3 + j] = TRUE;
						if( MetatileBehavior_IsCuttableGrass(
							    tileBehavior ) ==
							TRUE )
							sHyperCutTiles[6 +
								( i * 5 ) +
		...
		else
		{
			...
			for( i = 0; i < 16; i++ )
			{
				x = gPlayerFacingPosition.x +
					sHyperCutStruct[i].x;
				y = gPlayerFacingPosition.y +
					sHyperCutStruct[i].y;
				tileCuttable = TRUE;
				...
				do
				{
					if( sHyperCutStruct[i].unk2[j] == 0 )
						break;
					if( cutTiles[( u8 )( sHyperCutStruct[i]
								     .unk2[j] -
						    1 )] == FALSE )
					{
						tileCuttable = FALSE;
...
bool8 FldEff_CutGrass( void )
{
	...
	{
		sCutGrassSpriteArrayPtr[i] =
			CreateSprite( &sSpriteTemplate_CutGrass,
...
static void SetCutGrassMetatile( s16 x, s16 y )
{
	...
	case METATILE_ID_SECRET_BASE_LEFT_POKE_LONG_GRASS:
		MapGridSetMetatileIdAt(
			x, y, METATILE_ID_SECRET_BASE_LEFT_LONG_GRASS );
		break;
	case METATILE_ID_SECRET_BASE_CENTER_POKE_LONG_GRASS:
		MapGridSetMetatileIdAt(
			x, y, METATILE_ID_SECRET_BASE_CENTER_LONG_GRASS );
		break;
	case METATILE_ID_SECRET_BASE_RIGHT_POKE_LONG_GRASS:
		MapGridSetMetatileIdAt(
			x, y, METATILE_ID_SECRET_BASE_RIGHT_LONG_GRASS );
		break;
...
static void SetCutGrassMetatiles( s16 x, s16 y )
{
	...
	{
		s16 currentX = x + i;
		if( MapGridGetMetatileIdAt( currentX, y ) ==
			METATILE_ID_POKE_LONG_GRASS )
		{
			switch( GetLongGrassCaseAt( currentX, y + 1 ) )
			{
			case LONG_GRASS_FIELD:
				MapGridSetMetatileIdAt( currentX,
					y + 1,
					METATILE_ID_POKE_LONG_GRASS_START );
				break;
			case LONG_GRASS_BASE_LEFT:
				MapGridSetMetatileIdAt( currentX,
					y + 1,
					METATILE_ID_SECRET_BASE_LEFT_POKE_LONG_GRASS );
				break;
			case LONG_GRASS_BASE_CENTER:
				MapGridSetMetatileIdAt( currentX,
					y + 1,
					METATILE_ID_SECRET_BASE_CENTER_POKE_LONG_GRASS );
				break;
			case LONG_GRASS_BASE_RIGHT:
				MapGridSetMetatileIdAt( currentX,
					y + 1,
					METATILE_ID_SECRET_BASE_RIGHT_POKE_LONG_GRASS );
				break;
			}
		}
		if( MapGridGetMetatileIdAt( currentX, lowerY ) ==
			METATILE_ID_GRASS )
		{
			...
			if( MapGridGetMetatileIdAt( currentX, lowerY + 1 ) ==
				...
				MapGridSetMetatileIdAt( currentX,
					lowerY + 1,
					METATILE_ID_SECRET_BASE_LEFT_LONG_GRASS );
			if( MapGridGetMetatileIdAt( currentX, lowerY + 1 ) ==
				METATILE_ID_SECRET_BASE_CENTER_POKE_LONG_GRASS )
				MapGridSetMetatileIdAt( currentX,
					lowerY + 1,
					METATILE_ID_SECRET_BASE_CENTER_LONG_GRASS );
			if( MapGridGetMetatileIdAt( currentX, lowerY + 1 ) ==
				METATILE_ID_SECRET_BASE_RIGHT_POKE_LONG_GRASS )
				MapGridSetMetatileIdAt( currentX,
					lowerY + 1,
					METATILE_ID_SECRET_BASE_RIGHT_LONG_GRASS );
		}
	}
...
static void HandleLongGrassOnHyper( u8 caseId, s16 x, s16 y )
{
	...
	if( arr[0] == TRUE )
	{
		if( MapGridGetMetatileIdAt( newX, y + 3 ) ==
			METATILE_ID_POKE_LONG_GRASS_START )
			MapGridSetMetatileIdAt(
				newX, y + 3, METATILE_ID_GRASS );
		if( MapGridGetMetatileIdAt( newX, y + 3 ) ==
			METATILE_ID_SECRET_BASE_LEFT_POKE_LONG_GRASS )
			MapGridSetMetatileIdAt( newX,
				y + 3,
				METATILE_ID_SECRET_BASE_LEFT_LONG_GRASS );
		if( MapGridGetMetatileIdAt( newX, y + 3 ) ==
			METATILE_ID_SECRET_BASE_CENTER_POKE_LONG_GRASS )
	...
	if( arr[1] == TRUE )
	{
		if( MapGridGetMetatileIdAt( newX, y + 2 ) ==
			METATILE_ID_POKE_LONG_GRASS )
		{
...
static void CutGrassSpriteCallback2( struct Sprite* sprite )
{
	...
	sprite->data[0] += 1 + ( sprite->data[3] >> 2 ); // right shift by 2 is
							 // dividing by 4
	sprite->data[3]++;
	...
	if( sprite->data[1] != 28 )
		...
		sprite->callback =
			CutGrassSpriteCallbackEnd; // done rotating the grass,
						   // execute clean up
						   // function
}
...
static void CutGrassSpriteCallbackEnd( struct Sprite* sprite )
{
	...
	FieldEffectStop(
		&gSprites[sCutGrassSpriteArrayPtr[0]], FLDEFF_CUT_GRASS );
...
void FixLongGrassMetatilesWindowTop( s16 x, s16 y )
{
	...
	if( MetatileBehavior_IsLongGrass_Duplicate( metatileBehavior ) )
	{
		...
		case LONG_GRASS_FIELD:
			MapGridSetMetatileIdAt(
				x, y + 1, METATILE_ID_POKE_LONG_GRASS_START );
			break;
		case LONG_GRASS_BASE_LEFT:
