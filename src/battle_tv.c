...
static void AddPointsBasedOnWeather(
	u16 weatherFlags, u16 moveId, u8 moveSlot );
...
void BattleTv_SetDataBasedOnString( u16 stringId )
{
	...
	moveSlot = GetBattlerMoveSlotId(
		gBattlerAttacker, gBattleMsgDataPtr->currentMove );

	if( moveSlot >= 4 && sub_817E0B8( stringId ) &&
		stringId > BATTLESTRINGS_ID_ADDER )
	{
	...
	case STRINGID_PKMNANCHOREDITSELF:
		if( tvPtr->pos[defSide][defFlank].ingrainMonId != 0 )
		{
			AddMovePoints( 2,
				6,
				defSide,
				( tvPtr->pos[defSide][defFlank].ingrainMonId -
					1 ) * 4 +
	...
	case STRINGID_PKMNSSTATCHANGED3:
		if( gBattleTextBuff1[2] != 0 )
			AddMovePoints(
				0x18, moveSlot, gBattleTextBuff1[2] - 1, 0 );
		break;
	...
	case STRINGID_PKMNSAPPEDBYLEECHSEED:
		if( tvPtr->pos[atkSide][atkFlank].leechSeedMonId != 0 )
		{
			AddMovePoints( 8,
				1,
				tvPtr->pos[atkSide][atkFlank].leechSeedMonId -
					1,
				tvPtr->pos[atkSide][atkFlank]
					.leechSeedMoveSlot );
	...
	case STRINGID_PKMNLOCKEDINNIGHTMARE:
			...
			tvPtr->pos[atkSide][atkFlank].nightmareMonId != 0 )
		{
			AddMovePoints( 8,
				5,
				tvPtr->pos[atkSide][atkFlank].nightmareMonId -
					1,
				tvPtr->pos[atkSide][atkFlank]
					.nightmareMoveSlot );
	...
	case STRINGID_PKMNWASBURNED:
		tvPtr->mon[effSide][gBattlerPartyIndexes[gEffectBattler]]
			.brnMonId = gBattlerPartyIndexes[gBattlerAttacker] + 1;
		tvPtr->mon[effSide][gBattlerPartyIndexes[gEffectBattler]]
			.brnMoveSlot = moveSlot;
		break;
	case STRINGID_PKMNHURTBYBURN:
		if( GetMonData( atkMon, MON_DATA_HP, NULL ) != 0 )
		{
			if( tvPtr->mon[atkSide]
	...
	case STRINGID_PKMNWASPOISONED:
		tvPtr->mon[effSide][gBattlerPartyIndexes[gEffectBattler]]
			.psnMonId = gBattlerPartyIndexes[gBattlerAttacker] + 1;
		tvPtr->mon[effSide][gBattlerPartyIndexes[gEffectBattler]]
			.psnMoveSlot = moveSlot;
		break;
	...
	case STRINGID_PKMNISPARALYZED:
		if( tvPtr->mon[atkSide][gBattlerPartyIndexes[gBattlerAttacker]]
			...
			AddMovePoints( 9,
				2,
				tvPtr->mon[atkSide][gBattlerPartyIndexes
							    [gBattlerAttacker]]
						.prlzMonId -
					1,
				tvPtr->mon[atkSide][gBattlerPartyIndexes
	...
	case STRINGID_PKMNFELLASLEEP:
		tvPtr->mon[effSide][gBattlerPartyIndexes[gEffectBattler]]
			.slpMonId = gBattlerPartyIndexes[gBattlerAttacker] + 1;
		tvPtr->mon[effSide][gBattlerPartyIndexes[gEffectBattler]]
			.slpMoveSlot = moveSlot;
		break;
	...
	case STRINGID_PKMNWASFROZEN:
		tvPtr->mon[effSide][gBattlerPartyIndexes[gEffectBattler]]
			.frzMonId = gBattlerPartyIndexes[gBattlerAttacker] + 1;
		tvPtr->mon[effSide][gBattlerPartyIndexes[gEffectBattler]]
			.frzMoveSlot = moveSlot;
		break;
	...
	case STRINGID_ITHURTCONFUSION:
		if( tvPtr->pos[atkSide][atkFlank].confusionMonId != 0 )
			AddMovePoints( 9,
				1,
				tvPtr->pos[atkSide][atkFlank].confusionMonId -
					1,
				tvPtr->pos[atkSide][atkFlank]
					.confusionMoveSlot );
	...
	case STRINGID_PKMNPROTECTEDBYMIST:
		if( tvPtr->side[defSide].mistMonId != 0 )
			AddMovePoints( 16,
				0,
				tvPtr->side[defSide].mistMonId - 1,
	...
	u16 move, u16 weatherFlags, struct DisableStruct* disableStructPtr )
{
	...
	AddMovePoints(
		13, gBattleMoves[move].type, gBattleMoves[move].power, 0 );
	AddMovePoints(
		14, gBattleMoves[move].type, gBattleMoves[move].power, 0 );
	...
	case B_ANIM_FUTURE_SIGHT_HIT:
		if( tvPtr->side[atkSide].futureSightMonId != 0 )
		{
			AddMovePoints( 2,
				0,
				atkSide,
				( tvPtr->side[atkSide].futureSightMonId - 1 ) *
						4 +
					tvPtr->side[atkSide]
						.futureSightMoveSlot );
	...
	case B_ANIM_DOOM_DESIRE_HIT:
		if( tvPtr->side[atkSide].doomDesireMonId != 0 )
		{
			AddMovePoints( 2,
				1,
				atkSide,
				( tvPtr->side[atkSide].doomDesireMonId - 1 ) *
						4 +
					tvPtr->side[atkSide]
						.doomDesireMoveSlot );
...
void TryPutLinkBattleTvShowOnAir( void )
{
	...
	if( !( gBattleTypeFlags & BATTLE_TYPE_LINK ) ||
		countPlayer != countOpponent )
		return;

	for( i = 0; i < PARTY_SIZE; i++ )
	{
		species =
			GetMonData( &gPlayerParty[i], MON_DATA_SPECIES, NULL );
		if( species != SPECIES_NONE &&
			!GetMonData(
				&gPlayerParty[i], MON_DATA_IS_EGG, NULL ) )
		{
		...
		species =
			GetMonData( &gEnemyParty[i], MON_DATA_SPECIES, NULL );
		if( species != SPECIES_NONE &&
	...
	moveId = GetMonData(
		&gPlayerParty[playerBestMonId], MON_DATA_MOVE1 + i, NULL );
	...
	if( gBattleTypeFlags & BATTLE_TYPE_MULTI )
	{
					...
					gBattleScripting.multiplayerId ) ) )
		{
			...
			PutBattleUpdateOnTheAir(
				sub_806EF84(
					j, gBattleScripting.multiplayerId ),
				moveId,
				playerBestSpecies,
				opponentBestSpecies );
		}
	...
	{
		PutBattleUpdateOnTheAir( gBattleScripting.multiplayerId ^ 1,
			moveId,
			playerBestSpecies,
			opponentBestSpecies );
	}
...
static void AddMovePoints( u8 caseId, u16 arg1, u8 arg2, u8 arg3 )
{
	...
	case 19:
		...
		movePoints->points[arg2][0 * 4 + arg3] +=
	...
	case 2:
		...
		break;
	case 17:
		movePoints->points[atkSide][arg2 * 4 + arg3] +=
			sPointsArray[caseId][arg1];
		break;
	case 8:
	case 9:
	...
	case 21:
		...
		break;
	case 10:
		movePoints->points[arg1][arg2 * 4 + arg3] +=
	...
	case 11:
			...
			arg1 == 10 )
		{
			...
			{
				u32 id = ( tvPtr->pos[defSide][0]
							 .waterSportMonId -
						 1 ) *
					4;
				movePoints->points[defSide][id +
					tvPtr->pos[defSide][0]
						.waterSportMoveSlot] +=
					sPointsArray[caseId][0];
			}
	...
	case 13:
			...
			tvPtr->side[defSide].reflectMonId != 0 )
		{
			...
			movePoints->points[defSide][id +
				tvPtr->side[defSide].reflectMoveSlot] +=
				sPointsArray[caseId][0];
		}
		break;
	case 14:
		...
		{
			u32 id =
				( tvPtr->side[defSide].lightScreenMonId - 1 ) *
				4;
			movePoints->points[defSide][id +
				tvPtr->side[defSide].lightScreenMoveSlot] +=
				sPointsArray[caseId][0];
		}
		break;
	}
...
static void AddPointsOnFainting( bool8 targetFainted )
{
	...
	if( tvPtr->side[atkSide].faintCause != 0 )
	{
		...
		case 7:
			if( tvPtr->side[atkSide].spikesMonId != 0 )
			{
				AddMovePoints( 19,
					0,
					atkSide ^ BIT_SIDE,
					( tvPtr->side[atkSide].spikesMonId -
						1 ) * 4 +
						tvPtr->side[atkSide]
							.spikesMoveSlot );
			}
		...
		case 10:
			...
			break;
		case 11:
			if( tvPtr->side[atkSide ^ BIT_SIDE].destinyBondMonId !=
				0 )
			{
				AddMovePoints( 19,
					0,
		...
		case 12:
			for( i = 0; i < 2; i++ )
			{
				if( tvPtr->pos[atkSide][i].confusionMonId !=
					0 )
				{
					AddMovePoints( 19,
						0,
		...
		case 13:
			if( tvPtr->side[atkSide].explosion )
			{
				AddMovePoints( 19,
					0,
					atkSide,
					( tvPtr->side[atkSide].explosionMonId -
						1 ) * 4 +
						tvPtr->side[atkSide]
							.explosionMoveSlot );
			}
...
static void TrySetBattleSeminarShow( void )
{
	...
	if( gBattleTypeFlags &
		( BATTLE_TYPE_DOUBLE | BATTLE_TYPE_LINK |
		...
		( BATTLE_TYPE_PALACE | BATTLE_TYPE_PIKE |
			BATTLE_TYPE_PYRAMID ) )
		return;
	else if( gBattleMoves[gBattleMons[gBattlerAttacker].moves
	...
	for( i = 0; i < MAX_MON_MOVES; i++ )
	{
		...
		if( ShouldCalculateDamage(
			    gCurrentMove, &dmgByMove[i], &powerOverride ) )
		{
			u8 moveResultFlags;
			u16 sideStatus    = gSideStatuses[GET_BATTLER_SIDE(
                                gBattlerTarget )];
			...
			moveResultFlags = TypeCalc( gCurrentMove,
				gBattlerAttacker,
				gBattlerTarget );
			dmgByMove[i]    = gBattleMoveDamage;
			if( dmgByMove[i] == 0 &&
				!( moveResultFlags & MOVE_RESULT_NO_EFFECT ) )
				dmgByMove[i] = 1;
		}
	}
	...
	for( i = 0; i < MAX_MON_MOVES; i++ )
	{
		if( i != gMoveSelectionCursor[gBattlerAttacker] &&
			dmgByMove[i] > dmgByMove[gMoveSelectionCursor
						       [gBattlerAttacker]] )
		{
...
u8 GetBattlerMoveSlotId( u8 battlerId, u16 moveId )
{
	...
	while( 1 )
	{
		if( i >= MAX_MON_MOVES )
			break;
		if( GetMonData( &party[gBattlerPartyIndexes[battlerId]],
			    MON_DATA_MOVE1 + i,
			    NULL ) == moveId )
			break;
		i++;
