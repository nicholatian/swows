...
bool8 CheckObjectGraphicsInFrontOfPlayer( u16 graphicsId )
{
	...
	eventObjId = GetEventObjectIdByXYZ( gPlayerFacingPosition.x,
...
static void sub_813552C( u8 taskId )
{
	if( EventObjectCheckHeldMovementStatus(
		    &gEventObjects[gPlayerAvatar.eventObjectId] ) == TRUE )
	{
		FieldEffectStart( FLDEFF_FIELD_MOVE_SHOW_MON_INIT );
		gTasks[taskId].func = sub_8135578;
	}
...
{
	void ( *func )( void ) =
		( void ( * )( void ) )( ( (u16)gTasks[taskId].data[8] << 16 ) |
...
bool8 SetUpFieldMove_RockSmash( void )
{
	...
	// In Emerald, it is opened by using Rock Smash.
	...
	else if( CheckObjectGraphicsInFrontOfPlayer(
			 EVENT_OBJ_GFX_BREAKABLE_ROCK ) == TRUE )
	{
		gFieldCallback2        = FieldCallback_PrepareFadeInFromMenu;
