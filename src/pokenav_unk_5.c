...
{
	struct Pokenav5Struct* state =
		AllocSubstruct( 3, sizeof( struct Pokenav5Struct ) );
	if( !state )
		return FALSE;
...
{
	struct Pokenav5Struct_2* state =
		AllocSubstruct( 4, sizeof( struct Pokenav5Struct_2 ) );
	if( !state )
		return FALSE;
...
static u32 sub_81CC6F4( int taskState )
{
	...
	case 0:
		...
		SetBgMode( 1 );
		InitBgTemplates(
			gUnknown_086230D8, ARRAY_COUNT( gUnknown_086230D8 ) );
		regionMap = GetSubstructPtr( 16 );
		sub_8122CF8(
			regionMap, &gUnknown_086230D8[1], sub_81CC6D0( ) );
		sub_81CC9C0( );
		return 0;
	case 1:
		if( sub_8122DB0( ) )
...
static void sub_81CCB0C( struct Pokenav5Struct_2* state )
{
	...
	case MAPSECTYPE_CITY_CANFLY:
		...
		AddTextPrinterParameterized( state->unk8,
			7,
			regionMap->mapSecName,
			0,
			1,
			TEXT_SPEED_FF,
			NULL );
...
static bool32 sub_81CCC88( void ) { return FuncIsActiveTask( sub_81CCC9C ); }
...
static void sub_81CCD10( void ) { CreateLoopedTask( sub_81CCD34, 1 ); }
...
static u32 sub_81CCD34( int taskState )
{
	...
	{
		LZ77UnCompWram( gUnknown_08623118[taskState].data,
...
static void sub_81CCD70(
	struct Pokenav5Struct_2* state, int mapSecId, int pos )
{
	int i;
	for( i = 0; i < (int)ARRAY_COUNT( gUnknown_08623118 ) &&
		( gUnknown_08623118[i].mapSecId != mapSecId ||
			gUnknown_08623118[i].unk2 != pos );
		i++ )
		;
...
static void sub_81CCE58( void )
{
	...
	{
		u8 spriteId =
			CreateSprite( &gUnknown_086231D0, 152 + i * 32, y, 8 );
