...
enum
{ // Corresponds to gHealthboxElementsGfxTable (and the tables after it)
  // in graphics.c These are indexes into the tables, which are filled
  // with 8x8 square pixel data.
...
static void UpdateHpTextInHealthboxInDoubles(
	u8 healthboxSpriteId, s16 value, u8 maxOrCurrent );
static void UpdateStatusIconInHealthbox( u8 healthboxSpriteId );

static void TextIntoHealthboxObject(
	void* dest, u8* windowTileData, s32 windowWidth );
static void SafariTextIntoHealthboxObject(
	void* dest, u8* windowTileData, u32 windowWidth );
static void HpTextIntoHealthboxObject(
	void* dest, u8* windowTileData, u32 windowWidth );
...
static s32 CalcNewBarValue( s32 maxValue,
	s32 currValue,
	s32 receivedValue,
	s32* arg3,
	u8 arg4,
	u16 arg5 );
static u8 GetScaledExpFraction(
	s32 currValue, s32 receivedValue, s32 maxValue, u8 scale );
static void MoveBattleBarGraphically( u8 battlerId, u8 whichBar );
static u8 CalcBarFilledPixels( s32 maxValue,
	s32 oldValue,
	s32 receivedValue,
	s32* currValue,
	u8* arg4,
...
static const struct Subsprite sUnknown_0832C220[] = {{240, 0, 1, 3, 0, 1},
	{48, 0, 0, 2, 32, 1},
	{240, 32, 1, 1, 48, 1},
	{16, 32, 1, 1, 52, 1},
	{48, 32, 1, 1, 56, 1}};

static const struct Subsprite sUnknown_0832C234[] = {{240, 0, 1, 3, 64, 1},
	{48, 0, 0, 2, 96, 1},
	{240, 32, 1, 1, 112, 1},
	{16, 32, 1, 1, 116, 1},
	{48, 32, 1, 1, 120, 1}};
...
static const struct Subsprite sUnknown_0832C2AC[] = {{160, 0, 1, 1, 0, 1},
	{192, 0, 1, 1, 4, 1},
	{224, 0, 1, 1, 8, 1},
	{0, 0, 1, 1, 8, 1},
	{32, 0, 1, 1, 8, 1},
...
static const struct SubspriteTable sStatusSummaryBar_SubspriteTable[] = {
	{ARRAY_COUNT( sStatusSummaryBar_Subsprites_0 ),
...
static const u8 sUnknown_0832C2D4[] =
	INCBIN_U8( "graphics/battle_interface/unknown_32C2D4.4bpp" );
...
static const struct WindowTemplate sHealthboxWindowTemplate = {
	0, 0, 0, 8, 2, 0, 0}; // width = 8, height = 2
...
// into two sprites. healthboxLeft  or healthboxMain  is the left part that is
// used as the 'main' sprite. healthboxRight or healthboxOther is the right
// part of the healthbox. There's also the third sprite under name of
// healthbarSprite that refers to the healthbar visible on the healtbox.
...
u8 CreateBattlerHealthboxSprites( u8 battlerId )
{
	...
	if( !IsDoubleBattle( ) )
	{
		if( GetBattlerSide( battlerId ) == B_SIDE_PLAYER )
		{
			healthboxLeftSpriteId = CreateSprite(
				&sHealthboxPlayerSpriteTemplates[0],
				240,
				160,
				1 );
		...
		{
			healthboxLeftSpriteId = CreateSprite(
				&sHealthboxOpponentSpriteTemplates[0],
				240,
				160,
				1 );
			healthboxRightSpriteId = CreateSpriteAtEnd(
		...
		gSprites[healthboxRightSpriteId].callback =
	...
	healthbarSpriteId = CreateSpriteAtEnd(
		&sHealthbarSpriteTemplates[gBattlerPositions[battlerId]],
		140,
		60,
		0 );
	healthBarSpritePtr = &gSprites[healthbarSpriteId];
	SetSubspriteTables( healthBarSpritePtr,
		&sUnknown_0832C28C[GetBattlerSide( battlerId )] );
	healthBarSpritePtr->subspriteMode = 2;
	healthBarSpritePtr->oam.priority  = 1;

	CpuCopy32( GetHealthboxElementGfxPtr( HEALTHBOX_GFX_1 ),
		(void*)( OBJ_VRAM0 +
			healthBarSpritePtr->oam.tileNum * TILE_SIZE_4BPP ),
		64 );

	gSprites[healthboxLeftSpriteId].hMain_HealthBarSpriteId =
		healthbarSpriteId;
...
u8 CreateSafariPlayerHealthboxSprites( void )
{
	...
	healthboxLeftSpriteId =
		CreateSprite( &sHealthboxSafariSpriteTemplate, 240, 160, 1 );
	healthboxRightSpriteId = CreateSpriteAtEnd(
		&sHealthboxSafariSpriteTemplate, 240, 160, 1 );
...
void SetBattleBarStruct( u8 battlerId,
...
void SetHealthboxSpriteInvisible( u8 healthboxSpriteId )
{
	...
	gSprites[gSprites[healthboxSpriteId].hMain_HealthBarSpriteId]
		.invisible                                              = TRUE;
...
void SetHealthboxSpriteVisible( u8 healthboxSpriteId )
{
	...
	gSprites[gSprites[healthboxSpriteId].hMain_HealthBarSpriteId]
		.invisible = FALSE;
	gSprites[gSprites[healthboxSpriteId].oam.affineParam].invisible =
		FALSE;
}
...
void DestoryHealthboxSprite( u8 healthboxSpriteId )
{
	...
	DestroySprite( &gSprites[gSprites[healthboxSpriteId]
					 .hMain_HealthBarSpriteId] );
	DestroySprite( &gSprites[healthboxSpriteId] );
}

void DummyBattleInterfaceFunc(
	u8 healthboxSpriteId, bool8 isDoubleBattleBattlerOnly )
{
}
...
void UpdateOamPriorityInAllHealthboxes( u8 priority )
{
	...
	for( i = 0; i < gBattlersCount; i++ )
	{
		...
		u8 healthbarSpriteId = gSprites[gHealthboxSpriteIds[i]]
					       .hMain_HealthBarSpriteId;
...
static void UpdateLvlInHealthbox( u8 healthboxSpriteId, u8 lvl )
{
	...
	xPos = (u32)ConvertIntToDecimalStringN(
		text + 2, lvl, STR_CONV_MODE_LEFT_ALIGN, 3 );
		...
		!IsDoubleBattle( ) )
	{
		...
		if( maxOrCurrent != HP_CURRENT ) // singles, max
		{
			ConvertIntToDecimalStringN(
				text, value, STR_CONV_MODE_RIGHT_ALIGN, 3 );
			windowTileData =
		...
		else // singles, current
		{
			...
			HpTextIntoHealthboxObject(
				objVram, windowTileData, 1 );
			objVram = (void*)( OBJ_VRAM0 );
			objVram += spriteTileNum + 0xB00;
			HpTextIntoHealthboxObject(
				objVram, windowTileData + 0x20, 2 );
			RemoveWindowOnHealthbox( windowId );
		}
	}
	else
	{
			...
			GetBattlerSide( battler ) == B_SIDE_OPPONENT )
		{
			UpdateHpTextInHealthboxInDoubles(
				healthboxSpriteId, value, maxOrCurrent );
		}
		else
		{
			u32 var;
			...
			if( GetBattlerSide(
				    gSprites[healthboxSpriteId].data[6] ) ==
				B_SIDE_PLAYER )
			{
			...
			ConvertIntToDecimalStringN( text + 6,
				value,
				STR_CONV_MODE_RIGHT_ALIGN,
				3 );
			RenderTextFont9(
				gMonSpritesGfxPtr->barFontGfx, 9, text );

			for( i = 0; i < 3; i++ )
			{
				CpuCopy32( &gMonSpritesGfxPtr
						    ->barFontGfx[i * 64 + 32],
					(void*)( ( OBJ_VRAM0 ) +
						TILE_SIZE_4BPP *
	...
	u8 healthboxSpriteId, s16 value, u8 maxOrCurrent )
{
		...
		B_SIDE_PLAYER )
	{
		if( gBattleSpritesDataPtr
				->battlerData[gSprites[healthboxSpriteId]
						      .data[6]]
				.hpNumbersNoBars ) // don't print text if only
						   // bars are visible
		{
			spriteTileNum =
				gSprites[gSprites[healthboxSpriteId].data[5]]
					.oam.tileNum *
				TILE_SIZE_4BPP;
			...
			if( maxOrCurrent != HP_CURRENT ) // doubles, max hp
			{
				...
				windowTileData =
					AddTextPrinterAndCreateWindowOnHealthbox(
						text, 0, 5, 0, &windowId );
				HpTextIntoHealthboxObject(
					(void*)( OBJ_VRAM0 ) + spriteTileNum +
						0xC0,
					windowTileData,
					2 );
				RemoveWindowOnHealthbox( windowId );
				CpuCopy32( GetHealthboxElementGfxPtr(
						   HEALTHBOX_GFX_116 ),
	...
	else
	{
						   ...
						   // bars are visible
		{
			...
			txtPtr = ConvertIntToDecimalStringN( text + 6,
				value,
				STR_CONV_MODE_RIGHT_ALIGN,
				3 );
			if( !maxOrCurrent )
...
static void PrintSafariMonInfo( u8 healthboxSpriteId, struct Pokemon* mon )
{
	...
	memcpy( text, sUnknown_0832C3C4, sizeof( sUnknown_0832C3C4 ) );
	barFontGfx = &gMonSpritesGfxPtr->barFontGfx[0x520 +
		( GetBattlerPosition(
			  gSprites[healthboxSpriteId].hMain_Battler ) *
			384 )];
	var        = 5;
	...
	for( j = 6, i = 0; i < var; i++, j++ )
	{
		...
		if( ( text[j] >= 55 && text[j] <= 74 ) ||
			( text[j] >= 135 && text[j] <= 154 ) )
			elementId = 44;
		...
		CpuCopy32( GetHealthboxElementGfxPtr( elementId ),
			barFontGfx + ( i * 64 ),
			0x20 );
	}
	...
	for( j = 1; j < var + 1; j++ )
	{
		spriteTileNum =
			( gSprites[healthboxSpriteId].oam.tileNum +
				( j - ( j / 8 * 8 ) ) + ( j / 8 * 64 ) ) *
			TILE_SIZE_4BPP;
		CpuCopy32( barFontGfx,
			(void*)( OBJ_VRAM0 ) + ( spriteTileNum ),
			0x20 );
		barFontGfx += 0x20;

		spriteTileNum =
			( 8 + gSprites[healthboxSpriteId].oam.tileNum +
				( j - ( j / 8 * 8 ) ) + ( j / 8 * 64 ) ) *
			TILE_SIZE_4BPP;
		CpuCopy32( barFontGfx,
			(void*)( OBJ_VRAM0 ) + ( spriteTileNum ),
	...
	ConvertIntToDecimalStringN( text + 6,
		...
		STR_CONV_MODE_RIGHT_ALIGN,
		2 );
	ConvertIntToDecimalStringN( text + 9,
...
void SwapHpBarsWithHpText( void )
{
	...
	for( i = 0; i < gBattlersCount; i++ )
	{
				...
				GetBattlerSide( i ) != B_SIDE_PLAYER ) )
		{
			...
			gBattleSpritesDataPtr->battlerData[i]
				.hpNumbersNoBars ^= 1;
			noBars = gBattleSpritesDataPtr->battlerData[i]
					 .hpNumbersNoBars;
...
u8 CreatePartyStatusSummarySprites( u8 battlerId,
	struct HpAndStatus* partyInfo,
	u8 arg2,
	bool8 isBattleStart )
{
	bool8 isOpponent;
	...
	summaryBarSpriteId =
		CreateSprite( &sStatusSummaryBarSpriteTemplates[isOpponent],
			bar_X,
			bar_Y,
			10 );
	...
	for( i = 0; i < PARTY_SIZE; i++ )
	{
		...
		{
			gSprites[ballIconSpritesIds[i]].pos1.x -=
				10 * ( 5 - i ) + 24;
			gSprites[ballIconSpritesIds[i]].data[1] =
				( 6 - i ) * 7 + 10;
			gSprites[ballIconSpritesIds[i]].pos2.x = -120;
		}
...
void Task_HidePartyStatusSummary( u8 taskId )
{
	...
	if( isBattleStart )
	{
		for( i = 0; i < PARTY_SIZE; i++ )
		{
			if( GetBattlerSide( battlerId ) != B_SIDE_PLAYER )
			{
				gSprites[ballIconSpriteIds[5 - i]].data[1] =
					7 * i;
		...
		SetSubspriteTables(
			&gSprites[summaryBarSpriteId], sUnknown_0832C2CC );
...
static void sub_8073E08( u8 taskId )
{
	if( ( gTasks[taskId].data[11]++ % 2 ) == 0 )
	{
		...
		SetGpuReg( REG_OFFSET_BLDALPHA,
			BLDALPHA_BLEND( gTasks[taskId].data[15],
				16 - gTasks[taskId].data[15] ) );
	}
	if( gTasks[taskId].tData15 == 0 )
...
static void sub_8073E64( u8 taskId )
{
	...
	if( --gTasks[taskId].tData15 == -1 )
	{
		...
		for( i = 0; i < PARTY_SIZE; i++ )
			ballIconSpriteIds[i] =
				gTasks[taskId].tBallIconSpriteId( i );
...
static void UpdateNickInHealthbox( u8 healthboxSpriteId, struct Pokemon* mon )
{
	...
	// AddTextPrinterAndCreateWindowOnHealthbox's arguments are the same in
	// all 3 cases. It's possible they may have been different in early
	// development phases.
	switch( gender )
	{
	default:
		...
		B_SIDE_PLAYER )
	{
		TextIntoHealthboxObject(
			(void*)( VRAM + 0x10040 + spriteTileNum ),
			windowTileData,
			6 );
		ptr = (void*)( OBJ_VRAM0 );
		if( !IsDoubleBattle( ) )
	...
	{
		TextIntoHealthboxObject(
			(void*)( VRAM + 0x10020 + spriteTileNum ),
			windowTileData,
			7 );
	}
...
static void TryAddPokeballIconToHealthbox(
	u8 healthboxSpriteId, bool8 noStatus )
{
	...
	if( noStatus )
		CpuCopy32( GetHealthboxElementGfxPtr( HEALTHBOX_GFX_70 ),
			(void*)( OBJ_VRAM0 +
				( gSprites[healthBarSpriteId].oam.tileNum +
					8 ) *
					TILE_SIZE_4BPP ),
			32 );
	else
...
static void UpdateStatusIconInHealthbox( u8 healthboxSpriteId )
{
	...
	{
		statusGfxPtr =
			GetHealthboxElementGfxPtr( GetStatusIconForBattlerId(
	...
	CpuCopy16( gPlttBufferUnfaded + 0x100 + pltAdder,
		(void*)( OBJ_PLTT + pltAdder * 2 ),
		2 );
	CpuCopy32( statusGfxPtr,
		(void*)( OBJ_VRAM0 +
			( gSprites[healthboxSpriteId].oam.tileNum +
		...
		GetBattlerSide( battlerId ) == B_SIDE_OPPONENT )
	{
		...
		{
			CpuCopy32(
				GetHealthboxElementGfxPtr( HEALTHBOX_GFX_0 ),
				(void*)( OBJ_VRAM0 +
					gSprites[healthBarSpriteId]
							.oam.tileNum *
						TILE_SIZE_4BPP ),
...
static void UpdateLeftNoOfBallsTextOnHealthbox( u8 healthboxSpriteId )
{
	...
	ConvertIntToDecimalStringN(
		txtPtr, gNumSafariBalls, STR_CONV_MODE_LEFT_ALIGN, 2 );

	windowTileData = AddTextPrinterAndCreateWindowOnHealthbox( text,
		GetStringRightAlignXOffset( 0, text, 0x2F ),
		3,
		2,
		&windowId );
	spriteTileNum =
	...
	SafariTextIntoHealthboxObject(
		(void*)( OBJ_VRAM0 + 0x2C0 ) + spriteTileNum,
		windowTileData,
		2 );
	SafariTextIntoHealthboxObject(
		(void*)( OBJ_VRAM0 + 0xA00 ) + spriteTileNum,
		windowTileData + 0x40,
		4 );
	RemoveWindowOnHealthbox( windowId );
}

void UpdateHealthboxAttribute(
	u8 healthboxSpriteId, struct Pokemon* mon, u8 elementId )
{
	s32 maxHp, currHp;
		...
		B_SIDE_PLAYER )
	{
			...
			elementId == HEALTHBOX_ALL )
		{
			...
			SetBattleBarStruct( battlerId,
				...
				0 );
			MoveBattleBar(
				battlerId, healthboxSpriteId, HEALTH_BAR, 0 );
		}
		isDoubles = IsDoubleBattle( );
		if( !isDoubles &&
			( elementId == HEALTHBOX_EXP_BAR ||
				elementId == HEALTHBOX_ALL ) )
		{
			u16 species;
			...
			maxExpBarValue  = gExperienceTables[gBaseStats[species]
                                                                   .growthRate]
							  [level + 1] -
				currLevelExp;
			SetBattleBarStruct( battlerId,
				...
				isDoubles );
			MoveBattleBar(
				battlerId, healthboxSpriteId, EXP_BAR, 0 );
		}
		if( elementId == HEALTHBOX_NICK || elementId == HEALTHBOX_ALL )
			UpdateNickInHealthbox( healthboxSpriteId, mon );
		if( elementId == HEALTHBOX_STATUS_ICON ||
			elementId == HEALTHBOX_ALL )
...
s32 MoveBattleBar( u8 battlerId, u8 healthboxSpriteId, u8 whichBar, u8 unused )
{
	...
	if( whichBar == HEALTH_BAR ) // health bar
	{
		currentBarValue = CalcNewBarValue(
			...
			gBattleSpritesDataPtr->battleBars[battlerId]
				.receivedValue,
			&gBattleSpritesDataPtr->battleBars[battlerId]
				 .currValue,
	...
	else // exp bar
	{
		...
		if( expFraction == 0 )
			expFraction = 1;
		expFraction = abs( gBattleSpritesDataPtr->battleBars[battlerId]
					   .receivedValue /
			expFraction );
	...
	if( whichBar == EXP_BAR ||
		( whichBar == HEALTH_BAR &&
			!gBattleSpritesDataPtr->battlerData[battlerId]
				 .hpNumbersNoBars ) )
...
static void MoveBattleBarGraphically( u8 battlerId, u8 whichBar )
{
	...
	case HEALTH_BAR:
		...
		if( filledPixelsCount >
			( B_HEALTHBAR_PIXELS * 50 / 100 ) ) // more
							    // than 50
							    // % hp
			barElementId = HEALTHBOX_GFX_HP_BAR_GREEN;
		else if( filledPixelsCount >
			( B_HEALTHBAR_PIXELS * 20 / 100 ) ) // more
							    // than
							    // 20%
							    // hp
			...
			barElementId =
				HEALTHBOX_GFX_HP_BAR_RED; // 20 % or less

		for( i = 0; i < 6; i++ )
		{
			u8 healthbarSpriteId =
				gSprites[gBattleSpritesDataPtr
						 ->battleBars[battlerId]
						 .healthboxSpriteId]
					.hMain_HealthBarSpriteId;
			if( i < 2 )
				CpuCopy32( GetHealthboxElementGfxPtr(
	...
	u16 toAdd )
{
	...
	if( maxValue < scale )
	{
		if( newValue == Q_24_8_TO_INT( *currValue ) &&
			( *currValue & 0xFF ) == 0 )
			return -1;
	}
	else
	{
		if( newValue == *currValue ) // we're done, the bar's value has
					     // been updated
			return -1;
	}

	if( maxValue < scale ) // handle cases of max var having less pixels
			       // than the whole bar
	{
...
static s16 sub_8074F28(
	struct TestingBar* barInfo, s32* currValue, u16* arg2, s32 arg3 )
{
	s16 ret, var;
...
static void sub_8074F88(
	struct TestingBar* barInfo, s32* currValue, u16* arg2 )
{
	u8 sp8[6];
	u16 sp10[6];
	u8 i;
...
static u8 GetScaledExpFraction(
	s32 oldValue, s32 receivedValue, s32 maxValue, u8 scale )
{
	s32 newVal, result;
...
u8 GetHPBarLevel( s16 hp, s16 maxhp )
{
	...
	{
		u8 fraction =
			GetScaledHPFraction( hp, maxhp, B_HEALTHBAR_PIXELS );
		if( fraction > ( B_HEALTHBAR_PIXELS * 50 / 100 ) ) // more than
								   // 50 % hp
			result = HP_BAR_GREEN;
		else if( fraction >
			( B_HEALTHBAR_PIXELS * 20 / 100 ) ) // more than
							    // 20% hp
...
static void TextIntoHealthboxObject(
	void* dest, u8* windowTileData, s32 windowWidth )
{
	CpuCopy32( windowTileData + 256,
		dest + 256,
		windowWidth * TILE_SIZE_4BPP );
	// + 256 as that prevents the top 4 blank rows of
	// sHealthboxWindowTemplate from being copied
	if( windowWidth > 0 )
	{
		do
		{
