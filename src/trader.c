...
void CreateAvailableDecorationsMenu( u8 taskId )
{
	...
	for( i = 0; i < 4; i++ )
	{
		if( trader->decorIds[i] > NUM_DECORATIONS )
			...
			AddTextPrinterParameterized( data[3],
				1,
				gDecorations[trader->decorIds[i]].name,
				8,
				16 * i + 1,
				255,
				NULL );
	...
	AddTextPrinterParameterized(
		data[3], 1, gText_Exit, 8, 16 * i + 1, 255, NULL );
...
void Task_HandleGetDecorationMenuInput( u8 taskId )
{
	...
	default:
		...
		ConvertInternationalString(
			gStringVar1, trader->language[input] );
...
void ScrSpecial_IsDecorationFull( void )
{
	...
	if( gDecorations[gSpecialVar_0x8004].category !=
		...
		GetFirstEmptyDecorSlot(
			gDecorations[gSpecialVar_0x8004].category ) == -1 )
	{
...
void sub_8133DA0( u8 taskId )
{
	if( IsSelectedDecorInThePC( ) == TRUE )
	{
		...
		StringCopy(
			gStringVar3, gDecorations[gSpecialVar_0x8004].name );
		StringCopy(
			gStringVar2, gDecorations[gSpecialVar_0x8006].name );
	}
	else
	{
