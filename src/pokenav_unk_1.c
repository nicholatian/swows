...
{
	struct Pokenav1Struct* state =
		AllocSubstruct( 1, sizeof( struct Pokenav1Struct ) );
	if( !state )
		return FALSE;
...
void sub_81C9430( void ) { FreePokenavSubstruct( 1 ); }
...
static u32 sub_81C943C( struct Pokenav1Struct* a0 )
{
	...
	if( gMain.newKeys & A_BUTTON )
	{
		...
		case 0:
			a0->helpBarIndex =
				gSaveBlock2Ptr->regionMapZoom ? 2 : 1;
...
static u32 sub_81C97BC( struct Pokenav1Struct* a0 ) { return a0->unk8; }
...
static bool32 sub_81C9814( struct Pokenav1Struct* a0 )
{
	if( gMain.newKeys & DPAD_UP )
	{
		...
		a0->descriptionId =
			sDescriptionIds[a0->menuType][a0->cursorPos];
		return TRUE;
	}
