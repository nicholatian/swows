...
// start of .rodata
static const u8 gSpriteImage_858BBF8[] =
	INCBIN_U8( "graphics/naming_screen/pc_icon/0.4bpp" );
static const u8 gSpriteImage_858BCB8[] =
	INCBIN_U8( "graphics/naming_screen/pc_icon/1.4bpp" );
static const u16 gUnknown_0858BD78[] =
	INCBIN_U16( "graphics/naming_screen/0.gbapal" );
static const u16 gUnknown_0858BD98[] =
	INCBIN_U16( "graphics/naming_screen/1.gbapal" );

static const u8* const gUnknown_0858BDB8[] = {gText_PkmnTransferredSomeonesPC,
	gText_PkmnTransferredLanettesPC,
	gText_PkmnBoxSomeonesPCFull,
	gText_PkmnBoxLanettesPCFull};

static const u8 gUnknown_0858BDC8[] =
...
static const u8 gUnknown_0858BE40[] = __(
	"abcdef .ghijkl ,mnopqrs tuvwxyz ABCDEF .GHIJKL ,MNOPQRS TUVWXYZ 01234   56789   !?♂♀/-  …“”‘'   " );

static const u8 gUnknown_0858BEA0[] = {8, 8, 6};
static const u8 gUnknown_0858BEA3[] = {0,
	12,
	24,
	56,
	68,
...
static void NamingScreen_Init( void )
{
	...
	gNamingScreenData->template =
		sNamingScreenTemplates[gNamingScreenData->templateNum];
	gNamingScreenData->currentPage =
		gNamingScreenData->template->initialPage;
	gNamingScreenData->inputCharBaseXPos =
		( 240 - gNamingScreenData->template->maxChars * 8 ) / 2 + 6;
	...
	memset( gNamingScreenData->textBuffer,
		0xFF,
		sizeof( gNamingScreenData->textBuffer ) );
	if( gNamingScreenData->template->copyExistingString != 0 )
		StringCopy( gNamingScreenData->textBuffer,
			gNamingScreenData->destBuffer );
...
static void NamingScreen_InitBGs( void )
{
	...
	for( i = 0; i < 5; i++ )
		gNamingScreenData->windows[i] =
			AddWindow( &gUnknown_0858BE10[i] );
...
static u8 sub_80E3244( u8 a1 ) { return gUnknown_0858BEBB[a1]; }
...
static void DisplaySentToPCMessage( void )
{
	...
	if( !sub_813B260( ) )
	{
		StringCopy( gStringVar1,
			GetBoxNamePtr( VarGet( VAR_STORAGE_UNKNOWN ) ) );
	...
	else
	{
		...
		StringCopy( gStringVar2, gNamingScreenData->destBuffer );
		StringCopy(
			gStringVar3, GetBoxNamePtr( get_unknown_box_id( ) ) );
		stringToDisplay = 2;
	}
	...
	StringExpandPlaceholders(
		gStringVar4, gUnknown_0858BDB8[stringToDisplay] );
	DrawDialogueFrame( 0, 0 );
	gTextFlags.canABSpeedUpPrint = TRUE;
	AddTextPrinterParameterized2(
		0, 1, gStringVar4, GetPlayerTextSpeedDelay( ), 0, 2, 1, 3 );
...
{
	while( sPageSwapAnimStateFuncs[gTasks[taskId].tState](
		       &gTasks[taskId] ) != 0 )
		;
}
...
{
	u16* const arr[] = {&gNamingScreenData->bg2vOffset,
		&gNamingScreenData->bg1vOffset};

	task->tFrameCount += 4;
	*arr[gNamingScreenData->bgToReveal] = Sin( task->tFrameCount, 40 );
	*arr[gNamingScreenData->bgToHide] =
		Sin( ( task->tFrameCount + 128 ) & 0xFF, 40 );
	if( task->tFrameCount >= 64 )
	{
		u8 temp =
			gNamingScreenData->bg1Priority; // Why u8 and not u16?

		gNamingScreenData->bg1Priority =
			gNamingScreenData->bg2Priority;
...
static void Task_80E39BC( u8 taskId )
{
	...
	if( task->data[0] == 3 || task->data[2] == 0 )
		return;
	MultiplyInvertedPaletteRGBComponents( sub_80E3A74( task->data[0] ),
		task->data[3],
		task->data[3],
		task->data[3] );
	if( task->data[5] != 0 )
...
static void CursorInit( void )
{
	gNamingScreenData->cursorSpriteId =
		CreateSprite( &gUnknown_0858C138, 38, 88, 1 );
	sub_80E3E3C( 1 );
...
static void SetCursorPos( s16 x, s16 y )
{
	...
	if( x < gUnknown_0858BEA0[sub_80E3274( )] )
		cursorSprite->pos1.x =
			gUnknown_0858BEA3[x + sub_80E3274( ) * 8] + 38;
	else
...
static void sub_80E4050( void )
{
	struct Sprite* sprite =
		&gSprites[gNamingScreenData->selectBtnFrameSpriteId];
...
static u8 sub_80E40AC( struct Sprite* sprite )
{
	...
	sub_80E41B8( sub_80E3244( gNamingScreenData->currentPage ),
		sprite1,
		sprite2 );
	sprite->data[0]++;
	return 0;
...
static u8 sub_80E4100( struct Sprite* sprite ) { return 0; }
...
static u8 sub_80E4104( struct Sprite* sprite )
{
	...
	if( r4->pos2.y > 7 )
	{
		...
		r4->pos2.y    = -4;
		r4->invisible = TRUE;
		sub_80E41B8( sub_80E3244( ( (u8)sprite->data[1] + 1 ) % 3 ),
			r4,
			r5 );
	}
	return 0;
...
static void CreateUnderscoreSprites( void )
{
	...
	for( i = 0; i < gNamingScreenData->template->maxChars; i++, xPos += 8 )
	{
		spriteId = CreateSprite(
			&sSpriteTemplate_Underscore, xPos + 3, 0x3C, 0 );
		gSprites[spriteId].oam.priority = 3;
...
static void NamingScreen_CreatePlayerIcon( void )
{
	...
	rivalGfxId = GetRivalAvatarGraphicsIdByStateIdAndGender(
		0, gNamingScreenData->monSpecies );
	spriteId = AddPseudoEventObject(
		rivalGfxId, SpriteCallbackDummy, 0x38, 0x25, 0 );
	gSprites[spriteId].oam.priority = 3;
...
static void NamingScreen_CreateMonIcon( void )
{
	...
	LoadMonIconPalettes( );
	spriteId = CreateMonIcon( gNamingScreenData->monSpecies,
		SpriteCallbackDummy,
		0x38,
		0x28,
		0,
		gNamingScreenData->monPersonality,
		1 );
	gSprites[spriteId].oam.priority = 3;
}
...
static void NamingScreen_CreateWandaDadIcon( void )
{
	...
	spriteId = AddPseudoEventObject(
		EVENT_OBJ_GFX_MAN_1, SpriteCallbackDummy, 0x38, 0x25, 0 );
	gSprites[spriteId].oam.priority = 3;
	StartSpriteAnim( &gSprites[spriteId], 4 );
}
...
static void InputInit( void ) { CreateTask( Task_HandleInput, 1 ); }
...
static void InputState_2( struct Task* task ) { task->tKeyboardEvent = 0; }
...
static void sub_80E4894( void )
{
	FillWindowPixelBuffer(
		gNamingScreenData->windows[3], PIXEL_FILL( 1 ) );
	AddTextPrinterParameterized( gNamingScreenData->windows[3],
		1,
		gNamingScreenData->template->title,
		8,
		1,
		0,
		0 );
...
static void sub_80E48E8( void )
{
	...
	AddTextPrinterParameterized(
		gNamingScreenData->windows[3], 1, buffer, 8, 1, 0, 0 );
	PutWindowTilemap( gNamingScreenData->windows[3] );
}
...
static const u8 sGenderColors[2][3] = {{0, 9, 8}, {0, 5, 4}};
...
static void sub_80E49BC( void )
{
	...
	if( gNamingScreenData->monGender != MON_GENDERLESS )
	{
		...
		AddTextPrinterParameterized3( gNamingScreenData->windows[2],
			1,
			0x68,
			1,
			sGenderColors[isFemale],
			-1,
			genderSymbol );
	}
}
...
static bool8 sub_80E4B54( void )
{
	...
	if( GetPreviousTextCaretPosition( ) !=
		gNamingScreenData->template->maxChars - 1 )
		return FALSE;
	else
		return TRUE;
}
...
static void sub_80E4BE4( void )
{
	...
	for( i = 0; i < gNamingScreenData->template->maxChars; i++ )
	{
		if( gNamingScreenData->textBuffer[i] != CHAR_SPACE &&
			gNamingScreenData->textBuffer[i] != EOS )
		{
			StringCopyN( gNamingScreenData->destBuffer,
				gNamingScreenData->textBuffer,
				gNamingScreenData->template->maxChars + 1 );
			break;
		}
	}
...
static void sub_80E4D10( void )
{
	...
	FillWindowPixelBuffer(
		gNamingScreenData->windows[2], PIXEL_FILL( 1 ) );
	...
	for( i = 0; i < maxChars; i++ )
	{
		...
		AddTextPrinterParameterized( gNamingScreenData->windows[2],
			1,
			temp,
			i * 8 + unk + unk2,
			1,
...
static const u8 sFillValues[3] = {
	PIXEL_FILL( 0xE ), PIXEL_FILL( 0xD ), PIXEL_FILL( 0xF )};

static const u8* const sUnkColors[3] = {sUnkColorStruct.colors[1],
...
static const u8* const gUnknown_0858BF98[] = {
	gUnknown_08DD4620, gUnknown_08DD46E0, gUnknown_08DD47A0};
...
static void sub_80E4EF0( void )
{
	...
	FillWindowPixelBuffer(
		gNamingScreenData->windows[4], PIXEL_FILL( 15 ) );
	AddTextPrinterParameterized3( gNamingScreenData->windows[4],
		0,
		2,
		1,
		color,
...
static void VBlankCB_NamingScreen( void )
{
	...
	SetGpuReg(
		REG_OFFSET_BG1CNT, GetGpuReg( REG_OFFSET_BG1CNT ) & 0xFFFC );
	SetGpuRegBits( REG_OFFSET_BG1CNT, gNamingScreenData->bg1Priority );
	SetGpuReg(
		REG_OFFSET_BG2CNT, GetGpuReg( REG_OFFSET_BG2CNT ) & 0xFFFC );
...
static const struct Subsprite gUnknown_0858C004[] = {{-20, -16, 1, 1, 0, 1},
	{12, -16, 0, 0, 4, 1},
	{-20, -8, 1, 1, 5, 1},
	{12, -8, 0, 0, 9, 1},
	{-20, 0, 1, 1, 10, 1},
...
static const struct Subsprite gUnknown_0858C024[] = {
	{-12, -4, 1, 0, 0, 1}, {4, -4, 0, 0, 2, 1}};

static const struct Subsprite gUnknown_0858C02C[] = {{-20, -12, 1, 1, 0, 1},
	{12, -12, 0, 0, 4, 1},
	{-20, -4, 1, 1, 5, 1},
	{12, -4, 0, 0, 9, 1},
	{-20, 4, 1, 1, 10, 1},
...
static const struct Subsprite gUnknown_0858C044[] = {
	{-8, -12, 1, 0, 0, 3}, {-8, -4, 1, 0, 2, 3}, {-8, 4, 1, 0, 4, 3}};
...
static const union AnimCmd* const gSpriteAnimTable_858C0B4[] = {
	gSpriteAnim_858C090, gSpriteAnim_858C098};

static const union AnimCmd* const gSpriteAnimTable_858C0BC[] = {
	gSpriteAnim_858C0A4};

static const struct SpriteTemplate gUnknown_0858C0C0 = {.tileTag = 0x0002,
	.paletteTag                                              = 0x0004,
...
static const struct SpriteTemplate gUnknown_0858C0D8 = {.tileTag = 0x0003,
	.paletteTag                                              = 0x0001,
...
static const struct SpriteTemplate gUnknown_0858C0F0 = {.tileTag = 0x0004,
	.paletteTag                                              = 0x0004,
	.oam         = &gOamData_858BFEC,
	.anims       = gSpriteAnimTable_858C0B0,
	.images      = NULL,
...
static const struct SpriteTemplate gUnknown_0858C108 = {.tileTag = 0x0000,
	.paletteTag                                              = 0x0006,
	.oam         = &gOamData_858BFEC,
	.anims       = gSpriteAnimTable_858C0B0,
	.images      = NULL,
...
static const struct SpriteTemplate gUnknown_0858C120 = {.tileTag = 0x0001,
	.paletteTag                                              = 0x0007,
	.oam         = &gOamData_858BFEC,
	.anims       = gSpriteAnimTable_858C0B0,
	.images      = NULL,
...
static const struct SpriteTemplate gUnknown_0858C138 = {.tileTag = 0x0007,
	.paletteTag                                              = 0x0005,
...
static const struct SpriteTemplate gUnknown_0858C180 = {.tileTag = 0xFFFF,
	.paletteTag                                              = 0x0000,
	.oam         = &gOamData_858BFEC,
