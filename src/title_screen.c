...
static const u16 sUnusedUnknownPal[] =
	INCBIN_U16( "graphics/title_screen/unk_853EF78.gbapal" );
...
static void SpriteCB_VersionBannerLeft( struct Sprite* sprite )
{
	...
	else
	{
		...
		SetGpuReg( REG_OFFSET_BLDALPHA,
			gIntroWaterDropAlphaBlend[sprite->data[0]] );
	}
}
...
static void CreatePressStartBanner( s16 x, s16 y )
{
	...
	for( i = 0; i < 5; i++, x += 32 )
	{
		spriteId = CreateSprite(
			&sStartCopyrightBannerSpriteTemplate, x, y, 0 );
...
static void SpriteCB_PokemonLogoShine( struct Sprite* sprite )
{
	if( sprite->pos1.x < 272 )
	{
		if( sprite->data[0] ) // Flash background
		{
			...
			backgroundColor = _RGB( sprite->data[1],
				sprite->data[1],
				sprite->data[1] );
...
static void StartPokemonLogoShine( u8 flashBg )
{
	...
	case 2:
		spriteId = CreateSprite(
			&sPokemonLogoShineSpriteTemplate, 0, 68, 0 );
	...
	case 1:
		...
		spriteId = CreateSprite(
			&sPokemonLogoShineSpriteTemplate, -80, 68, 0 );
		gSprites[spriteId].callback    = SpriteCB_PokemonLogoShine2;
		gSprites[spriteId].oam.objMode = 2;
		break;
	}
...
void CB2_InitTitleScreen( void )
{
	...
	case 1:
		...
		LZ77UnCompVram(
			gUnknown_08DE0644, (void*)( BG_SCREEN_ADDR( 9 ) ) );
		LoadPalette( gTitleScreenBgPalettes, 0, 0x1E0 );
		LZ77UnCompVram( sTitleScreenRayquazaGfx,
			(void*)( BG_CHAR_ADDR( 2 ) ) );
		LZ77UnCompVram( sTitleScreenRayquazaTilemap,
			(void*)( BG_SCREEN_ADDR( 26 ) ) );
		LZ77UnCompVram(
			sTitleScreenCloudsGfx, (void*)( BG_CHAR_ADDR( 3 ) ) );
		LZ77UnCompVram(
			gUnknown_08DDE458, (void*)( BG_SCREEN_ADDR( 27 ) ) );
		ScanlineEffect_Stop( );
		ResetTasks( );
		ResetSpriteData( );
	...
	case 4:
		...
		SetGpuReg( REG_OFFSET_BG0CNT,
			BGCNT_PRIORITY( 3 ) | BGCNT_CHARBASE( 2 ) |
				BGCNT_SCREENBASE( 26 ) | BGCNT_16COLOR |
				BGCNT_TXT256x256 );
		SetGpuReg( REG_OFFSET_BG1CNT,
			BGCNT_PRIORITY( 2 ) | BGCNT_CHARBASE( 3 ) |
...
static void Task_TitleScreenPhase1( u8 taskId )
{
	...
	if( ( gMain.newKeys & A_B_START_SELECT ) ||
	...
	else
	{
		...
		SetGpuReg( REG_OFFSET_BLDCNT,
			BLDCNT_TGT1_OBJ | BLDCNT_EFFECT_BLEND |
				BLDCNT_TGT2_ALL );
...
static void Task_TitleScreenPhase2( u8 taskId )
{
	...
	else
	{
		...
		SetGpuReg( REG_OFFSET_BLDY, 0 );
		SetGpuReg( REG_OFFSET_DISPCNT,
			DISPCNT_MODE_1 | DISPCNT_OBJ_1D_MAP | DISPCNT_BG0_ON |
...
static void Task_TitleScreenPhase3( u8 taskId )
{
	...
	else if( ( gMain.heldKeys & CLEAR_SAVE_BUTTON_COMBO ) ==
		CLEAR_SAVE_BUTTON_COMBO )
	{
		SetMainCallback2( CB2_GoToClearSaveDataScreen );
	}
	else if( ( gMain.heldKeys & RESET_RTC_BUTTON_COMBO ) ==
			RESET_RTC_BUTTON_COMBO &&
		CanResetRTC( ) == TRUE )
	{
		FadeOutBGM( 4 );
	...
	else if( ( gMain.heldKeys & BERRY_UPDATE_BUTTON_COMBO ) ==
		BERRY_UPDATE_BUTTON_COMBO )
	{
		FadeOutBGM( 4 );
		BeginNormalPaletteFade( 0xFFFFFFFF, 0, 0, 0x10, RGB_BLACK );
