...
static void sub_8114CFC( u8 taskId )
{
	...
	case 0:
		...
		task->data[11] =
			GetBattlerSpriteBGPriorityRank( gBattleAnimAttacker );
		if( task->data[11] == 1 )
		{
	...
	case 2:
		...
		task->data[5] =
			task->data[3] + ( gSineTable[task->data[2]] >> 4 );
		if( task->data[11] == 1 )
		...
		if( task->data[5] > 63 )
		{
			...
			gSprites[task->data[10]].pos2.x =
				272 - gSprites[task->data[10]].pos1.x;
			task->data[0]++;
		}
		break;
	case 3:
...
static void sub_8114F54( u8 taskId )
{
	...
	case 0:
		...
		gSprites[task->data[10]].pos2.y =
			160 - gSprites[task->data[10]].pos1.y;
		task->data[0]++;
		break;
	case 1:
		DestroyAnimVisualTask( taskId );
...
void AnimDigDirtMound( struct Sprite* sprite )
{
	...
	sprite->pos1.x = GetBattlerSpriteCoord( battler, 0 ) - 16 +
...
{
	struct Task* newTask;
	u8 battler = ( gBattleAnimArgs[0] & 1 ) ? gBattleAnimTarget
						: gBattleAnimAttacker;
	...
	newTask->data[1] =
		( 32 - GetBattlerSpriteCoord( battler, 2 ) ) & 0x1FF;
