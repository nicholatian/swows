...
void sub_81D0E84( struct Pokenav10Struct2* structPtr )
{
	...
	ConvertIntToDecimalStringN(
		gStringVar1, sub_81D07D8( ), STR_CONV_MODE_LEFT_ALIGN, 2 );
	DynamicPlaceholderTextUtil_Reset( );
	DynamicPlaceholderTextUtil_SetPlaceholderPtr( 0, gStringVar1 );
	DynamicPlaceholderTextUtil_ExpandPlaceholders(
		gStringVar4, gText_RibbonsF700 );
	FillWindowPixelBuffer( structPtr->field_A, PIXEL_FILL( 4 ) );
	AddTextPrinterParameterized3(
		structPtr->field_A, 1, 0, 1, color, -1, gStringVar4 );
...
void sub_81D0FF0( struct Pokenav10Struct2* structPtr )
{
	...
	AddTextPrinterParameterized(
		windowId, 1, gStringVar3, 0, 1, TEXT_SPEED_FF, NULL );
	switch( gender )
	{
	case MON_MALE:
	...
	*( txtPtr++ ) = 5;
	ConvertIntToDecimalStringN(
		txtPtr, level, STR_CONV_MODE_LEFT_ALIGN, 3 );
	AddTextPrinterParameterized(
		windowId, 1, gStringVar1, 60, 1, TEXT_SPEED_FF, NULL );
...
void sub_81D10D0( struct Pokenav10Struct2* structPtr )
{
	...
	txtPtr = ConvertIntToDecimalStringN(
		gStringVar1, id, STR_CONV_MODE_RIGHT_ALIGN, 3 );
	*( txtPtr++ ) = CHAR_SLASH;
	ConvertIntToDecimalStringN(
		txtPtr, count, STR_CONV_MODE_RIGHT_ALIGN, 3 );
	x = GetStringCenterAlignXOffset( 1, gStringVar1, 56 );
	AddTextPrinterParameterized( structPtr->field_C,
		1,
		gStringVar1,
		x,
		1,
...
u16 sub_81D1184( s32 unused0, s32 unused1 )
{
	...
	spriteId = CreateMonPicSprite_HandleDeoxys(
		species, otId, personality, TRUE, 40, 104, 15, 0xFFFF );
...
{
	return ( gSprites[structPtr->field_10].callback !=
		SpriteCallbackDummy );
}
...
void sub_81D12D8( void )
{
	...
	for( gUnknown_030012C4 = 0; gUnknown_030012C4 < gUnknown_030012C0;
		gUnknown_030012C4++ )
...
void sub_81D1448( struct Pokenav10Struct* structPtr )
{
	...
	structPtr->field_14->oam.tileNum =
		( gUnknown_08624BF8[ribbonId].var0 * 16 ) +
		GetSpriteTileStartByTag( 9 );
	structPtr->field_14->oam.paletteNum = IndexOfSpritePaletteTag(
		gUnknown_08624BF8[ribbonId].var2 + 15 );
