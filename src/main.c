...
#define INTR_COUNT \
	( (int)( sizeof( gIntrTableTemplate ) / sizeof( IntrFunc ) ) )
	...
	*(vu16*)BG_PLTT = 0x7FFF;
	...
	for( ;; )
	{
		...
		if( gSoftResetDisabled == FALSE &&
			...
			( gMain.heldKeysRaw & B_START_SELECT ) ==
				B_START_SELECT )
		{
			rfu_REQ_stopMode( );
			rfu_waitREQComplete( );
...
{
	u16 gpuReg =
		( GetGpuReg( REG_OFFSET_DISPSTAT ) & 0xFF ) | ( 150 << 8 );
...
static void ReadKeys( void )
{
	...
	{
		// If there is no input or the input has changed, reset the
		// counter.
...
static void VBlankIntr( void )
{
	...
	if( gTrainerHillVBlankCounter &&
		*gTrainerHillVBlankCounter < 0xFFFFFFFF )
