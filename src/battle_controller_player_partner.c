...
static void sub_81BB02C( void )
{
	if( !gBattleSpritesDataPtr->healthBoxesData[gActiveBattler]
		...
		!gBattleSpritesDataPtr
			 ...
			 .ballAnimActive &&
		gSprites[gUnknown_03005D7C[gActiveBattler]].callback ==
			SpriteCallbackDummy &&
...
static void Task_GiveExpToMon( u8 taskId )
{
	...
	if( IsDoubleBattle( ) == TRUE ||
		monId != gBattlerPartyIndexes[battlerId] ) // give exp without
							   // the expbar
	{
...
static void sub_81BB688( u8 taskId )
{
			...
			.specialAnimActive )
	{
		...
		GetMonData( &gPlayerParty[monIndex],
			MON_DATA_LEVEL ); // Unused
					  // return value
...
static void sub_81BB9F4( void )
{
	if( gBattleSpritesDataPtr->healthBoxesData[gActiveBattler].field_1_x1 )
	{
		...
		HandleLowHpMusicChange(
			...
			gActiveBattler );
		StartSpriteAnim(
			&gSprites[gBattlerSpriteIds[gActiveBattler]], 0 );
		UpdateHealthboxAttribute( gHealthboxSpriteIds[gActiveBattler],
			&gPlayerParty[gBattlerPartyIndexes[gActiveBattler]],
			HEALTHBOX_ALL );
		sub_8076918( gActiveBattler );
...
static void PlayerPartnerHandleLoadMonSprite( void )
{
	...
	SetMultiuseSpriteTemplateToPokemon(
		species, GetBattlerPosition( gActiveBattler ) );
...
static void PlayerPartnerHandleDrawTrainerPic( void )
{
	...
	{
		trainerPicId =
			GetFrontierTrainerFrontSpriteId( gPartnerTrainerId );
		xPos = 32;
		yPos = ( 8 - gTrainerFrontPicCoords[trainerPicId].size ) * 4 +
			80;
	}
...
static void PlayerPartnerHandleChoosePokemon( void )
{
	...
	if( chosenMonId == 6 ) // just switch to the next mon
	{
		...
		{
			if( GetMonData( &gPlayerParty[chosenMonId],
				    MON_DATA_HP ) != 0 &&
				chosenMonId !=
					gBattlerPartyIndexes
						[playerMonIdentity] &&
				chosenMonId !=
					gBattlerPartyIndexes[selfIdentity] )
			{
				break;
			}
		}
...
static void PlayerPartnerHandleIntroTrainerBallThrow( void )
{
	...
	if( gPartnerTrainerId == TRAINER_STEVEN_PARTNER )
	{
		...
		LoadCompressedPalette(
			gTrainerBackPicPaletteTable[spriteId].data,
			0x100 + paletteNum * 16,
			32 );
	}
	else
	{
		u8 spriteId =
			GetFrontierTrainerFrontSpriteId( gPartnerTrainerId );
		LoadCompressedPalette(
			gTrainerFrontPicPaletteTable[spriteId].data,
			0x100 + paletteNum * 16,
			32 );
	}
