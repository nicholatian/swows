...
// These belong in battle_intro.c, but there putting them there causes 2 bytes
// of alignment padding between the two .rodata segments. Perhaps
// battle_intro.c actually belongs in this file, too.
const u8 gUnknown_0859741A[] = {REG_OFFSET_BG0CNT,
	REG_OFFSET_BG1CNT,
	REG_OFFSET_BG2CNT,
	REG_OFFSET_BG3CNT};
const u8 gUnknown_0859741E[] = {REG_OFFSET_BG0CNT,
	REG_OFFSET_BG1CNT,
	REG_OFFSET_BG2CNT,
	REG_OFFSET_BG3CNT};
...
void sub_8116620( u8 taskId )
{
	u32 selectedPalettes =
		UnpackSelectedBattleAnimPalettes( gBattleAnimArgs[0] );
...
static void AnimTask_BlendSpriteColor_Step2( u8 taskId )
{
	...
	if( gTasks[taskId].data[9] == gTasks[taskId].data[2] )
	{
		...
		selectedPalettes       = gTasks[taskId].data[0] |
			( gTasks[taskId].data[1] << 16 );
		while( selectedPalettes != 0 )
		{
			if( selectedPalettes & 1 )
				BlendPalette( singlePaletteMask,
					16,
...
static void sub_81169F8( u8 taskId )
{
	...
	if( task->data[4] )
	{
		...
		{
			task->data[6] =
				CloneBattlerSpriteWithBlend( task->data[0] );
			if( task->data[6] >= 0 )
			{
				gSprites[task->data[6]].oam.priority =
					task->data[0] ? 1 : 2;
				gSprites[task->data[6]].data[0] =
					task->data[3];
...
void sub_8116B14( u8 taskId )
{
	...
	{
		if( GetBattlerPosition( gBattleAnimAttacker ) ==
				B_POSITION_OPPONENT_RIGHT ||
			GetBattlerPosition( gBattleAnimAttacker ) ==
				B_POSITION_PLAYER_LEFT )
		{
			if( IsBattlerSpriteVisible( BATTLE_PARTNER(
				    gBattleAnimAttacker ) ) == TRUE )
			{
...
static void sub_8116D64( u8 taskId )
{
	...
	if( gTasks[taskId].data[10] == 64 )
	{
		...
		if( ++gTasks[taskId].data[11] == 4 )
		{
			...
			sprite = &gSprites[GetAnimBattlerSpriteId(
				0 )]; // unused
...
void sub_8116EB4( u8 taskId )
{
	...
	sAnimStatsChangeData =
		AllocZeroed( sizeof( struct AnimStatsChangeData ) );
	for( i = 0; i < 8; i++ )
...
static void sub_8116F04( u8 taskId )
{
	...
	if( IsDoubleBattle( ) && sAnimStatsChangeData->data[3] == 0 )
	{
				...
				B_POSITION_PLAYER_LEFT )
		{
			if( IsBattlerSpriteVisible(
				    sAnimStatsChangeData->battler2 ) == TRUE )
			{
				gSprites[gBattlerSpriteIds[sAnimStatsChangeData
								   ->battler2]]
					.oam.priority -= 1;
	...
	{
		if( GetBattlerSide( sAnimStatsChangeData->battler1 ) !=
			B_SIDE_PLAYER )
...
static void sub_81170EC( u8 taskId )
{
	...
	spriteId        = sub_80A89C8( sAnimStatsChangeData->battler1,
                battlerSpriteId,
                sAnimStatsChangeData->species );
	if( sAnimStatsChangeData->data[3] )
	{
		...
		spriteId2 = sub_80A89C8( sAnimStatsChangeData->battler2,
			battlerSpriteId,
			sAnimStatsChangeData->species );
	}
	...
	{
	case 0:
		LoadCompressedPalette( gBattleStatMask2_Pal,
			unknownStruct.paletteId * 16,
			32 );
		break;
	case 1:
		LoadCompressedPalette( gBattleStatMask1_Pal,
			unknownStruct.paletteId * 16,
			32 );
		break;
	case 2:
		LoadCompressedPalette( gBattleStatMask3_Pal,
			unknownStruct.paletteId * 16,
			32 );
		break;
	case 3:
		LoadCompressedPalette( gBattleStatMask4_Pal,
			unknownStruct.paletteId * 16,
			32 );
		break;
	case 4:
		LoadCompressedPalette( gBattleStatMask6_Pal,
			unknownStruct.paletteId * 16,
			32 );
		break;
	case 5:
		LoadCompressedPalette( gBattleStatMask7_Pal,
			unknownStruct.paletteId * 16,
			32 );
		break;
	case 6:
		LoadCompressedPalette( gBattleStatMask8_Pal,
			unknownStruct.paletteId * 16,
			32 );
		break;
	default:
		LoadCompressedPalette( gBattleStatMask5_Pal,
			unknownStruct.paletteId * 16,
			32 );
		break;
	}
	...
	if( sAnimStatsChangeData->data[0] == 0 )
		PlaySE12WithPanning(
			SE_W287, BattleAnimAdjustPanning2( -64 ) );
	else
		PlaySE12WithPanning(
			SE_W287B, BattleAnimAdjustPanning2( -64 ) );
}
	...
	const u32* palette )
{
	...
	else
	{
		if( GetBattlerSide( battler1 ) != B_SIDE_PLAYER )
			species = GetMonData(
				&gEnemyParty[gBattlerPartyIndexes[battler1]],
				MON_DATA_SPECIES );
		else
			species = GetMonData(
				&gPlayerParty[gBattlerPartyIndexes[battler1]],
				MON_DATA_SPECIES );
	}

	spriteId =
		sub_80A89C8( battler1, gBattlerSpriteIds[battler1], species );
	if( arg4 )
		spriteId2 = sub_80A89C8(
			battler2, gBattlerSpriteIds[battler2], species );

	sub_80A6B30( &unknownStruct );
	sub_80A6D60( &unknownStruct, tilemap, 0 );
	AnimLoadCompressedBgGfx(
		unknownStruct.bgId, gfx, unknownStruct.tilesOffset );
...
void sub_8117CA0( u8 taskId )
{
	...
	memcpy( &gMonSpritesGfxPtr->field_17C[gBattleAnimArgs[1] * 16],
...
void sub_8117D3C( u8 taskId )
{
	...
	memcpy( &gPlttBufferUnfaded[paletteIndex * 16],
		&gMonSpritesGfxPtr->field_17C[gBattleAnimArgs[1] * 16],
		32 );
	DestroyAnimVisualTask( taskId );
}
...
void sub_8117DD8( u8 taskId )
{
	...
	memcpy( &gPlttBufferUnfaded[paletteIndex * 16],
		&gPlttBufferFaded[paletteIndex * 16],
		32 );
	DestroyAnimVisualTask( taskId );
}
...
static void sub_8117FD0( u8 taskId )
{
	...
	{
		gBattleSpritesDataPtr->battlerData[gBattleAnimAttacker]
			.invisible = (u8)gTasks[taskId].data[0] & 1;
		DestroyTask( taskId );
	}
}
