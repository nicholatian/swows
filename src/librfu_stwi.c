...
void STWI_init_all( struct RfuIntrStruct* interruptStruct,
	IntrFunc* interrupt,
...
void STWI_set_MS_mode( u8 mode ) { gRfuState->msMode = mode; }
...
void STWI_init_Callback_M( void ) { STWI_set_Callback_M( 0 ); }

void STWI_init_Callback_S( void ) { STWI_set_Callback_S( 0 ); }

void STWI_set_Callback_M( void* callback ) { gRfuState->callbackM = callback; }

void STWI_set_Callback_S( void* callback ) { gRfuState->callbackS = callback; }

void STWI_set_Callback_ID( u32 id ) { gRfuState->callbackID = id; }
...
void STWI_send_DataTxREQ( void* in, u8 size )
{
	if( !STWI_init( RFU_DATA_TX ) )
	{
		...
		CpuCopy32( in,
			gRfuState->txPacket->rfuPacket32.data,
			gRfuState->txParams * sizeof( u32 ) );
		STWI_start_Command( );
	}
}
...
void STWI_send_TestModeREQ( u8 unk0, u8 unk1 )
{
	...
	{
		gRfuState->txParams = 1;
		gRfuState->txPacket->rfuPacket32.data[0] =
			unk0 | ( unk1 << 8 );
...
u16 STWI_init( u8 request )
{
	...
	else if( !gRfuState->msMode )
	{
		...
		if( gRfuState->callbackM )
			gRfuState->callbackM(
				request, gRfuState->unk_12, gRfuState );
		return TRUE;
	}
	else
	{
...
int STWI_start_Command( )
{
	...
	*(u32*)gRfuState->txPacket->rfuPacket8.data = 0x99660000 |
		( gRfuState->txParams << 8 ) | gRfuState->activeCommand;
	...
	REG_SIOCNT = SIO_INTR_ENABLE | SIO_32BIT_MODE | SIO_MULTI_BUSY |
		SIO_115200_BPS;
...
int STWI_restart_Command( void )
{
	...
	{
		if( gRfuState->activeCommand == RFU_MS_CHANGE ||
			gRfuState->activeCommand == RFU_DATA_TX_AND_CHANGE ||
			gRfuState->activeCommand == RFU_UNK35 ||
			gRfuState->activeCommand ==
				RFU_RESUME_RETRANSMIT_AND_CHANGE )
		{
			...
			if( gRfuState->callbackM )
				gRfuState->callbackM( gRfuState->activeCommand,
					gRfuState->unk_12 );
		}
		else
		{
...
int STWI_reset_ClockCounter( void )
{
	...
	REG_SIOCNT = SIO_INTR_ENABLE | SIO_32BIT_MODE | SIO_115200_BPS;
	REG_SIOCNT =
		( SIO_INTR_ENABLE | SIO_32BIT_MODE | SIO_115200_BPS ) + 0x7F;
