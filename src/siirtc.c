...
u8 SiiRtcProbe( void )
{
	...
	if( ( rtc.status & ( SIIRTCINFO_POWER | SIIRTCINFO_24HOUR ) ) ==
		...
		( rtc.status & ( SIIRTCINFO_POWER | SIIRTCINFO_24HOUR ) ) ==
			0 )
	{
		// The RTC is in 12-hour mode. Reset it and switch to 24-hour
		// mode.

		// Note that the conditions are redundant and equivalent to
		// simply
		// "(rtc.status & SIIRTCINFO_24HOUR) == 0". It's possible that
		// this was also intended to handle resetting the clock after
		// power failure but a mistake was made.
...
bool8 SiiRtcSetAlarm( struct SiiRtcInfo* rtc )
{
	...
	GPIOPortDirection = DIR_ALL_OUT; // Why is this the only instance that
					 // uses a symbol?
...
static u8 ReadData( )
{
	...
	for( i = 0; i < 8; i++ )
	{
		...
		value = ( value >> 1 ) | ( temp << 7 ); // UB: accessing
							// uninitialized var
	}
...
static void EnableGpioPortRead( ) { GPIO_PORT_READ_ENABLE = 1; }

static void DisableGpioPortRead( ) { GPIO_PORT_READ_ENABLE = 0; }
