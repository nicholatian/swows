...
// CONST
const u16 gCloudsWeatherPalette[] =
	INCBIN_U16( "graphics/weather/cloud.gbapal" );
...
const struct SpritePalette sSpritePalette_Weather0 = {
	gUnknown_083970E8, 0x1201};
const struct SpritePalette sCloudsSpritePalette = {
	gCloudsWeatherPalette, 0x1201};
const struct SpritePalette sSandstormSpritePalette = {
	gSandstormWeatherPalette, 0x1201};
...
void Drought_Main( void )
{
	...
	{
	case 0:
		if( gWeatherPtr->palProcessingState !=
...
// it appears and starts falling. (This is only for the initial raindrop
// spawn.)
...
static void StartRainSpriteFall( struct Sprite* sprite )
{
	...
	numFallingFrames =
		sRainSpriteFallingDurations[gWeatherPtr->isHeavyRain][0];
	...
	sprite->tPosX = tileX;
	sprite->tPosX <<= 7; // This is tileX * 8, using a fixed-point value
			     // with 4 decimal places

	sprite->tPosY = tileY;
	sprite->tPosY <<= 7; // This is tileX * 8, using a fixed-point value
			     // with 4 decimal places
...
static void UpdateRainSprite( struct Sprite* sprite )
{
	if( sprite->tState == 0 )
	{
		...
		sprite->tPosX +=
			sRainSpriteMovement[gWeatherPtr->isHeavyRain][0];
		sprite->tPosY +=
			sRainSpriteMovement[gWeatherPtr->isHeavyRain][1];
		...
		if( --sprite->tCounter == 0 )
		{
			...
			StartSpriteAnim(
				sprite, gWeatherPtr->isHeavyRain + 1 );
			sprite->tState = 1;
...
static void InitRainSpriteMovement( struct Sprite* sprite, u16 val )
{
	u16 numFallingFrames =
		sRainSpriteFallingDurations[gWeatherPtr->isHeavyRain][0];
	u16 numAdvanceRng = val /
		( sRainSpriteFallingDurations[gWeatherPtr->isHeavyRain][1] +
			numFallingFrames );
	u16 frameVal = val %
		( sRainSpriteFallingDurations[gWeatherPtr->isHeavyRain][1] +
			numFallingFrames );
...
static bool8 CreateRainSprite( void )
{
	...
	if( spriteId != MAX_SPRITES )
	{
		...
		gWeatherPtr->sprites.s1.rainSprites[spriteIndex] =
			&gSprites[spriteId];
	}
	else
	{
	...
	if( ++gWeatherPtr->rainSpriteCount == MAX_RAIN_SPRITES )
	{
		...
		for( i = 0; i < MAX_RAIN_SPRITES; i++ )
		{
			if( gWeatherPtr->sprites.s1.rainSprites[i] )
			{
				if( !gWeatherPtr->sprites.s1.rainSprites[i]
						->tWaiting )
					gWeatherPtr->sprites.s1.rainSprites[i]
						->callback = UpdateRainSprite;
				else
					gWeatherPtr->sprites.s1.rainSprites[i]
						->callback = WaitRainSprite;
			}
		}
...
static bool8 UpdateVisibleRainSprites( void )
{
	if( gWeatherPtr->curRainSpriteIndex ==
		gWeatherPtr->targetRainSpriteCount )
		return FALSE;
		...
		gWeatherPtr->rainSpriteVisibleDelay )
	{
			...
			gWeatherPtr->targetRainSpriteCount )
		{
			gWeatherPtr->sprites.s1
				.rainSprites[gWeatherPtr->curRainSpriteIndex++]
				->tActive = 1;
		}
		else
		{
	...
	while( gWeatherPtr->weatherGfxLoaded == FALSE )
	{
		...
		for( i = 0; i < gWeatherPtr->snowflakeSpriteCount; i++ )
			UpdateSnowflakeSprite(
				gWeatherPtr->sprites.s1.snowflakeSprites[i] );
	}
}
...
static bool8 DestroySnowflakeSprite( void )
{
	if( gWeatherPtr->snowflakeSpriteCount )
	{
		DestroySprite( gWeatherPtr->sprites.s1.snowflakeSprites
...
static void InitSnowflakeSpriteMovement( struct Sprite* sprite )
{
	...
	sprite->pos1.y =
		-3 - ( gSpriteCoordOffsetY + sprite->centerToCornerVecY );
	sprite->pos1.x =
		x - ( gSpriteCoordOffsetX + sprite->centerToCornerVecX );
...
static void WaitSnowflakeSprite( struct Sprite* sprite )
{
	if( gWeatherPtr->unknown_6E2 > 18 )
	{
		...
		sprite->pos1.y    = 250 -
			( gSpriteCoordOffsetY + sprite->centerToCornerVecY );
		sprite->tPosY            = sprite->pos1.y * 128;
...
static void UpdateSnowflakeSprite( struct Sprite* sprite )
{
	...
	x = ( sprite->pos1.x + sprite->centerToCornerVecX +
		    gSpriteCoordOffsetX ) &
		0x1FF;
	if( x & 0x100 )
		x |= -0x100;
	...
	if( x < -3 )
		sprite->pos1.x = 242 -
			( gSpriteCoordOffsetX + sprite->centerToCornerVecX );
	else if( x > 242 )
		sprite->pos1.x = -3 -
			( gSpriteCoordOffsetX + sprite->centerToCornerVecX );

	y = ( sprite->pos1.y + sprite->centerToCornerVecY +
		    gSpriteCoordOffsetY ) &
		0xFF;
	if( y > 163 && y < 171 )
	{
...
void Rain_Main( void )
{
	...
	case 8:
		sub_80ABC48( 19 );
		if( gWeatherPtr->unknown_6EB == 0 &&
			gWeatherPtr->unknown_6EC == 1 )
...
void Fog1_Main( void )
{
	gWeatherPtr->fog1ScrollPosX =
		( gSpriteCoordOffsetX - gWeatherPtr->fog1ScrollOffset ) & 0xFF;
...
static void Fog1SpriteCallback( struct Sprite* sprite )
{
	...
	sprite->pos1.x =
		gWeatherPtr->fog1ScrollPosX + 32 + sprite->tSpriteColumn * 64;
	if( sprite->pos1.x > 271 )
	{
		sprite->pos1.x = 480 + gWeatherPtr->fog1ScrollPosX -
	...
	if( !gWeatherPtr->fog1SpritesCreated )
	{
		...
		for( i = 0; i < NUM_FOG1_SPRITES; i++ )
		{
			...
			if( spriteId != MAX_SPRITES )
			{
				...
				gWeatherPtr->sprites.s2.fog1Sprites[i] =
					sprite;
				sprite->oam.paletteNum =
					gWeatherPtr->altGammaSpritePalIndex;
			}
			else
			{
	...
	if( gWeatherPtr->fog1SpritesCreated )
	{
		for( i = 0; i < NUM_FOG1_SPRITES; i++ )
		{
			if( gWeatherPtr->sprites.s2.fog1Sprites[i] != NULL )
				DestroySprite( gWeatherPtr->sprites.s2
						       .fog1Sprites[i] );
		}
...
void Ash_InitVars( void )
{
	...
	if( !gWeatherPtr->ashSpritesCreated )
	{
		...
		SetGpuReg( REG_OFFSET_BLDALPHA,
			BLDALPHA_BLEND( 64, 63 ) ); // These aren't valid blend
						    // coefficients!
	}
}
...
static void UpdateAshSprite( struct Sprite* sprite )
{
	...
	sprite->pos1.x =
		gWeatherPtr->ashBaseSpritesX + 32 + sprite->tSpriteColumn * 64;
	if( sprite->pos1.x > 271 )
	{
		sprite->pos1.x = gWeatherPtr->ashBaseSpritesX + 480 -
			( 4 - sprite->tSpriteColumn ) * 64;
		sprite->pos1.x &= 0x1FF;
	}
}
...
static void UpdateFog2Movement( void )
{
	...
	gWeatherPtr->fog2BaseSpritesX =
		( gSpriteCoordOffsetX - gWeatherPtr->fog2XOffset ) & 0xFF;
...
static void CreateFog2Sprites( void )
{
	...
	if( !gWeatherPtr->fog2SpritesCreated )
	{
		...
		{
			spriteId = CreateSpriteAtEnd( &sFog2SpriteTemplate,
				0,
				( i / 5 ) * 64,
				0xFF );
			if( spriteId != MAX_SPRITES )
			{
				...
				gWeatherPtr->sprites.s2.fog2Sprites[i] =
					sprite;
			}
			else
			{
...
static void UpdateFog2Sprite( struct Sprite* sprite )
{
	...
	sprite->pos1.x = gWeatherPtr->fog2BaseSpritesX + 32 +
		sprite->tSpriteColumn * 64;
	if( sprite->pos1.x > 271 )
	{
		sprite->pos1.x = gWeatherPtr->fog2BaseSpritesX + 480 -
			( 4 - sprite->tSpriteColumn ) * 64;
		sprite->pos1.x &= 0x1FF;
	}
}
...
void Sandstorm_InitVars( void )
{
	...
	{
		gWeatherPtr->sandstormXOffset = gWeatherPtr->sandstormYOffset =
			0;
		gWeatherPtr->sandstormWaveIndex   = 8;
		gWeatherPtr->sandstormWaveCounter = 0;
		// Dead code. How does the compiler not optimize this out?
		if( gWeatherPtr->sandstormWaveIndex >=
			0x80 - MIN_SANDSTORM_WAVE_INDEX )
			gWeatherPtr->sandstormWaveIndex =
				0x80 - gWeatherPtr->sandstormWaveIndex;
...
static void UpdateSandstormMovement( void )
{
	...
	gWeatherPtr->sandstormYOffset -=
		gSineTable[gWeatherPtr->sandstormWaveIndex];
	gWeatherPtr->sandstormBaseSpritesX =
		( gSpriteCoordOffsetX +
			( gWeatherPtr->sandstormXOffset >> 8 ) ) &
		0xFF;
...
static void DestroySandstormSprites( void )
{
	...
	if( gWeatherPtr->sandstormSwirlSpritesCreated )
	{
		...
		{
			if( gWeatherPtr->sprites.s2.sandstormSprites2[i] !=
				NULL )
				DestroySprite( gWeatherPtr->sprites.s2
						       .sandstormSprites2[i] );
		}
...
static void CreateSandstormSprites( void )
{
	...
	if( !gWeatherPtr->sandstormSpritesCreated )
	{
		...
		{
			spriteId =
				CreateSpriteAtEnd( &sSandstormSpriteTemplate,
					0,
					( i / 5 ) * 64,
					1 );
			if( spriteId != MAX_SPRITES )
			{
				gWeatherPtr->sprites.s2.sandstormSprites1[i] =
					&gSprites[spriteId];
				gWeatherPtr->sprites.s2.sandstormSprites1[i]
					->tSpriteColumn = i % 5;
				gWeatherPtr->sprites.s2.sandstormSprites1[i]
		...
		for( i = 0; i < NUM_SWIRL_SANDSTORM_SPRITES; i++ )
		{
			...
			if( spriteId != MAX_SPRITES )
			{
				...
				gWeatherPtr->sprites.s2.sandstormSprites2[i]
					->tRadiusCounter = 0;
				gWeatherPtr->sprites.s2.sandstormSprites2[i]
					->data[4] = 0x6730; // unused value
				gWeatherPtr->sprites.s2.sandstormSprites2[i]
					->tEntranceDelay =
					sSwirlEntranceDelays[i];
				StartSpriteAnim( gWeatherPtr->sprites.s2
							 .sandstormSprites2[i],
					1 );
				CalcCenterToCornerVec(
					gWeatherPtr->sprites.s2
						.sandstormSprites2[i],
					0,
					2,
					0 );
...
static void UpdateSandstormSprite( struct Sprite* sprite )
{
	...
	sprite->pos1.x = gWeatherPtr->sandstormBaseSpritesX + 32 +
		sprite->tSpriteColumn * 64;
	if( sprite->pos1.x > 271 )
	{
		sprite->pos1.x = gWeatherPtr->sandstormBaseSpritesX + 480 -
			( 4 - sprite->tSpriteColumn ) * 64;
		sprite->pos1.x &= 0x1FF;
	}
}
...
void Bubbles_Main( void )
{
		...
		sBubbleStartDelays[gWeatherPtr->bubblesDelayIndex] )
	{
		...
		if( ++gWeatherPtr->bubblesDelayIndex >
			ARRAY_COUNT( sBubbleStartDelays ) - 1 )
			gWeatherPtr->bubblesDelayIndex = 0;

		CreateBubbleSprite( gWeatherPtr->bubblesCoordsIndex );
		if( ++gWeatherPtr->bubblesCoordsIndex >
			ARRAY_COUNT( sBubbleStartCoords ) - 1 )
			gWeatherPtr->bubblesCoordsIndex = 0;
	}
}
...
u8 GetSav1Weather( void ) { return gSaveBlock1Ptr->weather; }
...
static u8 TranslateWeatherNum( u8 weather )
{
	...
	case WEATHER_RAIN_MED:
		return WEATHER_RAIN_MED;
	case WEATHER_FOG_1:
		return WEATHER_FOG_1;
	case WEATHER_ASH:
		return WEATHER_ASH;
	case WEATHER_SANDSTORM:
		return WEATHER_SANDSTORM;
	case WEATHER_FOG_2:
		return WEATHER_FOG_2;
	case WEATHER_FOG_3:
		return WEATHER_FOG_3;
	case WEATHER_SHADE:
	...
	case WEATHER_BUBBLES:
		return WEATHER_BUBBLES;
	case WEATHER_ALTERNATING:
		return WEATHER_ALTERNATING;
	case WEATHER_ROUTE119_CYCLE:
		return sWeatherCycleRoute119[gSaveBlock1Ptr
						     ->weatherCycleStage];
	case WEATHER_ROUTE123_CYCLE:
		return sWeatherCycleRoute123[gSaveBlock1Ptr
						     ->weatherCycleStage];
	default:
		return WEATHER_NONE;
	}
...
static void UpdateRainCounter( u8 newWeather, u8 oldWeather )
{
	if( newWeather != oldWeather &&
		( newWeather == WEATHER_RAIN_LIGHT ||
			newWeather == WEATHER_RAIN_MED ) )
