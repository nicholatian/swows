...
static const u32 gUnknown_08571298[] =
	INCBIN_U32( "graphics/frontier_pass/unknown_571298.bin" );
static const u32 gUnknown_085712C0[] =
	INCBIN_U32( "graphics/frontier_pass/record_frame.bin.lz" );
static const u32 gUnknown_085712F8[] = INCBIN_U32(
...
static const union AnimCmd* const sSpriteAnimTable_8571574[] = {
	sSpriteAnim_8571524, sSpriteAnim_857152C};
...
static u32 FreeFrontierPassData( void )
{
	...
	memset( sPassData, 0, sizeof( *sPassData ) ); // Why clear data, if
...
static u32 FreeFrontierPassGfx( void )
{
	...
	memset( sPassGfx, 0, sizeof( *sPassGfx ) ); // Why clear data, if it's
						    // going to be freed
						    // anyway?
...
static bool32 InitFrontierPass( void )
{
	...
	case 4:
		...
		InitBgsFromTemplates(
			1, sPassBgTemplates, ARRAY_COUNT( sPassBgTemplates ) );
	...
	case 6:
		sPassGfx->unk20 =
			malloc_and_decompress( gUnknown_085712F8, &sizeOut );
		sPassGfx->unk24 =
			malloc_and_decompress( gUnknown_08571060, &sizeOut );
		sPassGfx->unk28 =
			malloc_and_decompress( gUnknown_085712C0, &sizeOut );
		decompress_and_copy_tile_data_to_vram(
			1, gUnknown_08DE08C8, 0, 0, 0 );
		decompress_and_copy_tile_data_to_vram(
			2, gUnknown_08DE2084, 0, 0, 0 );
		break;
	case 7:
	...
	case 8:
		...
		LoadPalette( gUnknown_08DE07C8[1 + sPassData->trainerStars],
			0x10,
			0x20 );
		LoadPalette( stdpal_get( 0 ), 0xF0, 0x20 );
		sub_80C629C( );
		sub_80C6104(
			sPassData->cursorArea, sPassData->previousCursorArea );
...
static u8 GetCursorAreaFromCoords( s16 x, s16 y )
{
	...
	// Minus/Plus 1, because the table doesn't take into account the
	// nothing field.
...
static void Task_HandleFrontierPassInput( u8 taskId )
{
	u8 var = FALSE; // Reused, first informs whether the cursor moves, then
			// used as the new cursor area.
	...
	if( gMain.heldKeys & DPAD_DOWN &&
		sPassGfx->cursorSprite->pos1.y <= 135 )
	{
		sPassGfx->cursorSprite->pos1.y += 2;
	...
	if( gMain.heldKeys & DPAD_RIGHT &&
		sPassGfx->cursorSprite->pos1.x <= 231 )
	{
		sPassGfx->cursorSprite->pos1.x += 2;
	...
	{
		if( sPassData->cursorArea != CURSOR_AREA_NOTHING &&
			gMain.newKeys & A_BUTTON )
		{
			if( sPassData->cursorArea <=
				CURSOR_AREA_RECORD ) // Map, Card,
						     // Record
			{
				PlaySE( SE_SELECT );
				if( TryCallPassAreaFunction(
					    taskId, sPassData->cursorArea ) )
					return;
			}
			else if( sPassData->cursorArea == CURSOR_AREA_CANCEL )
			{
				...
				DestroyTask( taskId );
				// BUG. The function should return here.
				// Otherwise, it can play the same sound twice
	...
	else
	{
		var = GetCursorAreaFromCoords(
			sPassGfx->cursorSprite->pos1.x - 5,
			sPassGfx->cursorSprite->pos1.y + 5 );
		if( sPassData->cursorArea != var )
		{
			...
			sub_80C6104( sPassData->cursorArea,
				sPassData->previousCursorArea );
		}
	}
}
...
static void ShowAndPrintWindows( void )
{
	...
	AddTextPrinterParameterized3( WINDOW_EARNED_SYMBOLS,
		1,
		x,
		5,
		sTextColors[0],
	...
	AddTextPrinterParameterized3( WINDOW_BATTLE_RECORD,
		1,
		x,
		5,
		sTextColors[0],
	...
	AddTextPrinterParameterized3( WINDOW_BATTLE_POINTS,
		8,
		5,
		4,
		sTextColors[0],
	...
	sPassData->cursorArea = GetCursorAreaFromCoords(
		sPassData->cursorX - 5, sPassData->cursorY + 5 );
...
static void PrintAreaDescription( u8 cursorArea )
{
	...
	if( cursorArea == CURSOR_AREA_RECORD && !sPassData->hasBattleRecord )
		AddTextPrinterParameterized3( WINDOW_DESCRIPTION,
			1,
			2,
			0,
			sTextColors[1],
...
static void sub_80C5F58( bool8 arg0, bool8 arg1 )
{
	...
	case 2:
		if( arg0 )
			CopyToBgTilemapBufferRect_ChangePalette(
				2, sPassGfx->unk20 + 84, 16, 10, 12, 7, 16 );
		else
...
static void sub_80C6104( u8 cursorArea, u8 previousCursorArea )
{
	...
	case CURSOR_AREA_MAP:
		CopyToBgTilemapBufferRect_ChangePalette(
			1, sPassGfx->unk24, 16, 3, 12, 7, 17 );
		var = TRUE;
		break;
	case CURSOR_AREA_CARD:
		CopyToBgTilemapBufferRect_ChangePalette(
			1, sPassGfx->unk24 + 336, 16, 10, 12, 7, 17 );
		var = TRUE;
		break;
	case CURSOR_AREA_RECORD:
		if( !sPassData->hasBattleRecord )
		...
		{
			CopyToBgTilemapBufferRect_ChangePalette(
				1, sPassGfx->unk28, 2, 10, 12, 3, 17 );
			var = TRUE;
		}
		break;
	case CURSOR_AREA_CANCEL:
		CopyToBgTilemapBufferRect_ChangePalette(
			1, gUnknown_08DE3350, 21, 0, 9, 2, 17 );
		var = TRUE;
		break;
	default:
		var = FALSE;
	...
	case CURSOR_AREA_MAP:
		CopyToBgTilemapBufferRect_ChangePalette(
			1, sPassGfx->unk24 + 168, 16, 3, 12, 7, 17 );
		var = TRUE;
		break;
	case CURSOR_AREA_CARD:
		CopyToBgTilemapBufferRect_ChangePalette(
			1, sPassGfx->unk24 + 504, 16, 10, 12, 7, 17 );
		var = TRUE;
		break;
	case CURSOR_AREA_RECORD:
		if( !sPassData->hasBattleRecord )
		...
		CopyToBgTilemapBufferRect_ChangePalette(
			1, sPassGfx->unk28 + 72, 2, 10, 12, 3, 17 );
		var = TRUE;
		break;
	case CURSOR_AREA_CANCEL:
		CopyToBgTilemapBufferRect_ChangePalette(
			1, gUnknown_08DE3374, 21, 0, 9, 2, 17 );
		var = TRUE;
		break;
	default:
		var = FALSE;
	...
	{
		asm( "" ::: "r4" );
		if( previousCursorArea == CURSOR_AREA_NOTHING ||
			previousCursorArea > CURSOR_AREA_CANCEL )
			return;
	}
...
static void LoadCursorAndSymbolSprites( void )
{
	...
	spriteId               = CreateSprite( &sSpriteTemplates_Cursors[0],
                sPassData->cursorX,
                sPassData->cursorY,
                0 );
	...
	for( i = 0; i < NUM_FRONTIER_FACILITIES; i++ )
	{
		if( sPassData->facilitySymbols[i] != 0 )
		{
			...
			spriteId                   = CreateSprite( &sprite,
                                sPassAreasLayout[i + CURSOR_AREA_SYMBOL - 1]
                                                .xStart +
                                        8,
                                sPassAreasLayout[i + CURSOR_AREA_SYMBOL - 1]
                                                .yStart +
...
static void ShowFrontierMap( void ( *callback )( void ) )
{
	if( sMapData != NULL )
		SetMainCallback2(
			callback ); // This line doesn't make sense at all,
				    // since it gets overwritten later anyway.
...
static bool32 InitFrontierMap( void )
{
	...
	case 3:
		ResetBgsAndClearDma3BusyFlags( 0 );
		InitBgsFromTemplates(
			0, sMapBgTemplates, ARRAY_COUNT( sMapBgTemplates ) );
...
static void Task_HandleFrontierMap( u8 taskId )
{
	...
	case 1:
		...
		{
			if( sMapData->cursorPos >=
				NUM_FRONTIER_FACILITIES - 1 )
...
static u8 MapNumToFrontierFacilityId( u16 mapNum ) // id + 1, zero means not a
						   // frontier map number
{
	if( ( mapNum >= MAP_NUM( BATTLE_FRONTIER_BATTLE_TOWER_LOBBY ) &&
		    mapNum <=
			    MAP_NUM(
				    BATTLE_FRONTIER_BATTLE_TOWER_BATTLE_ROOM ) ) ||
		( mapNum >= MAP_NUM(
				    BATTLE_FRONTIER_BATTLE_TOWER_MULTI_BATTLE_ROOM ) &&
			mapNum <=
				MAP_NUM(
					BATTLE_FRONTIER_BATTLE_TOWER_BATTLE_ROOM2 ) ) )
		...
		mapNum ==
			MAP_NUM(
				BATTLE_FRONTIER_BATTLE_DOME_PRE_BATTLE_ROOM ) ||
		...
		mapNum ==
			MAP_NUM( BATTLE_FRONTIER_BATTLE_PALACE_BATTLE_ROOM ) )
		...
		mapNum ==
			MAP_NUM(
				BATTLE_FRONTIER_BATTLE_FACTORY_PRE_BATTLE_ROOM ) ||
		mapNum ==
			MAP_NUM( BATTLE_FRONTIER_BATTLE_FACTORY_BATTLE_ROOM ) )
		...
		mapNum ==
			MAP_NUM(
				BATTLE_FRONTIER_BATTLE_PIKE_THREE_PATH_ROOM ) ||
		mapNum ==
			MAP_NUM( BATTLE_FRONTIER_BATTLE_PIKE_RANDOM_ROOM1 ) ||
		mapNum ==
			MAP_NUM( BATTLE_FRONTIER_BATTLE_PIKE_RANDOM_ROOM2 ) ||
		...
		mapNum ==
			MAP_NUM(
				BATTLE_FRONTIER_BATTLE_PYRAMID_EMPTY_SQUARE ) ||
...
static void InitFrontierMapSprites( void )
{
	...
	// Create player indicator head sprite only if it's in vicinity of
	// battle frontier.
...
static void HandleFrontierMapCursorMove( u8 direction )
{
	...
	if( direction )
	{
		...
		sMapData->cursorPos =
			( oldCursorPos + 6 ) % NUM_FRONTIER_FACILITIES;
	}
	else
	{
		oldCursorPos = sMapData->cursorPos;
		sMapData->cursorPos =
			( oldCursorPos + 1 ) % NUM_FRONTIER_FACILITIES;
	}
	...
	StartSpriteAnim( sMapData->mapIndicatorSprite,
		sMapLandmarks[sMapData->cursorPos].animNum );
	sMapData->mapIndicatorSprite->pos1.x =
		sMapLandmarks[sMapData->cursorPos].x;
	sMapData->mapIndicatorSprite->pos1.y =
		sMapLandmarks[sMapData->cursorPos].y;
