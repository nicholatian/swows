...
static void DisplayPartyPokemonNickname(
	struct Pokemon*, struct Struct203CEDC*, u8 );
static void DisplayPartyPokemonLevelCheck(
	struct Pokemon*, struct Struct203CEDC*, u8 );
static void DisplayPartyPokemonGenderNidoranCheck(
	struct Pokemon*, struct Struct203CEDC*, u8 );
static void DisplayPartyPokemonHPCheck(
	struct Pokemon*, struct Struct203CEDC*, u8 );
static void DisplayPartyPokemonMaxHPCheck(
	struct Pokemon*, struct Struct203CEDC*, u8 );
static void DisplayPartyPokemonHPBarCheck(
	struct Pokemon*, struct Struct203CEDC* );
...
static void party_menu_link_mon_icon_anim(
	u16, u32, struct Struct203CEDC*, u8, u32 );
static void party_menu_link_mon_held_item_object(
	u16, u16, struct Struct203CEDC* );
static void party_menu_link_mon_pokeball_object( u16, struct Struct203CEDC* );
static void party_menu_link_mon_status_condition_object(
	u16, u8, struct Struct203CEDC* );
static void party_menu_held_item_object(
	struct Pokemon*, struct Struct203CEDC* );
static void party_menu_pokeball_object(
	struct Pokemon*, struct Struct203CEDC* );
static void party_menu_icon_anim(
	struct Pokemon*, struct Struct203CEDC*, u32 );
static void party_menu_status_condition_object(
	struct Pokemon*, struct Struct203CEDC* );
...
static void party_menu_get_status_condition_and_update_object(
	struct Pokemon*, struct Struct203CEDC* );
static void party_menu_update_status_condition_object(
	u8, struct Struct203CEDC* );
...
static void BlitBitmapToPartyWindow_Default1(
	u8 windowId, u8 x, u8 y, u8 width, u8 height, u8 f );
static void BlitBitmapToPartyWindow_Default2(
	u8 windowId, u8 x, u8 y, u8 width, u8 height, u8 f );
...
static const u32 gUnknown_086157C4[] =
	INCBIN_U32( "graphics/interface/unknown_6157C4.bin" );
static const u32 gUnknown_086157E0[] =
	INCBIN_U32( "graphics/interface/unknown_6157E0.bin" );
...
struct
{
	...
	[MENU_FIELD_MOVES + FIELD_MOVE_FLASH] = {gMoveNames[MOVE_FLASH],
		CursorCb_FieldMove},
	[MENU_FIELD_MOVES +
		FIELD_MOVE_ROCK_SMASH]        = {gMoveNames[MOVE_ROCK_SMASH],
                CursorCb_FieldMove},
	[MENU_FIELD_MOVES + FIELD_MOVE_STRENGTH] = {gMoveNames[MOVE_STRENGTH],
		CursorCb_FieldMove},
	[MENU_FIELD_MOVES +
	...
	[MENU_FIELD_MOVES +
		FIELD_MOVE_WATERFALL]            = {gMoveNames[MOVE_WATERFALL],
                CursorCb_FieldMove},
	[MENU_FIELD_MOVES + FIELD_MOVE_TELEPORT] = {gMoveNames[MOVE_TELEPORT],
		CursorCb_FieldMove},
	[MENU_FIELD_MOVES +
	...
	[MENU_FIELD_MOVES +
		...
		CursorCb_FieldMove},
	[MENU_FIELD_MOVES +
		FIELD_MOVE_MILK_DRINK]   = {gMoveNames[MOVE_MILK_DRINK],
                CursorCb_FieldMove},
	[MENU_FIELD_MOVES +
		FIELD_MOVE_SOFT_BOILED]  = {gMoveNames[MOVE_SOFT_BOILED],
                CursorCb_FieldMove},
	[MENU_FIELD_MOVES +
		FIELD_MOVE_SWEET_SCENT]  = {gMoveNames[MOVE_SWEET_SCENT],
                CursorCb_FieldMove},
};

static const u8 gUnknown_08615D10[] = {
	MENU_SUMMARY, MENU_SWITCH, MENU_CANCEL1};
static const u8 gUnknown_08615D13[] = {MENU_SHIFT, MENU_SUMMARY, MENU_CANCEL1};
static const u8 gUnknown_08615D16[] = {
...
static const u8 sListSizeTable[] = {0x00,
...
static const u32 sHeldItemGfx[] =
	INCBIN_U32( "graphics/interface/hold_icons.4bpp" );
static const u16 sHeldItemPalette[] =
	INCBIN_U16( "graphics/interface/hold_icons.gbapal" );
...
static const union AnimCmd sPokeballAnim_Open[] = {
	ANIMCMD_FRAME( 16, 0 ), ANIMCMD_END};
...
static const struct OamData sOamData_8615F20 = {.y = 0,
	.affineMode                                = 0,
	.objMode                                   = 0,
	.mosaic                                    = 0,
	.bpp                                       = 0,
...
static const union AnimCmd sSpriteAnim_8615F50[] = {
	ANIMCMD_FRAME( 20, 0 ), ANIMCMD_END};
...
static const union AnimCmd sSpriteAnim_StatusFaint[] = {
	ANIMCMD_FRAME( 24, 0 ), ANIMCMD_END};

static const union AnimCmd sSpriteAnim_Blank[] = {
	ANIMCMD_FRAME( 28, 0 ), ANIMCMD_END};
...
// code
static void InitPartyMenu( u8 a,
	u8 b,
	u8 c,
	u8 d,
	u8 messageId,
	...
	MainCallback callback )
{
	...
	else
	{
		...
		for( i = 0; i < ARRAY_COUNT( gUnknown_0203CEC4->windowId );
			i++ )
			gUnknown_0203CEC4->windowId[i] = 0xFF;

		if( d == 0 )
			gUnknown_0203CEC8.slotId = 0;
		else if( gUnknown_0203CEC8.slotId > 5 ||
			GetMonData( &gPlayerParty[gUnknown_0203CEC8.slotId],
				MON_DATA_SPECIES ) == SPECIES_NONE )
			gUnknown_0203CEC8.slotId =
				0; // wut why is this else if?
...
static void PartyMenuInitCallback( void )
{
	while( TRUE )
	{
		if( sub_81221EC( ) == TRUE || PartyMenuSetup( ) == TRUE ||
			sub_81221AC( ) == TRUE )
			break;
	}
}
...
static bool8 AllocPartyMenuBg( void )
{
	...
	InitBgsFromTemplates(
		0, gUnknown_086156B8, ARRAY_COUNT( gUnknown_086156B8 ) );
...
static bool8 AllocPartyMiscGfx( void )
{
	...
	case 0:
		gUnknown_0203CEE0 =
			malloc_and_decompress( gPartyMenuMisc_Gfx, &sizeout );
	...
	case 1:
		...
		{
			LZDecompressWram(
				gPartyMenuMisc_Tilemap, gUnknown_0203CEE4 );
			gUnknown_0203CEC4->data[0]++;
		}
		break;
	case 2:
...
static void PartyPaletteBufferCopy( u8 offset )
{
	offset *= 16;
	CpuCopy16(
		&gPlttBufferUnfaded[0x30], &gPlttBufferUnfaded[offset], 32 );
...
static void PartyMenuInitHelperStructs( u8 a )
{
	...
	gUnknown_0203CEDC =
		Alloc( sizeof( struct Struct203CEDC[PARTY_SIZE] ) );
	...
	else
	{
			...
			SPECIES_NONE )
		{
			...
			UpdateSelectedPartyBox(
				&gUnknown_0203CEDC[slot], 0x40 );
			CopyWindowToVram(
				gUnknown_0203CEDC[slot].windowId, 2 );
		}
		else
		{
...
static void DisplayPartyPokemonData( u8 slot )
{
	...
	{
		gUnknown_0203CEDC[slot].unk0->unk0(
			gUnknown_0203CEDC[slot].windowId, 0, 0, 0, 0, 1 );
		DisplayPartyPokemonNickname(
			&gPlayerParty[slot], &gUnknown_0203CEDC[slot], 0 );
	}
	else
	{
		gUnknown_0203CEDC[slot].unk0->unk0(
			gUnknown_0203CEDC[slot].windowId, 0, 0, 0, 0, 0 );
		DisplayPartyPokemonNickname(
			&gPlayerParty[slot], &gUnknown_0203CEDC[slot], 0 );
		...
		DisplayPartyPokemonHPBarCheck(
			&gPlayerParty[slot], &gUnknown_0203CEDC[slot] );
	}
}
	...
	else
	{
		...
		case 2:
			if( !GetMonData( currentPokemon, MON_DATA_IS_EGG ) &&
				GetEvolutionTargetSpecies( currentPokemon,
					3,
					item ) != SPECIES_NONE )
				return FALSE;
...
static void sub_81B0CEC( u8 slot )
{
	...
	else
	{
		...
		StringCopy(
			gStringVar1, gUnknown_02022FF8[actualSlot].nickname );
		StringGetEnd10( gStringVar1 );
		sub_81DB52C( gStringVar1 );
		DisplayPartyPokemonBarDetail( structPtr->windowId,
			gStringVar1,
			0,
			structPtr->unk0->unk4 );
		DisplayPartyPokemonLevel(
			gUnknown_02022FF8[actualSlot].level, structPtr );
		DisplayPartyPokemonGender(
			gUnknown_02022FF8[actualSlot].gender,
			gUnknown_02022FF8[actualSlot].species,
			gUnknown_02022FF8[actualSlot].nickname,
			structPtr );
		DisplayPartyPokemonHP(
			gUnknown_02022FF8[actualSlot].hp, structPtr );
		DisplayPartyPokemonMaxHP(
			gUnknown_02022FF8[actualSlot].maxhp, structPtr );
		DisplayPartyPokemonHPBar( gUnknown_02022FF8[actualSlot].hp,
			gUnknown_02022FF8[actualSlot].maxhp,
			structPtr );
	}
}
...
static void party_menu_add_per_mon_objects_internal( u8 slot )
{
	...
	if( gUnknown_0203CEC8.unk8_0 == 5 && slot > 2 )
	{
		...
		if( gUnknown_02022FF8[actualSlot].species != SPECIES_NONE )
		{
			party_menu_link_mon_icon_anim(
				gUnknown_02022FF8[actualSlot].species,
				gUnknown_02022FF8[actualSlot].personality,
				&gUnknown_0203CEDC[slot],
				0,
				0 );
			party_menu_link_mon_held_item_object(
				gUnknown_02022FF8[actualSlot].species,
				gUnknown_02022FF8[actualSlot].heldItem,
				&gUnknown_0203CEDC[slot] );
			...
			if( gUnknown_02022FF8[actualSlot].hp == 0 )
				...
				status = pokemon_ailments_get_primary(
					gUnknown_02022FF8[actualSlot].status );
		...
		SPECIES_NONE )
	{
		party_menu_icon_anim(
			&gPlayerParty[slot], &gUnknown_0203CEDC[slot], slot );
...
void sub_81B0FCC( u8 slot, u8 b )
{
	...
	default:
		if( GetMonData( &gPlayerParty[slot], MON_DATA_SPECIES ) !=
			SPECIES_NONE )
		{
			UpdateSelectedPartyBox( &gUnknown_0203CEDC[slot],
				GetPartyBoxPalBitfield( slot, b ) );
			AnimateSelectedPartyIcon(
				gUnknown_0203CEDC[slot].monSpriteId, b );
			sub_81B5F98(
				gUnknown_0203CEDC[slot].pokeballSpriteId, b );
		}
		return;
	case 6:
		if( b == 0 )
...
static u8 GetPartyBoxPalBitfield( u8 slot, u8 b )
{
	...
	{
		if( slot == gUnknown_0203CEC8.slotId ||
			slot == gUnknown_0203CEC8.unkA )
			returnVar |= 4;
	}
...
{
	if( gUnknown_0203CEC8.mode == 2 &&
		( slot == 1 || slot == 4 || slot == 5 ) )
		return TRUE;

	if( slot < 3 && ( gBattleTypeFlags & BATTLE_TYPE_ARENA ) &&
		gMain.inBattle &&
		( gBattleStruct->arenaLostPlayerMons >> sub_81B8F38( slot ) &
			1 ) )
		return TRUE;
...
static void sub_81B120C( void )
{
	CopyToBgTilemapBufferRect_ChangePalette(
		1, gUnknown_086157C4, 23, 16, 7, 2, 17 );
	CopyToBgTilemapBufferRect_ChangePalette(
		1, gUnknown_086157E0, 23, 18, 7, 2, 17 );
	schedule_bg_copy_tilemap_to_vram( 1 );
}
...
u8 GetCursorSelectionMonId( void ) { return gUnknown_0203CEC8.slotId; }

u8 sub_81B1360( void ) { return gUnknown_0203CEC8.unk8_0; }
...
static void sub_81B140C( u8 taskId, s8* ptr )
{
	...
	else
	{
		...
		case 0:
			...
			{
				if( gUnknown_0203CEC8.unk8_0 == 1 )
					gUnknown_0203CEC4->exitCallback =
						sub_81B9140;
...
static void SetNewPartySelectTarget2( s8* ptr, s8 b )
{
	...
	case 2:
		if( *ptr == 0 )
		{
			...
			{
				if( GetMonData( &gPlayerParty[3],
					    MON_DATA_SPECIES ) !=
					SPECIES_NONE )
					*ptr = 3;
			}
			else if( GetMonData( &gPlayerParty[2],
					 MON_DATA_SPECIES ) != SPECIES_NONE )
			{
				*ptr = 2;
			}
		...
		else if( *ptr == 1 )
		{
			...
			{
				if( GetMonData( &gPlayerParty[5],
					    MON_DATA_SPECIES ) !=
					SPECIES_NONE )
					*ptr = 5;
			}
			else if( GetMonData( &gPlayerParty[4],
					 MON_DATA_SPECIES ) != SPECIES_NONE )
			{
				*ptr = 4;
			}
...
static s8 sub_81B1B00( s8 a, s8 b )
{
	while( TRUE )
	{
		...
		if( (u8)a >= 6 )
			return -1;
		if( GetMonData( &gPlayerParty[a], MON_DATA_SPECIES ) !=
			SPECIES_NONE )
			return a;
	}
}
...
bool8 sub_81B1BD4( void ) { return FuncIsActiveTask( sub_81B1B8C ); }
...
static void pokemon_item_not_removed( u16 itemUnused )
{
	StringExpandPlaceholders(
		gStringVar4, gText_BagFullCouldNotRemoveItem );
}
...
static void sub_81B1E60( u8 taskId )
{
	...
	DisplayPartyPokemonHPCheck(
		&gPlayerParty[data[4]], &gUnknown_0203CEDC[data[4]], 1 );
	DisplayPartyPokemonHPBarCheck(
		&gPlayerParty[data[4]], &gUnknown_0203CEDC[data[4]] );
	if( data[3] == 0 || data[0] == 0 || data[0] == data[1] )
	{
		if( data[0] > data[5] )
			ConvertIntToDecimalStringN(
				gStringVar2, data[0] - data[5], 0, 3 );
...
u8 GetMonAilment( struct Pokemon* mon )
{
	...
	if( GetMonData( mon, MON_DATA_HP ) == 0 )
		return AILMENT_FNT;
	ailment = pokemon_ailments_get_primary(
		GetMonData( mon, MON_DATA_STATUS ) );
...
static bool16 sub_81B2134( struct Pokemon* mon )
{
	if( GetMonData( mon, MON_DATA_IS_EGG ) != TRUE &&
		sub_802C908( GetMonData( mon, MON_DATA_SPECIES ) ) )
		return TRUE;
	return FALSE;
}
...
static u8 CanMonLearnTMTutor( struct Pokemon* mon, u16 item, u8 tutor )
{
	...
	else if( CanLearnTutorMove( GetMonData( mon, MON_DATA_SPECIES ),
			 tutor ) == FALSE )
	{
		return CANNOT_LEARN_MOVE;
	}
	...
	if( gUnknown_0203CEC8.unk8_0 != 5 )
	{
		if( a == TRUE )
		{
			...
			FillWindowPixelBuffer(
				firstWindowId, PIXEL_FILL( 0 ) );
			mainOffset = GetStringCenterAlignXOffset(
				0, gMenuText_Confirm, 48 );
			AddTextPrinterParameterized4( firstWindowId,
				0,
				mainOffset,
				1,
				0,
...
static void BlitBitmapToPartyWindow(
	u8 windowId, const u8* b, u8 c, u8 x, u8 y, u8 width, u8 height )
{
	...
	u8 windowId, u8 x, u8 y, u8 width, u8 height, u8 isEgg )
{
	...
	if( isEgg == FALSE )
		BlitBitmapToPartyWindow(
			windowId, sMainSlotTileNums, 10, x, y, width, height );
	else
		BlitBitmapToPartyWindow( windowId,
...
{
	BlitBitmapToPartyWindow(
		windowId, sEmptySlotTileNums, 18, 0, 0, 18, 3 );
}

static void UpdateSelectedPartyBox( struct Struct203CEDC* ptr, u8 bitfield )
{
	u8 palNum =
		GetWindowAttribute( ptr->windowId, WINDOW_PALETTE_NUM ) * 16;

	if( bitfield & 0x40 )
	{
		LoadPalette(
			GetPartyMenuPaletteFromBuffer( gUnknown_08615AF1[0] ),
			gUnknown_08615AC0[0] + palNum,
			2 );
		LoadPalette(
			GetPartyMenuPaletteFromBuffer( gUnknown_08615AF1[1] ),
			gUnknown_08615AC0[1] + palNum,
			2 );
		LoadPalette(
			GetPartyMenuPaletteFromBuffer( gUnknown_08615AF1[2] ),
			gUnknown_08615AC0[2] + palNum,
			2 );
	}
	else if( bitfield & 0x20 )
	{
		if( bitfield & 1 )
		{
			LoadPalette( GetPartyMenuPaletteFromBuffer(
					     gUnknown_08615ADF[0] ),
				gUnknown_08615ABA[0] + palNum,
				2 );
			LoadPalette( GetPartyMenuPaletteFromBuffer(
					     gUnknown_08615ADF[1] ),
				gUnknown_08615ABA[1] + palNum,
				2 );
			LoadPalette( GetPartyMenuPaletteFromBuffer(
					     gUnknown_08615ADF[2] ),
				gUnknown_08615ABA[2] + palNum,
				2 );
			LoadPalette( GetPartyMenuPaletteFromBuffer(
					     gUnknown_08615AEB[0] ),
				gUnknown_08615ABD[0] + palNum,
				2 );
			LoadPalette( GetPartyMenuPaletteFromBuffer(
					     gUnknown_08615AEB[1] ),
				gUnknown_08615ABD[1] + palNum,
				2 );
			LoadPalette( GetPartyMenuPaletteFromBuffer(
					     gUnknown_08615AEB[2] ),
				gUnknown_08615ABD[2] + palNum,
				2 );
		}
		else
		{
	...
	{
		LoadPalette(
			GetPartyMenuPaletteFromBuffer( gUnknown_08615ADF[0] ),
			gUnknown_08615ABA[0] + palNum,
			2 );
		LoadPalette(
			GetPartyMenuPaletteFromBuffer( gUnknown_08615ADF[1] ),
			gUnknown_08615ABA[1] + palNum,
			2 );
		LoadPalette(
			GetPartyMenuPaletteFromBuffer( gUnknown_08615ADF[2] ),
			gUnknown_08615ABA[2] + palNum,
			2 );
		LoadPalette(
			GetPartyMenuPaletteFromBuffer( gUnknown_08615AEE[0] ),
			gUnknown_08615ABD[0] + palNum,
			2 );
		LoadPalette(
			GetPartyMenuPaletteFromBuffer( gUnknown_08615AEE[1] ),
			gUnknown_08615ABD[1] + palNum,
			2 );
		LoadPalette(
			GetPartyMenuPaletteFromBuffer( gUnknown_08615AEE[2] ),
			gUnknown_08615ABD[2] + palNum,
			2 );
	}
	else if( bitfield & 4 )
	...
	{
		LoadPalette(
			GetPartyMenuPaletteFromBuffer( gUnknown_08615AD6[0] ),
			gUnknown_08615ABA[0] + palNum,
			2 );
		LoadPalette(
			GetPartyMenuPaletteFromBuffer( gUnknown_08615AD6[1] ),
			gUnknown_08615ABA[1] + palNum,
			2 );
		LoadPalette(
			GetPartyMenuPaletteFromBuffer( gUnknown_08615AD6[2] ),
			gUnknown_08615ABA[2] + palNum,
			2 );
		LoadPalette(
			GetPartyMenuPaletteFromBuffer( gUnknown_08615AEB[0] ),
			gUnknown_08615ABD[0] + palNum,
			2 );
		LoadPalette(
			GetPartyMenuPaletteFromBuffer( gUnknown_08615AEB[1] ),
			gUnknown_08615ABD[1] + palNum,
			2 );
		LoadPalette(
			GetPartyMenuPaletteFromBuffer( gUnknown_08615AEB[2] ),
			gUnknown_08615ABD[2] + palNum,
			2 );
	}
	else
	{
		LoadPalette(
			GetPartyMenuPaletteFromBuffer( gUnknown_08615ACD[0] ),
			gUnknown_08615ABA[0] + palNum,
			2 );
		LoadPalette(
			GetPartyMenuPaletteFromBuffer( gUnknown_08615ACD[1] ),
			gUnknown_08615ABA[1] + palNum,
			2 );
		LoadPalette(
			GetPartyMenuPaletteFromBuffer( gUnknown_08615ACD[2] ),
			gUnknown_08615ABA[2] + palNum,
			2 );
		LoadPalette(
			GetPartyMenuPaletteFromBuffer( gUnknown_08615AE2[0] ),
			gUnknown_08615ABD[0] + palNum,
			2 );
		LoadPalette(
			GetPartyMenuPaletteFromBuffer( gUnknown_08615AE2[1] ),
			gUnknown_08615ABD[1] + palNum,
			2 );
		LoadPalette(
			GetPartyMenuPaletteFromBuffer( gUnknown_08615AE2[2] ),
			gUnknown_08615ABD[2] + palNum,
			2 );
	}
}

static void DisplayPartyPokemonBarDetail(
	u8 windowId, const u8* str, u8 color, const u8* align )
{
	AddTextPrinterParameterized3( windowId,
		0,
		align[0],
		align[1],
		sFontColorTable[color],
		0,
		str );
}

static void DisplayPartyPokemonNickname(
	struct Pokemon* mon, struct Struct203CEDC* ptr, u8 c )
{
	...
	if( GetMonData( mon, MON_DATA_SPECIES ) != SPECIES_NONE )
	{
		...
		DisplayPartyPokemonBarDetail(
			ptr->windowId, nickname, 0, ptr->unk0->unk4 );
	}
}
...
static void DisplayPartyPokemonLevel( u8 level, struct Struct203CEDC* ptr )
{
	...
	DisplayPartyPokemonBarDetail(
		ptr->windowId, gStringVar1, 0, &ptr->unk0->unk4[4] );
}
	...
	struct Pokemon* mon, struct Struct203CEDC* ptr, u8 c )
{
	...
	if( c == 1 )
		ptr->unk0->unk0( ptr->windowId,
			...
			0 );
	GetMonNickname( mon, nickname );
	DisplayPartyPokemonGender( GetMonGender( mon ),
...
static void DisplayPartyPokemonGender(
	u8 gender, u16 species, u8* nickname, struct Struct203CEDC* ptr )
{
	u8 palNum =
		GetWindowAttribute( ptr->windowId, WINDOW_PALETTE_NUM ) * 16;
	...
	case MON_MALE:
		LoadPalette(
			GetPartyMenuPaletteFromBuffer( gUnknown_08615AC3[0] ),
			gUnknown_08615AB6[0] + palNum,
			2 );
		LoadPalette(
			GetPartyMenuPaletteFromBuffer( gUnknown_08615AC3[1] ),
			gUnknown_08615AB6[1] + palNum,
			2 );
		DisplayPartyPokemonBarDetail( ptr->windowId,
			gText_MaleSymbol,
			2,
			&ptr->unk0->unk4[8] );
		break;
	case MON_FEMALE:
		LoadPalette(
			GetPartyMenuPaletteFromBuffer( gUnknown_08615AC5[0] ),
			gUnknown_08615AB6[0] + palNum,
			2 );
		LoadPalette(
			GetPartyMenuPaletteFromBuffer( gUnknown_08615AC5[1] ),
			gUnknown_08615AB6[1] + palNum,
			2 );
		DisplayPartyPokemonBarDetail( ptr->windowId,
			gText_FemaleSymbol,
	...
	struct Pokemon* mon, struct Struct203CEDC* ptr, u8 c )
{
	if( GetMonData( mon, MON_DATA_SPECIES ) != SPECIES_NONE )
	{
		if( c != 0 )
			ptr->unk0->unk0( ptr->windowId,
				ptr->unk0->unk4[12] >> 3,
				( ptr->unk0->unk4[13] >> 3 ) + 1,
				ptr->unk0->unk4[14] >> 3,
				ptr->unk0->unk4[15] >> 3,
				0 );
		if( c != 2 )
			DisplayPartyPokemonHP(
				GetMonData( mon, MON_DATA_HP ), ptr );
	}
}
...
static void DisplayPartyPokemonHP( u16 hp, struct Struct203CEDC* ptr )
{
	...
	DisplayPartyPokemonBarDetail(
		ptr->windowId, gStringVar1, 0, &ptr->unk0->unk4[12] );
}
	...
	struct Pokemon* mon, struct Struct203CEDC* ptr, u8 c )
{
	if( GetMonData( mon, MON_DATA_SPECIES ) != SPECIES_NONE )
	{
		if( c != 0 )
			ptr->unk0->unk0( ptr->windowId,
				( ptr->unk0->unk4[16] >> 3 ) + 1,
				( ptr->unk0->unk4[17] >> 3 ) + 1,
				ptr->unk0->unk4[18] >> 3,
				ptr->unk0->unk4[19] >> 3,
				0 );
		if( c != 2 )
			DisplayPartyPokemonMaxHP(
				GetMonData( mon, MON_DATA_MAX_HP ), ptr );
	}
}
...
static void DisplayPartyPokemonMaxHP( u16 maxhp, struct Struct203CEDC* ptr )
{
	...
	StringAppend( gStringVar1, gStringVar2 );
	DisplayPartyPokemonBarDetail(
		ptr->windowId, gStringVar1, 0, &ptr->unk0->unk4[16] );
}

static void DisplayPartyPokemonHPBarCheck(
	struct Pokemon* mon, struct Struct203CEDC* ptr )
{
	if( GetMonData( mon, MON_DATA_SPECIES ) != SPECIES_NONE )
...
static void DisplayPartyPokemonHPBar(
	u16 hp, u16 maxhp, struct Struct203CEDC* ptr )
{
	u8 palNum =
		GetWindowAttribute( ptr->windowId, WINDOW_PALETTE_NUM ) * 16;
	u8 hpFraction;
	...
	case HP_BAR_FULL:
		LoadPalette(
			GetPartyMenuPaletteFromBuffer( gUnknown_08615AC7[0] ),
			gUnknown_08615AB8[0] + palNum,
			2 );
		LoadPalette(
			GetPartyMenuPaletteFromBuffer( gUnknown_08615AC7[1] ),
			gUnknown_08615AB8[1] + palNum,
			2 );
		break;
	case HP_BAR_YELLOW:
		LoadPalette(
			GetPartyMenuPaletteFromBuffer( gUnknown_08615AC9[0] ),
			gUnknown_08615AB8[0] + palNum,
			2 );
		LoadPalette(
			GetPartyMenuPaletteFromBuffer( gUnknown_08615AC9[1] ),
			gUnknown_08615AB8[1] + palNum,
			2 );
		break;
	default:
		LoadPalette(
			GetPartyMenuPaletteFromBuffer( gUnknown_08615ACB[0] ),
			gUnknown_08615AB8[0] + palNum,
			2 );
		LoadPalette(
			GetPartyMenuPaletteFromBuffer( gUnknown_08615ACB[1] ),
			gUnknown_08615AB8[1] + palNum,
			2 );
		break;
	}
	...
	FillWindowPixelRect( ptr->windowId,
		gUnknown_08615AB8[1],
		ptr->unk0->unk4[20],
		ptr->unk0->unk4[21],
		hpFraction,
	...
	if( hpFraction != ptr->unk0->unk4[22] )
	{
		...
		FillWindowPixelRect( ptr->windowId,
			0x0D,
			ptr->unk0->unk4[20] + hpFraction,
			ptr->unk0->unk4[21],
			ptr->unk0->unk4[22] - hpFraction,
			1 );
		FillWindowPixelRect( ptr->windowId,
			0x02,
			ptr->unk0->unk4[20] + hpFraction,
...
static void DisplayPartyPokemonOtherText(
	u8 stringID, struct Struct203CEDC* ptr, u8 c )
{
	if( c != 0 )
	{
		int unk = ( ( ptr->unk0->unk1C % 8 ) + ptr->unk0->unk1E + 7 ) /
			8;
		int unk2 =
			( ( ptr->unk0->unk1D % 8 ) + ptr->unk0->unk1F + 7 ) /
			8;
		ptr->unk0->unk0( ptr->windowId,
			ptr->unk0->unk1C >> 3,
			ptr->unk0->unk1D >> 3,
	...
	if( c != 2 )
		AddTextPrinterParameterized3( ptr->windowId,
			1,
			ptr->unk0->unk1C,
			ptr->unk0->unk1D,
			sFontColorTable[0],
			0,
			sSelectionStringTable[stringID] );
}
...
void display_pokemon_menu_message( u32 stringID )
{
	...
	if( stringID != 0x7F )
	{
		...
		DrawStdFrameWithCustomTileAndPalette(
			*windowPtr, FALSE, 0x4F, 0xD );
		StringExpandPlaceholders(
			gStringVar4, sActionStringTable[stringID] );
		AddTextPrinterParameterized(
			*windowPtr, 1, gStringVar4, 0, 1, 0, 0 );
		schedule_bg_copy_tilemap_to_vram( 2 );
	}
}
...
static u8 sub_81B31B0( u8 a )
{
	...
	case 0:
		SetWindowTemplateFields( &window,
			2,
			19,
			19 - ( gUnknown_0203CEC4->listSize * 2 ),
			10,
			gUnknown_0203CEC4->listSize * 2,
			14,
			0x2E9 );
		break;
	case 1:
	...
	DrawStdFrameWithCustomTileAndPalette(
		gUnknown_0203CEC4->windowId[0], FALSE, 0x4F, 13 );
	if( a == 3 )
	...
	for( i = 0; i < gUnknown_0203CEC4->listSize; i++ )
	{
		...
		AddTextPrinterParameterized4( gUnknown_0203CEC4->windowId[0],
			1,
			cursorDimension,
			( i * 16 ) + 1,
			fontAttribute,
			...
			sFontColorTable[unk],
			0,
			sCursorOptions[gUnknown_0203CEC4->actions[i]].text );
	}

	InitMenuInUpperLeftCorner( gUnknown_0203CEC4->windowId[0],
		gUnknown_0203CEC4->listSize,
		0,
		1 );
	schedule_bg_copy_tilemap_to_vram( 2 );
...
static void sub_81B3300( const u8* text )
{
	...
	AddTextPrinterParameterized2(
		6, 1, text, GetPlayerTextSpeedDelay( ), 0, 2, 1, 3 );
}
	...
	if( !InBattlePike( ) )
	{
		if( GetMonData( &mons[1], MON_DATA_SPECIES ) != SPECIES_NONE )
			AppendToList( gUnknown_0203CEC4->actions,
				...
				MENU_SWITCH );
		if( ItemIsMail(
			    GetMonData( &mons[slotId], MON_DATA_HELD_ITEM ) ) )
			AppendToList( gUnknown_0203CEC4->actions,
				&gUnknown_0203CEC4->listSize,
				MENU_MAIL );
		else
...
static bool8 sub_81B3608( u8 taskId )
{
	...
	else
	{
		...
		{
			StringExpandPlaceholders(
				gStringVar4, gText_PkmnNotHolding );
			sub_81B1B5C( gStringVar4, 1 );
			schedule_bg_copy_tilemap_to_vram( 2 );
...
static void HandleMenuInput( u8 taskId )
{
	if( !gPaletteFade.active && sub_81221EC( ) != TRUE )
	{
		...
		case MENU_B_PRESSED:
			...
			sCursorOptions[gUnknown_0203CEC4->actions
					       [gUnknown_0203CEC4->listSize -
						       1]]
				.func( taskId );
			break;
		default:
			sub_81B302C( &gUnknown_0203CEC4->windowId[2] );
			sCursorOptions[gUnknown_0203CEC4->actions[input]].func(
				taskId );
			break;
		}
	}
...
static void sub_81B3828( void )
{
	if( gUnknown_0203CEC8.unk8_0 == 1 )
	{
		...
		ShowPokemonSummaryScreen( PSS_MODE_UNK1,
			gPlayerParty,
			gUnknown_0203CEC8.slotId,
			gPlayerPartyCount - 1,
			sub_81B3894 );
...
static void sub_81B3894( void )
{
	...
	InitPartyMenu( gUnknown_0203CEC8.unk8_0,
		0xFF,
		gUnknown_0203CEC8.unkB,
		1,
		21,
		sub_81B36FC,
		gUnknown_0203CEC8.exitCallback );
}
...
static void sub_81B3938( u8 taskId )
{
	...
	{
		windowIds[0] =
			gUnknown_0203CEDC[gUnknown_0203CEC8.slotId].windowId;
		data[0] = GetWindowAttribute(
			windowIds[0], WINDOW_TILEMAP_LEFT );
		data[1] =
			GetWindowAttribute( windowIds[0], WINDOW_TILEMAP_TOP );
		...
		if( data[2] == 10 )
			...
			data[10] = 1;
		windowIds[1] =
			gUnknown_0203CEDC[gUnknown_0203CEC8.unkA].windowId;
		data[4] = GetWindowAttribute(
			windowIds[1], WINDOW_TILEMAP_LEFT );
		data[5] =
			GetWindowAttribute( windowIds[1], WINDOW_TILEMAP_TOP );
...
static void sub_81B3B40(
	const void* rectSrc, s16 a, s16 b, s16 c, s16 d, s16 e )
{
	u8 f, g, h;
	...
	if( sub_81B3AD8( a, c, &f, &g, &h ) )
	{
		...
		if( sub_81B3AD8( a + e, c, &f, &g, &h ) )
			CopyRectToBgTilemapBufferRect(
				0, rectSrc, f, 0, c, d, g, b, h, d, 17, 0, 0 );
	}
}
...
static void sub_81B3C60( u8 taskId )
{
	...
	if( data[10] != 0 )
		sub_81B3C0C( &gUnknown_0203CEDC[gUnknown_0203CEC8.slotId],
			data[10] );
	if( data[11] != 0 )
		sub_81B3C0C(
			&gUnknown_0203CEDC[gUnknown_0203CEC8.unkA], data[11] );
}
...
static void swap_pokemon_and_oams( void )
{
	...
	*mon2         = *monBuffer;
	Free( monBuffer );
	oamt_swap_pos( &structPtrs[0]->pokeballSpriteId,
		&structPtrs[1]->pokeballSpriteId );
	oamt_swap_pos(
		&structPtrs[0]->itemSpriteId, &structPtrs[1]->itemSpriteId );
	oamt_swap_pos(
		&structPtrs[0]->monSpriteId, &structPtrs[1]->monSpriteId );
	oamt_swap_pos( &structPtrs[0]->statusSpriteId,
		&structPtrs[1]->statusSpriteId );
}
...
static void sub_81B41C4( void )
{
	if( InBattlePyramid( ) == FALSE )
		GoToBagMenu( RETURN_LOCATION_POKEMON_LIST,
			POCKETS_COUNT,
			c2_8123744 );
	else
...
static void c2_8123744( void )
{
	...
	else
	{
		...
		else if( ItemIsMail( gSpecialVar_ItemId ) )
		{
			...
			sub_81B1DB8( &gPlayerParty[gUnknown_0203CEC8.slotId],
				gSpecialVar_ItemId );
			sub_81B452C( );
		}
		else
...
static void sub_81B42D0( u8 taskId )
{
	...
	if( !gPaletteFade.active )
	{
		...
		sub_81B1C84(
			&gPlayerParty[gUnknown_0203CEC8.slotId], item, 0, 0 );
...
static void sub_81B4350( u8 taskId )
{
	if( !gPaletteFade.active )
	{
		sub_81B1D1C( &gPlayerParty[gUnknown_0203CEC8.slotId],
			gUnknown_0203CEFC,
			1 );
	...
	case 0:
		...
		else
		{
			sub_81B1DB8( &gPlayerParty[gUnknown_0203CEC8.slotId],
				gSpecialVar_ItemId );
			sub_81B1D68(
				gSpecialVar_ItemId, gUnknown_0203CEFC, 1 );
			gTasks[taskId].func = sub_81B469C;
		}
		break;
	case MENU_B_PRESSED:
...
{
	u8 mail = GetMonData(
		&gPlayerParty[gUnknown_0203CEC8.slotId], MON_DATA_MAIL );
...
static void sub_81B4624( u8 taskId )
{
	...
	{
		if( gUnknown_0203CEFC == ITEM_NONE )
			sub_81B1C84( &gPlayerParty[gUnknown_0203CEC8.slotId],
				gSpecialVar_ItemId,
				0,
				0 );
		else
			sub_81B1D68(
				gSpecialVar_ItemId, gUnknown_0203CEFC, 0 );
		gTasks[taskId].func = sub_81B469C;
	}
}
...
static void sub_81B469C( u8 taskId )
{
	...
	{
		sub_81B5C94(
			mon, &gUnknown_0203CEDC[gUnknown_0203CEC8.slotId] );
		if( gUnknown_0203CEC8.unk8_0 == 12 )
		{
			if( GetMonData( mon, MON_DATA_HELD_ITEM ) !=
				ITEM_NONE )
				DisplayPartyPokemonOtherText( 11,
					&gUnknown_0203CEDC[gUnknown_0203CEC8
								   .slotId],
					1 );
			else
				DisplayPartyPokemonOtherText( 12,
					&gUnknown_0203CEDC[gUnknown_0203CEC8
								   .slotId],
					1 );
		}
...
static void sub_81B48DC( u8 taskId )
{
	...
	case 0:
		CopyItemName(
			GetMonData( mon, MON_DATA_HELD_ITEM ), gStringVar1 );
...
static void sub_81B4988( u8 taskId )
{
	...
	if( sub_81B1BD4( ) != TRUE )
	{
		...
		DisplayPartyPokemonOtherText(
			12, &gUnknown_0203CEDC[gUnknown_0203CEC8.slotId], 1 );
		gTasks[taskId].func = sub_81B1C1C;
	}
}
...
static void sub_81B4A98( void )
{
	ReadMail( &gSaveBlock1Ptr->mail[GetMonData(
...
static void sub_81B4BA0( u8 taskId )
{
	...
	case 0:
		if( TakeMailFromMon2(
			    &gPlayerParty[gUnknown_0203CEC8.slotId] ) != 0xFF )
		{
...
static void sub_81B4C94( u8 taskId )
{
	...
	{
	case 0:
		item = GetMonData( &gPlayerParty[gUnknown_0203CEC8.slotId],
			MON_DATA_HELD_ITEM );
...
static void CursorCb_Cancel2( u8 taskId )
{
	...
	sub_81B302C( &gUnknown_0203CEC4->windowId[1] );
	sub_81B33B4(
		gPlayerParty, gUnknown_0203CEC8.slotId, sub_81B353C( mon ) );
	if( gUnknown_0203CEC8.unk8_0 != 12 )
	{
		sub_81B31B0( 0 );
		display_pokemon_menu_message( 21 );
	...
	else
	{
		sub_81B31B0( 1 );
		CopyItemName(
			GetMonData( mon, MON_DATA_HELD_ITEM ), gStringVar2 );
		display_pokemon_menu_message( 26 );
	}
	gTasks[taskId].data[0] = 0xFF;
	gTasks[taskId].func    = HandleMenuInput;
	...
	for( i = 0; i < unk; i++ )
	{
		if( gSelectedOrderFromParty[i] == 0 )
		{
			...
			gSelectedOrderFromParty[i] =
				gUnknown_0203CEC8.slotId + 1;
			DisplayPartyPokemonOtherText( i + 2,
...
static void CursorCb_NoEntry( u8 taskId )
{
	...
	for( i = 0; i < unk; i++ )
	{
		...
		{
			for( j = i; j < ( unk - 1 ); j++ )
				gSelectedOrderFromParty[j] =
					gSelectedOrderFromParty[j + 1];
			gSelectedOrderFromParty[j] = 0;
			break;
		}
	}
	DisplayPartyPokemonOtherText(
		1, &gUnknown_0203CEDC[gUnknown_0203CEC8.slotId], 1 );
	for( i = 0; i < ( unk - 1 ); i++ )
	{
		if( gSelectedOrderFromParty[i] != 0 )
			DisplayPartyPokemonOtherText( i + 2,
				&gUnknown_0203CEDC[gSelectedOrderFromParty[i] -
					1],
				1 );
	}
	display_pokemon_menu_message( 0 );
...
static void CursorCb_Register( u8 taskId )
{
	u16 species2 = GetMonData(
		&gPlayerParty[gUnknown_0203CEC8.slotId], MON_DATA_SPECIES2 );
	u16 species = GetMonData(
		&gPlayerParty[gUnknown_0203CEC8.slotId], MON_DATA_SPECIES );
	u8 obedience = GetMonData(
		&gPlayerParty[gUnknown_0203CEC8.slotId], MON_DATA_OBEDIENCE );

	switch( sub_807A8D0(
		*(struct UnkLinkRfuStruct_02022B14Substruct*)sub_800F7DC( ),
		species2,
		species,
		obedience ) )
	{
	case 1:
		StringExpandPlaceholders(
			gStringVar4, gText_PkmnCantBeTradedNow );
		break;
	case 2:
		StringExpandPlaceholders(
			gStringVar4, gText_EggCantBeTradedNow );
		break;
	default:
		PlaySE( SE_SELECT );
		sub_81B12C0( taskId );
...
static void CursorCb_Trade1( u8 taskId )
{
	...
	{
		StringExpandPlaceholders(
			gStringVar4, gUnknown_08615E0C[stringId - 1] );
		PlaySE( SE_HAZURE );
		sub_81B302C( &gUnknown_0203CEC4->windowId[0] );
		sub_81B302C( &gUnknown_0203CEC4->windowId[1] );
		StringAppend( gStringVar4, gText_PauseUntilPress );
...
static void CursorCb_Trade2( u8 taskId )
{
	...
	case 1:
		StringExpandPlaceholders(
			gStringVar4, gText_OnlyPkmnForBattle );
		break;
	case 2:
		StringExpandPlaceholders(
			gStringVar4, gText_PkmnCantBeTradedNow );
	...
	default:
		PlaySE( SE_SELECT );
		GetMonNickname(
			&gPlayerParty[gUnknown_0203CEC8.slotId], gStringVar1 );
		StringExpandPlaceholders(
			gStringVar4, gJPText_PutVar1IntoSpinner );
		sub_81B1B5C( gStringVar4, 1 );
...
static void CursorCb_FieldMove( u8 taskId )
{
	u8 fieldMove = gUnknown_0203CEC4->actions[Menu_GetCursorPos( )] -
		MENU_FIELD_MOVES;
	...
	if( sub_81221AC( ) == TRUE || InUnionRoom( ) == TRUE )
	{
		if( fieldMove == FIELD_MOVE_MILK_DRINK ||
			...
			display_pokemon_menu_message(
				sFieldMoveCursorCallbacks[fieldMove].msgId );
	...
	else
	{
				 ...
				 .fieldMoveFunc( ) == TRUE )
		{
			...
			case FIELD_MOVE_DIG:
				...
				sub_81245DC( gStringVar1,
					mapHeader->regionMapSectionId );
				StringExpandPlaceholders(
					gStringVar4, gText_EscapeFromHere );
				sub_81B5674( taskId );
				gUnknown_0203CEC4->data[0] = fieldMove;
				break;
			case FIELD_MOVE_FLY:
		...
		else
		{
			...
			default:
				display_pokemon_menu_message(
					sFieldMoveCursorCallbacks[fieldMove]
						.msgId );
				break;
			}
			gTasks[taskId].func =
...
{
	InitPartyMenu(
		0, 0, 0, 1, 0, sub_81B1370, CB2_ReturnToFieldWithOpenMenu );
}
...
static void party_menu_icon_anim(
	struct Pokemon* mon, struct Struct203CEDC* ptr, u32 a )
{
	u32 bit = 1;
	u16 species2;
	...
	party_menu_link_mon_icon_anim( species2,
...
static void party_menu_link_mon_icon_anim(
	u16 species, u32 pid, struct Struct203CEDC* ptr, u8 priority, u32 bit )
{
	if( species != SPECIES_NONE )
	{
		ptr->monSpriteId = CreateMonIcon( species,
			sub_80D3014,
			ptr->unk4[0],
			ptr->unk4[1],
			4,
	...
	struct Pokemon* mon, struct Struct203CEDC* ptr )
{
	if( GetMonData( mon, MON_DATA_SPECIES ) != SPECIES_NONE )
	{
		ptr->itemSpriteId = CreateSprite( &sSpriteTemplate_HeldItem,
			ptr->unk4[2],
			ptr->unk4[3],
			0 );
		sub_81B5C94( mon, ptr );
...
static void party_menu_link_mon_held_item_object(
	u16 species, u16 item, struct Struct203CEDC* ptr )
{
	if( species != SPECIES_NONE )
	{
		ptr->itemSpriteId = CreateSprite( &sSpriteTemplate_HeldItem,
...
static void sub_81B5DF0( u8 spriteId, u8 isMail )
{
	...
	u8 newSpriteId = CreateSprite(
		&sSpriteTemplate_HeldItem, 250, 170, subpriority - 1 );
...
static void SpriteCB_HeldItem( struct Sprite* sprite )
{
	...
	else
	{
		...
		sprite->pos1.x    = gSprites[otherSpriteId].pos1.x +
			gSprites[otherSpriteId].pos2.x;
		sprite->pos1.y = gSprites[otherSpriteId].pos1.y +
			gSprites[otherSpriteId].pos2.y;
	}
}
...
{
	if( GetMonData( mon, MON_DATA_SPECIES ) != SPECIES_NONE )
		ptr->pokeballSpriteId =
			CreateSprite( &sSpriteTemplate_MenuPokeball,
				ptr->unk4[6],
				ptr->unk4[7],
				8 );
}

static void party_menu_link_mon_pokeball_object(
	u16 species, struct Struct203CEDC* ptr )
{
	if( species != SPECIES_NONE )
	{
		ptr->pokeballSpriteId =
	...
	struct Pokemon* mon, struct Struct203CEDC* ptr )
{
	...
	{
		ptr->statusSpriteId =
			CreateSprite( &sSpriteTemplate_StatusIcons,
				ptr->unk4[4],
				ptr->unk4[5],
				0 );
...
static void party_menu_link_mon_status_condition_object(
	u16 species, u8 status, struct Struct203CEDC* ptr )
{
	if( species != SPECIES_NONE )
	{
		ptr->statusSpriteId =
...
static void party_menu_update_status_condition_object(
	u8 status, struct Struct203CEDC* ptr )
{
	switch( status )
	{
	case AILMENT_NONE:
...
static void GetMedicineItemEffectMessage( u16 item )
{
	...
	case ITEM_EFFECT_CURE_POISON:
		StringExpandPlaceholders(
			gStringVar4, gText_PkmnCuredOfPoison );
		break;
	...
	case ITEM_EFFECT_CURE_PARALYSIS:
		StringExpandPlaceholders(
			gStringVar4, gText_PkmnCuredOfParalysis );
		break;
	case ITEM_EFFECT_CURE_CONFUSION:
		StringExpandPlaceholders(
			gStringVar4, gText_PkmnSnappedOutOfConfusion );
		break;
	case ITEM_EFFECT_CURE_INFATUATION:
		StringExpandPlaceholders(
			gStringVar4, gText_PkmnGotOverInfatuation );
		break;
	case ITEM_EFFECT_CURE_ALL_STATUS:
		StringExpandPlaceholders(
			gStringVar4, gText_PkmnBecameHealthy );
		break;
	case ITEM_EFFECT_HP_EV:
		StringCopy( gStringVar2, gText_HP3 );
		StringExpandPlaceholders(
			gStringVar4, gText_PkmnBaseVar2StatIncreased );
		break;
	...
	case ITEM_EFFECT_PP_MAX:
		StringExpandPlaceholders(
			gStringVar4, gText_MovesPPIncreased );
		break;
...
static bool8 ExecuteTableBasedItemEffect__(
	u8 partyMonIndex, u16 item, u8 monMoveIndex )
{
	if( gMain.inBattle )
		return ExecuteTableBasedItemEffect(
			&gPlayerParty[partyMonIndex],
			item,
			sub_81B8F38( partyMonIndex ),
			monMoveIndex );
	else
		return ExecuteTableBasedItemEffect(
			&gPlayerParty[partyMonIndex],
...
void ItemUseCB_Medicine( u8 taskId, TaskFunc task )
{
	...
	if( UsingHPEVItemOnShedinja( mon, item ) )
	{
		...
		if( ExecuteTableBasedItemEffect__(
			    gUnknown_0203CEC8.slotId, item, 0 ) )
		{
		iTriedHonestlyIDid:
			gUnknown_0203CEE8 = 0;
			PlaySE( SE_SELECT );
	...
	party_menu_get_status_condition_and_update_object(
		mon, &gUnknown_0203CEDC[gUnknown_0203CEC8.slotId] );
	if( gSprites[gUnknown_0203CEDC[gUnknown_0203CEC8.slotId]
			     .statusSpriteId]
			.invisible )
		DisplayPartyPokemonLevelCheck(
			mon, &gUnknown_0203CEDC[gUnknown_0203CEC8.slotId], 1 );
	if( canHeal == TRUE )
	{
		if( hp == 0 )
			sub_81B0FCC( gUnknown_0203CEC8.slotId, 1 );
...
void sub_81B67C8( u8 taskId, TaskFunc task )
{
	...
	bool8 cannotUseEffect = ExecuteTableBasedItemEffect__(
		gUnknown_0203CEC8.slotId, item, 0 );
	...
	else
	{
		...
		{
			StringExpandPlaceholders(
				gStringVar4, gText_PkmnAdoresBaseVar2Fell );
		}
		sub_81B1B5C( gStringVar4, 1 );
		schedule_bg_copy_tilemap_to_vram( 2 );
		gTasks[taskId].func = task;
...
void dp05_rare_candy( u8 taskId, TaskFunc task )
{
	...
	if( GetMonData( mon, MON_DATA_LEVEL ) != MAX_LEVEL )
	{
		...
		cannotUseEffect = ExecuteTableBasedItemEffect__(
			gUnknown_0203CEC8.slotId, *itemPtr, 0 );
	...
	else
	{
		...
		GetMonNickname( mon, gStringVar1 );
		ConvertIntToDecimalStringN(
			gStringVar2, GetMonData( mon, MON_DATA_LEVEL ), 0, 3 );
		StringExpandPlaceholders(
			gStringVar4, gText_PkmnElevatedToLvVar2 );
		sub_81B1B5C( gStringVar4, 1 );
		schedule_bg_copy_tilemap_to_vram( 2 );
...
{
	if( WaitFanfare( FALSE ) && sub_81B1BD4( ) != TRUE &&
		( ( gMain.newKeys & A_BUTTON ) ||
			( gMain.newKeys & B_BUTTON ) ) )
	{
		PlaySE( SE_SELECT );
...
static void sub_81B7704( u8 taskId )
{
			...
			( gMain.newKeys & B_BUTTON ) ) )
	{
		...
		result = MonTryLearningNewMove(
			&gPlayerParty[gUnknown_0203CEC8.slotId], 1 );
...
{
	u16 result = MonTryLearningNewMove(
		&gPlayerParty[gUnknown_0203CEC8.slotId], 0 );
...
static void sub_81B7810( u8 taskId )
{
	...
	if( targetSpecies != SPECIES_NONE )
	{
		...
		BeginEvolutionScene(
			mon, targetSpecies, 1, gUnknown_0203CEC8.slotId );
		DestroyTask( taskId );
	}
	else
	{
...
static void sub_81B7A28( u8 taskId )
{
	...
	hp = GetMonData( mon, MON_DATA_HP );
	if( ExecuteTableBasedItemEffect__(
		    gUnknown_0203CEC8.slotId, gSpecialVar_ItemId, 0 ) )
	{
		gTasks[taskId].func = task_sacred_ash_party_loop;
		return;
	}
...
u8 GetItemEffectType( u16 item )
{
	...
	if( ( itemEffect[0] & ( ITEM0_HIGH_CRIT | ITEM0_X_ATTACK ) ) ||
		itemEffect[1] || itemEffect[2] ||
		( itemEffect[3] & ITEM3_MIST ) )
...
static void sub_81B7E4C( u8 taskId )
{
	...
	if( !gPaletteFade.active )
	{
		...
		case ALREADY_KNOWS_MOVE:
			...
			return;
		default:
			if( GiveMoveToMon( mon, gUnknown_0203CEC8.unkE ) !=
				0xFFFF )
			{
				sub_81B6EB4( taskId );
				return;
...
{
	InitPartyMenu(
		0, 0, 0, 0, 0, sub_81B1370, CB2_ReturnToFieldWithOpenMenu );
}

void sub_81B7F60( void )
{
	MainCallback callback =
		( InBattlePyramid( ) == FALSE ) ? c2_815ABFC : sub_81C4F84;
...
{
	gUnknown_0203CEFC = GetMonData(
		&gPlayerParty[gUnknown_0203CEC8.slotId], MON_DATA_HELD_ITEM );
	if( gUnknown_0203CEFC == ITEM_NONE )
	{
		sub_81B8044( taskId );
	}
...
static void sub_81B8088( u8 taskId )
{
	...
	if( !gPaletteFade.active )
	{
		...
		sub_81B1C84(
			&gPlayerParty[gUnknown_0203CEC8.slotId], item, 0, 1 );
		sub_81B1DB8( &gPlayerParty[gUnknown_0203CEC8.slotId], item );
		sub_81B83F0( item );
...
static void sub_81B8230( u8 taskId )
{
	if( !gPaletteFade.active )
	{
		if( gUnknown_0203CEFC != ITEM_NONE )
			sub_81B1D68(
				gUnknown_0203CEC8.unkC, gUnknown_0203CEFC, 0 );
		else
			sub_81B1C84( &gPlayerParty[gUnknown_0203CEC8.slotId],
				gUnknown_0203CEC8.unkC,
				0,
...
static void sub_81B8474( u8 taskId )
{
	...
	gUnknown_0203CEE8 = 0;
	mail = &gSaveBlock1Ptr->mail[playerPCItemPageInfo.itemsAbove + 6 +
...
{
	memset( gSelectedOrderFromParty,
		0,
		sizeof( gSelectedOrderFromParty ) );
}
...
static bool8 GetBattleEntryEligibility( struct Pokemon* mon )
{
	...
	switch( VarGet( VAR_FRONTIER_FACILITY ) ) // oddly the specific cases
						  // are beyond 6, turns out
						  // case 9 is apparently
						  // related to link battles
	{
	case 9:
		if( GetMonData( mon, MON_DATA_HP ) != 0 )
			return TRUE;
...
static u8 sub_81B865C( void )
{
	...
	{
		u16 species =
			GetMonData( &party[order[i] - 1], MON_DATA_SPECIES );
		u16 item =
			GetMonData( &party[order[i] - 1], MON_DATA_HELD_ITEM );
		for( j = i + 1; j < unk2; j++ )
		{
			if( species ==
				GetMonData( &party[order[j] - 1],
					MON_DATA_SPECIES ) )
				return 18;
			if( item != ITEM_NONE &&
				item ==
...
static const u8* sub_81B88BC( void )
{
	...
	else if( facilityNum == FRONTIER_FACILITY_DOME &&
...
static bool8 sub_81B8A7C( void )
{
		...
		( slot == 1 || slot == 4 || slot == 5 ) )
	{
		...
		StringExpandPlaceholders(
			gStringVar4, gText_CantSwitchWithAlly );
		return FALSE;
	}
	if( GetMonData( &gPlayerParty[slot], MON_DATA_HP ) == 0 )
	{
	...
	for( i = 0; i < gBattlersCount; i++ )
	{
		if( GetBattlerSide( i ) == B_SIDE_PLAYER &&
			sub_81B8F38( slot ) == gBattlerPartyIndexes[i] )
		{
			GetMonNickname( &gPlayerParty[slot], gStringVar1 );
			StringExpandPlaceholders(
				gStringVar4, gText_PkmnAlreadyInBattle );
			return FALSE;
		}
	}
	if( GetMonData( &gPlayerParty[slot], MON_DATA_IS_EGG ) )
	...
	if( sub_81B8F38( slot ) == gBattleStruct->field_8B )
	{
		GetMonNickname( &gPlayerParty[slot], gStringVar1 );
		StringExpandPlaceholders(
			gStringVar4, gText_PkmnAlreadySelected );
		return FALSE;
	}
	...
	if( gUnknown_0203CEC8.unkB == 2 )
	{
		...
		GetMonNickname( &gPlayerParty[pokemon_order_func(
					gBattlerPartyIndexes[currBattler] )],
			gStringVar1 );
		StringExpandPlaceholders(
			gStringVar4, gText_PkmnCantSwitchOut );
		return FALSE;
	}
...
void sub_81B8C68( void ) { sub_81B8C88( gUnknown_0203CF00, sub_806D7EC( ) ); }
...
static void sub_81B8C88( u8* ptr, bool8 multiplayerFlag )
{
	...
	{
		j               = 1;
		partyIndexes[0] = gBattlerPartyIndexes[GetBattlerAtPosition(
	...
	else
	{
		...
		partyIndexes[0] = gBattlerPartyIndexes[GetBattlerAtPosition(
			B_POSITION_PLAYER_LEFT )];
		partyIndexes[1] = gBattlerPartyIndexes[GetBattlerAtPosition(
	...
	for( i = 0; i < 3; i++ )
		ptr[i] = ( partyIndexes[0 + ( i * 2 )] << 4 ) |
			partyIndexes[1 + ( i * 2 )];
}

void sub_81B8D64( u8 battlerId, u8 multiplayerFlag )
{
	sub_81B8D88( gBattleStruct->field_60[battlerId],
		multiplayerFlag,
		battlerId );
}
	...
	if( IsMultiBattle( ) )
	{
		...
		for( i = 0; i < PARTY_SIZE; i++ )
		{
			if( possiblePartyIndexes[i] == unk )
			{
				...
				possiblePartyIndexes[i] =
					possiblePartyIndexBuffer;
				break;
			}
		}
		if( i != PARTY_SIZE )
		{
			...
			battleStructRelated[0] =
				( possiblePartyIndexes[0] << 4 ) |
				possiblePartyIndexes[1];
			battleStructRelated[1] =
				( possiblePartyIndexes[2] << 4 ) |
				possiblePartyIndexes[3];
			battleStructRelated[2] =
				( possiblePartyIndexes[4] << 4 ) |
				possiblePartyIndexes[5];
		}
	}
}
...
static void sub_81B8F6C( u8 slot, u8 setVal )
{
	...
	slot /= 2;
	if( modResult != 0 )
		gUnknown_0203CF00[slot] =
			( gUnknown_0203CF00[slot] & 0xF0 ) | setVal;
	else
		gUnknown_0203CF00[slot] =
			( gUnknown_0203CF00[slot] & 0xF ) | ( setVal << 4 );
}
...
static void pokemon_change_order( void )
{
	...
	for( i = 0; i < PARTY_SIZE; i++ )
		memcpy( &gPlayerParty[pokemon_order_func( i )],
			&partyBuffer[i],
...
static void sub_81B9080( void )
{
	...
	memcpy( partyBuffer, gPlayerParty, sizeof( gPlayerParty ) );
	for( i = 0; i < PARTY_SIZE; i++ )
		memcpy( &gPlayerParty[sub_81B8F38( i )],
			&partyBuffer[i],
			sizeof( struct Pokemon ) );
	Free( partyBuffer );
}
...
static void sub_81B91B4( u8 taskId )
{
	...
	if( !gPaletteFade.active )
	{
		...
		if( data[0] == 0 )
		{
			...
			{
				if( gUnknown_02022FF8[i - 3].species !=
					SPECIES_NONE )
					AnimateSelectedPartyIcon(
						gUnknown_0203CEDC[i]
							.monSpriteId,
...
static void sub_81B9294( u8 taskId )
{
	...
	for( i = 3; i < PARTY_SIZE; i++ )
	{
		...
		{
			sub_81B9270( gUnknown_0203CEDC[i].monSpriteId,
				data[0] - 8 );
			sub_81B9270( gUnknown_0203CEDC[i].itemSpriteId,
				data[0] - 8 );
			sub_81B9270( gUnknown_0203CEDC[i].pokeballSpriteId,
				data[0] - 8 );
			sub_81B9270( gUnknown_0203CEDC[i].statusSpriteId,
				data[0] - 8 );
		}
	}
	ChangeBgX( 2, 0x800, 1 );
...
void sub_81B95E0( void )
{
	...
	for( i = 0; i < 3; i++ )
	{
		if( GetMonData( &gPlayerParty[i], MON_DATA_HELD_ITEM ) !=
			ITEM_NONE )
		{
			gSpecialVar_Result = TRUE;
			break;
...
void sub_81B96D0( void )
{
	...
	for( i = 0; i < MAX_MON_MOVES; i++ )
	{
		if( GetMonData( &gPlayerParty[gSpecialVar_0x8004],
			    MON_DATA_MOVE1 + i ) != ITEM_NONE )
			gSpecialVar_Result++;
	}
}
...
void sub_81B9770( void )
{
	...
	SetMonMoveSlot( &gPlayerParty[gSpecialVar_0x8004],
		MOVE_NONE,
		gSpecialVar_0x8005 );
	RemoveMonPPBonus(
...
static void sub_81B97DC( struct Pokemon* mon, u8 slotTo, u8 slotFrom )
{
	...
	ppBonuses |= ( ppBonusMove1 << ( slotFrom * 2 ) ) +
		( ppBonusMove2 << ( slotTo * 2 ) );
