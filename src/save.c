...
static u8 ClearSaveData_2(
	u16 a1, const struct SaveSectionLocation* location );
static u8 TryWriteSector( u8 sector, u8* data );
static u8 HandleWriteSector(
	u16 a1, const struct SaveSectionLocation* location );
 ...
 * between them each time the game is saved, so that if the current save slot
 * is corrupt, we can load the previous one. We also rotate the sectors in each
...
// (u8 *)structure was removed from the first statement of the macro in
// Emerald. This is because malloc is used to allocate addresses so storing the
// raw addresses should not be done in the offsets information.
#define SAVEBLOCK_CHUNK( structure, chunkNum ) \
	{ \
		chunkNum *SECTOR_DATA_SIZE, \
			min( sizeof( structure ) - \
					chunkNum * SECTOR_DATA_SIZE, \
				SECTOR_DATA_SIZE ) \
	}
	...
	u16 a1, const struct SaveSectionLocation* location )
{
	...
	else
	{
		gLastKnownGoodSector =
			gLastWrittenSector; // backup the current
					    // written sector before
					    // attempting to write.
		gLastSaveCounter = gSaveCounter;
		gLastWrittenSector++;
		gLastWrittenSector = gLastWrittenSector %
			SECTOR_SAVE_SLOT_LENGTH; // array count save
						 // sector locations
		gSaveCounter++;
		retVal = 1;
...
static u8 HandleWriteSectorNBytes( u8 sector, u8* data, u16 size )
{
	...
	section->id =
		CalculateChecksum( data, size ); // though this appears to be
						 // incorrect, it might be
						 // some sector checksum
						 // instead of a whole save
						 // checksum and only appears
						 // to be relevent to HOF
						 // data, if used.
...
static u8 TryWriteSector( u8 sector, u8* data )
{
	if( ProgramFlashSectorAndVerify( sector, data ) != 0 ) // is damaged?
	{
		SetDamagedSectorBits(
			ENABLE, sector ); // set damaged sector bits.
		return 0xFF;
	}
	else
	{
		SetDamagedSectorBits(
			DISABLE, sector ); // unset damaged sector bits.
					   // it's safe now.
		return 1;
	}
}
...
static u32 RestoreSaveBackupVars(
	const struct SaveSectionLocation* location ) // only ever called once,
						     // and gSaveBlock2 is
						     // passed to this
...
static u8 ClearSaveData_2( u16 a1, const struct SaveSectionLocation* location )
{
	...
	{
		if( ProgramFlashByte(
			    sector, i, ( (u8*)gFastSaveSection )[i] ) )
		{
			status = 0xFF;
			break;
		}
...
static u8 sav12_xor_get( u16 a1, const struct SaveSectionLocation* location )
{
	...
	{
		// sector is damaged, so enable the bit in gDamagedSaveSectors
		// and restore the last written sector and save counter.
		SetDamagedSectorBits( ENABLE, sector );
		gLastWrittenSector = gLastKnownGoodSector;
		gSaveCounter       = gLastSaveCounter;
		return 0xFF;
...
static u8 sub_8152E10( u16 a1, const struct SaveSectionLocation* location )
{
	...
	for( i = 0; i < SECTOR_SAVE_SLOT_LENGTH; i++ )
	{
		...
		checksum = CalculateChecksum(
			gFastSaveSection->data, location[id].size );
		if( gFastSaveSection->security == UNKNOWN_CHECK_VALUE &&
			gFastSaveSection->checksum == checksum )
		{
			u16 j;
			for( j = 0; j < location[id].size; j++ )
				( (u8*)location[id].data )[j] =
					gFastSaveSection->data[j];
		}
	}
...
static u8 GetSaveValidStatus( const struct SaveSectionLocation* location )
{
	...
	for( i = 0; i < SECTOR_SAVE_SLOT_LENGTH; i++ )
	{
		...
		{
			securityPassed = TRUE;
			checksum = CalculateChecksum( gFastSaveSection->data,
				location[gFastSaveSection->id].size );
	...
	for( i = 0; i < SECTOR_SAVE_SLOT_LENGTH; i++ )
	{
		DoReadFlashWholeSection(
			i + SECTOR_SAVE_SLOT_LENGTH, gFastSaveSection );
		if( gFastSaveSection->security == UNKNOWN_CHECK_VALUE )
		{
			securityPassed = TRUE;
			checksum = CalculateChecksum( gFastSaveSection->data,
	...
	if( saveSlot1Status == 1 && saveSlot2Status == 1 )
	{
		...
		{
			if( (unsigned)( saveSlot1Counter + 1 ) <
				(unsigned)( saveSlot2Counter + 1 ) )
...
static void UpdateSaveAddresses( void )
{
	...
	gRamSaveSectionLocations[i].data =
		(void*)( gSaveBlock2Ptr ) + gSaveSectionOffsets[i].toAdd;
	gRamSaveSectionLocations[i].size = gSaveSectionOffsets[i].size;

	for( i = 1; i < 5; i++ )
	{
		gRamSaveSectionLocations[i].data = (void*)( gSaveBlock1Ptr ) +
			gSaveSectionOffsets[i].toAdd;
		gRamSaveSectionLocations[i].size = gSaveSectionOffsets[i].size;
	}
...
u8 HandleSavingData( u8 saveType )
{
	...
	switch( saveType )
	{
	case SAVE_HALL_OF_FAME_ERASE_BEFORE: // deletes HOF before overwriting
					     // HOF completely. unused
	...
	case SAVE_HALL_OF_FAME: // hall of fame.
		...
		HandleWriteSectorNBytes(
			SECTOR_ID_HOF_2, tempAddr + 0xF80, 0xF80 );
		break;
	case SAVE_NORMAL: // normal save. also called by overwriting your own
			  // save.
	default:
		SaveSerializedGame( );
		save_write_to_flash( 0xFFFF, gRamSaveSectionLocations );
	...
	/*
	case EREADER_SAVE: // used in mossdeep "game corner" before/after
	battling old man e-reader trainer SaveSerializedGame();
	save_write_to_flash(0, gRamSaveSectionLocations); break;
	*/
...
{
	u8 retVal = sub_81529D4(
		SECTOR_SAVE_SLOT_LENGTH, gRamSaveSectionLocations );
	if( gDamagedSaveSectors )
...
u8 Save_LoadGameData( u8 a1 )
{
	...
	case 3:
		...
		if( result == 1 )
			result = sub_81530DC(
				0x1D, gDecompressionBuffer + 0xF80, 0xF80 );
		break;
	}
...
u16 sub_815355C( void )
{
	...
	for( i = 0; i < SECTOR_SAVE_SLOT_LENGTH; i++ )
	{
		...
		if( gFastSaveSection->id == 0 )
			return savSection->data[10] + savSection->data[11] +
				savSection->data[12] + savSection->data[13];
	}
	return 0;
}
