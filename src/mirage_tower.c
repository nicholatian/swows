...
static const u16 sRootFossil_Pal[] =
	INCBIN_U16( "graphics/misc/fossil.gbapal" );
...
void TryStartMirageTowerPulseBlendEffect( void )
{
	...
	sMirageTowerPulseBlend =
		AllocZeroed( sizeof( *sMirageTowerPulseBlend ) );
	...
	MarkUsedPulseBlendPalettes(
		&sMirageTowerPulseBlend->pulseBlend, 0x1, TRUE );
...
static void PlayerDescendMirageTower( u8 taskId )
{
	...
	TryGetEventObjectIdByLocalIdAndMap( 45,
	...
	if( ( gSprites[fakePlayerEventObject->spriteId].pos1.y +
		    gSprites[fakePlayerEventObject->spriteId].pos2.y ) >=
		( gSprites[playerEventObject->spriteId].pos1.y +
			gSprites[playerEventObject->spriteId].pos2.y ) )
	{
		DestroyTask( taskId );
...
static void StartScreenShake(
	u8 yShakeOffset, u8 xShakeOffset, u8 numShakes, u8 shakeDelay )
{
...
static void MoveCeilingCrumbleSprite( struct Sprite* sprite )
{
	...
	if( ( ( sprite->pos1.y ) + ( sprite->pos2.y ) ) >
...
static void SetInvisibleMirageTowerMetatiles( void )
{
	...
	for( i = 0; i < ARRAY_COUNT( sInvisibleMirageTowerMetatiles ); i++ )
		MapGridSetMetatileIdAt(
			sInvisibleMirageTowerMetatiles[i].x + 7,
...
static void InitMirageTowerShake( u8 taskId )
{
	...
	case 1:
		sMirageTowerGfxBuffer =
			(u8*)AllocZeroed( MIRAGE_TOWER_GFX_LENGTH );
		...
		break;
	case 2:
		CpuSet( sBlankTile_Gfx,
			sMirageTowerGfxBuffer,
			MIRAGE_TOWER_GFX_LENGTH / 2 );
		LoadBgTiles(
			0, sMirageTowerGfxBuffer, MIRAGE_TOWER_GFX_LENGTH, 0 );
		gTasks[taskId].data[0]++;
		break;
	case 3:
		SetBgTilemapBuffer( 0, sMirageTowerTilemapBuffer );
		CopyToBgTilemapBufferRect_ChangePalette(
			0, &sMirageTowerTilemap, 12, 29, 6, 12, 17 );
...
static void DoMirageTowerDisintegration( u8 taskId )
{
	...
	{
	case 1:
		sUnknown_0203CF10 = AllocZeroed(
			OUTER_BUFFER_LENGTH * sizeof( struct Struct203CF10 ) );
		break;
	case 3:
		if( gTasks[taskId].data[3] <= ( OUTER_BUFFER_LENGTH - 1 ) )
		{
			if( gTasks[taskId].data[1] > 1 )
			{
				...
				sUnknown_0203CF10[index].buffer =
					Alloc( INNER_BUFFER_LENGTH );
				for( i = 0; i <= ( INNER_BUFFER_LENGTH - 1 );
					i++ )
					...
					i++ )
				{
					...
					rand1 = Random( ) % 0x30;
					rand2 = Random( ) % 0x30;
					SWAP( sUnknown_0203CF10[index]
							.buffer[rand2],
						sUnknown_0203CF10[index]
							.buffer[rand1],
						temp );
				}
				if( gTasks[taskId].data[3] <=
					( OUTER_BUFFER_LENGTH - 1 ) )
		...
		for( i = ( u8 )( gTasks[taskId].data[2] ); i < index; i++ )
		{
			...
			if( sUnknown_0203CF10[i].currIndex >
		...
		LoadBgTiles(
			0, sMirageTowerGfxBuffer, MIRAGE_TOWER_GFX_LENGTH, 0 );
		if( sUnknown_0203CF10[OUTER_BUFFER_LENGTH - 1].currIndex >
			( INNER_BUFFER_LENGTH - 1 ) )
			break;
		return;
	case 4:
...
static void DoFossilFallAndSink( u8 taskId )
{
	...
	case 1:
		sUnknown_0203CF0C =
			AllocZeroed( sizeof( *sUnknown_0203CF0C ) );
		sUnknown_0203CF0C->frameImageTiles =
			AllocZeroed( ROOT_FOSSIL_GFX_LENGTH );
		sUnknown_0203CF0C->frameImage = AllocZeroed(
			sizeof( *sUnknown_0203CF0C->frameImage ) );
		sUnknown_0203CF0C->unkC = AllocZeroed(
			ROOT_FOSSIL_GFX_RANDOMIZER_LENGTH * sizeof( u16 ) );
	...
	case 2:
		...
		break;
	case 3:
		sUnknown_0203CF0C->frameImage->data =
			sUnknown_0203CF0C->frameImageTiles;
	...
	case 4: {
		...
		fossilTemplate.images =
			(struct SpriteFrameImage*)( sUnknown_0203CF0C
							    ->frameImage );
		sUnknown_0203CF0C->spriteId =
			CreateSprite( &fossilTemplate, 128, -16, 1 );
	...
	case 6:
			...
			i++ )
		{
			...
			rand1 = Random( ) % 0x100;
			rand2 = Random( ) % 0x100;
			SWAP( sUnknown_0203CF0C->unkC[rand2],
				sUnknown_0203CF0C->unkC[rand1],
				temp );
		}
		gSprites[sUnknown_0203CF0C->spriteId].callback = sub_81BF248;
		break;
	case 7:
		if( gSprites[sUnknown_0203CF0C->spriteId].callback !=
			SpriteCallbackDummy )
			return;
...
static void sub_81BF2B8( u8* a, u16 b, u8 c, u8 d, u8 e )
{
	...
	// This part is non-matching. 99% sure it IS functionally equivalent,
	// though.
	b = ( b & 1 ) ^ 1;
	c = ( c << ( ( b ) << 2 ) ) | ( 15 << ( ( b ^ 1 ) << 2 ) );
