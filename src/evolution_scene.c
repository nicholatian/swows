...
// const data
static const u16 sUnknown_085B4114[] =
	INCBIN_U16( "graphics/evolution_scene/unknown_5B4114.gbapal" );
static const u32 sUnknown_085B4134[] =
	INCBIN_U32( "graphics/evolution_scene/bg.4bpp.lz" );
static const u32 sUnknown_085B482C[] =
	INCBIN_U32( "graphics/evolution_scene/bg.bin.lz" );
static const u32 sUnknown_085B4D10[] =
	INCBIN_U32( "graphics/evolution_scene/bg2.bin.lz" );
static const u16 sUnknown_085B51E4[] =
	INCBIN_U16( "graphics/evolution_scene/gray_transition_intro.gbapal" );
static const u16 sUnknown_085B53E4[] = INCBIN_U16(
	"graphics/evolution_scene/gray_transition_lighten.gbapal" );
...
static const u16 sUnknown_085B5884[] =
	INCBIN_U16( "graphics/evolution_scene/transition.gbapal" );
...
static void Task_BeginEvolutionScene( u8 taskID )
{
	...
	case 1:
		if( !gPaletteFade.active )
		{
			...
			DestroyTask( taskID );
			EvolutionScene(
				mon, speciesToEvolve, canStopEvo, partyID );
		}
		break;
	}
}

void BeginEvolutionScene( struct Pokemon* mon,
	u16 speciesToEvolve,
	bool8 canStopEvo,
	u8 partyID )
{
...
void EvolutionScene( struct Pokemon* mon,
	u16 speciesToEvolve,
	bool8 canStopEvo,
	u8 partyID )
{
	...
	pokePal = GetMonSpritePalStructFromOtIdPersonality(
		currSpecies, trainerId, personality );
	...
	DecompressPicFromTable_2( &gMonFrontPicTable[speciesToEvolve],
		...
		speciesToEvolve );
	pokePal = GetMonSpritePalStructFromOtIdPersonality(
		speciesToEvolve, trainerId, personality );
	...
	memcpy( &sEvoStructPtr->savedPalette,
		&gPlttBufferUnfaded[0x20],
		0x60 );
...
static void CB2_EvolutionSceneLoadGraphics( void )
{
	...
	DecompressPicFromTable_2( &gMonFrontPicTable[postEvoSpecies],
		...
		postEvoSpecies );
	pokePal = GetMonSpritePalStructFromOtIdPersonality(
		postEvoSpecies, trainerId, personality );
...
void TradeEvolutionScene( struct Pokemon* mon,
	u16 speciesToEvolve,
	u8 preEvoSpriteID,
	u8 partyID )
{
	...
	sEvoStructPtr->evoTaskID = ID =
		CreateTask( Task_TradeEvolutionScene, 0 );
	gTasks[ID].tState           = 0;
	gTasks[ID].tPreEvoSpecies   = currSpecies;
	gTasks[ID].tPostEvoSpecies  = speciesToEvolve;
	gTasks[ID].tLearnsFirstMove = TRUE;
...
static void CreateShedinja( u16 preEvoSpecies, struct Pokemon* mon )
{
		...
		gPlayerPartyCount < 6 )
	{
		...
		CopyMon( &gPlayerParty[gPlayerPartyCount],
			mon,
		...
		SetMonData( &gPlayerParty[gPlayerPartyCount],
			MON_DATA_NICKNAME,
			( gSpeciesNames[gEvolutionTable[preEvoSpecies][1]
						.targetSpecies] ) );
		SetMonData( &gPlayerParty[gPlayerPartyCount],
			MON_DATA_HELD_ITEM,
			( &data ) );
		...
		for( i = MON_DATA_COOL_RIBBON; i < MON_DATA_COOL_RIBBON + 5;
			i++ )
			SetMonData( &gPlayerParty[gPlayerPartyCount],
				i,
				( &data ) );
		for( i = MON_DATA_CHAMPION_RIBBON;
			i <= MON_DATA_FATEFUL_ENCOUNTER;
			i++ )
			SetMonData( &gPlayerParty[gPlayerPartyCount],
				i,
				( &data ) );
		...
		GetSetPokedexFlag(
			SpeciesToNationalPokedexNum( evos[1].targetSpecies ),
			FLAG_SET_SEEN );
		GetSetPokedexFlag(
			SpeciesToNationalPokedexNum( evos[1].targetSpecies ),
			FLAG_SET_CAUGHT );

		if( GetMonData( shedinja, MON_DATA_SPECIES ) ==
				SPECIES_SHEDINJA &&
			GetMonData( shedinja, MON_DATA_LANGUAGE ) ==
				LANGUAGE_JAPANESE &&
...
static void Task_EvolutionScene( u8 taskID )
{
	...
	case 1: // print 'whoa, poke is evolving!!!' msg
		if( !gPaletteFade.active )
		{
			StringExpandPlaceholders(
				gStringVar4, gText_PkmnIsEvolving );
	...
	case 2: // wait for string, animate mon(and play its cry)
		...
		{
			EvoScene_DoMonAnimation( sEvoStructPtr->preEvoSpriteID,
	...
	case 3:
		if( EvoScene_IsMonAnimFinished(
			    sEvoStructPtr->preEvoSpriteID ) ) // wait
							      // for
							      // animation,
							      // play
							      // tu
	...
	case 5: // launch moving bg task, preapre evo sparkles
		if( !gPaletteFade.active )
		{
			...
			sEvoGraphicsTaskID =
				LaunchTask_PreEvoSparklesSet1( 17 );
			gTasks[taskID].tState++;
		}
		break;
	...
	case 7: // launch task that flashes pre evo with post evo sprites
		...
		{
			sEvoGraphicsTaskID =
				sub_817C3A0( sEvoStructPtr->preEvoSpriteID,
	...
	case 12: // stop music, return screen to pre-fade state
		...
		{
			m4aMPlayAllStop( );
			memcpy( &gPlttBufferUnfaded[0x20],
				sEvoStructPtr->savedPalette,
				0x60 );
			sub_8140174( );
	...
	case 14: // congratulations string and rename prompt
		...
		{
			StringExpandPlaceholders(
				gStringVar4, gText_CongratsPkmnEvolved );
			BattlePutTextOnWindow( gStringVar4, 0 );
			PlayBGM( MUS_FANFA5 );
			gTasks[taskID].tState++;
			SetMonData( mon,
	...
	case 15: // check if it wants to learn a new move
		if( !IsTextPrinterActive( 0 ) )
		{
			var = MonTryLearningNewMove(
				mon, gTasks[taskID].tLearnsFirstMove );
			if( var != 0 && !gTasks[taskID].tEvoWasStopped )
			{
				u8 text[20];

				if( !( gTasks[taskID].tBits &
					    TASK_BIT_LEARN_MOVE ) )
				{
					StopMapMusic( );
				...
				if( var == 0xFFFF ) // no place to learn it
					...
					0xFFFE ) // it already knows that move
					break;
				else
					gTasks[taskID].tState =
						20; // has less than 4 moves,
						    // so it's been learned
			}
	...
	case 16: // task has finished, return
		if( !gPaletteFade.active )
		{
			...
			if( !gTasks[taskID].tEvoWasStopped )
				CreateShedinja(
					gTasks[taskID].tPreEvoSpecies, mon );
	...
	case 17: // evolution has been canceled, stop music and re-fade palette
		if( !gTasks[sEvoGraphicsTaskID].isActive )
		{
			m4aMPlayAllStop( );
			BeginNormalPaletteFade(
				0x6001C, 0, 0x10, 0, RGB_WHITE );
			gTasks[taskID].tState++;
		}
		break;
	case 18: // animate pokemon trying to evolve again, evolution has been
		 // stopped
		if( !gPaletteFade.active )
		{
			EvoScene_DoMonAnimation( sEvoStructPtr->preEvoSpriteID,
		...
		break;
	case 19: // after the animation, print the string 'WHOA IT DID NOT
		 // EVOLVE!!!'
		if( EvoScene_IsMonAnimFinished(
	...
	case 20: // pokemon learned a new move, print string and play a fanfare
		if( !IsTextPrinterActive( 0 ) && !IsSEPlaying( ) )
		{
			...
			gTasks[taskID].tLearnsFirstMove =
				0x40; // re-used as a counter
			gTasks[taskID].tState++;
		}
		break;
	...
	case 22: // try to learn a new move
		...
		case 4:
			...
			if( gMain.newKeys & A_BUTTON )
			{
				...
				{
					gTasks[taskID].tLearnMoveState =
						gTasks[taskID].tData7;
					if( gTasks[taskID].tLearnMoveState ==
						5 )
						BeginNormalPaletteFade(
							0xFFFFFFFF,
							0,
...
static void Task_TradeEvolutionScene( u8 taskID )
{
	...
	case 4:
		if( !gPaletteFade.active )
		{
			...
			var = gSprites[sEvoStructPtr->preEvoSpriteID]
					.oam.paletteNum +
				16;
			sEvoGraphicsTaskID =
				LaunchTask_PreEvoSparklesSet1( var );
			gTasks[taskID].tState++;
	...
	case 13:
			...
			IsFanfareTaskInactive( ) == TRUE )
		{
			...
			else
			{
				PlayBGM( MUS_SHINKA );
				sub_807F1A8(
					0, gText_CommunicationStandby5, 1 );
				gTasks[taskID].tState++;
			}
		}
		break;
	...
	case 17:
		...
		{
			StringExpandPlaceholders(
				gStringVar4, gText_EllipsisQuestionMark );
			sub_807F1A8( 0, gStringVar4, 1 );
...
static void sub_813FDEC( u8 taskId )
{
	...
	if( data[0]++ > sUnknown_085B58C9[data[2]][3] )
	{
		...
		{
			LoadPalette(
				&sEvoMovingBgPtr[data[1] * 16], 0xA0, 0x20 );
			data[0] = 0;
			data[1]++;
		}
	}
...
static void InitMovingBgValues( u16* movingBgs )
{
	...
	for( i = 0; i < 50; i++ )
	{
		...
		{
			movingBgs[i * 16 + j] =
				sUnknown_085B5884[sUnknown_085B58D9[i][j]];
		}
	}
}
...
static void InitMovingBackgroundTask( bool8 isLink )
{
	...
	if( !isLink )
	{
		...
		SetGpuReg( REG_OFFSET_DISPCNT,
			DISPCNT_OBJ_ON | DISPCNT_BG2_ON | DISPCNT_BG1_ON |
	...
	else
	{
		...
		SetGpuReg( REG_OFFSET_BLDALPHA, BLDALPHA_BLEND( 8, 8 ) );
		SetGpuReg( REG_OFFSET_DISPCNT,
			DISPCNT_OBJ_ON | DISPCNT_BG3_ON | DISPCNT_BG1_ON |
				DISPCNT_BG0_ON | DISPCNT_OBJ_1D_MAP );
	}
...
static void sub_8140174( void )
{
	...
	SetGpuReg( REG_OFFSET_DISPCNT,
		DISPCNT_OBJ_ON | DISPCNT_BG3_ON | DISPCNT_BG0_ON |
			DISPCNT_OBJ_1D_MAP );
	Free( sEvoMovingBgPtr );
}
...
static void EvoScene_DoMonAnimation( u8 monSpriteId, u16 speciesId )
{
	DoMonFrontSpriteAnimation(
		&gSprites[monSpriteId], speciesId, FALSE, 0 );
}
