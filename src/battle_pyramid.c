...
static bool8 TrySetPyramidEventObjectPositionInSquare(
	u8 arg0, u8* floorLayoutOffsets, u8 squareId, u8 eventObjectId );
static bool8 TrySetPyramidEventObjectPositionAtCoords( bool8 objType,
	u8 x,
	u8 y,
...
static const u16 sShortStreakRewardItems[] = {ITEM_HP_UP,
	ITEM_PROTEIN,
	ITEM_IRON,
	ITEM_CALCIUM,
	ITEM_CARBOS,
...
static void GetBattlePyramidData( void )
{
	...
	case 0:
		gSpecialVar_Result =
			gSaveBlock2Ptr->frontier.pyramidRewardItem;
		break;
	case 1:
		gSpecialVar_Result =
	...
	case 3:
		gSpecialVar_Result =
			gSaveBlock2Ptr->frontier
				.pyramidWinStreaks[FRONTIER_LVL_50];
		break;
	case 4:
		gSpecialVar_Result =
			gSaveBlock2Ptr->frontier
				.pyramidWinStreaks[FRONTIER_LVL_OPEN];
		break;
	case 5:
		gSpecialVar_Result =
			gSaveBlock2Ptr->frontier.field_CDC & 0x1000;
...
static void SetBattlePyramidRewardItem( void )
{
	...
	if( gSaveBlock2Ptr->frontier.pyramidWinStreaks[lvlMode] > 41 )
		gSaveBlock2Ptr->frontier.pyramidRewardItem =
			sLongStreakRewardItems[Random( ) %
				ARRAY_COUNT( sLongStreakRewardItems )];
	else
		gSaveBlock2Ptr->frontier.pyramidRewardItem =
...
static void GiveBattlePyramidRewardItem( void )
{
	if( AddBagItem( gSaveBlock2Ptr->frontier.pyramidRewardItem, 1 ) ==
		TRUE )
	{
		CopyItemName( gSaveBlock2Ptr->frontier.pyramidRewardItem,
			gStringVar1 );
...
static void SetPickupItem( void )
{
	...
	u32 round =
		( gSaveBlock2Ptr->frontier.pyramidWinStreaks[lvlMode] / 7 ) %
		TOTAL_ROUNDS;
	...
	itemIndex = ( gSpecialVar_LastTalked -
			    sPyramidFloorTemplates[id].numTrainers ) -
		1;
	...
	for( i = sPickupItemOffsets[floor];
		i < ARRAY_COUNT( sPickupItemSlots );
		i++ )
	{
	...
	if( lvlMode != FRONTIER_LVL_50 )
		gSpecialVar_0x8000 =
			sPickupItemsLvlOpen[round][sPickupItemSlots[i][1]];
	else
		gSpecialVar_0x8000 =
			sPickupItemsLvl50[round][sPickupItemSlots[i][1]];
...
static void HidePyramidItem( void )
{
	...
	for( ;; )
	{
		...
		{
			// Rather than using event flags to hide the item event
			// object, it moves them far off the map bounds.
			events[i].x = 0x7FFF;
			events[i].y = 0x7FFF;
			break;
		}
...
static void ShowPostBattleHintText( void )
{
	...
	struct EventObjectTemplate* events =
		gSaveBlock1Ptr->eventObjectTemplates;
	u16 trainerId = LocalIdToPyramidTrainerId(
		gEventObjects[gSelectedEventObject].localId );
	...
	hintType = sHintTextTypes[gEventObjects[gSelectedEventObject].localId -
		1];
	i        = 0;
	while( !i )
	{
		...
		case HINT_EXIT_DIRECTION:
			textIndex = GetPostBattleDirectionHintTextIndex(
				&hintType, 8, HINT_EXIT_DIRECTION );
			i = 1;
			break;
		case HINT_REMAINING_ITEMS:
			for( i = 0; i < GetNumBattlePyramidEventObjects( );
				i++ )
			{
				if( events[i].graphicsId ==
		...
		case HINT_EXIT_SHORT_REMAINING_TRAINERS:
			GetPostBattleDirectionHintTextIndex(
				&hintType, 8, HINT_REMAINING_TRAINERS );
			break;
		case HINT_EXIT_SHORT_REMAINING_ITEMS:
			GetPostBattleDirectionHintTextIndex(
				&hintType, 8, HINT_REMAINING_ITEMS );
			break;
		case HINT_EXIT_MEDIUM_REMAINING_TRAINERS:
			GetPostBattleDirectionHintTextIndex(
				&hintType, 16, HINT_REMAINING_TRAINERS );
			break;
		case HINT_EXIT_MEDIUM_REMAINING_ITEMS:
			GetPostBattleDirectionHintTextIndex(
				&hintType, 16, HINT_REMAINING_ITEMS );
			break;
		case HINT_EXIT_FAR_REMAINING_TRAINERS:
			GetPostBattleDirectionHintTextIndex(
				&hintType, 24, HINT_REMAINING_TRAINERS );
			break;
		case HINT_EXIT_FAR_REMAINING_ITEMS:
			GetPostBattleDirectionHintTextIndex(
				&hintType, 24, HINT_REMAINING_ITEMS );
			break;
		}
	}
...
static void ClearPyramidPartyHeldItems( void )
{
	...
	for( i = 0; i < PARTY_SIZE; i++ )
	{
		for( j = 0; j < 4; j++ )
		{
			if( gSaveBlock2Ptr->frontier.selectedPartyMons[j] !=
					0 &&
				gSaveBlock2Ptr->frontier.selectedPartyMons[j] -
						1 ==
					i )
...
static void Task_SetPyramidFloorPalette( u8 taskId )
{
	if( gPaletteFade.active )
	{
		CpuCopy16( gUnknown_08D856C8[gSaveBlock2Ptr->frontier
...
static void sub_81A9828( void ) { sub_809FDD4( ); }
...
bool8 GetBattlePyramidTrainerFlag( u8 eventId )
{
	return gSaveBlock2Ptr->frontier.pyramidTrainerFlags &
		gBitTable[gEventObjects[eventId].localId - 1];
}
...
void MarkApproachingPyramidTrainersAsBattled( void )
{
	...
	if( gBattleTypeFlags & BATTLE_TYPE_TWO_OPPONENTS )
	{
		gSelectedEventObject =
			GetChosenApproachingTrainerEventObjectId( 1 );
...
static void MarkPyramidTrainerAsBattled( u16 trainerId )
{
	...
	gEventObjects[gSelectedEventObject].movementType =
		MOVEMENT_TYPE_WANDER_AROUND;
	gSaveBlock1Ptr->eventObjectTemplates[gSpecialVar_LastTalked - 1]
		.movementType = MOVEMENT_TYPE_WANDER_AROUND;
	gEventObjects[gSelectedEventObject].initialCoords.x =
		gEventObjects[gSelectedEventObject].currentCoords.x;
	gEventObjects[gSelectedEventObject].initialCoords.y =
		gEventObjects[gSelectedEventObject].currentCoords.y;
}
...
void GenerateBattlePyramidWildMon( void )
{
	...
	u16 round = ( gSaveBlock2Ptr->frontier.pyramidWinStreaks[lvl] / 7 ) %
		TOTAL_ROUNDS;
	...
	SetMonData( &gEnemyParty[0],
		MON_DATA_EXP,
		&gExperienceTables[gBaseStats[wildMons[id].species].growthRate]
				  [lvl] );
	...
	case 1:
		SetMonData( &gEnemyParty[0],
			...
			&wildMons[id].abilityNum );
		break;
	case ABILITY_RANDOM:
	default:
		if( gBaseStats[wildMons[id].species].abilities[1] )
		{
			i = GetMonData( &gEnemyParty[0],
				    MON_DATA_PERSONALITY,
				    NULL ) %
				2;
			SetMonData(
				&gEnemyParty[0], MON_DATA_ABILITY_NUM, &i );
		}
		else
		{
			i = 0;
	...
	// BUG: Reading outside the array as lvl was used for mon level instead
	// of frontier lvl mode.
...
u8 InBattlePyramid( void )
{
	if( gMapHeader.mapLayoutId ==
		...
		return 1;
	else if( gMapHeader.mapLayoutId ==
		LAYOUT_BATTLE_FRONTIER_BATTLE_PYRAMID_TOP )
		return 2;
	else
		return FALSE;
}
	...
	for( i = 0; i < ARRAY_COUNT( sTrainerClassEncounterMusic ); i++ )
	{
		if( sTrainerClassEncounterMusic[i].trainerClass ==
			gFacilityClassToTrainerClass
				[gFacilityTrainers[trainerId].facilityClass] )
			return sTrainerClassEncounterMusic[i]
...
void sub_81A9F80( void )
{
	ScriptContext1_SetupScript(
		BattleFrontier_BattlePyramidEmptySquare_EventScript_252C88 );
}
...
void GenerateBattlePyramidFloorLayout(
	u16* backupMapData, bool8 setPlayerPosition )
{
	int y, x;
	int i;
	...
	for( i = 0; i < 16; i++ )
	{
		u16* map;
		int yOffset, xOffset;
		const struct MapLayout* mapLayout = gMapLayouts[floorLayoutOffsets
									[i] +
			LAYOUT_BATTLE_FRONTIER_BATTLE_PYRAMID_EMPTY_SQUARE];
		...
		map                     = backupMapData;
		yOffset = ( ( i / 4 * mapLayout->height ) + 7 ) *
		...
		for( y = 0; y < mapLayout->height; y++ )
		{
			...
			{
				if( ( layoutMap[x] & METATILE_ID_MASK ) !=
					FLOOR_EXIT_METATILE )
				{
					map[x] = layoutMap[x];
				}
				else if( i != exitSquareId )
				{
					...
					map[x] = ( layoutMap[x] & 0xFC00 ) |
						FLOOR_WALKABLE_METATILE;
				}
				else
				{
					map[x] = layoutMap[x];
...
void LoadBattlePyramidEventObjectTemplates( void )
{
	...
	for( i = 0; i < 2; i++ )
	{
		...
		if( i == OBJ_TRAINERS )
			objectPositionsType =
				sPyramidFloorTemplates[id].trainerPositions;
		else // OBJ_ITEMS
			objectPositionsType =
		...
		case OBJ_POSITIONS_NEAR_ENTRANCE:
			if( SetPyramidObjectPositionsNearSquare(
				    i, entranceSquareId ) )
				SetPyramidObjectPositionsUniformly( i );
			break;
...
static void GetPyramidEntranceAndExitSquareIds(
	u8* entranceSquareId, u8* exitSquareId )
{
	...
	*exitSquareId     = gSaveBlock2Ptr->frontier.pyramidRandoms[0] % 16;
	...
	{
		*entranceSquareId =
			( gSaveBlock2Ptr->frontier.pyramidRandoms[3] + 1 ) %
			16;
		*exitSquareId =
			( gSaveBlock2Ptr->frontier.pyramidRandoms[0] + 15 ) %
			16;
	}
}
...
static void SetPyramidObjectPositionsUniformly( u8 objType )
{
	...
	for( i = 0; i < numObjects; i++ )
	{
		do
		{
			...
			TrySetPyramidEventObjectPositionInSquare( objType,
				floorLayoutOffsets,
				squareId,
				objectStartIndex + i ) );
		bits &= 1;
...
static bool8 SetPyramidObjectPositionsInAndNearSquare(
	u8 objType, u8 squareId )
{
	int i;
	int objectStartIndex;
	int borderedIndex    = 0;
	...
	for( i = 0; i < numObjects; i++ )
	{
		if( r7 == 0 )
		{
			if( TrySetPyramidEventObjectPositionInSquare( objType,
				    floorLayoutOffsets,
				    squareId,
				    objectStartIndex + i ) )
				r7 = 1;
...
static bool8 SetPyramidObjectPositionsNearSquare( u8 objType, u8 squareId )
{
	...
	for( i = 0; i < numObjects; i++ )
	{
		...
		{
			borderOffset++;
			if( sBorderedSquareIds[squareId][borderOffset] ==
					0xFF ||
				borderOffset >= 4 )
				borderOffset = 0;
			numPlacedObjects++;
...
static bool8 TrySetPyramidEventObjectPositionInSquare(
	u8 objType, u8* floorLayoutOffsets, u8 squareId, u8 eventObjectId )
{
	int x, y;
	...
	if( gSaveBlock2Ptr->frontier.pyramidRandoms[0] & 1 )
	{
		for( y = 7; y > -1; y-- )
		{
			for( x = 7; x > -1; x-- )
			{
				if( !TrySetPyramidEventObjectPositionAtCoords(
					    objType,
					    x,
					    y,
					    floorLayoutOffsets,
...
static bool8 TrySetPyramidEventObjectPositionAtCoords( u8 objType,
	u8 x,
	u8 y,
	u8* floorLayoutOffsets,
	u8 squareId,
	u8 eventObjectId )
{
	...
	struct EventObjectTemplate* floorEvents =
		gSaveBlock1Ptr->eventObjectTemplates;

	mapHeader = Overworld_GetMapHeaderByGroupAndId(
		MAP_GROUP( BATTLE_PYRAMID_SQUARE01 ),
		floorLayoutOffsets[squareId] +
			MAP_NUM( BATTLE_PYRAMID_SQUARE01 ) );
	for( i = 0; i < mapHeader->events->eventObjectCount; i++ )
	{
		if( mapHeader->events->eventObjects[i].x != x ||
			mapHeader->events->eventObjects[i].y != y )
			continue;

		if( objType != OBJ_TRAINERS ||
			mapHeader->events->eventObjects[i].graphicsId ==
				EVENT_OBJ_GFX_ITEM_BALL )
		{
			if( objType != OBJ_ITEMS ||
				mapHeader->events->eventObjects[i]
						.graphicsId !=
					EVENT_OBJ_GFX_ITEM_BALL )
				continue;
		}

		// Ensure an object wasn't previously placed in the exact same
		// position.
		...
		if( j == eventObjectId )
		{
			...
			if( floorEvents[eventObjectId].graphicsId !=
				EVENT_OBJ_GFX_ITEM_BALL )
			{
				i = GetUniqueTrainerId( eventObjectId );
				floorEvents[eventObjectId].graphicsId =
					GetBattleFacilityTrainerGfxId( i );
				gSaveBlock2Ptr->frontier
					.trainerIds[eventObjectId] = i;
			}
			return FALSE;
		}
	}
...
static void GetPyramidFloorLayoutOffsets( u8* layoutOffsets )
{
	...
	{
		layoutOffsets[i] =
			sPyramidFloorTemplates[id].layoutOffsets[rand & 0x7];
		rand >>= 3;
		if( i == 7 )
		{
...
static u8 GetPyramidFloorTemplateId( void )
{
	...
	for( i = sFloorTemplateOffsets[floor];
		i < ARRAY_COUNT( sPyramidFloorTemplateOptions );
		i++ )
	{
...
static void InitPyramidBagItems( u8 lvlMode )
{
	...
	{
		gSaveBlock2Ptr->frontier.pyramidBag.itemId[lvlMode][i] =
			ITEM_NONE;
		gSaveBlock2Ptr->frontier.pyramidBag.quantity[lvlMode][i] =
			ITEM_NONE;
	}
