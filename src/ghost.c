...
static void sub_811160C( struct Sprite* sprite )
{
	...
	sprite->data[6]  = 16;
	SetGpuReg(
		REG_OFFSET_BLDCNT, ( BLDCNT_EFFECT_BLEND | BLDCNT_TGT2_ALL ) );
...
static void sub_8111764( struct Sprite* sprite )
{
	...
	if( ( r0 & 0xFF ) == 0 )
	{
		...
		SetGpuReg( REG_OFFSET_BLDALPHA,
			BLDALPHA_BLEND(
				sprite->data[6], 16 - sprite->data[6] ) );
...
static void sub_8111914( u8 taskId )
{
	...
	if( gTasks[taskId].data[10] == 3 )
	{
		...
		SetGpuReg( REG_OFFSET_BLDALPHA,
			BLDALPHA_BLEND( gTasks[taskId].data[2],
...
static void InitAnimShadowBall( struct Sprite* sprite )
{
	...
	sprite->data[6] = ( ( oldPosX - sprite->pos1.x ) << 4 ) /
		( gBattleAnimArgs[0] << 1 );
	sprite->data[7] = ( ( oldPosY - sprite->pos1.y ) << 4 ) /
		( gBattleAnimArgs[0] << 1 );
...
void sub_8111C50( u8 taskId )
{
	...
	StoreSpriteCallbackInData6(
		&gSprites[task->data[0]], SpriteCallbackDummy );
...
static void sub_8111E78( u8 taskId )
{
	...
	case 0:
		...
		else
		{
			...
			{
				s16 mask2;
				gSprites[task->data[0]].oam.paletteNum =
					task->data[14];
				gSprites[task->data[0]].oam.objMode =
					ST_OAM_OBJ_NORMAL;
	...
	case 1:
		...
		CpuSet( &gPlttBufferUnfaded[task->data[4]],
			&gPlttBufferFaded[task->data[14]],
			0x4000008 );
		BlendPalette( task->data[4], 16, 10, RGB( 13, 0, 15 ) );
		task->data[15]++;
		break;
	case 2:
		startLine = gSprites[task->data[13]].pos1.y +
			gSprites[task->data[13]].pos2.y - 32;
		if( startLine < 0 )
			startLine = 0;

		if( position == 1 )
			task->data[10] = ScanlineEffect_InitWave(
				startLine, startLine + 64, 2, 6, 0, 4, 1 );
		else
			task->data[10] = ScanlineEffect_InitWave(
				startLine, startLine + 64, 2, 6, 0, 8, 1 );
...
static void sub_8112264( struct Sprite* sprite )
{
	...
	if( gBattleAnimArgs[0] == 0 )
	{
		...
		battler1Y =
			GetBattlerSpriteCoord( gBattleAnimAttacker, 1 ) + 28;
	...
	sprite->data[2] =
		( ( battler2X - battler1X ) * 16 ) / gBattleAnimArgs[1];
...
void sub_81123C4( u8 taskId )
{
	...
	baseY = GetBattlerSpriteCoordAttr(
		gBattleAnimAttacker, BATTLER_COORD_ATTR_BOTTOM );
	if( !IsContest( ) )
	{
		for( battler = 0; battler < 4; battler++ )
		{
				...
				IsBattlerSpriteVisible( battler ) )
			{
				spriteId = CreateSprite(
					&gUnknown_08596DB8, baseX, baseY, 55 );
				if( spriteId != MAX_SPRITES )
				{
					x = GetBattlerSpriteCoord(
						battler, 2 );
					y = GetBattlerSpriteCoordAttr( battler,
						BATTLER_COORD_ATTR_BOTTOM );
					gSprites[spriteId].data[0] = baseX
						<< 4;
					gSprites[spriteId].data[1] = baseY
						<< 4;
	...
	{
		spriteId =
			CreateSprite( &gUnknown_08596DB8, baseX, baseY, 55 );
		if( spriteId != MAX_SPRITES )
		{
			x                          = 48;
			y                          = 40;
			...
			gSprites[spriteId].data[2] =
				( ( x - baseX ) << 4 ) / gBattleAnimArgs[1];
			gSprites[spriteId].data[3] =
				( ( y - baseY ) << 4 ) / gBattleAnimArgs[1];
	...
	SetGpuReg( REG_OFFSET_WINOUT,
		( ( WINOUT_WIN01_BG_ALL | WINOUT_WIN01_OBJ ) |
			( WINOUT_WINOBJ_BG_ALL | WINOUT_WINOBJ_OBJ |
				WINOUT_WINOBJ_CLR ) ) );
	SetGpuReg( REG_OFFSET_BLDCNT,
		( BLDCNT_TGT1_BG3 | BLDCNT_EFFECT_DARKEN ) );
...
static void sub_811280C( u8 taskId )
{
	...
	else
	{
		...
		BeginNormalPaletteFade(
			selectedPalettes, 0, 16, 16, RGB( 0, 0, 0 ) );
...
static void sub_8112D10( u8 taskId )
{
	...
	case 0:
		for( i = 0; i < 6; i++ )
		{
			spriteId = CreateSprite( &gUnknown_08596E30,
				task->data[9],
				task->data[10],
				task->data[6] );
			if( spriteId != MAX_SPRITES )
...
static void sub_8112FB8( struct Sprite* sprite )
{
	...
	{
		sprite->data[3]--;
		gSprites[sprite->data[5]].pos2.x =
			Sin( sprite->data[0], sprite->data[1] );
		gSprites[sprite->data[5]].pos2.y =
			Cos( sprite->data[0], sprite->data[1] );
		sprite->data[0] += sprite->data[2];
