...
int ProcessPlayerFieldInput( struct FieldInput* input )
{
	...
	if( input->tookStep )
	{
		...
		if( TryStartStepBasedScript( &position,
			    metatileBehavior,
	...
	if( input->pressedAButton &&
		TryStartInteractionScript( &position,
			metatileBehavior,
			playerDirection ) == TRUE )
		return TRUE;
...
{
	const u8* script = GetInteractedEventObjectScript(
		position, metatileBehavior, direction );
	if( script != NULL )
		return script;

	script = GetInteractedBackgroundEventScript(
		position, metatileBehavior, direction );
	if( script != NULL )
		return script;

	script = GetInteractedMetatileScript(
		position, metatileBehavior, direction );
	if( script != NULL )
		return script;

	script = GetInteractedWaterScript(
		position, metatileBehavior, direction );
	if( script != NULL )
		return script;
	...
	struct MapPosition* position, u8 metatileBehavior, u8 direction )
{
	...
	if( !MetatileBehavior_IsCounter( MapGridGetMetatileBehaviorAt(
		...
		eventObjectId = GetEventObjectIdByXYZ(
	...
	if( eventObjectId == EVENT_OBJECTS_COUNT ||
		gEventObjects[eventObjectId].localId == EVENT_OBJ_ID_PLAYER )
		return NULL;

	for( i = 0; i < 4; i++ )
	{
		if( gLinkPlayerEventObjects[i].active == TRUE &&
...
static const u8* GetInteractedEventObjectScript(
	struct MapPosition* position, u8 metatileBehavior, u8 direction )
{
	u8 eventObjectId;
	const u8* script;
	...
	if( InTrainerHill( ) == TRUE )
		...
		script = GetEventObjectScriptPointerByEventObjectId(
			eventObjectId );
...
static const u8* GetInteractedBackgroundEventScript(
	struct MapPosition* position, u8 metatileBehavior, u8 direction )
{
	struct BgEvent* bgEvent = GetBackgroundEventAtPosition( &gMapHeader,
		position->x - 7,
		position->y - 7,
		position->height );
	...
	struct MapPosition* position, u8 metatileBehavior, u8 direction )
{
	...
	if( MetatileBehavior_IsPlayerFacingTVScreen(
	...
	if( MetatileBehavior_IsCableBoxResults2(
		    metatileBehavior, direction ) == TRUE )
		return EventScript_CableBoxResults;
	...
	if( height == MapGridGetZCoordAt( position->x, position->y ) )
	{
		...
		if( MetatileBehavior_IsRecordMixingSecretBasePC(
		...
		if( MetatileBehavior_IsSecretBaseShieldOrToyTV(
			    metatileBehavior ) == TRUE )
...
{
	u8* script = GetCoordEventScriptAtPosition( &gMapHeader,
		position->x - 7,
		position->y - 7,
		position->height );
...
static bool8 TryStartMiscWalkingScripts( u16 metatileBehavior )
{
	...
	else if( MetatileBehavior_IsBattlePyramidWarp( metatileBehavior ) )
	{
		ScriptContext1_SetupScript(
			BattleFrontier_BattlePyramidEmptySquare_EventScript_252BE8 );
		return TRUE;
	}
...
static bool8 TryStartStepCountScript( u16 metatileBehavior )
{
		...
		!MetatileBehavior_IsForcedMovementTile( metatileBehavior ) )
	{
		...
		{
			ScriptContext1_SetupScript(
				UnusualWeather_EventScript_EndEventAndCleanup_1 );
			return TRUE;
		}
		if( ShouldDoBrailleRegicePuzzle( ) == TRUE )
		{
			ScriptContext1_SetupScript(
				IslandCave_EventScript_238EAF );
			return TRUE;
		}
		if( ShouldDoWallyCall( ) == TRUE )
		{
			ScriptContext1_SetupScript(
				MauvilleCity_EventScript_1DF7BA );
			return TRUE;
		}
		if( ShouldDoWinonaCall( ) == TRUE )
		{
			ScriptContext1_SetupScript(
				Route119_EventScript_1F49EC );
			return TRUE;
		}
		if( ShouldDoScottCall( ) == TRUE )
		{
			ScriptContext1_SetupScript(
				LittlerootTown_ProfessorBirchsLab_EventScript_1FA4D6 );
			return TRUE;
		}
		if( ShouldDoRoxanneCall( ) == TRUE )
		{
			ScriptContext1_SetupScript(
				RustboroCity_Gym_EventScript_21307B );
			return TRUE;
		}
		if( ShouldDoRivalRayquazaCall( ) == TRUE )
		{
			ScriptContext1_SetupScript(
				MossdeepCity_SpaceCenter_2F_EventScript_224175 );
			return TRUE;
		}
	}
	...
	{
		ScriptContext1_SetupScript(
			SSTidalCorridor_EventScript_23C050 );
		return TRUE;
	}
...
static bool8 CheckStandardWildEncounter( u16 metatileBehavior )
{
	...
	if( StandardWildEncounter( metatileBehavior,
...
static bool8 TryStartWarpEventScript(
	struct MapPosition* position, u16 metatileBehavior )
{
	s8 warpEventId = GetWarpEventAtMapPosition( &gMapHeader, position );
...
static s8 GetWarpEventAtMapPosition(
	struct MapHeader* mapHeader, struct MapPosition* position )
{
	return GetWarpEventAtPosition( mapHeader,
		position->x - 7,
		position->y - 7,
		position->height );
}

static void SetupWarp( struct MapHeader* unused,
	s8 warpEventId,
	struct MapPosition* position )
{
	...
	else
	{
		...
		SetWarpDestinationToMapWarp( warpEvent->mapGroup,
			warpEvent->mapNum,
			warpEvent->warpId );
		UpdateEscapeWarp( position->x, position->y );
		mapHeader = Overworld_GetMapHeaderByGroupAndId(
			warpEvent->mapGroup, warpEvent->mapNum );
		if( mapHeader->events->warps[warpEvent->warpId].mapNum ==
			MAP_NUM( NONE ) )
			SetDynamicWarp(
				mapHeader->events->warps[warpEventId].warpId,
	...
	struct MapPosition* position, u16 metatileBehavior, u8 direction )
{
	...
	if( direction == DIR_NORTH )
	{
		...
		if( MetatileBehavior_IsWarpDoor( metatileBehavior ) == TRUE )
		{
			warpEventId = GetWarpEventAtMapPosition(
				&gMapHeader, position );
			if( warpEventId != -1 &&
					...
					TRUE )
			{
				StoreInitialPlayerAvatarState( );
				SetupWarp(
					&gMapHeader, warpEventId, position );
				DoDoorWarp( );
				return TRUE;
			}
		}
...
static s8 GetWarpEventAtPosition(
...
static u8* GetCoordEventScriptAtPosition(
	struct MapHeader* mapHeader, u16 x, u16 y, u8 elevation )
{
	s32 i;
...
{
	return GetCoordEventScriptAtPosition( &gMapHeader,
		position->x - 7,
		position->y - 7,
		position->height );
}
...
const u8* GetEventObjectScriptPointerPlayerFacing( void )
{
	...
	GetInFrontOfPlayerPosition( &position );
	return GetInteractedEventObjectScript( &position,
		MapGridGetMetatileBehaviorAt( position.x, position.y ),
		direction );
}
...
int SetCableClubWarp( void )
{
	...
	SetupWarp( &gMapHeader,
		GetWarpEventAtMapPosition( &gMapHeader, &position ),
		&position );
	return 0;
}
