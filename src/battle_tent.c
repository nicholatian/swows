...
// const rom data
void static ( *const gUnknown_086160B4[] )( void ) = {sub_81B99D4,
	sub_81B9A28,
	sub_81B9A44,
	sub_81B9A60,
	sub_81B9A90,
...
void static ( *const gUnknown_086160D8[] )( void ) = {sub_81B9BA0,
	sub_81B9BF4,
	sub_81B9C10,
	sub_81B9C2C,
	sub_81B9C70,
...
void static ( *const gUnknown_086160F8[] )( void ) = {sub_81B9D28,
	sub_81B9D7C,
	sub_81B9D98,
	sub_81B9DB4,
	sub_81B9DF8,
...
// code
void sub_81B99B4( void ) { gUnknown_086160B4[gSpecialVar_0x8004]( ); }
...
static void sub_81B9B00( void )
{
	gSaveBlock2Ptr->frontier.field_E6A = sVerdanturfTentRewards[Random( ) %
		ARRAY_COUNT( sVerdanturfTentRewards )];
}
...
static void sub_81B9B28( void )
{
	...
	{
		CopyItemName(
			gSaveBlock2Ptr->frontier.field_E6A, gStringVar1 );
...
void sub_81B9B80( void ) { gUnknown_086160D8[gSpecialVar_0x8004]( ); }
...
static void sub_81B9C70( void )
{
	gSaveBlock2Ptr->frontier.field_E6C = sFallarborTentRewards[Random( ) %
		ARRAY_COUNT( sFallarborTentRewards )];
}
...
static void sub_81B9C98( void )
{
	...
	{
		CopyItemName(
			gSaveBlock2Ptr->frontier.field_E6C, gStringVar1 );
...
void sub_81B9D08( void ) { gUnknown_086160F8[gSpecialVar_0x8004]( ); }
...
static void sub_81B9DF8( void )
{
	gSaveBlock2Ptr->frontier.field_E6E = sSlateportTentRewards[Random( ) %
		ARRAY_COUNT( sSlateportTentRewards )];
}
...
static void sub_81B9E20( void )
{
	...
	{
		CopyItemName(
			gSaveBlock2Ptr->frontier.field_E6E, gStringVar1 );
...
static void sub_81B9E88( void ) { DoBattleFactorySwapScreen( ); }
...
static void sub_81BA040( void )
{
	u16 trainerId;
	s32 i, j, k;
	register const u16* monSets asm(
		"r9" ); // Fix me. Compiler insists on
			// moving that variable into stack.
	u16 species[3];
	u16 heldItems[3];
	s32 setsCount = 0;
	...
	while( 1 )
	{
		...
		while( gFacilityTrainers[gTrainerBattleOpponent_A]
				.monSets[setsCount] != 0xFFFF )
			setsCount++;
		if( setsCount > 8 )
			break;
		setsCount = 0;
	...
	while( i != 3 )
	{
		...
		{
			if( species[k] ==
				gFacilityTrainerMons[sRandMonSetId].species )
				break;
		}
		if( k != i )
			continue;
		...
		for( k = 0; k < i; k++ )
		{
			if( heldItems[k] != 0 &&
				heldItems[k] ==
					gBattleFrontierHeldItems
						[gFacilityTrainerMons[sRandMonSetId]
								.itemTableId] )
				break;
		}
		if( k != i )
		...
		heldItems[i] = gBattleFrontierHeldItems
			[gFacilityTrainerMons[sRandMonSetId].itemTableId];
