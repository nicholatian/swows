...
static const struct ListMenuItem sListMenuItems_Receive[] = {
	{gText_Receive, 0}, {gText_Cancel2, -2}};
...
bool32 HandleMysteryGiftOrEReaderSetup( s32 mg_or_ereader )
{
	...
	case 0:
		...
		InitBgsFromTemplates(
			0, sBGTemplates, ARRAY_COUNT( sBGTemplates ) );
		ChangeBgX( 0, 0, 0 );
		ChangeBgY( 0, 0, 0 );
		ChangeBgX( 1, 0, 0 );
		ChangeBgY( 1, 0, 0 );
...
void PrintMysteryGiftOrEReaderTopMenu(
	bool8 mg_or_ereader, bool32 usePickOkCancel )
{
	const u8* header;
	const u8* options;
	...
	if( mg_or_ereader == 0 )
	{
		...
		options = !usePickOkCancel ? gText_PickOKExit
					   : gText_PickOKCancel;
	}
	else
	{
	...
	AddTextPrinterParameterized4(
		0, 1, 4, 1, 0, 0, sMG_Ereader_TextColor_1, -1, header );
...
void AddTextPrinterToWindow1( const u8* str )
{
	...
	AddTextPrinterParameterized4(
		1, 1, 0, 1, 0, 0, sMG_Ereader_TextColor_2, 0, gStringVar4 );
...
static u32 MysteryGift_HandleThreeOptionMenu(
	u8* unused0, u16* unused1, u8 whichMenu )
{
	...
	response = DoMysteryGiftListMenu(
		&windowTemplate, &listMenuTemplate, 1, 0x00A, 0xE0 );
	if( response != -1 )
	{
	...
	case 0:
		if( cannotToss == 0 )
		{
			StringExpandPlaceholders(
				gStringVar4, gText_WhatToDoWithCards );
		}
		else
		{
			StringExpandPlaceholders(
				gStringVar4, gText_WhatToDoWithNews );
		}
...
static bool32 HandleLoadWonderCardOrNews( u8* state, bool32 cardOrNews )
{
	...
	case 0:
		if( cardOrNews == 0 )
		{
			InitWonderCardResources( GetSavedWonderCard( ),
...
static bool32 TearDownCardOrNews_ReturnToTopMenu(
	bool32 cardOrNews, bool32 arg1 )
{
	if( cardOrNews == 0 )
	{
...
static s32 mevent_message_prompt_discard(
	u8* textState, u16* windowId, bool32 cardOrNews )
{
	if( cardOrNews == 0 )
	{
		return mevent_message_print_and_prompt_yes_no( textState,
			windowId,
			TRUE,
...
static bool32 mevent_message_was_thrown_away(
	u8* textState, bool32 cardOrNews )
{
	if( cardOrNews == 0 )
	{
		return MG_PrintTextOnWindow1AndWaitButton(
			textState, gText_WonderCardThrownAway );
	}
	else
	{
		return MG_PrintTextOnWindow1AndWaitButton(
			textState, gText_WonderNewsThrownAway );
	}
}
...
static const u8* mevent_message( u32* a0, u8 a1, u8 cardOrNews, u32 msgId )
{
	...
	*a0           = 0;
	...
	case 2:
		*a0 = 1;
		msg = cardOrNews == 0 ? gText_WonderCardReceived
				      : gText_WonderCardReceivedFrom;
		break;
	case 3:
		*a0 = 1;
		msg = cardOrNews == 0 ? gText_WonderNewsReceived
	...
	case 10:
		*a0 = 0;
		msg = a1 == 0 ? gText_CantAcceptCardFromTrainer
...
static const u8* mevent_message_stamp_card_etc_send_status(
	u32* a0, u8 unused, u32 msgId )
{
...
{
	u32 flag;
	const u8* str = mevent_message_stamp_card_etc_send_status(
		&flag, arg2, msgId );
	if( flag )
	{
...
void task00_mystery_gift( u8 taskId )
{
	...
	case 4:
		switch( MysteryGift_HandleThreeOptionMenu(
			&data->textState, &data->curPromptWindowId, TRUE ) )
		{
		case 0:
			ClearTextWindow( );
			data->state  = 5;
	...
	case 8:
		...
		break;
	case 9:
		switch( (u32)mevent_message_print_and_prompt_yes_no(
			&data->textState,
	...
	case 10:
		if( MG_PrintTextOnWindow1AndWaitButton(
			    &data->textState, mevent_client_get_buffer( ) ) )
		{
			mevent_client_inc_flag( );
			data->state = 7;
		}
	...
	case 14:
		if( PrintStringAndWait2Seconds(
			    &data->textState, gText_CommunicationCompleted ) )
		{
			if( data->source == 1 )
			{
				StringCopy(
					gStringVar1, gLinkPlayers[0].name );
			}
			data->state = 15;
		}
		break;
	case 15: {
		...
		if( sp0 )
		{
			flag = PrintMGSuccessMessage( &data->textState,
				r1,
				&data->curPromptWindowId );
		}
		else
		{
			flag = MG_PrintTextOnWindow1AndWaitButton(
				&data->textState, r1 );
		}
		if( flag )
		{
	...
	case 18:
		if( HandleLoadWonderCardOrNews(
			    &data->textState, data->IsCardOrNews ) )
		{
			data->state = 20;
		}
		break;
	...
	case 22:
		switch( mevent_message_prompt_discard( &data->textState,
			&data->curPromptWindowId,
			data->IsCardOrNews ) )
		{
		case 0:
			if( data->IsCardOrNews == 0 &&
				CheckReceivedGiftFromWonderCard( ) == TRUE )
			{
				data->state = 23;
			}
			else
	...
	case 23:
		...
		break;
	case 24:
		if( TearDownCardOrNews_ReturnToTopMenu(
			    data->IsCardOrNews, 1 ) )
		{
	...
	case 25:
		...
		break;
	case 26:
		if( mevent_message_was_thrown_away(
			    &data->textState, data->IsCardOrNews ) )
		{
			data->state = 0;
			PrintMysteryGiftOrEReaderTopMenu( 0, 0 );
	...
	case 31: {
		...
		break;
	case 32:
		if( mevent_srv_common_do_exec( &data->curPromptWindowId ) ==
			3 )
		{
			data->prevPromptWindowId = data->curPromptWindowId;
			data->state              = 33;
...
void bgid_upload_textbox_1( u8 bgId )
{
	DecompressAndLoadBgGfxUsingHeap(
		bgId, gUnkTextboxBorderGfx, 0x100, 0, 0 );
}
