...
static void SetInitialEggData(
	struct Pokemon* mon, u16 species, struct DayCare* daycare );
...
static const u8 sJapaneseEggNickname[] = _( "タマゴ" ); // "tamago" ("egg" in
							// Japanese)
	...
	struct DayCare* daycare, struct RecordMixingDayCareMail* daycareMail )
{
	...
	for( i = 0; i < DAYCARE_MON_COUNT; i++ )
	{
		...
		{
			numDaycareMons++;
			if( GetBoxMonData( &daycare->mons[i].mon,
...
static void StorePokemonInDaycare(
	struct Pokemon* mon, struct DaycareMon* daycareMon )
{
	if( MonHasMail( mon ) )
	{
		u8 mailId;
		...
		StringCopy(
			daycareMon->mail.OT_name, gSaveBlock2Ptr->playerName );
...
static void StorePokemonInEmptyDaycareSlot(
	struct Pokemon* mon, struct DayCare* daycare )
{
...
void StoreSelectedPokemonInDaycare( void )
{
	...
	StorePokemonInEmptyDaycareSlot(
		&gPlayerParty[monId], &gSaveBlock1Ptr->daycare );
}
...
static void ApplyDaycareExperience( struct Pokemon* mon )
{
	...
	{
		// Add the mon's gained daycare experience level by level until
		// it can't level up anymore.
		if( TryIncrementMonLevel( mon ) )
		{
			// Teach the mon new moves it learned while in the
			// daycare.
			firstMove = TRUE;
			while( ( learnedMove = MonTryLearningNewMove(
					 mon, firstMove ) ) != 0 )
			{
				firstMove = FALSE;
...
static u16 TakeSelectedPokemonFromDaycare( struct DaycareMon* daycareMon )
{
	...
	{
		GiveMailToMon2( &gPlayerParty[PARTY_SIZE - 1],
...
static u16 TakeSelectedPokemonMonFromDaycareShiftSlots(
	struct DayCare* daycare, u8 slotId )
{
...
static u8 GetNumLevelsGainedFromSteps( struct DaycareMon* daycareMon )
{
	...
	levelAfter  = GetLevelAfterDaycareSteps(
                &daycareMon->mon, daycareMon->steps );
...
static u8 GetNumLevelsGainedForDaycareMon( struct DaycareMon* daycareMon )
{
	...
	ConvertIntToDecimalStringN(
		gStringVar2, numLevelsGained, STR_CONV_MODE_LEFT_ALIGN, 2 );
	GetBoxMonNick( &daycareMon->mon, gStringVar1 );
...
static u32 GetDaycareCostForSelectedMon( struct DaycareMon* daycareMon )
{
	...
	ConvertIntToDecimalStringN(
		gStringVar2, cost, STR_CONV_MODE_LEFT_ALIGN, 5 );
	return cost;
}
...
{
	if( GetBoxMonData( &gSaveBlock1Ptr->daycare.mons[gSpecialVar_0x8004],
		    MON_DATA_SPECIES ) != 0 )
...
static u16 GetEggSpecies( u16 species )
{
	...
	// Working backwards up to 5 times seems arbitrary, since the maximum
	// number of times would only be 3 for 3-stage evolutions.
	for( i = 0; i < EVOS_PER_MON; i++ )
	{
		...
		for( j = 1; j < NUM_SPECIES; j++ )
		{
			...
			{
				if( gEvolutionTable[j][k].targetSpecies ==
					species )
				{
					species = j;
					found   = TRUE;
...
static s32 GetSlotToInheritNature( struct DayCare* daycare )
{
	...
	{
		species[i] = GetBoxMonData(
			&daycare->mons[i].mon, MON_DATA_SPECIES );
	...
	if( GetBoxMonData( &daycare->mons[slot].mon, MON_DATA_HELD_ITEM ) !=
			ITEM_EVERSTONE ||
		Random( ) >= USHRT_MAX / 2 )
	{
		return -1;
...
static void _TriggerPendingDaycareEgg( struct DayCare* daycare )
{
	...
	if( natureSlot < 0 )
	{
		daycare->offspringPersonality = ( Random2( ) << 0x10 ) |
			( ( Random( ) % 0xfffe ) + 1 );
	}
	else
	{
		u8 wantedNature = GetNatureFromPersonality(
			GetBoxMonData( &daycare->mons[natureSlot].mon,
		...
		do
		{
			...
			if( wantedNature ==
					GetNatureFromPersonality(
						personality ) &&
				personality != 0 )
				break; // we found a personality with the same
				       // nature
...
static void InheritIVs( struct Pokemon* egg, struct DayCare* daycare )
{
	...
	for( i = 0; i < ARRAY_COUNT( selectedIvs ); i++ )
	{
		...
		{
		case 0:
			iv = GetBoxMonData( &daycare->mons[whichParent[i]].mon,
				MON_DATA_HP_IV );
...
static u8 GetEggMoves( struct Pokemon* pokemon, u16* eggMoves )
{
	...
	for( i = 0; i < EGG_MOVES_ARRAY_COUNT; i++ )
	{
		...
		{
			// TODO: the curly braces around this if statement are
			// required for a matching build.
			break;
		}
...
static void BuildEggMoveset( struct Pokemon* egg,
	struct BoxPokemon* father,
	struct BoxPokemon* mother )
{
	...
	numLevelUpMoves = GetLevelUpMovesBySpecies(
		GetMonData( egg, MON_DATA_SPECIES ), sHatchedEggLevelUpMoves );
	for( i = 0; i < MAX_MON_MOVES; i++ )
	{
		sHatchedEggFatherMoves[i] =
			GetBoxMonData( father, MON_DATA_MOVE1 + i );
		sHatchedEggMotherMoves[i] =
			GetBoxMonData( mother, MON_DATA_MOVE1 + i );
	}
	...
	for( i = 0; i < MAX_MON_MOVES; i++ )
	{
		if( sHatchedEggFatherMoves[i] != MOVE_NONE )
		{
			...
			{
				if( sHatchedEggFatherMoves[i] ==
					sHatchedEggEggMoves[j] )
				{
					if( GiveMoveToMon( egg,
	...
	for( i = 0; i < MAX_MON_MOVES; i++ )
	{
		...
		for( j = 0; j < numLevelUpMoves; j++ )
		{
			if( sHatchedEggLevelUpMoves[j] != MOVE_NONE &&
				sHatchedEggFinalMoves[i] ==
					sHatchedEggLevelUpMoves[j] )
			{
				if( GiveMoveToMon(
					    egg, sHatchedEggFinalMoves[i] ) ==
					0xFFFF )
					DeleteFirstMoveAndGiveMoveToMon( egg,
						sHatchedEggFinalMoves[i] );
				break;
			}
		}
...
static void AlterEggSpeciesWithIncenseItem(
	u16* species, struct DayCare* daycare )
{
	u16 motherItem, fatherItem;
	if( *species == SPECIES_WYNAUT || *species == SPECIES_AZURILL )
	{
		motherItem = GetBoxMonData(
			&daycare->mons[0].mon, MON_DATA_HELD_ITEM );
		fatherItem = GetBoxMonData(
			&daycare->mons[1].mon, MON_DATA_HELD_ITEM );
...
static void GiveVoltTackleIfLightBall(
	struct Pokemon* mon, struct DayCare* daycare )
{
	u32 motherItem =
		GetBoxMonData( &daycare->mons[0].mon, MON_DATA_HELD_ITEM );
	u32 fatherItem =
		GetBoxMonData( &daycare->mons[1].mon, MON_DATA_HELD_ITEM );

	if( motherItem == ITEM_LIGHT_BALL || fatherItem == ITEM_LIGHT_BALL )
	{
		if( GiveMoveToMon( mon, MOVE_VOLT_TACKLE ) == 0xFFFF )
			DeleteFirstMoveAndGiveMoveToMon(
				mon, MOVE_VOLT_TACKLE );
	}
}

static u16 DetermineEggSpeciesAndParentSlots(
	struct DayCare* daycare, u8* parentSlots )
{
	u16 i;
	u16 species[2];
	u16 eggSpecies;

	// Determine which of the daycare mons is the mother and father of the
	// egg. The 0th index of the parentSlots array is considered the mother
	// slot, and the 1st index is the father slot.
	for( i = 0; i < 2; i++ )
	{
		species[i] = GetBoxMonData(
			&daycare->mons[i].mon, MON_DATA_SPECIES );
		...
		else if( GetBoxMonGender( &daycare->mons[i].mon ) ==
			MON_FEMALE )
		{
			parentSlots[0] = i;
			parentSlots[1] = i ^ 1;
	...
	if( eggSpecies == SPECIES_NIDORAN_F &&
		daycare->offspringPersonality & 0x8000 )
	{
	...
	if( species[parentSlots[1]] == SPECIES_DITTO &&
		GetBoxMonGender( &daycare->mons[parentSlots[0]].mon ) !=
			MON_FEMALE )
	{
...
static void _GiveEggFromDaycare( struct DayCare* daycare ) // give_egg
{
	struct Pokemon egg;
	u16 species;
	u8 parentSlots[2]; // 0th index is "mother" daycare slot, 1st is
			   // "father"
	bool8 isEgg;
	...
	BuildEggMoveset( &egg,
		&daycare->mons[parentSlots[1]].mon,
...
void CreateEgg( struct Pokemon* mon, u16 species, bool8 setHotSpringsLocation )
{
	...
	{
		metLocation = 253; // hot springs; see
...
static bool8 _DoEggActions_CheckHatch( struct DayCare* daycare )
{
	...
	if( daycare->offspringPersonality == 0 && validEggs == 2 &&
	...
	if( ++daycare->stepCounter == 255 ) // hatch an egg
	{
		...
		for( i = 0; i < gPlayerPartyCount; i++ )
		{
			...
			steps = GetMonData(
				&gPlayerParty[i], MON_DATA_FRIENDSHIP );
...
{
	GetBoxMonNick(
		&gPlayerParty[GetCursorSelectionMonId( )].box, gStringVar1 );
...
static u8 GetDaycareCompatibilityScore( struct DayCare* daycare )
{
	...
	for( i = 0; i < 2; i++ )
	{
		...
		species[i] = GetBoxMonData(
			&daycare->mons[i].mon, MON_DATA_SPECIES );
		trainerIds[i] =
			GetBoxMonData( &daycare->mons[i].mon, MON_DATA_OT_ID );
		personality = GetBoxMonData(
			&daycare->mons[i].mon, MON_DATA_PERSONALITY );
		genders[i] = GetGenderFromSpeciesAndPersonality(
			species[i], personality );
		eggGroups[i][0] = gBaseStats[species[i]].eggGroup1;
		eggGroups[i][1] = gBaseStats[species[i]].eggGroup2;
	}

	// check unbreedable egg group
	if( eggGroups[0][0] == EGG_GROUP_UNDISCOVERED ||
		eggGroups[1][0] == EGG_GROUP_UNDISCOVERED )
		return 0;
	...
	else
	{
		if( genders[0] == genders[1] ) // no homo
			return 0;
		if( genders[0] == MON_GENDERLESS ||
			genders[1] == MON_GENDERLESS )
			return 0;
		if( !EggGroupsOverlap(
			    eggGroups[0], eggGroups[1] ) ) // not
							   // compatible
							   // with each
							   // other
			return 0;
		...
		{
			if( trainerIds[0] ==
				trainerIds[1] ) // same species and trainer
				return 50;
		...
		{
			if( trainerIds[0] !=
				trainerIds[1] ) // different trainers,
						// different species
				return 50;
...
bool8 NameHasGenderSymbol( const u8* name, u8 genderRatio )
{
	...
	if( genderRatio == MON_MALE && symbolsCount[0] != 0 &&
		symbolsCount[1] == 0 )
		return TRUE;
	if( genderRatio == MON_FEMALE && symbolsCount[1] != 0 &&
		symbolsCount[0] == 0 )
		return TRUE;
	...
	*dest = EOS;
	for( i = 0; i < 2; i++ )
	{
		...
		level = GetLevelAfterDaycareSteps(
			&daycare->mons[i].mon, daycare->mons[i].steps );
		ConvertIntToDecimalStringN(
			text, level, STR_CONV_MODE_LEFT_ALIGN, 3 );
...
static void DaycarePrintMonNick(
	struct DayCare* daycare, u8 windowId, u32 daycareSlotId, u32 y )
{
...
static void DaycarePrintMonLvl(
	struct DayCare* daycare, u8 windowId, u32 daycareSlotId, u32 y )
{
	u8 level;
	u32 x;
	...
	ConvertIntToDecimalStringN(
		intText, level, STR_CONV_MODE_LEFT_ALIGN, 3 );
...
static void DaycarePrintMonInfo( u8 windowId, s32 daycareSlotId, u8 y )
{
	...
	{
		DaycarePrintMonNick(
			&gSaveBlock1Ptr->daycare, windowId, daycareSlotId, y );
		DaycarePrintMonLvl(
			&gSaveBlock1Ptr->daycare, windowId, daycareSlotId, y );
	}
}
...
static void Task_HandleDaycareLevelMenuInput( u8 taskId )
{
	...
	if( gMain.newKeys & A_BUTTON )
	{
		...
		DestroyListMenuTask(
			gTasks[taskId].tMenuListTaskId, NULL, NULL );
