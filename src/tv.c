...
static void sub_80F0358(
	TVShow* player1, TVShow* player2, TVShow* player3, TVShow* player4 );
...
static void sub_80F0D60( PokeNews* player1,
	PokeNews* player2,
	PokeNews* player3,
	PokeNews* player4 );
...
static const u8* const sTVPokemonAnslerTextGroup[] = {
	gTVPokemonAnglerText00, gTVPokemonAnglerText01};

static const u8* const sTVWorldOfMastersTextGroup[] = {gTVWorldOfMastersText00,
...
static const u8* const sTVFindThatGamerTextGroup[] = {gTVFindThatGamerText00,
...
const u8 sTVSecretBaseSecretsStateLookup[] = {0x0a,
	0x0b,
	0x0c,
	0x0d,
	0x0e,
...
u8 special_0x44( void )
{
	...
	do
	{
		if( GetTVChannelByShowType(
			    gSaveBlock1Ptr->tvShows[j].common.kind ) != 4 )
		{
		...
		else
		{
			...
			if( show->massOutbreak.daysLeft == 0 &&
...
u8 FindAnyTVShowOnTheAir( void )
{
	...
	if( gSaveBlock1Ptr->outbreakPokemonSpecies != SPECIES_NONE &&
		gSaveBlock1Ptr->tvShows[response].common.kind ==
...
void UpdateTVScreensOnMap( int width, int height )
{
	...
	default:
		if( gSaveBlock1Ptr->location.mapGroup ==
				MAP_GROUP(
					LILYCOVE_CITY_COVE_LILY_MOTEL_1F ) &&
...
void SetTVMetatilesOnMap( int width, int height, u16 tileId )
{
	...
	for( y = 0; y < height; y++ )
	{
		for( x = 0; x < width; x++ )
		{
			if( MapGridGetMetatileBehaviorAt( x, y ) ==
				MB_TELEVISION )
			{
...
{
	SetTVMetatilesOnMap(
		gBackupMapLayout.width, gBackupMapLayout.height, 0x0002 );
	DrawWholeMapView( );
}
...
{
	SetTVMetatilesOnMap(
		gBackupMapLayout.width, gBackupMapLayout.height, 0x0003 );
	DrawWholeMapView( );
}
...
u8 FindFirstActiveTVShowThatIsNotAMassOutbreak( void )
{
	...
	{
		if( gSaveBlock1Ptr->tvShows[i].common.kind != 0 &&
			gSaveBlock1Ptr->tvShows[i].common.kind !=
				TVSHOW_MASS_OUTBREAK &&
...
void GabbyAndTyBeforeInterview( void )
{
	...
	if( !gBattleResults.usedMasterBall )
	{
		for( i = 0; i < 11; i++ )
		{
			...
			{
				gSaveBlock1Ptr->gabbyAndTyData
					.playerThrewABall = TRUE;
				break;
			}
		}
	}
...
bool8 GabbyAndTyGetLastQuote( void )
{
	...
	CopyEasyChatWord(
		gStringVar1, gSaveBlock1Ptr->gabbyAndTyData.quote[0] );
	gSaveBlock1Ptr->gabbyAndTyData.quote[0] = -1;
	return TRUE;
}
				...
				gBattleResults.caughtMonNick ) )
		{
			...
			if( sCurTVShowSlot != -1 &&
				HasMixableShowAlreadyBeenSpawnedWithPlayerID(
					TVSHOW_POKEMON_TODAY_CAUGHT, FALSE ) !=
					TRUE )
			{
				for( i = 0; i < 11; i++ )
				{
				...
				{
					ct   = 0;
					show = &gSaveBlock1Ptr->tvShows
							[sCurTVShowSlot];
					show->pokemonToday.kind =
					...
					else
					{
						for( i = 0; i < 11; i++ )
						{
							ct += gBattleResults.catchAttempts
								      [i];
						}
						if( ct > 0xFF )
						{
...
void PutPokemonTodayFailedOnTheAir( void )
{
	...
	if( !rbernoulli( 1, 1 ) )
	{
				...
				gBattleOutcome == B_OUTCOME_WON ) )
		{
			...
			if( sCurTVShowSlot != -1 &&
				HasMixableShowAlreadyBeenSpawnedWithPlayerID(
					TVSHOW_POKEMON_TODAY_FAILED, FALSE ) !=
					TRUE )
			{
				show = &gSaveBlock1Ptr
						->tvShows[sCurTVShowSlot];
				show->pokemonTodayFailed.kind =
					TVSHOW_POKEMON_TODAY_FAILED;
				show->pokemonTodayFailed.active = FALSE;
				show->pokemonTodayFailed.species =
					gBattleResults.playerMon1Species;
				show->pokemonTodayFailed.species2 =
					gBattleResults.lastOpponentSpecies;
				show->pokemonTodayFailed.nBallsUsed = ct;
				show->pokemonTodayFailed.outcome =
					gBattleOutcome;
				show->pokemonTodayFailed.location =
...
static void InterviewAfter_ContestLiveUpdates( void )
{
	...
	if( show->contestLiveUpdates.kind == TVSHOW_CONTEST_LIVE_UPDATES )
	{
		...
		StringCopy( show2->contestLiveUpdates.playerName,
			gSaveBlock2Ptr->playerName );
		show2->contestLiveUpdates.category =
			gSpecialVar_ContestCategory;
		show2->contestLiveUpdates.species =
		...
		show2->contestLiveUpdates.appealFlags2 =
			show->contestLiveUpdates.appealFlags2;
		show2->contestLiveUpdates.round1Rank =
			show->contestLiveUpdates.round1Rank;
		show2->contestLiveUpdates.round2Rank =
			show->contestLiveUpdates.round2Rank;
		show2->contestLiveUpdates.move = show->contestLiveUpdates.move;
		show2->contestLiveUpdates.appealFlags1 =
			show->contestLiveUpdates.appealFlags1;
		...
		show2->contestLiveUpdates.winningTrainerLanguage =
			show->contestLiveUpdates.winningTrainerLanguage;
		DeleteTVShowInArrayByIdx( gSaveBlock1Ptr->tvShows, 24 );
	}
}

void PutBattleUpdateOnTheAir( u8 opponentLinkPlayerId,
	u16 move,
	u16 speciesPlayer,
	u16 speciesOpponent )
{
	...
	sCurTVShowSlot = FindEmptyTVSlotWithinFirstFiveShowsOfArray(
		gSaveBlock1Ptr->tvShows );
	if( sCurTVShowSlot != -1 )
	{
		...
		if( gSpecialVar_Result != 1 )
		{
			...
			StringCopy( show->battleUpdate.playerName,
				gSaveBlock2Ptr->playerName );
			...
			StringCopy( name,
				gLinkPlayers[opponentLinkPlayerId].name );
			StripExtCtrlCodes( name );
			StringCopy(
				show->battleUpdate.linkOpponentName, name );
			tv_store_id_2x( show );
			...
			{
				show->battleUpdate.linkOpponentLanguage =
					gLinkPlayers[opponentLinkPlayerId]
						.language;
			}
		}
	}
...
void PutFanClubSpecialOnTheAir( void )
{
	...
	StringCopy(
		show->fanClubSpecial.playerName, gSaveBlock2Ptr->playerName );
...
void ContestLiveUpdates_BeforeInterview_5( u8 a0, u8 a1 )
{
	...
	{
		show->contestLiveUpdates.winningSpecies =
			gContestMons[a1].species;
		StringCopy( show->contestLiveUpdates.winningTrainerName,
			gContestMons[a1].trainerName );
		StripExtCtrlCodes(
			show->contestLiveUpdates.winningTrainerName );
		...
		if( a1 + 1 > gNumLinkContestPlayers )
		{
			...
			gLinkPlayers[a1].language == LANGUAGE_JAPANESE )
		{
			show->contestLiveUpdates.winningTrainerLanguage =
				LANGUAGE_JAPANESE;
		}
...
static void InterviewAfter_BravoTrainerBattleTowerProfile( void )
{
	...
	show = &gSaveBlock1Ptr->tvShows[sCurTVShowSlot];
	show->bravoTrainerTower.kind =
		TVSHOW_BRAVO_TRAINER_BATTLE_TOWER_PROFILE;
	show->bravoTrainerTower.active = TRUE;
	StringCopy( show->bravoTrainerTower.trainerName,
		gSaveBlock2Ptr->playerName );
	StringCopy( show->bravoTrainerTower.pokemonName,
		gSaveBlock2Ptr->frontier.field_BD8 );
	show->bravoTrainerTower.species = gSaveBlock2Ptr->frontier.field_BD4;
	show->bravoTrainerTower.defeatedSpecies =
		gSaveBlock2Ptr->frontier.field_BD6;
	show->bravoTrainerTower.numFights = GetCurrentBattleTowerWinStreak(
		gSaveBlock2Ptr->frontier.field_D07, 0 );
	show->bravoTrainerTower.wonTheChallenge =
		gSaveBlock2Ptr->frontier.field_D06;
	...
	if( show->bravoTrainerTower.language == LANGUAGE_JAPANESE ||
		gSaveBlock2Ptr->frontier.field_BEB == LANGUAGE_JAPANESE )
	{
	...
	{
		show->bravoTrainerTower.pokemonNameLanguage =
			gSaveBlock2Ptr->frontier.field_BEB;
	}
}
...
void SaveRecordedItemPurchasesForTVShow( void )
{
	...
	if( !( gSaveBlock1Ptr->location.mapGroup ==
			    MAP_GROUP( TRAINER_HILL_ENTRANCE ) &&
		    gSaveBlock1Ptr->location.mapNum ==
			    MAP_NUM( TRAINER_HILL_ENTRANCE ) ) &&
		!( gSaveBlock1Ptr->location.mapGroup ==
				MAP_GROUP( BATTLE_FRONTIER_MART ) &&
			gSaveBlock1Ptr->location.mapNum ==
				MAP_NUM( BATTLE_FRONTIER_MART ) ) &&
		!rbernoulli( 1, 3 ) )
	{
		...
		if( sCurTVShowSlot != -1 &&
			HasMixableShowAlreadyBeenSpawnedWithPlayerID(
				TVSHOW_SMART_SHOPPER, FALSE ) != TRUE )
		{
			...
			if( gMartPurchaseHistory[0].quantity >= 20 )
			{
				show = &gSaveBlock1Ptr
						->tvShows[sCurTVShowSlot];
				show->smartshopperShow.kind =
					TVSHOW_SMART_SHOPPER;
				show->smartshopperShow.active = FALSE;
				show->smartshopperShow.shopLocation =
					gMapHeader.regionMapSectionId;
				for( i = 0; i < 3; i++ )
				{
					show->smartshopperShow.itemIds[i] =
						gMartPurchaseHistory[i].itemId;
					show->smartshopperShow.itemAmounts[i] =
						gMartPurchaseHistory[i]
							.quantity;
				}
			...
			StringLength( gStringVar1 ) > 1 )
		{
			...
			show->nameRaterShow.randomSpecies =
				TV_GetSomeOtherSpeciesAlreadySeenByPlayer(
					show->nameRaterShow.species );
			StringCopy( show->nameRaterShow.trainerName,
				gSaveBlock2Ptr->playerName );
			GetMonData( &gPlayerParty[gSpecialVar_0x8004],
				MON_DATA_NICKNAME,
...
void StartMassOutbreak( void )
{
	...
	gSaveBlock1Ptr->outbreakPokemonProbability =
		show->massOutbreak.probability;
...
void PutLilycoveContestLadyShowOnTheAir( void )
{
	...
	if( gSpecialVar_Result != TRUE )
	{
		...
		sub_818E7E0( &show->contestLiveUpdates2.contestCategory,
...
static void InterviewAfter_FanClubLetter( void )
{
	...
	StringCopy(
		show->fanclubLetter.playerName, gSaveBlock2Ptr->playerName );
	show->fanclubLetter.species = GetMonData(
		&gPlayerParty[GetLeadMonIndex( )], MON_DATA_SPECIES, NULL );
	tv_store_id_2x( show );
...
static void InterviewAfter_RecentHappenings( void )
{
	...
	StringCopy( show->recentHappenings.playerName,
		gSaveBlock2Ptr->playerName );
...
static void InterviewAfter_PkmnFanClubOpinions( void )
{
	...
	show->fanclubOpinions.friendshipHighNybble =
		GetMonData( &gPlayerParty[GetLeadMonIndex( )],
			MON_DATA_FRIENDSHIP,
			NULL ) >>
		4;
	show->fanclubOpinions.questionAsked = gSpecialVar_0x8007;
	StringCopy(
		show->fanclubOpinions.playerName, gSaveBlock2Ptr->playerName );
	GetMonData( &gPlayerParty[GetLeadMonIndex( )],
		MON_DATA_NICKNAME,
		show->fanclubOpinions.nickname );
	StripExtCtrlCodes( show->fanclubOpinions.nickname );
	show->fanclubOpinions.species = GetMonData(
		&gPlayerParty[GetLeadMonIndex( )], MON_DATA_SPECIES, NULL );
	tv_store_id_2x( show );
	...
	{
		show->fanclubOpinions.pokemonNameLanguage = GetMonData(
			&gPlayerParty[GetLeadMonIndex( )], MON_DATA_LANGUAGE );
	}
}
	...
	if( FlagGet( FLAG_SYS_GAME_CLEAR ) )
	{
		...
		if( !rbernoulli( 1, 200 ) )
		{
			...
			if( sCurTVShowSlot != -1 )
			{
				...
				show->massOutbreak.active = TRUE;
				show->massOutbreak.level =
					sPokeOutbreakSpeciesList[outbreakIdx]
						.level;
	...
	if( gSaveBlock1Ptr->outbreakPokemonSpecies == SPECIES_NONE )
	{
		for( i = 0; i < 24; i++ )
		{
			if( gSaveBlock1Ptr->tvShows[i].massOutbreak.kind ==
					TVSHOW_MASS_OUTBREAK &&
				gSaveBlock1Ptr->tvShows[i]
						.massOutbreak.active == TRUE )
			{
...
void PutFishingAdviceShowOnTheAir( void )
{
	...
	if( sCurTVShowSlot != -1 &&
		HasMixableShowAlreadyBeenSpawnedWithPlayerID(
			TVSHOW_FISHING_ADVICE, FALSE ) != TRUE )
	{
		show = &gSaveBlock1Ptr->tvShows[sCurTVShowSlot];
		...
		show->pokemonAngler.nFails =
			sPokemonAnglerAttemptCounters >> 8;
		show->pokemonAngler.species = sPokemonAnglerSpecies;
		StringCopy( show->pokemonAngler.playerName,
			gSaveBlock2Ptr->playerName );
		tv_store_id_3x( show );
...
void sub_80EDA80( void )
{
	...
	if( !rbernoulli( 1, 1 ) )
	{
		...
		if( sCurTVShowSlot != -1 &&
			HasMixableShowAlreadyBeenSpawnedWithPlayerID(
				TVSHOW_WORLD_OF_MASTERS, FALSE ) != TRUE )
		{
			show2 = &gSaveBlock1Ptr->tvShows[sCurTVShowSlot];
			show2->worldOfMasters.kind   = TVSHOW_WORLD_OF_MASTERS;
			show2->worldOfMasters.active = FALSE;
			show2->worldOfMasters.numPokeCaught =
				show->worldOfMasters.numPokeCaught;
			show2->worldOfMasters.steps =
				GetGameStat( GAME_STAT_STEPS ) -
				show->worldOfMasters.steps;
			show2->worldOfMasters.caughtPoke =
				show->worldOfMasters.caughtPoke;
			show2->worldOfMasters.species =
				show->worldOfMasters.species;
			show2->worldOfMasters.location =
				show->worldOfMasters.location;
			StringCopy( show2->worldOfMasters.playerName,
				gSaveBlock2Ptr->playerName );
			tv_store_id_3x( show2 );
			show2->worldOfMasters.language = gGameLanguage;
			DeleteTVShowInArrayByIdx(
				gSaveBlock1Ptr->tvShows, 24 );
		}
	}
}
...
void sub_80EDB44( void )
{
	...
	HasMixableShowAlreadyBeenSpawnedWithPlayerID(
		TVSHOW_TODAYS_RIVAL_TRAINER, TRUE );
	sCurTVShowSlot = FindEmptyTVSlotBeyondFirstFiveShowsOfArray(
		gSaveBlock1Ptr->tvShows );
	if( sCurTVShowSlot != -1 )
	{
		...
		for( i = FLAG_BADGE01_GET, nBadges = 0;
			i < FLAG_BADGE01_GET + 8;
			i++ )
		{
			if( FlagGet( i ) )
			{
		...
		{
			show->rivalTrainer.dexCount =
				GetNationalPokedexCount( 0x01 );
		}
		else
		{
			show->rivalTrainer.dexCount =
				GetHoennPokedexCount( 0x01 );
		}
		...
		show->rivalTrainer.battlePoints =
			gSaveBlock2Ptr->frontier.battlePoints;
		StringCopy( show->rivalTrainer.playerName,
			gSaveBlock2Ptr->playerName );
		tv_store_id_3x( show );
...
void sub_80EDC60( const u16* words )
{
	...
	if( sCurTVShowSlot != -1 &&
		HasMixableShowAlreadyBeenSpawnedWithPlayerID(
			TVSHOW_TREND_WATCHER, FALSE ) != TRUE )
	{
		show = &gSaveBlock1Ptr->tvShows[sCurTVShowSlot];
		...
		StringCopy( show->trendWatcher.playerName,
			gSaveBlock2Ptr->playerName );
		tv_store_id_3x( show );
...
void sub_80EDCE8( void )
{
	...
	if( sCurTVShowSlot != -1 &&
		HasMixableShowAlreadyBeenSpawnedWithPlayerID(
			TVSHOW_TREASURE_INVESTIGATORS, FALSE ) != TRUE )
	{
		show = &gSaveBlock1Ptr->tvShows[sCurTVShowSlot];
		show->treasureInvestigators.kind =
			TVSHOW_TREASURE_INVESTIGATORS;
		show->treasureInvestigators.active = FALSE;
		show->treasureInvestigators.item   = gSpecialVar_0x8005;
		show->treasureInvestigators.location =
			gMapHeader.regionMapSectionId;
		show->treasureInvestigators.mapLayoutId =
...
void AlertTVOfNewCoinTotal( u16 nCoinsPaidOut )
{
	...
	if( sCurTVShowSlot != -1 &&
		HasMixableShowAlreadyBeenSpawnedWithPlayerID(
			TVSHOW_FIND_THAT_GAMER, FALSE ) != TRUE )
	{
		flag = FALSE;
		...
		case SLOT_MACHINE:
			...
			{
				flag      = TRUE;
				nCoinsWon = nCoinsPaidOut -
					sFindThatGamerCoinsSpent;
				break;
			}
			if( sFindThatGamerCoinsSpent >= 100 &&
				nCoinsPaidOut <=
					sFindThatGamerCoinsSpent - 100 )
			{
				nCoinsWon = sFindThatGamerCoinsSpent -
					nCoinsPaidOut;
				break;
			}
			return;
		...
		StringCopy( show->findThatGamer.playerName,
			gSaveBlock2Ptr->playerName );
		tv_store_id_3x( show );
...
static void sub_80EDE98( TVShow* show )
{
	...
	case 0:
		break;
	case 1:
		show->secretBaseVisit.decorations[0] =
			sTV_DecorationsBuffer[0];
		break;
	default:
		for( k = 0; k < n * n; k++ )
		{
		...
		{
			show->secretBaseVisit.decorations[i] =
				sTV_DecorationsBuffer[i];
		}
		break;
	}
}
	...
	for( i = 0, nPokemon = 0; i < PARTY_SIZE; i++ )
	{
			...
			!GetMonData( &gPlayerParty[i], MON_DATA_IS_EGG ) )
		{
			...
			sTV_SecretBaseVisitMonsTemp[nPokemon].species =
				GetMonData(
					&gPlayerParty[i], MON_DATA_SPECIES );
			nMoves = 0;
			...
			sTV_SecretBaseVisitMonsTemp[nPokemon].move =
				sTV_SecretBaseVisitMovesTemp[Random( ) %
					nMoves];
			nPokemon++;
		}
	}
...
void TV_PutSecretBaseVisitOnTheAir( void )
{
	...
	if( sCurTVShowSlot != -1 )
	{
		...
		StringCopy( show->secretBaseVisit.playerName,
			gSaveBlock2Ptr->playerName );
		sub_80EDE98( show );
		sub_80EDFB4( show );
		tv_store_id_3x( show );
...
void sub_80EE184( void )
{
	...
	if( sCurTVShowSlot != -1 &&
		HasMixableShowAlreadyBeenSpawnedWithPlayerID(
			TVSHOW_BREAKING_NEWS, FALSE ) != TRUE )
	{
		show = &gSaveBlock1Ptr->tvShows[sCurTVShowSlot];
		...
		StringCopy( show->breakingNews.playerName,
			gSaveBlock2Ptr->playerName );
		show->breakingNews.poke1Species =
			gBattleResults.playerMon1Species;
		...
		show->breakingNews.lastOpponentSpecies =
			gBattleResults.lastOpponentSpecies;
		...
		case 0:
			if( gBattleResults.usedMasterBall )
			{
				show->breakingNews.caughtMonBall =
					ITEM_MASTER_BALL;
			}
			else
			{
			...
			break;
		case 1:
			show->breakingNews.lastUsedMove =
				gBattleResults.lastUsedMovePlayer;
			break;
		case 2:
			break;
...
void sub_80EE35C( u16 foeSpecies,
	u16 species,
	u8 moveIdx,
	const u16* movePtr,
	u16 betterMove )
...
void sub_80EE4DC( struct Pokemon* pokemon, u8 ribbonMonDataIdx )
{
	...
	if( sCurTVShowSlot != -1 &&
		HasMixableShowAlreadyBeenSpawnedWithPlayerID(
			TVSHOW_CUTIES, FALSE ) != TRUE )
	{
		show                = &gSaveBlock1Ptr->tvShows[sCurTVShowSlot];
		show->cuties.kind   = TVSHOW_CUTIES;
		show->cuties.active = FALSE;
		StringCopy(
			show->cuties.playerName, gSaveBlock2Ptr->playerName );
		GetMonData(
			pokemon, MON_DATA_NICKNAME, show->cuties.nickname );
		StripExtCtrlCodes( show->cuties.nickname );
		show->cuties.nRibbons = GetRibbonCount( pokemon );
		show->cuties.selectedRibbon =
			TV_MonDataIdxToRibbon( ribbonMonDataIdx );
		tv_store_id_3x( show );
...
void sub_80EE72C( void )
{
	...
	if( sCurTVShowSlot != -1 &&
		HasMixableShowAlreadyBeenSpawnedWithPlayerID(
			TVSHOW_TRAINER_FAN_CLUB, FALSE ) != TRUE )
	{
		show = &gSaveBlock1Ptr->tvShows[sCurTVShowSlot];
		show->trainerFanClub.kind   = TVSHOW_TRAINER_FAN_CLUB;
		show->trainerFanClub.active = FALSE;
		StringCopy( show->trainerFanClub.playerName,
			gSaveBlock2Ptr->playerName );
		show->trainerFanClub.words[0] =
			gSaveBlock1Ptr->easyChatProfile[0];
		show->trainerFanClub.words[1] =
			gSaveBlock1Ptr->easyChatProfile[1];
		tv_store_id_3x( show );
...
bool8 sub_80EE818( void )
{
	...
	if( HasMixableShowAlreadyBeenSpawnedWithPlayerID(
		    TVSHOW_FRONTIER, FALSE ) == TRUE )
	{
		...
		for( showIdx = 5; showIdx < 24; showIdx++ )
		{
			if( shows[showIdx].common.kind == TVSHOW_FRONTIER &&
				( playerId & 0xFF ) ==
					shows[showIdx].common.trainerIdLo &&
				( ( playerId >> 8 ) & 0xFF ) ==
					shows[showIdx].common.trainerIdHi )
			{
				DeleteTVShowInArrayByIdx(
					gSaveBlock1Ptr->tvShows, showIdx );
...
void sub_80EE8C8( u16 winStreak, u8 facilityAndMode )
{
	...
	if( sCurTVShowSlot != -1 )
	{
		...
		StringCopy( show->frontier.playerName,
			gSaveBlock2Ptr->playerName );
		...
		case 13:
			show->frontier.species1 = GetMonData(
				&gPlayerParty[0], MON_DATA_SPECIES, NULL );
			show->frontier.species2 = GetMonData(
				&gPlayerParty[1], MON_DATA_SPECIES, NULL );
			show->frontier.species3 = GetMonData(
				&gPlayerParty[2], MON_DATA_SPECIES, NULL );
			break;
		case 2:
			show->frontier.species1 = GetMonData(
				&gPlayerParty[0], MON_DATA_SPECIES, NULL );
			...
			show->frontier.species3 = GetMonData(
				&gPlayerParty[2], MON_DATA_SPECIES, NULL );
			show->frontier.species4 = GetMonData(
				&gPlayerParty[3], MON_DATA_SPECIES, NULL );
			break;
		case 3:
			show->frontier.species1 = GetMonData(
				&gPlayerParty[0], MON_DATA_SPECIES, NULL );
...
void sub_80EEA70( void )
{
	...
	if( HasMixableShowAlreadyBeenSpawnedWithPlayerID(
		    TVSHOW_SECRET_BASE_SECRETS, FALSE ) != TRUE )
	{
		sCurTVShowSlot = FindEmptyTVSlotBeyondFirstFiveShowsOfArray(
			gSaveBlock1Ptr->tvShows );
		if( sCurTVShowSlot != -1 )
		{
			...
			StringCopy( show->secretBaseSecrets.baseOwnersName,
				strbuf );
			show->secretBaseSecrets.item =
				VarGet( VAR_SECRET_BASE_LAST_ITEM_USED );
			...
			if( show->secretBaseSecrets.language ==
					LANGUAGE_JAPANESE ||
				gSaveBlock1Ptr
						->secretBases[VarGet(
					...
					LANGUAGE_JAPANESE )
			{
				show->secretBaseSecrets
					.baseOwnersNameLanguage =
					LANGUAGE_JAPANESE;
			}
			else
			{
...
static void sub_80EEBF4( u8 actionIdx )
{
	...
	if( sCurTVShowSlot != -1 )
	{
		...
		StringCopy( show->numberOne.playerName,
			gSaveBlock2Ptr->playerName );
		show->numberOne.actionIdx = actionIdx;
		show->numberOne.count =
			VarGet( sNumberOneVarsAndThresholds[actionIdx][0] );
		tv_store_id_3x( show );
...
{
	VarSet( VAR_DAILY_PLANTED_BERRIES,
...
static void sub_80EED88( void )
{
	...
	if( FlagGet( FLAG_SYS_GAME_CLEAR ) )
	{
		...
		if( sCurTVShowSlot != -1 && rbernoulli( 1, 100 ) != TRUE )
		{
			...
			{
				gSaveBlock1Ptr->pokeNews[sCurTVShowSlot].kind =
					newsKind;
				gSaveBlock1Ptr->pokeNews[sCurTVShowSlot].days =
					4;
				gSaveBlock1Ptr->pokeNews[sCurTVShowSlot]
					.state = TRUE;
			}
		}
	}
...
static void sub_80EEEB8( void )
{
	...
	for( i = 0; i < 15; i++ )
	{
		if( gSaveBlock1Ptr->pokeNews[i].kind == POKENEWS_NONE )
		{
			for( j = i + 1; j < 16; j++ )
			{
				if( gSaveBlock1Ptr->pokeNews[j].kind !=
					POKENEWS_NONE )
				{
					gSaveBlock1Ptr->pokeNews[i] =
...
u8 FindAnyTVNewsOnTheAir( void )
{
	...
	{
		if( gSaveBlock1Ptr->pokeNews[i].kind != POKENEWS_NONE &&
			gSaveBlock1Ptr->pokeNews[i].state == TRUE &&
...
void DoPokeNews( void )
{
	...
	else
	{
		if( gSaveBlock1Ptr->pokeNews[i].days == 0 )
		{
			...
			{
				ShowFieldMessage( sPokeNewsTextGroup_Ongoing
						[gSaveBlock1Ptr->pokeNews[i]
								.kind] );
			}
			else
			{
				ShowFieldMessage( sPokeNewsTextGroup_Ending
						[gSaveBlock1Ptr->pokeNews[i]
								.kind] );
			}
		}
		else
		{
			...
			ConvertIntToDecimalStringN(
				gStringVar1, n, STR_CONV_MODE_LEFT_ALIGN, 1 );
			gSaveBlock1Ptr->pokeNews[i].state = 0;
			ShowFieldMessage( sPokeNewsTextGroup_Upcoming
					[gSaveBlock1Ptr->pokeNews[i].kind] );
		}
...
static void sub_80EF120( u16 days )
{
	...
	for( i = 0; i < 16; i++ )
	{
		if( gSaveBlock1Ptr->pokeNews[i].kind != POKENEWS_NONE )
		{
			...
			{
				if( gSaveBlock1Ptr->pokeNews[i].state == 0 &&
...
void SetContestCategoryStringVarForInterview( void )
{
	...
	CopyContestCategoryToStringVar(
		1, show->bravoTrainer.contestCategory );
}
...
void TV_PrintIntToStringVar( u8 varIdx, int value )
{
	...
	ConvertIntToDecimalStringN( gTVStringVarPtrs[varIdx],
		value,
...
void TV_SortPurchasesByQuantity( void )
{
	...
	for( i = 0; i < 2; i++ )
	{
		for( j = i + 1; j < 3; j++ )
		{
			if( gMartPurchaseHistory[i].quantity <
				gMartPurchaseHistory[j].quantity )
			{
				tmpId = gMartPurchaseHistory[i].itemId;
				tmpQn = gMartPurchaseHistory[i].quantity;
				gMartPurchaseHistory[i].itemId =
					gMartPurchaseHistory[j].itemId;
...
bool8 sub_80EF88C( u8 monIdx )
{
	...
	if( language == LANGUAGE_ENGLISH &&
		!StringCompare( gSpeciesNames[GetMonData(
					pokemon, MON_DATA_SPECIES, NULL )],
			gStringVar1 ) )
	{
		return FALSE;
	}
...
bool8 sub_80EF8F8( void ) { return sub_80EF88C( GetLeadMonIndex( ) ); }
...
u16 TV_GetSomeOtherSpeciesAlreadySeenByPlayer_AndPrintName(
	u8 varIdx, u16 passedSpecies )
{
	u16 species;
...
void TV_GetNicknameSubstring( u8 varIdx,
	u8 whichPosition,
	u8 charParam,
	u16 whichString,
	u16 species,
	TVShow* show )
{
	...
	if( whichString == 0 )
	{
		...
		{
			buff[0] =
				show->nameRaterShow.trainerName[whichPosition];
		}
		else if( charParam == 1 )
		{
			buff[0] = show->nameRaterShow
					  .trainerName[strlen - whichPosition];
		}
		else if( charParam == 2 )
		{
			buff[0] =
				show->nameRaterShow.trainerName[whichPosition];
			buff[1] = show->nameRaterShow
					  .trainerName[whichPosition + 1];
		}
		else
		{
			buff[0] = show->nameRaterShow.trainerName[strlen -
				( whichPosition + 2 )];
			buff[1] = show->nameRaterShow.trainerName[strlen -
				( whichPosition + 1 )];
		}
		ConvertInternationalString(
			buff, show->nameRaterShow.language );
	}
	else if( whichString == 1 )
	{
		...
		{
			buff[0] =
				show->nameRaterShow.pokemonName[whichPosition];
		}
		else if( charParam == 1 )
		{
			buff[0] = show->nameRaterShow
					  .pokemonName[strlen - whichPosition];
		}
		else if( charParam == 2 )
		{
			buff[0] =
				show->nameRaterShow.pokemonName[whichPosition];
			buff[1] = show->nameRaterShow
					  .pokemonName[whichPosition + 1];
		}
		else
		{
			buff[0] = show->nameRaterShow.pokemonName[strlen -
				( whichPosition + 2 )];
			buff[1] = show->nameRaterShow.pokemonName[strlen -
				( whichPosition + 1 )];
		}
		ConvertInternationalString(
			buff, show->nameRaterShow.pokemonNameLanguage );
	}
	else
	{
		...
		{
			buff[0] =
				gSpeciesNames[species][strlen - whichPosition];
		}
		else if( charParam == 2 )
		{
			buff[0] = gSpeciesNames[species][whichPosition];
		...
		{
			buff[0] = gSpeciesNames[species][strlen -
				( whichPosition + 2 )];
			buff[1] = gSpeciesNames[species][strlen -
				( whichPosition + 1 )];
		}
	}
...
void ChangePokemonNickname_CB( void )
{
	SetMonData( &gPlayerParty[gSpecialVar_0x8004],
		MON_DATA_NICKNAME,
		gStringVar2 );
...
{
	SetBoxMonNickAt(
		gSpecialVar_MonBoxId, gSpecialVar_MonBoxPos, gStringVar2 );
	CB2_ReturnToFieldContinueScriptPlayMapMusic( );
}
...
u8 GetTVChannelByShowType( u8 kind )
{
	...
	if( kind >= TVSHOW_FAN_CLUB_LETTER &&
		kind < TVSHOW_POKEMON_TODAY_CAUGHT )
	{
		return 2;
	}
	if( kind >= TVSHOW_POKEMON_TODAY_CAUGHT &&
		kind < TVSHOW_MASS_OUTBREAK )
	{
		return 3;
	}
...
void sub_80F01B8( void )
{
	...
	RemoveEventObjectByLocalIdAndMap( 5,
		gSaveBlock1Ptr->location.mapNum,
		gSaveBlock1Ptr->location.mapGroup );
...
void ReceiveTvShowsData( void* src, u32 size, u8 masterIdx )
{
	...
	if( rmBuffer2 != NULL )
	{
		...
		{
		case 0:
			sub_80F0358( gSaveBlock1Ptr->tvShows,
				( *rmBuffer )[1],
				( *rmBuffer )[2],
				( *rmBuffer )[3] );
			break;
		case 1:
			sub_80F0358( ( *rmBuffer )[0],
				gSaveBlock1Ptr->tvShows,
				( *rmBuffer )[2],
				( *rmBuffer )[3] );
			break;
...
static void sub_80F0358( TVShow player1[25],
	TVShow player2[25],
	TVShow player3[25],
	TVShow player4[25] )
{
	...
	while( 1 )
	{
		for( i = 0; i < sTVShowMixingNumPlayers; i++ )
		{
			...
			if( sTVShowMixingCurSlot == -1 )
			{
				...
				if( sRecordMixingPartnersWithoutShowsToShare ==
					sTVShowMixingNumPlayers )
				{
					return;
				}
			}
			...
			{
				for( j = 0; j < sTVShowMixingNumPlayers - 1;
					j++ )
				{
					sCurTVShowSlot =
						FindEmptyTVSlotBeyondFirstFiveShowsOfArray(
...
static bool8 sub_80F049C( TVShow* dest[25], TVShow* src[25], u8 idx )
{
	...
	tv2       = *src;
	value     = FALSE;
	switchval = GetTVChannelByShowType(
		tv2[sTVShowMixingCurSlot].common.kind );
	switch( switchval )
	{
	case 2:
		value = sub_80F0580( &tv1[sCurTVShowSlot],
			&tv2[sTVShowMixingCurSlot],
			idx );
		break;
	case 3:
		value = sub_80F05E8( &tv1[sCurTVShowSlot],
			&tv2[sTVShowMixingCurSlot],
			idx );
		break;
	case 4:
		value = sub_80F0668( &tv1[sCurTVShowSlot],
			&tv2[sTVShowMixingCurSlot],
			idx );
		break;
	}
...
static bool8 sub_80F05E8( TVShow* tv1, TVShow* tv2, u8 idx )
{
	...
	if( ( linkTrainerId & 0xFF ) == tv2->common.srcTrainerIdLo &&
		( ( linkTrainerId >> 8 ) & 0xFF ) ==
			tv2->common.srcTrainerIdHi )
	{
		return FALSE;
	}
	if( ( linkTrainerId & 0xFF ) == tv2->common.trainerIdLo &&
...
static void sub_80F0708( void ) // FIXME: register allocation shenanigans
{
	...
	for( i = 0; i < 24; i++ )
	{
		...
		{
		case TVSHOW_CONTEST_LIVE_UPDATES:
			sub_80F0B24( ( &gSaveBlock1Ptr->tvShows[i] )
					     ->contestLiveUpdates.species,
				i );
			sub_80F0B24(
				( &gSaveBlock1Ptr->tvShows[i] )
					->contestLiveUpdates.winningSpecies,
				i );
			break;
		case TVSHOW_3_CHEERS_FOR_POKEBLOCKS:
			break;
		case TVSHOW_BATTLE_UPDATE:
			sub_80F0B24( ( &gSaveBlock1Ptr->tvShows[i] )
					     ->battleUpdate.speciesPlayer,
				i );
			sub_80F0B24( ( &gSaveBlock1Ptr->tvShows[i] )
					     ->battleUpdate.speciesOpponent,
				i );
			break;
		case TVSHOW_FAN_CLUB_SPECIAL:
			break;
		...
		case TVSHOW_NAME_RATER_SHOW:
			sub_80F0B24( ( &gSaveBlock1Ptr->tvShows[i] )
					     ->nameRaterShow.species,
				i );
			sub_80F0B24( ( &gSaveBlock1Ptr->tvShows[i] )
					     ->nameRaterShow.randomSpecies,
				i );
			break;
		case TVSHOW_BRAVO_TRAINER_POKEMON_PROFILE:
			sub_80F0B24( ( &gSaveBlock1Ptr->tvShows[i] )
					     ->bravoTrainer.species,
				i );
			break;
		case TVSHOW_BRAVO_TRAINER_BATTLE_TOWER_PROFILE:
			sub_80F0B24( ( &gSaveBlock1Ptr->tvShows[i] )
					     ->bravoTrainerTower.species,
				i );
			sub_80F0B24(
				( &gSaveBlock1Ptr->tvShows[i] )
					->bravoTrainerTower.defeatedSpecies,
				i );
			break;

		case TVSHOW_POKEMON_TODAY_CAUGHT:
			sub_80F0B24( ( &gSaveBlock1Ptr->tvShows[i] )
					     ->pokemonToday.species,
				i );
			break;
		...
		case TVSHOW_POKEMON_TODAY_FAILED:
			sub_80F0B24( ( &gSaveBlock1Ptr->tvShows[i] )
					     ->pokemonTodayFailed.species,
				i );
			sub_80F0B24( ( &gSaveBlock1Ptr->tvShows[i] )
					     ->pokemonTodayFailed.species2,
				i );
			break;
		case TVSHOW_FISHING_ADVICE:
			sub_80F0B24( ( &gSaveBlock1Ptr->tvShows[i] )
					     ->pokemonAngler.species,
				i );
			break;
		case TVSHOW_WORLD_OF_MASTERS:
			sub_80F0B24( ( &gSaveBlock1Ptr->tvShows[i] )
					     ->worldOfMasters.species,
				i );
			sub_80F0B24( ( &gSaveBlock1Ptr->tvShows[i] )
					     ->worldOfMasters.caughtPoke,
				i );
			break;
		...
		case TVSHOW_BREAKING_NEWS:
			sub_80F0B24(
				( &gSaveBlock1Ptr->tvShows[i] )
					->breakingNews.lastOpponentSpecies,
				i );
			sub_80F0B24( ( &gSaveBlock1Ptr->tvShows[i] )
					     ->breakingNews.poke1Species,
				i );
			break;
		case TVSHOW_SECRET_BASE_VISIT:
			sub_80F0B24( ( &gSaveBlock1Ptr->tvShows[i] )
					     ->secretBaseVisit.species,
				i );
			break;
		...
		case TVSHOW_BATTLE_SEMINAR:
			sub_80F0B24( ( &gSaveBlock1Ptr->tvShows[i] )
					     ->battleSeminar.species,
				i );
			sub_80F0B24( ( &gSaveBlock1Ptr->tvShows[i] )
					     ->battleSeminar.foeSpecies,
				i );
			break;
		...
		case TVSHOW_FRONTIER:
			...
			sub_80F0B24( ( &gSaveBlock1Ptr->tvShows[i] )
					     ->frontier.species2,
				i );
			switch( ( &gSaveBlock1Ptr->tvShows[i] )
					->frontier.facility )
			{
			case 3:
			case 4:
...
static void sub_80F0B24( u16 species, u8 showIdx )
{
	if( !GetSetPokedexFlag(
		    SpeciesToNationalPokedexNum( species ), FLAG_GET_SEEN ) )
	{
		gSaveBlock1Ptr->tvShows[showIdx].common.active = FALSE;
	}
}
...
static void sub_80F0B64( void )
{
	...
	if( FlagGet( FLAG_SYS_GAME_CLEAR ) != TRUE )
	{
		for( i = 0; i < 24; i++ )
		{
			...
			else if( gSaveBlock1Ptr->tvShows[i].common.kind ==
				TVSHOW_MASS_OUTBREAK )
			{
				gSaveBlock1Ptr->tvShows[i].common.active =
					FALSE;
...
void sub_80F0BB8( void )
{
	...
	{
		if( GetTVChannelByShowType(
			    gSaveBlock1Ptr->tvShows[i].common.kind ) == 2 )
		{
...
void ReceivePokeNewsData( void* src, u32 size, u8 masterIdx )
{
	...
	if( rmBuffer2 != NULL )
	{
		...
		{
		case 0:
			sub_80F0D60( gSaveBlock1Ptr->pokeNews,
				( *rmBuffer )[1],
				( *rmBuffer )[2],
				( *rmBuffer )[3] );
			break;
		case 1:
			sub_80F0D60( ( *rmBuffer )[0],
...
static void sub_80F0D60( PokeNews player1[16],
	PokeNews player2[16],
	PokeNews player3[16],
	PokeNews player4[16] )
{
	...
	for( i = 0; i < 16; i++ )
	{
		for( j = 0; j < sTVShowNewsMixingNumPlayers; j++ )
		{
			...
			{
				for( k = 0;
					k < sTVShowNewsMixingNumPlayers - 1;
					k++ )
				{
					sCurTVShowSlot = sub_80EEE30(
						*argslist[( j + k + 1 ) %
							sTVShowNewsMixingNumPlayers] );
					if( sCurTVShowSlot != -1 )
					{
						sub_80F0E58(
							argslist[( j + k +
...
static void sub_80F0F64( TVShow* show, u32 language )
{
	...
	for( i = 0; i < 24; i++ )
	{
		...
		case TVSHOW_RECENT_HAPPENINGS:
			r4[0] = &show[i];
			tvlangfix( r4[0]->fanclubLetter.playerName,
				r4[0]->fanclubLetter.language,
				language );
			break;
		case TVSHOW_PKMN_FAN_CLUB_OPINIONS:
			r4[1] = &show[i];
			tvlangfix( r4[1]->fanclubOpinions.playerName,
				r4[1]->fanclubOpinions.language,
				language );
		...
		case TVSHOW_POKEMON_TODAY_CAUGHT:
			r4[6] = &show[i];
			tvlangfix( r4[6]->pokemonToday.playerName,
				r4[6]->pokemonToday.language,
				language );
			tvlangfix( r4[6]->pokemonToday.nickname,
				r4[6]->pokemonToday.language2,
				language );
			break;
		case TVSHOW_SMART_SHOPPER:
			r4[7] = &show[i];
			tvlangfix( r4[7]->smartshopperShow.playerName,
				r4[7]->smartshopperShow.language,
				language );
			break;
		case TVSHOW_BRAVO_TRAINER_BATTLE_TOWER_PROFILE:
			r4[5] = &show[i];
		...
		case TVSHOW_BRAVO_TRAINER_POKEMON_PROFILE:
			r4[4] = &show[i];
			tvlangfix( r4[4]->bravoTrainer.playerName,
				r4[4]->bravoTrainer.language,
				language );
		...
		case TVSHOW_NAME_RATER_SHOW:
			r4[3] = &show[i];
			tvlangfix( r4[3]->nameRaterShow.trainerName,
				r4[3]->nameRaterShow.language,
				language );
		...
		case TVSHOW_FISHING_ADVICE:
			r4[8] = &show[i];
			tvlangfix( r4[8]->pokemonAngler.playerName,
				r4[8]->pokemonAngler.language,
				language );
			break;
		case TVSHOW_WORLD_OF_MASTERS:
			r4[9] = &show[i];
			tvlangfix( r4[9]->worldOfMasters.playerName,
				r4[9]->worldOfMasters.language,
				language );
			break;
		case TVSHOW_MASS_OUTBREAK:
			r4[10]                        = &show[i];
...
void sub_80F1208( TVShow* shows )
{
	...
	for( curShow = shows; curShow < shows + 24; curShow++ )
	{
		if( curShow->bravoTrainerTower.kind ==
			TVSHOW_BRAVO_TRAINER_BATTLE_TOWER_PROFILE )
		{
			if( ( curShow->bravoTrainerTower.language ==
					    LANGUAGE_JAPANESE &&
				    curShow->bravoTrainerTower
						    .pokemonNameLanguage !=
					    LANGUAGE_JAPANESE ) ||
...
static void sub_80F12A4( TVShow* shows )
{
	...
	for( curShow = shows; curShow < shows + 24; curShow++ )
	{
		...
		{
		case TVSHOW_FAN_CLUB_LETTER:
			curShow->fanclubLetter.language = TV_GetStringLanguage(
				curShow->fanclubLetter.playerName );
			break;
		case TVSHOW_RECENT_HAPPENINGS:
			curShow->recentHappenings.language =
				TV_GetStringLanguage(
					curShow->recentHappenings.playerName );
			break;
		case TVSHOW_PKMN_FAN_CLUB_OPINIONS:
			curShow->fanclubOpinions.language =
				TV_GetStringLanguage(
					curShow->fanclubOpinions.playerName );
			curShow->fanclubOpinions.pokemonNameLanguage =
				TV_GetStringLanguage(
					curShow->fanclubOpinions.nickname );
			break;
		case TVSHOW_UNKN_SHOWTYPE_04:
			curShow->unkShow04.language = TV_GetStringLanguage(
				curShow->unkShow04.string_0b );
			break;
		case TVSHOW_NAME_RATER_SHOW:
			curShow->nameRaterShow.language = TV_GetStringLanguage(
				curShow->nameRaterShow.trainerName );
			curShow->nameRaterShow.pokemonNameLanguage =
				TV_GetStringLanguage(
					curShow->nameRaterShow.pokemonName );
			break;
		case TVSHOW_BRAVO_TRAINER_POKEMON_PROFILE:
			curShow->bravoTrainer.language = TV_GetStringLanguage(
				curShow->bravoTrainer.playerName );
			curShow->bravoTrainer
				.pokemonNameLanguage = TV_GetStringLanguage(
				curShow->bravoTrainer.pokemonNickname );
			break;
		case TVSHOW_BRAVO_TRAINER_BATTLE_TOWER_PROFILE:
			curShow->bravoTrainerTower
				.language = TV_GetStringLanguage(
				curShow->bravoTrainerTower.trainerName );
			curShow->bravoTrainerTower
				.pokemonNameLanguage = TV_GetStringLanguage(
				curShow->bravoTrainerTower.pokemonName );
			break;
		case TVSHOW_CONTEST_LIVE_UPDATES:
			curShow->contestLiveUpdates
				.language = TV_GetStringLanguage(
				curShow->contestLiveUpdates.playerName );
			...
			break;
		case TVSHOW_3_CHEERS_FOR_POKEBLOCKS:
			curShow->threeCheers.language = TV_GetStringLanguage(
				curShow->threeCheers.playerName );
			curShow->threeCheers
				.worstBlenderLanguage = TV_GetStringLanguage(
				curShow->threeCheers.worstBlenderName );
			break;
		case TVSHOW_BATTLE_UPDATE:
			curShow->battleUpdate.language = TV_GetStringLanguage(
				curShow->battleUpdate.playerName );
			curShow->battleUpdate
				.linkOpponentLanguage = TV_GetStringLanguage(
				curShow->battleUpdate.linkOpponentName );
			break;
		case TVSHOW_FAN_CLUB_SPECIAL:
			curShow->fanClubSpecial.language =
				TV_GetStringLanguage(
					curShow->fanClubSpecial.playerName );
			curShow->fanClubSpecial.idolNameLanguage =
				TV_GetStringLanguage(
					curShow->fanClubSpecial.idolName );
			break;
		case TVSHOW_CONTEST_LIVE_UPDATES_2:
			curShow->contestLiveUpdates2
				.language = TV_GetStringLanguage(
				curShow->contestLiveUpdates2.playerName );
			curShow->contestLiveUpdates2
				.pokemonNameLanguage = TV_GetStringLanguage(
				curShow->contestLiveUpdates2.nickname );
			break;

		case TVSHOW_POKEMON_TODAY_CAUGHT:
			curShow->pokemonToday.language = TV_GetStringLanguage(
				curShow->pokemonToday.playerName );
			curShow->pokemonToday.language2 = TV_GetStringLanguage(
				curShow->pokemonToday.nickname );
			break;
		case TVSHOW_SMART_SHOPPER:
			curShow->smartshopperShow.language =
				TV_GetStringLanguage(
					curShow->smartshopperShow.playerName );
			break;
		case TVSHOW_POKEMON_TODAY_FAILED:
			curShow->pokemonTodayFailed
				.language = TV_GetStringLanguage(
				curShow->pokemonTodayFailed.playerName );
			break;
		case TVSHOW_FISHING_ADVICE:
			curShow->pokemonAngler.language = TV_GetStringLanguage(
				curShow->pokemonAngler.playerName );
			break;
		case TVSHOW_WORLD_OF_MASTERS:
			curShow->worldOfMasters.language =
				TV_GetStringLanguage(
					curShow->worldOfMasters.playerName );
			break;
		case TVSHOW_TREND_WATCHER:
			curShow->trendWatcher.language = TV_GetStringLanguage(
				curShow->trendWatcher.playerName );
			break;
		case TVSHOW_BREAKING_NEWS:
			curShow->breakingNews.language = TV_GetStringLanguage(
				curShow->breakingNews.playerName );
			break;
		case TVSHOW_BATTLE_SEMINAR:
			curShow->battleSeminar.language = TV_GetStringLanguage(
				curShow->battleSeminar.playerName );
			break;
		case TVSHOW_FIND_THAT_GAMER:
		case TVSHOW_TRAINER_FAN_CLUB:
			curShow->trainerFanClub.language =
				TV_GetStringLanguage(
					curShow->trainerFanClub.playerName );
			break;
		case TVSHOW_CUTIES:
			curShow->cuties.language = TV_GetStringLanguage(
				curShow->cuties.playerName );
			curShow->cuties.pokemonNameLanguage =
				TV_GetStringLanguage(
					curShow->cuties.nickname );
			break;
		case TVSHOW_TODAYS_RIVAL_TRAINER:
		case TVSHOW_SECRET_BASE_VISIT:
		case TVSHOW_FRONTIER:
			curShow->rivalTrainer.language = TV_GetStringLanguage(
				curShow->rivalTrainer.playerName );
			break;
		case TVSHOW_TREASURE_INVESTIGATORS:
		case TVSHOW_LOTTO_WINNER:
		case TVSHOW_NUMBER_ONE:
			curShow->treasureInvestigators
				.language = TV_GetStringLanguage(
				curShow->treasureInvestigators.playerName );
			break;
		case TVSHOW_SECRET_BASE_SECRETS:
			curShow->secretBaseSecrets
				.language = TV_GetStringLanguage(
				curShow->secretBaseSecrets.playerName );
			curShow->secretBaseSecrets
				.baseOwnersNameLanguage = TV_GetStringLanguage(
				curShow->secretBaseSecrets.baseOwnersName );
			break;
		case TVSHOW_SAFARI_FAN_CLUB:
			curShow->safariFanClub.language = TV_GetStringLanguage(
				curShow->safariFanClub.playerName );
			break;
		case TVSHOW_MASS_OUTBREAK:
			break;
		}
...
void DoTVShow( void )
{
	...
	{
		switch( gSaveBlock1Ptr->tvShows[gSpecialVar_0x8004]
				.common.kind )
		{
		case TVSHOW_FAN_CLUB_LETTER:
...
static void DoTVShowBravoTrainerPokemonProfile( void )
{
	...
	case 0:
		TVShowConvertInternationalString( gStringVar1,
			show->bravoTrainer.playerName,
			show->bravoTrainer.language );
		CopyContestCategoryToStringVar(
			1, show->bravoTrainer.contestCategory );
		CopyContestRankToStringVar(
			2, show->bravoTrainer.contestRank );
	...
	case 1:
		...
		TVShowConvertInternationalString( gStringVar2,
			...
			show->bravoTrainer.pokemonNameLanguage );
		CopyContestCategoryToStringVar(
			2, show->bravoTrainer.contestCategory );
		sTVShowState = 2;
		break;
	case 2:
		TVShowConvertInternationalString( gStringVar1,
	...
	case 3:
		...
		TV_PrintIntToStringVar(
			2, show->bravoTrainer.contestResult + 1 );
		sTVShowState = 5;
		break;
	case 4:
		TVShowConvertInternationalString( gStringVar1,
...
static void DoTVShowBravoTrainerBattleTower( void )
{
	...
	case 0:
		TVShowConvertInternationalString( gStringVar1,
			show->bravoTrainerTower.trainerName,
	...
	case 2:
		TVShowConvertInternationalString( gStringVar1,
			...
			show->bravoTrainerTower.pokemonNameLanguage );
		TV_PrintIntToStringVar(
			1, show->bravoTrainerTower.numFights + 1 );
	...
	case 3:
		TVShowConvertInternationalString( gStringVar1,
			...
			show->bravoTrainerTower.pokemonNameLanguage );
		StringCopy( gStringVar2,
			gSpeciesNames[show->bravoTrainerTower
					      .defeatedSpecies] );
		if( show->bravoTrainerTower.interviewResponse == 0 )
			sTVShowState = 5;
		else
...
static void DoTVShowTodaysSmartShopper( void )
{
	...
	case 0:
		TVShowConvertInternationalString( gStringVar1,
			...
			show->smartshopperShow.language );
		GetMapName(
			gStringVar2, show->smartshopperShow.shopLocation, 0 );
	...
	case 1:
		TVShowConvertInternationalString( gStringVar1,
			...
			show->smartshopperShow.language );
		StringCopy( gStringVar2,
			ItemId_GetName( show->smartshopperShow.itemIds[0] ) );
		TV_PrintIntToStringVar(
			2, show->smartshopperShow.itemAmounts[0] );
	...
	case 3:
		TV_PrintIntToStringVar(
			2, show->smartshopperShow.itemAmounts[0] + 1 );
		if( show->smartshopperShow.itemIds[1] != ITEM_NONE )
		{
			sTVShowState = 6;
		}
	...
	case 6:
		StringCopy( gStringVar2,
			ItemId_GetName( show->smartshopperShow.itemIds[1] ) );
		TV_PrintIntToStringVar(
			2, show->smartshopperShow.itemAmounts[1] );
	...
	case 7:
		StringCopy( gStringVar2,
			ItemId_GetName( show->smartshopperShow.itemIds[2] ) );
		TV_PrintIntToStringVar(
			2, show->smartshopperShow.itemAmounts[2] );
		if( show->smartshopperShow.priceReduced == TRUE )
		{
			sTVShowState = 8;
		}
...
static void DoTVShowTheNameRaterShow( void )
{
	...
	case 15:
		...
		TV_GetNicknameSubstring(
			2, 0, 3, 2, show->nameRaterShow.species, show );
		sTVShowState = 16;
		break;
	case 16:
		TV_GetNicknameSubstring(
			0, 0, 2, 2, show->nameRaterShow.species, show );
		TV_GetNicknameSubstring( 2, 0, 3, 1, 0, show );
		sTVShowState = 17;
		break;
	case 17:
		...
		TV_GetNicknameSubstring(
			2, 0, 3, 2, show->nameRaterShow.randomSpecies, show );
		sTVShowState = 18;
		break;
	case 12:
		state        = 18;
...
static void DoTVShowPokemonTodaySuccessfulCapture( void )
{
	...
	case 2:
		StringCopy( gStringVar2,
			ItemId_GetName( show->pokemonToday.ball ) );
...
static void DoTVShowPokemonTodayFailedCapture( void )
{
	...
	case 1:
		TVShowConvertInternationalString( gStringVar1,
			...
			show->pokemonTodayFailed.language );
		GetMapName(
			gStringVar2, show->pokemonTodayFailed.location, 0 );
	...
	case 3:
		TVShowConvertInternationalString( gStringVar1,
			...
			show->pokemonTodayFailed.language );
		TV_PrintIntToStringVar(
			1, show->pokemonTodayFailed.nBallsUsed );
		if( Random( ) % 3 == 0 )
		{
			sTVShowState = 5;
		}
...
static void DoTVShowPokemonFanClubLetter( void )
{
	...
	case 50:
		ConvertEasyChatWordsToString(
			gStringVar4, show->fanclubLetter.words, 2, 2 );
...
static void DoTVShowRecentHappenings( void )
{
	...
	case 50:
		ConvertEasyChatWordsToString(
			gStringVar4, show->recentHappenings.words, 2, 2 );
		ShowFieldMessage( gStringVar4 );
		sTVShowState = 1;
		return;
	}
...
static void DoTVShowPokemonFanClubOpinions( void )
{
	...
	case 3:
		...
		StringCopy( gStringVar2,
			gSpeciesNames[show->fanclubOpinions.species] );
		CopyEasyChatWord(
			gStringVar3, show->fanclubOpinions.words[0] );
		sTVShowState = 4;
		break;
	case 4:
		TVShowConvertInternationalString( gStringVar1,
			...
			show->fanclubOpinions.language );
		CopyEasyChatWord(
			gStringVar3, show->fanclubOpinions.words[1] );
		TVShowDone( );
		break;
	}
...
static void DoTVShowPokemonContestLiveUpdates( void )
{
	...
	case 0:
		...
		StringCopy( gStringVar2,
			gSpeciesNames[show->contestLiveUpdates.species] );
		TVShowConvertInternationalString( gStringVar3,
			show->contestLiveUpdates.playerName,
			show->contestLiveUpdates.language );
		if( show->contestLiveUpdates.round1Rank ==
			show->contestLiveUpdates.round2Rank )
		{
		...
		else if( show->contestLiveUpdates.round1Rank >
			show->contestLiveUpdates.round2Rank )
		{
			sTVShowState = 2;
		}
	...
	case 22:
		StringCopy( gStringVar2,
			gSpeciesNames[show->contestLiveUpdates.species] );
		StringCopy( gStringVar3,
			gMoveNames[show->contestLiveUpdates.move] );
		sTVShowState = 23;
		break;
	case 23:
		StringCopy( gStringVar1,
		...
		TVShowConvertInternationalString( gStringVar2,
			...
			show->contestLiveUpdates.winningTrainerLanguage );
		StringCopy( gStringVar3,
			gSpeciesNames[show->contestLiveUpdates
					      .winningSpecies] );
...
void DoTVShowInSearchOfTrainers( void )
{
	...
	case 0:
		GetMapName( gStringVar1,
			gSaveBlock1Ptr->gabbyAndTyData.mapnum,
			0 );
...
static void DoTVShowTheWorldOfMasters( void )
{
	...
	case 0:
		...
		TV_PrintIntToStringVar(
			2, show->worldOfMasters.numPokeCaught );
		sTVShowState = 1;
		break;
	case 1:
		StringCopy( gStringVar1,
...
static void DoTVShowTodaysRivalTrainer( void )
{
	...
	case 1:
		...
		{
			if( show->rivalTrainer.nSilverSymbols ||
				show->rivalTrainer.nGoldSymbols )
			{
				sTVShowState = 4;
			}
...
static void DoTVShowHoennTreasureInvestigators( void )
{
	...
	case 0:
		StringCopy( gStringVar1,
			ItemId_GetName( show->treasureInvestigators.item ) );
	...
	case 1:
		...
		GetMapName(
			gStringVar3, show->treasureInvestigators.location, 0 );
		TVShowDone( );
		break;
	case 2:
		StringCopy( gStringVar1,
...
static void DoTVShowBreakingNewsTV( void )
{
	...
	case 1:
		TVShowConvertInternationalString( gStringVar1,
			show->breakingNews.playerName,
			show->breakingNews.language );
		StringCopy( gStringVar2,
			gSpeciesNames[show->breakingNews
					      .lastOpponentSpecies] );
	...
	case 3:
		...
		StringCopy( gStringVar2,
			ItemId_GetName( show->breakingNews.caughtMonBall ) );
		sTVShowState = 4;
		break;
	case 4:
		TVShowConvertInternationalString( gStringVar1,
...
static void DoTVShowSecretBaseVisit( void )
{
	...
	case 12:
		...
		StringCopy(
			gStringVar3, gMoveNames[show->secretBaseVisit.move] );
		sTVShowState = 13;
		break;
	case 13:
		TVShowDone( );
...
static void DoTVShowThePokemonBattleSeminar( void )
{
	...
	case 1:
		...
		StringCopy( gStringVar2,
			gSpeciesNames[show->battleSeminar.foeSpecies] );
		StringCopy(
			gStringVar3, gMoveNames[show->battleSeminar.move] );
		sTVShowState = 2;
		break;
	case 2:
		StringCopy( gStringVar1,
	...
	case 6:
		...
		StringCopy(
			gStringVar2, gMoveNames[show->battleSeminar.move] );
		TVShowDone( );
		break;
	}
...
static void DoTVShowTrainerFanClub( void )
{
	...
	case 0:
		...
		playerId = ( ( show->common.trainerIdHi << 8 ) +
			show->common.trainerIdLo );
		switch( playerId % 10 )
		{
		case 0:
			sTVShowState = 1;
...
static void DoTVShowPokemonNewsBattleFrontier( void )
{
	...
	case 14:
		StringCopy(
			gStringVar1, gSpeciesNames[show->frontier.species1] );
		StringCopy(
			gStringVar2, gSpeciesNames[show->frontier.species2] );
		StringCopy(
			gStringVar3, gSpeciesNames[show->frontier.species3] );
		sTVShowState = 18;
		break;
	case 15:
		StringCopy(
	...
	case 17:
		StringCopy(
			gStringVar1, gSpeciesNames[show->frontier.species4] );
		sTVShowState = 18;
		break;
	case 18:
		TVShowConvertInternationalString( gStringVar1,
...
static void DoTVShowSecretBaseSecrets( void )
{
	...
	case 0:
		...
		else
		{
			...
			sTVSecretBaseSecretsRandomValues[0] =
				Random( ) % bitCount;
			sTVShowState = SecretBaseSecrets_GetStateForFlagNumber(
	...
	case 3:
		...
		TVShowConvertInternationalString( gStringVar2,
			...
			show->secretBaseSecrets.language );
		TV_PrintIntToStringVar(
			2, show->secretBaseSecrets.stepsInBase );
	...
	case 19:
		StringCopy( gStringVar2,
			ItemId_GetName( show->secretBaseSecrets.item ) );
		sTVShowState = show->secretBaseSecrets.savedState;
		break;
	case 20:
...
static void DoTVShowPokemonContestLiveUpdates2( void )
{
	...
	{
	case 0:
		sub_818E868( gStringVar1,
			show->contestLiveUpdates2.contestCategory );
