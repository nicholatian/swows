...
static void sub_8064520( void )
{
	...
	else
	{
		if( gSprites[gHealthboxSpriteIds[gActiveBattler]].callback ==
				SpriteCallbackDummy &&
			gSprites[gHealthboxSpriteIds[BATTLE_PARTNER(
					 gActiveBattler )]]
	...
	if( r8 )
	{
				...
				BATTLE_PARTNER( gActiveBattler ) ) )
		{
			if( !gBattleSpritesDataPtr
				...
				return;
			if( !gBattleSpritesDataPtr
					->healthBoxesData[BATTLE_PARTNER(
						gActiveBattler )]
					.field_1_x1 )
				return;
...
static void sub_8064734( void )
{
			 ...
			 .field_1_x1 )
	{
		sub_8172EF0( BATTLE_PARTNER( gActiveBattler ),
			&gEnemyParty[gBattlerPartyIndexes[BATTLE_PARTNER(
				gActiveBattler )]] );
	}
	...
	if( r10 )
	{
				...
				SpriteCallbackDummy )
		{
			...
			if( IsDoubleBattle( ) &&
				!( gBattleTypeFlags & BATTLE_TYPE_MULTI ) )
			{
				DestroySprite( &gSprites[gUnknown_03005D7C
						[BATTLE_PARTNER(
							gActiveBattler )]] );
				SetBattlerShadowSpriteCallback(
					...
					GetMonData(
						&gEnemyParty
							[gBattlerPartyIndexes[BATTLE_PARTNER(
								gActiveBattler )]],
						MON_DATA_SPECIES ) );
			}
...
static void LinkOpponentHandleDrawTrainerPic( void )
{
	...
	if( gBattleTypeFlags & BATTLE_TYPE_MULTI )
	{
		...
		{
			if( ( gLinkPlayers[GetBattlerMultiplayerId(
						   gActiveBattler )]
					    .version &
				    0xFF ) == VERSION_FIRE_RED ||
				( gLinkPlayers[GetBattlerMultiplayerId(
						       gActiveBattler )]
						.version &
					0xFF ) == VERSION_LEAF_GREEN )
			{
				if( gLinkPlayers[GetBattlerMultiplayerId(
							 gActiveBattler )]
						.gender != MALE )
					trainerPicId = gFacilityClassToPicIndex
						[FACILITY_CLASS_LEAF];
				else
					trainerPicId = gFacilityClassToPicIndex
						[FACILITY_CLASS_RED];
			}
			else if( ( gLinkPlayers[GetBattlerMultiplayerId(
							gActiveBattler )]
						 .version &
					 0xFF ) == VERSION_RUBY ||
				( gLinkPlayers[GetBattlerMultiplayerId(
	...
	else
	{
		...
		else if( ( gLinkPlayers[GetMultiplayerId( ) ^ BIT_SIDE]
					 .version &
				 0xFF ) == VERSION_FIRE_RED ||
			( gLinkPlayers[GetMultiplayerId( ) ^ BIT_SIDE]
					.version &
				0xFF ) == VERSION_LEAF_GREEN )
		{
			if( gLinkPlayers[GetMultiplayerId( ) ^ BIT_SIDE]
					.gender != MALE )
				trainerPicId = gFacilityClassToPicIndex
					[FACILITY_CLASS_LEAF];
			else
...
static void LinkOpponentHandleTrainerSlide( void )
{
	...
	gBattlerControllerFuncs[gActiveBattler] =
		CompleteOnBankSpriteCallbackDummy2; // this line is redundant,
						    // because
						    // LinkOpponentBufferExecCompleted
						    // changes the battle
						    // battlerId function
...
static void LinkOpponentHandleIntroTrainerBallThrow( void )
{
	...
	StoreSpriteCallbackInData6(
		&gSprites[gBattlerSpriteIds[gActiveBattler]], sub_80676FC );
...
static void LinkOpponentHandleCmd55( void )
{
	...
	if( gBattleBufferA[gActiveBattler][1] == B_OUTCOME_DREW )
		...
		gBattleOutcome =
			gBattleBufferA[gActiveBattler][1] ^ B_OUTCOME_DREW;
