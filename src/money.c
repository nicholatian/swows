...
static const struct CompressedSpritePalette sSpritePalette_MoneyLabel = {
	.data = gMenuMoneyPal, .tag = MONEY_LABEL_TAG};
...
void PrintMoneyAmount( u8 windowId, u8 x, u8 y, int amount, u8 speed )
{
	...
	ConvertIntToDecimalStringN(
		gStringVar1, amount, STR_CONV_MODE_LEFT_ALIGN, 6 );
	...
	AddTextPrinterParameterized(
		windowId, 1, gStringVar4, x, y, speed, NULL );
}

void PrintMoneyAmountInMoneyBoxWithBorder(
	u8 windowId, u16 tileStart, u8 pallete, int amount )
{
	DrawStdFrameWithCustomTileAndPalette(
		windowId, FALSE, tileStart, pallete );
...
void DrawMoneyBox( int amount, u8 x, u8 y )
{
	...
	PrintMoneyAmountInMoneyBoxWithBorder(
		sMoneyBoxWindowId, 0x214, 14, amount );
...
void AddMoneyLabelObject( u16 x, u16 y )
{
	...
	sMoneyLabelSpriteId =
		CreateSprite( &sSpriteTemplate_MoneyLabel, x, y, 0 );
}
