...
static void sub_810A308( struct Sprite* sprite )
{
	...
	else
	{
		...
		sprite->pos1.y = GetBattlerSpriteCoord(
			battler, BATTLER_COORD_Y_PIC_OFFSET );
	}

	sprite->pos2.x =
		( gSineTable[gBattleAnimArgs[0]] * gBattleAnimArgs[1] ) >> 8;
	sprite->pos2.y =
		( gSineTable[gBattleAnimArgs[0] + 64] * gBattleAnimArgs[1] ) >>
		8;
...
static void sub_810AA2C( struct Sprite* sprite )
{
	...
	sprite->pos1.x += gBattleAnimArgs[0];
	sprite->pos1.y += gBattleAnimArgs[1];
	spriteId = CreateSprite( &gUnknown_08595840,
		sprite->pos1.x + 32,
		sprite->pos1.y,
		sprite->subpriority );
...
void sub_810AAFC( u8 taskId )
{
	...
	if( !gBattleAnimArgs[0] )
	{
		...
		task->data[15] = GetBattlerSpriteCoord(
...
void sub_810AB78( u8 taskId )
{
	...
	if( task->data[6] )
	{
		if( ++task->data[12] > task->data[13] )
		{
			u8 spriteId;
			task->data[12] = 0;
			spriteId       = CreateSprite( &gUnknown_085958A8,
                                task->data[14],
                                task->data[15],
                                2 );
			if( spriteId != MAX_SPRITES )
			{
				...
				sprite->pos1.x +=
					gUnknown_08595858[task->data[9]][0];
				sprite->pos1.y +=
					gUnknown_08595858[task->data[9]][1];
...
void sub_810AFCC( u8 taskId )
{
	...
	case 0:
		...
		{
		case 0:
			task->data[3] = GetBattlerSpriteCoord(
				gBattleAnimAttacker, BATTLER_COORD_X_2 );
			task->data[5] =
	...
	case 1:
		...
		{
			task->data[2] = 0;
			if( sub_810B154( task, taskId ) ||
				sub_810B154( task, taskId ) )
				task->data[0]++;
		}
		break;
	case 2:
...
static bool8 sub_810B154( struct Task* task, u8 taskId )
{
	u8 spriteId = CreateSprite(
		&gUnknown_08595A00, task->data[3], task->data[5], 35 );
	if( spriteId != MAX_SPRITES )
	{
...
void sub_810B29C( u8 taskId )
{
	...
	{
	case 0:
		task->data[6] = GetBattlerSpriteCoord(
			gBattleAnimAttacker, BATTLER_COORD_X_2 );
		task->data[7] = GetBattlerSpriteCoord(
			gBattleAnimAttacker, BATTLER_COORD_Y_PIC_OFFSET );
		task->data[8]  = 4;
		task->data[10] = GetBattlerSpriteCoord(
			gBattleAnimTarget, BATTLER_COORD_X_2 );
		...
		task->data[11] = 12;
		task->data[12] =
			BattleAnimAdjustPanning( task->data[11] - 76 );
...
{
	u8 spriteId = CreateSprite(
		&gUnknown_08595A30, task->data[6], task->data[7], 35 );
	if( spriteId != MAX_SPRITES )
	{
...
{
	u8 spriteId = CreateSprite( &gUnknown_085956C0,
		task->data[13],
		task->data[14],
		task->data[12] );
