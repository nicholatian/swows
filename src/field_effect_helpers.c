...
#define EVENT_OBJ_PAL_TAG_NONE \
	0x11FF // duplicate of define in event_object_movement.c

static void UpdateObjectReflectionSprite( struct Sprite* );
static void LoadObjectReflectionPalette(
...
static void CreateBobbingEffect(
	struct EventObject*, struct Sprite*, struct Sprite* );
...
void SetUpReflection( struct EventObject* eventObject,
	struct Sprite* sprite,
	bool8 stillReflection )
{
	struct Sprite* reflectionSprite;

	reflectionSprite               = &gSprites[CreateCopySpriteAt(
                sprite, sprite->pos1.x, sprite->pos1.y, 0x98 )];
...
{
	return GetEventObjectGraphicsInfo( eventObject->graphicsId )->height -
		2;
}

#define EVENT_OBJ_PAL_TAG_10 0x1102

void LoadObjectReflectionPalette(
	struct EventObject* eventObject, struct Sprite* sprite )
{
	u8 bridgeType;
	u16 bridgeReflectionVerticalOffsets[] = {12, 28, 44};
	sprite->data[2]                       = 0;
	if( !GetEventObjectGraphicsInfo( eventObject->graphicsId )
	...
	{
		// When walking on a bridge high above water (Route 120), the
		// reflection is a solid dark blue color. This is so the sprite
		// blends in with the dark water metatile underneath the
		// bridge.
		sprite->data[2] =
			bridgeReflectionVerticalOffsets[bridgeType - 1];
		LoadEventObjectPalette( EVENT_OBJ_PAL_TAG_10 );
		sprite->oam.paletteNum =
			IndexOfSpritePaletteTag( EVENT_OBJ_PAL_TAG_10 );
...
void LoadSpecialReflectionPalette( struct Sprite* sprite )
{
	...
	CpuCopy16( &gPlttBufferUnfaded[0x100 + sprite->oam.paletteNum * 16],
	...
	/*
	   ...
	   case 0:
	   default:*/
	TintPalette_CustomTone( gReflectionPaletteBuffer,
		16,
		Q_8_8( 1.0 ),
		Q_8_8( 1.0 ),
		Q_8_8( 3.5 ) );
...
static void UpdateObjectReflectionSprite( struct Sprite* reflectionSprite )
{
	...
	if( !eventObject->active || !eventObject->hasReflection ||
		eventObject->localId != reflectionSprite->data[1] )
	{
	...
	else
	{
		...
		reflectionSprite->oam.matrixNum =
			mainSprite->oam.matrixNum | 0x10;
		reflectionSprite->oam.tileNum = mainSprite->oam.tileNum;
		reflectionSprite->subspriteTables =
			mainSprite->subspriteTables;
		reflectionSprite->subspriteTableNum =
			mainSprite->subspriteTableNum;
		reflectionSprite->invisible = mainSprite->invisible;
		reflectionSprite->pos1.x    = mainSprite->pos1.x;
		// reflectionSprite->data[2] holds an additional vertical
		// offset, used by the high bridges on Route 120
		reflectionSprite->pos1.y = mainSprite->pos1.y +
			GetReflectionVerticalOffset( eventObject ) +
			reflectionSprite->data[2];
		reflectionSprite->centerToCornerVecX =
			mainSprite->centerToCornerVecX;
		reflectionSprite->centerToCornerVecY =
			mainSprite->centerToCornerVecY;
		reflectionSprite->pos2.x = mainSprite->pos2.x;
		reflectionSprite->pos2.y = -mainSprite->pos2.y;
		reflectionSprite->coordOffsetEnabled =
			mainSprite->coordOffsetEnabled;
		...
		{
			// Sets the reflection sprite's rot/scale matrix to the
			// appropriate matrix based on whether or not the main
			// sprite is horizontally flipped. If the sprite is
			// facing to the east, then it is flipped, and its
			// matrixNum is 8.
...
u8 CreateWarpArrowSprite( void )
{
	...
	spriteId = CreateSpriteAtEnd(
		gFieldEffectObjectTemplatePointers[8], 0, 0, 0x52 );
...
const u8 gShadowEffectTemplateIds[] = {0, 1, 2, 3};
...
u32 FldEff_Shadow( void )
{
	...
	eventObjectId =
		GetEventObjectIdByLocalIdAndMap( gFieldEffectArguments[0],
			gFieldEffectArguments[1],
			gFieldEffectArguments[2] );
	graphicsInfo = GetEventObjectGraphicsInfo(
		gEventObjects[eventObjectId].graphicsId );
	LoadFieldEffectPalette(
		gShadowEffectTemplateIds[graphicsInfo->shadowSize] );
	spriteId = CreateSpriteAtEnd(
...
void UpdateShadowFieldEffect( struct Sprite* sprite )
{
	...
	if( TryGetEventObjectIdByLocalIdAndMap( sprite->data[0],
		    sprite->data[1],
		    sprite->data[2],
		    &eventObjectId ) )
	{
...
u32 FldEff_TallGrass( void )
{
	...
	sub_80930E0( &x, &y, 8, 8 );
	spriteId = CreateSpriteAtEnd(
		gFieldEffectObjectTemplatePointers[4], x, y, 0 );
	if( spriteId != MAX_SPRITES )
	{
		sprite                     = &gSprites[spriteId];
		sprite->coordOffsetEnabled = TRUE;
...
void UpdateTallGrassFieldEffect( struct Sprite* sprite )
{
			...
			gSaveBlock1Ptr->location.mapGroup != mapGroup ) )
	{
		...
		sprite->data[5] =
			( (u8)gSaveBlock1Ptr->location.mapNum << 8 ) |
	...
	{
		eventObject = &gEventObjects[eventObjectId];
		if( ( eventObject->currentCoords.x != sprite->data[1] ||
			    eventObject->currentCoords.y !=
				    sprite->data[2] ) &&
			( eventObject->previousCoords.x != sprite->data[1] ||
				eventObject->previousCoords.y !=
					sprite->data[2] ) )
			sprite->data[7] = TRUE;
...
u32 FldEff_JumpTallGrass( void )
{
	...
	sub_80930E0( (s16*)&gFieldEffectArguments[0],
		...
		8,
		12 );
	spriteId = CreateSpriteAtEnd( gFieldEffectObjectTemplatePointers[10],
...
u32 FldEff_LongGrass( void )
{
	...
	sub_80930E0( &x, &y, 8, 8 );
	spriteId = CreateSpriteAtEnd(
		gFieldEffectObjectTemplatePointers[15], x, y, 0 );
	if( spriteId != MAX_SPRITES )
	{
		sprite                     = &gSprites[spriteId];
		sprite->coordOffsetEnabled = TRUE;
		sprite->oam.priority =
			ZCoordToPriority( gFieldEffectArguments[2] );
		sprite->data[0] = gFieldEffectArguments[2];
		sprite->data[1] = gFieldEffectArguments[0];
		sprite->data[2] = gFieldEffectArguments[1];
		sprite->data[3] = gFieldEffectArguments[4];
...
u32 FldEff_JumpLongGrass( void )
{
	...
	sub_80930E0( (s16*)&gFieldEffectArguments[0],
		...
		8,
		8 );
	spriteId = CreateSpriteAtEnd( gFieldEffectObjectTemplatePointers[16],
		gFieldEffectArguments[0],
		gFieldEffectArguments[1],
		0 );
	if( spriteId != MAX_SPRITES )
...
u32 FldEff_ShortGrass( void )
{
	...
	eventObject = &gEventObjects[eventObjectId];
	spriteId    = CreateSpriteAtEnd(
                gFieldEffectObjectTemplatePointers[30], 0, 0, 0 );
	if( spriteId != MAX_SPRITES )
	{
		sprite                     = &( gSprites[spriteId] );
		sprite->coordOffsetEnabled = TRUE;
		sprite->oam.priority =
			gSprites[eventObject->spriteId].oam.priority;
...
void UpdateShortGrassFieldEffect( struct Sprite* sprite )
{
	...
	if( TryGetEventObjectIdByLocalIdAndMap( sprite->data[0],
		    ...
		    sprite->data[2],
		    &eventObjectId ) ||
		!gEventObjects[eventObjectId].inShortGrass )
	{
	...
	else
	{
		...
		sprite->oam.priority = linkedSprite->oam.priority;
		UpdateEventObjectSpriteVisibility(
			sprite, linkedSprite->invisible );
	}
}
...
u32 FldEff_SandFootprints( void )
{
	...
	sub_80930E0( (s16*)&gFieldEffectArguments[0],
		...
		8,
		8 );
	spriteId = CreateSpriteAtEnd( gFieldEffectObjectTemplatePointers[11],
		gFieldEffectArguments[0],
		gFieldEffectArguments[1],
		gFieldEffectArguments[2] );
	if( spriteId != MAX_SPRITES )
...
u32 FldEff_DeepSandFootprints( void )
{
	...
	sub_80930E0( (s16*)&gFieldEffectArguments[0],
		...
		8,
		8 );
	spriteId = CreateSpriteAtEnd( gFieldEffectObjectTemplatePointers[23],
		gFieldEffectArguments[0],
		gFieldEffectArguments[1],
		gFieldEffectArguments[2] );
	if( spriteId != MAX_SPRITES )
...
u32 FldEff_BikeTireTracks( void )
{
	...
	sub_80930E0( (s16*)&gFieldEffectArguments[0],
		...
		8,
		8 );
	spriteId = CreateSpriteAtEnd( gFieldEffectObjectTemplatePointers[27],
		gFieldEffectArguments[0],
		gFieldEffectArguments[1],
		gFieldEffectArguments[2] );
	if( spriteId != MAX_SPRITES )
...
u32 FldEff_Splash( void )
{
	...
	eventObject = &gEventObjects[eventObjectId];
	spriteId    = CreateSpriteAtEnd(
                gFieldEffectObjectTemplatePointers[13], 0, 0, 0 );
	if( spriteId != MAX_SPRITES )
	{
		graphicsInfo =
...
void UpdateSplashFieldEffect( struct Sprite* sprite )
{
	...
	if( sprite->animEnded ||
		TryGetEventObjectIdByLocalIdAndMap( sprite->data[0],
			sprite->data[1],
			sprite->data[2],
			&eventObjectId ) )
	{
	...
	{
		sprite->pos1.x =
			gSprites[gEventObjects[eventObjectId].spriteId].pos1.x;
		sprite->pos1.y =
			gSprites[gEventObjects[eventObjectId].spriteId].pos1.y;
		UpdateEventObjectSpriteVisibility( sprite, FALSE );
	}
}
...
u32 FldEff_JumpSmallSplash( void )
{
	...
	sub_80930E0( (s16*)&gFieldEffectArguments[0],
		...
		8,
		12 );
	spriteId = CreateSpriteAtEnd( gFieldEffectObjectTemplatePointers[14],
		gFieldEffectArguments[0],
		gFieldEffectArguments[1],
		0 );
	if( spriteId != MAX_SPRITES )
...
u32 FldEff_JumpBigSplash( void )
{
	...
	sub_80930E0( (s16*)&gFieldEffectArguments[0],
		...
		8,
		8 );
	spriteId = CreateSpriteAtEnd( gFieldEffectObjectTemplatePointers[12],
		gFieldEffectArguments[0],
		gFieldEffectArguments[1],
		0 );
	if( spriteId != MAX_SPRITES )
...
static void UpdateFeetInFlowingWaterFieldEffect( struct Sprite* sprite )
{
	...
	if( TryGetEventObjectIdByLocalIdAndMap( sprite->data[0],
		    ...
		    sprite->data[2],
		    &eventObjectId ) ||
		!gEventObjects[eventObjectId].inShallowFlowingWater )
	{
	...
	else
	{
		...
		UpdateEventObjectSpriteVisibility( sprite, FALSE );
		if( eventObject->currentCoords.x != sprite->data[3] ||
			eventObject->currentCoords.y != sprite->data[4] )
		{
...
u32 FldEff_Ripple( void )
{
	...
	spriteId = CreateSpriteAtEnd( gFieldEffectObjectTemplatePointers[5],
		gFieldEffectArguments[0],
		gFieldEffectArguments[1],
		gFieldEffectArguments[2] );
	if( spriteId != MAX_SPRITES )
...
u32 FldEff_HotSpringsWater( void )
{
	...
	eventObject = &gEventObjects[eventObjectId];
	spriteId    = CreateSpriteAtEnd(
                gFieldEffectObjectTemplatePointers[31], 0, 0, 0 );
	if( spriteId != MAX_SPRITES )
	{
		sprite                     = &gSprites[spriteId];
		sprite->coordOffsetEnabled = TRUE;
...
void UpdateHotSpringsWaterFieldEffect( struct Sprite* sprite )
{
	...
	if( TryGetEventObjectIdByLocalIdAndMap( sprite->data[0],
		    ...
		    sprite->data[2],
		    &eventObjectId ) ||
		!gEventObjects[eventObjectId].inHotSprings )
	{
	...
	else
	{
		...
		linkedSprite =
			&gSprites[gEventObjects[eventObjectId].spriteId];
		sprite->pos1.x = linkedSprite->pos1.x;
		sprite->pos1.y = ( graphicsInfo->height >> 1 ) +
			linkedSprite->pos1.y - 8;
		sprite->subpriority = linkedSprite->subpriority - 1;
		UpdateEventObjectSpriteVisibility( sprite, FALSE );
	}
}
...
u32 FldEff_Unknown19( void )
{
	...
	sub_80930E0( (s16*)&gFieldEffectArguments[0],
		...
		8,
		8 );
	spriteId = CreateSpriteAtEnd( gFieldEffectObjectTemplatePointers[17],
		gFieldEffectArguments[0],
		gFieldEffectArguments[1],
		gFieldEffectArguments[2] );
	if( spriteId != MAX_SPRITES )
...
u32 FldEff_Unknown20( void )
{
	...
	sub_80930E0( (s16*)&gFieldEffectArguments[0],
		...
		8,
		8 );
	spriteId = CreateSpriteAtEnd( gFieldEffectObjectTemplatePointers[18],
		gFieldEffectArguments[0],
		gFieldEffectArguments[1],
		gFieldEffectArguments[2] );
	if( spriteId != MAX_SPRITES )
...
u32 FldEff_Unknown21( void )
{
	...
	sub_80930E0( (s16*)&gFieldEffectArguments[0],
		...
		8,
		8 );
	spriteId = CreateSpriteAtEnd( gFieldEffectObjectTemplatePointers[19],
		gFieldEffectArguments[0],
		gFieldEffectArguments[1],
		gFieldEffectArguments[2] );
	if( spriteId != MAX_SPRITES )
...
u32 FldEff_Unknown22( void )
{
	...
	sub_80930E0( (s16*)&gFieldEffectArguments[0],
		...
		8,
		8 );
	spriteId = CreateSpriteAtEnd( gFieldEffectObjectTemplatePointers[20],
		gFieldEffectArguments[0],
		gFieldEffectArguments[1],
		gFieldEffectArguments[2] );
	if( spriteId != MAX_SPRITES )
...
u32 FldEff_Ash( void )
{
	...
	y = gFieldEffectArguments[1];
	sub_80930E0( &x, &y, 8, 8 );
	spriteId = CreateSpriteAtEnd( gFieldEffectObjectTemplatePointers[6],
		x,
		y,
		gFieldEffectArguments[2] );
	if( spriteId != MAX_SPRITES )
...
static void UpdateAshFieldEffect_Step1( struct Sprite* sprite )
{
	...
	MapGridSetMetatileIdAt(
		sprite->data[1], sprite->data[2], sprite->data[3] );
	CurrentMapDrawMetatileAt( sprite->data[1], sprite->data[2] );
	gEventObjects[gPlayerAvatar.eventObjectId].triggerGroundEffectsOnMove =
		TRUE;
	sprite->data[0] = 2;
}
...
u32 FldEff_SurfBlob( void )
{
	...
	spriteId = CreateSpriteAtEnd( gFieldEffectObjectTemplatePointers[7],
		gFieldEffectArguments[0],
		gFieldEffectArguments[1],
		0x96 );
	if( spriteId != MAX_SPRITES )
...
void sub_81555AC( u8 spriteId, u8 value )
{
	gSprites[spriteId].data[0] =
		( gSprites[spriteId].data[0] & ~0xF ) | ( value & 0xF );
}

void sub_81555D8( u8 spriteId, u8 value )
{
	gSprites[spriteId].data[0] = ( gSprites[spriteId].data[0] & ~0xF0 ) |
		( ( value & 0xF ) << 4 );
}

void sub_8155604( u8 spriteId, u8 value, s16 data1 )
{
	gSprites[spriteId].data[0] = ( gSprites[spriteId].data[0] & ~0xF00 ) |
		( ( value & 0xF ) << 8 );
	...
	struct EventObject* eventObject, struct Sprite* sprite )
{
	...
	if( sub_8155640( sprite ) == 0 )
		StartSpriteAnimIfDifferent( sprite,
			surfBlobDirectionAnims[eventObject
						       ->movementDirection] );
}
...
void sub_81556E8( struct EventObject* eventObject, struct Sprite* sprite )
{
	...
	if( spriteY == 0 && ( x != sprite->data[6] || y != sprite->data[7] ) )
	{
		...
		for( i = DIR_SOUTH; i <= DIR_EAST;
			i++, x = sprite->data[6], y = sprite->data[7] )
		{
			MoveCoords( i, &x, &y );
...
static void CreateBobbingEffect( struct EventObject* eventObject,
	struct Sprite* linkedSprite,
	struct Sprite* sprite )
{
	...
	if( v0 != 0 )
	{
		if( ( ( u16 )( ++sprite->data[4] ) &
			    unk_085CDC6A[sprite->data[5]] ) == 0 )
		{
...
u32 FldEff_Dust( void )
{
	...
	sub_80930E0( (s16*)&gFieldEffectArguments[0],
		...
		8,
		12 );
	spriteId = CreateSpriteAtEnd( gFieldEffectObjectTemplatePointers[9],
		gFieldEffectArguments[0],
		gFieldEffectArguments[1],
		0 );
	if( spriteId != MAX_SPRITES )
...
u32 FldEff_SandPile( void )
{
	...
	eventObject = &gEventObjects[eventObjectId];
	spriteId    = CreateSpriteAtEnd(
                gFieldEffectObjectTemplatePointers[29], 0, 0, 0 );
	if( spriteId != MAX_SPRITES )
	{
		graphicsInfo =
			GetEventObjectGraphicsInfo( eventObject->graphicsId );
...
void UpdateSandPileFieldEffect( struct Sprite* sprite )
{
	...
	if( TryGetEventObjectIdByLocalIdAndMap( sprite->data[0],
		    ...
		    sprite->data[2],
		    &eventObjectId ) ||
		!gEventObjects[eventObjectId].inSandPile )
	{
...
u32 FldEff_Bubbles( void )
{
	...
	sub_80930E0( (s16*)&gFieldEffectArguments[0],
		...
		8,
		0 );
	spriteId = CreateSpriteAtEnd( gFieldEffectObjectTemplatePointers[34],
		gFieldEffectArguments[0],
		gFieldEffectArguments[1],
		0x52 );
	if( spriteId != MAX_SPRITES )
...
u32 FldEff_BerryTreeGrowthSparkle( void )
{
	...
	spriteId = CreateSpriteAtEnd( gFieldEffectObjectTemplatePointers[22],
		gFieldEffectArguments[0],
		gFieldEffectArguments[1],
		gFieldEffectArguments[2] );
	if( spriteId != MAX_SPRITES )
...
static u32 ShowDisguiseFieldEffect( u8 fldEff, u8 templateIdx )
{
	...
	spriteId = CreateSpriteAtEnd(
		gFieldEffectObjectTemplatePointers[templateIdx], 0, 0, 0 );
	if( spriteId != MAX_SPRITES )
	{
		sprite = &gSprites[spriteId];
...
void UpdateDisguiseFieldEffect( struct Sprite* sprite )
{
	...
	if( TryGetEventObjectIdByLocalIdAndMap( sprite->data[2],
		    sprite->data[3],
		    sprite->data[4],
		    &eventObjectId ) )
	{
	...
	sprite->pos1.x    = linkedSprite->pos1.x;
	sprite->pos1.y =
		( graphicsInfo->height >> 1 ) + linkedSprite->pos1.y - 16;
	sprite->subpriority = linkedSprite->subpriority - 1;
	if( sprite->data[0] == 1 )
	{
		sprite->data[0]++;
...
u32 FldEff_Sparkle( void )
{
	...
	sub_80930E0( (s16*)&gFieldEffectArguments[0],
		...
		8,
		8 );
	spriteId = CreateSpriteAtEnd( gFieldEffectObjectTemplatePointers[35],
		gFieldEffectArguments[0],
		gFieldEffectArguments[1],
		0x52 );
	if( spriteId != MAX_SPRITES )
...
void sub_8155F80( struct Sprite* sprite )
{
	...
	case 0:
		SetGpuReg(
			REG_OFFSET_BG0VOFS, 0x78 - ( sprite->data[0] / 3 ) );
		...
		break;
	case 1:
		sprite->pos1.y = ( gSineTable[sprite->data[0] / 3] >> 2 ) +
			sprite->data[4];
...
static void sub_81561FC( struct Sprite* sprite, u8 z, u8 offset )
{
	...
	for( i = 0; i < EVENT_OBJECTS_COUNT; i++ )
	{
		...
		if( eventObject->active )
		{
			...
			if( xhi < linkedSprite->pos1.x &&
				var > linkedSprite->pos1.x )
			{
				lyhi = linkedSprite->pos1.y +
					linkedSprite->centerToCornerVecY;
				...
				if( ( lyhi < yhi || lyhi < ylo ) &&
					var > yhi &&
					sprite->subpriority <=
						linkedSprite->subpriority )
				{
					sprite->subpriority =
						linkedSprite->subpriority + 2;
					break;
				}
			}
		}
