...
static void CB2_ReshowBattleScreenAfterMenu( void )
{
	...
	case 5:
		...
		break;
	case 6:
		if( BattleLoadAllHealthBoxesGfx(
	...
	case 19: {
		...
		species = GetMonData(
			&gEnemyParty[gBattlerPartyIndexes[opponentBattler]],
			MON_DATA_SPECIES );
		SetBattlerShadowSpriteCallback( opponentBattler, species );

		if( IsDoubleBattle( ) )
		{
			opponentBattler = GetBattlerAtPosition(
			...
			SetBattlerShadowSpriteCallback(
				opponentBattler, species );
		}

		ActionSelectionCreateCursorAt(
			gActionSelectionCursor[gBattlerInMenuId], 0 );
...
static bool8 LoadBattlerSpriteGfx( u8 battler )
{
	if( battler < gBattlersCount )
	{
		...
		{
			if( !gBattleSpritesDataPtr->battlerData[battler]
					.behindSubstitute )
			...
			battler ==
				B_POSITION_PLAYER_LEFT ) // Should be checking
							 // position, not
							 // battler.
			DecompressTrainerBackPic(
				gSaveBlock2Ptr->playerGender, battler );
			...
			battler ==
				B_POSITION_PLAYER_LEFT ) // Should be checking
							 ...
							 // battler.
			DecompressTrainerBackPic(
				TRAINER_BACK_PIC_WALLY, battler );
		else if( !gBattleSpritesDataPtr->battlerData[battler]
				  .behindSubstitute )
...
static void CreateBattlerSprite( u8 battler )
{
	if( battler < gBattlersCount )
	{
		...
		if( GetBattlerSide( battler ) != B_SIDE_PLAYER )
		{
			...
			gSprites[gBattlerSpriteIds[battler]].oam.paletteNum =
				battler;
			gSprites[gBattlerSpriteIds[battler]].callback =
				SpriteCallbackDummy;
			...
			if( gBattleSpritesDataPtr->battlerData[battler]
					.transformSpecies == SPECIES_CASTFORM )
...
static void CreateHealthboxSprite( u8 battler )
{
	if( battler < gBattlersCount )
	{
		...
		if( gBattleTypeFlags & BATTLE_TYPE_SAFARI &&
			...
			healthboxSpriteId =
				CreateBattlerHealthboxSprites( battler );
		...
		if( GetBattlerPosition( battler ) ==
				B_POSITION_OPPONENT_RIGHT ||
			GetBattlerPosition( battler ) ==
				B_POSITION_PLAYER_RIGHT )
			DummyBattleInterfaceFunc(
				gHealthboxSpriteIds[battler], TRUE );
		else
			DummyBattleInterfaceFunc(
				gHealthboxSpriteIds[battler], FALSE );
