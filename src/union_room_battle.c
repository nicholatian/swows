...
static void sub_801A8B0(
	u8 windowId, u8 fontId, const u8* str, u8 x, u8 y, u8 mode );
bool32 sub_801AA30( u32* a0, u32* a1, u32* a2, u8 taskId );

const u16 gWirelessInfoScreenPal[] =
	INCBIN_U16( "graphics/interface/wireless_info_screen.gbapal" );
const u32 gWirelessInfoScreenGfx[] =
	INCBIN_U32( "graphics/interface/wireless_info_screen.4bpp.lz" );
const u32 gWirelessInfoScreenTilemap[] =
	INCBIN_U32( "graphics/interface/wireless_info_screen.bin.lz" );

const struct BgTemplate gUnknown_082F0D34[] = {
	{.bg = 0, .charBaseIndex = 2, .mapBaseIndex = 0x1F, .priority = 0},
	{.bg = 1, .charBaseIndex = 0, .mapBaseIndex = 0x08, .priority = 1}};
...
const u8* const gUnknown_082F0D5C[] = {gText_WirelessCommStatus,
	gText_PeopleTrading,
	gText_PeopleBattling,
	gText_PeopleInUnionRoom,
	gText_PeopleCommunicating};

const u8 gUnknown_082F0D70[][3] = {{0x01, 0x01, 0x02},
	{0x02, 0x01, 0x02},
	{0x03, 0x01, 0x04},
	{0x04, 0x00, 0x02},
	{0x06, 0x03, 0x02},
...
const struct WindowTemplate gUnknown_082F0DD4[] = {{.bg                 = 0,
							   .tilemapLeft = 3,
							   .tilemapTop  = 15,
							   .width       = 24,
							   .height      = 4,
...
void sub_801A42C( void ) { SetMainCallback2( sub_801A43C ); }
...
void sub_801A43C( void )
{
	...
	InitBgsFromTemplates(
		0, gUnknown_082F0D34, ARRAY_COUNT( gUnknown_082F0D34 ) );
...
void sub_801A600( void )
{
	...
	sub_801A8B0( 0,
		1,
		gUnknown_082F0D5C[0],
		GetStringCenterAlignXOffset( 1, gUnknown_082F0D5C[0], 0xC0 ),
		6,
		3 );
	for( i = 0; i < 3; i++ )
	{
		sub_801A8B0(
			1, 1, gUnknown_082F0D5C[i + 1], 0, 30 * i + 8, 1 );
	}
...
void sub_801A6C0( u8 taskId )
{
	...
	case 2:
		...
		break;
	case 3:
		if( sub_801AA30( gUnknown_03000DAC->unk00,
			    gUnknown_03000DAC->unk10,
			    gUnknown_03000DAC->unk20,
			    gUnknown_03000DAC->unk61 ) )
		{
			FillWindowPixelBuffer( 2, 0 );
			for( i = 0; i < 4; i++ )
			{
				ConvertIntToDecimalStringN( gStringVar4,
					gUnknown_03000DAC->unk00[i],
		...
		sub_801A5BC(
			&gTasks[taskId].data[7], &gTasks[taskId].data[8] );
		break;
	case 4:
	...
	u8 windowId, u8 fontId, const u8* str, u8 x, u8 y, u8 mode )
{
	...
	AddTextPrinterParameterized4(
		windowId, fontId, x, y, 0, 0, color, -1, str );
}
...
void sub_801AB68( u8 windowId, const u8* str, u8 x, u8 y, s32 speed )
{
	...
	FillWindowPixelBuffer( windowId,
		( gUnknown_082F0DE4[0] << 4 ) | gUnknown_082F0DE4[0] );
...
void sub_801AC54( void )
{
	...
	case 0:
		...
		break;
	case 1:
		if( sub_801ABDC( &gUnknown_02022C6C->a0,
	...
	case 4:
		...
		{
			if( gBlockRecvBuffer[0][0] == 0x51 &&
				gBlockRecvBuffer[1][0] == 0x51 )
			{
	...
	case 7:
		if( sub_801ABDC(
			    &gUnknown_02022C6C->a0, gText_RefusedBattle, 1 ) )
		{
