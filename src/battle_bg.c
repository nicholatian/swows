...
static const struct SpriteTemplate gUnknown_0831A9D0 = {.tileTag = 0x2710,
	.paletteTag                                              = 0x2710,
...
static const struct SpriteTemplate gUnknown_0831A9E8 = {.tileTag = 0x2710,
	.paletteTag                                              = 0x2710,
...
void BattleInitBgsAndWindows( void )
{
	...
	InitBgsFromTemplates(
		0, gBattleBgTemplates, ARRAY_COUNT( gBattleBgTemplates ) );
...
void sub_80356D0( void )
{
	...
	SetGpuReg( REG_OFFSET_DISPCNT,
		DISPCNT_OBJWIN_ON | DISPCNT_WIN0_ON | DISPCNT_OBJ_ON |
			DISPCNT_OBJ_1D_MAP );
}
...
void LoadBattleMenuWindowGfx( void )
{
	...
	if( gBattleTypeFlags & BATTLE_TYPE_ARENA )
	{
		...
		CpuCopy16( &gPlttBufferUnfaded[0x76],
			&gPlttBufferFaded[0x76],
			2 );
	}
}
...
void DrawMainBattleBackground( void )
{
			...
			BATTLE_TYPE_EREADER_TRAINER | BATTLE_TYPE_x2000000 ) )
	{
		...
		LoadCompressedPalette(
			gBattleTerrainPalette_Frontier, 0x20, 0x60 );
	}
	else if( gBattleTypeFlags & BATTLE_TYPE_GROUDON )
	{
		...
		LZDecompressVram( gBattleTerrainTilemap_Cave,
			(void*)( BG_SCREEN_ADDR( 26 ) ) );
		LoadCompressedPalette(
			gBattleTerrainPalette_Groudon, 0x20, 0x60 );
	}
	else if( gBattleTypeFlags & BATTLE_TYPE_KYOGRE )
	{
		LZDecompressVram( gBattleTerrainTiles_Water,
			(void*)( BG_CHAR_ADDR( 2 ) ) );
		LZDecompressVram( gBattleTerrainTilemap_Water,
			(void*)( BG_SCREEN_ADDR( 26 ) ) );
		LoadCompressedPalette(
			gBattleTerrainPalette_Kyogre, 0x20, 0x60 );
	}
	else if( gBattleTypeFlags & BATTLE_TYPE_RAYQUAZA )
	{
		LZDecompressVram( gBattleTerrainTiles_Rayquaza,
			(void*)( BG_CHAR_ADDR( 2 ) ) );
		LZDecompressVram( gBattleTerrainTilemap_Rayquaza,
			(void*)( BG_SCREEN_ADDR( 26 ) ) );
		LoadCompressedPalette(
			gBattleTerrainPalette_Rayquaza, 0x20, 0x60 );
	}
	else
	{
		if( gBattleTypeFlags & BATTLE_TYPE_TRAINER )
		{
			u8 trainerClass = gTrainers[gTrainerBattleOpponent_A]
						  .trainerClass;
			if( trainerClass == TRAINER_CLASS_LEADER )
			{
				...
				LZDecompressVram(
					...
					(void*)( BG_SCREEN_ADDR( 26 ) ) );
				LoadCompressedPalette(
					gBattleTerrainPalette_BuildingLeader,
					0x20,
					0x60 );
				return;
			}
			else if( trainerClass == TRAINER_CLASS_CHAMPION )
			{
				...
				LZDecompressVram(
					...
					(void*)( BG_SCREEN_ADDR( 26 ) ) );
				LoadCompressedPalette(
					gBattleTerrainPalette_StadiumWallace,
					0x20,
					0x60 );
				return;
			}
		...
		case MAP_BATTLE_SCENE_NORMAL:
			LZDecompressVram(
				gBattleTerrainTable[gBattleTerrain].tileset,
				(void*)( BG_CHAR_ADDR( 2 ) ) );
			LZDecompressVram(
				gBattleTerrainTable[gBattleTerrain].tilemap,
				(void*)( BG_SCREEN_ADDR( 26 ) ) );
			LoadCompressedPalette(
				gBattleTerrainTable[gBattleTerrain].palette,
				0x20,
				0x60 );
			break;
		case MAP_BATTLE_SCENE_GYM:
			...
			LZDecompressVram( gBattleTerrainTilemap_Building,
				(void*)( BG_SCREEN_ADDR( 26 ) ) );
			LoadCompressedPalette(
				gBattleTerrainPalette_BuildingGym,
				0x20,
				0x60 );
			break;
		case MAP_BATTLE_SCENE_MAGMA:
			...
			LZDecompressVram( gBattleTerrainTilemap_Stadium,
				(void*)( BG_SCREEN_ADDR( 26 ) ) );
			LoadCompressedPalette(
				gBattleTerrainPalette_StadiumMagma,
				0x20,
				0x60 );
			break;
		case MAP_BATTLE_SCENE_AQUA:
			...
			LZDecompressVram( gBattleTerrainTilemap_Stadium,
				(void*)( BG_SCREEN_ADDR( 26 ) ) );
			LoadCompressedPalette(
				gBattleTerrainPalette_StadiumAqua,
				0x20,
				0x60 );
			break;
		...
		case MAP_BATTLE_SCENE_PHOEBE:
			...
			LZDecompressVram( gBattleTerrainTilemap_Stadium,
				(void*)( BG_SCREEN_ADDR( 26 ) ) );
			LoadCompressedPalette(
				gBattleTerrainPalette_StadiumPhoebe,
				0x20,
				0x60 );
			break;
		case MAP_BATTLE_SCENE_GLACIA:
			...
			LZDecompressVram( gBattleTerrainTilemap_Stadium,
				(void*)( BG_SCREEN_ADDR( 26 ) ) );
			LoadCompressedPalette(
				gBattleTerrainPalette_StadiumGlacia,
				0x20,
				0x60 );
			break;
		case MAP_BATTLE_SCENE_DRAKE:
			...
			LZDecompressVram( gBattleTerrainTilemap_Stadium,
				(void*)( BG_SCREEN_ADDR( 26 ) ) );
			LoadCompressedPalette(
				gBattleTerrainPalette_StadiumDrake,
				0x20,
				0x60 );
			break;
...
static void sub_8035C4C( void )
{
	...
	else if( gBattleTypeFlags & BATTLE_TYPE_MULTI )
	{
		...
		{
			switch( gLinkPlayers[gBattleScripting.multiplayerId]
					.id )
			{
			case 0:
...
void sub_8035D74( u8 taskId )
{
	...
	case 1:
		...
		gPlttBufferUnfaded[palId * 16 + 0x10F] =
			gPlttBufferFaded[palId * 16 + 0x10F] = 0x7FFF;
		gBattleStruct->linkBattleVsSpriteId_V =
			CreateSprite( &gUnknown_0831A9D0, 111, 80, 0 );
		gBattleStruct->linkBattleVsSpriteId_S =
			CreateSprite( &gUnknown_0831A9E8, 129, 80, 0 );
		gSprites[gBattleStruct->linkBattleVsSpriteId_V].invisible =
			TRUE;
		gSprites[gBattleStruct->linkBattleVsSpriteId_S].invisible =
			TRUE;
		gTasks[taskId].data[0]++;
		break;
	case 2:
		...
		{
			gBattle_BG1_X = -( 20 ) -
				( Sin2( gTasks[taskId].data[1] ) / 32 );
			gBattle_BG2_X = -( 140 ) -
				( Sin2( gTasks[taskId].data[2] ) / 32 );
			gBattle_BG1_Y = -36;
			gBattle_BG2_Y = -36;
		}
		else
		{
			gBattle_BG1_X = -( 20 ) -
				( Sin2( gTasks[taskId].data[1] ) / 32 );
			gBattle_BG1_Y =
				( Cos2( gTasks[taskId].data[1] ) / 32 ) - 164;
			gBattle_BG2_X = -( 140 ) -
				( Sin2( gTasks[taskId].data[2] ) / 32 );
			gBattle_BG2_Y =
				( Cos2( gTasks[taskId].data[2] ) / 32 ) - 164;
		}
...
void DrawBattleEntryBackground( void )
{
	if( gBattleTypeFlags & BATTLE_TYPE_LINK )
	{
		...
		LZDecompressVram(
			gUnknown_08D77B0C, (void*)( VRAM + 0x10000 ) );
		...
		( BATTLE_TYPE_FRONTIER | BATTLE_TYPE_LINK |
			BATTLE_TYPE_x2000000 | BATTLE_TYPE_EREADER_TRAINER ) )
	{
	...
	{
		LZDecompressVram( gBattleTerrainAnimTiles_Cave,
			(void*)( BG_CHAR_ADDR( 1 ) ) );
	...
	else
	{
		if( gBattleTypeFlags & BATTLE_TYPE_TRAINER )
		{
			...
			{
				LZDecompressVram(
					gBattleTerrainAnimTiles_Building,
					(void*)( BG_CHAR_ADDR( 1 ) ) );
				LZDecompressVram(
		...
		{
			LZDecompressVram( gBattleTerrainTable[gBattleTerrain]
						  .entryTileset,
				(void*)( BG_CHAR_ADDR( 1 ) ) );
			LZDecompressVram( gBattleTerrainTable[gBattleTerrain]
						  .entryTilemap,
...
bool8 LoadChosenBattleElement( u8 caseId )
{
	...
	case 3:
		...
		else
		{
			...
			case MAP_BATTLE_SCENE_NORMAL:
				LZDecompressVram(
					gBattleTerrainTable[gBattleTerrain]
						.tileset,
					(void*)( BG_CHAR_ADDR( 2 ) ) );
				break;
			case MAP_BATTLE_SCENE_GYM:
	...
	case 5:
		...
		else
		{
			...
			case MAP_BATTLE_SCENE_FRONTIER:
				LoadCompressedPalette(
					gBattleTerrainPalette_Frontier,
					0x20,
					0x60 );
				break;
			}
