...
static void mevent_srv_init_common(
	struct mevent_srv_common*, const void*, u32, u32 );
...
void mevent_srv_init_wnews( void )
{
	s_mevent_srv_common_ptr =
		AllocZeroed( sizeof( struct mevent_srv_common ) );
	mevent_srv_init_common(
		s_mevent_srv_common_ptr, s_mevent_wonder_news, 0, 1 );
}
...
void mevent_srv_new_wcard( void )
{
	s_mevent_srv_common_ptr =
		AllocZeroed( sizeof( struct mevent_srv_common ) );
	mevent_srv_init_common(
		s_mevent_srv_common_ptr, s_mevent_wonder_card, 0, 1 );
}
...
static void mevent_srv_init_common( struct mevent_srv_common* svr,
	...
	u32 recvPlayerNo )
{
	...
	svr->mevent_unk1442cc =
		AllocZeroed( sizeof( struct MEventStruct_Unk1442CC ) );
...
static const void* mevent_first_if_not_null_else_second(
...
static u32 common_mainseq_4( struct mevent_srv_common* svr )
{
	...
	case 2:
		...
		mevent_srv_sub_init_recv(
			&svr->manager, cmd->flag, svr->recvBuffer );
		svr->mainseqno = 2;
		break;
	case 3:
		// jump
	...
	case 5:
		...
		AGB_ASSERT( cmd->flag == FALSE );
		AGB_ASSERT( cmd->parameter == NULL );
		memcpy( svr->mevent_unk1442cc,
			svr->recvBuffer,
			sizeof( struct MEventStruct_Unk1442CC ) );
		break;
	case 6:
	...
	case 7:
		// check_crc
		AGB_ASSERT( cmd->flag == FALSE );
		ptr = mevent_first_if_not_null_else_second(
			cmd->parameter, svr->wonder_card );
	...
	case 9:
		AGB_ASSERT( cmd->flag == FALSE );
		ptr = mevent_first_if_not_null_else_second(
			cmd->parameter, &svr->sendWord );
		svr->param = sub_801B708( ptr, svr->mevent_unk1442cc, ptr );
		break;
	case 10:
		AGB_ASSERT( cmd->parameter == NULL );
		svr->param = MEventStruct_Unk1442CC_GetValueNFrom_unk_20(
			svr->mevent_unk1442cc, cmd->flag );
		break;
	case 11:
		AGB_ASSERT( cmd->flag == FALSE );
		svr->param = MEventStruct_Unk1442CC_CompareField_unk_16(
			svr->mevent_unk1442cc, cmd->parameter );
		break;
	case 12:
		AGB_ASSERT( cmd->flag == FALSE );
		svr->param = mevent_compare_pointers(
			cmd->parameter, *(void**)svr->recvBuffer );
		break;
	case 14:
		AGB_ASSERT( cmd->flag == FALSE );
	...
	case 15:
		if( cmd->parameter == NULL )
			...
			mevent_srv_common_init_send(
				svr, 0x19, cmd->parameter, cmd->flag );
		break;
	case 18:
		if( cmd->parameter == NULL )
			mevent_srv_common_init_send( svr,
			...
			mevent_srv_common_init_send(
				svr, 0x10, cmd->parameter, cmd->flag );
		break;
	case 19:
		AGB_ASSERT( cmd->flag == FALSE );
	...
	case 20:
		mevent_srv_common_init_send(
			svr, 0x15, cmd->parameter, cmd->flag );
		break;
	case 17:
		mevent_srv_common_init_send(
			svr, 0x1c, cmd->parameter, cmd->flag );
		break;
	case 22:
		AGB_ASSERT( cmd->flag == FALSE );
	...
	case 29:
		mevent_srv_common_init_send(
			svr, 0x1b, cmd->parameter, cmd->flag );
		break;
	}
