...
static const struct WindowTemplate sSafariBallsWindowTemplate = {
	0, 1, 1, 9, 4, 0xF, 8};
...
static const struct WindowTemplate sPyramidFloorWindowTemplate_2 = {
	0, 1, 1, 0xA, 4, 0xF, 8};
static const struct WindowTemplate sPyramidFloorWindowTemplate_1 = {
	0, 1, 1, 0xC, 4, 0xF, 8};
...
static const struct WindowTemplate sUnknown_085105AC[] = {
	{0, 2, 0xF, 0x1A, 4, 0xF, 0x194}, DUMMY_WIN_TEMPLATE};
...
static void ShowSafariBallsWindow( void )
{
	...
	ConvertIntToDecimalStringN(
		gStringVar1, gNumSafariBalls, STR_CONV_MODE_RIGHT_ALIGN, 2 );
	StringExpandPlaceholders( gStringVar4, gText_SafariBallStock );
	AddTextPrinterParameterized(
		sSafariBallsWindowId, 1, gStringVar4, 0, 1, 0xFF, NULL );
...
static void ShowPyramidFloorWindow( void )
{
	if( gSaveBlock2Ptr->frontier.curChallengeBattleNum == 7 )
		...
		sBattlePyramidFloorWindowId =
			AddWindow( &sPyramidFloorWindowTemplate_2 );

	PutWindowTilemap( sBattlePyramidFloorWindowId );
	DrawStdWindowFrame( sBattlePyramidFloorWindowId, FALSE );
	StringCopy( gStringVar1,
		sPyramindFloorNames[gSaveBlock2Ptr->frontier
					    .curChallengeBattleNum] );
	StringExpandPlaceholders( gStringVar4, gText_BattlePyramidFloor );
	AddTextPrinterParameterized( sBattlePyramidFloorWindowId,
		1,
		gStringVar4,
		0,
		1,
...
static void RemoveExtraStartMenuWindows( void )
{
	...
	{
		ClearStdWindowAndFrameToTransparent(
			sBattlePyramidFloorWindowId, FALSE );
...
static bool32 PrintStartMenuActions( s8* pIndex, u32 count )
{
	...
	do
	{
		if( sStartMenuItems[sCurrentStartMenuActions[index]]
				.func.u8_void == StartMenuPlayerNameCallback )
		{
			PrintPlayerNameOnWindow( GetStartMenuWindowId( ),
				sStartMenuItems
					[sCurrentStartMenuActions[index]]
						.text,
				8,
				( index << 4 ) + 9 );
		}
...
static bool32 InitStartMenuStep( void )
{
	...
	case 2:
		sub_81973A4( );
		DrawStdWindowFrame(
			sub_81979C4( sNumStartMenuActions ), FALSE );
	...
	case 4:
		...
		break;
	case 5:
		sStartMenuCursorPos = sub_81983AC( GetStartMenuWindowId( ),
			1,
			0,
			9,
			16,
...
void ShowStartMenu( void ) // Called from overworld.c and
...
static bool8 HandleStartMenuInput( void )
{
	...
	if( gMain.newKeys & A_BUTTON )
	{
		PlaySE( SE_SELECT );
		if( sStartMenuItems
				[sCurrentStartMenuActions[sStartMenuCursorPos]]
					.func.u8_void ==
...
static bool8 StartMenuPokemonCallback( void )
{
	if( !gPaletteFade.active )
	{
		...
		CleanupOverworldWindowsAndTilemaps( );
		SetMainCallback2(
			CB2_PartyMenuFromStartMenu ); // Display party menu
...
static bool8 StartMenuBagCallback( void )
{
	if( !gPaletteFade.active )
	{
		...
		CleanupOverworldWindowsAndTilemaps( );
		SetMainCallback2(
			CB2_BagMenuFromStartMenu ); // Display bag menu
	...
	if( !gPaletteFade.active )
	{
		...
		if( IsUpdateLinkStateCBActive( ) || InUnionRoom( ) )
			ShowPlayerTrainerCard(
				CB2_ReturnToFieldWithOpenMenu ); // Display
								 // trainer
								 // card
...
static bool8 StartMenuLinkModePlayerNameCallback( void )
{
	if( !gPaletteFade.active )
	{
		...
		ShowTrainerCardInLink(
			gLocalLinkPlayerId, CB2_ReturnToFieldWithOpenMenu );
...
static void sub_80A0014( void ) { ClearDialogWindowAndFrame( 0, TRUE ); }
...
static u8 SaveConfirmSaveCallback( void )
{
	...
	{
		ShowSaveMessage(
			gText_BattlePyramidConfirmRest, SaveYesNoCallback );
	}
	else
	{
...
static u8 SaveFileExistsCallback( void )
{
	...
	{
		ShowSaveMessage(
			gText_AlreadySavedFile, SaveConfirmOverwriteCallback );
	}
...
static u8 SaveConfirmOverwriteDefaultNoCallback( void )
{
	DisplayYesNoMenuWithDefault( 1 ); // Show Yes/No menu (No selected as
					  // default)
...
static void sub_80A03D8( void ) { TransferPlttBuffer( ); }
...
static void ShowSaveInfoWindow( void )
{
	...
	AddTextPrinterParameterized(
		sSaveInfoWindowId, 1, gStringVar4, 0, yOffset, 0xFF, NULL );

	// Print player name
	yOffset = 0x11;
	AddTextPrinterParameterized( sSaveInfoWindowId,
		1,
		gText_SavingPlayer,
		0,
		yOffset,
	...
	PrintPlayerNameOnWindow(
		sSaveInfoWindowId, gStringVar4, xOffset, yOffset );
...
void sub_80A08CC(
	void ) // Referenced in data/specials.inc and
	       // data/scripts/maps/BattleFrontier_BattleTowerLobby.inc
{
