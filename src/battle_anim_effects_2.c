...
void AnimTask_Withdraw( u8 taskId )
{
	PrepareBattlerSpriteForRotScale(
		gBattlerSpriteIds[gBattleAnimAttacker], ST_OAM_OBJ_NORMAL );
...
// pointing in the same direction it's moving. arg 0: initial x pixel offset
...
void AnimSonicBoomProjectile( struct Sprite* sprite )
{
	...
	InitSpritePosToAnimAttacker( sprite, TRUE );
	targetXPos = GetBattlerSpriteCoord( gBattleAnimTarget, 2 ) +
		gBattleAnimArgs[2];
	targetYPos = GetBattlerSpriteCoord( gBattleAnimTarget, 3 ) +
		gBattleAnimArgs[3];
	rotation = ArcTan2Neg(
		targetXPos - sprite->pos1.x, targetYPos - sprite->pos1.y );
	rotation += 0xF000;
	if( IsContest( ) )
		rotation -= 0x6000;
...
void sub_8103AA4( struct Sprite* sprite )
{
	...
	if( sprite->data[0]-- <= 0 )
	{
		...
		sprite->data[6] = 0;
		sprite->data[5] = 0;
		sprite->data[3] = sub_8151534(
			sub_8151534( b, a ), sub_8151624( 0x1C0 ) );
		sprite->data[4] = sub_8151534(
			sub_8151534( c, a ), sub_8151624( 0x1C0 ) );
...
static void sub_8103C0C( u8 taskId )
{
	if( gTasks[taskId].data[0]-- <= 0 )
	{
		...
		spriteId = CreateSprite( &gUnknown_085933B0,
...
void sub_8103CF0( u8 taskId )
{
		...
		IsBattlerSpriteVisible( BATTLE_PARTNER( gBattleAnimTarget ) ) )
	{
		SetAverageBattlerPositions(
			gBattleAnimTarget, 0, &targetX, &targetY );
	}
	else
	{
		targetX = GetBattlerSpriteCoord(
			gBattleAnimTarget, BATTLER_COORD_X );
		targetY = GetBattlerSpriteCoord(
	...
	gTasks[taskId].data[5] =
		sub_8151534( xDiff, sub_8151624( gBattleAnimArgs[2] & ~1 ) );
	...
	if( targetY >= attackerY )
	{
		...
		gTasks[taskId].data[8] =
			sub_8151534( yDiff,
				sub_8151624( gTasks[taskId].data[5] ) ) &
			~1;
	}
	else
	{
	...
	if( gBattleAnimArgs[4] & 0x80 )
	{
		...
		{
			u16 var = GetBattlerSpriteSubpriority(
					  gBattleAnimTarget ) +
...
void sub_8103FE8( struct Sprite* sprite )
{
	InitSpritePosToAnimAttacker( sprite, FALSE );
	sprite->data[0] = 0x100 +
		( IndexOfSpritePaletteTag( gUnknown_085934A0.paletteTag )
			<< 4 );
...
static void sub_8104018( struct Sprite* sprite )
{
	...
	if( ++sprite->data[1] == 2 )
	{
		...
		for( i = 8; i < 16; i++ )
			gPlttBufferFaded[i + id] =
				gPlttBufferFaded[i + id + 1];
...
void sub_8104088( struct Sprite* sprite )
{
	...
	r7 = GetBattlerSpriteCoord( gBattleAnimTarget, 3 ) +
		gBattleAnimArgs[3];
	if( GetBattlerSide( gBattleAnimAttacker ) != B_SIDE_PLAYER )
		gBattleAnimArgs[2] = -gBattleAnimArgs[2];
...
void AnimTask_GrowAndGreyscale( u8 taskId )
{
	...
	SetGreyscaleOrOriginalPalette(
		gSprites[spriteId].oam.paletteNum + 16, FALSE );
...
static void AnimTask_GrowAndGreyscaleStep( u8 taskId )
{
	if( --gTasks[taskId].data[0] == -1 )
	{
		...
		ResetSpriteRotScale( spriteId );
		SetGreyscaleOrOriginalPalette(
			gSprites[spriteId].oam.paletteNum + 16, TRUE );
		DestroyAnimVisualTask( taskId );
	}
}
...
static void AnimTask_MinimizeStep1( u8 taskId )
{
	...
	{
	case 0:
		if( task->data[2] == 0 || task->data[2] == 3 ||
			task->data[2] == 6 )
			CreateMinimizeSprite( task, taskId );
		task->data[2]++;
		task->data[4] += 0x28;
		SetSpriteRotScale(
			task->data[0], task->data[4], task->data[4], 0 );
...
static void CreateMinimizeSprite( struct Task* task, u8 taskId )
{
	...
	if( spriteId >= 0 )
	{
		...
		else
		{
			...
			SetSpriteRotScale(
				spriteId, task->data[4], task->data[4], 0 );
...
void AnimTask_Splash( u8 taskId )
{
	...
	else
	{
		...
		PrepareAffineAnimInTaskData(
			task, spriteId, gSplashEffectAffineAnimCmds );
...
void AnimTask_GrowAndShrink( u8 taskId )
{
	...
	u8 spriteId       = GetAnimBattlerSpriteId( ANIM_ATTACKER );
	PrepareAffineAnimInTaskData(
		task, spriteId, gGrowAndShrinkAffineAnimCmds );
...
void Anim_BreathPuff( struct Sprite* sprite )
{
	if( GetBattlerSide( gBattleAnimAttacker ) == B_SIDE_PLAYER )
	{
		...
		sprite->pos1.x = GetBattlerSpriteCoord( gBattleAnimAttacker,
					 BATTLER_COORD_X_2 ) +
			32;
		sprite->data[1] = 64;
	}
...
void sub_8104E74( u8 taskId )
{
	...
	s16 i;
	task->data[0] =
		GetBattlerYCoordWithElevation( gBattleAnimTarget ) + 32;
	task->data[1]  = 4;
	task->data[2]  = 0;
	task->data[3]  = 0;
	task->data[4]  = 0;
	task->data[5]  = 0;
	task->data[15] = GetBattlerSpriteCoordAttr(
...
static void sub_8104F54( u8 taskId )
{
	...
	case 1:
		if( ++task->data[1] > 3 )
		{
			...
			if( task->data[5] >= 0 )
			{
				gScanlineEffectRegBuffers[0][task->data[5]] =
					task->data[6];
				gScanlineEffectRegBuffers[1][task->data[5]] =
					task->data[6];
			}
...
void sub_810501C( struct Sprite* sprite )
{
	...
	sprite->pos1.y =
		GetBattlerYCoordWithElevation( gBattleAnimTarget ) + 16;
	sprite->data[0] = 0;
	sprite->data[1] = 0;
	sprite->data[2] = 0;
	sprite->data[3] = 16;
	sprite->data[4] = 0;
	sprite->data[5] = GetBattlerSpriteCoordAttr( gBattleAnimTarget,
...
void sub_81051C4( struct Sprite* sprite )
{
		...
		IsBattlerSpriteVisible( BATTLE_PARTNER( battler ) ) )
	{
		...
		if( GetBattlerSide( battler ) != B_SIDE_PLAYER )
			gBattleAnimArgs[0] -=
				( sp0 - r4 ) - gBattleAnimArgs[0]; // This is
								   // weird.
		else
...
void sub_81052A4( struct Sprite* sprite )
{
	...
	{
		r9 = GetBattlerSpriteCoord( battler1, r10 ) +
			gBattleAnimArgs[0];
		if( IsBattlerSpriteVisible( BATTLE_PARTNER( battler2 ) ) )
			sprite->subpriority =
				gSprites[gBattlerSpriteIds[BATTLE_PARTNER(
						 battler2 )]]
					.subpriority -
				1;
		else
			sprite->subpriority =
				gSprites[gBattlerSpriteIds[battler2]]
					.subpriority -
				1;
	}
	else
	{
		r9 = GetBattlerSpriteCoord( battler1, r10 ) -
			gBattleAnimArgs[0];
		if( !IsContest( ) &&
			IsBattlerSpriteVisible( BATTLE_PARTNER( battler1 ) ) )
		{
			if( gSprites[gBattlerSpriteIds[battler1]].pos1.x <
				...
				sprite->subpriority =
					gSprites[gBattlerSpriteIds[battler1]]
						.subpriority -
					1;
		}
		else
	...
	if( !IsContest( ) &&
		IsBattlerSpriteVisible( BATTLE_PARTNER( battler2 ) ) )
	{
		SetAverageBattlerPositions(
			battler2, gBattleAnimArgs[6], &sp0, &sp1 );
	}
	else
	{
...
void sub_81054E8( struct Sprite* sprite )
{
	...
	if( index != 0xFF )
	{
		BlendPalette( ( ( index << 20 ) + 0x1010000 ) >> 16,
			15,
			gBattleAnimArgs[5],
			gBattleAnimArgs[4] );
	}
...
void sub_8105538( struct Sprite* sprite )
{
	s16 r1;
	InitSpritePosToAnimAttacker( sprite, FALSE );
	r1 = GetBattlerSide( gBattleAnimAttacker ) != B_SIDE_PLAYER ? -160
								    : 160;
	sprite->data[0]  = 0x380;
	sprite->data[1]  = r1;
...
static void sub_810557C( struct Sprite* sprite )
{
	...
	sprite->data[0] -= 32;
	add = GetBattlerSide( gBattleAnimAttacker ) != B_SIDE_PLAYER ? -160
								     : 160;
	sprite->data[1] += add;
	if( sprite->pos2.y > 0 )
	{
...
static void sub_8105694( struct Sprite* sprite )
{
	...
	if( ++sprite->data[0] == 9 )
	{
		...
		SetGpuReg( REG_OFFSET_BLDALPHA,
			BLDALPHA_BLEND( (u16)sprite->data[0], 0 ) );
...
static void sub_81056D4( struct Sprite* sprite )
{
	if( sprite->data[1]++ % 3 == 0 )
	{
		...
		SetGpuReg( REG_OFFSET_BLDALPHA,
			BLDALPHA_BLEND(
				sprite->data[0], 16 - sprite->data[0] ) );
		if( sprite->data[0] == 0 )
			sprite->callback = sub_810571C;
	}
}
...
void sub_8105AAC( u8 taskId )
{
	...
	task->data[13]    = 0;
	task->data[14] =
		GetBattlerSpriteCoord( gBattleAnimAttacker, ANIM_ATTACKER );
	task->data[15] =
		GetBattlerSpriteCoord( gBattleAnimAttacker, ANIM_TARGET );
...
static void sub_8105B08( u8 taskId )
{
	...
	case 0:
		if( ++task->data[1] > 4 )
		{
			u8 spriteId;
			task->data[1] = 0;
			spriteId      = CreateSprite( &gUnknown_085938DC,
                                task->data[14],
                                task->data[15],
                                0 );
			if( spriteId != MAX_SPRITES )
			{
				...
				gSprites[spriteId].pos2.x =
					gUnknown_085938F4[task->data[2]][0];
				gSprites[spriteId].pos2.y =
					gUnknown_085938F4[task->data[2]][1];
				task->data[13]++;
				if( ++task->data[2] > 3 )
				{
					task->data[2] = 0;
...
void sub_8105CB4( u8 taskId )
{
	...
	LZDecompressWram( gBattleAnimSpritePal_MusicNotes2,
		gMonSpritesGfxPtr->field_17C );
	for( i = 0; i < 3; i++ )
		LoadPalette( &gMonSpritesGfxPtr->field_17C[i * 32],
			( u16 )( ( paletteNums[i] << 4 ) + 0x100 ),
			32 );
...
static void sub_8105D88( struct Sprite* sprite, u8 a, u8 b )
{
	...
	sprite->oam.paletteNum =
		IndexOfSpritePaletteTag( gUnknown_08593950[b >> 1] );
}
...
void sub_8105DE8( struct Sprite* sprite )
{
	...
	sprite->data[0] = gBattleAnimArgs[4];
	sprite->data[2] = GetBattlerSpriteCoord( gBattleAnimAttacker, 0 ) +
		gBattleAnimArgs[2];
	sprite->data[4] = GetBattlerSpriteCoord( gBattleAnimAttacker, 1 ) +
		gBattleAnimArgs[3];
	sprite->callback = StartAnimLinearTranslation;
	StoreSpriteCallbackInData6( sprite, DestroyAnimSprite );
...
static void sub_8105F30( u8 taskId )
{
	...
	{
		gBattle_WIN0H = gTasks[taskId].data[1] |
...
static void sub_8105F84( u8 taskId )
{
	if( ++gTasks[taskId].data[10] == 5 )
	{
		...
		SetGpuReg( REG_OFFSET_BLDCNT,
			BLDCNT_TGT1_BG3 | BLDCNT_EFFECT_LIGHTEN );
		BlendPalettes( sub_80A75AC( 1, 0, 0, 0, 0, 0, 0 ),
			16,
			RGB( 31, 31, 31 ) );
	}
...
void AnimTask_HeartsBackground( u8 taskId )
{
	...
	AnimLoadCompressedBgGfx(
		animBg.bgId, &gUnknown_08C232E0, animBg.tilesOffset );
...
void sub_81064F8( u8 taskId )
{
	...
	if( IsContest( ) )
		...
		sub_80A6D60(
			&animBg, &gBattleAnimBgTilemap_ScaryFacePlayer, 0 );
	else
		sub_80A6D60(
			&animBg, &gBattleAnimBgTilemap_ScaryFaceOpponent, 0 );

	AnimLoadCompressedBgGfx(
		animBg.bgId, gUnknown_08C249F8, animBg.tilesOffset );
...
static void AnimOrbitScatterStep( struct Sprite* sprite )
{
	...
	if( sprite->pos1.x + sprite->pos2.x + 16 > 272u ||
...
void sub_8106D90( u8 taskId )
{
	...
	PrepareAffineAnimInTaskData(
		&gTasks[taskId], spriteId, gUnknown_08593B98 );
...
void sub_8106E00( struct Sprite* sprite )
{
	int var1;
	u8 battler =
		!gBattleAnimArgs[0] ? gBattleAnimAttacker : gBattleAnimTarget;

	if( GetBattlerSide( battler ) == B_SIDE_OPPONENT )
		gBattleAnimArgs[1] *= -1;

	sprite->pos1.x =
		GetBattlerSpriteCoord( battler, 2 ) + gBattleAnimArgs[1];
	sprite->pos1.y =
		GetBattlerSpriteCoord( battler, 3 ) + gBattleAnimArgs[2];
	sprite->data[0] = 0;
...
void sub_8106F60( struct Sprite* sprite )
{
	...
	sprite->data[1] =
		( sprite->data[0] + ( (u16)sprite->data[0] >> 31 ) ) / 2;
	...
	sprite->pos2.y =
		sprite->data[1] + Sin( index, 10 ) + Cos( sprite->data[6], 4 );
...
static void sub_810703C( struct Sprite* sprite )
{
	...
	if( sprite->data[4] > 3 )
	{
		...
		sprite->invisible = var1 -
			( ( ( s32 )( var1 + ( (u32)var1 >> 31 ) ) >> 1 )
				<< 1 );
		DestroyAnimSprite( sprite );
	}
...
void sub_81070AC( struct Sprite* sprite )
{
	...
	else
	{
		...
		sprite->pos1.y =
			GetBattlerSpriteCoord( gBattleAnimAttacker, 1 ) + 40;
	}
