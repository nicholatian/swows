...
static bool32 TryCalculateWallpaper( u16* backgroundClr,
	u16* foregroundClr,
	u8* iconId,
	u8* patternId,
	u16 trainerId,
	u8* phrase );
static void sub_81D9D5C(
	u8* array, u8* letterTableIds, u32 arg2, u32 arg3, u32 loopCount );
...
void DoWaldaNamingScreen( void )
{
	...
	DoNamingScreen( NAMING_SCREEN_WALDA,
		gStringVar2,
		0,
		0,
		0,
...
u16 TryGetWallpaperWithWaldaPhrase( void )
{
	...
	gSpecialVar_Result = TryCalculateWallpaper( &backgroundClr,
		&foregroundClr,
		&iconId,
		&patternId,
		trainerId,
	...
	u8* phrase )
{
	...
	{
		sub_81D9D5C(
			array, charsByTableId, ( 5 * i ), 3 + ( 8 * i ), 5 );
	}

	sub_81D9D5C( array, charsByTableId, 70, 115, 2 );

	if( sub_81D9DAC( array, 0, 3 ) !=
		sub_81D9DAC( charsByTableId, 117, 3 ) )
		return FALSE;
	...
	if( array[6] !=
		( array[0] ^ array[2] ^ array[4] ^ ( trainerId >> 8 ) ) )
		return FALSE;

	if( array[7] !=
		( array[1] ^ array[3] ^ array[5] ^ ( trainerId & 0xFF ) ) )
		return FALSE;
