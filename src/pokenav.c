...
#define LOOPED_TASK_ID( primary, secondary ) \
	( ( ( secondary ) << 16 ) | ( primary ) )
...
static bool32 AnyMonHasRibbon( void )
{
	...
	for( i = 0; i < PARTY_SIZE; i++ )
	{
		if( GetMonData(
			...
			!GetMonData(
				&gPlayerParty[i], MON_DATA_SANITY_IS_EGG ) &&
			GetMonData( &gPlayerParty[i],
				MON_DATA_RIBBON_COUNT ) != 0 )
		{
			return TRUE;
		}
	}
...
void sub_81C742C( u8 taskId )
{
	...
	case 3:
		...
		{
			PokenavMenuCallbacks[gPokenavResources
						     ->currentMenuIndex]
				.unk18( );
			PokenavMenuCallbacks[gPokenavResources
						     ->currentMenuIndex]
	...
	case 5:
		...
		{
			bool32 calledFromScript = ( gPokenavResources->mode !=
				POKENAV_MODE_NORMAL );
...
static bool32 SetActivePokenavMenu( u32 menuId )
{
	...
	sub_81C7834( PokenavMenuCallbacks[index].unkC,
		PokenavMenuCallbacks[index].unk10 );
	gPokenavResources->currentMenuCb1   = PokenavMenuCallbacks[index].unk4;
	gPokenavResources->currentMenuIndex = index;
	return TRUE;
}

u32 sub_81C75D4( void ) { return sub_81C786C( ); }

u32 sub_81C75E0( void ) { return gPokenavResources->currentMenuCb1( ); }
...
u16 sub_81C76AC( void ) { return gPokenavResources->fieldA; }
