...
bool8 SetUpFieldMove_SoftBoiled( void )
{
	...
	maxHp = GetMonData(
		&gPlayerParty[GetCursorSelectionMonId( )], MON_DATA_MAX_HP );
	hp = GetMonData(
		&gPlayerParty[GetCursorSelectionMonId( )], MON_DATA_HP );
...
void sub_81615A8( u8 taskId )
{
	...
	if( hp == 0 || slotId == pokemonIndex ||
		GetMonData( &gPlayerParty[pokemonIndex], MON_DATA_MAX_HP ) ==
			hp )
	{
		sub_81617B8( taskId );
		return;
	...
	sub_81B1F18( taskId,
		slotId,
		-1,
		GetMonData( &gPlayerParty[slotId], MON_DATA_MAX_HP ) / 5,
		sub_816166C );
}
