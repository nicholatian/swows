...
void m4aSoundInit( void )
{
	...
	CpuCopy32( (void*)( (s32)SoundMainRAM & ~1 ),
		SoundMainRAM_Buffer,
...
void MPlayExtender( struct CgbChannel* cgbChans )
{
	...
	REG_SOUNDCNT_X = SOUND_MASTER_ENABLE | SOUND_4_ON | SOUND_3_ON |
...
void SoundInit( struct SoundInfo* soundInfo )
{
	...
	REG_SOUNDCNT_H = SOUND_B_FIFO_RESET | SOUND_B_TIMER_0 |
		SOUND_B_LEFT_OUTPUT | SOUND_A_FIFO_RESET | SOUND_A_TIMER_0 |
...
void SampleFreqSet( u32 freq )
{
	...
	soundInfo->pcmDmaPeriod =
		PCM_DMA_BUF_SIZE / soundInfo->pcmSamplesPerVBlank;
...
void m4aSoundVSyncOff( void )
{
		...
		soundInfo->ident <= ID_NUMBER + 1 )
	{
		...
		if( REG_DMA1CNT & ( DMA_REPEAT << 16 ) )
			REG_DMA1CNT =
				( ( DMA_ENABLE | DMA_START_NOW | DMA_32BIT |
					  DMA_SRC_INC | DMA_DEST_FIXED )
					<< 16 ) |
				4;
...
void MPlayStart(
	struct MusicPlayerInfo* mplayInfo, struct SongHeader* songHeader )
{
	s32 i;
	u8 unk_B;
...
void TrkVolPitSet(
	struct MusicPlayerInfo* mplayInfo, struct MusicPlayerTrack* track )
{
...
void CgbModVol( struct CgbChannel* chan )
{
	...
	if( ( soundInfo->mode & 1 ) || !CgbPan( chan ) )
	{
		chan->pan = 0xFF;
		chan->eg =
			( u32 )( chan->rightVolume + chan->leftVolume ) >> 4;
	}
	else
	{
		// Force chan->rightVolume and chan->leftVolume to be read from
		// memory again, even though there is no reason to do so. The
		// command line option "-fno-gcse" achieves the same result as
		// this.
		asm( "" : : : "memory" );
...
void CgbSound( void )
{
	...
	for( ch = 1, channels = soundInfo->cgbChans; ch <= 4;
		ch++, channels++ )
	{
		if( !( channels->sf & 0xc7 ) )
			continue;
	...
	loc_82E0FD6:
		...
		if( channels->mo & 1 )
		{
			REG_NR51 = ( REG_NR51 & ~channels->panMask ) |
				channels->pan;
			if( ch == 3 )
			{
...
void m4aMPlayTempoControl( struct MusicPlayerInfo* mplayInfo, u16 tempo )
{
	if( mplayInfo->ident == ID_NUMBER )
	{
		...
		mplayInfo->tempoI =
			( mplayInfo->tempoD * mplayInfo->tempoU ) >> 8;
		mplayInfo->ident = ID_NUMBER;
	}
}
...
void m4aMPlayPitchControl(
	struct MusicPlayerInfo* mplayInfo, u16 trackBits, s16 pitch )
{
	s32 i;
	u32 bit;
	struct MusicPlayerTrack* track;
...
void m4aMPlayPanpotControl(
	struct MusicPlayerInfo* mplayInfo, u16 trackBits, s8 pan )
{
	s32 i;
	u32 bit;
	struct MusicPlayerTrack* track;
...
void m4aMPlayModDepthSet(
	struct MusicPlayerInfo* mplayInfo, u16 trackBits, u8 modDepth )
{
	s32 i;
	u32 bit;
	struct MusicPlayerTrack* track;
...
void m4aMPlayLFOSpeedSet(
	struct MusicPlayerInfo* mplayInfo, u16 trackBits, u8 lfoSpeed )
{
	s32 i;
	u32 bit;
	struct MusicPlayerTrack* track;
...
struct MusicPlayerInfo* SetPokemonCryTone( struct ToneData* tone )
{
	...
	for( i = 0; i < MAX_POKEMON_CRIES; i++ )
	{
		...
		if( !track->flags &&
			( !track->chan || track->chan->track != track ) )
			goto start_song;
...
void SetPokemonCryChorus( s8 val )
{
	if( val )
	{
		...
		gPokemonCrySong.tuneValue2 =
			( val + gPokemonCrySong.tuneValue ) & 0x7F;
	}
	else
	{
...
void SetPokemonCryStereo( u32 val )
{
	...
	else
	{
		REG_SOUNDCNT_H = SOUND_B_TIMER_0 | SOUND_B_LEFT_OUTPUT |
			...
			SOUND_B_MIX_HALF | SOUND_A_MIX_HALF |
			SOUND_CGB_MIX_FULL;
		soundInfo->mode |= 1;
	}
}
