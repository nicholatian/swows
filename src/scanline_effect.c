...
void ScanlineEffect_SetParams( struct ScanlineEffectParams params )
{
	...
	{
		// Set the DMA src to the value for the second scanline because
		// the first DMA transfer occurs in HBlank *after* the first
		// scanline is drawn
		gScanlineEffect.dmaSrcBuffers[0] =
			(u16*)gScanlineEffectRegBuffers[0] + 1;
		gScanlineEffect.dmaSrcBuffers[1] =
			(u16*)gScanlineEffectRegBuffers[1] + 1;
	...
	else // assume 32-bit
	{
		...
		// scanline is drawn
		gScanlineEffect.dmaSrcBuffers[0] =
			(u32*)gScanlineEffectRegBuffers[0] + 1;
		gScanlineEffect.dmaSrcBuffers[1] =
			(u32*)gScanlineEffectRegBuffers[1] + 1;
...
void ScanlineEffect_InitHBlankDmaTransfer( void )
{
	...
	{
		DmaStop( 0 );
		// Set DMA to copy to dest register on each HBlank for the next
		// frame. The HBlank DMA transfers do not occurr during VBlank,
		// so the transfer will begin on the HBlank after the first
		// scanline
		DmaSet( 0,
			gScanlineEffect
				.dmaSrcBuffers[gScanlineEffect.srcBuffer],
...
static void TaskFunc_UpdateWavePerFrame( u8 taskId )
{
	...
	else
	{
		...
		if( gTasks[taskId].tFramesUntilMove != 0 )
		{
			...
			for( i = gTasks[taskId].tStartLine;
				i < gTasks[taskId].tEndLine;
				i++ )
			{
				gScanlineEffectRegBuffers
	...
	bool8 applyBattleBgOffsets )
{
	...
	GenerateWave( &gScanlineEffectRegBuffers[0][320],
		frequency,
		amplitude,
		endLine - startLine );
	...
	for( i = startLine; i < endLine; i++ )
	{
		gScanlineEffectRegBuffers[0][i] =
			gScanlineEffectRegBuffers[0][offset];
