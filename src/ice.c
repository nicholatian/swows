...
bool8 GenerateHailParticle(
	u8 hailStructId, u8 affineAnimNum, u8 taskId, u8 c );
...
static void sub_810B6C4( struct Sprite* sprite )
{
	...
	for( ; ( targetX >= -32 && targetX <= 272 ) &&
		( targetY >= -32 && targetY <= 192 );
		targetX += sprite->data[1], targetY += sprite->data[2] )
		;

	sprite->data[1] = -sprite->data[1];
	sprite->data[2] = -sprite->data[2];
	for( ; ( attackerX >= -32 && attackerX <= 272 ) &&
		( attackerY >= -32 && attackerY <= 192 );
...
// Animates the small snowballs that swirl around the target in Blizzard and
// Icy Wind. arg 0: initial x pixel offset arg 1: initial y pixel offset arg 2:
...
void AnimSwirlingSnowball_Step2( struct Sprite* sprite )
{
	...
	tempVar = GetBattlerSide( gBattleAnimAttacker ) != B_SIDE_PLAYER ? 20
									 : -20;
...
void AnimSwirlingSnowball_Step3( struct Sprite* sprite )
{
	...
	{
		sprite->pos2.x =
			Sin( sprite->data[0], tempVar ) - sprite->data[3];
...
void AnimSwirlingFogAnim( struct Sprite* sprite )
{
	if( !AnimTranslateLinear( sprite ) )
	{
		...
		if( ( u16 )( sprite->data[5] - 64 ) <= 0x7F )
			sprite->oam.priority =
				GetBattlerSpriteBGPriority( sprite->data[7] );
		else
			sprite->oam.priority =
				GetBattlerSpriteBGPriority( sprite->data[7] ) +
				1;
...
void AnimTask_Haze1( u8 taskId )
{
	...
	sub_80A6B30( &animBg );
	LoadBgTiles(
		animBg.bgId, gWeatherFog1Tiles, 0x800, animBg.tilesOffset );
	...
	case 0:
		if( ++gTasks[taskId].data[10] == 4 )
		{
			...
			gTasks[taskId].data[11] =
				gUnknown_08595C5C[gTasks[taskId].data[9]];
...
void AnimTask_OverlayFogTiles( u8 taskId )
{
	...
	case 0:
		...
		gTasks[taskId].data[11] =
			gUnknown_08595C88[gTasks[taskId].data[9]];
		SetGpuReg( REG_OFFSET_BLDALPHA,
			BLDALPHA_BLEND( gTasks[taskId].data[11],
				17 - gTasks[taskId].data[11] ) );
...
void InitPoisonGasCloudAnim( struct Sprite* sprite )
{
	...
	if( !( gBattlerPositions[gBattleAnimTarget] & 1 ) )
	{
		...
		if( ( sprite->data[7] & 0x8000 ) &&
			!( gBattlerPositions[gBattleAnimAttacker] & 1 ) )
			sprite->subpriority =
				gSprites[GetAnimBattlerSpriteId( ANIM_TARGET )]
					.subpriority +
				1;
	...
	if( gBattleAnimArgs[7] )
	{
		...
		sprite->data[4] = GetBattlerSpriteCoord( gBattleAnimTarget,
			...
			gBattleAnimArgs[4];
		sprite->data[7] |=
			GetBattlerSpriteBGPriority( gBattleAnimTarget ) << 8;
	}
	else
	{
		sprite->data[1] = sprite->pos1.x + gBattleAnimArgs[1];
...
void MovePoisonGasCloud( struct Sprite* sprite )
{
	...
	case 0:
		...
		if( sprite->data[0] <= 0 )
		{
			...
			if( IsContest( ) )
				sprite->data[5] = 80;
			else if( GET_BATTLER_SIDE2( gBattleAnimTarget ) !=
				B_SIDE_PLAYER )
				sprite->data[5] = 204;
			else
				sprite->data[5] = 80;
	...
	case 1:
		...
		sprite->pos2.x += value >> 3;
		sprite->pos2.y +=
			( gSineTable[sprite->data[5] + 0x40] * -3 ) >> 8;
		if( !IsContest( ) )
		{
	...
	case 1:
		if( task->data[5] == 0 )
		{
			if( GenerateHailParticle(
				    task->data[3], task->data[2], taskId, 1 ) )
				task->data[1]++;
	...
	u8 hailStructId, u8 affineAnimNum, u8 taskId, u8 c )
{
	...
	if( unk != 2 )
	{
		id = GetBattlerAtPosition(
			gUnknown_08595CB4[hailStructId].unk2 );
		if( IsBattlerSpriteVisible( id ) )
		{
			possibleBool = TRUE;
			battlerX =
				GetBattlerSpriteCoord( id, BATTLER_COORD_X_2 );
			battlerY = GetBattlerSpriteCoord(
				id, BATTLER_COORD_Y_PIC_OFFSET );
			switch( unk )
			{
			case 0:
				battlerX -=
					GetBattlerSpriteCoordAttr( id,
