...
void SetBagItemsPointers( void )
{
	...
	gBagPockets[BERRIES_POCKET].itemSlots =
...
void CopyItemNameHandlePlural( u16 itemId, u8* dst, u32 quantity )
{
	...
	else
	{
		if( itemId >= ITEM_CHERI_BERRY && itemId <= ITEM_ENIGMA_BERRY )
			GetBerryCountString( dst,
				gBerries[itemId - ITEM_CHERI_BERRY].name,
				quantity );
		else
			StringCopy( dst, ItemId_GetName( itemId ) );
	}
	...
	for( i = 0; i < gBagPockets[pocket].capacity; i++ )
	{
		if( gBagPockets[pocket].itemSlots[i].itemId == itemId )
		{
			...
			quantity = GetBagItemQuantity(
				&gBagPockets[pocket].itemSlots[i].quantity );
			if( quantity >= count )
				return TRUE;
			count -= quantity;
			// Does this item slot and all previous slots contain
			// enough of the item?
			if( count == 0 )
				return TRUE;
		}
...
bool8 CheckBagHasSpace( u16 itemId, u16 count )
{
	...
	else
	{
		...
		// Check space in any existing item slots that already contain
		// this item
		for( i = 0; i < gBagPockets[pocket].capacity; i++ )
		{
			if( gBagPockets[pocket].itemSlots[i].itemId == itemId )
		...
		if( count > 0 )
		{
			...
			if( count > 0 )
				return FALSE; // No more item slots. The bag is
					      // full
		}
	...
	else
	{
		...
		for( i = 0; i < itemPocket->capacity; i++ )
		{
			if( newItems[i].itemId == itemId )
			{
				...
				if( ownedCount + count <= slotCapacity )
				{
					// successfully added to already
					// existing item's count
					SetBagItemQuantity(
						&newItems[i].quantity,
						ownedCount + count );
				...
				{
					// try creating another instance of the
					// item if possible
					if( pocket == TMHM_POCKET ||
						pocket == BERRIES_POCKET )
					{
					...
					else
					{
						...
						// instance of the item if it's
						// at max slot capacity and
						// count is equal to 0
						if( count == 0 )
						{
							goto SUCCESS_ADD_ITEM;
		...
		if( count > 0 )
		{
			// either no existing item was found or we have to
			// create another instance, because the capacity was
			// exceeded
			for( i = 0; i < itemPocket->capacity; i++ )
			{
				if( newItems[i].itemId == ITEM_NONE )
				{
					...
					if( count > slotCapacity )
					{
						// try creating a new slot with
						// max capacity if duplicates
						// are possible
						if( pocket == TMHM_POCKET ||
							pocket ==
								BERRIES_POCKET )
	...
	else
	{
		...
		for( i = 0; i < itemPocket->capacity; i++ )
		{
			if( itemPocket->itemSlots[i].itemId == itemId )
				totalQuantity += GetBagItemQuantity(
					&itemPocket->itemSlots[i].quantity );
		}
		...
		{
			ownedCount = GetBagItemQuantity(
				&itemPocket->itemSlots[var].quantity );
			if( ownedCount >= count )
			{
				SetBagItemQuantity(
					&itemPocket->itemSlots[var].quantity,
...
bool8 CheckPCHasItem( u16 itemId, u16 count )
{
	...
	for( i = 0; i < PC_ITEMS_COUNT; i++ )
	{
		if( gSaveBlock1Ptr->pcItems[i].itemId == itemId &&
			GetPCItemQuantity(
				&gSaveBlock1Ptr->pcItems[i].quantity ) >=
				count )
			return TRUE;
	}
	return FALSE;
...
bool8 AddPCItem( u16 itemId, u16 count )
{
	...
	for( i = 0; i < PC_ITEMS_COUNT; i++ )
	{
		...
		{
			ownedCount =
				GetPCItemQuantity( &newItems[i].quantity );
			if( ownedCount + count <= 999 )
			{
				SetPCItemQuantity( &newItems[i].quantity,
					ownedCount + count );
				memcpy( gSaveBlock1Ptr->pcItems,
					newItems,
					sizeof( gSaveBlock1Ptr->pcItems ) );
				Free( newItems );
				return TRUE;
		...
		for( j = i + 1; j < PC_ITEMS_COUNT; j++ )
		{
			if( gSaveBlock1Ptr->pcItems[i].itemId == 0 )
			{
				struct ItemSlot temp =
					gSaveBlock1Ptr->pcItems[i];
				gSaveBlock1Ptr->pcItems[i] =
					gSaveBlock1Ptr->pcItems[j];
...
void CompactItemsInBagPocket( struct BagPocket* bagPocket )
{
	...
	for( i = 0; i < bagPocket->capacity - 1; i++ )
	{
		...
		{
			if( GetBagItemQuantity(
				    &bagPocket->itemSlots[i].quantity ) == 0 )
				SwapItemSlots( &bagPocket->itemSlots[i],
					&bagPocket->itemSlots[j] );
		}
	}
}
				    ...
				    &bagPocket->itemSlots[i].quantity ) != 0 )
			{
				if( GetBagItemQuantity(
					    &bagPocket->itemSlots[j]
						     .quantity ) == 0 )
					continue;
				if( bagPocket->itemSlots[i].itemId <=
					bagPocket->itemSlots[j].itemId )
					continue;
			}
			SwapItemSlots( &bagPocket->itemSlots[i],
				&bagPocket->itemSlots[j] );
...
void ClearBag( void )
{
	...
	{
		ClearItemSlots(
			gBagPockets[i].itemSlots, gBagPockets[i].capacity );
	}
}
...
bool8 RemovePyramidBagItem( u16 itemId, u16 count )
{
	...
	i = gPyramidBagCursorData.cursorPosition +
	...
	{
		u16* newItems =
			Alloc( PYRAMID_BAG_ITEMS_COUNT * sizeof( u16 ) );
		u8* newQuantities =
			Alloc( PYRAMID_BAG_ITEMS_COUNT * sizeof( u8 ) );
