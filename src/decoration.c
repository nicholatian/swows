...
EWRAM_DATA struct PlaceDecorationGraphicsDataBuffer
...
EWRAM_DATA static struct DecorRearrangementDataBuffer
	sDecorRearrangementDataBuffer[16]             = {};
...
static void DecorationItemsMenu_OnCursorMove(
	s32 itemIndex, bool8 flag, struct ListMenu* menu );
...
void ConfigureCameraObjectForPlacingDecoration(
	struct PlaceDecorationGraphicsDataBuffer* data, u8 decor );
void SetUpPlacingDecorationPlayerAvatar(
...
static const struct
{
	...
	[DECORSHAPE_1x1] = {SPRITE_SHAPE( 16x16 ),
		SPRITE_SIZE( 16x16 ),
		120,
		78},
	[DECORSHAPE_2x1] = {SPRITE_SHAPE( 32x16 ),
		SPRITE_SIZE( 32x16 ),
		128,
		78},
	[DECORSHAPE_3x1] = {SPRITE_SHAPE( 64x32 ),
		SPRITE_SIZE( 64x32 ),
		144,
		86},
	[DECORSHAPE_4x2] = {SPRITE_SHAPE( 64x32 ),
		SPRITE_SIZE( 64x32 ),
		144,
		70},
	[DECORSHAPE_2x2] = {SPRITE_SHAPE( 32x32 ),
		SPRITE_SIZE( 32x32 ),
		128,
		70},
	[DECORSHAPE_1x2] = {SPRITE_SHAPE( 16x32 ),
		SPRITE_SIZE( 16x32 ),
		120,
		70},
	[DECORSHAPE_1x3] = {SPRITE_SHAPE( 32x64 ),
		SPRITE_SIZE( 32x64 ),
		128,
		86},
	[DECORSHAPE_2x4] = {SPRITE_SHAPE( 32x64 ),
		SPRITE_SIZE( 32x64 ),
		128,
		54},
	[DECORSHAPE_3x3] = {SPRITE_SHAPE( 64x64 ),
		SPRITE_SIZE( 64x64 ),
		144,
		70},
	[DECORSHAPE_3x2] = {SPRITE_SHAPE( 64x32 ),
		SPRITE_SIZE( 64x32 ),
		144,
		70},
};
...
const struct SpriteTemplate sDecorWhilePlacingSpriteTemplate = {0x0000,
	0x0000,
	&sDecorSelectorOam,
	sDecorSelectorAnimCmds,
	NULL,
...
const u16 gUnknown_085A7308[] =
	INCBIN_U16( "graphics/decorations/unk_85a7308.gbapal" );

const u16 gUnknown_085A7328[] =
	INCBIN_U16( "graphics/decorations/unk_85a7328.gbapal" );
...
const u8 gUnknown_085A7358[] =
	INCBIN_U8( "graphics/misc/decoration_unk_85a7358.4bpp" );
...
const struct SpriteTemplate gUnknown_085A7404 = {0xFFFF,
...
void InitDecorationContextItems( void )
{
	if( sCurDecorationCategory < DECORCAT_COUNT )
		gCurDecorationItems =
			gDecorationInventories[sCurDecorationCategory].items;

	if( gDecorationContext.isPlayerRoom == FALSE )
	{
		gDecorationContext.items =
			gSaveBlock1Ptr->secretBases[0].decorations;
		gDecorationContext.pos =
			gSaveBlock1Ptr->secretBases[0].decorationPositions;
	}
...
static u8 AddDecorationWindow( u8 windowIndex )
{
	...
	{
		*windowId =
			AddWindow( &sDecorationWindowTemplates[windowIndex] );
	}
...
static void RemoveDecorationWindow( u8 windowIndex )
{
	ClearStdWindowAndFrameToTransparent(
		sDecorMenuWindowIds[windowIndex], FALSE );
...
static void AddDecorationActionsWindow( void )
{
	...
	InitMenuInUpperLeftCornerPlaySoundWhenAPressed(
		windowId, 4, sDecorationActionsCursorPos );
}
...
static void DecorationMenuAction_PutAway( u8 taskId )
{
	if( !sub_81299AC( taskId ) )
	{
		StringExpandPlaceholders(
			gStringVar4, gText_NoDecorationsInUse );
		DisplayItemMessageOnField( taskId,
			gStringVar4,
			ReturnToDecorationActionsAfterInvalidSelection );
	}
...
static void sub_8126E44( u8 taskId )
{
	...
	PrintDecorationCategoryMenuItems( taskId );
	InitMenuInUpperLeftCornerPlaySoundWhenAPressed( sDecorMenuWindowIds[1],
		DECORCAT_COUNT + 1,
...
static void PrintDecorationCategoryMenuItems( u8 taskId )
{
	...
	{
		// Only DOLL and CUSHION decorations are enabled when
		// decorating the player's room.
		if( shouldDisable == TRUE && i != DECORCAT_DOLL &&
			i != DECORCAT_CUSHION )
			PrintDecorationCategoryMenuItem(
				windowId, i, 8, i * 16, TRUE, TEXT_SPEED_FF );
		else
			PrintDecorationCategoryMenuItem(
				windowId, i, 8, i * 16, FALSE, TEXT_SPEED_FF );
	}

	AddTextPrinterParameterized( windowId,
		1,
		gTasks[taskId].data[11] == 2 ? gText_Exit : gText_Cancel,
		8,
		i * 16 + 1,
		0,
		NULL );
	...
	u8 winid, u8 category, u8 x, u8 y, bool8 disabled, u8 speed )
{
	...
	AddTextPrinterParameterized(
		winid, 1, gStringVar4, x, y, speed, NULL );
	str        = ConvertIntToDecimalStringN( str,
                GetNumOwnedDecorationsInCategory( category ),
                STR_CONV_MODE_RIGHT_ALIGN,
                2 );
	*( str++ ) = CHAR_SLASH;
	ConvertIntToDecimalStringN( str,
		gDecorationInventories[category].size,
		STR_CONV_MODE_RIGHT_ALIGN,
		2 );
...
static void InitDecorationItemsMenuLimits( void )
{
	sDecorationItemsMenu->numMenuItems =
		sNumOwnedDecorationsInCurCategory + 1;
...
static void PrintDecorationItemMenuItems( u8 taskId )
{
	...
	if( ( sCurDecorationCategory < DECORCAT_DOLL ||
		...
		gDecorationContext.isPlayerRoom == TRUE && data[11] == 0 )
	...
	for( i = 0; i < sDecorationItemsMenu->numMenuItems - 1; i++ )
	{
		...
		sDecorationItemsMenu->items[i].name =
			sDecorationItemsMenu->names[i];
	...
	gMultiuseListMenuTemplate.maxShowed =
		sDecorationItemsMenu->maxShownItems;
}
...
static void InitDecorationItemsWindow( u8 taskId )
{
	...
	data[13] = ListMenuInit( &gMultiuseListMenuTemplate,
		sDecorationsScrollOffset,
		sDecorationsCursorPos );
...
static void HandleDecorationItemsMenuInput( u8 taskId )
{
	...
	if( !gPaletteFade.active )
	{
		...
		ListMenuGetScrollAndRow( data[13],
			&sDecorationsScrollOffset,
			&sDecorationsCursorPos );
		switch( input )
		{
		...
		case LIST_CANCEL:
			PlaySE( SE_SELECT );
			sSecretBasePC_SelectedDecorationActions[data[11]][1](
				taskId );
			break;
		default:
			PlaySE( SE_SELECT );
			...
			DestroyListMenuTask( data[13],
				&sDecorationsScrollOffset,
				&sDecorationsCursorPos );
			RemoveDecorationWindow( 1 );
			RemoveDecorationItemsOtherWindows( );
			free( sDecorationItemsMenu );
			sSecretBasePC_SelectedDecorationActions[data[11]][0](
				taskId );
			break;
		}
	}
...
{
	PrintDecorationCategoryMenuItem(
		AddDecorationWindow( 2 ), category, 0, 0, 0, 0 );
}
...
static void IdentifyOwnedDecorationsCurrentlyInUseInternal( u8 taskId )
{
	...
	count = 0;
	memset( sSecretBaseItemsIndicesBuffer,
		0,
		sizeof( sSecretBaseItemsIndicesBuffer ) );
	memset( sPlayerRoomItemsIndicesBuffer,
		0,
		sizeof( sPlayerRoomItemsIndicesBuffer ) );

	for( i = 0; i < ARRAY_COUNT( sSecretBaseItemsIndicesBuffer ); i++ )
	{
		if( gSaveBlock1Ptr->secretBases[0].decorations[i] !=
			DECOR_NONE )
		{
			for( j = 0; j <
				gDecorationInventories[sCurDecorationCategory]
					.size;
				j++ )
			{
				if( gCurDecorationItems[j] ==
					gSaveBlock1Ptr->secretBases[0]
						.decorations[i] )
				{
					for( k = 0; k < count &&
...
static void ShowDecorationOnMap_(
	u16 mapX, u16 mapY, u8 decWidth, u8 decHeight, u16 decoration )
{
	u16 i, j;
	s16 x, y;
	u16 behavior;
	...
	for( j = 0; j < decHeight; j++ )
	{
		...
		{
			x        = mapX + i;
			behavior = GetBehaviorByMetatileId( 0x200 +
				gDecorations[decoration]
					.tiles[j * decWidth + i] );
			if( MetatileBehavior_IsSecretBaseImpassable(
				    behavior ) == TRUE ||
				( gDecorations[decoration].permission !=
			...
			if( gDecorations[decoration].permission !=
					DECORPERM_NA_WALL &&
...
void sub_8127E18( void )
{
	...
	for( i = 0; i < 14; i++ )
	{
		if( FlagGet( FLAG_DECORATION_1 + i ) == TRUE )
		{
			...
			for( j = 0; j < gMapHeader.events->eventObjectCount;
				j++ )
			{
				if( gMapHeader.events->eventObjects[j]
						.flagId ==
					FLAG_DECORATION_1 + i )
					break;
			}
			...
			VarSet( VAR_OBJ_GFX_ID_0 +
					( gMapHeader.events->eventObjects[j]
							.graphicsId -
						EVENT_OBJ_GFX_VAR_0 ),
				sPlaceDecorationGraphicsDataBuffer.decoration
					->tiles[0] );

			gSpecialVar_0x8005 =
				gMapHeader.events->eventObjects[j].localId;
			gSpecialVar_0x8006 = sCurDecorMapX;
			gSpecialVar_0x8007 = sCurDecorMapY;
			TrySpawnEventObject( gSpecialVar_0x8005,
				gSaveBlock1Ptr->location.mapNum,
				gSaveBlock1Ptr->location.mapGroup );
			TryMoveEventObjectToMapCoords( gSpecialVar_0x8005,
				gSaveBlock1Ptr->location.mapNum,
				gSaveBlock1Ptr->location.mapGroup,
				gSpecialVar_0x8006,
				gSpecialVar_0x8007 );
...
void sub_8127F68( u8 taskId )
{
	...
	else if( IsSelectedDecorInThePC( ) == TRUE )
	{
		...
		else
		{
			...
			{
				StringExpandPlaceholders(
					gStringVar4, gText_NoMoreDecorations );
			}
			else
			{
			...
			DisplayItemMessageOnField(
				taskId, gStringVar4, sub_8127A5C );
		}
	}
	else
	{
...
void sub_8128060( u8 taskId )
{
	...
	case 1:
		...
		SetUpPlacingDecorationPlayerAvatar(
			taskId, &sPlaceDecorationGraphicsDataBuffer );
		pal_fill_black( );
	...
	struct PlaceDecorationGraphicsDataBuffer* data, u8 decor )
{
	sDecor_CameraSpriteObjectIdx1 =
		gSprites[gFieldCamera.spriteId].data[0];
	gFieldCamera.spriteId =
		gpu_pal_decompress_alloc_tag_and_upload( data, decor );
	...
	u8 taskId, struct PlaceDecorationGraphicsDataBuffer* data )
{
	...
	x = 16 * (u8)gTasks[taskId].data[5] +
		sDecorationMovementInfo[data->decoration->shape].cameraX -
		8 * ( (u8)gTasks[taskId].data[5] - 1 );
	...
	if( gSaveBlock2Ptr->playerGender == MALE )
		sDecor_CameraSpriteObjectIdx2 =
			AddPseudoEventObject( EVENT_OBJ_GFX_BRENDAN_DECORATING,
				SpriteCallbackDummy,
				x,
				72,
				0 );
		...
		sDecor_CameraSpriteObjectIdx2 =
			AddPseudoEventObject( EVENT_OBJ_GFX_MAY_DECORATING,
				SpriteCallbackDummy,
				x,
				72,
				0 );
...
{
	if( x == gTasks[taskId].data[3] + 7 &&
		y == gTasks[taskId].data[4] + 7 && decor != DECOR_NONE )
	{
		return FALSE;
	}
	return TRUE;
...
bool8 sub_81284F4( u16 behaviorAt, const struct Decoration* decoration )
{
	if( MetatileBehavior_IsBlockDecoration( behaviorAt ) != TRUE )
	{
		if( decoration->id == DECOR_SOLID_BOARD &&
			MetatileBehavior_IsSecretBaseHole( behaviorAt ) ==
				TRUE )
			return TRUE;
...
bool8 sub_812853C( u8 taskId, const struct Decoration* decoration )
{
		...
		for( i = 0; i < mapY; i++ )
		{
			...
			for( j = 0; j < mapX; j++ )
			{
				...
				behaviorAt = MapGridGetMetatileBehaviorAt(
					curX, curY );
				behaviorBy =
					GetBehaviorByMetatileId( 0x200 +
						decoration->tiles[( mapY - 1 -
									  i ) *
								mapX +
							j] ) &
				...
				if( !sub_81284AC(
					    taskId, curX, curY, behaviorBy ) )
					return FALSE;

				behaviorAt =
					GetEventObjectIdByXYZ( curX, curY, 0 );
	...
	case DECORPERM_BEHIND_FLOOR:
		for( i = 0; i < mapY - 1; i++ )
		{
			...
			for( j = 0; j < mapX; j++ )
			{
				...
				if( GetEventObjectIdByXYZ( curX, curY, 0 ) !=
					16 )
					return FALSE;
			}
		}
	...
	case DECORPERM_SPRITE:
		...
		for( j = 0; j < mapX; j++ )
		{
			...
			{
				if( !MetatileBehavior_HoldsLargeDecoration(
					    behaviorAt ) )
					return FALSE;
			}
...
void sub_8128950( u8 taskId )
{
	...
	else
	{
		PlaySE( SE_HAZURE );
		StringExpandPlaceholders(
			gStringVar4, gText_CantBePlacedHere );
...
void sub_81289F0( u8 taskId )
{
	ClearDialogWindowAndFrame( 0, 0 );
	sub_8128AAC( taskId );
	if( gDecorations[gCurDecorationItems[gCurDecorationIndex]]
			.permission != DECORPERM_SPRITE )
	{
		ShowDecorationOnMap( gTasks[taskId].data[0],
			gTasks[taskId].data[1],
			gCurDecorationItems[gCurDecorationIndex] );
	}
	else
...
void sub_8128AAC( u8 taskId )
{
	...
	for( i = 0; i < gDecorationContext.size; i++ )
	{
		if( gDecorationContext.items[i] == DECOR_NONE )
		{
			gDecorationContext.items[i] =
				gCurDecorationItems[gCurDecorationIndex];
			gDecorationContext.pos[i] =
				( ( gTasks[taskId].data[0] - 7 ) << 4 ) +
	...
	if( !gDecorationContext.isPlayerRoom )
	{
		for( i = 0; i < 16; i++ )
		{
			...
			{
				sSecretBaseItemsIndicesBuffer[i] =
					gCurDecorationIndex + 1;
				break;
			}
		}
	}
...
bool8 sub_8128D10( u8 taskId )
{
	...
	if( sDecorationLastDirectionMoved == DIR_NORTH &&
		data[1] - 7 >= gMapHeader.mapLayout->height )
	{
		data[1]--;
		return FALSE;
	}
	...
	if( sDecorationLastDirectionMoved == DIR_EAST &&
		data[0] + data[5] - 8 >= gMapHeader.mapLayout->width )
	{
		data[0]--;
		return FALSE;
	}
...
{
	CpuFastCopy( &( (u16*)gTilesetPointer_SecretBase->palettes )[pal << 4],
		dest,
		32 );
}
...
void sub_8129088( u8* dest, u16 tile )
{
	...
	CpuFastCopy( &( (u8*)gTilesetPointer_SecretBase->tiles )[tile << 5],
		buffer,
		32 );
	switch( mode )
	{
...
{
	return ( (u16*)gTilesetPointer_SecretBaseRedCave->metatiles )[tile] &
		0xFFF;
}
...
void sub_81291E8( struct PlaceDecorationGraphicsDataBuffer* data )
{
	...
	{
		data->tiles[gUnknown_085A71B0[shape].tiles[i]] = sub_81291CC(
			data->decoration->tiles[gUnknown_085A71B0[shape]
							.y[i]] *
				8 +
	...
	struct PlaceDecorationGraphicsDataBuffer* data, u8 decor )
{
	...
	if( data->decoration->permission == DECORPERM_SPRITE )
		return AddPseudoEventObject( data->decoration->tiles[0],
			SpriteCallbackDummy,
			0,
			0,
			1 );
	...
	u16 tilesTag, u16 paletteTag, u8 decor )
{
	...
	CopyItemIconPicTo4x4Buffer(
		gItemIconDecompressionBuffer, gItemIcon4x4Buffer );
...
u8 AddDecorationIconObjectFromEventObject(
	u16 tilesTag, u16 paletteTag, u8 decor )
{
	u8 spriteId;
	struct SpriteSheet sheet;
		...
		DECORPERM_SPRITE )
	{
		...
		sub_8129068( sPlaceDecorationGraphicsDataBuffer.palette,
			( (u16*)gTilesetPointer_SecretBaseRedCave->metatiles )
					[( sPlaceDecorationGraphicsDataBuffer
							 .decoration
							 ->tiles[0] *
		...
		sheet.size =
			gUnknown_085A72F4[sPlaceDecorationGraphicsDataBuffer
						  .decoration->shape]
			<< 5;
		sheet.tag = tilesTag;
		LoadSpriteSheet( &sheet );
...
u8 AddDecorationIconObject(
	u8 decor, s16 x, s16 y, u8 priority, u16 tilesTag, u16 paletteTag )
{
	u8 spriteId;

	if( decor > NUM_DECORATIONS )
	{
		spriteId = AddDecorationIconObjectFromIconTable(
			tilesTag, paletteTag, DECOR_NONE );
	...
	{
		spriteId = AddDecorationIconObjectFromEventObject(
			tilesTag, paletteTag, decor );
		if( spriteId == MAX_SPRITES )
			return MAX_SPRITES;

		gSprites[spriteId].pos2.x = x;
		if( decor == DECOR_SILVER_SHIELD ||
			decor == DECOR_GOLD_SHIELD )
			gSprites[spriteId].pos2.y = y - 4;
		else
...
void sub_8129708( void )
{
	...
	else if( gDecorations[gDecorationContext
				      .items[sDecorRearrangementDataBuffer
						      [gSpecialVar_0x8004]
							      .idx]]
			 .permission == DECORPERM_SPRITE )
	{
		gSpecialVar_0x8005 =
			sDecorRearrangementDataBuffer[gSpecialVar_0x8004]
				.flagId;
		sub_81296EC( sDecorRearrangementDataBuffer[gSpecialVar_0x8004]
				     .idx );
		for( i = 0; i < gMapHeader.events->eventObjectCount; i++ )
		{
			if( gMapHeader.events->eventObjects[i].flagId ==
				gSpecialVar_0x8005 )
			{
				gSpecialVar_0x8006 =
					gMapHeader.events->eventObjects[i]
						.localId;
				break;
			}
		}
...
void sub_81297AC( void )
{
	...
	for( i = 0; i < gMapHeader.events->eventObjectCount; i++ )
	{
		...
		{
			gSpecialVar_0x8005 =
				gMapHeader.events->eventObjects[i].localId;
			break;
		}
	}
}
...
void sub_81297F8( void )
{
	...
	{
		perm = gDecorations[gDecorationContext.items
					    [sDecorRearrangementDataBuffer[i]
							    .idx]]
			       .permission;
		posX = gDecorationContext
				.pos[sDecorRearrangementDataBuffer[i].idx] >>
			4;
		posY = gDecorationContext
				.pos[sDecorRearrangementDataBuffer[i].idx] &
			0x0F;
		if( perm != DECORPERM_SPRITE )
		{
			for( y = 0;
				y < sDecorRearrangementDataBuffer[i].height;
				y++ )
			{
				for( x = 0; x <
...
void sub_81298EC( u8 taskId )
{
	...
	case 3:
		...
		{
			StringExpandPlaceholders(
				gStringVar4, gText_DecorationReturnedToPC );
			DisplayItemMessageOnField(
				taskId, gStringVar4, sub_8129D64 );
...
void sub_8129BF8( u8 taskId )
{
	...
	StringExpandPlaceholders(
		gStringVar4, gText_StopPuttingAwayDecorations );
...
void sub_8129C74( u8 taskId )
{
	...
	{
		StringExpandPlaceholders(
			gStringVar4, gText_ReturnDecorationToPC );
	...
	else
	{
		...
		{
			gSprites[sDecor_CameraSpriteObjectIdx1].invisible =
				FALSE;
			gSprites[sDecor_CameraSpriteObjectIdx1].callback =
				SpriteCallbackDummy;
			StringExpandPlaceholders( gStringVar4,
				gText_StopPuttingAwayDecorations );
			DisplayItemMessageOnField(
				taskId, gStringVar4, sub_812A1F0 );
		}
		else
		{
			StringExpandPlaceholders(
				gStringVar4, gText_NoDecorationHere );
			DisplayItemMessageOnField(
				taskId, gStringVar4, sub_8129D64 );
		}
	}
...
bool8 sub_8129E74(
	u8 taskId, u8 idx, struct DecorRearrangementDataBuffer* data )
{
	u8 x;
	u8 y;
	u8 xOff;
	...
	ht   = data->height;
	if( gDecorationContext.items[idx] == DECOR_SAND_ORNAMENT &&
		MapGridGetMetatileIdAt( xOff + 7, yOff + 7 ) == 0x28C )
		ht--;
...
void sub_8129F20( void )
{
	...
	xOff = gDecorationContext
			.pos[sDecorRearrangementDataBuffer
					[sCurDecorSelectedInRearrangement]
						.idx] >>
		4;
	yOff = gDecorationContext
			.pos[sDecorRearrangementDataBuffer
					[sCurDecorSelectedInRearrangement]
						.idx] &
		0x0F;
	for( i = 0; i < 0x40; i++ )
	{
		if( gSaveBlock1Ptr->eventObjectTemplates[i].x == xOff &&
			gSaveBlock1Ptr->eventObjectTemplates[i].y == yOff &&
			!FlagGet( gSaveBlock1Ptr->eventObjectTemplates[i]
					  .flagId ) )
		{
			sDecorRearrangementDataBuffer
				[sCurDecorSelectedInRearrangement]
					.flagId =
				gSaveBlock1Ptr->eventObjectTemplates[i].flagId;
			break;
		}
	}
}
...
bool8 sub_8129FC8( u8 taskId )
{
	...
	for( i = 0; i < gDecorationContext.size; i++ )
	{
		...
		{
			if( gDecorations[gDecorationContext.items[i]]
					.permission == DECORPERM_SPRITE )
			{
				sub_8129D8C( gDecorationContext.items[i],
					sDecorRearrangementDataBuffer );
				if( sub_8129E74( taskId,
					    i,
					    sDecorRearrangementDataBuffer ) ==
					TRUE )
				{
...
void sub_812A040( u8 left, u8 top, u8 right, u8 bottom )
{
	...
	for( i = 0; i < gDecorationContext.size; i++ )
	{
		...
		if( decorIdx != 0 &&
			gDecorations[decorIdx].permission ==
				DECORPERM_SPRITE &&
			left <= xOff && top <= yOff && right >= xOff &&
			bottom >= yOff )
		{
			sDecorRearrangementDataBuffer
				[sCurDecorSelectedInRearrangement]
...
void sub_812A0E8( u8 taskId )
{
	...
	if( sub_8129FC8( taskId ) != TRUE )
	{
		for( i = 0; i < gDecorationContext.size; i++ )
		{
			...
			{
				sub_8129D8C( var1,
					&sDecorRearrangementDataBuffer[0] );
				if( sub_8129E74( taskId,
					    i,
					    &sDecorRearrangementDataBuffer
						    [0] ) == TRUE )
				{
					sDecorRearrangementDataBuffer[0].idx =
						i;
					sCurDecorSelectedInRearrangement++;
					break;
				}
		...
		{
			xOff = gDecorationContext
					.pos[sDecorRearrangementDataBuffer[0]
							.idx] >>
				4;
			yOff = gDecorationContext
					.pos[sDecorRearrangementDataBuffer[0]
							.idx] &
				0x0F;
			var1 = yOff - sDecorRearrangementDataBuffer[0].height +
				1;
			var2 = sDecorRearrangementDataBuffer[0].width + xOff -
				1;
...
void sub_812A3D4( u8 taskId )
{
	if( IsSelectedDecorInThePC( ) == TRUE )
	{
		StringCopy( gStringVar1,
			gDecorations[gCurDecorationItems[gCurDecorationIndex]]
				.name );
		StringExpandPlaceholders(
			gStringVar4, gText_DecorationWillBeDiscarded );
		DisplayItemMessageOnField( taskId, gStringVar4, sub_812A458 );
	}
	else
	{
		StringExpandPlaceholders(
			gStringVar4, gText_CantThrowAwayInUse );
		DisplayItemMessageOnField( taskId, gStringVar4, sub_8127A5C );
	}
}
