...
#define AI_THINKING_STRUCT \
	( (struct AI_ThinkingStruct*)( gBattleResources->ai ) )
...
void BattleAI_HandleItemUseBeforeAISetup( u8 defaultScoreMoves )
{
	...
	if( ( gBattleTypeFlags & BATTLE_TYPE_TRAINER ) &&
		!( gBattleTypeFlags &
			( BATTLE_TYPE_LINK | BATTLE_TYPE_SAFARI |
...
void BattleAI_SetupAIData( u8 defaultScoreMoves )
{
	...
	if( gBattleTypeFlags & BATTLE_TYPE_RECORDED )
		...
		AI_THINKING_STRUCT->aiFlags = AI_SCRIPT_CHECK_BAD_MOVE |
			AI_SCRIPT_CHECK_VIABILITY | AI_SCRIPT_TRY_TO_FAINT;
	...
	if( gBattleTypeFlags & BATTLE_TYPE_DOUBLE )
		AI_THINKING_STRUCT->aiFlags |=
			AI_SCRIPT_DOUBLE_BATTLE; // act smart in
						 // doubles and
						 // don't attack
						 // your partner
}
...
static u8 ChooseMoveOrAction_Singles( void )
{
	...
	for( i = 1; i < MAX_MON_MOVES; i++ )
	{
		...
		{
			// In ruby, the order of these if statements is
			// reversed.
			if( currentMoveArray[0] ==
				AI_THINKING_STRUCT->score[i] )
			{
				currentMoveArray[numOfBestMoves] =
					AI_THINKING_STRUCT->score[i];
...
static u8 ChooseMoveOrAction_Doubles( void )
{
	...
	for( i = 0; i < MAX_BATTLERS_COUNT; i++ )
	{
		...
		else
		{
			...
			else if( AI_THINKING_STRUCT->aiAction &
				AI_ACTION_WATCH )
			{
			...
			else
			{
				...
				{
					if( gBattleMons[sBattler_AI]
							.moves[j] != 0 )
					{
						if( mostViableMovesScores[0] ==
							AI_THINKING_STRUCT
								->score[j] )
						{
				...
				actionOrMoveIndex[i] =
					mostViableMovesIndices[Random( ) %
						mostViableMovesNo];
				bestMovePointsForTarget[i] =
					mostViableMovesScores[0];

				// Don't use a move against ally if it has less
				// than 100 points.
				if( i == ( sBattler_AI ^ BIT_FLANK ) &&
	...
	gBattlerTarget =
		mostViableTargetsArray[Random( ) % mostViableTargetsNo];
...
static void BattleAI_DoAIProcessing( void )
{
	while( AI_THINKING_STRUCT->aiState != AIState_FinishedProcessing )
	{
		...
		case AIState_SettingUp:
			gAIScriptPtr = gBattleAI_ScriptsTable
				[AI_THINKING_STRUCT->aiLogicId]; // set
								 // AI
								 // ptr
								 // to
								 // logic
			...
			else
			{
				AI_THINKING_STRUCT->moveConsidered =
					gBattleMons[sBattler_AI]
						.moves[AI_THINKING_STRUCT
								->movesetIndex];
			}
			AI_THINKING_STRUCT->aiState++;
			break;
		case AIState_Processing:
			...
			{
				sBattleAICmdTable
					[*gAIScriptPtr]( ); // Run AI command.
			}
			else
			{
				AI_THINKING_STRUCT->score
					[AI_THINKING_STRUCT->movesetIndex] = 0;
			...
			if( AI_THINKING_STRUCT->aiAction & AI_ACTION_DONE )
			{
				...
				if( AI_THINKING_STRUCT->movesetIndex <
						MAX_MON_MOVES &&
...
static void RecordLastUsedMoveByTarget( void )
{
	...
	for( i = 0; i < MAX_MON_MOVES; i++ )
	{
		if( BATTLE_HISTORY->usedMoves[gBattlerTarget].moves[i] ==
			...
			break;
		if( BATTLE_HISTORY->usedMoves[gBattlerTarget].moves[i] !=
				gLastMoves[gBattlerTarget] // HACK: This
							   // redundant
							   // condition is a
							   // hack to make the
							   // asm match.
...
static void BattleAICmd_score( void )
{
	AI_THINKING_STRUCT->score[AI_THINKING_STRUCT->movesetIndex] +=
		gAIScriptPtr[1]; // Add the result to the array of the move
				 // consider's score.
...
static void BattleAICmd_if_hp_less_than( void )
{
	...
	if( ( u32 )( 100 * gBattleMons[battlerId].hp /
		    gBattleMons[battlerId].maxHP ) < gAIScriptPtr[2] )
...
static void BattleAICmd_if_user_has_attacking_move( void )
{
	...
	for( i = 0; i < MAX_MON_MOVES; i++ )
	{
		if( gBattleMons[sBattler_AI].moves[i] != 0 &&
			gBattleMoves[gBattleMons[sBattler_AI].moves[i]]
					.power != 0 )
			break;
	}
...
static void BattleAICmd_get_how_powerful_move_is( void )
{
		...
		sDiscouragedPowerfulMoveEffects[i] == 0xFFFF )
	{
		...
		*( &gBattleStruct->dynamicMoveType ) = 0;
		...
		for( checkedMove = 0; checkedMove < MAX_MON_MOVES;
			checkedMove++ )
		{
			for( i = 0;
				sDiscouragedPowerfulMoveEffects[i] != 0xFFFF;
				i++ )
			{
				if( gBattleMoves[gBattleMons[sBattler_AI]
							 .moves[checkedMove]]
						.effect ==
					sDiscouragedPowerfulMoveEffects[i] )
					break;
			...
			if( gBattleMons[sBattler_AI].moves[checkedMove] !=
					MOVE_NONE &&
				sDiscouragedPowerfulMoveEffects[i] == 0xFFFF &&
				gBattleMoves[gBattleMons[sBattler_AI]
						     .moves[checkedMove]]
						.power > 1 )
			{
				gCurrentMove = gBattleMons[sBattler_AI]
						       .moves[checkedMove];
				AI_CalcDmg( sBattler_AI, gBattlerTarget );
				TypeCalc( gCurrentMove,
					sBattler_AI,
					gBattlerTarget );
				moveDmgs[checkedMove] = gBattleMoveDamage *
					AI_THINKING_STRUCT
						->simulatedRNG[checkedMove] /
					100;
		...
		for( checkedMove = 0; checkedMove < MAX_MON_MOVES;
			checkedMove++ )
		{
			if( moveDmgs[checkedMove] >
				moveDmgs[AI_THINKING_STRUCT->movesetIndex] )
				break;
		}

		if( checkedMove == MAX_MON_MOVES )
			AI_THINKING_STRUCT->funcResult =
				MOVE_MOST_POWERFUL; // Is the most
						    // powerful.
		else
			AI_THINKING_STRUCT->funcResult =
				MOVE_NOT_MOST_POWERFUL; // Not the
							// most
							// powerful.
	}
	else
...
static void BattleAICmd_if_user_goes( void )
{
	if( GetWhoStrikesFirst( sBattler_AI, gBattlerTarget, TRUE ) ==
		gAIScriptPtr[1] )
		gAIScriptPtr = T1_READ_PTR( gAIScriptPtr + 2 );
	else
		gAIScriptPtr += 6;
...
static void BattleAICmd_count_usable_party_mons( void )
{
	...
	if( gBattleTypeFlags & BATTLE_TYPE_DOUBLE )
	{
		...
		battlerOnField2 =
			gBattlerPartyIndexes[GetBattlerAtPosition( position )];
	}
...
static void BattleAICmd_get_ability( void )
{
	...
	if( gActiveBattler != battlerId )
	{
		...
		{
			AI_THINKING_STRUCT->funcResult =
				BATTLE_HISTORY->abilities[battlerId];
			gAIScriptPtr += 2;
			return;
		}
		...
		if( gBaseStats[gBattleMons[battlerId].species].abilities[0] !=
			ABILITY_NONE )
		{
			if( gBaseStats[gBattleMons[battlerId].species]
					.abilities[1] != ABILITY_NONE )
			{
				// AI has no knowledge of opponent, so it
				// guesses which ability.
				if( Random( ) & 1 )
					AI_THINKING_STRUCT->funcResult =
						gBaseStats[gBattleMons[battlerId]
								   .species]
							.abilities[0];
				else
					AI_THINKING_STRUCT->funcResult =
		...
		else
		{
			AI_THINKING_STRUCT->funcResult =
				gBaseStats[gBattleMons[battlerId].species]
					.abilities[1]; // AI can't actually
						       // reach this part since
						       // no pokemon has
						       // ability 2 and no
						       // ability 1.
		}
...
static void BattleAICmd_check_ability( void )
{
	...
	if( gAIScriptPtr[1] == AI_TARGET ||
		gAIScriptPtr[1] == AI_TARGET_PARTNER )
	{
		...
		// Abilities that prevent fleeing.
		else if( gBattleMons[battlerId].ability ==
				ABILITY_SHADOW_TAG ||
			gBattleMons[battlerId].ability ==
				ABILITY_MAGNET_PULL ||
			gBattleMons[battlerId].ability == ABILITY_ARENA_TRAP )
		...
		else if( gBaseStats[gBattleMons[battlerId].species]
				 .abilities[0] != ABILITY_NONE )
		{
			if( gBaseStats[gBattleMons[battlerId].species]
					.abilities[1] != ABILITY_NONE )
			{
				u8 abilityDummyVariable =
					ability; // Needed to match.
				if( gBaseStats[gBattleMons[battlerId].species]
							.abilities[0] !=
			...
			{
				ability = gBaseStats[gBattleMons[battlerId]
							     .species]
						  .abilities[0];
			}
		}
	...
	if( ability == 0 )
		...
		AI_THINKING_STRUCT->funcResult = 1; // Pokemon has the ability
						    // we wanted to check.
	else
		AI_THINKING_STRUCT->funcResult =
			0; // Pokemon doesn't have the ability
			   // we wanted to check.
	...
	*dynamicMoveType               = 0;
	...
	for( i = 0; i < MAX_MON_MOVES; i++ )
	{
		...
		if( gCurrentMove != MOVE_NONE )
		{
			...
			if( AI_THINKING_STRUCT->funcResult <
				gBattleMoveDamage )
				AI_THINKING_STRUCT->funcResult =
					gBattleMoveDamage;
		}
...
static void BattleAICmd_if_type_effectiveness( void )
{
	...
	// Store gBattleMoveDamage in a u8 variable because gAIScriptPtr[1] is
	// a u8.
...
static void BattleAICmd_if_status_in_party( void )
{
	...
	party = ( GetBattlerSide( battlerId ) == B_SIDE_PLAYER ) ? gPlayerParty
								 : gEnemyParty;
...
static void BattleAICmd_if_status_not_in_party( void )
{
	...
	for( i = 0; i < PARTY_SIZE; i++ )
	{
		...
		{
			gAIScriptPtr +=
				10; // UB: Still bugged in Emerald. Uncomment
				    // the return statement to fix.
			// return;
		}
	}
...
static void BattleAICmd_if_stat_level_less_than( void )
{
	...
	if( gBattleMons[battlerId].statStages[gAIScriptPtr[2]] <
		gAIScriptPtr[3] )
...
static void BattleAICmd_if_can_faint( void )
{
	...
	gBattleMoveDamage = gBattleMoveDamage *
		AI_THINKING_STRUCT
			->simulatedRNG[AI_THINKING_STRUCT->movesetIndex] /
		100;
...
static void BattleAICmd_if_has_move( void )
{
	...
	case AI_USER_PARTNER:
		...
		else
		{
			for( i = 0; i < MAX_MON_MOVES; i++ )
			{
				if( gBattleMons[sBattler_AI ^ BIT_FLANK]
						.moves[i] == *movePtr )
					break;
			}
		}
...
{
	u8 safariFleeRate =
		gBattleStruct->safariEscapeFactor * 5; // Safari flee
						       // rate, from
						       // 0-20.
...
static void BattleAICmd_get_hold_effect( void )
{
	...
	if( gActiveBattler != battlerId )
		AI_THINKING_STRUCT->funcResult = ItemId_GetHoldEffect(
			BATTLE_HISTORY->itemEffects[battlerId] );
	else
		AI_THINKING_STRUCT->funcResult =
...
static void BattleAICmd_get_stockpile_count( void )
{
	...
	AI_THINKING_STRUCT->funcResult =
		gDisableStructs[battlerId].stockpileCounter;
...
#ifdef NONMATCHING
	AI_THINKING_STRUCT->funcResult =
		gBattleStruct->usedHeldItems[battlerId];
#else
	AI_THINKING_STRUCT->funcResult = *(u8*)( (u8*)( gBattleStruct ) +
		offsetof( struct BattleStruct, usedHeldItems ) +
		( battlerId * 2 ) );
#endif // NONMATCHING
...
static void BattleAICmd_get_protect_count( void )
{
	...
	AI_THINKING_STRUCT->funcResult =
		gDisableStructs[battlerId].protectUses;
...
static void BattleAICmd_if_level_cond( void )
{
	...
	{
	case 0: // greater than
		if( gBattleMons[sBattler_AI].level >
...
static void BattleAICmd_if_flash_fired( void )
{
	...
	if( gBattleResources->flags->flags[battlerId] &
