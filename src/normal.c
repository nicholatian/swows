...
u32 UnpackSelectedBattleAnimPalettes( s16 selector )
{
	...
	return sub_80A75AC( battleBackground,
		attacker,
		target,
		attackerPartner,
		targetPartner,
...
void sub_81158A4( struct Sprite* sprite )
{
	...
	BlendPalettes(
		selectedPalettes, gBattleAnimArgs[4], gBattleAnimArgs[3] );
	sprite->invisible = 1;
...
static void sub_81158F8( struct Sprite* sprite )
{
	...
	if( sprite->data[1] & 0x100 )
		BlendPalettes(
			selectedPalettes, sprite->data[4], sprite->data[3] );
	else
		BlendPalettes(
			selectedPalettes, sprite->data[6], sprite->data[5] );
...
static void sub_8115984( struct Sprite* sprite )
{
	...
	{
		selectedPalettes =
			UnpackSelectedBattleAnimPalettes( sprite->data[7] );
...
static void sub_8115A54(
	u8 taskId, u8 initialBlendAmount, u8 targetBlendAmount )
{
	u32 selectedPalettes =
		UnpackSelectedBattleAnimPalettes( gTasks[taskId].data[0] );
	BeginNormalPaletteFade( selectedPalettes,
...
static void sub_8115BC8(
	u8 taskId, u8 initialBlendAmount, u8 targetBlendAmount )
{
	u32 selectedPalettes = ( (u16)gTasks[taskId].data[9] << 16 ) |
		(u16)gTasks[taskId].data[10];
	BeginNormalPaletteFade( selectedPalettes,
		gTasks[taskId].data[1],
		initialBlendAmount,
		targetBlendAmount,
...
static void sub_8115E00( u8 taskId )
{
	...
	selectedPalettes = 1
		<< ( IndexOfSpritePaletteTag( gTasks[taskId].data[7] ) + 16 );
...
static void sub_8115EB8( u8 taskId )
{
	...
	if( !gPaletteFade.active )
	{
		selectedPalettes = 1 << ( IndexOfSpritePaletteTag(
						  gTasks[taskId].data[7] ) +
					   16 );
		BeginNormalPaletteFade(
			selectedPalettes, 0, 0, 0, RGB( 0, 0, 0 ) );
		DestroyAnimVisualTask( taskId );
	}
}
...
void sub_8115F94( u8 taskId )
{
	...
	if( gTasks[taskId].data[2] & 0x1 )
	{
		paletteIndex = IndexOfSpritePaletteTag(
			gSprites[gHealthboxSpriteIds[attackerBattler]]
				.template->paletteTag );
	...
	TintPlttBuffer( selectedPalettes,
		gTasks[taskId].data[5],
		gTasks[taskId].data[6],
		gTasks[taskId].data[7] );
...
void sub_81160A4( struct Sprite* sprite )
{
	...
	case 2:
		StoreSpriteCallbackInData6(
			sprite, (void*)&gSpriteCoordOffsetX );
		break;
	default:
		StoreSpriteCallbackInData6(
			sprite, (void*)&gSpriteCoordOffsetY );
		break;
	}
...
static void sub_8116148( struct Sprite* sprite )
{
	...
	if( sprite->data[3] > 0 )
	{
		...
		else
		{
			...
			*(u16*)( sprite->data[6] |
				( sprite->data[7] << 16 ) ) += sprite->data[0];
...
void sub_81164F0( struct Sprite* sprite )
{
	...
	sprite->pos1.x  = gSprites[sprite->data[0]].pos1.x +
		gSprites[sprite->data[0]].pos2.x;
	sprite->pos1.y = gSprites[sprite->data[0]].pos1.y +
		gSprites[sprite->data[0]].pos2.y;
