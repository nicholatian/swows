...
void sub_80F88DC( void ) { sub_80DEDA8( 0xFF ); }
...
void sub_80F8B94( void )
{
	...
	if( gLinkContestFlags & LINK_CONTEST_FLAG_IS_LINK )
	{
		for( i = 0; i < gNumLinkContestPlayers; i++ )
		{
			eventObjectId = GetEventObjectIdByLocalIdAndMap(
				gUnknown_0858D8EC[i],
				gSaveBlock1Ptr->location.mapNum,
				gSaveBlock1Ptr->location.mapGroup );
			sprite = &gSprites[gEventObjects[eventObjectId]
						   .spriteId];
...
u8 GiveMonArtistRibbon( void )
{
	...
	hasArtistRibbon = GetMonData(
		&gPlayerParty[gContestMonPartyIndex], MON_DATA_ARTIST_RIBBON );
	if( !hasArtistRibbon &&
		...
		gUnknown_02039F08[gContestPlayerMonIndex] >= 800 )
	{
		...
		if( GetRibbonCount( &gPlayerParty[gContestMonPartyIndex] ) >
			4 )
			sub_80EE4DC( &gPlayerParty[gContestMonPartyIndex],
				MON_DATA_ARTIST_RIBBON );
	...
	if( FindTaskIdByFunc( sub_80F8EE8 ) == 0xFF )
	{
		...
		if( gLinkContestFlags & LINK_CONTEST_FLAG_IS_LINK )
		{
			if( !( gLinkContestFlags &
				    LINK_CONTEST_FLAG_HAS_RS_PLAYER ) )
				DoMonFrontSpriteAnimation( &gSprites[spriteId],
					species,
					FALSE,
					0 );
		}
		else
		{
			DoMonFrontSpriteAnimation(
				&gSprites[spriteId], species, FALSE, 0 );
		}
...
void sub_80F9154( void ) { gLinkContestFlags = 0; }
...
void HealPlayerParty( void )
{
	...
	for( i = 0; i < gPlayerPartyCount; i++ )
	{
		...
		ppBonuses =
			GetMonData( &gPlayerParty[i], MON_DATA_PP_BONUSES );
		...
		for( j = 0; j < MAX_MON_MOVES; j++ )
		{
			arg[0] = CalculatePPWithBonus(
				GetMonData(
					&gPlayerParty[i], MON_DATA_MOVE1 + j ),
				ppBonuses,
				j );
			SetMonData( &gPlayerParty[i], MON_DATA_PP1 + j, arg );
		}

		// since status is u32, the four 0 assignments here are
		// probably for safety to prevent undefined data from reaching
		// SetMonData.
		arg[0] = 0;
		arg[1] = 0;
		arg[2] = 0;
...
u8 ScriptGiveMon(
	u16 species, u8 level, u16 item, u32 unused1, u32 unused2, u8 unused3 )
{
	u16 nationalDexNum;
	int sentToPc;
	u8 heldItem[2];
...
bool8 sub_80F9370( void )
{
	...
	if( hasItem == TRUE )
		GetBerryNameByBerryType(
			ItemIdToBerryType( ITEM_ENIGMA_BERRY ), gStringVar1 );
...
void ReducePlayerPartyToSelectedMons( void )
{
	...
	for( i = 0; i < 4; i++ )
		if( gSelectedOrderFromParty[i] ) // as long as the order keeps
						 // going (did the player
						 // select 1 mon? 2? 3?), do
						 // not stop
