...
void ProcessDma3Requests( void )
{
	...
	// as long as there are DMA requests to process (unless size or vblank
	// is an issue), do not exit
	while( gDma3Requests[gDma3RequestCursor].size != 0 )
	{
		...
		case DMA_REQUEST_COPY32: // regular 32-bit copy
			...
			break;
		case DMA_REQUEST_FILL32: // repeat a single 32-bit value across
					 // RAM
			Dma3FillLarge32_(
				gDma3Requests[gDma3RequestCursor].value,
				gDma3Requests[gDma3RequestCursor].dest,
				gDma3Requests[gDma3RequestCursor].size );
			break;
		case DMA_REQUEST_COPY16: // regular 16-bit copy
			Dma3CopyLarge16_(
				...
				gDma3Requests[gDma3RequestCursor].size );
			break;
		case DMA_REQUEST_FILL16: // repeat a single 16-bit value across
					 // RAM
			Dma3FillLarge16_(
				gDma3Requests[gDma3RequestCursor].value,
				gDma3Requests[gDma3RequestCursor].dest,
		...
		if( gDma3RequestCursor >=
			MAX_DMA_REQUESTS ) // loop back to the first
					   // DMA request
			gDma3RequestCursor = 0;
	}
}
