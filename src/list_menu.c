...
static bool8 ListMenuChangeSelection( struct ListMenu* list,
	...
	u8 count,
	bool8 movingDown );
static void ListMenuPrintEntries(
	struct ListMenu* list, u16 startIndex, u16 yOffset, u16 count );
static void ListMenuDrawCursor( struct ListMenu* list );
static void ListMenuCallSelectionChangedCallback(
	struct ListMenu* list, u8 onInit );
...
static u8 ListMenuAddCursorObjectInternal(
	struct CursorStruct* cursor, u32 cursorKind );
static void ListMenuUpdateCursorObject(
	u8 taskId, u16 x, u16 y, u32 cursorKind );
...
static const u16 sRedArrowPal[] =
	INCBIN_U16( "graphics/interface/red_arrow.gbapal" );
static const u32 sRedArrowOtherGfx[] =
	INCBIN_U32( "graphics/interface/red_arrow_other.4bpp.lz" );
static const u32 sSelectorOutlineGfx[] =
	INCBIN_U32( "graphics/interface/selector_outline.4bpp.lz" );
static const u32 sRedArrowGfx[] =
	INCBIN_U32( "graphics/interface/red_arrow.4bpp.lz" );
	...
	u16 palNum )
{
	...
	default:
		...
		{
		case 2:
			LoadUserWindowBorderGfx( sMysteryGiftLinkMenu.windowId,
				tileNum,
				palNum );
		case 1:
			DrawTextBorderOuter( sMysteryGiftLinkMenu.windowId,
				tileNum,
				palNum / 16 );
			break;
		}
		...
		break;
	case 1:
		sMysteryGiftLinkMenu.currItemId = ListMenu_ProcessInput(
			sMysteryGiftLinkMenu.listTaskId );
		...
		if( sMysteryGiftLinkMenu.state == 2 )
		{
			...
			else
			{
				switch( arg2 )
				{
				case 0: // can never be reached, because of the
					// if
					// statement above
	...
	case 2:
		DestroyListMenuTask(
			sMysteryGiftLinkMenu.listTaskId, NULL, NULL );
...
u8 ListMenuInit( struct ListMenuTemplate* listMenuTemplate,
	u16 scrollOffset,
	u16 selectedRow )
{
	u8 taskId = ListMenuInitInternal(
		listMenuTemplate, scrollOffset, selectedRow );
...
s32 ListMenu_ProcessInput( u8 listTaskId )
{
	...
	if( gMain.newKeys & A_BUTTON )
	{
		return list->template
			.items[list->scrollOffset + list->selectedRow]
			.id;
	}
	...
	else // try to move by one window scroll
	{
		...
		if( leftButton )
		{
			ListMenuChangeSelection(
				list, TRUE, list->template.maxShowed, FALSE );
			return LIST_NOTHING_CHOSEN;
		}
		else if( rightButton )
		{
			ListMenuChangeSelection(
				list, TRUE, list->template.maxShowed, TRUE );
			return LIST_NOTHING_CHOSEN;
		}
		else
		{
...
void DestroyListMenuTask( u8 listTaskId, u16* scrollOffset, u16* selectedRow )
{
	...
	if( list->taskId != TASK_NONE )
		ListMenuRemoveCursorObject(
			list->taskId, list->template.cursorKind - 2 );
...
void RedrawListMenu( u8 listTaskId )
{
	...
	ListMenuPrintEntries(
		list, list->scrollOffset, 0, list->template.maxShowed );
	ListMenuDrawCursor( list );
	CopyWindowToVram( list->template.windowId, 2 );
}

// unused
void ChangeListMenuPals(
	u8 listTaskId, u8 cursorPal, u8 fillValue, u8 cursorShadowPal )
{
	struct ListMenu* list = (void*)gTasks[listTaskId].data;
...
void ListMenuGetScrollAndRow(
	u8 listTaskId, u16* scrollOffset, u16* selectedRow )
{
	struct ListMenu* list = (void*)gTasks[listTaskId].data;
...
{
	struct ListMenu* list = (void*)gTasks[listTaskId].data;
	u8 yMultiplier        = GetFontAttribute( list->template.fontId,
	...
	for( i = 0; i < count; i++ )
	{
		...
		if( list->template.itemPrintFunc != NULL )
			list->template.itemPrintFunc( list->template.windowId,
				list->template.items[startIndex].id,
				y );

		ListMenuPrint(
			list, list->template.items[startIndex].name, x, y );
		startIndex++;
	}
}
...
static void ListMenuDrawCursor( struct ListMenu* list )
{
	...
	case 2:
		...
		ListMenuUpdateCursorObject( list->taskId,
			GetWindowAttribute( list->template.windowId,
				WINDOW_TILEMAP_LEFT ) *
					8 -
				1,
			GetWindowAttribute(
				list->template.windowId, WINDOW_TILEMAP_TOP ) *
					8 +
				y - 1,
			0 );
		break;
...
static u8 ListMenuAddCursorObject( struct ListMenu* list, u32 cursorKind )
{
	...
	cursor.top  = 160;
	cursor.rowWidth =
		GetWindowAttribute( list->template.windowId, WINDOW_WIDTH ) *
			8 +
		2;
	cursor.rowHeight = GetFontAttribute( list->template.fontId,
				   FONTATTR_MAX_LETTER_HEIGHT ) +
		2;
...
static void ListMenuErasePrintedCursor(
	struct ListMenu* list, u16 selectedRow )
{
	...
	if( cursorKind == 0 )
	{
		u8 yMultiplier = GetFontAttribute( list->template.fontId,
					 FONTATTR_MAX_LETTER_HEIGHT ) +
			list->template.itemVerticalPadding;
		u8 width = GetMenuCursorDimensionByFont(
			list->template.fontId, 0 );
		u8 height = GetMenuCursorDimensionByFont(
			list->template.fontId, 1 );
...
static u8 ListMenuUpdateSelectedRowIndexAndScrollOffset(
	struct ListMenu* list, bool8 movingDown )
{
	...
	else
	{
		if( list->template.maxShowed == 1 )
			...
			newRow = ( ( list->template.maxShowed / 2 ) +
				( list->template.maxShowed % 2 ) );
	...
	else
	{
		...
		if( !movingDown )
		{
			...
			ScrollWindow( list->template.windowId,
				...
				PIXEL_FILL( list->template.fillValue ) );
			ListMenuPrintEntries(
				list, list->scrollOffset, 0, count );

			y = ( list->template.maxShowed * yMultiplier ) +
				list->template.upText_Y;
			width = GetWindowAttribute( list->template.windowId,
					WINDOW_WIDTH ) *
				8;
			height = ( GetWindowAttribute( list->template.windowId,
					   WINDOW_HEIGHT ) *
					 8 ) -
				y;
			FillWindowPixelRect( list->template.windowId,
	...
	bool8 movingDown )
{
	...
	for( i = 0; i < count; i++ )
	{
		do
		{
			u8 ret = ListMenuUpdateSelectedRowIndexAndScrollOffset(
				list, movingDown );
			selectionChange |= ret;
			if( ret != 2 )
				break;
...
// unused
void ListMenuOverrideSetColors(
	u8 cursorPal, u8 fillValue, u8 cursorShadowPal )
{
...
void ListMenuDefaultCursorMoveFunc(
...
static u8 AddScrollIndicatorArrowObject(
	u8 arrowDir, u8 x, u8 y, u16 tileTag, u16 palTag )
{
	u8 spriteId;
	...
	gSprites[spriteId].tAnimNum =
		sScrollIndicatorTemplates[arrowDir].animNum;
	gSprites[spriteId].tBounceDir =
		sScrollIndicatorTemplates[arrowDir].bounceDir;
	gSprites[spriteId].tMultiplier =
		sScrollIndicatorTemplates[arrowDir].multiplier;
	gSprites[spriteId].tFrequency =
		sScrollIndicatorTemplates[arrowDir].frequency;
...
u8 AddScrollIndicatorArrowPair(
	const struct ScrollArrowsTemplate* arrowInfo, u16* scrollOffset )
{
	...
	{
		LoadPalette( sRedArrowPal,
			( 16 * arrowInfo->palNum ) + 0x100,
			0x20 );
	}
	else
	{
	...
	data->topSpriteId =
		AddScrollIndicatorArrowObject( arrowInfo->firstArrowType,
			arrowInfo->firstX,
			arrowInfo->firstY,
			arrowInfo->tileTag,
			arrowInfo->palTag );
	data->bottomSpriteId =
		AddScrollIndicatorArrowObject( arrowInfo->secondArrowType,
			arrowInfo->secondX,
			arrowInfo->secondY,
			arrowInfo->tileTag,
			arrowInfo->palTag );
	...
	if( arrowInfo->palTag == SPRITE_INVALID_TAG )
	{
		...
		gSprites[data->bottomSpriteId].oam.paletteNum =
			arrowInfo->palNum;
	}
	...
	u16* scrollOffset )
{
	...
	return AddScrollIndicatorArrowPair(
		&gTempScrollArrowTemplate, scrollOffset );
}
...
static u8 ListMenuAddRedOutlineCursorObject( struct CursorStruct* cursor )
{
	...
	{
		LoadPalette(
			sRedArrowPal, ( 16 * cursor->palNum ) + 0x100, 0x20 );
	}
	else
	{
		spritePal.data = sRedArrowPal;
	...
	data->spriteId = CreateSprite(
		&spriteTemplate, cursor->left + 120, cursor->top + 120, 0 );
...
static u8 ListMenuAddRedArrowCursorObject( struct CursorStruct* cursor )
{
	...
	data->spriteId =
		CreateSprite( &spriteTemplate, cursor->left, cursor->top, 0 );
