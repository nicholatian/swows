...
static void ( *const sBattlePikeFunctions[] )( void ) = {SetRoomType,
	GetBattlePikeData,
	SetBattlePikeData,
	GetInFinalRoom,
...
static void GetBattlePikeData( void )
{
	...
	case 1:
		gSpecialVar_Result =
			gSaveBlock2Ptr->frontier.pikeWinStreaks
	...
	case 3:
		gSpecialVar_Result =
			gSaveBlock2Ptr->frontier.pikeTotalStreaks
				[gSaveBlock2Ptr->frontier.lvlMode];
		break;
	case 4:
...
static void sub_81A7400( void ) { sUnknown_0300128E = TRUE; }

static void sub_81A740C( void ) { sUnknown_0300128E = FALSE; }
...
static bool8 DoesTypePreventStatus( u16 species, u32 status )
{
	...
	case STATUS1_TOXIC_POISON:
		if( gBaseStats[species].type1 == TYPE_STEEL ||
			gBaseStats[species].type1 == TYPE_POISON ||
			gBaseStats[species].type2 == TYPE_STEEL ||
			gBaseStats[species].type2 == TYPE_POISON )
			ret = TRUE;
		break;
	case STATUS1_FREEZE:
		if( gBaseStats[species].type1 == TYPE_ICE ||
	...
	case STATUS1_PARALYSIS:
		...
		break;
	case STATUS1_BURN:
		if( gBaseStats[species].type1 == TYPE_FIRE ||
...
static bool8 TryInflictRandomStatus( void )
{
	...
	do
	{
		...
		if( status != sStatusFlags )
		{
			...
			for( i = 0; i < 3; i++ )
			{
				...
				if( pokemon_ailments_get_primary( GetMonData(
					    mon, MON_DATA_STATUS ) ) == 0 &&
	...
	{
		mon = &gPlayerParty[indices[i]];
		if( pokemon_ailments_get_primary(
			...
			GetMonData( mon, MON_DATA_HP ) != 0 )
		{
			...
			if( !DoesAbilityPreventStatus( mon, sStatusFlags ) &&
				!DoesTypePreventStatus(
					species, sStatusFlags ) )
				SetMonData(
					mon, MON_DATA_STATUS, &sStatusFlags );
		}
		if( j == count )
			break;
	}
...
static u8 GetNextRoomType( void )
{
	...
	// Check if the player walked into the same room that the lady gave a
	// hint about.
	...
	// The room types associated with the lady's hint cannot be in the
	// other two rooms.
	roomTypeGroup =
	...
	// Remove room type candidates that would have no effect on the
	// player's party.
	if( roomTypesAvailability[PIKE_ROOM_DOUBLE_BATTLE] != 1 &&
		!AtLeastTwoAliveMons( ) )
	{
		roomTypesAvailability[PIKE_ROOM_DOUBLE_BATTLE] = 1;
		numRoomCandidates--;
	}
	if( roomTypesAvailability[PIKE_ROOM_STATUS] != 1 &&
...
static u8 sub_81A7B84( void ) { return sUnknown_0300128E; }
...
bool32 TryGenerateBattlePikeWildMon( bool8 checkKeenEyeIntimidate )
{
	...
	if( checkKeenEyeIntimidate == TRUE &&
		!CanEncounterWildMon( monLevel ) )
		return FALSE;

	SetMonData( &gEnemyParty[0],
		MON_DATA_EXP,
		&gExperienceTables
			[gBaseStats[wildMons[headerId][pikeMonId].species]
					.growthRate][monLevel] );
	...
	for( i = 0; i < MAX_MON_MOVES; i++ )
		SetMonMoveSlot( &gEnemyParty[0],
			wildMons[headerId][pikeMonId].moves[i],
			i );
...
{
	while( sStatusInflictionScreenFadeFuncs[gTasks[taskId].data[0]](
		&gTasks[taskId] ) )
		;
}
...
static void StartStatusInflictionScreenFade( s16 fadeOutDelay,
	s16 fadeInDelay,
	s16 numFades,
	s16 fadeOutSpeed,
	s16 fadeInSpped )
...
static void TryHealMons( u8 healCount )
{
	...
	for( i = 0; i < 3; i++ )
	{
		...
		else if( pokemon_ailments_get_primary(
				 GetMonData( mon, MON_DATA_STATUS ) ) != 0 )
		{
			canBeHealed = TRUE;
		}
...
bool8 InBattlePike( void )
{
	return gMapHeader.mapLayoutId ==
		...
		LAYOUT_BATTLE_FRONTIER_BATTLE_PIKE_RANDOM_ROOM1 ||
		gMapHeader.mapLayoutId ==
		LAYOUT_BATTLE_FRONTIER_BATTLE_PIKE_RANDOM_ROOM3 ||
...
static void SetHintedRoom( void )
{
	...
	else
	{
		...
		for( i = 0, id = 0; i < count; i++ )
		{
			if( gSaveBlock2Ptr->frontier.pikeHealingRoomsDisabled )
			{
				if( i != PIKE_ROOM_HEAL_FULL &&
					i != PIKE_ROOM_HEAL_PART )
					roomCandidates[id++] = i;
			}
			else
			{
...
static void PrepareOneTrainer( bool8 difficult )
{
	...
	do
	{
		...
		for( i = 0;
			i < gSaveBlock2Ptr->frontier.curChallengeBattleNum - 1;
			i++ )
		{
...
static void BufferRecordMixingTrainerMessage( void )
{
	if( gSpecialVar_0x8005 == 0 )
	{
		if( gTrainerBattleOpponent_A < TRAINER_RECORD_MIXING_FRIEND )
			FrontierSpeechToString(
				gFacilityTrainers[gTrainerBattleOpponent_A]
					.speechBefore );
	}
	else if( gSpecialVar_0x8005 == 1 )
	{
		if( gTrainerBattleOpponent_B < TRAINER_RECORD_MIXING_FRIEND )
			FrontierSpeechToString(
				gFacilityTrainers[gTrainerBattleOpponent_B]
					.speechBefore );
	}
}
...
static u8 GetPikeQueenFightType( u8 nextRoom )
{
	...
	winStreak += nextRoom;
	numPikeSymbols =
		GetPlayerSymbolCountForFacility( FRONTIER_FACILITY_PIKE );
	...
	case 1:
		if( winStreak ==
			sPikeQueenWinStreakAppearances[var][numPikeSymbols] -
				sPikeQueenWinStreakAppearances[var][3] )
			ret = numPikeSymbols + 1;
		break;
	case 2:
	default:
		if( winStreak ==
			sPikeQueenWinStreakAppearances[var][0] -
				sPikeQueenWinStreakAppearances[var][3] )
			ret = 3;
		else if( winStreak ==
				sPikeQueenWinStreakAppearances[var][1] -
					sPikeQueenWinStreakAppearances[var]
								      [3] ||
...
{
	u8 toHealCount =
		sNumMonsToHealBeforePikeQueen[gSaveBlock2Ptr->frontier
						      .pikeHintedRoomIndex]
					     [gSpecialVar_0x8007];
...
static void RestoreMonHeldItems( void )
{
	...
	for( i = 0; i < 3; i++ )
	{
		SetMonData( &gPlayerParty[gSaveBlock2Ptr->frontier
						  .selectedPartyMons[i] -
				    1],
			MON_DATA_HELD_ITEM,
...
static bool8 CanEncounterWildMon( u8 enemyMonLevel )
{
	if( !GetMonData( &gPlayerParty[0], MON_DATA_SANITY_IS_EGG ) )
	{
		...
		if( monAbility == ABILITY_KEEN_EYE ||
			monAbility == ABILITY_INTIMIDATE )
		{
			u8 playerMonLevel =
				GetMonData( &gPlayerParty[0], MON_DATA_LEVEL );
			if( playerMonLevel > 5 &&
				enemyMonLevel <= playerMonLevel - 5 &&
				Random( ) % 2 == 0 )
				return FALSE;
		}
	}
