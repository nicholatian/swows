...
static const u16 sUnknown_085979C0[] =
	INCBIN_U16( "graphics/misc/interview_frame.gbapal" );
static const u32 sUnknown_085979E0[] =
	INCBIN_U32( "graphics/misc/interview_frame.4bpp.lz" );
static const u16 sUnknown_08597B14[] =
	INCBIN_U16( "graphics/misc/interview_frame_orange.gbapal" );
static const u16 sUnknown_08597B34[] =
	INCBIN_U16( "graphics/misc/interview_frame_green.gbapal" );
static const u32 sUnknown_08597B54[] =
	INCBIN_U32( "graphics/misc/interview_frame_2.4bpp.lz" );
static const u16 sUnknown_08597C1C[] =
	INCBIN_U16( "graphics/misc/8597C1C.gbapal" );
static const u16 sUnknown_08597C24[] =
	INCBIN_U16( "graphics/misc/8597C24.gbapal" );
...
void DoEasyChatScreen(
	u8 type, u16* words, MainCallback callback, u8 displayedPersonType )
{
	u8 taskId;
...
static void sub_811A2FC( u8 taskId )
{
	...
	case 4:
		if( !gPaletteFade.active )
			sub_811A4D0( (MainCallback)GetWordTaskArg(
				taskId, EZCHAT_TASK_MAINCALLBACK ) );
		break;
	case 5:
		if( !gPaletteFade.active )
			data[EZCHAT_TASK_STATE] = 1;
...
void ShowEasyChatScreen( void )
{
	...
	DoEasyChatScreen( gSpecialVar_0x8004,
		words,
...
void sub_811A858( void ) { SetMainCallback2( sub_811A7E4 ); }
...
static bool8 EasyChat_AllocateResources(
	u8 type, u16* words, u8 displayedPersonType )
{
	u8 templateId;
	int i;
	...
	else
	{
		...
		sEasyChatScreen->titleText =
			sEasyChatScreenTemplates[templateId].titleText;
	}

	sEasyChatScreen->numColumns =
		sEasyChatScreenTemplates[templateId].numColumns;
	sEasyChatScreen->numRows =
		sEasyChatScreenTemplates[templateId].numRows;
	sEasyChatScreen->unk_07 =
	...
	{
		CpuCopy16( words,
			sEasyChatScreen->ecWordBuffer,
			sEasyChatScreen->unk_07 * sizeof( u16 ) );
	}
	else
	{
...
static u16 sub_811AB68( void )
{
	...
	if( sEasyChatScreen->mainCursorRow < 0 )
		sEasyChatScreen->mainCursorRow =
			sEasyChatScreenTemplates[sEasyChatScreen->templateId]
				.numRows;
...
static u8 sub_811B2A4( void ) { return sEasyChatScreen->stateBackup; }
...
static u8 sub_811B4AC( void )
{
	...
	for( i = 0; i < sEasyChatScreen->unk_07; i++ )
	{
		if( sEasyChatScreen->ecWordBuffer[i] !=
			sEasyChatScreen->words[i] )
			return 1;
	}
...
static void sub_811B768( void )
{
	...
	{
		sEasyChatScreen->unk_0a =
			sub_811B960( sEasyChatScreen->unk_0b );
	}
}
...
static u16 sub_811B794( u32 arg0 )
{
	...
	case 2:
		...
		break;
	case 3:
		if( sEasyChatScreen->unk_11 + sEasyChatScreen->unk_0e <
			sEasyChatScreen->unk_0f )
		{
	...
	case 5:
		if( sEasyChatScreen->unk_0e <= sEasyChatScreen->unk_0f - 4 )
		{
			sEasyChatScreen->unk_0e += 4;
			if( sEasyChatScreen->unk_0e >
				sEasyChatScreen->unk_0f - 3 )
				sEasyChatScreen->unk_0e =
					sEasyChatScreen->unk_0f + 0xFD;
...
static u16 sub_811B8E8( void )
{
	return 2 * ( sEasyChatScreen->unk_0b + sEasyChatScreen->unk_0c ) +
		sEasyChatScreen->unk_0a;
}
...
{
	int var0 =
		(u8)sEasyChatScreen->unk_0a < 7 ? sEasyChatScreen->unk_0a : 0;
	int var1 =
		(u8)sEasyChatScreen->unk_0b < 4 ? sEasyChatScreen->unk_0b : 0;
	return sUnknown_08597748[var1][var0];
}

static u16 sub_811B940( void )
{
	return 2 * ( sEasyChatScreen->unk_11 + sEasyChatScreen->unk_0e ) +
		sEasyChatScreen->unk_10;
}
...
{
	*str1 = sEasyChatScreenTemplates[sEasyChatScreen->templateId]
			.instructionsText1;
	*str2 = sEasyChatScreenTemplates[sEasyChatScreen->templateId]
			.instructionsText2;
}
...
u8 sub_811BBB0( void ) { return sEasyChatScreen->unk_09; }

u8 sub_811BBBC( void ) { return sEasyChatScreen->unk_0c; }
...
u8 sub_811BBDC( void ) { return sEasyChatScreen->unk_0e; }

u8 sub_811BBE8( void ) { return sEasyChatScreen->unk_0f; }

static u8 unref_sub_811BBF4( void ) { return 0; }
...
static void sub_811BE9C( void )
{
	...
	case EASY_CHAT_TYPE_TRENDY_PHRASE:
		sub_811BE54( );
		gSpecialVar_0x8004 =
			sub_81226D8( sEasyChatScreen->ecWordBuffer );
		break;
	case EASY_CHAT_TYPE_GOOD_SAYING:
...
static int sub_811BF20( void )
{
	return sub_811BC7C( sMysteryGiftPhrase,
		       ARRAY_COUNT( sMysteryGiftPhrase ) ) == 0;
}
...
static u16 sub_811BF40( void )
{
	...
	for( i = 0; i < 5; i++ )
	{
		if( !sub_811BC7C( sBerryMasterWifePhrases[i],
			    ARRAY_COUNT( *sBerryMasterWifePhrases ) ) )
			return i + 1;
	}
...
static void sub_811BF78( void ) { sEasyChatScreen->unk_13 = 0; }

static int sub_811BF88( int easyChatWord ) { return 0; }
	...
	void ( *callback )( struct TextPrinterTemplate*, u16 ) )
{
	AddTextPrinterParameterized(
		windowId, fontId, str, x, y, speed, callback );
}

static void sub_811D058( u8 windowId,
	u8 fontId,
	const u8* str,
	u8 left,
	u8 top,
	...
	u8 blue )
{
	...
	AddTextPrinterParameterized3(
		windowId, fontId, left, top, color, speed, str );
}
...
{
	CreateYesNoMenu(
		&sEasyChatYesNoWindowTemplate, 1, 14, initialCursorPos );
}
	...
	for( i = 0; i < numRows; i++ )
	{
		...
		for( j = 0; j < numColumns; j++ )
		{
			...
			else
			{
				...
				if( !var1 )
				{
					str = WriteColorChangeControlCode(
						str, 0, 4 );
					for( k = 0; k < 12; k++ )
					{
						*str = CHAR_HYPHEN;
		...
		*str = EOS;
		sub_811D028( sUnknown_0203A11C->windowId,
			1,
			sUnknown_0203A11C->unkB,
			0,
			i * 16 + 1,
			0xFF,
			0 );
...
static void sub_811D794( void ) { sub_811D864( 0, 4 ); }
...
static void sub_811D9CC( int arg0 )
{
	...
	sUnknown_0203A11C->unk8 =
		sUnknown_0203A11C->unk6 < sUnknown_0203A11C->unk7 ? 1 : -1;
}
...
static int sub_811DE48( void ) { return sUnknown_0203A11C->unk2CE; }
...
static void sub_811DFB0( void ) { sUnknown_0203A11C->unk2D8->data[1] = 1; }
...
static void sub_811E1A4( s8 arg0, s8 arg1 )
{
	...
	if( arg0 != -1 )
	{
		...
		{
			x += sUnknown_08597D08
				[arg0 < ARRAY_COUNT( sUnknown_08597D08 ) ? arg0
									 : 0];
			anim = 3;
		}
...
static bool8 sub_811E418( void )
{
	...
	case 0:
		...
		if( sUnknown_0203A11C->unk2E8->pos2.x >= 0 )
		{
			...
			if( !sub_811BBB0( ) )
				StartSpriteAnim(
					sUnknown_0203A11C->unk2EC, 1 );
			else
				StartSpriteAnim(
					sUnknown_0203A11C->unk2EC, 2 );
...
static void sub_811E828( void )
{
	...
	spriteId = AddPseudoEventObject(
		graphicsId, SpriteCallbackDummy, 76, 40, 0 );
	if( spriteId != MAX_SPRITES )
	{
	...
	spriteId = AddPseudoEventObject( gSaveBlock2Ptr->playerGender == MALE
...
u8* CopyEasyChatWord( u8* dest, u16 easyChatWord )
{
	...
	else if( easyChatWord != 0xFFFF )
	{
		...
		u8 groupId = EC_GROUP( easyChatWord );
		resultStr =
			StringCopy( dest, GetEasyChatWord( groupId, index ) );
	}
	else
	{
		*dest     = EOS;
...
u8* ConvertEasyChatWordsToString(
	u8* dest, const u16* src, u16 columns, u16 rows )
{
	u16 i, j;
...
void sub_811EECC( void )
{
	...
	case 1:
		...
		if( sub_811EDC4(
			    gSaveBlock1Ptr->easyChatBattleStart, 3, 2, 18 ) )
		{
			columns = 2;
			rows    = 3;
		}
	...
	ConvertEasyChatWordsToString(
		gStringVar4, easyChatWords, columns, rows );
...
static bool8 sub_811F0F8( void ) { return IsNationalPokedexEnabled( ); }
...
void InitEasyChatPhrases( void )
{
	...
	// BUG: This is supposed to clear 64 bits, but this loop is clearing 64
	// bytes. However, this bug has no resulting effect on gameplay because
	// only the Mauville old man data is corrupted, which is initialized
	// directly after this function is called when starting a new game.
	for( i = 0; i < 64; i++ )
...
static void sub_811F2D4( void )
{
	...
	if( GetNationalPokedexCount( FLAG_GET_SEEN ) )
		sUnknown_0203A120->unk2[sUnknown_0203A120->unk0++] =
			EC_GROUP_POKEMON;
...
static u8 sub_811F3AC( void ) { return sUnknown_0203A120->unk0; }
	...
	for( i = 0; i < 27; i++ )
	{
		...
		for( j = 0; j < numWords; j++ )
		{
			...
			for( k = 0; k < numToProcess; k++ )
			{
				...
				{
					sUnknown_0203A120->unk64[i][index++] =
						words[k];
...
static u16 sub_811F5B0( void ) { return sUnknown_0203A120->unk3BA0; }
		...
		groupId == EC_GROUP_MOVE_1 || groupId == EC_GROUP_MOVE_2 )
	{
		...
		for( i = 0, totalWords = 0; i < numWords; i++ )
		{
			if( sub_811F764( list[i], groupId ) )
				sUnknown_0203A120->unk3984[totalWords++] =
					EC_WORD( groupId, list[i] );
		}
...
static u16 sub_811F6B8( u16 alphabeticalGroup )
{
	...
	for( i = 0, totalWords = 0;
		i < sUnknown_0203A120->unk2E[alphabeticalGroup];
		i++ )
		sUnknown_0203A120->unk3984[totalWords++] =
			sUnknown_0203A120->unk64[alphabeticalGroup][i];
...
static bool8 sub_811F764( u16 wordIndex, u8 groupId )
{
	...
	case EC_GROUP_POKEMON:
		return GetSetPokedexFlag(
			SpeciesToNationalPokedexNum( wordIndex ),
			FLAG_GET_SEEN );
	case EC_GROUP_POKEMON_2:
	...
	default:
		return gEasyChatGroups[groupId]
