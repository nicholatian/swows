...
static const TaskFunc sPerStepCallbacks[] = {DummyPerStepCallback,
...
static const struct PacifidlogMetatileOffsets
	sHalfSubmergedBridgeMetatileOffsets[] = {{0, 0, 0x259},
		{0, 1, 0x261},
		{0, -1, 0x259},
		{0, 0, 0x261},
		{0, 0, 0x252},
...
static const struct PacifidlogMetatileOffsets
	sFullySubmergedBridgeMetatileOffsets[] = {{0, 0, 0x25A},
		{0, 1, 0x262},
		{0, -1, 0x25A},
		{0, 0, 0x262},
		{0, 0, 0x254},
...
static const struct PacifidlogMetatileOffsets
	sFloatingBridgeMetatileOffsets[] = {{0, 0, 0x258},
		{0, 1, 0x260},
		{0, -1, 0x258},
		{0, 0, 0x260},
		{0, 0, 0x250},
	...
	const struct PacifidlogMetatileOffsets* offsets, u16 metatileBehavior )
{
	if( MetatileBehavior_IsPacifilogVerticalLog1( metatileBehavior ) )
		...
		return &offsets[1 * 2];
	else if( MetatileBehavior_IsPacifilogHorizontalLog1(
			 metatileBehavior ) )
		return &offsets[2 * 2];
	else if( MetatileBehavior_IsPacifilogHorizontalLog2(
			 metatileBehavior ) )
		return &offsets[3 * 2];
	else
		return NULL;
...
static void SetPacifidlogBridgeMetatiles(
	const struct PacifidlogMetatileOffsets* offsets,
	s16 x,
	s16 y,
	bool32 redrawMap )
{
	...
	if( offsets )
	{
		MapGridSetMetatileIdAt( x + offsets[0].x,
			y + offsets[0].y,
			offsets[0].tileId );
		if( redrawMap )
		...
		MapGridSetMetatileIdAt( x + offsets[1].x,
			y + offsets[1].y,
			offsets[1].tileId );
		if( redrawMap )
			CurrentMapDrawMetatileAt(
				x + offsets[1].x, y + offsets[1].y );
	}
}

static void UpdateHalfSubmergedBridgeMetatiles(
	s16 x, s16 y, bool32 redrawMap )
{
	SetPacifidlogBridgeMetatiles(
		sHalfSubmergedBridgeMetatileOffsets, x, y, redrawMap );
}

static void UpdateFullySubmergedBridgeMetatiles(
	s16 x, s16 y, bool32 redrawMap )
{
	SetPacifidlogBridgeMetatiles(
		sFullySubmergedBridgeMetatileOffsets, x, y, redrawMap );
}

static void UpdateFloatingBridgeMetatiles( s16 x, s16 y, bool32 redrawMap )
{
	SetPacifidlogBridgeMetatiles(
		sFloatingBridgeMetatileOffsets, x, y, redrawMap );
}
...
static void PacifidlogBridgePerStepCallback( u8 taskId )
{
	...
	case 1:
		...
		{
			if( StandingOnNewPacifidlogBridge(
				    x, y, data[2], data[3] ) )
			{
			...
			if( StandingOnSamePacifidlogBridge(
				    x, y, data[2], data[3] ) )
			{
				UpdateHalfSubmergedBridgeMetatiles(
					x, y, TRUE );
...
static void FortreeBridgePerStepCallback( u8 taskId )
{
	...
	case 1:
		...
		isFortreeBridgePrev = MetatileBehavior_IsFortreeBridge(
			MapGridGetMetatileBehaviorAt( x2, y2 ) );
		z    = PlayerGetZCoord( );
		flag = 0;
		if( ( u8 )( z & 1 ) == 0 )
			flag = 1;
		...
		if( flag &&
			( isFortreeBridgeCur == 1 ||
				isFortreeBridgePrev == 1 ) )
			PlaySE( SE_HASHI );
...
static void MarkIcePuzzleCoordVisited( s16 x, s16 y )
{
	if( CoordInIcePuzzleRegion( x, y ) )
		*GetVarPointer( sSootopolisGymIceRowVars[y] ) |=
			( 1 << ( x - 3 ) );
}
...
static void SootopolisGymIcePerStepCallback( u8 taskId )
{
	...
	case 1:
		...
		if( x != data[2] || y != data[3] )
		{
			...
			else if( MetatileBehavior_IsCrackedIce(
					 tileBehavior ) == TRUE )
			{
				*iceStepCount = 0;
				data[6]       = 4;
...
static void AshGrassPerStepCallback( u8 taskId )
{
	...
	if( x != data[1] || y != data[2] )
	{
			    ...
			    MapGridGetMetatileBehaviorAt( x, y ) ) )
		{
			...
			{
				ashGatherCount =
					GetVarPointer( VAR_ASH_GATHER_COUNT );
...
static void SetCrackedFloorHoleMetatile( s16 x, s16 y )
{
	MapGridSetMetatileIdAt( x,
		y,
		MapGridGetMetatileIdAt( x, y ) == 0x22f ? 0x206 : 0x237 );
	CurrentMapDrawMetatileAt( x, y );
}
...
static void CrackedFloorPerStepCallback( u8 taskId )
{
	...
	if( ( x != data[2] || y != data[3] ) )
	{
		...
		{
			if( GetPlayerSpeed( ) != 4 )
				VarSet( VAR_ICE_STEP_COUNT,
					0 ); // this var does double duty
