...
static void SetEReaderTrainerChecksum(
...
const u8 gTowerMaleTrainerGfxIds[30] = {EVENT_OBJ_GFX_HIKER,
...
const u8 gTowerFemaleTrainerGfxIds[20] = {EVENT_OBJ_GFX_WOMAN_2,
...
} const gUnknown_085DD500[] = {{FACILITY_CLASS_LASS, gUnknown_085DD118},
...
static const u8* const* const gUnknown_085DD690[] = {gUnknown_085DCFD8,
	gUnknown_085DCFEC,
	gUnknown_085DD000,
	gUnknown_085DD014,
	gUnknown_085DD028,
...
struct
{
	...
	u8 evs[6];
	u16 moves[4];
} const sStevenMons[3] = {{.species        = SPECIES_METANG,
				  .fixedIV = 31,
				  .level   = 42,
				  .nature  = NATURE_BRAVE,
				  .evs     = {0, 252, 252, 0, 6, 0},
...
// code
void sub_8161F74( void ) { gUnknown_085DF96C[gSpecialVar_0x8004]( ); }
...
static void sub_8161F94( void )
{
	...
	if( !( gSaveBlock2Ptr->frontier.field_CDC &
		    gUnknown_085DF9AC[battleMode][lvlMode] ) )
...
static void sub_8162054( void )
{
	...
	case 1:
		gSpecialVar_Result =
			GetCurrentBattleTowerWinStreak( lvlMode, battleMode );
		break;
	case 2:
		gSpecialVar_Result =
		...
		break;
	case 3:
		gSaveBlock2Ptr->frontier.field_D07 =
...
static void sub_81620F4( void )
{
	...
	case 2:
		if( gSpecialVar_0x8006 )
			...
			gSaveBlock2Ptr->frontier.field_CDC &=
				gUnknown_085DF9CC[battleMode][lvlMode];
		break;
	case 3:
		gSaveBlock2Ptr->frontier.field_D07 =
			gSaveBlock2Ptr->frontier.lvlMode;
...
static bool8 ChooseSpecialBattleTowerTrainer( void )
{
	...
	{
		u32* record =
			(u32*)( &gSaveBlock2Ptr->frontier.towerRecords[i] );
		u32 recordHasData = 0;
		u32 checksum      = 0;
		for( j = 0; j <
			( sizeof( struct EmeraldBattleTowerRecord ) - 4 ) / 4;
			j++ ) // - 4, because of the last field being the
			      // checksum itself.
		{
		...
		for( j = 0; j < 4; j++ )
		{
			if( gSaveBlock2Ptr->frontier.towerRecords[i]
						.party[j]
						.species != 0 &&
				gSaveBlock2Ptr->frontier.towerRecords[i]
						.party[j]
						.level <=
		...
		if( validMons >= gUnknown_085DF9F6[battleMode] &&
			gSaveBlock2Ptr->frontier.towerRecords[i].winStreak ==
				winStreak &&
			gSaveBlock2Ptr->frontier.towerRecords[i].lvlMode ==
				lvlMode &&
			recordHasData &&
			gSaveBlock2Ptr->frontier.towerRecords[i].checksum ==
				checksum )
		{
			trainerIds[idsCount] =
				i + TRAINER_RECORD_MIXING_FRIEND;
			idsCount++;
		}
	}
	...
	if( battleMode == FRONTIER_MODE_SINGLES )
	{
		...
		for( i = 0; i < 4; i++ )
		{
			if( gSaveBlock2Ptr->apprentices[i].lvlMode != 0 &&
				gUnknown_085DF9EC[gSaveBlock2Ptr
							  ->apprentices[i]
							  .field_1] ==
					winStreak &&
				gSaveBlock2Ptr->apprentices[i].lvlMode - 1 ==
					lvlMode )
			{
				trainerIds[idsCount] =
					i + TRAINER_RECORD_MIXING_APPRENTICE;
				idsCount++;
			}
		}
	}
	...
	else
	{
			...
			battleMode == FRONTIER_MODE_LINK_MULTIS )
		{
			...
			gTrainerBattleOpponent_A =
				gSaveBlock2Ptr->frontier.trainerIds[id * 2];
		...
		else if( ChooseSpecialBattleTowerTrainer( ) )
		{
			SetBattleFacilityTrainerGfxId(
				gTrainerBattleOpponent_A, 0 );
			gSaveBlock2Ptr->frontier
				.trainerIds[gSaveBlock2Ptr->frontier
		...
		else
		{
			...
			while( 1 )
			{
				id = sub_8162548( challengeNum,
					gSaveBlock2Ptr->frontier
						.curChallengeBattleNum );

				// Ensure trainer wasn't previously fought in
				// this challenge.
				for( i = 0; i < gSaveBlock2Ptr->frontier
							.curChallengeBattleNum;
					i++ )
				{
...
u16 sub_8162548( u8 challengeNum, u8 battleNum )
{
	...
	if( challengeNum <= 7 )
	{
		...
		{
			trainerId =
				( gUnknown_085DFA1A[challengeNum][1] -
					gUnknown_085DFA1A[challengeNum][0] ) +
				1;
		...
		{
			trainerId =
				( gUnknown_085DF9FA[challengeNum][1] -
					gUnknown_085DF9FA[challengeNum][0] ) +
				1;
	...
	{
		trainerId =
			( gUnknown_085DF9FA[7][1] - gUnknown_085DF9FA[7][0] ) +
			1;
		trainerId =
			gUnknown_085DF9FA[7][0] + ( Random( ) % trainerId );
	}

	return trainerId;
}

static void sub_81625B4( u8 challengeNum,
	u8 battleNum,
	u16* trainerIdPtr,
	u8* arg3 ) // Unused
{
	...
	if( challengeNum <= 7 )
	{
		if( battleNum == 6 )
		{
			count = ( gUnknown_085DFA1A[challengeNum][1] -
					gUnknown_085DFA1A[challengeNum][0] ) +
				1;
			trainerId = gUnknown_085DFA1A[challengeNum][0];
		}
		...
		{
			count = ( gUnknown_085DF9FA[challengeNum][1] -
					gUnknown_085DF9FA[challengeNum][0] ) +
				1;
			trainerId = gUnknown_085DF9FA[challengeNum][0];
		}
	...
	{
		count = ( gUnknown_085DF9FA[7][1] - gUnknown_085DF9FA[7][0] ) +
			1;
...
void PutNewBattleTowerRecord( struct EmeraldBattleTowerRecord* newRecordEm )
{
	...
	s32 i, j, k;
	s32 slotsCount                             = 0;
	struct EmeraldBattleTowerRecord* newRecord = newRecordEm; // Needed to
								  // match.
	...
	for( i = 0; i < 5; i++ )
	{
		...
		if( j == 4 )
		{
			...
			{
				// BUG: Wrong variable used, 'j' instead of
				// 'k'.
				if( gSaveBlock2Ptr->frontier.towerRecords[i]
						.name[j] !=
					newRecord->name[j] )
	...
	for( i = 1; i < 5; i++ )
	{
		...
		{
			if( gSaveBlock2Ptr->frontier.towerRecords[i]
					.winStreak < slotValues[j] )
			{
				j             = 0;
				slotsCount    = 1;
				slotValues[0] = gSaveBlock2Ptr->frontier
							.towerRecords[i]
							.winStreak;
				slotIds[0] = i;
				break;
			}
			else if( gSaveBlock2Ptr->frontier.towerRecords[i]
					 .winStreak > slotValues[j] )
			{
				break;
			}
...
u8 GetFrontierTrainerFrontSpriteId( u16 trainerId )
{
	...
	if( trainerId == TRAINER_EREADER )
	{
		return gFacilityClassToPicIndex[gSaveBlock2Ptr->frontier
							.ereaderTrainer
							.facilityClass];
	}
	else if( trainerId == TRAINER_FRONTIER_BRAIN )
	...
	{
		return gFacilityClassToPicIndex[gFacilityTrainers[trainerId]
							.facilityClass];
	}
	else if( trainerId < TRAINER_RECORD_MIXING_APPRENTICE )
	{
	...
	else
	{
		if( gBattleTypeFlags & BATTLE_TYPE_RECORDED )
			return gFacilityClassToPicIndex
				[gApprentices[GetRecordedBattleApprenticeId( )]
						.facilityClass];
		else
			return gFacilityClassToPicIndex
				[gApprentices[gSaveBlock2Ptr
...
u8 GetFrontierOpponentClass( u16 trainerId )
{
	...
	{
		trainerClass =
			gFacilityClassToTrainerClass[gSaveBlock2Ptr->frontier
							     .ereaderTrainer
							     .facilityClass];
	}
	else if( trainerId == TRAINER_FRONTIER_BRAIN )
...
void GetFrontierTrainerName( u8* dst, u16 trainerId )
{
	...
	if( trainerId == TRAINER_EREADER )
	{
		for( i = 0; i < PLAYER_NAME_LENGTH; i++ )
			dst[i] = gSaveBlock2Ptr->frontier.ereaderTrainer
					 .name[i];
	}
	else if( trainerId == TRAINER_FRONTIER_BRAIN )
	{
	...
	else if( trainerId < TRAINER_RECORD_MIXING_APPRENTICE )
	{
		...
		else
		{
			...
			TVShowConvertInternationalString(
				dst, record->name, record->language );
			return;
		}
	}
	else
	{
		...
		TVShowConvertInternationalString( dst,
			GetApprenticeNameInLanguage( id, language ),
			language );
		return;
	}
...
static void FillTrainerParty( u16 trainerId, u8 firstMonId, u8 monCount )
{
	...
	{
		// Record mixed player.
		for( j = 0, i = firstMonId; i < firstMonId + monCount;
			j++, i++ )
		{
			if( gSaveBlock2Ptr->frontier
						.towerRecords[trainerId -
	...
	// Attempt to fill the trainer's party with random Pokemon until 3 have
	// been successfully chosen. The trainer's party may not have duplicate
	// pokemon species or duplicate held items.
	...
	while( i != monCount )
	{
		...
		for( j = 0; j < i + firstMonId; j++ )
		{
			if( GetMonData( &gEnemyParty[j],
				    MON_DATA_HELD_ITEM,
				    NULL ) != 0 &&
				GetMonData( &gEnemyParty[j],
					MON_DATA_HELD_ITEM,
					NULL ) ==
					gBattleFrontierHeldItems
						[gFacilityTrainerMons[monSetId]
								.itemTableId] )
				break;
		}
		if( j != i + firstMonId )
		...
		// Ensure this exact pokemon index isn't a duplicate. This
		// check doesn't seem necessary because the species and held
		// items were already checked directly above.
		for( j = 0; j < i; j++ )
		{
		...
		for( j = 0; j < MAX_MON_MOVES; j++ )
		{
			SetMonMoveSlot( &gEnemyParty[i + firstMonId],
				...
				j );
			if( gFacilityTrainerMons[monSetId].moves[j] ==
				MOVE_FRUSTRATION )
				friendship = 0; // Frustration is more powerful
						// the lower the pokemon's
						// friendship is.
		}
		...
		// The pokemon was successfully added to the trainer's party,
		// so it's safe to move on to the next party slot.
		i++;
	}
}
...
static void FillFactoryFrontierTrainerParty( u16 trainerId, u8 firstMonId )
{
	...
	{
		u8 lvlMode =
			gSaveBlock2Ptr->frontier.lvlMode; // Unused variable.
		u8 battleMode   = VarGet( VAR_FRONTIER_BATTLE_MODE );
		u8 challengeNum = gSaveBlock2Ptr->frontier
					  .towerWinStreaks[battleMode][0] /
			7;
...
static void sub_8163914( void )
{
	...
	if( trainerId == TRAINER_EREADER )
		FrontierSpeechToString(
			gSaveBlock2Ptr->frontier.ereaderTrainer.greeting );
	else if( trainerId < TRAINER_RECORD_MIXING_FRIEND )
		FrontierSpeechToString(
...
static void HandleSpecialTrainerBattleEnd( void )
{
	...
	case SPECIAL_BATTLE_SECRET_BASE:
		...
		{
			u16 itemBefore =
				GetMonData( &gSaveBlock1Ptr->playerParty[i],
					MON_DATA_HELD_ITEM );
...
void DoSpecialTrainerBattle( void )
{
	...
	case SPECIAL_BATTLE_TOWER:
		gBattleTypeFlags =
			BATTLE_TYPE_TRAINER | BATTLE_TYPE_BATTLE_TOWER;
		...
		case FRONTIER_MODE_MULTIS:
			...
			gPartnerTrainerId =
				gSaveBlock2Ptr->frontier.trainerIds[17];
		...
		case FRONTIER_MODE_LINK_MULTIS:
			gBattleTypeFlags |= BATTLE_TYPE_DOUBLE |
				BATTLE_TYPE_LINK | BATTLE_TYPE_MULTI |
				BATTLE_TYPE_x800000;
			FillFrontierTrainersParties( 2 );
			break;
		}
	...
	case SPECIAL_BATTLE_SECRET_BASE:
		for( i = 0; i < PARTY_SIZE; i++ )
		{
			u16 itemBefore = GetMonData(
				&gPlayerParty[i], MON_DATA_HELD_ITEM );
			SetMonData( &gSaveBlock1Ptr->playerParty[i],
				MON_DATA_HELD_ITEM,
				&itemBefore );
		}
		CreateTask( Task_StartBattleAfterTransition, 1 );
	...
	case SPECIAL_BATTLE_PIKE_DOUBLE:
		gBattleTypeFlags = BATTLE_TYPE_TRAINER |
			BATTLE_TYPE_BATTLE_TOWER | BATTLE_TYPE_DOUBLE |
	...
	case SPECIAL_BATTLE_STEVEN:
		...
		BattleSetup_ConfigureTrainerBattle(
			MossdeepCity_SpaceCenter_2F_EventScript_224157 + 1 );
		gApproachingTrainerId = 1;
		BattleSetup_ConfigureTrainerBattle(
			MossdeepCity_SpaceCenter_2F_EventScript_224166 + 1 );
...
static void sub_8163EE4( void )
{
	...
	{
		class = gTowerFemaleFacilityClasses
	...
	{
		class = gTowerMaleFacilityClasses
			[( gSaveBlock2Ptr->playerTrainerId[0] +
				 gSaveBlock2Ptr->playerTrainerId[1] +
				 gSaveBlock2Ptr->playerTrainerId[2] +
				 gSaveBlock2Ptr->playerTrainerId[3] ) %
	...
	CopyTrainerId(
		playerRecord->trainerId, gSaveBlock2Ptr->playerTrainerId );
	...
	for( i = 0; i < 6; i++ )
	{
		playerRecord->greeting[i] =
			gSaveBlock1Ptr->easyChatBattleStart[i];
		playerRecord->speechWon[i] =
			gSaveBlock1Ptr->easyChatBattleWon[i];
		playerRecord->speechLost[i] =
			gSaveBlock1Ptr->easyChatBattleLost[i];
	}
...
static void SaveBattleTowerProgress( void )
{
	...
	s32 challengeNum =
		(signed)( gSaveBlock2Ptr->frontier
	...
	for( i = 0; i < 3; i++ )
	{
		u16 apprenticeSpecies =
			gSaveBlock2Ptr
				...
				.party[i]
				.species;
		if( apprenticeSpecies != species1 &&
			apprenticeSpecies != species2 )
		{
...
static void sub_81642A0( void )
{
	...
	do
	{
		do
		{
			...
			for( i = 0; i < j; i++ )
			{
				if( gSaveBlock2Ptr->frontier.trainerIds[i] ==
					trainerId )
					break;
				if( gFacilityTrainers[gSaveBlock2Ptr->frontier
							      .trainerIds[i]]
						.facilityClass ==
					gFacilityTrainers[trainerId]
						.facilityClass )
					break;
			}
		} while( i != j );
	...
	for( i = 0; i < 6; i++ )
	{
		...
		eventObjTemplates[i + 1].graphicsId =
	...
	{
		if( gSaveBlock2Ptr->apprentices[i].lvlMode != 0 &&
			gUnknown_085DF9EC[gSaveBlock2Ptr->apprentices[i]
						  .field_1] /
					7 <=
				challengeNum &&
	...
	if( r10 != 0 )
	{
		gSaveBlock2Ptr->frontier.trainerIds[6] =
			spArray[Random( ) % r10];
		eventObjTemplates[7].graphicsId =
	...
	for( i = 0; i < 5; i++ )
	{
		...
		if( gSaveBlock2Ptr->frontier.towerRecords[i].winStreak / 7 <=
				challengeNum &&
			gSaveBlock2Ptr->frontier.towerRecords[i].lvlMode ==
				lvlMode &&
			recordHasData &&
	...
	if( r10 != 0 )
	{
		gSaveBlock2Ptr->frontier.trainerIds[7] =
			spArray[Random( ) % r10];
		eventObjTemplates[8].graphicsId =
			GetBattleFacilityTrainerGfxId(
				gSaveBlock2Ptr->frontier.trainerIds[7] );
...
static void sub_8164828( void )
{
	...
	{
		if( gUnknown_085DD500[arrId].facilityClass ==
	...
	{
		ShowFieldMessage(
			gUnknown_085DD500[arrId].strings[gSpecialVar_0x8005] );
	}
	else if( trainerId < TRAINER_RECORD_MIXING_APPRENTICE )
	{
		ShowFieldMessage(
...
static void sub_8164B74( void )
{
	...
	case 1:
		if( ( GetBlockReceivedStatus( ) & 3 ) == 3 )
		{
			...
			for( i = 0; i < 14; i++ )
			{
				...
				if( i == j ) // This condition is always true,
					     // because of the loop above.
					gSaveBlock2Ptr->frontier
						.trainerIds[i] = trainerId;
			}
	...
	case 2:
		if( IsLinkTaskFinished( ) )
		{
			SendBlock( bitmask_all_link_players_but_self( ),
				&gSaveBlock2Ptr->frontier.trainerIds,
				sizeof( gSaveBlock2Ptr->frontier
						.trainerIds ) );
	...
	case 3:
		if( ( GetBlockReceivedStatus( ) & 3 ) == 3 )
		{
			...
			gTrainerBattleOpponent_A =
				gSaveBlock2Ptr->frontier
					.trainerIds[battleNum * 2];
			gTrainerBattleOpponent_B =
				gSaveBlock2Ptr->frontier
					.trainerIds[battleNum * 2 + 1];
			SetBattleFacilityTrainerGfxId(
				gTrainerBattleOpponent_A, 0 );
			SetBattleFacilityTrainerGfxId(
				gTrainerBattleOpponent_B, 1 );
...
{
	SetBattleFacilityTrainerGfxId(
		gSaveBlock2Ptr->frontier.trainerIds[17], 0xF );
}
...
static void sub_8164E04( void )
{
	...
	GetBattleTowerTrainerLanguage( &gSaveBlock2Ptr->frontier.field_BEB,
		gTrainerBattleOpponent_A );
	gSaveBlock2Ptr->frontier.field_BD6 =
		GetMonData( &gEnemyParty[gBattlerPartyIndexes[1]],
			MON_DATA_SPECIES,
			NULL );
	gSaveBlock2Ptr->frontier.field_BD4 =
		GetMonData( &gPlayerParty[gBattlerPartyIndexes[0]],
			MON_DATA_SPECIES,
			NULL );
	for( i = 0; i < POKEMON_NAME_LENGTH + 1; i++ )
		gSaveBlock2Ptr->frontier.field_BE0[i] =
...
static void ValidateBattleTowerRecordChecksums( void )
{
	...
	for( j = 0; j < ( sizeof( struct EmeraldBattleTowerRecord ) - 4 ) / 4;
		j++ ) // - 4, because of the last field being the checksum
		      // itself.
	{
		checksum += record[j];
	}
	...
	for( i = 0; i < 5; i++ )
	{
		...
		if( gSaveBlock2Ptr->frontier.towerRecords[i].checksum !=
			checksum )
			ClearBattleTowerRecord(
				&gSaveBlock2Ptr->frontier.towerRecords[i] );
	}
...
void CalcEmeraldBattleTowerChecksum( struct EmeraldBattleTowerRecord* record )
{
	...
	for( i = 0; i < ( sizeof( struct EmeraldBattleTowerRecord ) - 4 ) / 4;
		i++ ) // - 4, because of the last field being the checksum
		      // itself.
...
static void AwardBattleTowerRibbons( void )
{
	s32 i;
	u32 partyIndex;
	struct RibbonCounter ribbons[3]; // BUG: 4 Pokemon can receive ribbons
					 // in a double battle mode.
	u8 ribbonType = 0;
	u8 lvlMode    = gSaveBlock2Ptr->frontier.lvlMode;
	u8 battleMode = VarGet( VAR_FRONTIER_BATTLE_MODE );
	...
	if( GetCurrentBattleTowerWinStreak( lvlMode, battleMode ) > 55 )
	{
		for( i = 0; i < monCount; i++ )
		{
			...
			ribbons[i].count      = 0;
			if( !GetMonData(
				    &gSaveBlock1Ptr->playerParty[partyIndex],
				    ribbonType ) )
			{
				gSpecialVar_Result = TRUE;
				SetMonData( &gSaveBlock1Ptr
						     ->playerParty[partyIndex],
					ribbonType,
					&gSpecialVar_Result );
				ribbons[i].count = GetRibbonCount(
					&gSaveBlock1Ptr
						 ->playerParty[partyIndex] );
			}
		}
	...
	if( gSpecialVar_Result )
	{
		...
		if( ribbons[0].count > 4 )
		{
			sub_80EE4DC(
				&gSaveBlock1Ptr
					 ->playerParty[ribbons[0].partyIndex],
				ribbonType );
		}
	}
}
...
static void FillEReaderTrainerWithPlayerData( void )
{
	...
	CopyTrainerId(
		ereaderTrainer->trainerId, gSaveBlock2Ptr->playerTrainerId );
...
void ClearEReaderTrainer( struct BattleTowerEReaderTrainer* ereaderTrainer )
{
	...
	for( i = 0; i < ( sizeof( struct BattleTowerEReaderTrainer ) ) / 4;
		i++ )
...
static void CopyEReaderTrainerFarewellMessage( void )
{
	if( gBattleOutcome == B_OUTCOME_DREW )
		...
		FrontierSpeechToString( gSaveBlock2Ptr->frontier.ereaderTrainer
						.farewellPlayerWon );
	else
		FrontierSpeechToString( gSaveBlock2Ptr->frontier.ereaderTrainer
						.farewellPlayerLost );
...
static void FillPartnerParty( u16 trainerId )
{
	...
	if( trainerId == TRAINER_STEVEN_PARTNER )
	{
		for( i = 0; i < 3; i++ )
		{
			...
			CreateMon( &gPlayerParty[3 + i],
				...
				TRUE,
				i, // BUG: personality was stored in the 'j'
				   // variable. As a result, Steven's pokemon
				   // do not have the intended natures.
				TRUE,
				STEVEN_OTID );
			for( j = 0; j < 6; j++ )
				SetMonData( &gPlayerParty[3 + i],
					MON_DATA_HP_EV + j,
					&sStevenMons[i].evs[j] );
			for( j = 0; j < MAX_MON_MOVES; j++ )
				SetMonMoveSlot( &gPlayerParty[3 + i],
			...
			j = MALE;
			SetMonData(
				&gPlayerParty[3 + i], MON_DATA_OT_GENDER, &j );
	...
	else if( trainerId < TRAINER_RECORD_MIXING_FRIEND )
	{
		...
		for( i = 0; i < 2; i++ )
		{
			...
			for( j = 0; j < PLAYER_NAME_LENGTH + 1; j++ )
				trainerName[j] = gFacilityTrainers[trainerId]
							 .trainerName[j];
			SetMonData( &gPlayerParty[3 + i],
				MON_DATA_OT_NAME,
				&trainerName );
	...
	else
	{
		...
		for( i = 0; i < 2; i++ )
		{
			CreateApprenticeMon( &gPlayerParty[3 + i],
				&gSaveBlock2Ptr->apprentices[trainerId],
	...
	struct RSBattleTowerRecord* src, struct EmeraldBattleTowerRecord* dst )
{
	...
	else
	{
		...
		// BUG: Reading outside the array. sRubyFacilityClassToEmerald
		// has less than FACILITY_CLASSES_COUNT entries.
		for( i = 0; i < FACILITY_CLASSES_COUNT; i++ )
		{
			if( sRubyFacilityClassToEmerald[i][0] ==
				src->facilityClass )
				break;
		}
	...
	struct EmeraldBattleTowerRecord* src, struct RSBattleTowerRecord* dst )
{
	...
	else
	{
		...
		for( i = 0; i < FACILITY_CLASSES_COUNT; i++ )
		{
			if( sRubyFacilityClassToEmerald[i][1] ==
				src->facilityClass )
				break;
		}
		if( i != FACILITY_CLASSES_COUNT )
			...
			dst->facilityClass = 0x24; // FACILITY_CLASS_YOUNGSTER
						   // in Ruby/Sapphire.
	...
	for( i = 0; i < PARTY_SIZE; i++ )
	{
		if( GetMonData( &gPlayerParty[i], MON_DATA_SPECIES, NULL ) &&
			GetMonData( &gPlayerParty[i],
				...
				NULL ) != SPECIES_EGG )
		{
			s32 level = GetMonData(
				&gPlayerParty[i], MON_DATA_LEVEL, NULL );
...
bool32 ValidateBattleTowerRecord( u8 recordId ) // unused
{
	s32 i;
	u32* record =
		(u32*)( &gSaveBlock2Ptr->frontier.towerRecords[recordId] );
	u32 checksum = 0;
	u32 hasData  = 0;
	for( i = 0; i < ( sizeof( struct EmeraldBattleTowerRecord ) - 4 ) / 4;
		i++ ) // - 4, because of the last fjeld bejng the checksum
		      // jtself.
	{
		checksum += record[i];
		hasData |= record[i];
	...
	{
		ClearBattleTowerRecord(
			&gSaveBlock2Ptr->frontier.towerRecords[recordId] );
		return FALSE;
	}
	else
	{
...
void sub_8166188( void )
{
	if( gBattleTypeFlags & ( BATTLE_TYPE_LINK | BATTLE_TYPE_x2000000 ) )
	{
		...
		for( i = 0; i < PARTY_SIZE; i++ )
		{
			u32 species = GetMonData(
				&gEnemyParty[i], MON_DATA_SPECIES, NULL );
			if( species )
			{
