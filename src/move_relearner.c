 ...
 * the new move. Both are always loaded in memory, but only the currently
 * active one is copied to VRAM. The exception to this is the appeal and jam
 * hearts, which are sprites. MoveRelearnerShowHideHearts is called while
 * reloading the contest display to control them. DoMoveRelearnerMain:
...
static const struct SpritePalette sMoveRelearnerPalette = {
	.data = sMoveRelearnerPaletteData, .tag = 5526};
...
static void DoMoveRelearnerMain( void )
{
	...
	case MENU_STATE_TEACH_MOVE_CONFIRM: {
		...
		if( selection == 0 )
		{
			if( GiveMoveToMon( &gPlayerParty[sMoveRelearnerStruct
								 ->partyMon],
				    GetCurrentSelectedMove( ) ) != 0xFFFF )
			{
				FormatAndPrintText(
	...
	case MENU_STATE_PRINT_STOP_TEACHING:
		StringCopy(
			gStringVar2, gMoveNames[GetCurrentSelectedMove( )] );
		...
		if( !gPaletteFade.active )
		{
			...
			else
			{
				u16 moveId = GetMonData(
					&gPlayerParty[sMoveRelearnerStruct
							      ->partyMon],
					MON_DATA_MOVE1 +
...
static void HideHeartSpritesAndShowTeachMoveText( bool8 onlyHideSprites )
{
	...
	for( i = 0; i < 16; i++ )
	{
		gSprites[sMoveRelearnerStruct->heartSpriteIds[i]].invisible =
			TRUE;
	}
	...
	if( !onlyHideSprites )
	{
		...
		AddTextPrinterParameterized(
			3, 1, gStringVar4, 0, 1, 0, NULL );
	}
}

static void HandleInput( bool8 showContest )
{
	s32 itemId = ListMenu_ProcessInput(
		sMoveRelearnerStruct->moveListMenuTask );
	...
	case LIST_CANCEL:
		...
		StringExpandPlaceholders(
			gStringVar4, gText_MoveRelearnerGiveUp );
		MoveRelearnerPrintText( gStringVar4 );
		break;
	default:
		PlaySE( SE_SELECT );
...
static void CreateUISprites( void )
{
	...
	for( i = 0; i < 8; i++ )
	{
		sMoveRelearnerStruct->heartSpriteIds[i] =
			CreateSprite( &sConstestMoveHeartSprite,
				( i - ( i / 4 ) * 4 ) * 8 + 104,
				( i / 4 ) * 8 + 36,
				0 );
	}
	...
	for( i = 0; i < 8; i++ )
	{
		sMoveRelearnerStruct->heartSpriteIds[i + 8] =
			CreateSprite( &sConstestMoveHeartSprite,
				...
				0 );
		StartSpriteAnim(
			&gSprites[sMoveRelearnerStruct->heartSpriteIds[i + 8]],
			2 );
	}
...
static void AddScrollArrows( void )
{
	if( sMoveRelearnerStruct->moveDisplayArrowTask == 0xFF )
	{
		sMoveRelearnerStruct->moveDisplayArrowTask =
			AddScrollIndicatorArrowPair(
				...
				&sMoveRelearnerStruct->scrollOffset );
	}
	...
	if( sMoveRelearnerStruct->moveListScrollArrowTask == 0xFF )
	{
		...
		sMoveRelearnerStruct->moveListScrollArrowTask =
			AddScrollIndicatorArrowPair( &gTempScrollArrowTemplate,
				&sMoveRelearnerMenuSate.listOffset );
	}
}
...
static void CreateLearnableMovesList( void )
{
	...
	sMoveRelearnerStruct->numMenuChoices++;
	sMoveRelearnerStruct->numToShowAtOnce =
		LoadMoveRelearnerMovesList( sMoveRelearnerStruct->menuItems,
			sMoveRelearnerStruct->numMenuChoices );
}
...
void MoveRelearnerShowHideHearts( s32 moveId )
{
	...
	if( !sMoveRelearnerMenuSate.showContestInfo || moveId == LIST_CANCEL )
	{
		for( i = 0; i < 16; i++ )
		{
			gSprites[sMoveRelearnerStruct->heartSpriteIds[i]]
				.invisible = TRUE;
		}
	}
	else
	{
		numHearts = ( u8 )(
			gContestEffects[gContestMoves[moveId].effect].appeal /
			10 );
		...
		numHearts = ( u8 )(
			gContestEffects[gContestMoves[moveId].effect].jam /
			10 );
