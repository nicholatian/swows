...
{
	u16 gammaShiftColors[8][0x1000]; // 0x1000 is the number of bytes that
					 // make up all palettes.
};
...
static void ApplyGammaShiftWithBlend( u8 startPalIndex,
	u8 numPalettes,
	s8 gammaIndex,
	u8 blendCoeff,
	u16 blendColor );
static void ApplyDroughtGammaShiftWithBlend(
	s8 gammaIndex, u8 blendCoeff, u16 blendColor );
...
// this pointer. This is likely the result of compiler optimization, since
// using the pointer in this file produces the same result as accessing
// gWeather directly.
...
{
	if( weather != WEATHER_RAIN_LIGHT && weather != WEATHER_RAIN_MED &&
		weather != WEATHER_RAIN_HEAVY )
	{
...
static void Task_WeatherMain( u8 taskId )
{
	if( gWeatherPtr->currWeather != gWeatherPtr->nextWeather )
	{
		...
		{
			// Finished cleaning up previous weather. Now
...
static void UpdateWeatherGammaShift( void )
{
		...
		WEATHER_PAL_STATE_SCREEN_FADING_OUT )
	{
		...
		{
			if( ++gWeatherPtr->gammaStepFrameCounter >=
				gWeatherPtr->gammaStepDelay )
			{
				gWeatherPtr->gammaStepFrameCounter = 0;
				if( gWeatherPtr->gammaIndex <
					gWeatherPtr->gammaTargetIndex )
					gWeatherPtr->gammaIndex++;
				else
					gWeatherPtr->gammaIndex--;

				ApplyGammaShift(
					0, 32, gWeatherPtr->gammaIndex );
			}
		}
	}
}
...
static bool8 FadeInScreen_Drought( void )
{
	...
	ApplyDroughtGammaShiftWithBlend( -6,
		16 - gWeatherPtr->fadeScreenCounter,
		gWeatherPtr->fadeDestColor );
	return TRUE;
}
...
static void ApplyGammaShift( u8 startPalIndex, u8 numPalettes, s8 gammaIndex )
{
	...
	if( gammaIndex > 0 )
	{
		...
		// Loop through the speficied palette range and apply necessary
		// gamma shifts to the colors.
		while( curPalIndex < numPalettes )
		{
			...
			else
			{
				...
				if( sPaletteGammaTypes[curPalIndex] ==
						GAMMA_ALT ||
					curPalIndex - 16 ==
						gWeatherPtr
							->altGammaSpritePalIndex )
					gammaTable =
						gWeatherPtr->altGammaShifts
							[gammaIndex];
				else
					gammaTable = gWeatherPtr->gammaShifts
							     [gammaIndex];
				...
				for( i = 0; i < 16; i++ )
				{
					// Apply gamma shift to the original
					// color.
					struct RGBColor baseColor =
						*(struct
							RGBColor*)&gPlttBufferUnfaded
							[palOffset];
	...
	{
		// A negative gammIndex value means that the blending will come
		// from the special Drought weather's palette tables.
		...
		while( curPalIndex < numPalettes )
		{
			...
			else
			{
				for( i = 0; i < 16; i++ )
				{
					gPlttBufferFaded[palOffset] = sDroughtWeatherColors
						[gammaIndex]
						[DROUGHT_COLOR_INDEX(
							gPlttBufferUnfaded
								[palOffset] )];
	...
	while( curPalIndex < numPalettes )
	{
		...
		else
		{
			...
			if( sPaletteGammaTypes[curPalIndex] == GAMMA_NORMAL )
				gammaTable =
					gWeatherPtr->gammaShifts[gammaIndex];
			else
				gammaTable =
					gWeatherPtr
						->altGammaShifts[gammaIndex];

			for( i = 0; i < 16; i++ )
			{
				struct RGBColor baseColor =
					*(struct RGBColor*)&gPlttBufferUnfaded
						[palOffset];
				u8 r = gammaTable[baseColor.r];
				u8 g = gammaTable[baseColor.g];
				u8 b = gammaTable[baseColor.b];

				// Apply gamma shift and target blend color to
				// the original color.
	...
	for( curPalIndex = 0; curPalIndex < 32; curPalIndex++ )
	{
		...
		else
		{
			for( i = 0; i < 16; i++ )
			{
				...
				color1 = *(struct RGBColor*)&gPlttBufferUnfaded
						 [palOffset];
				r1 = color1.r;
				g1 = color1.g;
				b1 = color1.b;
				...
				offset = ( ( b1 & 0x1E ) << 7 ) |
					...
					( ( r1 & 0x1E ) >> 1 );
				color2 = *(
					struct RGBColor*)&sDroughtWeatherColors
						 [gammaIndex][offset];
				r2 = color2.r;
				g2 = color2.g;
				b2 = color2.b;
...
static void ApplyFogBlend( u8 blendCoeff, u16 blendColor )
{
	...
	for( curPalIndex = 16; curPalIndex < 32; curPalIndex++ )
	{
		...
		{
			BlendPalette(
				curPalIndex * 16, 16, blendCoeff, blendColor );
		}
	}
}
...
static void MarkFogSpritePalToLighten( u8 paletteIndex )
{
	...
	{
		gWeatherPtr->lightenedFogSpritePals
...
void FadeScreen( u8 mode, s8 delay )
{
	...
	if( fadeOut )
	{
		if( useWeatherPal )
			CpuFastCopy(
				gPlttBufferFaded, gPlttBufferUnfaded, 0x400 );
	...
	else
	{
		...
		if( useWeatherPal )
			...
			BeginNormalPaletteFade(
				0xFFFFFFFF, delay, 16, 0, fadeColor );
		...
		Weather_SetBlendCoeffs(
			gWeatherPtr->currBlendEVA, gWeatherPtr->currBlendEVB );
		gWeatherPtr->readyForInit = TRUE;
	}
}
...
void UpdateSpritePaletteWithWeather( u8 spritePaletteIndex )
{
	...
	case WEATHER_PAL_STATE_SCREEN_FADING_IN:
		if( gWeatherPtr->unknown_6CA != 0 )
		{
			...
			paletteIndex *= 16;
			for( i = 0; i < 16; i++ )
				gPlttBufferFaded[paletteIndex + i] =
					gWeatherPtr->fadeDestColor;
		}
		break;
	...
	default:
		...
		{
			ApplyGammaShift(
				paletteIndex, 1, gWeatherPtr->gammaIndex );
		}
		else
		{
			paletteIndex *= 16;
			BlendPalette(
				paletteIndex, 16, 12, RGB( 28, 31, 28 ) );
		}
		break;
	}
}
...
void sub_80ABFE0( s8 gammaIndex ) { sub_80ABC48( -gammaIndex - 1 ); }
...
void sub_80AC01C( void )
{
	...
	case 0:
		if( ++gWeatherPtr->unknown_740 > 5 )
		{
			...
			{
				gWeatherPtr->unknown_73E =
					gWeatherPtr->unknown_73C;
		...
		break;
	case 1:
		gWeatherPtr->unknown_740 =
			( gWeatherPtr->unknown_740 + 3 ) & 0x7F;
		gWeatherPtr->unknown_73C =
			( ( gSineTable[gWeatherPtr->unknown_740] - 1 ) >> 6 ) +
			2;
...
bool8 Weather_UpdateBlend( void )
{
	...
	if( ++gWeatherPtr->blendFrameCounter > gWeatherPtr->blendDelay )
	{
		...
		{
			if( gWeatherPtr->currBlendEVA <
				gWeatherPtr->targetBlendEVA )
				gWeatherPtr->currBlendEVA++;
			else if( gWeatherPtr->currBlendEVA >
				gWeatherPtr->targetBlendEVA )
				gWeatherPtr->currBlendEVA--;
		}
		else
		{
			if( gWeatherPtr->currBlendEVB <
				gWeatherPtr->targetBlendEVB )
				gWeatherPtr->currBlendEVB++;
			else if( gWeatherPtr->currBlendEVB >
				gWeatherPtr->targetBlendEVB )
				gWeatherPtr->currBlendEVB--;
		}
	}
	...
	SetGpuReg( REG_OFFSET_BLDALPHA,
		BLDALPHA_BLEND( gWeatherPtr->currBlendEVA,
			gWeatherPtr->currBlendEVB ) );
