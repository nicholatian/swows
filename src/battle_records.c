...
static const u32 sTrainerHillWindowTileset[] =
	INCBIN_U32( "graphics/unknown/unknown_5B3484.4bpp" );
static const u16 sTrainerHillWindowPalette[] =
	INCBIN_U16( "graphics/unknown/unknown_5B3484.gbapal" );
static const u32 sTrainerHillWindowTilemap[] =
	INCBIN_U32( "graphics/unknown/unknown_5B3564.bin" );
...
static s32 FindLinkBattleRecord(
	struct LinkBattleRecord* records, const u8* name, u16 trainerId )
{
	s32 i;
	...
	{
		if( !StringCompareN(
			    records[i].name, name, PLAYER_NAME_LENGTH ) &&
...
static void SortLinkBattleRecords( struct LinkBattleRecords* records )
{
	...
	for( i = LINK_B_RECORDS_COUNT - 1; i > 0; i-- )
	{
		for( j = i - 1; j >= 0; j-- )
		{
			s32 totalBattlesI = GetLinkBattleRecordTotalBattles(
				&records->entries[i] );
			s32 totalBattlesJ = GetLinkBattleRecordTotalBattles(
				&records->entries[j] );
...
static void UpdateLinkBattleRecord(
	struct LinkBattleRecord* record, s32 battleOutcome )
{
	...
	u8 battlerId )
{
	...
	if( index == LINK_B_RECORDS_COUNT )
	{
		...
		StringCopyN( records->entries[index].name,
			name,
			PLAYER_NAME_LENGTH );
...
static void PrintLinkBattleWinsLossesDraws( struct LinkBattleRecord* records )
{
	...
	AddTextPrinterParameterized(
		gRecordsWindowId, 1, gStringVar4, x, 0x11, 0, NULL );
}

static void PrintLinkBattleRecord(
	struct LinkBattleRecord* record, u8 y, s32 language )
{
...
void ShowLinkBattleRecords( void )
{
	...
	AddTextPrinterParameterized(
		gRecordsWindowId, 1, gStringVar4, x, 1, 0, NULL );
	PrintLinkBattleWinsLossesDraws(
		gSaveBlock1Ptr->linkBattleRecords.entries );

	StringExpandPlaceholders( gStringVar4, gText_WinLoseDraw );
	AddTextPrinterParameterized(
		gRecordsWindowId, 1, gStringVar4, 0, 41, 0, NULL );

	for( i = 0; i < LINK_B_RECORDS_COUNT; i++ )
	{
		PrintLinkBattleRecord(
			&gSaveBlock1Ptr->linkBattleRecords.entries[i],
			7 + ( i * 2 ),
...
static void LoadTrainerHillRecordsWindowGfx( u8 bgId )
{
	LoadBgTiles( bgId,
		...
		sizeof( sTrainerHillWindowTileset ),
		0 );
	CopyToBgTilemapBufferRect(
		bgId, sTrainerHillWindowTilemap, 0, 0, 0x20, 0x20 );
