...
static u8 sub_808C3A4( struct Task* task,
	struct EventObject* playerObject,
	struct EventObject* strengthObject );
static u8 do_boulder_dust( struct Task* task,
	struct EventObject* playerObject,
	struct EventObject* strengthObject );
static u8 sub_808C484( struct Task* task,
	struct EventObject* playerObject,
	struct EventObject* strengthObject );

static void DoPlayerMatJump( void );
static void DoPlayerAvatarSecretBaseMatJump( u8 taskId );
static u8 PlayerAvatar_DoSecretBaseMatJump(
...
static const u16 sRivalAvatarGfxIds[][2] = {
	...
	{EVENT_OBJ_GFX_RIVAL_BRENDAN_MACH_BIKE,
		EVENT_OBJ_GFX_RIVAL_MAY_MACH_BIKE},
	{EVENT_OBJ_GFX_RIVAL_BRENDAN_ACRO_BIKE,
	...
	{EVENT_OBJ_GFX_RIVAL_BRENDAN_FIELD_MOVE,
...
static const u16 sRSAvatarGfxIds[] = {
	EVENT_OBJ_GFX_LINK_RS_BRENDAN, EVENT_OBJ_GFX_LINK_RS_MAY};
...
static bool8 ( *const gUnknown_08497530[] )(
	struct Task*, struct EventObject*, struct EventObject* ) = {
	sub_808C3A4,
	do_boulder_dust,
	sub_808C484,
};

static bool8 ( *const sPlayerAvatarSecretBaseMatJump[] )(
	struct Task*, struct EventObject* ) = {
	PlayerAvatar_DoSecretBaseMatJump,
};

static bool8 ( *const sPlayerAvatarSecretBaseMatSpin[] )(
	struct Task*, struct EventObject* ) = {
...
static u8 EventObjectCB2_NoMovement2( void ) { return 0; }
...
void player_step( u8 direction, u16 newKeys, u16 heldKeys )
{
	...
	if( gPlayerAvatar.preventStep == FALSE )
	{
		...
		if( TryInterruptEventObjectSpecialAnim(
			    playerEventObj, direction ) == 0 )
		{
...
static bool8 TryInterruptEventObjectSpecialAnim(
	struct EventObject* playerEventObj, u8 direction )
{
#ifdef NONMATCHING
	u8 r5 = direction;
	u8 r6 = direction;
		...
		!EventObjectClearHeldMovementIfFinished( playerEventObj ) )
	{
		u8 heldMovementActionId =
			EventObjectGetHeldMovementActionId( playerEventObj );
		if( heldMovementActionId > MOVEMENT_ACTION_WALK_FAST_RIGHT &&
			heldMovementActionId <
...
static void MovePlayerAvatarUsingKeypadInput(
	u8 direction, u16 newKeys, u16 heldKeys )
{
...
static bool8 ForcedMovement_None( void )
{
	if( gPlayerAvatar.flags & PLAYER_AVATAR_FLAG_6 )
	{
		...
		SetEventObjectDirection(
			playerEventObj, playerEventObj->facingDirection );
...
static bool8 ForcedMovement_MuddySlope( void )
{
	...
	if( playerEventObj->movementDirection != DIR_NORTH ||
		GetPlayerSpeed( ) <= 3 )
	{
...
{
	gUnknown_08497490[CheckMovementInputNotOnBike( direction )](
		direction, heldKeys );
}
...
static void PlayerNotOnBikeMoving( u8 direction, u16 heldKeys )
{
	...
	if( !( gPlayerAvatar.flags & PLAYER_AVATAR_FLAG_UNDERWATER ) &&
		...
		IsRunningDisallowed( gEventObjects[gPlayerAvatar.eventObjectId]
...
static u8 CheckForPlayerAvatarCollision( u8 direction )
{
	...
	return CheckForEventObjectCollision( playerEventObj,
		x,
		y,
		direction,
...
static u8 sub_808B028( u8 direction )
{
	...
	y = playerEventObj->currentCoords.y;
	MoveCoords( direction, &x, &y );
	return sub_808B164( playerEventObj,
		x,
		y,
		direction,
		MapGridGetMetatileBehaviorAt( x, y ) );
...
u8 CheckForEventObjectCollision(
	struct EventObject* a, s16 x, s16 y, u8 direction, u8 e )
{
	u8 collision;
	...
	struct EventObject* a, s16 x, s16 y, u8 direction, u8 e )
{
	...
	if( collision == 0 )
	{
		if( CheckForRotatingGatePuzzleCollisionWithoutAnimation(
			    direction, x, y ) != 0 )
			return 8;
		check_acro_bike_metatile( x, y, e, &collision );
	}
	...
	if( FlagGet( FLAG_SYS_USE_STRENGTH ) )
	{
		...
		if( eventObjectId != 16 &&
			gEventObjects[eventObjectId].graphicsId ==
				EVENT_OBJ_GFX_PUSHABLE_BOULDER )
		{
...
bool8 IsPlayerCollidingWithFarawayIslandMew( u8 direction )
{
	...
	mewObjectId = GetEventObjectIdByLocalIdAndMap( 1,
		MAP_NUM( FARAWAY_ISLAND_INTERIOR ),
		MAP_GROUP( FARAWAY_ISLAND_INTERIOR ) );
...
{
	EventObjectSetGraphicsId( eventObj,
...
static void PlayerAvatarTransition_ReturnToField(
...
{
	u8 movementActionId =
		gEventObjects[gPlayerAvatar.eventObjectId].movementActionId;
...
static bool8 PlayerCheckIfAnimFinishedOrInactive( void )
{
	return EventObjectCheckHeldMovementStatus(
		&gEventObjects[gPlayerAvatar.eventObjectId] );
}
...
{
	PlayerSetAnimId(
		GetAcroWheelieFaceDirectionMovementAction( direction ), 1 );
}
...
{
	PlayerSetAnimId(
		GetAcroPopWheelieFaceDirectionMovementAction( direction ), 1 );
}
...
{
	PlayerSetAnimId(
		GetAcroEndWheelieFaceDirectionMovementAction( direction ), 1 );
}
...
void PlayerStandingHoppingWheelie( u8 a )
{
	...
	PlayerSetAnimId(
		GetAcroWheelieHopFaceDirectionMovementAction( a ), 1 );
}
...
void PlayerAcroTurnJump( u8 direction )
{
	PlaySE( SE_JITE_PYOKO );
	PlayerSetAnimId(
		GetJumpInPlaceTurnAroundMovementAction( direction ), 1 );
}
...
void sub_808B980( u8 direction )
{
	PlaySE( SE_WALL_HIT );
	PlayerSetAnimId(
		GetAcroWheelieInPlaceDirectionMovementAction( direction ), 2 );
}

void sub_808B9A4( u8 direction )
{
	PlayerSetAnimId(
		GetAcroPopWheelieMoveDirectionMovementAction( direction ), 2 );
}

void sub_808B9BC( u8 direction )
{
	PlayerSetAnimId(
		GetAcroWheelieMoveDirectionMovementAction( direction ), 2 );
}

void sub_808B9D4( u8 direction )
{
	PlayerSetAnimId(
		GetAcroEndWheelieMoveDirectionMovementAction( direction ), 2 );
}

static void PlayCollisionSoundIfNotFacingWarp( u8 a )
{
	s16 x, y;
	u8 metatileBehavior = gEventObjects[gPlayerAvatar.eventObjectId]
				      .currentMetatileBehavior;
...
void sub_808BC90( s16 x, s16 y )
{
	MoveEventObjectToMapCoords(
		&gEventObjects[gPlayerAvatar.eventObjectId], x, y );
}

u8 TestPlayerAvatarFlags( u8 a ) { return gPlayerAvatar.flags & a; }

u8 sub_808BCD0( void ) { return gPlayerAvatar.flags; }

u8 GetPlayerAvatarObjectId( void ) { return gPlayerAvatar.spriteId; }

void sub_808BCE8( void ) { ForcedMovement_None( ); }
...
bool8 IsPlayerFacingSurfableFishableWater( void )
{
	...
	if( GetCollisionAtCoords(
		    playerEventObj, x, y, playerEventObj->facingDirection ) ==
			3 &&
		PlayerGetZCoord( ) == 3 &&
...
{
	gPlayerAvatar.flags &= ( PLAYER_AVATAR_FLAG_DASH |
		PLAYER_AVATAR_FLAG_6 | PLAYER_AVATAR_FLAG_5 );
	gPlayerAvatar.flags |= flags;
}

static u8 GetPlayerAvatarStateTransitionByGraphicsId(
	u16 graphicsId, u8 gender )
{
	u8 i;
...
{
	u8 unk = GetPlayerAvatarStateTransitionByGraphicsId(
...
void InitPlayerAvatar( s16 x, s16 y, u8 direction, u8 gender )
{
	...
	playerEventObjTemplate.graphicsId =
		GetPlayerAvatarGraphicsIdByStateIdAndGender(
			PLAYER_AVATAR_STATE_NORMAL, gender );
	...
	SetPlayerAvatarStateMask(
		PLAYER_AVATAR_FLAG_5 | PLAYER_AVATAR_FLAG_ON_FOOT );
}
...
void sub_808C0A8( u8 a )
{
	...
	if( TestPlayerAvatarFlags( PLAYER_AVATAR_FLAG_SURFING ) )
		gSprites[gEventObjects[gPlayerAvatar.eventObjectId]
				 .fieldEffectSpriteId]
			.invisible = a;
}
...
{
	EventObjectSetGraphicsId( &gEventObjects[gPlayerAvatar.eventObjectId],
		GetPlayerAvatarGraphicsIdByStateId(
...
static void sub_808C280( struct EventObject* eventObject )
{
	...
	for( x = 0, direction = DIR_SOUTH; x < 4; x++, direction++ )
	{
		if( sArrowWarpMetatileBehaviorChecks2[x]( metatileBehavior ) &&
			direction == eventObject->movementDirection )
		{
			x = eventObject->currentCoords.x;
			y = eventObject->currentCoords.y;
			MoveCoords( direction, &x, &y );
			ShowWarpArrowSprite( eventObject->warpArrowSpriteId,
				direction,
				x,
				y );
			return;
...
static bool8 sub_808C3A4( struct Task* task,
	struct EventObject* playerObject,
	struct EventObject* strengthObject )
{
...
static bool8 do_boulder_dust( struct Task* task,
	struct EventObject* playerObject,
	struct EventObject* strengthObject )
{
		...
		!EventObjectIsMovementOverridden( strengthObject ) )
	{
		...
		EventObjectSetHeldMovement( playerObject,
			GetWalkInPlaceNormalMovementAction(
				(u8)task->data[2] ) );
		EventObjectSetHeldMovement( strengthObject,
			GetWalkSlowMovementAction( (u8)task->data[2] ) );
		...
		gFieldEffectArguments[3] =
			gSprites[strengthObject->spriteId].oam.priority;
...
static bool8 sub_808C484( struct Task* task,
	struct EventObject* playerObject,
	struct EventObject* strengthObject )
{
	...
	struct Task* task, struct EventObject* eventObject )
{
	...
	if( EventObjectClearHeldMovementIfFinished( eventObject ) )
	{
		...
		task->data[0]++;
		if( task->data[2] > 3 &&
			direction == GetOppositeDirection( task->data[1] ) )
			task->data[0]++;
	}
	return FALSE;
}
	...
	struct Task* task, struct EventObject* eventObject )
{
	...
	{
		EventObjectSetHeldMovement(
			eventObject, actions[task->data[2]] );
		task->data[0] = 1;
	}
	return FALSE;
}
	...
	if( EventObjectClearHeldMovementIfFinished( eventObject ) )
	{
		EventObjectSetHeldMovement( eventObject,
			GetWalkSlowMovementAction(
				GetOppositeDirection( task->data[1] ) ) );
		ScriptContext2_Disable( );
		gPlayerAvatar.preventStep = FALSE;
		DestroyTask(
			FindTaskIdByFunc( PlayerAvatar_DoSecretBaseMatSpin ) );
	}
	return FALSE;
}
...
static void taskFF_0805D1D4( u8 taskId )
{
	...
	EventObjectSetHeldMovement( playerEventObj,
		GetJumpSpecialMovementAction( (u8)gTasks[taskId].data[0] ) );
...
static void sub_808C814( u8 taskId )
{
	...
	if( EventObjectClearHeldMovementIfFinished( playerEventObj ) )
	{
		EventObjectSetGraphicsId( playerEventObj,
			GetPlayerAvatarGraphicsIdByStateId(
				PLAYER_AVATAR_STATE_NORMAL ) );
		EventObjectSetHeldMovement( playerEventObj,
			GetFaceDirectionMovementAction(
				playerEventObj->facingDirection ) );
		gPlayerAvatar.preventStep = FALSE;
		ScriptContext2_Disable( );
		DestroySprite(
			&gSprites[playerEventObj->fieldEffectSpriteId] );
		DestroyTask( taskId );
	}
}
...
static bool8 Fishing2( struct Task* task )
{
	...
	task->tMinRoundsRequired = arr1[task->tFishingRod] +
		( Random( ) % arr2[task->tFishingRod] );
	task->tPlayerGfxId =
		gEventObjects[gPlayerAvatar.eventObjectId].graphicsId;
...
{
	const s16 arr[][2] = {{0, 0}, {40, 10}, {70, 30}};
...
static bool8 Fishing10( struct Task* task )
{
	...
	AddTextPrinterParameterized2(
		0, 1, gText_PokemonOnHook, 1, 0, 2, 1, 3 );
	task->tStep++;
	task->tFrameCounter = 0;
	return FALSE;
}
	...
	if( task->tFrameCounter == 0 )
	{
		if( !IsTextPrinterActive( 0 ) )
		{
			...
			EventObjectSetGraphicsId(
				playerEventObj, task->tPlayerGfxId );
			EventObjectTurn( playerEventObj,
				playerEventObj->movementDirection );
			if( gPlayerAvatar.flags & PLAYER_AVATAR_FLAG_SURFING )
				sub_8155604(
					gEventObjects[gPlayerAvatar
							      .eventObjectId]
						.fieldEffectSpriteId,
					0,
					0 );
...
static bool8 Fishing12( struct Task* task )
{
	...
	FillWindowPixelBuffer( 0, PIXEL_FILL( 1 ) );
	AddTextPrinterParameterized2(
		0, 1, gText_NotEvenANibble, 1, 0, 2, 1, 3 );
...
static bool8 Fishing15( struct Task* task )
{
	...
	if( gSprites[gPlayerAvatar.spriteId].animEnded )
	{
		...
		EventObjectTurn(
			playerEventObj, playerEventObj->movementDirection );
		if( gPlayerAvatar.flags & PLAYER_AVATAR_FLAG_SURFING )
			sub_8155604( gEventObjects[gPlayerAvatar.eventObjectId]
					     .fieldEffectSpriteId,
				0,
				0 );
		gSprites[gPlayerAvatar.spriteId].pos2.x = 0;
...
static void AlignFishingAnimationFrames( void )
{
	...
	if( playerSprite->anims[playerSprite->animNum][animCmdIndex].type ==
		-1 )
	{
		animCmdIndex--;
	}
	else
	{
		...
		if( playerSprite->anims[playerSprite->animNum][animCmdIndex]
				.type == -1 )
			animCmdIndex--;
	}
	animType =
		playerSprite->anims[playerSprite->animNum][animCmdIndex].type;
...
void sub_808D074( u8 a0 ) { gUnknown_0203734C = a0; }
...
void sub_808D194( void ) { sub_808D1FC( CreateTask( sub_808D1FC, 0 ) ); }

bool32 sub_808D1B4( void ) { return FuncIsActiveTask( sub_808D1FC ); }

void sub_808D1C8( void ) { sub_808D094( CreateTask( sub_808D094, 0 ) ); }

bool32 sub_808D1E8( void ) { return FuncIsActiveTask( sub_808D094 ); }

static const u8 gUnknown_084975BC[] = {
...
static void sub_808D1FC( u8 taskId )
{
	...
	case 0:
		data[5] = sub_808D080( );
		EventObjectForceSetHeldMovement( object,
			GetFaceDirectionMovementAction(
				gUnknown_084975BC[data[5]] ) );
		data[1]        = 0;
		data[2]        = 116;
...
static u8 sub_808D38C( struct EventObject* object, s16* a1 )
{
	...
	EventObjectForceSetHeldMovement( object,
		GetFaceDirectionMovementAction(
			gUnknown_084975BC[object->facingDirection] ) );
	*a1 = 0;
