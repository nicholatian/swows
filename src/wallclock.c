...
static const u32 sUnknown_085B1F58[] =
	INCBIN_U32( "graphics/wallclock/graphics_85b1f58.4bpp.lz" );
static const u16 sUnknown_085B21D4[] =
	INCBIN_U16( "graphics/wallclock/palette_85b21d4.gbapal" );
...
static const struct SpritePalette gUnknown_085B2218[] = {
	{.data = gWallclockMale_Pal, .tag = TAG_PAL_WALL_CLOCK_HAND},
...
static void Task_SetClock2( u8 taskId )
{
	...
	else
	{
		...
		gTasks[taskId].tHourHandAngle =
			( gTasks[taskId].tHours % 12 ) * 30 +
			( gTasks[taskId].tMinutes / 10 ) * 5;
...
static void Task_SetClock3( u8 taskId )
{
	...
	AddTextPrinterParameterized(
		0, 1, gText_IsThisTheCorrectTime, 0, 1, 0, NULL );
	PutWindowTilemap( 0 );
...
static void Task_SetClock5( u8 taskId )
{
	RtcInitLocalTimeOffset(
		gTasks[taskId].tHours, gTasks[taskId].tMinutes );
...
static void InitClockWithRtc( u8 taskId )
{
	...
	gTasks[taskId].tMinuteHandAngle = gTasks[taskId].tMinutes * 6;
	gTasks[taskId].tHourHandAngle   = ( gTasks[taskId].tHours % 12 ) * 30 +
		( gTasks[taskId].tMinutes / 10 ) * 5;
