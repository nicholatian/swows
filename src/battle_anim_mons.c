...
u8 GetBattlerSpriteCoord( u8 battlerId, u8 coordType )
{
	...
	case BATTLER_COORD_X_2:
		retVal = sBattlerCoords[IS_DOUBLE_BATTLE( )]
	...
	default:
		...
		if( coordType == BATTLER_COORD_Y_PIC_OFFSET )
			retVal = GetBattlerSpriteFinal_Y(
				battlerId, species, TRUE );
		else
			retVal = GetBattlerSpriteFinal_Y(
				battlerId, species, FALSE );
		break;
	}
...
u8 GetBattlerYDelta( u8 battlerId, u16 species )
{
	...
	if( GetBattlerSide( battlerId ) == B_SIDE_PLAYER || IsContest( ) )
	{
		if( species == SPECIES_UNOWN )
		{
			...
			else
			{
				...
				if( !spriteInfo[battlerId].transformSpecies )
					...
					personality = gTransformedPersonalities
						[battlerId];
			}
		...
		{
			ret = sCastformBackSpriteYCoords
	...
	else
	{
		if( species == SPECIES_UNOWN )
		{
			...
			if( !spriteInfo[battlerId].transformSpecies )
				...
				personality =
					gTransformedPersonalities[battlerId];
			letter = GET_UNOWN_LETTER( personality );
			if( !letter )
				coordSpecies = species;
			else
		...
		{
			ret = gCastformFrontSpriteCoords
				      [gBattleMonForms[battlerId]]
					      .y_offset;
		}
		else if( species > NUM_SPECIES )
...
u8 GetBattlerSpriteFinal_Y( u8 battlerId, u16 species, bool8 a3 )
{
	...
	y = offset +
		sBattlerCoords[IS_DOUBLE_BATTLE( )]
			      [GetBattlerPosition( battlerId )]
				      .y;
	if( a3 )
	{
...
u8 GetBattlerSpriteCoord2( u8 battlerId, u8 coordType )
{
		...
		coordType == BATTLER_COORD_Y_PIC_OFFSET_DEFAULT )
	{
		...
		if( coordType == BATTLER_COORD_Y_PIC_OFFSET )
			return GetBattlerSpriteFinal_Y(
				battlerId, species, TRUE );
		else
			return GetBattlerSpriteFinal_Y(
				battlerId, species, FALSE );
...
{
	return GetBattlerSpriteCoord(
		battlerId, BATTLER_COORD_Y_PIC_OFFSET_DEFAULT );
}
...
u8 GetAnimBattlerSpriteId( u8 animBattler )
{
	...
	else if( animBattler == ANIM_ATK_PARTNER )
	{
		if( !IsBattlerSpriteVisible(
			...
			return gBattlerSpriteIds[BATTLE_PARTNER(
				gBattleAnimAttacker )];
	}
	else
	{
...
void StoreSpriteCallbackInData6(
	struct Sprite* sprite, void ( *callback )( struct Sprite* ) )
{
...
void TranslateSpriteInGrowingCircleOverDuration( struct Sprite* sprite )
{
	if( sprite->data[3] )
	{
		sprite->pos2.x = Sin( sprite->data[0],
...
void sub_80A64EC( struct Sprite* sprite )
{
	...
	sprite->data[2] =
		( sprite->data[4] - sprite->data[3] ) / sprite->data[0];
	sprite->data[1] = old;
}
...
bool8 TranslateAnimHorizontalArc( struct Sprite* sprite )
{
	...
	sprite->pos2.y +=
		Sin( ( u8 )( sprite->data[7] >> 8 ), sprite->data[5] );
	return FALSE;
}
...
void InitSpritePosToAnimTarget(
	struct Sprite* sprite, bool8 respectMonPicOffsets )
{
	// Battle anim sprites are automatically created at the anim target's
	// center, which is why there is no else clause for the
	// "respectMonPicOffsets" check.
	if( !respectMonPicOffsets )
	{
		sprite->pos1.x = GetBattlerSpriteCoord2(
...
void InitSpritePosToAnimAttacker(
	struct Sprite* sprite, bool8 respectMonPicOffsets )
{
	if( !respectMonPicOffsets )
	{
...
static bool8 sub_80A7238( void )
{
	if( IsContest( ) )
	{
		if( gSprites[GetAnimBattlerSpriteId( ANIM_ATTACKER )]
				.data[2] == SPECIES_UNOWN )
			return FALSE;
		else
			return TRUE;
	}
...
void SetBattlerSpriteYOffsetFromRotation( u8 spriteId )
{
	...
	// The "c" component of the battler sprite matrix contains the sine of
	// the rotation angle divided by some scale amount.
...
void TrySetSpriteRotScale( struct Sprite* sprite,
	bool8 recalcCenterVector,
	s16 xScale,
	s16 yScale,
	u16 rotation )
{
...
static u16 ArcTan2_( s16 a, s16 b ) { return ArcTan2( a, b ); }
...
void SetGreyscaleOrOriginalPalette(
	u16 paletteNum, bool8 restoreOriginalColor )
{
	int i;
	...
	if( !restoreOriginalColor )
	{
		for( i = 0; i < 16; i++ )
		{
			originalColor = (struct PlttData*)&gPlttBufferUnfaded
				[paletteNum + i];
			average = originalColor->r + originalColor->g +
				originalColor->b;
			average /= 3;

			destColor = (struct PlttData*)&gPlttBufferFaded
				[paletteNum + i];
			destColor->r = average;
			destColor->g = average;
			destColor->b = average;
	...
	{
		CpuCopy32( &gPlttBufferUnfaded[paletteNum],
			&gPlttBufferFaded[paletteNum],
			32 );
	}
}

u32 sub_80A75AC( u8 battleBackground,
	u8 attacker,
	u8 target,
	u8 attackerPartner,
	u8 targetPartner,
...
u32 sub_80A76C4( u8 a1, u8 a2, u8 a3, u8 a4 )
{
	...
	else
	{
		if( a1 )
		{
			if( IsBattlerSpriteVisible( GetBattlerAtPosition(
...
u8 sub_80A77AC( u8 a1 ) { return a1; }
...
void TranslateAnimSpriteToTargetMonLocation( struct Sprite* sprite )
{
	...
	sprite->data[2] =
		...
		gBattleAnimArgs[2];
	sprite->data[4] =
		GetBattlerSpriteCoord( gBattleAnimTarget, coordType ) +
		gBattleAnimArgs[3];
...
void sub_80A7938( struct Sprite* sprite )
{
	...
	sprite->data[2] =
		...
		gBattleAnimArgs[2];
	sprite->data[4] = GetBattlerSpriteCoord( battlerId, coordType ) +
		gBattleAnimArgs[3];
	sprite->callback = StartAnimLinearTranslation;
	StoreSpriteCallbackInData6( sprite, DestroyAnimSprite );
}
...
void AnimTask_BlendMonInAndOut( u8 task )
{
	...
	gTasks[task].data[0] =
		( gSprites[spriteId].oam.paletteNum * 0x10 ) + 0x101;
...
void PrepareAffineAnimInTaskData( struct Task* task,
	u8 spriteId,
...
{
	gAnimTaskAffineAnim =
		LoadPointerFromVars( task->data[13], task->data[14] ) +
		( task->data[7] << 3 );
	...
	default:
		...
		task->data[11] += gAnimTaskAffineAnim->frame.yScale;
		task->data[12] += gAnimTaskAffineAnim->frame.rotation;
		SetSpriteRotScale( task->data[15],
			task->data[10],
			task->data[11],
			task->data[12] );
	...
	case AFFINEANIMCMDTYPE_LOOP:
		if( gAnimTaskAffineAnim->loop.count )
		{
			...
			{
				task->data[9] =
					gAnimTaskAffineAnim->loop.count;
			}
			if( !task->data[7] )
			{
				break;
		...
		if( gBattlerSpriteIds[i] == spriteId )
		{
			...
			else
			{
				...
				else
				{
					...
					if( species == SPECIES_CASTFORM )
						return sCastformElevations[gBattleMonForms
								[battlerId]];
					else
						return gMonFrontPicCoords
							[species]
								.y_offset;
				}
			}
...
void sub_80A805C(
	struct Task* task, u8 a2, s16 a3, s16 a4, s16 a5, s16 a6, u16 a7 )
{
	task->data[8]  = a7;
...
void sub_80A8174( u8 priority )
{
	if( IsBattlerSpriteVisible( gBattleAnimTarget ) )
		gSprites[gBattlerSpriteIds[gBattleAnimTarget]].oam.priority =
			priority;
	if( IsBattlerSpriteVisible( gBattleAnimAttacker ) )
		gSprites[gBattlerSpriteIds[gBattleAnimAttacker]].oam.priority =
			priority;
	...
	u32 a10 )
{
	...
	RequestDma3Copy( gMonSpritesGfxPtr->field_17C,
		(void*)( OBJ_VRAM0 + ( sheet * 0x20 ) ),
		0x800,
		1 );
	FREE_AND_SET_NULL( gMonSpritesGfxPtr->field_17C );

	if( !isBackpic )
		spriteId = CreateSprite( &sUnknown_08525F90[a3],
			x,
		...
		spriteId = CreateSprite( &sUnknown_08525F90[a3],
			x,
			y + gMonBackPicCoords[species].y_offset,
			subpriority );
...
void SetAverageBattlerPositions(
	u8 battlerId, bool8 respectMonPicOffsets, s16* x, s16* y )
{
	...
	{
		partnerX = GetBattlerSpriteCoord(
			BATTLE_PARTNER( battlerId ), xCoordType );
		partnerY = GetBattlerSpriteCoord(
			BATTLE_PARTNER( battlerId ), yCoordType );
	}
	else
	{
		partnerX = battlerX;
