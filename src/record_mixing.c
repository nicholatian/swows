...
static void ReceiveDaycareMailData(
	struct RecordMixingDayCareMail*, size_t, u8, TVShow* );
...
static void ReceiveApprenticeData(
	struct Apprentice* arg0, size_t arg1, u32 arg2 );
static void ReceiveRankingHallRecords(
	struct PlayerHallRecords* hallRecords, size_t arg1, u32 arg2 );
...
static void SanitizeEmeraldBattleTowerRecord(
	struct EmeraldBattleTowerRecord* arg0 );
...
{
	memcpy( dest->secretBases,
		sSecretBasesSave,
		sizeof( dest->secretBases ) );
	...
	memcpy( dest->easyChatPairs,
		sEasyChatPairsSave,
		sizeof( dest->easyChatPairs ) );
	sub_80E89F8( &dest->dayCareMail );
	EmeraldBattleTowerRecordToRuby(
		sBattleTowerSave, &dest->battleTowerRecord );
...
static void PrepareExchangePacket( void )
{
	...
	else
	{
		...
		memcpy( sSentRecord->emerald.tvShows,
			sTvShowsSave,
			sizeof( sSentRecord->emerald.tvShows ) );
		memcpy( sSentRecord->emerald.pokeNews,
			sPokeNewsSave,
			sizeof( sSentRecord->emerald.pokeNews ) );
		memcpy( &sSentRecord->emerald.oldMan,
			sOldManSave,
		...
		SanitizeEmeraldBattleTowerRecord(
			&sSentRecord->emerald.battleTowerRecord );

		if( GetMultiplayerId( ) == 0 )
			sSentRecord->emerald.giftItem = GetRecordMixingGift( );

		sub_80E8110(
			sSentRecord->emerald.apprentice, sApprenticesSave );
...
static void ReceiveExchangePacket( u32 which )
{
	if( Link_AnyPartnersPlayingRubyOrSapphire( ) )
	{
		...
		ReceiveBattleTowerData(
			&sReceivedRecords->ruby.battleTowerRecord,
			sizeof( struct PlayerRecordsRS ),
			which );
		...
		ReceiveOldManData( &sReceivedRecords->ruby.oldMan,
			sizeof( struct PlayerRecordsRS ),
			which );
		ReceiveEasyChatPairsData( sReceivedRecords->ruby.easyChatPairs,
			sizeof( struct PlayerRecordsRS ),
			which );
	...
	else
	{
		...
		ReceiveEasyChatPairsData(
			sReceivedRecords->emerald.easyChatPairs,
			sizeof( struct PlayerRecordsEmerald ),
			which );
		...
		ReceiveBattleTowerData(
			&sReceivedRecords->emerald.battleTowerRecord,
			sizeof( struct PlayerRecordsEmerald ),
			which );
		ReceiveGiftItem( &sReceivedRecords->emerald.giftItem, which );
		ReceiveLilycoveLadyData(
			&sReceivedRecords->emerald.lilycoveLady,
			sizeof( struct PlayerRecordsEmerald ),
			which );
		ReceiveApprenticeData( sReceivedRecords->emerald.apprentice,
			sizeof( struct PlayerRecordsEmerald ),
			(u8)which );
		ReceiveRankingHallRecords(
			&sReceivedRecords->emerald.hallRecords,
			sizeof( struct PlayerRecordsEmerald ),
			(u8)which );
	}
}
...
static void Task_RecordMixing_Main( u8 taskId )
{
	...
	case 0: // init
		...
		tSndEffTaskId =
			CreateTask( Task_RecordMixing_SoundEffect, 81 );
		break;
...
static void Task_MixingRecordsRecv( u8 taskId )
{
	...
	break;
	case 201:
		// We're the link master. Delay for 30 frames per connected
		// player.
	...
	case 2: {
		...
		// Note: This task is destroyed by Task_CopyReceiveBuffer when
		// it's done.
	}
	break;
...
static void Task_SendPacket( u8 taskId )
{
	...
	{
		void* recordData = LoadPtrFromTaskData( &task->data[2] ) +
			task->data[4] * BUFFER_CHUNK_SIZE;
...
static void Task_CopyReceiveBuffer( u8 taskId )
{
	...
	if( status == sub_800A9D8( ) )
	{
		...
		for( i = 0; i < GetLinkPlayerCount( ); i++ )
		{
			...
			if( ( status >> i ) & 1 )
			{
				dest = LoadPtrFromTaskData(
					       (u16*)&task->data[5] ) +
					task->data[i + 1] * BUFFER_CHUNK_SIZE +
					sRecordStructSize * i;
				...
				task->data[i + 1]++;
				if( task->data[i + 1] ==
					sRecordStructSize / BUFFER_CHUNK_SIZE +
						1 )
					handledPlayers++;
			}
		}
...
static void ReceiveOldManData( OldMan* oldMan, size_t recordSize, u8 which )
{
	...
	memcpy( sOldManSave,
		(void*)oldMan + recordSize * mixIndices[which],
		sizeof( OldMan ) );
	ResetMauvilleOldManFlag( );
}

static void ReceiveBattleTowerData(
	void* battleTowerRecord, size_t recordSize, u8 which )
{
	...
	if( Link_AnyPartnersPlayingRubyOrSapphire( ) )
	{
		if( RubyBattleTowerRecordToEmerald( (void*)battleTowerRecord +
				    recordSize * mixIndices[which],
			    (void*)battleTowerRecord + recordSize * which ) ==
			TRUE )
		{
			dest = (void*)battleTowerRecord + recordSize * which;
			dest->language =
				gLinkPlayers[mixIndices[which]].language;
...
static void ReceiveLilycoveLadyData(
	LilycoveLady* lilycoveLady, size_t recordSize, u8 which )
{
	LilycoveLady* dest;
	u32 mixIndices[4];

	ShufflePlayerIndices( mixIndices );
	memcpy( (void*)lilycoveLady + recordSize * which,
		sLilycoveLadySave,
		sizeof( LilycoveLady ) );
...
static void sub_80E7AA4( struct RecordMixingDayCareMail* src,
	size_t recordSize,
	u8 ( *idxs )[2],
	u8 which0,
	u8 which1 )
{
	...
	memcpy( &buffer,
		&mail1->mail[idxs[which0][1]],
		sizeof( struct DayCareMail ) );
	mail2 = (void*)src + recordSize * idxs[which1][0];
	memcpy( &mail1->mail[idxs[which0][1]],
		&mail2->mail[idxs[which1][1]],
		sizeof( struct DayCareMail ) );
	memcpy( &mail2->mail[idxs[which1][1]],
		&buffer,
		sizeof( struct DayCareMail ) );
}
...
static u8 sub_80E7B54( void ) { return gUnknown_03001160; }

static void ReceiveDaycareMailData( struct RecordMixingDayCareMail* src,
	size_t recordSize,
	u8 which,
	TVShow* shows )
{
	...
	for( i = 0; i < GetLinkPlayerCount( ); i++ )
	{
		...
		for( j = 0; j < _src->numDaycareMons; j++ )
		{
			...
			if( anyRS )
			{
				...
				{
					StripExtCtrlCodes(
						recordMixingMail->OT_name );
					otNameLanguage = language;
				}

				if( recordMixingMail->monName[0] ==
						EXT_CTRL_CODE_BEGIN &&
					recordMixingMail->monName[1] ==
						EXT_CTRL_CODE_JPN )
				{
					StripExtCtrlCodes(
						recordMixingMail->monName );
				...
				{
					recordMixingMail->gameLanguage =
						otNameLanguage;
					recordMixingMail->monLanguage =
						nicknameLanguage;
				}
			}
	...
	memcpy( &gSaveBlock1Ptr->daycare.mons[0].mail,
		&_src->mail[0],
		sizeof( struct DayCareMail ) );
	memcpy( &gSaveBlock1Ptr->daycare.mons[1].mail,
		&_src->mail[1],
		sizeof( struct DayCareMail ) );
	SeedRng( oldSeed );
}
...
static void ReceiveGiftItem( u16* item, u8 which )
{
	if( which != 0 && *item != ITEM_NONE &&
		GetPocketByItemId( *item ) == POCKET_KEY_ITEMS )
	{
		if( !CheckBagHasItem( *item, 1 ) &&
			!CheckPCHasItem( *item, 1 ) && AddBagItem( *item, 1 ) )
		{
...
static void sub_80E8110( struct Apprentice* dst, struct Apprentice* src )
{
	...
	for( i = 0; i < 2; i++ )
	{
		id = ( ( i + gSaveBlock2Ptr->playerApprentice.field_B2_1 ) %
			     3 ) +
			1;
	...
	case 2:
		if( Random2( ) > 0x3333 )
		{
			dst[1] = src[gSaveBlock2Ptr->playerApprentice
					     .field_B2_1 +
				1];
		}
		else
...
void GetPlayerHallRecords( struct PlayerHallRecords* dst )
{
	...
	for( i = 0; i < HALL_FACILITIES_COUNT; i++ )
	{
		for( j = 0; j < 2; j++ )
		{
			CopyTrainerId( dst->onePlayer[i][j].id,
				gSaveBlock2Ptr->playerTrainerId );
			dst->onePlayer[i][j].language = GAME_LANGUAGE;
			StringCopy( dst->onePlayer[i][j].name,
	...
	for( j = 0; j < 2; j++ )
	{
		...
		CopyTrainerId( dst->twoPlayers[j].id1,
			gSaveBlock2Ptr->playerTrainerId );
		CopyTrainerId( dst->twoPlayers[j].id2,
			gSaveBlock2Ptr->frontier.field_EF1[j] );
		StringCopy(
			dst->twoPlayers[j].name1, gSaveBlock2Ptr->playerName );
		StringCopy( dst->twoPlayers[j].name2,
			gSaveBlock2Ptr->frontier.opponentName[j] );
	}
	...
	for( i = 0; i < 2; i++ )
	{
		dst->onePlayer[0][i].winStreak =
			gSaveBlock2Ptr->frontier
				.towerRecordWinStreaks[FRONTIER_MODE_SINGLES]
						      [i];
		dst->onePlayer[1][i].winStreak =
			gSaveBlock2Ptr->frontier
				.towerRecordWinStreaks[FRONTIER_MODE_DOUBLES]
						      [i];
		dst->onePlayer[2][i].winStreak =
			gSaveBlock2Ptr->frontier
				.towerRecordWinStreaks[FRONTIER_MODE_MULTIS]
						      [i];
		dst->onePlayer[3][i].winStreak =
			gSaveBlock2Ptr->frontier
				.domeRecordWinStreaks[FRONTIER_MODE_SINGLES]
						     [i];
		dst->onePlayer[4][i].winStreak =
			gSaveBlock2Ptr->frontier
				.palaceRecordWinStreaks[FRONTIER_MODE_SINGLES]
						       [i];
		dst->onePlayer[5][i].winStreak =
			gSaveBlock2Ptr->frontier.arenaRecordStreaks[i];
		dst->onePlayer[6][i].winStreak =
			gSaveBlock2Ptr->frontier
				.factoryRecordWinStreaks[FRONTIER_MODE_SINGLES]
							[i];
		dst->onePlayer[7][i].winStreak =
			gSaveBlock2Ptr->frontier.pikeRecordStreaks[i];
		dst->onePlayer[8][i].winStreak =
			gSaveBlock2Ptr->frontier.pyramidRecordStreaks[i];

		dst->twoPlayers[i].winStreak =
			gSaveBlock2Ptr->frontier.towerRecordWinStreaks
				[FRONTIER_MODE_LINK_MULTIS][i];
	}
}
...
static bool32 sub_80E841C( struct Apprentice* arg0, struct Apprentice* arg1 )
{
	...
	for( i = 0; i < 4; i++ )
	{
		if( GetTrainerId( arg0->playerId ) ==
				GetTrainerId( arg1[i].playerId ) &&
	...
	struct Apprentice* arg0, size_t arg1, u32 arg2 )
{
	...
	for( i = 0; i < 2; i++ )
	{
		if( structPtr[i].playerName[0] != EOS &&
			!sub_80E841C( &structPtr[i],
	...
	case 1:
		...
		gSaveBlock2Ptr->playerApprentice.field_B2_1 =
			( gSaveBlock2Ptr->playerApprentice.field_B2_1 + 1 ) %
			3;
		break;
	case 2:
		for( i = 0; i < 2; i++ )
		{
			structId = ( ( i ^ 1 ) +
					   gSaveBlock2Ptr->playerApprentice
						   .field_B2_1 ) %
					3 +
				1;
		...
		gSaveBlock2Ptr->playerApprentice.field_B2_1 =
			( gSaveBlock2Ptr->playerApprentice.field_B2_1 + 2 ) %
			3;
		break;
	}
}
	...
	s32 linkPlayerCount )
{
	...
	{
		for( k = 0; k < 3; k++ )
			dst->hallRecords2P[j][k] =
				gSaveBlock2Ptr->hallRecords2P[j][k];
		...
		for( k = 0; k < linkPlayerCount - 1; k++ )
		{
			...
			{
				if( GetTrainerId(
					    dst->hallRecords2P[j][l].id1 ) ==
						GetTrainerId(
							gUnknown_03001168[k]
								->twoPlayers[j]
								.id1 ) &&
					GetTrainerId( dst->hallRecords2P[j][l]
							      .id2 ) ==
						GetTrainerId(
							gUnknown_03001168[k]
								->twoPlayers[j]
				...
				{
					var_68++;
					if( dst->hallRecords2P[j][l]
							.winStreak <
						gUnknown_03001168[k]
							->twoPlayers[j]
							.winStreak )
						dst->hallRecords2P[j][l] =
							gUnknown_03001168[k]
								->twoPlayers
									[j];
				}
			...
			if( var_68 == 0 )
				dst->hallRecords2P[j][k + 3] =
					gUnknown_03001168[k]->twoPlayers[j];
		}
	}
}

static void sub_80E8880(
	struct RankingHall1P* arg0, struct RankingHall1P* arg1 )
{
	s32 i, j;
...
static void sub_80E88CC(
	struct RankingHall2P* arg0, struct RankingHall2P* arg1 )
{
	s32 i, j;
...
static void sub_80E8924( struct RecordMixingHallRecords* arg0 )
{
	...
	{
		for( j = 0; j < 2; j++ )
			sub_80E8880( gSaveBlock2Ptr->hallRecords1P[i][j],
				arg0->hallRecords1P[i][j] );
	}
	for( j = 0; j < 2; j++ )
		sub_80E88CC( gSaveBlock2Ptr->hallRecords2P[j],
			arg0->hallRecords2P[j] );
}
...
static void sub_80E89F8( struct RecordMixingDayCareMail* dst )
{
	...
	InitDaycareMailRecordMixing(
		&gSaveBlock1Ptr->daycare, &gUnknown_02039F9C );
	...
	for( i = 0; i < src->numDaycareMons; i++ )
	{
		...
		if( mail->message.itemId != 0 )
		{
			if( mail->gameLanguage != LANGUAGE_JAPANESE )
				PadNameString(
					mail->OT_name, EXT_CTRL_CODE_BEGIN );

			ConvertInternationalString(
				mail->monName, mail->monLanguage );
		}
	}
}
