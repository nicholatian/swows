...
static u32 GetMonSizeHash( struct Pokemon* pkmn )
{
	...
	u32 hibyte =
		( ( attackIV ^ defenseIV ) * hpIV ) ^ ( personality & 0xFF );
	u32 lobyte =
		( ( spAtkIV ^ spDefIV ) * speedIV ) ^ ( personality >> 8 );
...
static u32 GetMonSize( u16 species, u16 b )
{
	...
	height = GetPokedexHeightWeight(
		SpeciesToNationalPokedexNum( species ), 0 );
