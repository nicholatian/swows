...
static void Task_DoPokeballSendOutAnim( u8 taskId )
{
	...
	LoadBallGfx( ballId );
	ballSpriteId =
		CreateSprite( &gBallSpriteTemplates[ballId], 32, 80, 29 );
	...
	case POKEBALL_OPPONENT_SENDOUT:
		gSprites[ballSpriteId].pos1.x =
			GetBattlerSpriteCoord( battlerId, BATTLER_COORD_X );
		gSprites[ballSpriteId].pos1.y =
			GetBattlerSpriteCoord( battlerId, BATTLER_COORD_Y ) +
			24;
		gBattlerTarget                  = battlerId;
...
static void SpriteCB_TestBallThrow( struct Sprite* sprite )
{
	if( TranslateAnimHorizontalArc( sprite ) )
	{
		...
		sprite->pos2.y  = 0;
		sprite->data[5] = 0;
		ballId          = ItemIdToBallId(
                        GetBattlerPokeballItemId( opponentBattler ) );
		AnimateBallOpenParticles(
			sprite->pos1.x, sprite->pos1.y - 5, 1, 0x1C, ballId );
		sprite->data[0] = LaunchBallFadeMonTask(
			FALSE, opponentBattler, 14, ballId );
...
static void sub_80756E0( struct Sprite* sprite )
{
	if( ++sprite->data[5] == 10 )
	{
		...
		StartSpriteAffineAnim(
			&gSprites[gBattlerSpriteIds[sprite->sBattler]], 2 );
		AnimateSprite(
			&gSprites[gBattlerSpriteIds[sprite->sBattler]] );
...
static void Task_PlayCryWhenReleasedFromBall( u8 taskId )
{
	...
	struct Pokemon* mon =
		(void*)( u32 )( ( gTasks[taskId].tCryTaskMonPtr1 << 0x10 ) |
	...
	case 1:
		...
		gBattleSpritesDataPtr->healthBoxesData[battlerId].field_1_x40 =
			0;
		DestroyTask( taskId );
		break;
	case 2:
...
static void SpriteCB_ReleaseMonFromBall( struct Sprite* sprite )
{
	...
	AnimateBallOpenParticles(
		sprite->pos1.x, sprite->pos1.y - 5, 1, 0x1C, ballId );
	sprite->data[0] =
		LaunchBallFadeMonTask( 1, sprite->sBattler, 14, ballId );
	...
	if( gMain.inBattle )
	{
		...
		if( ( battlerId ==
			...
			IsDoubleBattle( ) &&
			gBattleSpritesDataPtr->animationData->field_9_x1 )
		{
		...
		gTasks[taskId].tCryTaskMonSpriteId =
			gBattlerSpriteIds[sprite->sBattler];
	...
	StartSpriteAffineAnim(
		&gSprites[gBattlerSpriteIds[sprite->sBattler]], 1 );
...
static void HandleBallAnimEnd( struct Sprite* sprite )
{
	...
	{
		StartSpriteAffineAnim(
			&gSprites[gBattlerSpriteIds[battlerId]], 0 );
	...
	if( sprite->animEnded && affineAnimEnded )
	{
		...
		{
			if( gBattleSpritesDataPtr->healthBoxesData[i]
					.ballAnimActive == 0 )
				doneBattlers++;
		}
...
static void sub_80760F8( struct Sprite* sprite )
{
	...
	{
		FreeOamMatrix( gSprites[gBattlerSpriteIds[sprite->sBattler]]
				       .oam.matrixNum );
		DestroySprite(
			&gSprites[gBattlerSpriteIds[sprite->sBattler]] );
...
static void SpriteCB_PlayerMonSendOut_2( struct Sprite* sprite )
{
	...
	if( HIBYTE( sprite->data[7] ) >= 35 && HIBYTE( sprite->data[7] ) < 80 )
	{
		...
		if( ( sprite->oam.affineParam & 0xFF00 ) == 0 )
		{
			...
			r7 = sprite->data[2] & 1;
			sprite->data[1] =
				( ( sprite->data[1] / 3 ) & ~1 ) | r6;
			sprite->data[2] =
				( ( sprite->data[2] / 3 ) & ~1 ) | r7;
		...
		sprite->pos2.y +=
			Sin( HIBYTE( sprite->data[7] ), sprite->data[5] );
...
static u8 AnimateBallOpenParticlesForPokeball(
	u8 x, u8 y, u8 kindOfStars, u8 d )
{
	return AnimateBallOpenParticles( x, y, kindOfStars, d, BALL_POKE );
}

static u8 LaunchBallFadeMonTaskForPokeball(
	bool8 unFadeLater, u8 battlerId, u32 arg2 )
{
	return LaunchBallFadeMonTask(
		unFadeLater, battlerId, arg2, BALL_POKE );
}
...
static void sub_8076524( struct Sprite* sprite )
{
	if( sprite->data[1] == 0 )
	{
		...
		StartSpriteAnim( sprite, 1 );
		AnimateBallOpenParticlesForPokeball( sprite->pos1.x,
			sprite->pos1.y - 5,
			sprite->oam.priority,
			r5 );
		sprite->data[1] =
			LaunchBallFadeMonTaskForPokeball( 1, battlerId, r4 );
...
static void sub_80765E0( struct Sprite* sprite )
{
	...
	var1 = ( sprite->data[5] - sprite->pos1.x ) * sprite->data[7] / 128 +
		sprite->pos1.x;
	var2 = ( sprite->data[6] - sprite->pos1.y ) * sprite->data[7] / 128 +
		sprite->pos1.y;
...
u8 sub_807671C(
	u8 a, u8 b, u8 x, u8 y, u8 oamPriority, u8 subPriority, u8 g, u32 h )
{
	u8 spriteId;
...
static void sub_80767D4( struct Sprite* sprite )
{
	if( sprite->data[1] == 0 )
	{
		...
		AnimateBallOpenParticlesForPokeball( sprite->pos1.x,
			...
			r6 );
		sprite->data[1] =
			LaunchBallFadeMonTaskForPokeball( 1, r8, r5 );
...
void sub_8076918( u8 battlerId )
{
	struct Sprite* healthboxSprite =
		&gSprites[gHealthboxSpriteIds[battlerId]];
...
void DoHitAnimHealthboxEffect( u8 battlerId )
{
	...
	spriteId = CreateInvisibleSpriteWithCallback(
...
void LoadBallGfx( u8 ballId )
{
	...
	if( GetSpriteTileStartByTag( gBallSpriteSheets[ballId].tag ) ==
		0xFFFF )
	{
		LoadCompressedSpriteSheetUsingHeap(
			&gBallSpriteSheets[ballId] );
		LoadCompressedSpritePaletteUsingHeap(
			&gBallSpritePalettes[ballId] );
	}
	switch( ballId )
	{
	case BALL_DIVE:
	...
	default:
		...
		LZDecompressVram( gOpenPokeballGfx,
			(void*)( VRAM + 0x10100 + var * 32 ) );
		break;
	}
}
